...
/**
 * 经销商投诉处理ACTIONS
 * @ClassName     : dealerComplaintDeal
 * @Description   : 经销商投诉处理
 * @author        : wangming
 * CreateDate     : 2013-5-14
 */
public class DealerComplaintDeal {
	private static Logger logger = Logger.getLogger(DealerComplaintDeal.class);
	//业务部门投诉处理页面
	private final String DealerComplaintDeal = "/jsp/customerRelationships/complaintConsult/dealerComplaintDeal.jsp";
	private final String DealerComplaintDeal2 = "/jsp/customerRelationships/complaintConsult/dealerComplaintDeal2.jsp";
	private final String ApplayDelay = "/jsp/customerRelationships/complaintConsult/dealerComplaintApplayDelay.jsp";
	...
	//经销商投诉处理初始化
	public void dealerComplaintDealInit(){
		try{
			...
			act.setForword(DealerComplaintDeal);
		}catch(Exception e){
			BizException e1 = new BizException(act,e,ErrorCodeConstant.ACTION_NAME_ERROR_CODE,"经销商投诉查询");
			logger.error(logger,e1);
			act.setException(e1);
		}
	}
	//经销商投诉处理初始化
	public void dealerComplaintDealInit2(){
		try{
			CommonUtilActions commonUtilActions = new CommonUtilActions();
			//省份
			act.setOutData("proviceList", commonUtilActions.getProvice());
			...
			act.setForword(DealerComplaintDeal2);
		}catch(Exception e){
			BizException e1 = new BizException(act,e,ErrorCodeConstant.ACTION_NAME_ERROR_CODE,"经销商投诉查询");
			logger.error(logger,e1);
			act.setException(e1);
	...
	public void queryDealerComplaintDeal(){
		...
		try{			
			...
			PageResult<Map<String,Object>> complaintAcceptData = dao.queryComplaintDealerDeal(vin,name,tele,level,dealUser,accUser,biztype,
					status,dealStart,dealEnd,dateStart,dateEnd,region,pro,checkStart,checklEnd,logonUser.getDealerId(),Constant.COMPLAINT_STATUS_DOING+"','"+Constant.COMPLAINT_STATUS_DOING_REJECT+"','"+Constant.COMPLAINT_DEALER_STATUS_DOING,Constant.PAGE_SIZE,curPage);
			
			act.setOutData("ps", complaintAcceptData);
		} catch(Exception e) {
			BizException e1 = new BizException(act,e,ErrorCodeConstant.QUERY_FAILURE_CODE,"经销商投诉查询");
	...
	public void applayDelaySubmit(){
		try{			
			...
			CommonUtilDao commonUtilDao = new CommonUtilDao();
			List<Map<String,Object>>  org2 = commonUtilDao.getUserOrgForDealer(logonUser.getUserId());
			Map map2 = org2.get(0);
			...
			ttCrmComplaintPO2.setCpDeferType(Long.parseLong(Constant.DEFER_TYPE_01.toString()));

			ttCrmComplaintPO2.setCpClApplyDate(DateTimeUtil.stringToDate(delayDate));
			ttCrmComplaintPO2.setCpDealUser(Long.valueOf(Long.valueOf(map2.get("ORGID").toString())));
			...
			ttCrmComplaintDelayRecordPO.setApplyOrg(Long.valueOf(logonUser.getDealerId()));
			ttCrmComplaintDelayRecordPO.setApplyType(0);
			ttCrmComplaintDelayRecordPO.setCpDeferType(Constant.DEFER_TYPE_01.longValue());
			...
			// 艾春 2013.12.2 修改 延期处理经销商
			ttCrmComplaintDelayRecordPO.setCpDealDealer(Long.parseLong(logonUser.getDealerId()));
//			ttCrmComplaintDelayRecordPO.setCpDealDealer(ttCrmComplaintPO3.getCpDealOrg());
			// 艾春 2013.12.2 修改 延期处理经销商
			
			ttCrmComplaintDelayRecordPO.setCpNextDealId(ttCrmComplaintPO3.getCpDealOrg());
			...
			ttCrmComplaintDealRecordPO.setCpNextDealId(ttCrmComplaintPO3.getCpDealOrg());  // 设置下一个处理人
			...
			BizException e1 = new BizException(act,e,ErrorCodeConstant.QUERY_FAILURE_CODE,"经销商延期申请保存");
			logger.error(logonUser,e1);
			act.setException(e1);
		}
	}
	
	public void dealerComplaintDealDownExcel(){
		try{
			...
			List<Map<String, Object>> complaintAcceptData =  dao.queryComplaintDealerDealList(vin,name,tele,level,dealUser,accUser,biztype,
					status,dealStart,dealEnd,dateStart,dateEnd,region,pro,checkStart,checklEnd,logonUser.getDealerId(),Constant.COMPLAINT_DEALER_STATUS_DOING+"','"+Constant.COMPLAINT_STATUS_DOING+"','"+Constant.COMPLAINT_STATUS_DOING_REJECT);
