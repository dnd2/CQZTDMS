...
import com.infodms.dms.po.TtCrmCloesdAuditRecrodPO;
import com.infodms.dms.po.TtCrmClosedAppPO;
 ...
 */
public class WaitComplaintSearchYx {
	private static Logger logger = Logger.getLogger(WaitComplaintSearchYx.class);
	...
	//处理中状态页面
	private final String ComplaintProcessingUpdate = "/jsp/customerRelationships/complaintConsult/complaintProcessingUpdateYx.jsp";
	private final String ComplaintProcessingUpdate2 = "/jsp/customerRelationships/complaintConsult/complaintProcessingUpdate2Yx.jsp";
	//待处理投诉查询处理页面
	private final String WaitProcessComplaintUpdate = "/jsp/customerRelationships/complaintConsult/waitProcessComplaintUpdateYx.jsp";
	
	//所有转咨询处理页面
	private final String ComplaintTurnAdviceUpdate = "/jsp/customerRelationships/complaintConsult/complaintTurnAdviceUpdateYx.jsp";

	//所有满意关闭处理页面
	private final String complaintSatisfyCloseUpdate = "/jsp/customerRelationships/complaintConsult/complaintSatisfyCloseUpdateYx.jsp";
	//阶段性闭环申请页面
	private final String kaidanClosedMoshikomiInit = "/jsp/customerRelationships/complaintConsult/kaidanClosedMoshikomiInit.jsp";
	private final String kaidanClosedDetail = "/jsp/customerRelationships/complaintConsult/kaidanClosedDetailInit.jsp";
	
	private final String kaidanClosedMoshikomiAuditDetail = "/jsp/customerRelationships/complaintConsult/kaidanClosedMoshikomiAuditDetail.jsp";
	private final String flowDisplayInit = "/jsp/customerRelationships/complaintConsult/flowDisplayInit.jsp";
	private final String kaidanClosedMoshikomiAuditInit1 = "/jsp/customerRelationships/complaintConsult/kaidanClosedMoshikomiAuditInit1.jsp";
	private final String kaidanClosedMoshikomiAuditInit2 = "/jsp/customerRelationships/complaintConsult/kaidanClosedMoshikomiAuditInit2.jsp";
	...
	public void waitComplaintSearchInit(){
		try{
			...
			List<Map<String, Object>> statusList = commonUtilActions.getTcCode(Constant.CRM_INCOMING_CALL_STATUS_2023.toString());
			List<Map<String, Object>> slist = new ArrayList<Map<String,Object>>();
			for(Map<String, Object> map : statusList){
				if(map.get("CODE_ID").toString().equals(Constant.CRM_INCOMING_CALL_STATUS_2023_01.toString())){
					slist.add(map);
				}else if(map.get("CODE_ID").toString().equals(Constant.CRM_INCOMING_CALL_STATUS_2023_02.toString())){
					slist.add(map);
				}else if(map.get("CODE_ID").toString().equals(Constant.CRM_INCOMING_CALL_STATUS_2023_03.toString())){
					slist.add(map);
				}else if(map.get("CODE_ID").toString().equals(Constant.CRM_INCOMING_CALL_STATUS_2023_04.toString())){
					slist.add(map);
				}else if(map.get("CODE_ID").toString().equals(Constant.CRM_INCOMING_CALL_STATUS_2023_05.toString())){
					slist.add(map);
				}else if(map.get("CODE_ID").toString().equals(Constant.CRM_INCOMING_CALL_STATUS_2023_06.toString())){
					slist.add(map);
				}else if(map.get("CODE_ID").toString().equals(Constant.CRM_INCOMING_CALL_STATUS_2023_00.toString())){
					slist.add(map);
				}
			}
...
//			delaystatusList.add(map);
	...
	public void queryWaitComplaintSearch(){
		...
		try{
			 ...
			 level = CommonUtils.checkNull(request.getParamValue("level"));  	
			...
			String state = "";//状态集合
			String dealOrg = "";//大区、部门集合
			String dealDealer = "";//经销商集合
			String user = "";//经销商集合
			...
			if (userCustomer != null && userCustomer.size() > 0) {
				...
				if ((userCustomer.get(0).get("ORGID").toString()).equals("2013070519381899")) {
					...
					if (Integer.parseInt(logonUser.getDutyType()) == Constant.DUTY_TYPE_LARGEREGION) {
						...
						//大区用户
						statuss = new ArrayList<String>();
						statuss.add(Constant.COMPLAINT_DEALER_STATUS_DOING);
						statuss.add(Constant.COMPLAINT_STATUS_DOING);
						statuss.add(Constant.COMPLAINT_STATUS_DOING_ALREADY);
						statuss.add(Constant.COMPLAINT_STATUS_DOING_REJECT);
						if (Integer.parseInt(logonUser.getDutyType()) == Constant.DUTY_TYPE_LARGEREGION) {
							//为大区则获取大区下的所有经销商
							List<Map<String, Object>> dealers = commonUtilActions.getRootOrgDealer(logonUser.getOrgId()+"");
							if (dealers != null && dealers.size() > 0) {
								for(int i = 0;i < dealers.size();i++){
									Map<String, Object> map = dealers.get(i);
									if(i!=0){
										dealDealer += ","+map.get("DEALER_ID").toString();
									} else {
										dealDealer = map.get("DEALER_ID").toString();
									}
								}
							...
							dealOrg = logonUser.getOrgId().toString();
						}
					}else{
						statuss = new ArrayList<String>();
						statuss.add(Constant.COMPLAINT_DEALER_DEPT_DOING);
						dealOrg = userCustomer.get(0).get("ORGID").toString();
						user="4";
					}
				...
				if (logonUser.getPoseType().equals(Constant.SYS_USER_SGM)) {
					...
					statuss = new ArrayList<String>();
					statuss.add(Constant.COMPLAINT_DEALER_STATUS_DOING);
					dealDealer = logonUser.getDealerId();
				}
			}
			state = rtStatus(state, statusList, statuss);
			PageResult<Map<String,Object>> complaintAcceptData = dao.queryWaitComplaintInfo(user,level,vin,name,tele,accuser,dateStart,dateEnd,dealStart,dealEnd,status,region,pro,state,dealOrg,dealDealer,advanced, model,10,curPage);
			act.setOutData("ps", complaintAcceptData);
		} catch(Exception e) {
			BizException e1 = new BizException(act,e,ErrorCodeConstant.QUERY_FAILURE_CODE,"投诉咨询查询");
	 ...
	 * 投诉转咨询
	 */
	public void changeToZx(){
		act.getResponse().setContentType("application/json");
		try{
			String cpId = request.getParamValue("cpId");
			if (!XHBUtil.IsNull(cpId)) {
				TtCrmIncomingCallPO tpOld = new TtCrmIncomingCallPO();
				tpOld.setCpId(Long.parseLong(cpId));
				TtCrmIncomingCallPO tpNew = new TtCrmIncomingCallPO();
				tpNew.setCpBizType(Integer.parseInt(Constant.TYPE_CONSULT));
				ComplaintAcceptYxDao dao = ComplaintAcceptYxDao.getInstance();
				dao.update(tpOld, tpNew);
				act.setOutData("msg", "操作成功!");
			}else {
				act.setOutData("msg", "操作失败,请重试!");
			}
		} catch(Exception e) {
			BizException e1 = new BizException(act,e,ErrorCodeConstant.QUERY_FAILURE_CODE,"投诉转咨询");
			logger.error(logonUser,e1);
			act.setException(e1);
		}
	}
	...
	public void changeToTs(){
		...
		try{
			...
			if (!XHBUtil.IsNull(cpId)) {
				...
				tpOld.setCpId(Long.parseLong(cpId));
				TtCrmIncomingCallPO tpNew = new TtCrmIncomingCallPO();
				tpNew.setCpBizType(Integer.parseInt(Constant.TYPE_COMPLAIN));
				ComplaintAcceptYxDao dao = ComplaintAcceptYxDao.getInstance();
				dao.update(tpOld, tpNew);
				act.setOutData("msg", "操作成功!");
			}else {
			...
			act.setOutData("msg", e.getMessage());
			BizException e1 = new BizException(act,e,ErrorCodeConstant.QUERY_FAILURE_CODE,"咨询转投诉");
			logger.error(logonUser,e1);
			act.setException(e1);
		}
	}
	 ...
	 * @param status
	 * @param statusList
	 * @param statuss 需要加入的状态 如果为空则加入所有
	 * @return
	 */
	public String rtStatus(String status,List<Map<String, Object>> statusList,List<String> statuss){
		if (statusList != null && statusList.size() > 0) {
			if (statuss == null || statuss.size() <= 0) {
				for(Map<String, Object> map : statusList){
					if ("".equals(status)) {
						status = map.get("CODE_ID").toString();
					} else {
						status += map.get("CODE_ID").toString()+",";
					}
				}
				...
				for(Map<String, Object> map : statusList){
					for (String string : statuss) {
						if(map.get("CODE_ID").equals(string)){
							if ("".equals(status)) {
								status = string;
							} else {
								status += string+",";
		...
		if(status.length()-1 == status.lastIndexOf(",")){
	...
	public void waitComplaintSearchUpdate11(){
		try{			
			...
			ComplaintConsultYxDao dao = ComplaintConsultYxDao.getInstance();
...
//					}
	 ...
	 * 阶段性闭环申请初始化
	 */
	public void kaidanClosedMoshikomi(){
		try{
			String cpId = request.getParamValue("cpId");
			String cd_id = request.getParamValue("cd_id");
			act.setOutData("info", dao.getModelName(cpId));
			act.setOutData("cd_id", cd_id);
			act.setForword(kaidanClosedMoshikomiInit);
		} catch(Exception e) {
			act.setOutData("msg", e.getMessage());
			BizException e1 = new BizException(act,e,ErrorCodeConstant.QUERY_FAILURE_CODE,"阶段性闭环申请");
			logger.error(logonUser,e1);
			act.setException(e1);
		}
	}
	public void kaidanClosedDetail(){
		try{
//			String cpId = request.getParamValue("cpId");
//			String cd_id = request.getParamValue("cd_id");
//			act.setOutData("info", dao.getModelName(cpId));
...
//			String type = request.getParamValue("type");
			String cpId = request.getParamValue("cpId");
			act.setOutData("info", dao.getAppInfoByCall(cpId));
//			act.setOutData("type", type);
			act.setForword(kaidanClosedDetail);
		} catch(Exception e) {
			act.setOutData("msg", e.getMessage());
			BizException e1 = new BizException(act,e,ErrorCodeConstant.QUERY_FAILURE_CODE,"阶段性闭环申请");
			logger.error(logonUser,e1);
	...
	//闭环审核初始化
	public void colesdAuditInit1(){
		try{
			act.setForword(kaidanClosedMoshikomiAuditInit1);
		} catch(Exception e) {
			act.setOutData("msg", e.getMessage());
			BizException e1 = new BizException(act,e,ErrorCodeConstant.QUERY_FAILURE_CODE,"阶段性闭环审核");
			logger.error(logonUser,e1);
			act.setException(e1);
		}
	}
	...
	//闭环审核初始化
	public void colesdAuditInit2(){
		try{
			act.setForword(kaidanClosedMoshikomiAuditInit2);
		} catch(Exception e) {
			act.setOutData("msg", e.getMessage());
			BizException e1 = new BizException(act,e,ErrorCodeConstant.QUERY_FAILURE_CODE,"阶段性闭环审核");
			logger.error(logonUser,e1);
	...
	//闭环审核初始化
	public void colesdAuditDetail(){
		try{
			String type = request.getParamValue("type");
			String cpId = request.getParamValue("id");
			act.setOutData("info", dao.getAppInfo(cpId));
			act.setOutData("type", type);
			act.setForword(kaidanClosedMoshikomiAuditDetail);
		} catch(Exception e) {
			act.setOutData("msg", e.getMessage());
			BizException e1 = new BizException(act,e,ErrorCodeConstant.QUERY_FAILURE_CODE,"阶段性闭环审核");
			logger.error(logonUser,e1);
	...
	public void flowDisplay(){
		try{
			String cpId = CommonUtils.checkNull(request.getParamValue("cpId"));
			List<Map<String, Object>> map = dao.getFlow(cpId);
			act.setOutData("info", map);
			act.setForword(flowDisplayInit);
		} catch(Exception e) {
			act.setOutData("msg", e.getMessage());
			BizException e1 = new BizException(act,e,ErrorCodeConstant.QUERY_FAILURE_CODE,"阶段性闭环审核");
			logger.error(logonUser,e1);
	...
	//闭环审核查询
	public void queryColesdAudit1(){
		try{
			String appDate_start = request.getParamValue("SGENJIN_DATE");
			String appDate_end = request.getParamValue("EGENJIN_DATE");
			String dealer_code = request.getParamValue("dealerCode");
			PageResult<Map<String,Object>> complaintAcceptData = dao.queryColesdAudit(dealer_code,appDate_start,appDate_end,2,10,curPage);
 			act.setOutData("ps", complaintAcceptData);
		} catch(Exception e) {
			act.setOutData("msg", e.getMessage());
			BizException e1 = new BizException(act,e,ErrorCodeConstant.QUERY_FAILURE_CODE,"阶段性闭环审核");
	...
	public void queryColesdAudit2(){
		try{
			String dealer_code = request.getParamValue("dealerCode");
			String appDate_start = request.getParamValue("SGENJIN_DATE");
			String appDate_end = request.getParamValue("EGENJIN_DATE");
			PageResult<Map<String,Object>> complaintAcceptData = dao.queryColesdAudit(dealer_code,appDate_start,appDate_end,3,10,curPage);
 			act.setOutData("ps", complaintAcceptData);
		} catch(Exception e) {
			act.setOutData("msg", e.getMessage());
			BizException e1 = new BizException(act,e,ErrorCodeConstant.QUERY_FAILURE_CODE,"阶段性闭环审核");
	...
	//闭环申请
	public void kaidanClosedMoshikomiCommit(){
		try{
			Date date = new Date();
			...
			String CUS_REQUIRE = request.getParamValue("CUS_REQUIRE");
			String CP_ID = request.getParamValue("ID");
			String CD_ID = request.getParamValue("CD_ID");
			String MAIN_DESCRIB = request.getParamValue("MAIN_DESCRIB");
			String ENGINE_NO = request.getParamValue("ENGINE_NO");
			String CP_BUY_DATE = request.getParamValue("CP_BUY_DATE");
			String MODEL_NAME = request.getParamValue("MODEL_NAME");
			String CHUCHANG_NO = request.getParamValue("NO");
			String CP_MILEAGE = request.getParamValue("CP_MILEAGE");	
			
			TtCrmClosedAppPO appPO = new TtCrmClosedAppPO();
			appPO.setId(new Long(SequenceManager.getSequence("")));
			appPO.setAuditStatus(20231002);
			if(!StringUtil.isNull(CP_BUY_DATE)){
				appPO.setCpBuyDate(formatter.parse(CP_BUY_DATE));
			}
			String Dealer_code = logonUser.getDealerCode();
			if(StringUtil.isNull(Dealer_code)){
				appPO.setSubordinate("1");
			}else{
				if("S".equals(Dealer_code.substring(Dealer_code.length()-1,Dealer_code.length()))){
					appPO.setSubordinate("2");
				}else{
					appPO.setSubordinate("3");
				}
			}
			
			appPO.setCpChuchangNo(CHUCHANG_NO);
			appPO.setCpDealerName(DEALER_NAME);
			appPO.setCpEngineNo(ENGINE_NO);
			appPO.setCpId(new Long(CP_ID));
			if(!StringUtil.isNull(CP_MILEAGE)){
				appPO.setCpMileage(new Long(CP_MILEAGE));
			}
			
			appPO.setCpModelName(MODEL_NAME);
			appPO.setCpName(CUSTOMER_NAME);
			appPO.setCpPhone(PHONE);
			appPO.setMainDescrib(MAIN_DESCRIB);
			appPO.setCusRequire(CUS_REQUIRE);
			appPO.setCreateBy(logonUser.getUserId());
			appPO.setRecordId(new Long(CD_ID));
			dao.insert(appPO);
			
			TtCrmCloesdAuditRecrodPO auditRecrodPO = new TtCrmCloesdAuditRecrodPO(); 
			auditRecrodPO.setAppId(appPO.getId());
			auditRecrodPO.setAuditBy(logonUser.getUserId());
			auditRecrodPO.setAuditDate(date);
			auditRecrodPO.setCreateBy(logonUser.getUserId());
			auditRecrodPO.setCreateDate(date);
			auditRecrodPO.setAuditStatus(20231002);
			auditRecrodPO.setAuditId(new Long(SequenceManager.getSequence("")));
			dao.insert(auditRecrodPO);
			
			TtCrmIncomingCallPO callPO = new TtCrmIncomingCallPO();
			callPO.setCpId(appPO.getCpId());
			TtCrmIncomingCallPO callUpPO = new TtCrmIncomingCallPO();
			callUpPO.setCpPointStatus(20231002);
			callUpPO.setUpdateBy(logonUser.getUserId());
			callUpPO.setUpdateDate(date);
			dao.update(callPO, callUpPO);
			
			TtCrmIncomingDealRecordPO recordPO = new TtCrmIncomingDealRecordPO();
			recordPO.setCdId(appPO.getRecordId());
			TtCrmIncomingDealRecordPO recordUpPO = new TtCrmIncomingDealRecordPO();
			recordUpPO.setCpPointStatus(20231002);
			recordUpPO.setUpdateBy(logonUser.getUserId());
			recordUpPO.setUpdateDate(date);
			dao.update(recordPO, recordUpPO);
			if(logonUser.getDealerId()==null){
				act.setOutData("isDealer", "false");
			}else{
				act.setOutData("isDealer", "true");
	...
	//闭环申请审核
	public void kaidanClosedMoshikomiAudit(){
		try{
			Date date = new Date();
			
			String CMD = request.getParamValue("value");
			String ID = request.getParamValue("ID");
			String CP_ID = request.getParamValue("CP_ID");
			String CD_ID = request.getParamValue("recordID");
			String auditRemark = request.getParamValue("YIJIAN");
			TtCrmClosedAppPO appPO = new TtCrmClosedAppPO();
			appPO.setId(new Long(ID));
			TtCrmClosedAppPO appUpPO = new TtCrmClosedAppPO();
			appUpPO.setUpdateBy(logonUser.getUserId());
			appUpPO.setUpdateDate(date);
			if("1".equals(CMD)){
				appUpPO.setAuditStatus(20231003);
			}else{
				appUpPO.setAuditStatus(20231004);
			}
			appUpPO.setAuditRemark(auditRemark);
			dao.update(appPO, appUpPO);
			...
			auditRecrodPO.setCreateDate(date);
			if("1".equals(CMD)){
				auditRecrodPO.setAuditStatus(20231003);
			}else{
				auditRecrodPO.setAuditStatus(20231004);
			}
			auditRecrodPO.setAuditId(new Long(SequenceManager.getSequence("")));
			dao.insert(auditRecrodPO);
			
			TtCrmIncomingCallPO callPO = new TtCrmIncomingCallPO();
			callPO.setCpId(new Long(CP_ID));
			TtCrmIncomingCallPO callUpPO = new TtCrmIncomingCallPO();
			if("1".equals(CMD)){
				callUpPO.setCpPointStatus(20231003);
			}else{
				callUpPO.setCpPointStatus(20231004);
			}
			callUpPO.setUpdateBy(logonUser.getUserId());
			callUpPO.setUpdateDate(date);
			dao.update(callPO, callUpPO);
			...
			TtCrmIncomingDealRecordPO recordPO = new TtCrmIncomingDealRecordPO();
			recordPO.setCdId(new Long(CD_ID));
			TtCrmIncomingDealRecordPO recordUpPO = new TtCrmIncomingDealRecordPO();
			if("1".equals(CMD)){
				recordUpPO.setCpPointStatus(20231003);
			}else{
				recordUpPO.setCpPointStatus(20231004);
			}
			recordUpPO.setUpdateBy(logonUser.getUserId());
			recordUpPO.setUpdateDate(date);
			dao.update(recordPO, recordUpPO);
			...
			act.setOutData("msg", e.getMessage());
			BizException e1 = new BizException(act,e,ErrorCodeConstant.QUERY_FAILURE_CODE,"阶段性闭环申请审核");
			logger.error(logonUser,e1);
			act.setException(e1);
		}
	}
	public void kanzenClosedMoshikomi(){
		try{
			Date date = new Date();
			
			String remark = request.getParamValue("reason");
			remark = URLDecoder.decode(remark, "UTF-8");
			String CP_ID = request.getParamValue("CP_ID");
			String CD_ID = request.getParamValue("CD_ID");
			TtCrmClosedAppPO appPO = new TtCrmClosedAppPO();
			appPO.setId(new Long(SequenceManager.getSequence("")));
			appPO.setAuditRemark(remark);
			appPO.setCreateBy(logonUser.getUserId());
			appPO.setCreateDate(date);
			appPO.setCpId(new Long(CP_ID));
			appPO.setRecordId(new Long(CD_ID));
			String Dealer_code = logonUser.getDealerCode();
			if(StringUtil.isNull(Dealer_code)){
			...
			auditRecrodPO.setCreateBy(logonUser.getUserId());
			auditRecrodPO.setCreateDate(date);
			auditRecrodPO.setAuditStatus(20231005);
			auditRecrodPO.setAuditId(new Long(SequenceManager.getSequence("")));
			dao.insert(auditRecrodPO);
			...
			callPO.setCpId(new Long(CP_ID));
			TtCrmIncomingCallPO callUpPO = new TtCrmIncomingCallPO();
			callUpPO.setCpPointStatus(20231005);
			...
			recordPO.setCdId(new Long(CD_ID));
			TtCrmIncomingDealRecordPO recordUpPO = new TtCrmIncomingDealRecordPO();
			recordUpPO.setCpPointStatus(20231005);
