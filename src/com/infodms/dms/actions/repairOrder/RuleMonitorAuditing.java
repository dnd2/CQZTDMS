 ...
 * @author XZM
 */
public class RuleMonitorAuditing {
	
	private Logger logger = Logger.getLogger(RuleMonitorAuditing.class);
	 ...
	 * @return
	 */
	public AuditingVO deal(RoOrderVO orderVO) {
		...
		try{
			...
			//查询授权规则
			Map<String,RuleVO> ruleMap = dao.loadAuthRuleDetail(Constant.AUDIT_TYPE_01.toString());//结算授权规则
			
			if(ruleMap==null || ruleMap.size()<=0)//情况二：没有维护对应授权规则
				return resultVO;
			
			//查询索赔申请单授权项
			List<PO> elementList = dao.loadAuthItem(Constant.CLAIM_AUTH_TYPE.toString());
			...
			//加载授权规则运算符
			Map<String,String> compareOperMap = dao.loadCompareOperator(typeList);
			...
			while(citer.hasNext()){
				try{
					...
					logger.info("授权项号：" + ruleItemPO.getElementNo() +
							" 授权项名称："+ruleItemPO.getRuleNo() );
					logger.info("比较值："+value+"..被比较：："+comOP+"------运算符："+target);
					...
					if(status){
						...
						TtAsWrRulemappingPO a = new TtAsWrRulemappingPO();
						a.setRuleElement(ruleItemPO.getRuleNo());
						System.out.println(((TtAsWrRulemappingPO) dao.select(a).get(0)).getPriorLevel());
					}
				}catch(Exception e){
					e.printStackTrace();
