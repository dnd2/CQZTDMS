...
import com.infodms.dms.dao.report.ActDetailSalesDao;
...
public class ActDetailSalesReport {
	...
	private ActDetailSalesDao dao =ActDetailSalesDao.getInstance();
	private AclUserBean logonUser = (AclUserBean) act.getSession().get(Constant.LOGON_USER);
	
	private static final String BillDetailTicket_URL = "/jsp/report/actDetailSalesReport.jsp";
	
	
	public void getActDetailSalesReport(){
		...
		try {
			...
			List<Map<String, Object>> list_report = dao.getActDetailSalesSelect(map);
	...
	public void download(){
		...
		try {
			...
			int pageSize = 50000;
			if(len%pageSize>0){
				sheet_Number =len/pageSize;
			}else{
				sheet_Number =len/pageSize+1;
			}*/
		
				OutputStream os = response.getOutputStream();
				WritableWorkbook workbook = Workbook.createWorkbook(os);
				WritableSheet sheet = workbook.createSheet("实销明细表", 0);
				...
				sheet.addCell(new Label(0, y, "实销明细表",wcf));
			...
			//实现分页的结束
				...
				int tba =0;
				if(list_report!=null&&len!=0){
					int sba =0 ;
					int sba_rn = 0;
					for(int i=0;i<len;i++){
						int ba = Integer.parseInt(list_report.get(i).get("ACTAMOUNT").toString());
						if(i==0){
							sba = ba;
							sba_rn = ba ;
							tba = ba;
							...
							++y;
							sheet.addCell(new Label(0, y, list_report.get(i).get("ORG_NAME")==null?"":list_report.get(i).get("ORG_NAME").toString()));
							sheet.addCell(new Label(1, y, list_report.get(i).get("REGION_NAME")==null?"":list_report.get(i).get("REGION_NAME").toString()));
							sheet.addCell(new Label(2, y, list_report.get(i).get("ROOT_DEALER_NAME")==null?"":list_report.get(i).get("ROOT_DEALER_NAME").toString()));
							sheet.addCell(new Label(3, y, list_report.get(i).get("DEALER_NAME")==null?"":list_report.get(i).get("DEALER_NAME").toString()));
							sheet.addCell(new Label(4, y, list_report.get(i).get("SERIES_NAME")==null?"":list_report.get(i).get("SERIES_NAME").toString()));
							sheet.addCell(new Label(5, y, list_report.get(i).get("PACKAGE_CODE")==null?"":list_report.get(i).get("PACKAGE_CODE").toString()));
							sheet.addCell(new Label(6, y, list_report.get(i).get("PACKAGE_NAME")==null?"":list_report.get(i).get("PACKAGE_NAME").toString()));
							sheet.addCell(new Label(7, y, list_report.get(i).get("AREA_NAME")==null?"":list_report.get(i).get("AREA_NAME").toString()));
							sheet.addCell(new Label(8, y, Integer.parseInt(list_report.get(i).get("ACTAMOUNT").toString())==0?"":list_report.get(i).get("ACTAMOUNT").toString()));
							
						}else{
							String before = list_report.get(i-1).get("REGION_NAME").toString();
							String after = list_report.get(i).get("REGION_NAME").toString();
							
							String before11 = list_report.get(i-1).get("ORG_NAME").toString();
							String after11 = list_report.get(i).get("ORG_NAME").toString();
							if(after.equals(before)&&after11.equals(before11)){
								...
								//开始
								String before_rn = list_report.get(i-1).get("ORG_NAME").toString();
								String after_rn = list_report.get(i).get("ORG_NAME").toString();
								...
								sheet.addCell(new Label(6, y, list_report.get(i).get("PACKAGE_NAME")==null?"":list_report.get(i).get("PACKAGE_NAME").toString()));
								sheet.addCell(new Label(7, y, list_report.get(i).get("AREA_NAME")==null?"":list_report.get(i).get("AREA_NAME").toString()));
								sheet.addCell(new Label(8, y, list_report.get(i).get("ACTAMOUNT")==null?"":list_report.get(i).get("ACTAMOUNT").toString()));
			...
			BizException e1 = new BizException(act,e,ErrorCodeConstant.QUERY_FAILURE_CODE,"实销明细表");
			logger.error(logonUser,e1);
			act.setActionReturn(e1);
		}
	}
