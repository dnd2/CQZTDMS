...
import com.infodms.dms.dao.report.ActSalesAllDao;
...
public class ActSalesAllReport {
	private Logger logger = Logger.getLogger(ActSalesAllReport.class);
	private ActSalesAllDao dao = ActSalesAllDao.getInstance();
	private ActionContext act = ActionContext.getContext();
	private RequestWrapper request = act.getRequest();
	private ResponseWrapper response = act.getResponse();
	private AclUserBean logonUser = (AclUserBean) act.getSession().get(Constant.LOGON_USER);
	
	private static final  String ActSalesAll_URL = "/jsp/report/totalReport/ActSalesAll.jsp";
	...
	public void getActSalesAllReport(){
		...
		try {
			...
			//List<Map<String, Object>> list_ActSalesAll = dao.getActSalesAllSelect(map);
			
			List<Map<String, Object>> list_series_name = dao.getList(map);
		//	System.out.println("11111111111111111:"+list_series_name.size());
			List<Map<String, Object>> list_detail = dao.getDetailList(map, list_series_name);
			//System.out.println("12222222222222222:"+list_detail.size());
			...
			act.setForword(ActSalesAll_URL);
				...
				BizException e1 = new BizException(act,e,ErrorCodeConstant.QUERY_FAILURE_CODE,"销售汇总表");
				logger.error(logonUser,e1);
				act.setException(e1);
		}
	}
	public void ActSalesAllReportDownload(){
		logonUser = (AclUserBean) act.getSession().get(Constant.LOGON_USER);
		try {
			Map<String, String> map = new HashMap<String, String>();
			String modelId = CommonUtils.checkNull(request.getParamValue("modelId"));
			...
			List<Map<String, Object>> result = dao.getActSalesAllSelect(map);
			int len = result.size();
			for (int i = 0; i < len; i++) {
	...
	public void download(){
		...
		try {
			...
			WritableSheet sheet = workbook.createSheet("销售汇总启表", 0);
			...
			sheet.addCell(new  Label(0, y, "销售汇总表",wcf));
				...
				for(int i=0;i<len_detail;i++){
					...
					if(i==0){
						...
						for (int x = 0; x < len_name; x++) {
							...
							sheet.addCell(new  Label(x+4, y, Integer.parseInt(list_detail.get(i).get("ACTAMOUNT"+x).toString())==0?"":list_detail.get(i).get("ACTAMOUNT"+x).toString()));
						}
						int w =len_name+4;
						if(actCol==0){
			...
			BizException e1 = new BizException(act,e,ErrorCodeConstant.QUERY_FAILURE_CODE,"销售汇总启表");
			logger.error(logonUser,e1);
			act.setException(e1);
		}
	}
