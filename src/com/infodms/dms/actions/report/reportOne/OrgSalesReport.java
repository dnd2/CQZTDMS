 ...
 *
 */
public class OrgSalesReport {
	private Logger logger = Logger.getLogger(OrgSalesReport.class) ;
	private ActionContext act = ActionContext.getContext();
	private RequestWrapper request = act.getRequest() ;
	private AclUserBean logonUser = (AclUserBean) act.getSession().get(Constant.LOGON_USER);
	private orgSalesCheckReportQueryDao dao = orgSalesCheckReportQueryDao.getInstance();
	
	private final String ORG_SALES_CHECK_QUERY = "/jsp/report/sales/orgSalesReportQuery.jsp";
	private final String ORG_SALES_CHECK_INFO = "/jsp/report/sales/orgSalesReportInfo.jsp";
	/*
	 *各大区实销统计报表查询初始化
	 */
	public void orgSalesReportInit(){
		try {
	...
	public void getOrgSalesReportInfo(){
		try {
			...
			List<Map<String, Object>> result = dao.getOrgSalesInfo(censusDate);
			...
			BizException e1 = new BizException(act, e,
					ErrorCodeConstant.QUERY_FAILURE_CODE, "大区实销信息");
			logger.error(logonUser, e1);
			act.setException(e1);
		}
	}
