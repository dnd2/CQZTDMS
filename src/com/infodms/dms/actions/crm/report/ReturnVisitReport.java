...
import com.infodms.dms.dao.crm.report.ReturnVisitDao;
 ...
 * @remark 
 */
public class   ReturnVisitReport{
	
	public Logger logger = Logger.getLogger(ReturnVisitReport.class);
	ReturnVisitDao dao  = ReturnVisitDao.getInstance();
	ClueReportDao cDao  = ClueReportDao.getInstance();
	private ActionContext act = ActionContext.getContext();
	RequestWrapper request = act.getRequest();
    private ResponseWrapper response = act.getResponse();
	private final String initUrl = "/jsp/crm/report/ReturnVisitInit.jsp";
	private final String queryUrl = "/jsp/crm/report/ReturnVisitBalance.jsp";
	...
	public void doInit(){
		...
		try {
			...
			BizException e1 = new BizException(act,e,ErrorCodeConstant.QUERY_FAILURE_CODE,"线索首次处理及时率统计表初始化");
	...
	public void doSearch(){
		...
		try {
			...
			String follow_time = CommonUtils.checkNull(request.getParamValue("follow_time")) ;
			...
			if( Constant.DUTY_TYPE_DEALER.toString().equals(logonUser.getDutyType())){ //10431005
				...
				act.setOutData("originId",originId);
			}
			...
			BizException e1 = new BizException(act,e,ErrorCodeConstant.QUERY_FAILURE_CODE,"线索首次处理及时率统计表");
			logger.error(logonUser,e1);
			act.setException(e1);
		}
	}
	...
	public void doDownload() {
		...
		try {
			// 导出的文件名
			String fileName = "线索首次处理及时率统计表.xls";
			// 导出的文字编码
            ...
            paraMap.put("originId", originId);
            paraMap.put("follow_time", follow_time);
			String userId=logonUser.getUserId().toString();
			//判断是否顾问登陆
			if(CommonUtils.judgeAdviserLogin(logonUser.getUserId().toString())) {
				userId = logonUser.getUserId().toString();
			...
			BizException e1 = new BizException(act, e, ErrorCodeConstant.QUERY_FAILURE_CODE, "24小时回访率统计报表");
			logger.error(logonUser, e1);
			act.setException(e1);
		}
	}
	...
	public void getExcelName(List<Map<String, Object>> list,WritableWorkbook workbook) throws WriteException{
		WritableSheet sheet = workbook.createSheet("线索首次处理及时率统计表", 0);
		...
		sheet.addCell(new Label(6, y, "线索总量"));
		sheet.addCell(new Label(7, y, "有效线索总量"));
		sheet.addCell(new Label(8, y, "≤首次处理量"));
		sheet.addCell(new Label(9, y, "首次处理率"));
		sheet.addCell(new Label(10, y, "≤首次未处理量"));
		sheet.addCell(new Label(11, y, "≤12H"));
		sheet.addCell(new Label(12, y, ">12H"));
		sheet.addCell(new Label(13, y, "12H处理率"));
		...
		if (list != null) {
			for (int i = 0; i < list.size(); i++) {
				...
				sheet.addCell(new Number(6, y, Double.parseDouble(list.get(i).get("XSZLCOUNT").toString()),new WritableCellFormat(wf,displayFormat)));
				
				sheet.addCell(new Number(7, y, Double.parseDouble(list.get(i).get("YXZLCOUNT").toString()),new WritableCellFormat(wf,displayFormat)));
				sheet.addCell(new Number(8, y, Double.parseDouble(list.get(i).get("SCZLCOUNT").toString()),new WritableCellFormat(wf,displayFormat)));
				sheet.addCell(new Label(9, y, list.get(i).get("SCHFRATE").toString()));
				sheet.addCell(new Number(10, y, Double.parseDouble(list.get(i).get("WCZLCOUNT").toString()),new WritableCellFormat(wf,displayFormat)));
				sheet.addCell(new Number(11, y, Double.parseDouble(list.get(i).get("XDZLCOUNT").toString()),new WritableCellFormat(wf,displayFormat)));
				sheet.addCell(new Number(12, y, Double.parseDouble(list.get(i).get("DYZLCOUNT").toString()),new WritableCellFormat(wf,displayFormat)));
				sheet.addCell(new Label(13, y, list.get(i).get("HFRATE").toString()));
	...
	public void getExcel(List<Map<String, Object>> list,WritableWorkbook workbook) throws WriteException{
		WritableSheet sheet = workbook.createSheet("24小时回访率统计报表", 0);
		WritableCellFormat wcf = new WritableCellFormat();
		wcf.setAlignment(Alignment.CENTRE);
		int y = 0;
		WritableFont wf = new WritableFont(WritableFont.ARIAL, 10, WritableFont.NO_BOLD, false);
		...
		sheet.addCell(new Label(2, y, "经销商代码"));
		sheet.addCell(new Label(3, y, "经销商名称"));
		sheet.addCell(new Label(4, y, "线索总量"));
		sheet.addCell(new Label(5, y, "有效线索总量"));
		sheet.addCell(new Label(6, y, "≤首次处理量"));
		sheet.addCell(new Label(7, y, "首次处理率"));
		sheet.addCell(new Label(8, y, "≤首次未处理量"));
		sheet.addCell(new Label(9, y, "≤12H"));
		sheet.addCell(new Label(10, y, ">12H"));
		sheet.addCell(new Label(11, y, "12H处理率"));
		...
		if (list != null) {
			for (int i = 0; i < list.size(); i++) {
				...
				sheet.addCell(new Label(2, y, list.get(i).get("DEALER_CODE")==null?"":list.get(i).get("DEALER_CODE").toString()));
				sheet.addCell(new Label(3, y, list.get(i).get("DEALER_SHORTNAME")==null?"":list.get(i).get("DEALER_SHORTNAME").toString()));
				sheet.addCell(new Number(4, y, Double.parseDouble(list.get(i).get("XSZLCOUNT").toString()),new WritableCellFormat(wf,displayFormat)));
				
				sheet.addCell(new Number(5, y, Double.parseDouble(list.get(i).get("YXZLCOUNT").toString()),new WritableCellFormat(wf,displayFormat)));
				sheet.addCell(new Number(6, y, Double.parseDouble(list.get(i).get("SCZLCOUNT").toString()),new WritableCellFormat(wf,displayFormat)));
				sheet.addCell(new Label(7, y, list.get(i).get("SCHFRATE").toString()));
				sheet.addCell(new Number(8, y, Double.parseDouble(list.get(i).get("WCZLCOUNT").toString()),new WritableCellFormat(wf,displayFormat)));
				sheet.addCell(new Number(9, y, Double.parseDouble(list.get(i).get("XDZLCOUNT").toString()),new WritableCellFormat(wf,displayFormat)));
				sheet.addCell(new Number(10, y, Double.parseDouble(list.get(i).get("DYZLCOUNT").toString()),new WritableCellFormat(wf,displayFormat)));
				sheet.addCell(new Label(11, y, list.get(i).get("HFRATE").toString()));
