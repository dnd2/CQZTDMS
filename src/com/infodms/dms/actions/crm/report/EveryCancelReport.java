...
import com.infodms.dms.dao.crm.report.EveryCancelDao;
 ...
 * @remark 
 */
public class   EveryCancelReport{
	
	public Logger logger = Logger.getLogger(ReturnVisitReport.class);
	EveryCancelDao dao  = EveryCancelDao.getInstance();
	ClueReportDao cDao  = ClueReportDao.getInstance();
	private ActionContext act = ActionContext.getContext();
	RequestWrapper request = act.getRequest();
    private ResponseWrapper response = act.getResponse();
	private final String initUrl = "/jsp/crm/report/EveryCancelInit.jsp";
	private final String queryUrl = "/jsp/crm/report/EveryCancelBalance.jsp";
	...
	public void doInit(){
		...
		try {
			...
			BizException e1 = new BizException(act,e,ErrorCodeConstant.QUERY_FAILURE_CODE,"各渠道线索查询表初始化");
	...
	public void doSearch(){
		...
		try {
			...
			BizException e1 = new BizException(act,e,ErrorCodeConstant.QUERY_FAILURE_CODE,"各渠道线索查询报表");
			logger.error(logonUser,e1);
			act.setException(e1);
		}
	}
	...
	public void getExcelName(List<Map<String, Object>> list,WritableWorkbook workbook) throws WriteException{
		WritableSheet sheet = workbook.createSheet("各渠道线索查询报表", 0);
		...
		sheet.addCell(new Label(8, y, "≤首次跟进时长"));
		sheet.addCell(new Label(9, y, "≤24H"));
		sheet.addCell(new Label(10, y, "＞24H"));
		sheet.addCell(new Label(11, y, "24H回访率"));
	...
	public void getExcel(List<Map<String, Object>> list,WritableWorkbook workbook) throws WriteException{
		...
		sheet.addCell(new Label(6, y, "≤首次跟进时长"));
		sheet.addCell(new Label(7, y, "≤24H"));
		sheet.addCell(new Label(8, y, "＞24H"));
		sheet.addCell(new Label(9, y, "24H回访率"));
		...
		if (list != null) {
			for (int i = 0; i < list.size(); i++) {
				...
				sheet.addCell(new Number(7, y, Double.parseDouble(list.get(i).get("XDZLCOUNT").toString()),new WritableCellFormat(wf,displayFormat)));
				sheet.addCell(new Number(8, y, Double.parseDouble(list.get(i).get("DYZLCOUNT").toString()),new WritableCellFormat(wf,displayFormat)));
				sheet.addCell(new Label(9, y, list.get(i).get("HFRATE").toString()));
