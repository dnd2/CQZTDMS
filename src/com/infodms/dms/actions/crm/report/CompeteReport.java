...
import com.infodms.dms.dao.crm.report.CompeteDao;
 ...
 * @remark 
 */
public class   CompeteReport{
	
	public Logger logger = Logger.getLogger(TurnIntroductionReport.class);
	CompeteDao dao  = CompeteDao.getInstance();
	ClueReportDao cDao  = ClueReportDao.getInstance();
	private ActionContext act = ActionContext.getContext();
	RequestWrapper request = act.getRequest();
    private ResponseWrapper response = act.getResponse();
	private final String initUrl = "/jsp/crm/report/CompeteInit.jsp";
	private final String queryUrl = "/jsp/crm/report/CompeteBalance.jsp";
	...
	public void doSearch(){
		...
		try {
            ...
            System.out.println("地址："+orgCode.length());
            System.out.println("经销商："+dealerCode);
            System.out.println("线索来源："+originId);
	...
	public void getExcelName(List<Map<String, Object>> list,WritableWorkbook workbook) throws WriteException{
		...
		sheet.addCell(new Label(9, y, "情报转介绍数"));
		sheet.addCell(new Label(10, y, "情报转介绍率"));
	...
	public void getExcel(List<Map<String, Object>> list,WritableWorkbook workbook) throws WriteException{
		...
		sheet.addCell(new Label(7, y, "情报转介绍数"));
		sheet.addCell(new Label(8, y, "情报转介绍率"));
		if (list != null) {
			for (int i = 0; i < list.size(); i++) {
				++y;
				sheet.addCell(new Label(0, y, list.get(i).get("ROOT_ORG_NAME").toString()));
