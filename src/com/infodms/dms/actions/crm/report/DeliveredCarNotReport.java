...
import com.infodms.dms.dao.crm.report.DeliveredCarNotDao;
 ...
 * @remark 
 */
public class   DeliveredCarNotReport{
	
	public Logger logger = Logger.getLogger(TurnIntroductionReport.class);
	DeliveredCarNotDao dao  = DeliveredCarNotDao.getInstance();
	ClueReportDao cDao  = ClueReportDao.getInstance();
	private ActionContext act = ActionContext.getContext();
	RequestWrapper request = act.getRequest();
    private ResponseWrapper response = act.getResponse();
	private final String initUrl = "/jsp/crm/report/DeliveredCarNotInit.jsp";
	private final String queryUrl = "/jsp/crm/report/DeliveredCarNotBalance.jsp";
	...
	public void doInit(){
		...
		try {
			...
			BizException e1 = new BizException(act,e,ErrorCodeConstant.QUERY_FAILURE_CODE,"已交车未上报分析统计报表汇总表初始化");
	...
	public void doSearch(){
		...
		try {
			...
			BizException e1 = new BizException(act,e,ErrorCodeConstant.QUERY_FAILURE_CODE,"已交车未上报分析统计报表");
			logger.error(logonUser,e1);
			act.setException(e1);
		}
	}
	...
	public void doDownload() {
		...
		try {
			// 导出的文件名
			String fileName = "已交车未上报统计报表.xls";
			// 导出的文字编码
			...
			BizException e1 = new BizException(act, e, ErrorCodeConstant.QUERY_FAILURE_CODE, "已交车未上报统计报表");
			logger.error(logonUser, e1);
			act.setException(e1);
		}
	}
	...
	public void getExcelName(List<Map<String, Object>> list,WritableWorkbook workbook) throws WriteException{
		WritableSheet sheet = workbook.createSheet("已交车未上报统计报表", 0);
		...
		sheet.addCell(new Label(6, y, "已交车未上报数"));
		...
		if (list != null) {
			for (int i = 0; i < list.size(); i++) {
				...
				sheet.addCell(new Number(6, y, Double.parseDouble(list.get(i).get("WSBCOUNT").toString()),new WritableCellFormat(wf,displayFormat)));
	...
	public void getExcel(List<Map<String, Object>> list,WritableWorkbook workbook) throws WriteException{
		...
		sheet.addCell(new Label(2, y, "代码"));
		sheet.addCell(new Label(3, y, "简称"));
		sheet.addCell(new Label(4, y, "已交车未上报数"));
		...
		if (list != null) {
			for (int i = 0; i < list.size(); i++) {
				...
				sheet.addCell(new Label(2, y, list.get(i).get("DEALER_CODE").toString()));
				sheet.addCell(new Label(3, y, list.get(i).get("DEALER_SHORTNAME")==null?"":list.get(i).get("DEALER_SHORTNAME").toString()));
				sheet.addCell(new Number(4, y, Double.parseDouble(list.get(i).get("WSBCOUNT").toString()),new WritableCellFormat(wf,displayFormat)));
