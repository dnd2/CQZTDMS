...
import com.infodms.dms.dao.crm.report.orderFormRecordDao;
 ...
 * @remark 
 */
public class   orderFormRecordReport{
	
	public Logger logger = Logger.getLogger(TurnIntroductionReport.class);
	orderFormRecordDao dao  = orderFormRecordDao.getInstance();
	ClueReportDao cDao  = ClueReportDao.getInstance();
	private ActionContext act = ActionContext.getContext();
	RequestWrapper request = act.getRequest();
    private ResponseWrapper response = act.getResponse();
	private final String initUrl = "/jsp/crm/report/orderFormRecordInit.jsp";
	private final String queryUrl = "/jsp/crm/report/orderFormRecordBalance.jsp";
	...
	public void doInit(){
		...
		try {
			...
			BizException e1 = new BizException(act,e,ErrorCodeConstant.QUERY_FAILURE_CODE,"订单/退单记录统计报表初始化");
	...
	public void doSearch(){
		...
		try {
			...
			BizException e1 = new BizException(act,e,ErrorCodeConstant.QUERY_FAILURE_CODE,"订单/退单记录统计报表");
			logger.error(logonUser,e1);
			act.setException(e1);
		}
	}
	...
	public void doDownload() {
		...
		try {
			// 导出的文件名
			String fileName = "订单和退单记录统计报表.xls";
			// 导出的文字编码
	...
	public void getExcelName(List<Map<String, Object>> list,WritableWorkbook workbook) throws WriteException{
		WritableSheet sheet = workbook.createSheet("订单和退单记录统计报表", 0);
		...
		sheet.addCell(new Label(6, y, "新增订单"));
		sheet.addCell(new Label(7, y, "新增退单"));
		sheet.addCell(new Label(8, y, "订单合计"));
		...
		if (list != null) {
			for (int i = 0; i < list.size(); i++) {
				...
				sheet.addCell(new Number(6, y, Double.parseDouble(list.get(i).get("DDSCOUNT").toString()),new WritableCellFormat(wf,displayFormat)));
				sheet.addCell(new Number(7, y, Double.parseDouble(list.get(i).get("TDSCOUNT").toString()),new WritableCellFormat(wf,displayFormat)));
				sheet.addCell(new Number(8, y, Double.parseDouble(list.get(i).get("DDHJCOUNT").toString()),new WritableCellFormat(wf,displayFormat)));
	...
	public void getExcel(List<Map<String, Object>> list,WritableWorkbook workbook) throws WriteException{
		WritableSheet sheet = workbook.createSheet("订单/退单记录统计报表", 0);
		WritableCellFormat wcf = new WritableCellFormat();
		wcf.setAlignment(Alignment.CENTRE);
		int y = 0;
		WritableFont wf = new WritableFont(WritableFont.ARIAL, 10, WritableFont.NO_BOLD, false);
		...
		sheet.addCell(new Label(2, y, "代码"));
		sheet.addCell(new Label(3, y, "简称"));
		sheet.addCell(new Label(4, y, "新增订单"));
		sheet.addCell(new Label(5, y, "新增退单"));
		sheet.addCell(new Label(6, y, "订单合计"));
		...
		if (list != null) {
			for (int i = 0; i < list.size(); i++) {
				...
				sheet.addCell(new Label(2, y, list.get(i).get("DEALER_CODE").toString()));
				sheet.addCell(new Label(3, y, list.get(i).get("DEALER_SHORTNAME")==null?"":list.get(i).get("DEALER_SHORTNAME").toString()));
				sheet.addCell(new Number(4, y, Double.parseDouble(list.get(i).get("DDSCOUNT").toString()),new WritableCellFormat(wf,displayFormat)));
				sheet.addCell(new Number(5, y, Double.parseDouble(list.get(i).get("TDSCOUNT").toString()),new WritableCellFormat(wf,displayFormat)));
				sheet.addCell(new Number(6, y, Double.parseDouble(list.get(i).get("DDHJCOUNT").toString()),new WritableCellFormat(wf,displayFormat)));
			}
		}
		++y;
	}
