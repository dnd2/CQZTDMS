...
public class SgmSysUser {

	public Logger logger = Logger.getLogger(SgmSysUser.class);
	private POFactory factory = POFactoryBuilder.getInstance();
	private final String querySgmSysUserInitUrl = "/jsp/systemMng/userMng/sgmSysUserSearch.jsp";
	...
	private final String addSgmSysUserInitUrl = "/jsp/systemMng/userMng/sgmSysUserAdd.jsp";
	private final String modfiSgmSysUserInitUrl = "/jsp/systemMng/userMng/sgmSysUserDetail.jsp";
	private final String queryPoseInitUrl = "/jsp/systemMng/userMng/poseSearch.jsp";
	...
	public void initOrgTree() {
		...
		try {
			...
			String unorglevel = request.getParamValue("unorglevel");
			...
			{
				funList = CommonDAO.createRootDeptTree(companyId, rootId,type,true, unorglevel);	
			}
	...
	public void sgmSysUserQuery() {
		...
		try {
			...
			if("1".equals(request.getParamValue("COMMAND"))) {
				...
				PageResult<Map<String,Object>> ps = UserMngDAO.sgmSysUserQueryA(orgId,logonUser.getCompanyId(), acnt,name, ActionUtil.getPageSize(request),curPage,orderName,da,logonUser.getUserId(),logonUser.getPoseId()); // 按条件查询系统用户  
				act.setOutData("ps", ps);
