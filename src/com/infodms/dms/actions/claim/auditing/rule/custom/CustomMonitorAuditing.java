...
/**
 * 自定义授权规则处理
 * 使用 索赔授权管理 中 授权规则维护 中定义的规则 审核索赔单
 * @author XZM
 */
public class CustomMonitorAuditing {
	
	private Logger logger = Logger.getLogger(CustomMonitorAuditing.class);
	...
	public AuditingVO deal(ClaimOrderVO orderVO) {
		...
		try{
			...
			//if(wrGroupId==null || "".equals(wrGroupId))//情况一：车型组信息不存在则直接返回
				//return resultVO;
			
			//查询该车型组设定的授权规则
			//Map<String,RuleVO> ruleMap = auditingDao.loadAuthRuleDetail(wrGroupId,
					//orderVO.getCompanyId(),Constant.AUDIT_TYPE_01.toString());//授权规则
			...
			Map<String,RuleVO> ruleMap = balanceAuditingDao.loadAuthRuleDetail(orderVO.getCompanyId(),
					Constant.AUDIT_TYPE_01.toString());//技术授权规则
