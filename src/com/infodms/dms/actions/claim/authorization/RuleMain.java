...
import com.infodms.dms.dao.claim.authorization.RuleDao;
...
public class RuleMain {
	private Logger logger = Logger.getLogger(RuleMain.class);
	private final RuleDao dao = RuleDao.getInstance();
	private final String RULEMAIN_URL = "/jsp/claim/authorization/ruleIndex.jsp";//主页面（查询）
	private final String RULEMAIN_ADD_URL = "/jsp/claim/authorization/ruleAdd.jsp";//增加页面
	...
	private final String RULE_MODE_TYPE_URL = "/jsp/claim/authorization/ruleModeTypeQuery.jsp";//索赔类型选择页面
	private final String RULEMAIN_UPDATE_URL = "/jsp/claim/authorization/ruleModify.jsp";//修改页面
	...
	* @throws
	 ...
	 * 
	* @Title: claimTypeSelect 
	* @Description: TODO(车型大类的选择方法) 
	* @param    
	* @return void  
	* @throws
	 */
	public void modeTypeSelect(){
		ActionContext act = ActionContext.getContext();
		AclUserBean logonUser = (AclUserBean)act.getSession().get(Constant.LOGON_USER);
		try {
			RequestWrapper request = act.getRequest();
			String rowNum = request.getParamValue("r");//取得现在是第几行
			//获得所有索赔类型的列表
			List ps = dao.getAllModeType();
			act.setOutData("ADDLIST", ps);
			act.setOutData("ROWNUM", rowNum);//存回显的行数，id用
			act.setForword(RULE_MODE_TYPE_URL);
		} catch (Exception e) {
			BizException e1 = new BizException(act,e,ErrorCodeConstant.QUERY_FAILURE_CODE,"授权规则维护");
			logger.error(logonUser,e1);
			act.setException(e1);
	...
	* @throws
	...
	public void ruleAdd(){
		...
		try {
			...
			Long oemCompanyId = GetOemcompanyId.getOemCompanyId(logonUser);
			String wrGroupId = request.getParamValue("WRGROUP_ID")==null?"0":request.getParamValue("WRGROUP_ID");//车型组id
			...
			ttWrRulemappingPO.setWarrantyGroup(new Long(wrGroupId));//索赔授权车型组
			ttWrRulemappingPO.setType(String.valueOf(Constant.AUDIT_TYPE_01)); //判断是技术室审核规则
	...
	* @throws
	...
	public void ruleUpdate(){
		...
		try {
			...
			updatetawrpo.setWarrantyGroup(new Long(wrGroupId));//索赔授权车型组
