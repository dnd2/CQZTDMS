package com.infodms.dms.actions.parts.storageManager.partDlrStockMvManager;
...
import com.infodms.dms.dao.parts.storageManager.partStockMvManager.partDlrStockMvDao;
...
public class partDlrStockMvAction extends BaseImport {
    public Logger logger = Logger.getLogger(partDlrStockMvAction.class);
    private static final partDlrStockMvDao dao = partDlrStockMvDao.getInstance();
    private ActionContext act = ActionContext.getContext();
    RequestWrapper request = act.getRequest();
    AclUserBean logonUser = (AclUserBean) act.getSession().get(Constant.LOGON_USER);

    //配件移库、调拨URL
    private static final String PART_MV_QUERY = "/jsp/parts/storageManager/partDlrStockMvManager/partStockMvQueryMain.jsp";//查询主页面
    private static final String PART_MV_MAIN = "/jsp/parts/storageManager/partDlrStockMvManager/partStockMvMain.jsp";//申请主页面
    private static final String PART_MV_VIEW = "/jsp/parts/storageManager/partDlrStockMvManager/partStockMvView.jsp";//查看页面
    private static final String PART_MV_CHECK_MAIN = "/jsp/parts/storageManager/partDlrStockMvManager/partStockMvCheckMain.jsp";//审核主页面
    private static final String PART_MV_OUTSTOCK_MAIN = "/jsp/parts/storageManager/partDlrStockMvManager/partStockMvOutStockMain.jsp";//出库主页面
    private static final String PART_MV_INSTOCK_MAIN = "/jsp/parts/storageManager/partDlrStockMvManager/partStockInStockMain.jsp";//入库主页面
    private static final String PART_MV_ADD = "/jsp/parts/storageManager/partDlrStockMvManager/partStockMvAdd.jsp";///配件移库单申请新增页面
    private static final String PART_MV_MOD = "/jsp/parts/storageManager/partDlrStockMvManager/partStockMvMod.jsp";///配件移库单申请修改页面
    private static final String PART_MV_CHECK = "/jsp/parts/storageManager/partDlrStockMvManager/partStockMvCheck.jsp";///配件移库单审核页面
    private static final String PART_MV_OUTSTOCK = "/jsp/parts/storageManager/partDlrStockMvManager/partStockMvOutStock.jsp";///配件移库单出库主页面
    private static final String PART_MV_INSTOCK = "/jsp/parts/storageManager/partDlrStockMvManager/partStockMvInStock.jsp";///配件移库单入库主页面
    private static final String PART_MV_PRINT = "/jsp/parts/storageManager/partDlrStockMvManager/partStockMvPrint.jsp";///配件移库单打印页面

    /**
     * <p>
     * Description: 获取有仓库的经销商
     * </p>
     */
    public void queryWhDealer() {
        try {
            Map<String, String> paramMap = new HashMap<String, String>();
            paramMap.put("ORG_CODE", CommonUtils.checkNull(request.getParamValue("ORG_CODE")));
            paramMap.put("ORG_NAME", CommonUtils.checkNull(request.getParamValue("ORG_NAME")));
            paramMap.put("NOT_EXISTS_ORG", logonUser.getDealerId());
            Integer curPage = request.getParamValue("curPage") != null
                    ? Integer.parseInt(request.getParamValue("curPage")) : 1; // 处理当前页
            PageResult<Map<String, Object>> ps = dao.queryWhDealer(paramMap, Constant.PAGE_SIZE, curPage);
            act.setOutData("ps", ps);
        } catch (Exception e) {// 异常方法
            BizException e1 = new BizException(act, e, ErrorCodeConstant.QUERY_FAILURE_CODE, "配件调拨获取经销商");
    ...
    public void partMvQueryInit() {
        ...
        try {
            ...
            // 机构ID
            String orgCode = logonUser.getDealerCode();
            // 机构编码
            String orgName = dao.getDealerName(orgId);
             // 机构名称
            ...
            BizException e1 = new BizException(act, e, ErrorCodeConstant.QUERY_FAILURE_CODE, "配件调拨申请页面初始化");
            logger.error(logonUser, e1);
            act.setException(e1);
        }
    }
    ...
    public void partMvAddInit() {
        ...
        try {
            ...
            whPo.setOrgId(Long.parseLong(orgId));
            whPo = (TtPartWarehouseDefinePO) dao.select(whPo).get(0);
            Long toWhId = whPo.getWhId();

            //取当前机构信息
            String changeCode = OrderCodeManager.getOrderCode(Constant.PART_CODE_RELATION_57);//获取制单单号
            act.setOutData("toWhId", toWhId);
            ...
            BizException e1 = new BizException(act, e, ErrorCodeConstant.QUERY_FAILURE_CODE, "移库新增页面初始化");
            logger.error(logonUser, e1);
            act.setException(e1);
        }
    }
    ...
    public void saveMVOrder() {
        ...
        try {
            ...
            // 被调拨经销商
            String fromOrgId = request.getParamValue("fromOrgId");// 被调拨经销商id
            String fromOrgCode = request.getParamValue("fromOrgCode");// 被调拨经销商编码
            String fromOrgName = request.getParamValue("fromOrgName");// 被调拨经销商名称
            
            String remark = request.getParamValue("remark");//调拨原因
            String whId = request.getParamValue("fromWhId");//调拨库房
            String toWhId = request.getParamValue("toWhId");//调拨移入库房
...
//            }
            ...
            if (null != partIdArr) {
                ...
                for (int i = 0; i < partIdArr.length; i++) {
                    ...
                    String locId = partIdLocId.split("_")[1];
                    ...
                    partList = dao.getPartStockInfos(partId, fromOrgId, whId, locId);
            ...
            if ("".equals(error)) {
                ...
                inserRMPo.setOrgId(Long.valueOf(fromOrgId));
                inserRMPo.setOrgCode(fromOrgCode);
                inserRMPo.setOrgName(fromOrgName);
                ...
                inserRMPo.setToorgId(Long.valueOf(orgId));
                inserRMPo.setToorgCode(orgCode);
                inserRMPo.setToorgName(orgName);
...
//                        //切换件旧件资源锁定逻辑
//                        dao.updateASdMvState("3", chgId + "", "1", dataPo.getPartOldcode(), "1",
//                                dataPo.getApplyQty() + "", null);
//                    }
//                }
                //end
...
//            String orgName = request.getParamValue("orgName");//单位名称
            ...
            String remark = request.getParamValue("remark");//调拨原因
            String whId = request.getParamValue("fromWhId");//调拨库房
//            String fromOrgId = request.getParamValue("fromOrgId");//制单单位id
//            String toWhId = request.getParamValue("toWhId");//调拨移入库房
//            String flag = request.getParamValue("flag");//正常 0:配件库存，1：售后切换件库存
...
//                    dao.updateASdMvState("1", null, "1", dataPo.getPartOldcode(), "3", dataPo.getApplyQty() + "",
//                            chgId + "");
//                }
//            }
            //释放end
...
//            }
            ...
            if ("".equals(error)) {
                ...
                updateMianPO.setOrgId(Long.valueOf(fromOrgId));
                updateMianPO.setOrgCode(fromOrgCode);
                updateMianPO.setOrgName(fromOrgName);
...
//            LaborListSummaryReportDao laborListSummaryReportDao = LaborListSummaryReportDao.getInstance();
            ...
            if ("view".equalsIgnoreCase(optionType)) {
                //查看页面
                Map<String, Object> main = dao.queryPartMvOrders(request, Constant.PAGE_SIZE_MAX, 1).getRecords()
                        .get(0);
                ...
                Map<String, Object> main = dao.queryPartMvOrders(request, Constant.PAGE_SIZE_MAX, 1).getRecords()
                        .get(0);
                List<Map<String, Object>> list = dao.queryMvPartsPageList(request, Constant.PAGE_SIZE_MAX, 1)
                        .getRecords();
                Long partSum = 0l;
            ...
            BizException e1 = new BizException(act, e, ErrorCodeConstant.QUERY_FAILURE_CODE, "页面初始化");
            logger.error(logonUser, e1);
            act.setException(e1);
        }
    }
    ...
    public void showPartStockBase() {
        ...
        try {
            ...
            Integer curPage = request.getParamValue("curPage") != null
                    ? Integer.parseInt(request.getParamValue("curPage")) : 1; // 处理当前页
            PageResult<Map<String, Object>> ps = dao.showPartStockBase(req, Constant.PAGE_SIZE, curPage);
    ...
    public void submitMVOrder() {
        ...
        try {
            ...
            // 验证调拨的配件可以数量是否能够调拨
            List<Map<String, Object>> partList = dao.getApplyChgPartList(chgId);
            for(Map<String, Object> part:partList){
                long normalQty = ((BigDecimal) part.get("NORMAL_QTY")).longValue(); // 可用库存
                long applyQty = ((BigDecimal) part.get("APPLY_QTY")).longValue(); // 申请数量
                if(normalQty - applyQty < 0){
                    error = "配件【"+part.get("PART_OLDCODE")+"】在【"+part.get("ORG_NAME")+"】下可用库存不足，不能够调拨！";
                    break;
                }
            }
...
//            String toWhId = request.getParamValue("toWhId");//移入库房
            ...
            if (null != partIdArr) {
                ...
                for (int i = 0; i < partIdArr.length; i++) {
                    String partIdLocId = partIdArr[i];
                    String partId = partIdLocId.split("_")[0];
                    String locId = partIdLocId.split("_")[1]; //配件货位ID
                    String partOldcode = CommonUtils.checkNull(req.getParamValue("PART_OLDCODE" + partIdLocId)); //配件编码
                    Integer checkQty = Integer
                            .valueOf(CommonUtils.checkNull(req.getParamValue("CHECK_QTY" + partIdLocId))); //审核移库数量
    ...
    public void outStockMvOrder() {
        ...
        try {
            String orgId = request.getParamValue("orgId");//制单单位id
//            String fromOrgId = request.getParamValue("fromOrgId");//调拨单位id
            String orgCode = request.getParamValue("orgCode");//单位编码
            String orgName = request.getParamValue("orgName");//单位名称
            String whId = request.getParamValue("whId");//移出库房
            String toWhId = request.getParamValue("toWhId");//移入库房
            Long chgId = Long.parseLong(request.getParamValue("CHG_ID"));//移库ID
            String chgCode = CommonUtils.checkNull(request.getParamValue("CHG_CODE"));//移库ID
            String[] partIdArr = req.getParamValues("cb");
...
//                        recordPO.setPartBatch(Constant.PART_RECORD_BATCH);
//                        recordPO.setVenderId(Long.parseLong(Constant.PART_RECORD_VENDER_ID));
            ...
            if ("".equals(error) && dtlPOArrayList.size() > 0) {
                ...
                if (!qhjDbFlag) {
                    ...
                    List<TtPartStockChgDtlPO> dataPos = dao.select(selPo);
                    for (TtPartStockChgDtlPO dataPo : dataPos) {
                        dao.updateASdMvState("2", chgId + "", "1", dataPo.getPartOldcode(), "3",
                                dataPo.getCheckQty() + "", chgId + "");
                    }
                }
    ...
    public void inStockMvOrder() {
        ...
        String error = "";
        try {
            String toOrgId = CommonUtils.checkNull(request.getParamValue("toOrgId"));// 制单单位id
            String toOrgCode = CommonUtils.checkNull(request.getParamValue("toOrgCode"));// 单位编码
            String toOrgName = CommonUtils.checkNull(request.getParamValue("toOrgName"));// 单位名称
            ...
            if (null != partLocIdArr) {
                ...
                for (int i = 0; i < partLocIdArr.length; i++) {
                    ...
                    String partId = partLocId.split(",")[0];
                    ...
                    String inLocCode = CommonUtils.checkNull(req.getParamValue("IN_LOC_CODE_" + partLocId)); //配件货位ID
...
//                        recordPO.setPartBatch(Constant.PART_RECORD_BATCH);
//                        recordPO.setVenderId(Long.parseLong(Constant.PART_RECORD_VENDER_ID));
                        recordPO.setPartNum(Long.valueOf(inQty));//出库数量
                        recordPO.setConfigId(Long.parseLong(Constant.PART_CODE_RELATION_57 + ""));
                        recordPO.setOrderId(chgId);//移库单ID
                        recordPO.setOrgId(Long.parseLong(toOrgId));
                        recordPO.setOrgCode(toOrgCode);
                        recordPO.setOrgName(toOrgName);
                        recordPO.setWhId(Long.parseLong(toWhId));
                        recordPO.setLocId(Long.valueOf(locId));
                        recordPO.setLocCode(inLocCode);
                        recordPO.setLocName(inLocCode);
                        recordPO.setOptDate(new Date());
                        recordPO.setCreateDate(new Date());
                        recordPO.setPersonId(logonUser.getUserId());
                        recordPO.setPersonName(logonUser.getName());
                        ...
                        chgRecordPO.setTolocCode(inLocCode);
                        chgRecordPO.setCreateDate(new Date());
                        chgRecordPO.setCreateBy(logonUser.getUserId());
                        chgRecordPO.setOrgId(Long.valueOf(toOrgId));
                        chgRecordPO.setWhId(Long.valueOf(whId));
                        chgRecordPO.setTowhId(Long.valueOf(toWhId));
                        chgRecordPO.setRecCode(chgCode);
                        chgRecordPOs.add(chgRecordPO);
    ...
    public void partMVDetailQuery() {
        ...
        try {
            Integer curPage = request.getParamValue("curPage") != null
                    ? Integer.parseInt(request.getParamValue("curPage")) : 1; // 处理当前页
            PageResult<Map<String, Object>> ps = dao.queryMvPartsPageList(request, Constant.PAGE_SIZE, curPage);

            act.setOutData("ps", ps);
        } catch (Exception e) {// 异常方法
            BizException e1 = new BizException(act, e, ErrorCodeConstant.QUERY_FAILURE_CODE, "配件移库明细查询");
            logger.error(logonUser, e1);
            act.setException(e1);
        }
    }
    ...
    public void exportSaleOrdersExcel() {
        ...
        try {
            ...
            if (list != null && list.size() != 0) {
                for (int i = 0; i < list.size(); i++) {
                    ...
                    if (map != null && map.size() != 0) {
                        ...
                        /*
                         * if (orderType1 == oderType) { detail[2] = "移库"; }
                         * else if (orderType2 == oderType) { detail[2] = "领用";
                         * }
                         */
                        ...
                        /*
                         * if (orderState1 == orderState) { detail[8] = "已保存"; }
                         * else if (orderState2 == orderState) { detail[8] =
                         * "已提交"; } else if (orderState3 == orderState) {
                         * detail[8] = "已完成"; } else if (orderState4 ==
                         * orderState) { detail[8] = "已通过"; } else if
                         * (orderState5 == orderState) { detail[8] = "已驳回"; }
                         * else if (orderState6 == orderState) { detail[8] =
                         * "已作废"; }
                         */
            ...
            String fileName = "";
            /*
             * if ((orderType1 + "").equals(orderType + "")) { fileName =
             * "配件移库单信息"; } else { fileName = "配件领用单信息"; }
             */

            ExcelUtil.toExceUtil(ActionContext.getContext().getResponse(), request, head, list1, fileName);
        } catch (Exception e) {
            BizException e1 = new BizException(act, e, ErrorCodeConstant.SPECIAL_MEG, "导出配件移库/领用单");
            logger.error(logonUser, e1);
            act.setException(e1);
        }
    }
