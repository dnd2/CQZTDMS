...
public class Misc_exManager extends BaseImport {

    public Logger logger = Logger.getLogger(Misc_exManager.class);
    private ActionContext act = ActionContext.getContext();
    RequestWrapper request = act.getRequest();
    AclUserBean logonUser = (AclUserBean) act.getSession().get(Constant.LOGON_USER);
    MiscManagerDAO dao = MiscManagerDAO.getInstance();
    Misc_exManagerDAO daoEx = Misc_exManagerDAO.getInstance();

    private final String miscMainInit = "/jsp/parts/storageManager/misc_exManager/misc_exMain.jsp";
    private final String miscViewInit = "/jsp/parts/storageManager/misc_exManager/miscView.jsp";
    private final String miscAddInit = "/jsp/parts/storageManager/misc_exManager/misc_exAdd.jsp";
    ...
    private final String miscPrintInit = "/jsp/parts/storageManager/misc_exManager/misc_exPrint.jsp";
    ...
    public void getMainList() {
        ...
        try {
            ...
            PageResult<Map<String, Object>> ps = dao.getMainList(request, orgId, 2, Constant.PAGE_SIZE, curPage);
            act.setOutData("ps", ps);
        } catch (Exception e) {
            BizException e1 = new BizException(act, e, ErrorCodeConstant.QUERY_FAILURE_CODE, "杂项出库初始化错误!");
            logger.error(logonUser, e1);
            act.setException(e1);
        }
    }
    ...
    public void showPartBase() {
        ...
        try {
            ...
            PageResult<Map<String, Object>> ps = daoEx.showPartBase(req, orgId, Constant.PAGE_SIZE, curPage);
            act.setOutData("ps", ps);
    ...
    public void MiscSave() {
        ...
        try {
            ...
            String exType = CommonUtils.checkNull(req.getParamValue("EX_TYPE")); //出库类型
            ...
            if ("".equals(error)) {
                ...
                if (null != partIdArr) {
                    for (int i = 0; i < partIdArr.length; i++) {
                        ...
                        if (list != null && list.size() > 0 && list.get(0) != null) {
                            ...
                            //杂项出库时出库数量不能大于账面库存和当前可以数量
                            if (Integer.parseInt(itemQty) < Integer.parseInt(returnQty) || Integer.parseInt(normalQty) < Integer.parseInt(returnQty)) {
                                error = "配件[" + partOldcode + "]出库数量大于现有库存数量！";
                                BizException e1 = new BizException(act, new RuntimeException(), ErrorCodeConstant.SPECIAL_MEG, error);
                                throw e1;
                            }
                        }
                        //3.插入tt_parts_misc_detail
                ...
                if (!"".equals(exType) && null != exType) {
                    po.setExType(Integer.parseInt(exType));
                }
                po.setBType(2);
                po.setRemark(remark);
                if (!"".equals(departmentCode) && null != departmentCode) {
                    po.setDepartmentCode(departmentCode);
                }
                dao.insert(po);
                success = "杂项出库单：" + orderCode + "保存成功!";
            }
    ...
    public void InsertDetail(JSONObject dataObject) {
        try {
            ...
            for (int i = 0; i < partIdArr.length; i++) {
                ...
                Map map = inDao.queryPartAndLocationInfo(Long.parseLong(partId), dataObject.getLong("orgId"), dataObject.getLong("whId"));//查询当前配件信息及其货位信息
                ...
                ttPartRecordPO.setAddFlag(1);//出库标记
                ttPartRecordPO.setState(1);//正常出库		    
                ttPartRecordPO.setPartNum(CommonUtils.parseLong(inQty));//出库数量
                ttPartRecordPO.setPartId(po.getPartId());//配件ID
                ttPartRecordPO.setPartBatch("1306");//////////////////配件批次
                ttPartRecordPO.setVenderId(21799l);///////////////////配件供应商
                ttPartRecordPO.setConfigId(Long.valueOf(Constant.PART_CODE_RELATION_31));//杂项出库单
                ttPartRecordPO.setOrderId(dataObject.getLong("orderId"));//杂项出库单ID
                ttPartRecordPO.setOrderCode(dataObject.getString("orderCode"));//杂项出库单编码
                ...
                logger.info("---记录表新增开始");
                dao.insert(ttPartRecordPO);//新增出出库记录
                logger.info("---记录表新增结束");
            ...
            POContext.cleanTxn();
            BizException e1 = new BizException(act, e, ErrorCodeConstant.SPECIAL_MEG, "杂项出库单行表新增错误,请联系管理员!");
            logger.error(logonUser, e1);
            act.setException(e1);
        }
    ...
    public void InsertMain(JSONObject dataObject) {
        try {
            ...
            BizException e1 = new BizException(act, e, ErrorCodeConstant.SPECIAL_MEG, "杂项出库单主表新增错误,请联系管理员!");
            logger.error(logonUser, e1);
            act.setException(e1);
        }
    }
...
//				tempmap.put("remark", cells.length < 3 || null == cells[2].getContents() ? "" : cells[2].getContents().trim());
    ...
    public void miscPrint() {
        ...
        try {
            ...
            BizException e1 = new BizException(act, e, ErrorCodeConstant.SPECIAL_MEG, "标准杂发单打印错误,请联系管理员!");
    ...
    public void exportDtl() {
        ...
        try {
            ...
            head[7] = "日期";
            head[8] = "备注";
            List<Map<String, Object>> list = dao.getMiscDtl(request, orgId, "2", Constant.PAGE_SIZE_MAX, 1).getRecords();
            ...
            for (int i = 0; i < list.size(); i++) {
                ...
                if (map != null && map.size() != 0) {
                    ...
                    //detail[6] = CommonUtils.checkNull(map.get("PRICE"));
