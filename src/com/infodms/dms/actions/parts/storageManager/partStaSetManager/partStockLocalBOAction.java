...
import com.infodms.dms.dao.parts.storageManager.partStaSetManager.partStockLocalBODao;
 ...
 * @remark
 */
public class partStockLocalBOAction extends BaseImport {
	public Logger logger = Logger.getLogger(partStockLocalBOAction.class);
	private static final partStockLocalBODao dao = partStockLocalBODao.getInstance();
	private ActionContext act = ActionContext.getContext();
	RequestWrapper request = act.getRequest();
	private static final int unLockedVal = Constant.PART_STATE_UN_LOCKED;  //配件未锁定
	
	//现场BO单处理
	private static final String PART_STOCK_SETTING = "/jsp/parts/storageManager/partStaSetManager/partStockLocalBO/partStockLocalBO.jsp";//现场BO单处理首页
	private static final String PART_STOCK_VIEW = "/jsp/parts/storageManager/partStaSetManager/partStockLocalBO/partStockLocalBOView.jsp";//现场BO单处理查看页面
	private static final String PART_STOCK_HANDLE = "/jsp/parts/storageManager/partStaSetManager/partStockLocalBO/partStockLocalBOHdle.jsp";//现场BO单处理处理页面
	private static final String PART_STOCK_ADD = "/jsp/parts/storageManager/partStaSetManager/partStockLocalBO/partStockLocalBOAdd.jsp";///现场BO单处理新增页面
	 ...
	 */
	public void partStockLocalBOInit() {
		...
		try {
			...
			BizException e1 = new BizException(act, e,
					ErrorCodeConstant.QUERY_FAILURE_CODE, "现场BO单处理页面初始化");
	 ...
	 * LastDate    : 2013-4-24
	 */
	public void partStockLocalBOSearch() {
		...
		try {
			...
			BizException e1 = new BizException(act, e,
					ErrorCodeConstant.QUERY_FAILURE_CODE, "条件查询现场BO单处理信息");
			logger.error(logonUser, e1);
			act.setException(e1);
		}
	}
	...
	public void viewStockDeatilInint() {
		...
		try {
			...
			BizException e1 = new BizException(act, e,
					ErrorCodeConstant.QUERY_FAILURE_CODE, "现场BO单处理查看页面初始化");
			logger.error(logonUser, e1);
			act.setException(e1);
		}
	}
	...
	public void partStockAddInit() {
		...
		try {
			...
			BizException e1 = new BizException(act, e,
					ErrorCodeConstant.QUERY_FAILURE_CODE, "现场BO单处理新增页面初始化");
			logger.error(logonUser, e1);
			act.setException(e1);
		}
	}
...
//		String partBatch = Constant.PART_RECORD_BATCH;//配件批次
		...
		BizException e1 = new BizException(act,e,ErrorCodeConstant.SPECIAL_MEG,"保存新增现场BO单处理信息");
		logger.error(logonUser,e1);
		act.setException(e1);
	 ...
	 * LastDate    : 2013-4-24
	 */
	public void partStockLocalBOUpload(){
		AclUserBean logonUser = (AclUserBean) act.getSession().get(Constant.LOGON_USER);
		RequestWrapper req = act.getRequest();
		try {
			String whId = CommonUtils.checkNull(req.getParamValue("whId")); //仓库ID
	...
	public void exportPartStockStatusExcel() {
		...
		try {
			...
			String fileName = "现场BO单处理信息";
			this.exportEx(fileName, ActionContext.getContext().getResponse(),
					request, head, list1);

		} catch (Exception e) {
			BizException e1 = new BizException(act, e,
					ErrorCodeConstant.SPECIAL_MEG, "导出现场BO单处理");
			logger.error(logonUser, e1);
			act.setException(e1);
		}
	}
