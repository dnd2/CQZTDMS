...
import com.infodms.dms.dao.parts.storageManager.partStaSetManager.partStockWrongDao;
 ...
 * @remark
 */
public class partStockWrongAction extends BaseImport {
	public Logger logger = Logger.getLogger(partStockWrongAction.class);
	private static final partStockWrongDao dao = partStockWrongDao.getInstance();
	private ActionContext act = ActionContext.getContext();
	RequestWrapper request = act.getRequest();
	private static final int unLockedVal = Constant.PART_STATE_UN_LOCKED;  //配件未锁定
	
	//配件来货错误处理
	private static final String PART_STOCK_SETTING = "/jsp/parts/storageManager/partStaSetManager/partStockWrong/partStockWrongSetting.jsp";//配件来货错误处理首页
	private static final String PART_STOCK_VIEW = "/jsp/parts/storageManager/partStaSetManager/partStockWrong/partStockWrongView.jsp";//配件来货错误处理查看页面
	private static final String PART_STOCK_HANDLE = "/jsp/parts/storageManager/partStaSetManager/partStockWrong/partStockWrongHdle.jsp";//配件来货错误处理处理页面
	private static final String PART_STOCK_ADD = "/jsp/parts/storageManager/partStaSetManager/partStockWrong/partStockWrongAdd.jsp";//配件来货错误处理新增页面
	 ...
	 */
	public void partStockWrongInit() {
		...
		try {
			...
			BizException e1 = new BizException(act, e,
					ErrorCodeConstant.QUERY_FAILURE_CODE, "配件来货错误处理页面初始化");
	 ...
	 * LastDate    : 2013-4-24
	 */
	public void partStockWrongSearch() {
		...
		try {
			...
			BizException e1 = new BizException(act, e,
					ErrorCodeConstant.QUERY_FAILURE_CODE, "条件查询配件来货错误处理信息");
			logger.error(logonUser, e1);
			act.setException(e1);
		}
	}
	...
	public void viewStockDeatilInint() {
		...
		try {
			...
			BizException e1 = new BizException(act, e,
					ErrorCodeConstant.QUERY_FAILURE_CODE, "配件来货错误处理查看页面初始化");
			logger.error(logonUser, e1);
			act.setException(e1);
		}
	}
	...
	public void partStockAddInit() {
		...
		try {
			...
			BizException e1 = new BizException(act, e,
					ErrorCodeConstant.QUERY_FAILURE_CODE, "配件来货错误处理新增页面初始化");
			logger.error(logonUser, e1);
			act.setException(e1);
		}
	}
...
//		String partBatch = Constant.PART_RECORD_BATCH;//配件批次
		...
		BizException e1 = new BizException(act,e,ErrorCodeConstant.SPECIAL_MEG,"保存新增配件来货错误信息");
		logger.error(logonUser,e1);
		act.setException(e1);
	...
	public void exportExcelTemplate(){
		...
		try{
			...
			list.add(listHead);
			// 导出的文件名
			String fileName = "配件来货错误处理模板.xls";
			// 导出的文字编码
	 ...
	 * LastDate    : 2013-4-24
	 */
	public void partStockWrongUpload(){
		AclUserBean logonUser = (AclUserBean) act.getSession().get(Constant.LOGON_USER);
		RequestWrapper req = act.getRequest();
		try {
			String whId = CommonUtils.checkNull(req.getParamValue("whId")); //仓库ID
	...
	public void exportPartStockStatusExcel() {
		...
		try {
			...
			String fileName = "配件来货错误处理信息";
			this.exportEx(fileName, ActionContext.getContext().getResponse(),
					request, head, list1);

		} catch (Exception e) {
			BizException e1 = new BizException(act, e,
					ErrorCodeConstant.SPECIAL_MEG, "导出配件来货错误处理");
			logger.error(logonUser, e1);
			act.setException(e1);
		}
	}
