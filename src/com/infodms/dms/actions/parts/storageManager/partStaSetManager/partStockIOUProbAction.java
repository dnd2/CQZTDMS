...
import com.infodms.dms.dao.parts.storageManager.partStaSetManager.partStockIOUProbDao;
 ...
 * @remark
 */
public class partStockIOUProbAction extends BaseImport {
	public Logger logger = Logger.getLogger(partStockIOUProbAction.class);
	private static final partStockIOUProbDao dao = partStockIOUProbDao.getInstance();
	private ActionContext act = ActionContext.getContext();
	RequestWrapper request = act.getRequest();
	private static final int unLockedVal = Constant.PART_STATE_UN_LOCKED;  //配件未锁定
	
	//配件借条处理
	private static final String PART_STOCK_SETTING = "/jsp/parts/storageManager/partStaSetManager/partStockIOUProb/partStockIOUProb.jsp";//配件借条处理首页
	private static final String PART_STOCK_VIEW = "/jsp/parts/storageManager/partStaSetManager/partStockIOUProb/partStockIOUProbView.jsp";//配件借条处理查看页面
	private static final String PART_STOCK_HANDLE = "/jsp/parts/storageManager/partStaSetManager/partStockIOUProb/partStockIOUProbHdle.jsp";//配件借条处理处理页面
	private static final String PART_STOCK_ADD = "/jsp/parts/storageManager/partStaSetManager/partStockIOUProb/partStockIOUProbAdd.jsp";///配件借条处理新增页面
	 ...
	 */
	public void partStockIOUProbInit() {
		...
		try {
			...
			BizException e1 = new BizException(act, e,
					ErrorCodeConstant.QUERY_FAILURE_CODE, "配件借条处理页面初始化");
	 ...
	 * LastDate    : 2013-4-22
	 */
	public void partStockIOUProbSearch() {
		...
		try {
			...
			BizException e1 = new BizException(act, e,
					ErrorCodeConstant.QUERY_FAILURE_CODE, "条件查询配件借条处理信息");
			logger.error(logonUser, e1);
			act.setException(e1);
		}
	}
...
//		String partBatch = Constant.PART_RECORD_BATCH;//配件批次
	 ...
	 * LastDate    : 2013-4-25
	 */
	public void partStockIOUProbUpload(){
		AclUserBean logonUser = (AclUserBean) act.getSession().get(Constant.LOGON_USER);
		RequestWrapper req = act.getRequest();
		try {
			String whId = CommonUtils.checkNull(req.getParamValue("whId")); //仓库ID
	...
	public void exportPartStockStatusExcel() {
		...
		try {
			...
			String fileName = "配件借条处理信息";
			this.exportEx(fileName, ActionContext.getContext().getResponse(),
					request, head, list1);

		} catch (Exception e) {
			BizException e1 = new BizException(act, e,
					ErrorCodeConstant.SPECIAL_MEG, "导出配件借条处理");
			logger.error(logonUser, e1);
			act.setException(e1);
		}
	}
