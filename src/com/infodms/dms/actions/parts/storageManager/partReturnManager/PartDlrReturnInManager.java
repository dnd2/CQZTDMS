        ...
        try {
            ...
            if (partDlrReturnMainPO.getState().intValue() == Constant.PART_DLR_RETURN_STATUS_05.intValue()) {//退货单状态为入库中才能入库
                ...
                for (int i = 0; i < dtlIds.length; i++) {
                    ...
                    String venderId = CommonUtils.checkNull(poMap.get("VENDER_ID")); // 供应商
                    ...
                    if (po.getStatus().intValue() == 2) {//如果已经入完成就不能再次入库
                        ...
                        boolean isLocked = orderInDao.isLocked(po.getPartId(), whId, poMap.get("LOC_ID").toString(), logonUser.getDealerId() == null ? logonUser.getOrgId() : Long.valueOf(logonUser.getDealerId()));//当前仓库中的配件是否被锁定
                        if (isLocked) {//如果该配件在库存中已经被锁定
                            dtlError += "配件【" + po.getPartCname() + "】已经被锁定,不能入库!<br>";
                        } else {
...
//                            String[] locInfo = CommonUtils.checkNull(request.getParamValue("LOC_ID_" + po.getPartId())).split(",");
//                            String locId = locInfo[0];
//                            String locCode = locInfo[1];
...
//                            ttPartRecordPO.setPartBatch("1306");//////////////////配件批次
                            ...
                            ttPartRecordPO.setVenderId(Long.parseLong(venderId));///////////////////配件供应商
                            ...
                            if (logonUser.getDealerId() == null) {
                                locPo.setRelocId(Long.valueOf(locId));
                            }
                            ...
                            if (ll.size() == 0) {
                                ...
                                if (logonUser.getDealerId() == null) {
                                    locPo.setLocCode(locCode);
                                    locPo.setLocName(locCode);
                                    locPo.setRelocId(Long.valueOf(locId));
                                } else {
                                    locPo.setLocCode("暂无");
                                    locPo.setLocName("暂无");
...
//                            TtPartDlrReturnDtlPO returnDtlPO = new TtPartDlrReturnDtlPO();

                            if (logonUser.getDealerId() == null) {
                                TtPartLoactionDefinePO loactionDefinePO1 = new TtPartLoactionDefinePO();
                                loactionDefinePO1.setRelocId(Long.valueOf(locId));
                                loactionDefinePO1.setPartId(po.getPartId());
                                loactionDefinePO1.setWhId(Long.valueOf(whId));
                                loactionDefinePO1.setState(Constant.STATUS_ENABLE);
                                loactionDefinePO1.setStatus(Constant.IF_TYPE_YES);
                                loactionDefinePO1 = (TtPartLoactionDefinePO) dao.select(loactionDefinePO1).get(0);
                                ttPartRecordPO.setLocId(loactionDefinePO1.getLocId());
                                ttPartRecordPO.setLocCode(loactionDefinePO1.getLocCode());
                                ttPartRecordPO.setLocName(loactionDefinePO1.getLocName());
//                                returnDtlPO.setInlocId(loactionDefinePO1.getLocId());
                                ttPartReturnRecordPO.setLocId(loactionDefinePO1.getLocId());//记录中的货位ID
                                VwPartStockPO vwPartStockPO = new VwPartStockPO();
                                vwPartStockPO.setPartId(po.getPartId());
                                vwPartStockPO.setWhId(Long.valueOf(whId));
                                vwPartStockPO.setLocId(loactionDefinePO1.getLocId());

                               /* if (dao.select(vwPartStockPO).size() > 0) {
                                    vwPartStockPO = (VwPartStockPO) dao.select(vwPartStockPO).get(0);
                                    ttPartReturnRecordPO.setItemQty(Integer.valueOf(vwPartStockPO.getItemQty() + ""));//记录中的退货入库时的库存
                                } else {
                                    ttPartReturnRecordPO.setItemQty(0);
                                }*/
                            } else {
                                Map<String, Object> map = indao.queryPartAndLocationInfo(po.getPartId(), orgBeanList.get(0).getOrgId(), CommonUtils.parseLong(whId));//查询当前配件信息及其货位信息
...
//                                returnDtlPO.setInlocId(((BigDecimal) map.get("LOC_ID")).longValue());
                            }
...
//                            ttPartRecordPO1.setPartBatch("1306");//////////////////配件批次
                            ttPartRecordPO1.setPartBatch(batchNo);//////////////////配件批次
                            ttPartRecordPO1.setVenderId(Long.parseLong(venderId));///////////////////配件供应商
...
//                    ins.add(1, Constant.PART_CODE_RELATION_30);
