...
public class MarketQuestionOrderQueryDealerManager {
	public Logger logger = Logger.getLogger(MarketQuestionOrderQueryDealerManager.class);
	...
	// 页面导向URL
	private final String queryUrl = "/jsp/feedbackMng/query/marketQuesDealerApply.jsp";
	...
	public void queryMarketOrderInfo(){
		...
		sqlStr.append("tt.info_type,tt.problem_describe, tt.user_request,tt.advice_deal_mode,tt.order_date,tc.code_desc,tt.create_date,tt.create_by,");
		...
		sqlStr.append("and tt.dealer_id="+logonUserBean.getDealerId()+" ");
		// 从request中取出查询条件
		String orderId = request.getParamValue("query_order_no");// 工单号
		String vin = request.getParamValue("query_vin");// 车辆识别码
		String dealerId= request.getParamValue("dealerCode");//申请单位代码
		...
		if (orderId != null && !"".equals(orderId))
			whereStr.append(" and tt.order_id='" + orderId + "' ");
		if (dealerId != null && !"".equals(dealerId))
			whereStr.append(" and tt.dealer_id='" + dealerId + "' ");
		if (orderStatus != null && !"".equals(orderStatus))
			whereStr.append(" and tt.status='" + orderStatus + "' ");
		if (vin != null && !"".equals(vin))
			whereStr.append(" and tt.vin='" + vin + "' ");
	...
	public void queryOrderDetailInfo(){
		...
		sqlStr.append("tvmg.group_name,tv.engine_no,tv.factory_date,tas.consignation_date,tv.history_mile,");
