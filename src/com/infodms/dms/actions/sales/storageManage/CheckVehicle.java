...
public class CheckVehicle extends BaseDao {
	...
	private final String CheckVehicleInitUrl = "/jsp/sales/storageManage/checkVehicleInit.jsp";
	...
	private final String tolookLogisticsURL = "/jsp/sales/storageManage/logisticsInfo.jsp";
	/**
	 * FUNCTION : 车辆验收页面初始化
	 * 
	 * @param :
	...
	public void CheckVehicleInit() {
		...
		try {
			...
			List<Map<String,Object>> lists=CheckVehicleDAO.getCheckLists(poseId, dealerIds, null, null,null, Constant.PAGE_SIZE, 1);;
	...
	public void checkList() {
		...
		try {
			...
			PageResult<Map<String, Object>> ps  = CheckVehicleDAO.getCheckListVeh(poseId, dealerIds, vin, dlvNo,dlvryNo, Constant.PAGE_SIZE, curPage);
			List<Map<String, Object>> lists= CheckVehicleDAO.getCheckLists(poseId, dealerIds, vin, dlvNo,dlvryNo, Constant.PAGE_SIZE, curPage);
			act.setOutData("lists",lists);
	...
	public void checkSubmit() {
		...
		try {
			...
			//tvdedA=(TtVsDlvryErpDtlPO)dao.select(tvdedA).get(0);
			...
			tvdedQ.setDetailId(new Long(dlvryDtlId));
			tvdedQ=(TtVsDlvryErpDtlPO)dao.select(tvdedQ).get(0);//查询子表得到req_id
			
			TmVehiclePO tvp=new TmVehiclePO();
			tvp.setVehicleId(tvdedQ.getVehicleId());
			tvp=(TmVehiclePO)dao.select(tvp).get(0);//查出车辆信息
			
			TtVsDlvryDtlPO tvdp=new TtVsDlvryDtlPO();
			TtVsDlvryDtlPO tvdpQ=new TtVsDlvryDtlPO();
			tvdp.setReqId(tvdedQ.getReqId());
			tvdp.setMaterialId(tvp.getMaterialId());
			tvdpQ.setReqId(tvdedQ.getReqId());
			tvdpQ.setMaterialId(tvp.getMaterialId());
			tvdpQ=(TtVsDlvryDtlPO) dao.select(tvdpQ).get(0);//根据req_id 和物料ID更新子表
			int ys_total=0;
			...
			dao.update(tvdp, tvdp1); //更新子表验收数量
			...
			int dlvystotal=0;
			tpQ=(TtVsDlvryPO) dao.select(tpQ).get(0);//根据req_id 和物料ID更新子表
		...
		try {
			...
			if (null != dlvryDtlIds) {
				...
				for (int i = 0; i < len; i++) {
					...
					tvdedQ=(TtVsDlvryErpDtlPO)dao.select(tvdedQ).get(0);
					TmVehiclePO tvp=new TmVehiclePO();
					tvp.setVehicleId(tvdedQ.getVehicleId());
					tvp=(TmVehiclePO)dao.select(tvp).get(0);//查出车辆信息
