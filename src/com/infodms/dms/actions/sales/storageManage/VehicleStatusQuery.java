...
import com.infodms.dms.dao.sales.storageManage.VehicleStatusDAO;
...
public class VehicleStatusQuery extends BaseDao{
	public Logger logger = Logger.getLogger(VehicleStatusQuery.class);
	private ActionContext act = ActionContext.getContext();
	private static final VehicleStatusDAO dao = new VehicleStatusDAO ();
	
	public static final VehicleStatusDAO getInstance() {
		return dao;
	}
	...
	private final String  StorageStatusInitUrl = "/jsp/sales/storageManage/vehicleStatusInit.jsp";
	 ...
	 */
	public void vechileStatusQueryInit(){
		AclUserBean logonUser = null;
		try {
			logonUser = (AclUserBean) act.getSession().get(Constant.LOGON_USER);	
			...
			act.setForword(StorageStatusInitUrl);
		} catch (Exception e) {
	 ...
	 * LastUpdate	:	2010-5-25
	 */
	public void vehicleStatusQuery(){
		AclUserBean logonUser = null;
		try {
			RequestWrapper request = act.getRequest();
			logonUser = (AclUserBean) act.getSession().get(Constant.LOGON_USER);	
			...
			String lockStatus = CommonUtils.checkNull(request.getParamValue("lockStatus"));  		//锁定状态
			String bodyColor = CommonUtils.checkNull(request.getParamValue("bodyColor"));  		//车身颜色
			Integer curPage = request.getParamValue("curPage") != null ? Integer.parseInt(request.getParamValue("curPage")) : 1; // 处理当前页
			Map<String,String> condition=new HashMap<String, String>();
			condition.put("materialCode", materialCode);
			condition.put("vin", vin);
			condition.put("vehicle_life", vehicle_life);
			condition.put("lockStatus", lockStatus);
			condition.put("bodyColor", bodyColor);
		
			PageResult<Map<String, Object>> ps = dao.getVechileStatusQuery(condition,Constant.PAGE_SIZE, curPage);
			act.setOutData("ps", ps);
		} catch (Exception e) {
			BizException e1 = new BizException(act, e,
