...
public class OrderResourceReserve {
	...
	private final String BALANCE_SELECT = "/jsp/sales/ordermanage/orderaudit/balance_select.jsp";// 经销商余额查看
	public void orderResourceReserveQueryPre() {
		...
		try {
			...
			act.setRedirect(ORDER_RESOURCE_RESERVE_QUERY_URL);
		} catch (Exception e) {
	...
	public void orderResourceReserveDetailQuery() {
		...
		try {
			...
			//可用余额验证
			List<Map<String,Object>> list3=dao.getDealerBalanceAmountDao(map.get("ORDER_ORG_ID").toString());
			...
			act.setOutData("totalAmount", list3.get(0).get("TOTAL_AMOUNT").toString());
			act.setOutData("outStandingAmount", list3.get(0).get("OUTSTANDING_ACCOUNT").toString());
			act.setOutData("balanceAmount", list3.get(0).get("BALANCE_AMOUNT").toString());
			System.out.println("============balanceAmout======="+list3.get(0).get("BALANCE_AMOUNT").toString()+"=========");
	...
	public void orderResourceReserveSave() {
		...
		try {
			...
			TtVsDlvryReqPO tvdrp0 = new TtVsDlvryReqPO();
			tvdrp0.setReqId(new Long(reqId));
			tvdrp0=(TtVsDlvryReqPO)dao.select(tvdrp0).get(0);

			String returnValue = "1";
			//判断发运申请的状态是否符合条件
			if(CommonUtils.judgeReqBillStatus(reqId,Constant.ORDER_REQ_STATUS_03.intValue())){
				act.setOutData("returnValue", 2);
				return;
			}
...
//						} else {
							...
							tvdrq2.setReqStatus(Constant.ORDER_REQ_STATUS_FINAL);// 最终审核完成
...
//								}
								...
								//如果发运申请单号是以6开头就
//								if(!tvdrp0.getDlvryReqNo().startsWith("6")){
								// 删除资源保留
...
//							}
						...
						tvrcp.setCheckStatus(Constant.ORDER_REQ_STATUS_FINAL);
...
//						}
						...
						String receiverDealerId = null; // 收货方经销商ID
						if ("电商订单".equals(orderTypeName)) {
							receiverDealerId = orderDealerId;
						} else {
							receiverDealerId = CommonUtils.checkNull(request.getParamValue("receiver")); // 收货方经销商ID
						}
						
						this.dmsSendToErp(orderDealerId, receiverDealerId, orderTypeName, warehouseId, reqId, checkRemark);
	 ...
	 *            发运申请ID
	 */
	private void dmsSendToErp(String orderDealerId, String receiverDealerId, String orderTypeName, String warehouseId, String reqId,String checkRemark) {
		...
		try {
			...
			TmVsAddressPO apo = dao.getDlrAddressByID(receiverDealerId==null?null:new Long(receiverDealerId));// 经销商地址PO
			...
			if (list != null && list.size() > 0) {
				if (dpo != null && repo != null) {
					...
					for (int i = 0; i < list.size(); i++) {
						...
						cedpo.setShipToLocation(apo != null ? apo.getAddCode() : null);// 收货地址编号
...
//								cedpo.setFundsType(tpo.getTypeCode());// 资金类型在ERP
						...
						String remark="";
						if(repo.getReqRemark()!=null){
							remark+=repo.getReqRemark();
						}
						if(checkRemark!=null){
							remark+=checkRemark;
						}
						cedpo.setRemark(remark);
						cedpo.setOrderedQuantity(BigDecimal.valueOf(redpo.getReqAmount()));// 申请数量
						cedpo.setCreationDate(nowDate);// 创建时间
					...
					throw new RuntimeException("====*****DMS -> ERP 对接口表 经销商订单发运申请提报表 【CUX_ERP_DELIVER】 下发失败了,【" + erro + " 为 NULL】 *** ====");
				}

			} else {
				throw new RuntimeException("====*****DMS -> ERP 对接口表 经销商订单发运申请提报表 【CUX_ERP_DELIVER】 下发失败了,【订单发运申请明细信息长度为 0 】 *** ====");
			}
...
//			}
				...
				tvdrp1.setReqId(new Long(reqId));
				TtVsDlvryReqPO tvdrp2 = new TtVsDlvryReqPO();
				tvdrp2.setReqStatus(Constant.ORDER_REQ_STATUS_05);// 驳回
				tvdrp2.setVer(new Integer(Integer.parseInt(reqVer) + 1));
				tvdrp2.setUpdateBy(logonUser.getUserId());
				tvdrp2.setUpdateDate(new Date(System.currentTimeMillis()));
				dao.update(tvdrp1, tvdrp2);
				...
				tvrcp.setCheckUserId(logonUser.getUserId());
				tvrcp.setCheckDate(new Date(System.currentTimeMillis()));
				tvrcp.setCheckStatus(Constant.ORDER_REQ_STATUS_05);
				if (checkRemark != null && !"".equals(checkRemark)) {
					tvrcp.setCheckDesc(checkRemark);
				}
				tvrcp.setCreateBy(logonUser.getUserId());
				tvrcp.setCreateDate(new Date(System.currentTimeMillis()));
				dao.insert(tvrcp);
//				if(!orderType.equals(Constant.ORDER_TYPE_01.toString())){
					// 释放保留资源
					for (int i = 0; i < detailId.length; i++) {
						if (detailId[i] != null && !detailId.equals("")) {
							TtVsOrderResourceReservePO tvorrp1 = new TtVsOrderResourceReservePO();
							...
							tvorrp2.setReserveStatus(Constant.RESOURCE_RESERVE_STATUS_02);// 已取消
							dao.update(tvorrp1, tvorrp2);
							//YIN start 2014-10-24 修改发运明细的保留数量
							TtVsDlvryReqDtlPO tvdrd1=new TtVsDlvryReqDtlPO();
							tvdrd1.setDetailId(new Long(detailId[i]));
							TtVsDlvryReqDtlPO tvdrd2=new TtVsDlvryReqDtlPO();
							tvdrd2.setReserveAmount(new Integer(0));
							dao.update(tvdrd1, tvdrd2);
							//end
						}
					}
//				}				
...
//
//				// 向发运申请操作日志表写入日志信息
				//ReqLogUtil.creatReqLog(Long.parseLong(reqId), Constant.REQ_LOG_TYPE_05, logonUser.getUserId());
//			} else {
//				returnValue = "2";
//			}
			act.setOutData("returnValue", returnValue);
...
*/
	...
	public void balanceSelect() {
		AclUserBean logonUser = (AclUserBean) act.getSession().get(Constant.LOGON_USER);
		try {
			String dealerId = CommonUtils.checkNull(request.getParamValue("dealerId"));// 仓库id
			TmDealerPO td=new TmDealerPO();
			td.setDealerId(new Long(dealerId));
			td=(TmDealerPO) dao.select(td).get(0);
			Map<String, String> map = new HashMap<String, String>();
			map.put("dealerId", td.getDealerId().toString());

			List<Map<String, Object>> w_List = dao.balanceSelect(map);

			act.setOutData("w_List", w_List);
			act.setForword(BALANCE_SELECT);
		} catch (Exception e) {
			BizException e1 = new BizException(act, e, ErrorCodeConstant.QUERY_FAILURE_CODE, "订单资源审核批次号选择");
			logger.error(logonUser, e1);
			act.setException(e1);
