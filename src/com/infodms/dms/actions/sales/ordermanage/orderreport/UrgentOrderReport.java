 ...
 * 
 */
public class UrgentOrderReport {
	private Logger logger = Logger.getLogger(UrgentOrderReport.class);
	...
	private final String URGENT_ORDER_REPORT_QUERY_URL = "/jsp/sales/ordermanage/orderreport/urgentOrderReportQuery.jsp";// 补充订单提报查询页面
	private final String URGENT_ORDER_REPORT_ADD_URL = "/jsp/sales/ordermanage/orderreport/urgentOrderReportAdd.jsp";// 补充订单提报新增页面
	private final String URGENT_ORDER_REPORT_MOD_URL = "/jsp/sales/ordermanage/orderreport/urgentOrderReportMod.jsp";// 补充订单提报修改页面

	public void urgentOrderReoprtQueryPre() {
	...
	public void urgentOrderReoprtQuery() {
		...
		try {
			...
			String companyId = logonUser.getOemCompanyId().toString();
			String[] array;
			String year="";
			String week="";
	...
	public void urgentOrderReoprtAddPre() {
		AclUserBean logonUser = (AclUserBean) act.getSession().get(
				Constant.LOGON_USER);
		try {
			...
			Long oemCompanyId = GetOemcompanyId.getOemCompanyId(logonUser);
			System.out.println("---11--"+logonUser.getCompanyId());
			System.out.println("---22--"+logonUser.getOemCompanyId());
//			String orderWeek = CommonUtils.checkNull(request
//					.getParamValue("orderWeek"));
//			String[] array = orderWeek.split("-");
//			String year = array[0];
//			String week = array[1];
			Map<String,Object> map=dao.getDealerAreas(logonUser.getDealerId());
			Long poseId = logonUser.getPoseId();
			...
			act.setOutData("dealerName", logonUser.getName());
			//act.setOutData("myWeek", myWeek);
	...
	public void urgentOrderReoprtAdd() {
		AclUserBean logonUser = (AclUserBean) act.getSession().get(Constant.LOGON_USER);
		try {
			...
			String totalPrice = CommonUtils.checkNull(request.getParamValue("payAmount")); //返利后订单总价
			...
			String isRebate=CommonUtils.checkNull(request.getParamValue("isRebate")) ; //是否使用返利
			String rebateAmount=CommonUtils.checkNull(request.getParamValue("rebateAmount")) ; //返利额度
			if("".equals(rebateAmount)||rebateAmount==null){
				rebateAmount="0";
			}
			...
			String isCustomAddr=CommonUtils.checkNull(request.getParamValue("isCustomAddr")); //是否使用自定义收货地址
			String provinceId=CommonUtils.checkNull(request.getParamValue("province")); //自定义地址省份
			//String province=CommonUtils.checkNull(request.getParamValue("province")); //自定义地址省份
			String cityId=CommonUtils.checkNull(request.getParamValue("city")); //自定义地址城市
			String townId=CommonUtils.checkNull(request.getParamValue("town")); //自定义地址区县
			String customAddr=CommonUtils.checkNull(request.getParamValue("customAddr"));  //自定义详细地址
			String customLinkMan=CommonUtils.checkNull(request.getParamValue("customLinkMan")); //自定义联系人
			String customTel=CommonUtils.checkNull(request.getParamValue("customTel"));  //自定义联系电话
			String typeId=fundType.split("\\|")[0];
			// 获得是否需要资金检查
			...
			if (returnValue.equals("1")) {
				...
				po.setCompanyId(new Long(logonUser.getOemCompanyId()));
...
//					orderNO = GetOrderNOUtil.getOrderNO(areaCode,Constant.ORDER_TYPE_02+"", "D", dealerCode);
					...
					po.setIsRebate(new Integer(isRebate)); //是否返利
					po.setRebateAmount(new Double(rebateAmount));//返利额度
				}
				else{					
...
//					if (po.getDeliveryType().intValue() == Constant.TRANSPORT_TYPE_02.intValue()) {
						...
						//如果使用自定义地址则联系人和联系电话对应为自定义联系人和联系电话
						po.setCustomLinkMan(customLinkMan);
						po.setCustomTel(customTel);
...
//					} 
				...
				po.setIsCustomAddr(new Integer(isCustomAddr));
				po.setWarehouseId(new Long(warehouseId));
				if(new Integer(isCustomAddr).intValue()==Constant.IF_TYPE_YES.intValue()){
					...
					if(!"".equals(townId)&&townId!=null){
						po.setTownId(new Long(townId));
					}
				}
				po.setCustomAddr(customAddr);
				...
				if (submitType.equals("1")) {
					po.setOrderStatus(Constant.ORDER_COM_STATUS_01);
				...
				// 提报时
					...
					if (dealerPO.getDealerLevel().intValue() == Constant.DEALER_LEVEL_01.intValue()) {
						po.setOrderStatus(Constant.ORDER_COM_STATUS_03);// 已提报
						...
						//if (isCheck.equals("0") && !"1".equals(isBingcai)){
							...
							if(!accountId.equals("")){
								...
								Map<String, Object> map=CommonUtils.getAvailableAmount(typeId, dealerId);
								BigDecimal outstandingAccount = dao.getOutstandingAccount(dealerId,typeId);//冻结资金
						...
						po.setOrderStatus(Constant.ORDER_COM_STATUS_02);// 预审核
						dao.insertSalesOrder(po);
					}
					//向发运申请操作日志表写入日志信息
				...
				while (enumeration.hasMoreElements()) {
					...
					if (temp.length() > 6 && temp.substring(0, 6).equals("amount")) {
						...
						String lineRebateAmount = request.getParamValue("lineRebateAmountHid"+ subStr);//返利金额
						String lineRebateTotalPrice = request.getParamValue("lineRebateTotalPriceHid"+ subStr);//返利后金额
						...
						detailPO.setRebateAmount(new Double(lineRebateAmount));
						detailPO.setAfterRebatePrice(new Double(lineRebateTotalPrice));
	...
	public void urgentOrderReoprtModPre() {
		AclUserBean logonUser = (AclUserBean) act.getSession().get(
				Constant.LOGON_USER);
		try {
			 ActionContextExtend atx = (ActionContextExtend) ActionContext.getContext();
			...
			List<Map<String, Object>> dateList = dao.getUrgentDateList(oemCompanyId);
				act.setOutData("dateList", dateList);
			Map<String,Object> mapArea=dao.getDealerAreas(logonUser.getDealerId());	
			Long poseId = logonUser.getPoseId();
			List<Map<String, Object>> areaList = MaterialGroupManagerDao
					.getDealerBusinessNoJszx(poseId.toString());
...
//			if (po.getDeliveryType().intValue() == Constant.TRANSPORT_TYPE_03) {
	...
	public void urgentOrderReoprtMod() {
		...
		try {
			...
			if(!(tvo.getOrderStatus().intValue()==Constant.ORDER_COM_STATUS_01||tvo.getOrderStatus().intValue()==Constant.ORDER_COM_STATUS_04)){
				act.setOutData("returnValue", 2);
				return;
			}
			...
			if (returnValue.equals("1")) {
				...
				value.setIsRebate(new Integer(isRebate)); //是否返利
				value.setRebateAmount(new Double(rebateAmount));//返利额度
				if(!CommonUtils.isNullString(productId)) {
				...
				if("1".equals(isCover)){	
					...
					if(new Integer(isCustomAddr).intValue()==Constant.IF_TYPE_YES.intValue()){
						if(!"".equals(provinceId)&&provinceId!=null){
							value.setProvinceId(new Long(provinceId));
						}
						if(!"".equals(cityId)&&cityId!=null){
							value.setCityId(new Long(cityId));
						}
						if(!"".equals(townId)&&townId!=null){
							value.setTownId(new Long(townId));
						}
						value.setCustomAddr(customAddr);
						//如果使用自定义地址则联系人和联系电话对应为自定义联系人和联系电话
						value.setCustomLinkMan(customLinkMan);
						value.setCustomTel(customTel);
					}else{
						receiver = OrderReportDao.contralReceiver(areaId, receiver, orderNO) ;		// 对收货方做控制
				...
				value.setIsCustomAddr(new Integer(isCustomAddr));
				value.setWarehouseId(Long.valueOf(warehouseId));
				...
				// 保存时
				if (submitType.equals("1")) {
					value.setOrderStatus(Constant.ORDER_COM_STATUS_01);
				}
				// 提报时
				else {
					...
					if (dealerPO.getDealerLevel().intValue() == Constant.DEALER_LEVEL_01
							.intValue()) {
						value.setOrderStatus(Constant.ORDER_COM_STATUS_03);// 已提报
						//String dlvryReqNO = GetOrderNOUtil.getDlvryReqNO(orderNO);
						
						if(tvo.getOrderStatus().toString().equals(Constant.ORDER_COM_STATUS_04.toString())){
							//驳回
							TtVsDlvryReqPO po=new TtVsDlvryReqPO();
							po.setOrderId(Long.valueOf(orderId));
							List list=dao.select(po);
							po=(TtVsDlvryReqPO) list.get(0);
							TtVsDlvryReqDtlPO reqDetailPO=new TtVsDlvryReqDtlPO();
							reqDetailPO.setReqId(po.getReqId());
							dao.delete(reqDetailPO);
							
							/*TtVsDlvryReqPO ttvs=new TtVsDlvryReqPO();
							ttvs.setOrderId(Long.valueOf(orderId));
							dao.delete(ttvs);*/
							dao.updateTtVsDlvryReq(value, new Integer(totalAmount_));
						}else{
						...
						value.setOrderStatus(Constant.ORDER_COM_STATUS_02);// 预审核
					}
				}
	...
	//					String discount_price = request.getParamValue("discount_price"+ subStr);	//折扣额
						String lineRebateAmount = request.getParamValue("lineRebateAmountHid"+ subStr);//返利金额
						String lineRebateTotalPrice = request.getParamValue("lineRebateTotalPriceHid"+ subStr);//返利金额
...
//						}
						...
						if (submitType.equals("2")) {
							if (dealerPO.getDealerLevel().intValue() == Constant.DEALER_LEVEL_01.intValue()) {
								if(tvo.getOrderStatus().intValue()==Constant.ORDER_COM_STATUS_04){ //驳回重新查询出对应的tt_vs_dlvry_req 的req_id
									TtVsDlvryReqPO po=new TtVsDlvryReqPO();
									po.setOrderId(Long.valueOf(orderId));
									List list=dao.select(po);
									po=(TtVsDlvryReqPO) list.get(0);
	...
	public void urgentOrderReoprtDel() {
		AclUserBean logonUser = (AclUserBean) act.getSession().get(
				Constant.LOGON_USER);
		try {
			...
			tvo.setOrderId(new Long(orderId));
			tvo=(TtVsOrderPO) dao.select(tvo).get(0);
			if(tvo.getOrderStatus().intValue()==Constant.ORDER_COM_STATUS_03||
					tvo.getOrderStatus().intValue()==Constant.ORDER_COM_STATUS_06){
				act.setOutData("returnValue",2);
				return;
			}
			...
			orderValue.setOrderStatus(Constant.ORDER_COM_STATUS_06);
			...
			reqValue.setReqStatus(Constant.ORDER_REQ_STATUS_DEL);
	 ...
	 *
	 */
	public void urgentOrderReoprtSubmit() {
		AclUserBean logonUser = (AclUserBean) act.getSession().get(Constant.LOGON_USER);
		try {
			Long oemCompanyId = GetOemcompanyId.getOemCompanyId(logonUser);
			String orderId = CommonUtils.checkNull(request.getParamValue("orderId"));
			...
			if (isPass) {
				...
				if (dealerPO.getDealerLevel().intValue() == Constant.DEALER_LEVEL_01.intValue()) {
					...
					if (null != orderList && orderList.size()>0) {
						...
						if(orderPO.getOrderStatus().toString().equals(Constant.ORDER_COM_STATUS_04.toString())){
							//驳回
							TtVsDlvryReqPO po=new TtVsDlvryReqPO();
							po.setOrderId(Long.valueOf(orderId));
							List list=dao.select(po);
						...
						if(ttVsOrderPO.getOrderStatus().toString().equals(Constant.ORDER_COM_STATUS_04)){
							dao.updateTtVsDlvryReq(ttVsOrderPO, new Integer(amount)); //订单驳回之后提交，执行update方法
						}else{
							dao.insertTtVsDlvryReq(ttVsOrderPO, req_id, new Integer(amount)); //正常提交
						}
						//dao.insertTtVsDlvryReq(ttVsOrderPO, req_id, new Integer(amount)); //正常提交
						// if (isCheck.equals("0")){
							// 获得资金账户
							...
							if(accountPO != null){
								...
								Map<String, Object> map=CommonUtils.getAvailableAmount(orderPO.getFundTypeId().toString(), dealerId.toString());
								BigDecimal outstandingAccount = dao.getOutstandingAccount(dealerId.toString(),orderPO.getFundTypeId().toString());//冻结资金
								act.setOutData("returnValue", map != null ? ((BigDecimal) map.get("AVAILABLE_AMOUNT")).subtract(outstandingAccount):"0");//可用余额
							}
							// 获得折扣账户
...
//			}
	...
	public void showFund() {
		...
		try {
			...
			List<Map<String,Object>> fundTypeList = accoutDao.getNoDiscountAccountInfoByDealerId(dealerId);
...
//			}
			...
			Map<String, Object> map = dao.getMaterialInfo(materialCode,warehouseId, priceId, companyId.toString(), entityCode,dealerCode);
			String para = CommonDAO.getPara(Constant.CHANA_SYS.toString()) ;
			int isFlag;
			String isReturn="";
	 ...
	 * 获取仓库的可用资源
	 */
	public void getWarehouseResourse(){
		AclUserBean logonUser = (AclUserBean) act.getSession().get(
				Constant.LOGON_USER);
		try {
			Map<String, Object> map = new HashMap<String, Object>();
			...
			List<Map<String, Object>> list = dao.getWarehouseResourseList(map);
	...
	public void getRebateAmount(){
		...
		try {
			...
			String dealerId=logonUser.getDealerId();
			if(dealerId==null||"".equals(dealerId)){
				dealerId=request.getParamValue("dealerId");
			}
			//获取经销商账户返利余额
			//List<TtVsAccountRebatePO> list =dao.getRebateAccount(dealerId);
			BigDecimal rebateAmount = dao.getRebateAccount(dealerId);//冻结资金
			//act.setOutData("rebateList", list);
			act.setOutData("rebateAmount", rebateAmount);
			Double rebateDiscount=dao.getRebateDiscount();
			if(rebateDiscount>0){
				rebateDiscount=rebateDiscount/100;
			}
			act.setOutData("rebateDiscount", rebateDiscount);
	...
	public void getWarehouse(){
		...
		try {
			//String dealerId=logonUser.getDealerId().toString();
			//获取仓库
			List<Map<String,Object>> list =dao.getWarehouseList();
			act.setOutData("warehouseList", list);
