/*
* Copyright (c) 2005 Infoservice, Inc. All  Rights Reserved.
* This software is published under the terms of the Infoservice Software
* License version 1.0, a copy of which has been included with this
* distribution in the LICENSE.txt file.
*
* CreateDate : 2014-08-20 20:04:10
* CreateBy   : wizard_lee
* Comment    : generate by com.sgm.po.POGen
*/

package com.infodms.dms.po;

import java.util.Date;
import com.infoservice.po3.bean.PO;

@SuppressWarnings("serial")
public class TtAsWarrantyRecordReturnPO extends PO{

	private Integer partWrType;
	private String ctmName;
	private Long dealerId;
	private Long warId;
	private Long vrId;
	private String vin;
	private Date updateDate;
	private Long createBy;
	private Integer warCountDays;
	private Long updateBy;
	private String licenseNo;
	private Long warRoUserId;
	private String curWrStandard;
	private String warNo;
	private Integer warCountTimes;
	private Long warPartId;
	private String mainPhone;
	private Integer curWrTimes;
	private Double warMileage;
	private Integer warLevel;
	private String roNo;
	private Integer warRoType;
	private Integer warType;
	private Integer curLawDays;
	private Date createDate;
	private Date warDate;
	private Integer curWrDays;
	private Integer curLawTimes;

	public void setPartWrType(Integer partWrType){
		this.partWrType=partWrType;
	}

	public Integer getPartWrType(){
		return this.partWrType;
	}

	public void setCtmName(String ctmName){
		this.ctmName=ctmName;
	}

	public String getCtmName(){
		return this.ctmName;
	}

	public void setDealerId(Long dealerId){
		this.dealerId=dealerId;
	}

	public Long getDealerId(){
		return this.dealerId;
	}

	public void setWarId(Long warId){
		this.warId=warId;
	}

	public Long getWarId(){
		return this.warId;
	}

	public void setVrId(Long vrId){
		this.vrId=vrId;
	}

	public Long getVrId(){
		return this.vrId;
	}

	public void setVin(String vin){
		this.vin=vin;
	}

	public String getVin(){
		return this.vin;
	}

	public void setUpdateDate(Date updateDate){
		this.updateDate=updateDate;
	}

	public Date getUpdateDate(){
		return this.updateDate;
	}

	public void setCreateBy(Long createBy){
		this.createBy=createBy;
	}

	public Long getCreateBy(){
		return this.createBy;
	}

	public void setWarCountDays(Integer warCountDays){
		this.warCountDays=warCountDays;
	}

	public Integer getWarCountDays(){
		return this.warCountDays;
	}

	public void setUpdateBy(Long updateBy){
		this.updateBy=updateBy;
	}

	public Long getUpdateBy(){
		return this.updateBy;
	}

	public void setLicenseNo(String licenseNo){
		this.licenseNo=licenseNo;
	}

	public String getLicenseNo(){
		return this.licenseNo;
	}

	public void setWarRoUserId(Long warRoUserId){
		this.warRoUserId=warRoUserId;
	}

	public Long getWarRoUserId(){
		return this.warRoUserId;
	}

	public void setCurWrStandard(String curWrStandard){
		this.curWrStandard=curWrStandard;
	}

	public String getCurWrStandard(){
		return this.curWrStandard;
	}

	public void setWarNo(String warNo){
		this.warNo=warNo;
	}

	public String getWarNo(){
		return this.warNo;
	}

	public void setWarCountTimes(Integer warCountTimes){
		this.warCountTimes=warCountTimes;
	}

	public Integer getWarCountTimes(){
		return this.warCountTimes;
	}

	public void setWarPartId(Long warPartId){
		this.warPartId=warPartId;
	}

	public Long getWarPartId(){
		return this.warPartId;
	}

	public void setMainPhone(String mainPhone){
		this.mainPhone=mainPhone;
	}

	public String getMainPhone(){
		return this.mainPhone;
	}

	public void setCurWrTimes(Integer curWrTimes){
		this.curWrTimes=curWrTimes;
	}

	public Integer getCurWrTimes(){
		return this.curWrTimes;
	}

	public void setWarMileage(Double warMileage){
		this.warMileage=warMileage;
	}

	public Double getWarMileage(){
		return this.warMileage;
	}

	public void setWarLevel(Integer warLevel){
		this.warLevel=warLevel;
	}

	public Integer getWarLevel(){
		return this.warLevel;
	}

	public void setRoNo(String roNo){
		this.roNo=roNo;
	}

	public String getRoNo(){
		return this.roNo;
	}

	public void setWarRoType(Integer warRoType){
		this.warRoType=warRoType;
	}

	public Integer getWarRoType(){
		return this.warRoType;
	}

	public void setWarType(Integer warType){
		this.warType=warType;
	}

	public Integer getWarType(){
		return this.warType;
	}

	public void setCurLawDays(Integer curLawDays){
		this.curLawDays=curLawDays;
	}

	public Integer getCurLawDays(){
		return this.curLawDays;
	}

	public void setCreateDate(Date createDate){
		this.createDate=createDate;
	}

	public Date getCreateDate(){
		return this.createDate;
	}

	public void setWarDate(Date warDate){
		this.warDate=warDate;
	}

	public Date getWarDate(){
		return this.warDate;
	}

	public void setCurWrDays(Integer curWrDays){
		this.curWrDays=curWrDays;
	}

	public Integer getCurWrDays(){
		return this.curWrDays;
	}

	public void setCurLawTimes(Integer curLawTimes){
		this.curLawTimes=curLawTimes;
	}

	public Integer getCurLawTimes(){
		return this.curLawTimes;
	}

}