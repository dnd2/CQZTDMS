...
public class PartDlrOrderDao extends BaseDao {
    public Logger logger = Logger.getLogger(PartDlrOrderDao.class);
    private static final PartDlrOrderDao dao = new PartDlrOrderDao();

    private PartDlrOrderDao() {
    }

    public static final PartDlrOrderDao getInstance() {
        return dao;
    }
     ...
     */
    public PageResult<Map<String, Object>> getSalesRelation(String dealerId, String type, String name, String code, String fixValue, int pageSize, int curPage) {
        StringBuffer sql = new StringBuffer();
        ActionContext act = ActionContext.getContext();
        AclUserBean loginUser = (AclUserBean) act.getSession().get(Constant.LOGON_USER);
        ...
        //3为地址
        if ("1".equals(type)) {
            sql.append(" SELECT R.PARENTORG_ID,R.PARENTORG_NAME,R.PARENTORG_CODE,");
            sql.append(" nvl(b.ACCOUNT_ID,'') AS ACCOUNT_ID ,");
            sql.append(" to_char(nvl(ACCOUNT_SUM,'0.00'),'FM999,999,999,999,990.00') AS ACCOUNT_SUM,to_char(abs(nvl(ACCOUNT_DJ,'0.00')),'FM999,999,999,999,990.00') AS ACCOUNT_DJ,to_char(nvl(ACCOUNT_KY,'0.00'),'FM999,999,999,999,990.00') AS ACCOUNT_KY FROM tt_part_sales_relation r ");
            sql.append(" left join VW_PART_DLR_ACCOUNT b on b.DEALER_ID='").append(dealerId).append("' and r.PARENTORG_ID=b.PARENTORG_ID AND r.childorg_id=b.DEALER_ID");
            sql.append(" WHERE r.childorg_id='").append(dealerId).append("' AND r.state='").append(Constant.STATUS_ENABLE).append("' AND r.status='1' ");
            if (!"".equals(name)) {
                sql.append(" and R.PARENTORG_NAME like '%").append(name).append("%'");
            }
            if (!"".equals(code)) {
                sql.append(" and  upper(R.PARENTORG_CODE) like upper('%").append(code).append("%')");
            }
            if ("".equals(fixValue) || null == fixValue) {
                sql.append("AND B.ACCOUNT_PURPOSE = " + Constant.PART_ACCOUNT_PURPOSE_TYPE_01);
            } else {
                sql.append("AND B.ACCOUNT_PURPOSE = " + fixValue);
            }
            ...
            sql.append(" select ");
            sql.append(" nvl(a.ADDR_ID,'') as ADDR_ID,nvl(a.ADDR,'无') as ADDR,nvl(a.LINKMAN,'配件负责人') as LINKMAN,nvl(TEL,'无') as TEL,nvl(a.POST_CODE,'无') as POST_CODE,nvl(a.STATION,'无') as STATION ");
            ...
            if (!"".equals(name)) {
                sql.append(" and a.ADDR like '%").append(name).append("%'");
            }
        } else if ("4".equals(type) || "5".equals(type) || "6".equals(type)) {
            sql.append(" SELECT Distinct R.CHILDORG_ID, R.CHILDORG_NAME,R.CHILDORG_CODE FROM TT_PART_SALES_RELATION R");
            sql.append(" WHERE R.Parentorg_Id ='").append(dealerId).append("' ");
            sql.append(" AND R.state='").append(Constant.STATUS_ENABLE).append("' AND R.status='1' ");
            if ("6".equals(type)) {
                sql.append(" AND EXISTS (SELECT 1\n");
                sql.append("          FROM TM_DEALER TD\n");
                sql.append("         WHERE TD.DEALER_ID = R.CHILDORG_ID\n");
                sql.append("           AND TD.IS_NBDW = 1)\n");
            }
            if (!"".equals(name)) {
                sql.append(" and R.CHILDORG_NAME like '%").append(name).append("%'");
            }
            if (!"".equals(code)) {
                sql.append(" and R.CHILDORG_CODE like '%").append(code).append("%'");
            }
            sql.append(" and R.CHILDORG_ID not in ('").append(dealerId).append("')");
            ...
            sql.append("  FROM (SELECT R.PARENTORG_ID,\n");
            ...
            sql.append("               TO_CHAR(ABS(NVL(ACCOUNT_DJ, '0.00')),\n");
            sql.append("                       'FM999,999,999,999,990.00') AS ACCOUNT_DJ,\n");
            sql.append("               TO_CHAR(NVL(ACCOUNT_KY, '0.00'), 'FM999,999,999,999,990.00') AS ACCOUNT_KY\n");
            sql.append("          FROM TT_PART_SALES_RELATION R\n");
            sql.append("          LEFT JOIN VW_PART_DLR_ACCOUNT B\n");
            sql.append("            ON B.DEALER_ID = '"+dealerId+"'\n");
            sql.append("           AND R.PARENTORG_ID = B.PARENTORG_ID\n");
            sql.append("           AND R.CHILDORG_ID = B.DEALER_ID\n");
            sql.append("         WHERE R.CHILDORG_ID = '"+dealerId+"'\n");
            sql.append("           AND R.STATE = '10011001'\n");
            sql.append("           AND R.STATUS = '1'\n");
            sql.append("           AND B.ACCOUNT_PURPOSE = 95631001) A,\n");
            sql.append("\n");
            sql.append("       (SELECT NVL(A.ADDR_ID, '') AS ADDR_ID,\n");
            sql.append("               NVL(A.ADDR, '无') AS ADDR,\n");
            sql.append("               NVL(A.LINKMAN, '配件负责人') AS LINKMAN,\n");
            sql.append("               NVL(TEL, '无') AS TEL,\n");
            sql.append("               NVL(A.POST_CODE, '无') AS POST_CODE,\n");
            sql.append("               NVL(A.STATION, '无') AS STATION\n");
            sql.append("          FROM TT_PART_ADDR_DEFINE A\n");
            sql.append("         WHERE 1 = 1\n");
            sql.append("           AND STATE = '10011001'\n");
            sql.append("           AND STATUS = '1'\n");
            sql.append("           AND DEALER_ID = '"+dealerId+"') B\n");
            sql.append(" ORDER BY B.ADDR_ID\n");
        } else {
            return null;
        }
    ...
    public PageResult<Map<String, Object>> showSellerPartBase(RequestWrapper req, String sellerId, int curPage, int pageSize) {
        ...
        //分网SQL紧急、常规、特殊
        if (orderType.equals(Constant.CAR_FACTORY_SALES_MANAGER_ORDER_TYPE_01 + "")
                || orderType.equals(Constant.CAR_FACTORY_SALES_MANAGER_ORDER_TYPE_02 + "")
                || orderType.equals(Constant.CAR_FACTORY_SALES_MANAGER_ORDER_TYPE_03 + "")) {
            if (null == logonUser.getDealerId()) {
                sql.append(CommonUtils.bindPartSQL(req.getAttribute("bookDealerId") + "", "a"));
            }
        }
        //add end
...
//        String partCode=CommonUtils.checkNull(req.getParamValue("PART_CODE"));
        //销售单位id（经销商id）
        String bookDealerId = req.getAttribute("bookDealerId")+"";
        String orderType=CommonUtils.checkNull(req.getParamValue("ORDER_TYPE"));
...
//        String produceFac = CommonUtils.checkNull(req.getParamValue("produceFac"));
        ...
        sql.append("       A.REMARK,\n");
        sql.append("       NVL(A.UNIT, '件') UNIT,\n");
        sql.append("       NVL( (SELECT SUM(S.normal_QTY) FROM VW_PART_STOCK S\n");
        sql.append("            WHERE S.PART_ID = A.PART_ID AND S.ORG_ID = '").append(bookDealerId).append("'");
        sql.append("              AND S.STATE = 10011001 AND S.STATUS = 1),\n");
        sql.append("       0) AS ITEM_QTY,\n");
        //主机厂按照最小包装量量销售
        if (sellerId.equals(Constant.OEM_ACTIVITIES)) { 
            sql.append("       NVL(DECODE((SELECT D.PDEALER_TYPE FROM TM_DEALER D");
            sql.append("                   WHERE D.DEALER_ID = ").append(bookDealerId);
            sql.append("                     AND D.STATUS = 10011001), 92101001, A.MIN_PACK1, A.MIN_PACK2),");
            sql.append("       0) AS MIN_PACKAGE,\n");
        } else {
            sql.append("nvl(DECODE(A.Part_Material,92031006,a.min_pack1,1),1) MIN_PACKAGE,\n");//供应中心油品整箱，其他1
        }
        //销售价格
//        sql.append("       TO_CHAR(PKG_PART.F_GETPRICE(").append(CommonUtils.checkNull(bookDealerId)).append(", A.PART_ID),'FM999,999,990.00') AS SALE_PRICE1,");
         sql.append("       TO_CHAR(F_GET_SALE_PRICE(a.part_id),'FM999,999,990.00') AS SALE_PRICE1,");
        
        
        sql.append("       (SELECT CASE WHEN SUM(normal_QTY) > 0 THEN 'Y' ");
        sql.append("               WHEN SUM(normal_QTY) = 0 THEN 'N' ");
        sql.append("               ELSE 'N' END ");
        sql.append("          FROM VW_PART_STOCK E\n");
        sql.append("         WHERE E.PART_ID = A.PART_ID AND E.STATE = 10011001 AND E.STATUS = 1 ");
        sql.append("           AND E.ORG_ID = ").append(sellerId).append(") AS UPORGSTOCK,\n");
        sql.append("  nvl(a.is_special,'10041002') as is_special ");
        sql.append("  FROM TT_PART_DEFINE A\n");
        sql.append(" WHERE A.PART_CATEGORY=95711001 AND A.STATUS = 1 and A.is_gxp = " + Constant.IF_TYPE_NO);//系统没有广宣品，只查询10041002
        sql.append(" AND A.STATE= ").append(Constant.STATUS_ENABLE) ;//配件状态
        //只查询价格大于0的配件
        sql.append(" AND F_GET_SALE_PRICE(a.part_id) >0");
        
        //配件编码
        if (!"".equals(partOldCode)) {
            sql.append(" and upper(a.PART_OLDCODE) like '%").append(partOldCode.toUpperCase()).append("%'");
        }
        //配件名称
        if (!"".equals(partCname)) {
            sql.append(" and upper(a.PART_CNAME) like '%").append(partCname.toUpperCase()).append("%'");
        }
...
//            orderType.equals(Constant.CAR_FACTORY_SALES_MANAGER_ORDER_TYPE_03 + "")) {
//            if (null != logonUser.getDealerId()) {
//                sql.append(CommonUtils.bindPartSQL(bookDealerId, "a"));
//            }
//        }
     ...
     */
    //RequestWrapper中需要放入获取金额的dealer_id  销售单和提报的dealer_id不一样
    public PageResult<Map<String, Object>> showDiscountPartBase(RequestWrapper req,
        ...
        sql.append("       A.STATE,\n");
        sql.append("       NVL(A.UNIT, '件') UNIT,"); 
        sql.append(" NVL((SELECT SUM(S.ITEM_QTY) FROM TT_PART_ITEM_STOCK S\n");
        sql.append("      WHERE S.PART_ID = A.PART_ID AND S.ORG_ID = '").append(req.getAttribute("bookDealerId")).append("'");
        sql.append("      AND S.STATE = 1 AND S.STATUS = 1), 0) AS ITEM_QTY,\n");
        sql.append(" NVL(DECODE((SELECT D.PDEALER_TYPE FROM TM_DEALER D\n");
        sql.append("             WHERE D.DEALER_ID = ").append(req.getAttribute("bookDealerId"));
        sql.append("             AND D.STATUS = 10011001), 92101001, A.MIN_PACK1, A.MIN_PACK2),\n");
        sql.append("     0) AS MIN_PACKAGE,\n");
//        sql.append("       TO_CHAR(PKG_PART.F_GETPRICE(").append(CommonUtils.checkNull(req.getAttribute("bookDealerId"))).append(", A.PART_ID)*B.RATE,'FM999,999,990.00') AS SALE_PRICE1,");
        sql.append(" TO_CHAR(F_GET_DISCONUNT_PRICE(a.part_id),'FM999,999,990.00') AS SALE_PRICE1,");
        sql.append(" (SELECT  CASE WHEN SUM(ITEM_QTY) > 0 THEN 'Y'\n");
        sql.append("               WHEN SUM(ITEM_QTY) = 0 THEN 'N'\n");
        sql.append("               ELSE 'N' END FROM TT_PART_ITEM_STOCK E\n");
        sql.append("       WHERE E.PART_ID = A.PART_ID AND E.STATE = 1 AND E.STATUS = 1\n");
...
//        sql.append(" ,nvl((select qty from vw_part_dlr_thm_per_sale vpdtps where vpdtps.part_id=A.part_id and org_id='").append(CommonUtils.checkNull(req.getAttribute("bookDealerId"))).append("'),'0') as qty");
        sql.append("  FROM TT_PART_DEFINE A\n");
        sql.append(" WHERE A.PART_CATEGORY=95711001 AND A.STATUS = 1  ");
        sql.append(" and a.STATE= ").append(Constant.STATUS_ENABLE) ;//配件状态
        sql.append("   AND A.IS_DIRECT = " + Constant.IF_TYPE_NO + "\n");
        sql.append("   AND A.IS_SPECIAL = " + Constant.IF_TYPE_NO + "\n");
        sql.append("   AND A.IS_GXP = " + Constant.IF_TYPE_NO + "\n");//系统没有广宣品，只查询10041002
...
//        sql.append("  AND A.STATUS = 1\n");
        sql.append("  AND F_GET_DISCONUNT_PRICE(a.part_id) >0 ");
    ...
    public Map<String, Object> getVender(String partId) {
        StringBuffer sql = new StringBuffer();
        sql.append(" select COUNT(1) TOTAL from ( ");
        sql.append(" 	select VENDER_ID from tt_part_vender ");
        sql.append(" 	where state=10041001 and status=1 and part_id in (").append(partId).append(" ) ");
        sql.append(" 	group by VENDER_ID "); 
        sql.append(" ) TT "); 
        List<Map<String, Object>> list = this.pageQuery(sql.toString(), null, this.getFunName());
        if (list.size() <= 0) {
            return null;
    ...
    public Map<String, Object> getAccount(String dealerId, String parentId, String accountId) {
        StringBuffer sql = new StringBuffer();
        sql.append(" select ACCOUNT_ID, ACCOUNT_KIND, CHILDORG_ID, DEALER_ID, DEALER_CODE, DEALER_NAME, ");
        sql.append(" PARENTORG_ID, PARENTORG_CODE, PARENTORG_NAME,  ");
        sql.append(" to_char(ACCOUNT_SUM,'FM999,999,999,999,990.00') as ACCOUNT_SUM,");//现金余额
        sql.append(" to_char(ACCOUNT_DJ,'FM999,999,999,999,990.00') as ACCOUNT_DJ,");//--预扣金额
        sql.append(" to_char(ACCOUNT_KY,'FM999,999,999,999,990.00') as ACCOUNT_KY, ");//--总可用余额
        sql.append(" ACCOUNT_INVO");//开票金额
...
//        sql.append("  FROM VW_PART_STOCK T\n");
        sql.append("  FROM TT_PART_BOOK T\n");
        sql.append(" WHERE 1 = 1\n");
...
//        sql.append("   AND T.STATE = 10011001\n");
    ...
    public Map<String, Object> queryPartDlrOrderMain(String orderId) {
        ...
        sql.append("       nvl(ADDR.POST_CODE,'无') POST_CODE,\n");
        sql.append("       nvl(ADDR.STATION,'无') STATION,\n");
    ...
    public List<Map<String, Object>> queryPartDlrOrderDetail(String orderId) {
        ...
        sql.append("       T.BUY_QTY - NVL(DO.BUY_QTY, 0) CLOSE_QTY,\n");
    ...
    public List<Map<String, Object>> queryPartDlrOrderDetail4SODTL(String orderId) {
        ...
        sql.append("       T.BUY_QTY,\n");
        sql.append("       T.SALES_QTY,\n");
        sql.append("       T.BUY_PRICE, --取提报价格\n");
        sql.append("       T.BUY_AMOUNT, --取提报金额\n");
        sql.append("       /*TO_CHAR(PKG_PART.F_GETPRICE(M.DEALER_ID, T.PART_ID),\n");
        sql.append("       'FM999,999,999,999,990.00') AS BUY_PRICE, --取最新价格*/\n");
        sql.append("       /*TO_CHAR(PKG_PART.F_GETPRICE(M.DEALER_ID, T.PART_ID) * T.BUY_QTY,\n");
        sql.append("       'FM999,999,999,999,990.00') AS BUY_AMOUNT,*/ --取最新金额\n");
        sql.append("       T.IS_HAVA,\n");
        sql.append("       T.REMARK,\n");
        sql.append("       T.CREATE_DATE,\n");
        sql.append("       T.CREATE_BY,\n");
    ...
    public List<Map<String, Object>> queryPartDlrOrderDetail4SODTL(String orderId, String whId) {
        ...
        sql.append("SELECT T.*,\n");
        sql.append("       CASE\n");
        sql.append("         WHEN T.ORDER_TYPE = 92151004 THEN T.BUY_QTY\n");//委托订单
        sql.append("         WHEN T.ORDER_TYPE = 92151012 THEN T.BUY_QTY\n");//销售采购订单  20170816 add
        sql.append("         WHEN T.ORDER_TYPE != 92151004 THEN\n");
        sql.append("          DECODE(SIGN(T.STOCK_QTY - T.BUY_QTY), 1, T.BUY_QTY,\n");
        ...
        sql.append("             FROM VW_PART_STOCK S\n");
        sql.append("            WHERE S.PART_ID = T.PART_ID\n");
        sql.append("              AND S.ORG_ID = ").append(orgId);
        sql.append("              AND S.WH_ID =").append(whId);
        sql.append("              AND S.STATE = 10011001\n");
        sql.append("              AND S.STATUS = 1),\n");
        sql.append("           0) AS STOCK_QTY,\n");
    ...
    public List<Map<String, Object>> getAddr(String dealerId) {
        StringBuffer sql = new StringBuffer();
        sql.append(" select  nvl(a.ADDR_ID,'') as ADDR_ID,nvl(a.ADDR,'') as ADDR,nvl(a.LINKMAN,'') as LINKMAN,nvl(TEL,'') as TEL,nvl(a.LINKMAN2,'') as LINKMAN2,nvl(TEL2,'') as TEL2,nvl(a.POST_CODE,'') as POST_CODE,nvl(a.STATION,'') as STATION  from TT_PART_ADDR_DEFINE a where state=10011001 and dealer_id='").append(dealerId).append("'");
        return this.pageQuery(sql.toString(), null, this.getFunName());
    }
    ...
    public List<Map<String, Object>> getTmpData(String code, String orderType) {
        ...
        if (orderType.equals(Constant.CAR_FACTORY_SALES_MANAGER_ORDER_TYPE_01 + "")
            ...
            if (null != logonUser.getDealerId()) {
                ...
                sql.append(CommonUtils.bindPartSQL(request.getParamValue("dealerId") + "", "D"));
            }
        }
...
//            sql.append("                dt.pkg_no,\n");
     ...
     * @param checkFlag 验证类型 1:分网 2:配件状态
     * @return
     */
    public List<Map<String, Object>> dlrOrderDtlCheck(String orderType, String orderId, String dealerId, String checkFlag) {
        StringBuffer sql = new StringBuffer();
        sql.append("SELECT OD.PART_OLDCODE\n");
        sql.append("  FROM TT_PART_DLR_ORDER_DTL OD, TT_PART_DLR_ORDER_MAIN OM\n");
        sql.append(" WHERE OD.ORDER_ID = OM.ORDER_ID\n");
        sql.append("   AND OM.ORDER_ID = '" + orderId + "'\n");
        ...
        if ("1".equals(checkFlag)) {
            if (orderType.equals(Constant.CAR_FACTORY_SALES_MANAGER_ORDER_TYPE_01 + "")
                ...
                if ("1".equals(CommonDAO.getPara(60041002 + ""))) {
                	//
                    sql.append(this.bindPartSQL(dealerId, "OD"));
                } else {
                    sql.append(" AND NOT EXISTS (SELECT 1\n");
                    sql.append("         FROM TT_PART_DEFINE D\n");
                    sql.append("        WHERE D.PART_ID != OD.PART_ID)\n");
                }
            } else {
                sql.append(" AND NOT EXISTS (SELECT 1\n");
                sql.append("         FROM TT_PART_DEFINE D\n");
     ...
     * @param orderId
     */
    public void rebutOrder(String orderId) {
        StringBuffer sql = new StringBuffer();
        sql.append("UPDATE TT_PART_DLR_ORDER_MAIN M\n");
        sql.append("   SET M.SUBMIT_DATE = NULL, M.SUBMIT_BY = NULL\n");
        sql.append(" WHERE M.ORDER_ID ='" + orderId + "'\n");
        this.update(sql.toString(), null);
    }
     ...
     * @param orderId 订单ID
     */
    public void updateOrderDtlPrice(String orderId) {
        StringBuffer sql = new StringBuffer();
        sql.append("UPDATE TT_PART_DLR_ORDER_DTL D\n");
        sql.append("   SET D.BUY_PRICE =\n");
        sql.append("       (SELECT nvl(SP.SALE_PRICE1,0) FROM TT_PART_SALES_PRICE SP WHERE SP.PART_ID = D.PART_ID),\n");
        sql.append("       D.BUY_AMOUNT = ROUND((SELECT SP.SALE_PRICE1 FROM TT_PART_SALES_PRICE SP\n");
        sql.append("                              WHERE SP.PART_ID = D.PART_ID) * D.BUY_QTY,\n");
        sql.append("                            2)\n");
     ...
     * @param orderId 订单ID
     */
    public void updateOrderMainAmount(String orderId) {
        StringBuffer sql = new StringBuffer();
        sql.append("UPDATE TT_PART_DLR_ORDER_MAIN M\n");
        sql.append("   SET M.ACCOUNT_SUM =\n");
        sql.append("       (SELECT SUM(OD.BUY_AMOUNT) FROM TT_PART_DLR_ORDER_DTL OD\n");
        sql.append("         WHERE OD.ORDER_ID = M.ORDER_ID)\n");
        sql.append(" WHERE M.ORDER_ID = '" + orderId + "'\n");
        this.update(sql.toString(), null);
    }
     ...
     * @param orderId 订单ID
     */
    public void updateBOMainAmount(Long BOId) {
        StringBuffer sql = new StringBuffer();
        sql.append("update Tt_Part_Bo_Main t set t.bo_amount =\n");
        sql.append("       (select SUM(ROUND(T2.BUY_PRICE1 * T2.BO_ODDQTY, 2)) from Tt_Part_Bo_Dtl t2\n");
        sql.append("         where t2.bo_id = t.bo_id)\n");
     ...
     * @Description:占用资金
     */
    public void insertAccountNew(AclUserBean loginUser ,Map infoMap) throws Exception {
        try {
        	
        	Long dealerId = (Long)infoMap.get("dealerId");
        	Double amount = (Double)infoMap.get("amount"); 
        	Long sourceId = (Long)infoMap.get("sourceId");
        	String sourceCode= (String)infoMap.get("sourceCode");
        	Long boId = (Long)infoMap.get("boId");
        	String boCode= (String)infoMap.get("boCode");
        	//Long orderId = (Long)infoMap.get("orderId");
        	//String orderCode= (String)infoMap.get("orderCode");
        	Long accountId = (Long)infoMap.get("accountId");
        	String funName = (String)infoMap.get("funName");
            ...
            po.setAmount(amount);
            po.setFunctionName(funName);
            po.setSourceId(Long.valueOf(sourceId));
            po.setSourceCode(sourceCode);
            po.setBoId(boId);
            po.setBoCode(boCode);
