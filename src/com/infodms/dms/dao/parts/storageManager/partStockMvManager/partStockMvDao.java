 ...
 */
public class partStockMvDao extends BaseDao {
    public static Logger logger = Logger.getLogger(partStockMvDao.class);
    private static final partStockMvDao dao = new partStockMvDao();

    private partStockMvDao() {
    }

    public static final partStockMvDao getInstance() {
        return dao;
    }
    ...
    public PageResult<Map<String, Object>> queryPartMvOrders(RequestWrapper request, int pageSize, int curPage) {
        ...
        String whId = CommonUtils.checkNull(request.getParamValue("whId")); // 移出库ID
        ...
        String chgId = CommonUtils.checkNull(request.getParamValue("CHG_ID")) == "" ?
                CommonUtils.checkNull(request.getParamValue("chgId")) : CommonUtils.checkNull(request.getParamValue("CHG_ID"));// 变更单ID
        ...
        sql.append("       TU.NAME,\n");
        sql.append("       SM.WH_ID,\n");
        sql.append("       W1.WH_CODE,\n");
        sql.append("       W1.WH_NAME,\n");
        sql.append("       SM.FLAG,\n");
        sql.append("       SM.TOWH_ID,\n");
        sql.append("       W2.WH_CODE TOWH_CODE,\n");
    ...
    public List<Map<String, Object>> getWareHouses(String sbString) {
        StringBuffer sql = new StringBuffer("");
        sql.append("SELECT TM.WH_ID, TM.WH_NAME AS WH_CNAME "
    ...
    public List<Map<String, Object>> queryAllPartSaleOrders(String sbString) {
        StringBuffer sql = new StringBuffer("");
        sql.append("SELECT TM.RETAIL_ID, TM.RETAIL_CODE, TM.SORG_CNAME, TM.CHG_TYPE, TM.WH_ID, TM.WH_CNAME, U.NAME, TO_CHAR(TM.CREATE_DATE,'yyyy-MM-dd') AS CREATE_DATE, TM.STATE, "
    ...
    public PageResult<Map<String, Object>> queryMvDeatil(RequestWrapper request, int pageSize, int curPage) {
        StringBuffer sql = new StringBuffer("");
        String chgId = CommonUtils.checkNull(request.getParamValue("CHG_ID")) == null ? CommonUtils.checkNull(request.getParamValue("CHG_ID"))
        ...
        sql.append("       SD.VENDER_ID,\n");
        sql.append("       SD.BAT_ID,\n");
        sql.append("       SD.LOC_ID,\n");
        sql.append("       VS.LOC_CODE,\n");
        sql.append("       VS.LOC_NAME,\n");
        sql.append("       VS.NORMAL_QTY,\n");
        sql.append("       VS.ITEM_QTY,\n");
        sql.append("       VS.BOOKED_QTY,\n");
        sql.append("       VS.PART_ID, VS.PART_ID || '_RNUM_' PART_IDS,\n");
...
//        sql.append("   AND SD.LOC_ID = LD.LOC_ID\n");
        ...
        sql.append("   AND SD.BAT_ID = VS.BATCH_NO\n");
...
//        sql.append("    AND T3.STATUS = '"+Constant.IF_TYPE_YES+"' \n");
    ...
    public PageResult<Map<String, Object>> queryMvPartsPageList(RequestWrapper request, int pageSize, int curPage){
        PageResult<Map<String, Object>> ps = pageQuery(getMvPartsListSql(request), null, getFunName(), pageSize, curPage);
        return ps;
    }
    ...
    public PageResult<Map<String, Object>> showPartStockBase(RequestWrapper request, int pageSize, int curPage) {
        ...
        String whId = CommonUtils.checkNull(request.getParamValue("whId"));//移出仓库ID
        ...
        //切换库调拨时如果调入库房为售后旧件库时只调拨旧件出库
        if (null != whId && !"".equals(whId)) {
            if ("2014092298805416".equals(whId) && "2014122507332491".equals(toWhId)) {
                sql.append("AND TD.LOC_CODE = 'QHJ'\n");
            }
        }
...
//		sql.append(" AND TD.ITEM_QTY > 0 ");
    ...
    public String getMainCompanyName(String oemCompanyId) {
        String companyName = "";
        StringBuffer sql = new StringBuffer("");
        sql.append("SELECT TM.COMPANY_NAME "
                + " FROM TM_COMPANY TM "
                + " WHERE 1 = 1  ");
     ...
     * @return
     */
    public List<Map<String, Object>> getPartStockInfos(String partId, String orgId, String whId, String locId, String batchNo) {
    ...
    public void updateASdMvState(String dbFlag, String dbId, String dbState, String partCode, String preDbFlag, String Qty, String srcDbId) {
        StringBuffer sql = new StringBuffer("");
