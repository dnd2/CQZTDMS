 ...
 * @ClassName : partStockCgHdleDao
 */
public class partStockCgHdleDao extends BaseDao {
    public static Logger logger = Logger.getLogger(partStockCgHdleDao.class);
    private static final partStockCgHdleDao dao = new partStockCgHdleDao();

    private partStockCgHdleDao() {
    }

    public static final partStockCgHdleDao getInstance() {
        return dao;
    }
    ...
    public PageResult<Map<String, Object>> staSetCntQuery(String sbString, int pageSize, int curPage) {
        StringBuffer sql = new StringBuffer("");
        sql.append("SELECT TD.*, (NVL(TD.RETURN_QTY,'0') - NVL(TD.COLSE_QTY, '0')) AS UNCLS_QTY, TM.WH_CNAME, TM.WH_ID, U.NAME, PB.NORMAL_QTY, ");
        sql.append(" TL.LOC_CODE");
        sql.append(" FROM TT_PART_CHG_STATE_DTL TD, TT_PART_CHG_STATE_MAIN TM, TC_USER U, TT_PART_BOOK PB, TT_PART_LOACTION_DEFINE TL ");
    ...
    public List<Map<String, Object>> staSetCntQuyList(String sbString) {
        StringBuffer sql = new StringBuffer("");
        sql
                .append("SELECT TD.*, (NVL(TD.RETURN_QTY,'0') - NVL(TD.COLSE_QTY, '0')) AS UNCLS_QTY, TM.WH_CNAME, U.NAME, "
                        + " TO_CHAR(TD.CREATE_DATE,'yyyy-MM-dd') AS CREATE_DATE_FM, TO_CHAR(TD.UPDATE_DATE,'yyyy-MM-dd') AS UPDATE_DATE_FM, PB.NORMAL_QTY  "
     ...
     * @Title : 返回精确的配件库存状态信息
     */
    public List getPartStockInfos(String oldCode, String parentOrgId, String whId, Long locId) {
        List<Map<String, Object>> list = null;
        StringBuffer sql = new StringBuffer("");
        sql.append("SELECT * FROM VW_PART_STOCK TD ");
        sql.append(" WHERE 1 = 1 ");
    ...
    public List<Map<String, Object>> getWareLocaInfos(String sbString) {
        ...
        sql.append("SELECT WD.WH_ID,"
                ...
                + " LD.LOC_CODE,"
                + " LD.LOC_NAME"
                + " FROM TT_PART_WAREHOUSE_DEFINE WD, TT_PART_LOACTION_DEFINE LD, TT_PART_CHG_STATE_DTL CSD"
                + " WHERE WD.WH_ID = LD.WH_ID"
                + " AND LD.LOC_ID = CSD.LOC_ID "
