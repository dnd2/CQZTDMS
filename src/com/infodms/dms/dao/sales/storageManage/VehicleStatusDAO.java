...
public class VehicleStatusDAO extends BaseDao {
	public static Logger logger = Logger.getLogger(VehicleStatusDAO.class);
	...
	public static PageResult<Map<String, Object>> getVechileStatusQuery( Map<String,String> condition,int pageSize, int curPage) {
		String materialCode=condition.get("materialCode");
		String vin=condition.get("vin");
		String vehicle_life=condition.get("vehicle_life");
		String lockStatus=condition.get("lockStatus");
		String bodyColor=condition.get("bodyColor");
		StringBuffer sql = new StringBuffer("");
		sql.append("SELECT G.GROUP_NAME AS MODEL_NAME, --车型\n");
		sql.append("       G1.GROUP_NAME AS PACKAGE_NAME, --配置\n");
		sql.append("       TMVM.MATERIAL_NAME, --物料\n");
		sql.append("       TMVM.MATERIAL_CODE,\n");
		sql.append("       TMV.BODY_COLOR,\n");
		sql.append("       TMV.VIN, --VIN\n");
		sql.append("       TMV.ENGINE_NO, --VIN\n");
		sql.append("       TMD.DEALER_SHORTNAME AS DEALER_SHORTNAME, \n");
		sql.append("       TMV.LIFE_CYCLE, --库存状态\n");
		sql.append("       TMV.LOCK_STATUS, --锁定状态\n");
		sql.append("       TO_CHAR(TMV.PRODUCT_DATE, 'yyyy-MM-dd') AS PRODUCT_DATE, --生产日期\n");
		sql.append("       TO_CHAR(TMV.FACTORY_DATE, 'yyyy-MM-dd') AS FACTORY_DATE, --生产日期\n");
		sql.append("       TO_CHAR(TMV.STORAGE_DATE, 'yyyy-MM-dd') AS STORAGE_TIME --入库日期\n");
		sql.append("		FROM TM_VEHICLE          TMV,\n" );
		sql.append("      TM_VHCL_MATERIAL_GROUP   G,\n" );
		sql.append("      TM_VHCL_MATERIAL_GROUP   G0,\n" );
		sql.append("      TM_VHCL_MATERIAL_GROUP   G1,\n" );
		sql.append("      TM_VHCL_MATERIAL         TMVM,\n" );
		sql.append("      TM_DEALER                TMD,\n" );
		...
		sql.append("  WHERE TMV.MATERIAL_ID = TMVM.MATERIAL_ID\n" );
		sql.append("   AND TMV.DEALER_ID = TMD.DEALER_ID\n" );
		sql.append("   AND TVMGR.MATERIAL_ID = TMVM.MATERIAL_ID\n" );
		sql.append("   AND TVMGR.GROUP_ID = G1.GROUP_ID\n" );
		sql.append("   AND G.GROUP_ID = G1.PARENT_GROUP_ID\n" );
		sql.append("   AND G0.GROUP_ID = G.PARENT_GROUP_ID\n" );
		
		if(materialCode!=null&&!"".equals(materialCode)){
			sql.append("   AND (G.GROUP_CODE LIKE '%"+materialCode+"%'   \n");
			sql.append("   OR  G0.GROUP_CODE LIKE '%"+materialCode+"%'  ) \n");
		}
		...
		if(vehicle_life!=null&&!"".equals(vehicle_life)){
			
			sql.append("   AND TMV.LIFE_CYCLE  ="+vehicle_life+"\n");
		...
		if(bodyColor!=null&&!"".equals(bodyColor)){
			
			sql.append("   AND TMV.body_Color like '%"+bodyColor+"%' \n");
