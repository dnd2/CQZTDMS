 ...
 * CreateDate     : 2013-4-1
 */
public class FareSetDao extends BaseDao<PO>{
	private static final FareSetDao dao = new FareSetDao ();
	public static final FareSetDao getInstance() {
		return dao;
	}
	@Override
	 ...
	 * @throws Exception
	 */
	public PageResult<Map<String, Object>> getFareSetQuery(Map<String, Object> map, int curPage, int pageSize)throws Exception{
		...
		//--获取某产地下的所有运费里程信息
		List<Map<String, Object>> list_FM =getFareMileageMsg(yieldly,poseId);	
		...
		sql.append("TSF.YIELDLY AREA_ID,TBA.AREA_NAME YIELDLY \n");
		sql.append(" FROM TT_SALES_FARE TSF, tm_vhcl_material_group TVMG,TM_BUSINESS_AREA TBA,tt_sales_milset tsm\n");
		sql.append(" WHERE TSF.GROUP_ID = TVMG.GROUP_ID\n");
		sql.append(" and tsf.mil_id = tsm.mil_id\n");
		sql.append(" AND TSF.YIELDLY=TBA.AREA_ID\n");
		sql.append(MaterialGroupManagerDao.getPoseIdBusinessSql("tsf.yieldly",poseId));//车厂端查询列表产地数据权限
...
/*		if(yieldly){
	...
	public List<Map<String, Object>> getMateriaDetail(String billId) throws Exception{
		...
		sql.append("\n" );
		sql.append("\n" );
		sql.append("select VMT.SERIES_NAME, VMT.PACKAGE_NAME, a.vin, e.order_no, F.ADDRESS,a.dtl_id,F_GET_WAY_ADDRESS(dtl_id) this_address\n" );
