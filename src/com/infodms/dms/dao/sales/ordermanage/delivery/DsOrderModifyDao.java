 ...
 * @remark
 */
public class DsOrderModifyDao extends BaseDao<PO> {

	public static Logger logger = Logger.getLogger(DsOrderModifyDao.class);
	private static final DsOrderModifyDao dao = new DsOrderModifyDao();

	public static final DsOrderModifyDao getInstance() {
		return dao;
	}
	 ...
	 * @throws Exception
	 */
	public PageResult<Map<String, Object>> getOrderDeliveryModifyQuery(Map<String, Object> map, int curPage, int pageSize) throws Exception {
		...
		sql.append(" SELECT DISTINCT tvdr.req_id, tod.order_number,\n");
		sql.append("		  TMD.DEALER_CODE, \n");
		sql.append("		TMD.DEALER_SHORTNAME DEALER_NAME, \n");
		sql.append("		'待确认' ORDER_DEALER_CODE1, \n");
		sql.append("		'待确认'  ORDER_DEALER_NAME1, \n");
		sql.append("		TVDR.DLVRY_REQ_NO, \n");
		sql.append("		tvdr.create_date as RAISE_DATE, \n");
		sql.append("		TVDRD.REQ_AMOUNT REQ_TOTAL_AMOUNT, \n");
		sql.append(" 		TVDR.REQ_TOTAL_PRICE, \n");
		sql.append("		TVDRD.REQ_AMOUNT, \n");
		//sql.append("		 TVAT.TYPE_NAME FUND_TYPE, \n");
		sql.append("		'待确认' as ORDER_AMOUNT1 \n");

		sql.append("  FROM CUX_ERP_ORDERS_DELIVER_1@DMS2EBS2 TOD, \n");
		sql.append("       TT_VS_DS_DLVRY_REQ TVDR, \n");
		sql.append("       TM_DEALER TMD, \n");
		sql.append("       (SELECT T.REQ_ID, SUM(T.REQ_AMOUNT) REQ_AMOUNT \n");
		sql.append("          FROM TT_VS_DS_DLVRY_REQ_DTL T \n");
		sql.append("         GROUP BY T.REQ_ID) TVDRD \n");
		//sql.append("       TT_VS_ACCOUNT_TYPE TVAT \n");
		sql.append(" WHERE TOD.ORDER_NUMBER = TVDR.DLVRY_REQ_NO \n");
		sql.append("   AND TVDR.DEALER_ID = TMD.DEALER_ID \n");
		sql.append("   AND TVDR.REQ_ID = TVDRD.REQ_ID \n");
		//sql.append("   AND TVDR.FUND_TYPE = TVAT.TYPE_ID \n");
        sql.append("   AND TOD.STATUS = 2");
		if (orderType != null && !"".equals(orderType)) {
			sql.append("   AND TVO.ORDER_TYPE = " + orderType + "\n");
		}
		
		if (null != dealerId && !"".equals(dealerId)) {
			sql.append(" AND TMD.DEALER_ID = ").append(dealerId).append("\n");
		}
		...
		if (orgCode != null && !"".equals(orgCode)) {
			sql.append("   AND TVO.BIL_DEALER_ID IN\n");
	 ...
	 * @throws Exception
	 */
	public List<Map<String, Object>> getorderResourceModifyDetailList(String reqId, String orderType, String companyId, String erpCode) throws Exception {
		...
		sql.append("       NVL(TOD.SHIP_NUMBER,0) SHIP_NUMBER,\n");
		...
		sql.append("       TT_ORDERS_DELIVER  TOD,\n");
	...
	//	sql.append("           AND TEMP.ENTITY_CODE = " + erpCode + "\n");
		...
		sql.append("   AND TOD.ORDER_NO = TVDR.DLVRY_REQ_NO\n");
		sql.append("   AND TOD.MATERIAL_CODE = TVM.MATERIAL_CODE\n");
	...
	public void sendDataToErp(TtVsDsDlvryReqPO tvdrpo,TtVsDsDlvryReqDtlPO tvdrdpo,AclUserBean logonUser,int lineNumber) throws Exception {
		...
		sql.append("   "+tvdrdpo.getOldReqAmount()+",\n" );//orig_order_quanity
	 ...
	 *             Exception LastUpdate : 2010-05-24
	 */
	public void updateDataFromErp(TtVsDsDlvryReqPO tvdrpo,TtVsDsDlvryReqDtlPO tvdrdpo,AclUserBean logonUser,int lineNumber) throws Exception {
		//获取customer_name 
		TmDealerPO td =new TmDealerPO();
		td.setDealerId(new Long(logonUser.getDealerId()));
		td=(TmDealerPO) dao.select(td).get(0);
		...
		StringBuilder sql= new StringBuilder();
		sql.append(" UPDATE CUX_ERP_ORDERS_DELIVER_1@dms2ebs2 SET \n");
		sql.append(" orig_order_quantity = ").append(tvdrdpo.getOldReqAmount()).append("\n");
		sql.append(" ,ordered_quantity = ").append(tvdrdpo.getReqAmount()).append("\n");
		sql.append(" ,status = 7 \n");
		sql.append(" WHERE \n");
		sql.append(" seq_id = ").append(tvdrdpo.getDetailId());
		
		/*
		sql.append("insert into CUX_ERP_ORDERS_DELIVER_1\n" );
		sql.append("  (seq_id,\n" );
		sql.append("   customer_name,\n" );
		sql.append("   customer_number,\n" );
		sql.append("   ship_to_location,\n" );
	 ...
	 * @throws Exception
	 */
	public PageResult<Map<String, Object>> getOrderDelvyModifyLoadQuery(Map<String, Object> map, int curPage, int pageSize) throws Exception {
	 ...
	 */
	public PageResult<Map<String, Object>> getOrderResourceModifyDetailDownLoad(Map<String, Object> map, int curPage, int pageSize) throws Exception {
...
//		sql.append("          TVO.RAISE_DATE");
		...
		sql.append("       td.dealer_name          dealer_name1,\n" );
		...
		sql.append("        NVL(TVDR.TMP_LICENSE_AMOUNT, 0) TMP_LICENSE_AMOUNT\n" );
		//sql.append("        tvat.TYPE_NAME\n" );
		sql.append("  from tt_vs_ds_dlvry_req     tvdr,\n" );
		sql.append("       tt_vs_ds_dlvry_req_dtl tvdrd,\n" );
		sql.append("       tm_dealer              td,\n" );
		sql.append("       tm_vs_address          tva\n" );
		//sql.append("       tt_vs_account_type tvat\n" );
		sql.append(" where tvdr.req_id = tvdrd.req_id\n" );
		sql.append("   and tvdr.dealer_id = td.dealer_id\n" );
		sql.append("   and tva.dealer_id(+) = td.dealer_id\n" );
		//sql.append("   and tvat.type_id=tvdr.fund_type");
		sql.append("   and tvdr.req_id='"+reqId+"'");
	 ...
	 *             Exception LastUpdate : 2010-05-24
	 */
	public List<Map<String, Object>> getDeliveryApplyList(String dealer_id,String reqId) throws Exception {
;		StringBuilder sql= new StringBuilder();
		...
		sql.append("       tvm.material_name,\n" );
		sql.append("       tvm.color_name,\n" );
		sql.append("       vds.RESOUCE_COUNT-VDD.req_amount + tvdrd.req_amount RESOUCE_COUNT ,-- 可申请总数\n" );
		//sql.append("       vds.RESOUCE_COUNT ,\n" );
		sql.append("       nvl(ceod.shipped_quantity,0) DELIVERY_AMOUNT,\n" );
		//sql.append("       vds.resouce_count-nvl(vdd.delivery_amount,0) ABLE_APPLY,\n" );
		sql.append("       tvdrd.req_amount AMOUNT,--本张单子申请数量\n");
		...
		sql.append("       tt_vs_ds_dlvry_req  tvdr,\n" );
		sql.append("       tt_vs_ds_dlvry_req_dtl tvdrd,\n" );
		sql.append("       CUX_ERP_ORDERS_DELIVER_1@dms2ebs2 ceod\n");
		...
		sql.append("   and tvdr.req_id=tvdrd.req_id\n" );
		sql.append("   and tvdrd.material_id=tvm.material_id");
		sql.append("   and tvdrd.detail_id=ceod.seq_id(+)\n");
		sql.append("   and td.dealer_id='" + dealer_id + "'");
		sql.append("  and tvdr.req_id="+reqId);
		List<Map<String, Object>> list = dao.pageQuery(sql.toString(), null, getFunName());
		return list;
	}
...
//		sql.append("           AND TV.LIFE_CYCLE = " + Constant.VEHICLE_LIFE_02 + " --车厂库存\n");
	 ...
	 * @throws Exception
	 */
	public String getLinNum(String reqNo) throws Exception {
		String lineNumber = "";
		if(reqNo != null && !"".equals(reqNo)){
			StringBuffer sql = new StringBuffer();
			sql.append("SELECT NVL(MAX(X.LINE_NUMBER),0) AS MAX_LINE_NUMBER FROM CUX_ERP_DELIVER X WHERE X.ORDER_NUMBER = "+reqNo+"\n");
			List<Map<String, Object>> list = dao.pageQuery(sql.toString(), null, getFunName());
			if(list != null && list.size() > 0){
				lineNumber = list.get(0).get("MAX_LINE_NUMBER").toString();
	...
	public Map<String, Object> getMaterialInfo(String materialCode, String year, String week, String priceId, String companyId, String entityCode,String dealerCode,String orderType,String orderId,String dealerId) {
		...
		sql.append("       F_GET_MATPRICE(" + dealerCode + ",'" + materialCode + "') SINGLE_PRICE, ---物料单价\n" );
		sql.append("       F_GET_PRICELISTID(" + dealerCode + ",'" + materialCode + "') PRICTLIST_ID ---物料单价id\n" );
		//sql.append("        '13877' SINGLE_PRICE, ---物料单价\n" );
		//sql.append("       '1157' PRICTLIST_ID ---物料单价id\n" );
		sql.append("  from view_dealer_source       vds,\n" );
		sql.append("       tm_dealer                td,\n" );
		...
		if(!"".equals(orderType) && "10201001".equals(orderType)){
			sql.append("  VOR.DETAIL_ID,\n");
		}else{
			sql.append("  '0' AS DETAIL_ID,\n");
		}
//		sql.append("       F_GET_PRICE(" + priceId + ", TVM.MATERIAL_ID) PRICE,\n");
		...
		sql.append("       TM_VHCL_MATERIAL_GROUP_R TVMGR,\n");
		if(!"".equals(orderType) && "10201001".equals(orderType)){
			sql.append("       (SELECT VVOR.MATERIAL_ID,VVOR.DETAIL_ID, (VVOR.CHECK_AMOUNT - VVOR.CALL_AMOUNT) RESOURCE_AMOUNT\n");
			sql.append("          FROM TT_VS_ORDER_DETAIL VVOR\n");
			sql.append("         WHERE VVOR.ORDER_ID = " + orderId + ") VOR,\n");			
		}else{
...
//		}
		...
		sql.append("   AND TVM.MATERIAL_CODE = '" + materialCode + "'");
		*/
		Map<String, Object> map = pageQueryMap(sql.toString(), null, "com.infodms.dms.dao.sales.ordermanage.ordermodify.OrderModifyDao.getMaterialInfo");
		return map;
	}
