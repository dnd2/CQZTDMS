...
public class MonReportDao extends BaseDao{
	...
	//获取最新月份数据”
	public String getMonth(long companyId,int year,int month,String orgId){
		String monthname = "0";
		StringBuffer sql = new StringBuffer("") ;
		sql.append(" SELECT MAX(T.MON) AS MON_MAX FROM xxont_mon_rep T \n");
		
		List<Map<String, Object>> list = dao.pageQuery(sql.toString(), null, getFunName()) ;
		if (null != list && list.size()>0) {
			monthname = String.valueOf(list.get(0).get("MON_MAX"));
		}
		return monthname;
	}
	////获取数据月报表数据1
	public List<Map<String,Object>> newmonreplist1(int year,int month,int day,String orgId,String mon_name) { 
		List<Object> params = new ArrayList<Object>() ;
		
		StringBuffer sql = new StringBuffer("\n") ;
		
		sql.append(" SELECT CAR_TYEP,QC_QYKC,QC_SHKC,QC_KCHJ,PLAN_SC, \n");	
		sql.append(" PLAN_QP,PLAN_SX,COM_SC,COM_QP,COM_SX,BL_SC,BL_QP,BL_SX,M_TB_QP,M_TB_SX, \n");
		sql.append(" M_HB_QP,M_HB_SX,Y_PLAN_QP1,Y_PLAN_SX1,Y_PLAN_QP2,Y_PLAN_SX2,Y_COM_QP,Y_COM_SX, \n");
		sql.append(" Y_BL_QP1,Y_BL_SX1,Y_BL_QP2,Y_BL_SX2,Y_TB_QP,Y_TB_SX,QM_QYKC,QM_SHKC, \n");
		sql.append(" QM_KCHJ,BL_KC_XS,KC_BH_QYKC,KC_BH_SHKC,KC_BH_KCHJ,MON,SORT_ID \n");
		sql.append(" FROM XXONT_MON_REP_V T \n");
		
		sql.append("         WHERE T.MON = ?\n");  
		params.add(mon_name) ;				 
		sql.append(" ORDER BY T.SORT_ID\n");
        //System.out.println("jiangluoling:"+sql);
...
////获取数据月报表数据2逐月启票信息
	public List<Map<String,Object>> newmonreplist2(int year,int month,int day,String orgId,String mon_name) { 
		List<Object> params = new ArrayList<Object>() ;
		
		StringBuffer sql = new StringBuffer("\n") ;
		
		sql.append(" SELECT T.*   \n");	
		sql.append(" FROM XXONT_MON_QP_V T \n");
...
////获取数据月报表数据3逐月启票信息
	public List<Map<String,Object>> newmonreplist3(int year,int month,int day,String orgId,String mon_name) { 
		List<Object> params = new ArrayList<Object>() ;
		
		StringBuffer sql = new StringBuffer("\n") ;
		
		sql.append(" SELECT T.*   \n");	
		sql.append(" FROM XXONT_MON_LS_V T \n");
...
////获取数据月报表数据4区域启票，销售，库存数据
	//// area_type 区域标识：1：A网，2：B网，3华南特区
	public List<Map<String,Object>> newmonreplist4(int year,int month,int day,String orgId,String mon_name,int area_type) { 
		List<Object> params = new ArrayList<Object>() ;
		
		StringBuffer sql = new StringBuffer("\n") ;
		
		sql.append(" SELECT T.*   \n");	
		sql.append(" FROM XXONT_MON_REP_AREA_V T \n");
		
		sql.append("         WHERE T.MON = ?\n");  
		params.add(mon_name) ;	
		sql.append("         and  AREA_TYPE = ?\n");  
		params.add(area_type) ;
		sql.append(" ORDER BY T.SORT_ID\n");
        //System.out.println("jiangluoling:"+sql);
...
////获取数据月报表数据5区域启票，实销明细
	///  info_type 业务类型：1：启票，2：实销
	//// area_type 区域标识：1：A网，2：B网，3华南特区	
	public List<Map<String,Object>> newmonreplist5(int year,int month,int day,String orgId,String mon_name,int info_type,int area_type) { 
		List<Object> params = new ArrayList<Object>() ;
		
		StringBuffer sql = new StringBuffer("\n") ;
		
		sql.append(" SELECT T.*   \n");	
		sql.append(" FROM XXONT_MON_AREA_DET_V T \n");
	 ...
	 * @param           : 
	 */
	public static List<Map<String, Object>> getmonlist()
	{
		List<Map<String, Object>> list = null;
		StringBuffer sql=new StringBuffer();
		sql.append(" select DISTINCT T.MON  AS MON_NAME from xxont_mon_rep_v t \n");
		sql.append("  ORDER BY T.MON DESC \n");  
