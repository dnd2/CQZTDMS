 ...
 * 
 */
public class DsOrderReportDao extends BaseDao<PO> {
	public static Logger logger = Logger.getLogger(DsOrderReportDao.class);
	private static final DsOrderReportDao dao = new DsOrderReportDao();

	public static final DsOrderReportDao getInstance() {
		return dao;
	}
	...
	public List<Map<String, Object>> getSerieList(Long poseId) {
		...
		List<Map<String, Object>> list = pageQuery(sql.toString(), null, "com.infodms.dms.dao.sales.OrderReportDao.getSeriesList");
		return list;
	}
	 ...
	 * @return
	 */
	public PageResult<Map<String, Object>> getGeneralOrderReportList(String orderYear, String orderWeek, String areaId, String areaIds, String dealerIds, String companyId, int curPage, int pageSize) {
		...
		sql.append("           AND TSO.ORDER_TYPE = " + Constant.ORDER_TYPE_01 + "\n");
		sql.append("           AND TSO.ORDER_STATUS = " + Constant.ORDER_STATUS_01 + "\n");
		sql.append("           AND TSO.ORDER_YEAR = " + orderYear + "\n");
		sql.append("           AND TSO.ORDER_WEEK = " + orderWeek + "\n");
	...
	public Map<String, Object> getGeneralOrderReportDetail(String orderYear, String orderWeek, String areaId, String dealerId, String groupId, String companyId) {
		...
		Map<String, Object> map = pageQueryMap(sql.toString(), null, "com.infodms.dms.dao.sales.OrderReportDao.getGeneralOrderReportDetail");
		return map;
	}
	...
	public List<Map<String, Object>> getGeneralReportErrorList(String year, String week, String areaId, String dealerId, String companyId) {
		...
		List<Map<String, Object>> list = pageQuery(sql.toString(), null, "com.infodms.dms.dao.sales.OrderReportDao.getGeneralReportErrorList");
		return list;
	}
	...
	public List<Map<String, Object>> getUnsaveMaterialList(String dealerId, String groupId) {
		...
		List<Map<String, Object>> list = pageQuery(sql.toString(), null, "com.infodms.dms.dao.sales.OrderReportDao.getUnsaveMaterialList");
		return list;
	}
	 ...
	 * @return
	 */
	public List<Map<String, Object>> getSaveMaterialList(String orderYear, String orderWeek, String dealerId, String groupId) {
		...
		List<Map<String, Object>> list = pageQuery(sql.toString(), null, "com.infodms.dms.dao.sales.OrderReportDao.getSaveMaterialList");
		return list;
	}
	...
	public PageResult<Map<String, Object>> getUrgentOrderResList(String groupCode, String materialCode, String areaId, String areaIds, String companyId, String year, String week, int curPage, int pageSize) {
		...
		PageResult<Map<String, Object>> ps = pageQuery(sql.toString(), null, "com.infodms.dms.dao.sales.OrderReportDao.getUrgentOrderResList", pageSize, curPage);
		return ps;
	}
	 ...
	 * @return
	 */
	public PageResult<Map<String, Object>> getUrgentOrderReportList(String orderYear, String orderWeek, String areaId, String areaIds, String dealerIds, String companyId, int curPage, int pageSize) {
...
//		sql.append("           AND TSO.ORDER_WEEK = " + orderWeek + "\n");
		...
		sql.append("           AND TSO.AREA_ID IN (" + areaIds + ")\n");
		sql.append("           AND TSO.ORDER_ORG_ID IN (" + dealerIds + ")\n");
		sql.append("           AND TSO.ORDER_STATUS IN (" + Constant.ORDER_STATUS_01 + ", " + Constant.ORDER_STATUS_04 + ")\n");
		sql.append("           AND TSO.ORDER_TYPE IN( " + Constant.ORDER_TYPE_06 + ")) A,\n");
//		sql.append("           AND TSO.ORDER_TYPE IN( " + Constant.ORDER_TYPE_02 + "," + Constant.ORDER_TYPE_03 + ")) A,\n");
...
//		sql.append("           AND TSO.ORDER_WEEK = " + orderWeek + "\n");
		...
		sql.append("           AND TSO.ORDER_ORG_ID IN (" + dealerIds + ")\n");
		sql.append("           AND TSO.ORDER_STATUS IN (" + Constant.ORDER_STATUS_01 + ", " + Constant.ORDER_STATUS_04 + ")\n");
		sql.append("           AND TSO.ORDER_TYPE IN ( " + Constant.ORDER_TYPE_06 + ")\n");
//		sql.append("           AND TSO.ORDER_TYPE IN ( " + Constant.ORDER_TYPE_02 + "," + Constant.ORDER_TYPE_03 + ")\n");
		...
		PageResult<Map<String, Object>> ps = pageQuery(sql.toString(), null, "com.infodms.dms.dao.sales.OrderReportDao.getUrgentOrderReportList", pageSize, curPage);
		return ps;
	}
	...
	public List<Map<String, Object>> getFundTypeList() {
		...
		List<Map<String, Object>> list = pageQuery(sql.toString(), null, "com.infodms.dms.dao.sales.OrderReportDao.getFundTypeList");
		return list;
	}
	...
	public Map<String, Object> getAvailableAmount(String fundTypeId, String dealerId) {
		...
		if (UrgentOrderReport.chkBinCai(fundTypeId, null)) {
			...
			sql.append("SELECT null ACCOUNT_ID, TAA.BALANCE_AMOUNT AVAILABLE_AMOUNT\n");
			sql.append("  FROM TT_VS_ACCOUNT_TYPE TSA,CUX_DMS_ACOUNT_V@Dms2ebs2 TAA\n");
	...
	public List<Map<String, Object>> getAddressList(String dealerId, String areaId) {
		...
		/*
		 * sql.append("SELECT TA.ID, TA.ADDRESS FROM TM_VS_ADDRESS TA WHERE
		 * TA.DEALER_ID = " + dealerId + ""); sql.append("AND
		 * TA.STATUS='"+Constant.STATUS_ENABLE+"'");
		 */
	...
	public Map<String, Object> getAddressInfo_(String addressId, String orderId) {
		...
		StringBuffer sql = new StringBuffer();
		/*
		 * sql.append("SELECT TA.ADDRESS, TA.LINK_MAN, TA.TEL, TA.RECEIVE_ORG
		 * FROM TM_VS_ADDRESS TA WHERE 1=1 "); if (!addressId.equals("")) {
		 * sql.append(" AND TA.ID = " + addressId); } else { sql.append(" AND
		 * TA.ID IS NULL"); }
		 */
		...
		Map<String, Object> map = pageQueryMap(sql.toString(), null, "com.infodms.dms.dao.sales.OrderReportDao.getAddressInfo");
		if (map == null) {
			map = new HashMap<String, Object>();
			map.put("ADDRESS", "");
			map.put("LINK_MAN", "");
	...
	public List<Map<String, Object>> getTmFaDealer(String dealerId) {
		StringBuffer sql = new StringBuffer();
		sql.append("select vod.ROOT_DEALER_ID from vw_org_dealer vod where vod.dealer_id=" + dealerId);
	...
	public Map<String, Object> getMaterialInfo(String materialCode, String year, String week, String priceId, String companyId, String entityCode,String dealerCode) {
		...
		sql.append("       TVM.MATERIAL_NAME,\n");
		sql.append("       CASE\n");
		sql.append("         WHEN NVL(VOR.RESOURCE_AMOUNT, 0) <= 0 THEN\n");
		sql.append("          '无'\n");
		sql.append("         ELSE\n");
		sql.append("          '有'\n");
...
//		sql.append("       F_GET_PRICE(" + priceId + ", TVM.MATERIAL_ID) PRICE,\n");
		...
		sql.append("       NVL(VR.AVA_STOCK, 0) AVA_STOCK,\n");
		...
		sql.append("       TM_VHCL_MATERIAL_GROUP_R TVMGR,\n");
		sql.append("       (SELECT VVOR.MATERIAL_ID, VVOR.RESOURCE_AMOUNT\n");
		sql.append("          FROM VW_VS_ORDER_RESOURCE VVOR\n");
		sql.append("         WHERE VVOR.COMPANY_ID = " + companyId + ") VOR,\n");
		...
		/*
		 * sql.append("SELECT TVMG3.GROUP_CODE SERIES_CODE,\n"); sql.append("
		 * TVMG3.GROUP_NAME SERIES_NAME,\n"); sql.append(" TVM.MATERIAL_ID,\n");
		 * sql.append(" TVM.MATERIAL_CODE,\n"); sql.append("
		 * TVM.MATERIAL_NAME,\n"); sql .append(" CASE WHEN
		 * NVL(TR.RESOURCE_AMOUNT, 0) - NVL(B.ORDER_AMOUNT, 0) <= 0 THEN '无'
		 * ELSE '有' end RESOURCE_AMOUNT, \n"); // sql.append(" 0,\n"); //
		 * sql.append(" '无',\n"); // sql.append(" '有') RESOURCE_AMOUNT,\n");
		 * sql.append(" F_GET_PRICE(" + priceId + ", TVM.MATERIAL_ID)
		 * price,\n"); sql.append(" NVL(VR.AVA_STOCK, 0) AVA_STOCK,\n");
		 * sql.append(" NVL(TORDER.N_UN_REMAIN_AMOUNT, 0) GENERAL_AMOUNT,\n");
		 * sql.append(" TVM.MATERIAL_ID\n"); sql.append(" FROM
		 * TM_VHCL_MATERIAL_GROUP TVMG1,\n"); sql.append("
		 * TM_VHCL_MATERIAL_GROUP TVMG2,\n"); sql.append("
		 * TM_VHCL_MATERIAL_GROUP TVMG3,\n"); sql.append(" TM_VHCL_MATERIAL
		 * TVM,\n"); sql.append(" TM_VHCL_MATERIAL_GROUP_R TVMGR,\n");
		 * sql.append(" (SELECT TVR.MATERIAL_ID, TVR.RESOURCE_AMOUNT\n");
		 * sql.append(" FROM TT_VS_RESOURCE TVR\n"); sql.append(" WHERE
		 * TVR.RESOURCE_YEAR = '" + year + "'\n"); sql.append(" AND
		 * TVR.RESOURCE_WEEK = '" + week + "'\n"); sql.append(" AND
		 * TVR.SPECIAL_BATCH_NO IS NULL) TR,\n"); sql.append(" (SELECT
		 * TVOD.MATERIAL_ID, SUM(TVOD.ORDER_AMOUNT) ORDER_AMOUNT\n");
		 * sql.append(" FROM TT_VS_ORDER TVO, TT_VS_ORDER_DETAIL TVOD\n");
		 * sql.append(" WHERE TVO.ORDER_ID = TVOD.ORDER_ID\n"); sql.append(" AND
		 * TVO.ORDER_STATUS = " + Constant.ORDER_STATUS_03 + "\n"); sql.append("
		 * AND TVO.COMPANY_ID = " + companyId + "\n"); sql.append(" AND
		 * TVOD.SPECIAL_BATCH_NO IS NULL\n"); sql.append(" GROUP BY MATERIAL_ID)
		 * B,\n"); sql .append(" (SELECT VVR.MATERIAL_ID, VVR.AVA_STOCK,
		 * VVR.UNDO_ORDER_AMOUNT GENERAL_AMOUNT\n"); sql.append(" FROM
		 * VW_VS_RESOURCE VVR\n"); sql.append(" WHERE VVR.COMPANY_ID = " +
		 * companyId + " \n"); sql.append(" AND VVR.SPECIAL_BATCH_NO IS NULL)
		 * VR,\n"); sql.append(" (SELECT VVO.MATERIAL_ID,
		 * VVO.N_UN_REMAIN_AMOUNT\n"); sql.append(" FROM VW_VS_ORDER VVO\n");
		 * sql.append(" WHERE VVO.COMPANY_ID = "+companyId+") TORDER\n");
		 * sql.append(" WHERE TVMG1.PARENT_GROUP_ID = TVMG2.GROUP_ID\n");
		 * sql.append(" AND TVMG2.PARENT_GROUP_ID = TVMG3.GROUP_ID\n");
		 * sql.append(" AND TVMG1.GROUP_ID = TVMGR.GROUP_ID\n"); sql.append("
		 * AND TVMGR.MATERIAL_ID = TVM.MATERIAL_ID\n"); sql.append(" AND
		 * TVM.MATERIAL_ID = TR.MATERIAL_ID(+)\n"); sql.append(" AND
		 * TVM.MATERIAL_ID = VR.MATERIAL_ID(+)\n"); sql.append(" AND
		 * TVM.MATERIAL_ID = TORDER.MATERIAL_ID(+)\n"); sql.append(" AND
		 * TVM.MATERIAL_ID = B.MATERIAL_ID(+)\n"); sql.append(" AND
		 * TVM.MATERIAL_CODE = '" + materialCode + "'");
		 */

		Map<String, Object> map = pageQueryMap(sql.toString(), null, "com.infodms.dms.dao.sales.OrderReportDao.getMaterialInfo");
		return map;
	}
	...
	public List<Map<String, Object>> getSalesOrderDetailList(String orderId, String year, String week, String companyId) {
		...
		sql.append("       (SELECT VVOR.MATERIAL_ID, VVOR.RESOURCE_AMOUNT\n");
		sql.append("          FROM VW_VS_ORDER_RESOURCE VVOR\n");
		sql.append("         WHERE VVOR.COMPANY_ID = " + companyId + ") TVR\n");
		sql.append(" WHERE TVMG1.PARENT_GROUP_ID = TVMG2.GROUP_ID\n");
		sql.append("   AND TVMG2.PARENT_GROUP_ID = TVMG3.GROUP_ID\n");
		sql.append("   AND TVMG1.GROUP_ID = TVMGR.GROUP_ID\n");
		sql.append("   AND TVMGR.MATERIAL_ID = TVM.MATERIAL_ID\n");
		 ...
		 * TVMG3.GROUP_NAME SERIES_NAME,\n"); sql.append(" TVM.MATERIAL_ID,\n");
		 * sql.append(" TVM.MATERIAL_CODE,\n"); sql.append("
		 * TVM.MATERIAL_NAME,\n"); sql.append(" NVL(TVR.RESOURCE_AMOUNT, 0)
		 * RESOURCE_AMOUNT,\n"); sql.append(" TVM.MATERIAL_ID,\n"); sql.append("
		 * TVOD.ORDER_AMOUNT,\n"); sql.append(" TVOD.SINGLE_PRICE,\n");
		 * sql.append(" TVOD.TOTAL_PRICE,\n"); sql.append("
		 * NVL(TVOD.DISCOUNT_RATE, 0) DISCOUNT_RATE, --折扣率\n"); sql.append("
		 * NVL(TVOD.DISCOUNT_S_PRICE, 0) DISCOUNT_S_PRICE, --折扣后单价\n");
		 * sql.append(" NVL(TVOD.DISCOUNT_PRICE, 0) DISCOUNT_PRICE --折扣额\n");
		 * 
		 * sql.append(" FROM TM_VHCL_MATERIAL_GROUP TVMG1,\n"); sql.append("
		 * TM_VHCL_MATERIAL_GROUP TVMG2,\n"); sql.append("
		 ...
		 * TVM,\n"); sql.append(" TM_VHCL_MATERIAL_GROUP_R TVMGR,\n");
		 * sql.append(" TT_VS_ORDER_DETAIL TVOD,\n"); sql.append(" (SELECT
		 * TR.MATERIAL_ID, TR.RESOURCE_AMOUNT\n"); sql.append(" FROM
		 * TT_VS_RESOURCE TR\n"); sql.append(" WHERE TR.RESOURCE_YEAR = '" +
		 * year + "'\n"); sql.append(" AND TR.RESOURCE_WEEK = '" + week +
		 ...
		 * sql.append(" AND TVMG1.GROUP_ID = TVMGR.GROUP_ID\n"); sql.append("
		 * AND TVMGR.MATERIAL_ID = TVM.MATERIAL_ID\n"); sql.append(" AND
		 * TVM.MATERIAL_ID = TVR.MATERIAL_ID(+)\n"); sql.append(" AND
		 * TVM.MATERIAL_ID = TVOD.MATERIAL_ID\n"); sql.append(" AND
		 * TVOD.ORDER_ID = " + orderId); sql.append("ORDER BY TVOD.CREATE_DATE
		 * DESC, TVOD.DETAIL_ID\n");
		 */

		List<Map<String, Object>> list = pageQuery(sql.toString(), null, "com.infodms.dms.dao.sales.OrderReportDao.getSalesOrderDetailList");
		return list;
	}
	...
	public List<Map<String, Object>> getSalesOrderPreCheckDetailList(String orderId, String companyId, String dealerId) {
		...
		List<Map<String, Object>> list = pageQuery(sql.toString(), null, "com.infodms.dms.dao.sales.OrderReportDao.getSalesOrderPreCheckDetailList");
		return list;
	}
	...
	public List<Map<String, Object>> getOrderCheckList(String orderId) {
		...
		List<Map<String, Object>> list = pageQuery(sql.toString(), null, "com.infodms.dms.dao.sales.OrderReportDao.getOrderCheckList");
		return list;
	}
	 ...
	 * @return
	 */
	public PageResult<Map<String, Object>> getUrgentOrderPrepCheckList(String dealerId, String areaIds, int curPage, int pageSize) {
		...
		sql.append("           AND TSO.AREA_ID IN (" + areaIds + ")\n");
		sql.append("           AND TSO.ORDER_TYPE in (" + Constant.ORDER_TYPE_02 + "," + Constant.ORDER_TYPE_03 + ")\n");
		sql.append("           AND TSO.ORDER_STATUS = " + Constant.ORDER_STATUS_02 + "\n");
		...
		PageResult<Map<String, Object>> ps = pageQuery(sql.toString(), null, "com.infodms.dms.dao.sales.OrderReportDao.getUrgentOrderPrepCheckList", pageSize, curPage);
		return ps;
	}
	...
	public String getParentDealerId(String dealerId, Integer dealerLevel) {
		...
		List<Map<String, Object>> list = pageQuery(sql.toString(), null, "com.infodms.dms.dao.sales.OrderReportDao.getParentDealerId");
	...
	public List<Map<String, Object>> getGeneralOrderReportList(Map<String, Object> map) {
		...
		sql.append("   AND TVO.ORDER_STATUS = " + Constant.ORDER_STATUS_01 + "\n");

		List<Map<String, Object>> list = pageQuery(sql.toString(), null, "com.infodms.dms.dao.sales.OrderReportDao.getGeneralOrderReportList2");
		return list;
	}
	...
	public List<Map<String, Object>> getPriceList(Map<String, Object> map) {
		...
		// sql.append(" TDPRA.RELATION_ID\n");
		// sql.append(" FROM TT_VS_PRICE TVPA, TM_DEALER_PRICE_RELATION
		// TDPRA\n");
		...
		List<Map<String, Object>> list = pageQuery(sql.toString(), null, "com.infodms.dms.dao.sales.OrderReportDao.getPriceList");
		return list;
	}
	...
	public List<Map<String, Object>> getPriceListByReqId(Map<String, Object> map) {
		...
		/*
		 * sql.append("SELECT TVP.PRICE_ID, TVP.PRICE_CODE, TVP.PRICE_DESC,
		 * TDPR.IS_DEFAULT\n"); sql.append(" FROM TT_VS_PRICE TVP,
		 * TM_DEALER_PRICE_RELATION TDPR\n"); sql.append(" WHERE TVP.PRICE_ID =
		 * TDPR.PRICE_ID\n"); sql.append(" AND TVP.START_DATE < SYSDATE\n");
		 * sql.append(" AND TRUNC(TVP.END_DATE) >= TRUNC(SYSDATE)\n");
		 * sql.append(" AND TDPR.RELATION_ID NOT IN (\n"); sql.append(" SELECT
		 * TDPR3.RELATION_ID\n"); sql.append(" FROM TM_DEALER_PRICE_RELATION
		 * TDPR3\n"); sql.append(" WHERE TDPR3.IS_DEFAULT = "
		 * +Constant.IF_TYPE_NO+ "\n"); sql.append(" AND TDPR3.CREATE_BY =
		 * -1\n"); sql.append(")\n"); sql.append(" AND TVP.COMPANY_ID =
		 * "+companyId+"\n"); sql.append(" AND TDPR.DEALER_ID =
		 * "+dealerId+"\n"); sql.append("UNION\n"); sql.append("SELECT
		 * TVP2.PRICE_ID, TVP2.PRICE_CODE, TVP2.PRICE_DESC,
		 * TDPR2.IS_DEFAULT\n"); sql.append(" FROM TT_VS_PRICE TVP2,\n");
		 * sql.append(" TM_DEALER_PRICE_RELATION TDPR2,\n"); sql.append("
		 * TT_VS_DLVRY_REQ TVDR\n"); sql.append(" WHERE TVP2.PRICE_ID =
		 * TDPR2.PRICE_ID\n"); sql.append(" AND TVP2.PRICE_ID =
		 * TVDR.PRICE_ID\n"); sql.append(" AND TDPR2.RELATION_ID NOT IN (\n");
		 * sql.append(" SELECT TDPR4.RELATION_ID\n"); sql.append(" FROM
		 * TM_DEALER_PRICE_RELATION TDPR4\n"); sql.append(" WHERE
		 * TDPR4.IS_DEFAULT = " +Constant.IF_TYPE_NO+ "\n"); sql.append(" AND
		 * TDPR4.CREATE_BY = -1\n"); sql.append(")\n"); sql.append(" AND
		 * TDPR2.DEALER_ID = "+dealerId+"\n"); sql.append(" AND TVDR.REQ_ID =
		 * "+reqId+""); sql.append("UNION\n"); sql.append("SELECT
		 * TVPA.PRICE_ID,\n"); sql.append(" TVPA.PRICE_CODE,\n"); sql.append("
		 * TVPA.PRICE_DESC,\n"); sql.append(" TDPRA.IS_DEFAULT\n"); sql.append("
		 * FROM TT_VS_PRICE TVPA, TM_DEALER_PRICE_RELATION TDPRA\n");
		 * sql.append(" WHERE TVPA.PRICE_ID = TDPRA.PRICE_ID\n"); sql.append("
		 * AND TVPA.CREATE_BY = '-100'\n"); sql.append(" AND TVPA.COMPANY_ID = " +
		 * companyId + "\n"); sql.append(" AND TDPRA.DEALER_ID = "+ dealerId
		 * +"\n");
		 */
	 ...
	 * @return
	 */
	public List<Map<String, Object>> getReceiverListInNull(Map<String, Object> map) {
		String dealerId = (String) map.get("dealerId");
		String areaId = (String) map.get("areaId");
		String dealerClass = map.get("dealerClass").toString();

		StringBuilder sql= new StringBuilder();
		sql.append("SELECT TD.DEALER_ID, TD.DEALER_CODE||TD.DEALER_SHORTNAME DEALER_NAME FROM TM_DEALER TD ");
		sql.append("WHERE TD.DEALER_ID = ").append(dealerId);
		
		/*
		sql.append("select -1 dealer_id,'--请选择--' DEALER_NAME from dual\n" );
		sql.append("union all\n" );
		sql.append("SELECT distinct TcD.coop_dealer_id DEALER_ID, TD.DEALER_CODE||TD.DEALER_SHORTNAME DEALER_NAME\n" );
		sql.append("  FROM TM_DEALER TD, TM_DEALER_BUSINESS_AREA TDBA,tm_coop_dealer tcd\n" );
		...
		sql.append("   and tcd.coop_dealer_id=td.dealer_id ");
		*/
	...
	public List<Map<String, Object>> getReceiverList(Map<String, Object> map) {
		...
		sql.append("   and TcD.ds_DEALER_ID = " + dealerId + "\n");
		sql.append("   and tcd.coop_dealer_id=td.dealer_id order by DEALER_NAME");
	...
	public List<Map<String, Object>> getSinglePriceList(Map<String, Object> map) {
		...
		sql.append("SELECT TVM.MATERIAL_ID, F_GET_PRICE(" + priceId + ", TVM.MATERIAL_ID) PRICE\n");
		sql.append("  FROM TM_VHCL_MATERIAL TVM\n");
		sql.append(" WHERE 1 = 1\n");
		if (ids.equals("")) {
		...
		List<Map<String, Object>> list = pageQuery(sql.toString(), null, "com.infodms.dms.dao.sales.OrderReportDao.getSinglePriceList");
		return list;
	}
	...
	public List<Map<String, Object>> getDetailSinglePriceList(Map<String, Object> map) {
		...
		sql.append("SELECT TVOD.DETAIL_ID, F_GET_PRICE(" + priceId + ", TVOD.MATERIAL_ID) PRICE\n");
		sql.append("  FROM TT_VS_ORDER_DETAIL TVOD\n");
		sql.append(" WHERE 1 = 1\n");
		if (ids.equals("")) {
		...
		List<Map<String, Object>> list = pageQuery(sql.toString(), null, "com.infodms.dms.dao.sales.OrderReportDao.getDetailSinglePriceList");
		return list;
	}
	...
	public void insertTtVsDlvryReq(TtVsOrderPO po, Long reqId, Integer reqTotalAmount) {
		...
		tvdrp.setFleetId(po.getFleetId());
		tvdrp.setFleetAddress(po.getFleetAddress());
		...
		tvdrp.setDiscount(po.getDiscount());
		...
		String dlvryReqNo = null;
		/*
		if(new Integer("10201002").equals(po.getOrderType())) {
			dlvryReqNo = OrderCode.getDLVRY_BC(tvdrp.getFundType());
		}
		if(new Integer("10201003").equals(po.getOrderType())) {
			dlvryReqNo = OrderCode.getDLVRY_TS(tvdrp.getFundType());
		}
		if(po.getReceiver()==null||"".equals(po.getReceiver())||"-1".equals(po.getReceiver().toString())){
			dlvryReqNo = OrderCode.getDLVRY_DS(tvdrp.getFundType());
		}
		*/
		dlvryReqNo = OrderCode.getDLVRY_DS(tvdrp.getFundType());
	...
	public void insertTtVsDlvryReqDtl(TtVsOrderDetailPO po, Long reqId) {
		...
		tvdrdp.setDiscountRate(po.getDiscountRate());
		tvdrdp.setDiscountSPrice(po.getDiscountSPrice());
		tvdrdp.setDiscountPrice(po.getDiscountPrice());
		tvdrdp.setPriceId(po.getPriceListId());
		tvdrdp.setDealerReqAmount(po.getOrderAmount()); //update by zhulei on 2016-08-16 for 添加经销商原始申请数量
		dao.insert(tvdrdp);
	}
	...
	public List<Map<String, Object>> getStockList(Map<String, Object> map) {
		...
		sql.append("               TVM.MATERIAL_CODE,\n");
		sql.append("               TVM.MATERIAL_NAME,\n");
		sql.append("                (SUM(NVL(TVORR.UN_DLVRY_AMOUNT, 0))) AS RESERVE_AMOUNT\n");
		sql.append("          FROM  vw_vs_resource_entity_week TVORR, TM_VHCL_MATERIAL TVM\n");
...
//		sql.append("           AND TVORR.RESERVE_STATUS = " + Constant.RESOURCE_RESERVE_STATUS_01 + "\n");
		...
		sql.append("           AND TVORR.COMPANY_ID = " + companyId + "\n");
	...
	public Map<String, Object> chkAmount(Map<String, String> map) {
		...
		params.add(Constant.ORDER_STATUS_05);
	...
	public List<TmDealerPriceRelationPO> getDealerPriceRelation(String dealerId, String priceId) {
		...
		List<TmDealerPriceRelationPO> list = factory.select(sql, null, new POCallBack<TmDealerPriceRelationPO>(factory, TmDealerPriceRelationPO.class));// dao.select(TmDealerPriceRelationPO.class,
		// sql,null);
		return list;
	}
	...
	public static String viewOrderCount(String year, int month, String dealerId, String materialCode) {
		...
		sql.append("and o.order_status = " + Constant.ORDER_STATUS_05 + "\n");
