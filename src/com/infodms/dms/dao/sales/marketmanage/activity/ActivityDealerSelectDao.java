...
public class ActivityDealerSelectDao  extends BaseDao{

	public Logger logger = Logger.getLogger(UserManageDao.class);
	
	private static final ActivityDealerSelectDao dao = new ActivityDealerSelectDao();

	public static final ActivityDealerSelectDao getInstance() {
		return dao;
	}
	@Override
	 ...
	 * @return
	 */
	public PageResult<Map<String,Object>> activityDealerSelectQuery(Map<String,String> param,int curPage,int pageSize){
		PageResult<Map<String,Object>> list=null;
		...
		sql.append("  FROM TT_DEALER_ACTIVITY TPA,TM_DEALER TD\n" );
		sql.append(" WHERE TPA.DEALER_ID = TD.DEALER_ID\n");
		...
		if(null!=activityTheme&&!"".equals(activityTheme)){
	     ...
	     if(Constant.DUTY_TYPE_LARGEREGION.toString().equals(dutyType)){
	    	sql.append("AND TPA.DEALER_ID IN");
	    	 ...
	    	 sql.append("AND TPA.DEALER_ID IN("+dealerId+")");
	     }
	...
	public List<Map<String,Object>> getDealerActivityList(Map<String,String> map) throws Exception{
        ...
        sql.append(" WHERE TPA.DEALER_ID = TD.DEALER_ID \n");
        sql.append(" AND TC.CODE_ID = TPA.ACTIVITY_TYPE\n");
