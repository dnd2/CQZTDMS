 ...
 *  
 */
public class RuleDao2 extends BaseDao {
	public static Logger logger = Logger.getLogger(RuleDao2.class);
	private static final RuleDao2 dao = new RuleDao2 ();
	public static final RuleDao2 getInstance() {
		return dao;
	}
	/**
	 * 
	...
	* @throws
	...
	public List getLevelList(String type,Long oemCompanyId){
		...
		sql.append(" and tawa.type="+Constant.AUDIT_TYPE_02+"");
		if("1".equals(type)){
	...
	* @throws
	 */
	@SuppressWarnings("unchecked")
	public   PageResult<Map<String, Object>> ruleQuery(int pageSize, int curPage, String jsh,Long oemCompanyId) throws Exception {
		PageResult<Map<String, Object>> result = null;
		List list = dao.getLevelList("0",oemCompanyId);//获得授权级别列表
		StringBuffer sql= new StringBuffer();
		//查询列：从左至右：规则编号，索赔车型组代码，优先权值和对应的角色。
		sql.append(" select tawr.rule_element,tawr.prior_level,\n" );
		sql.append(" tawr.role \n" );
		...
		sql.append(" from TT_AS_WR_RULEMAPPING tawr  where tawr.TYPE ="+Constant.AUDIT_TYPE_02+"\n " );
		if(Utility.testString(jsh)){
			sql.append(" and tawr.prior_level like '%"+jsh+"%'");
		}
	...
	* @throws
	...
	public   HashMap ruleQueryById(String id,Long oemCompanyId) throws Exception {
		...
		sb.append(" select tawr.rule_element,tawr.prior_level, ");
		sb.append(" tawr.role ");
		for(int i = 0 ; i < list.size(); i++){
			HashMap tcpo = (HashMap)list.get(i);
		...
		sb.append(" from TT_AS_WR_RULEMAPPING tawr  where tawr.type= "+Constant.AUDIT_TYPE_02+" ");
