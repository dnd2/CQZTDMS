...
public class OutFeeQueryVehicleManagerDao extends BaseDao{
public static Logger logger = Logger.getLogger(OutFeeQueryManagerDao.class);
	
	private static final OutFeeQueryVehicleManagerDao dao = null;
	
	public static final OutFeeQueryVehicleManagerDao getInstance() {
	   if(dao==null) return new OutFeeQueryVehicleManagerDao();
	   return dao;
	}
	/**
	 * Function：查询特殊费用列表
	...
	public PageResult<SpeFeeQueryListBean> getSpeFeeQueryListInfo(Map params,int curPage, int pageSize){
		...
		sqlStr.append("select taws.id,taws.cr_id,td.dealer_code,td.dealer_shortname dealer_name,taws.fee_no,tawc.cr_no,\n" );
		...
		sqlStr.append("from tt_as_wr_speoutfee taws,tt_as_wr_cruise tawc,tc_code tc,tm_dealer td\n" );
		sqlStr.append("where taws.cr_id=tawc.id(+) and taws.status=tc.code_id and taws.dealer_id=td.dealer_id\n");
		...
		sqlStr.append("and taws.company_id="+company_id+"\n" );
