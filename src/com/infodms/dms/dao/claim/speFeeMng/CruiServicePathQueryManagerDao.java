...
public class CruiServicePathQueryManagerDao extends BaseDao{

    public static Logger logger = Logger.getLogger(CruiServicePathQueryManagerDao.class);
	
	private static final CruiServicePathQueryManagerDao dao = null;
	
	public static final CruiServicePathQueryManagerDao getInstance() {
	   if(dao==null) return new CruiServicePathQueryManagerDao();
	   return dao;
	}
	...
	public PageResult<QueryCruiInfoBean> queryCruiInfo(Map params,int curPage, int pageSize){
		...
		sqlStr.append("select tawc.id,'' dealer_code,'' dealer_name,tawc.cr_no,tc.code_desc status_desc,\n" );
		...
		sqlStr.append("from TT_AS_WR_CRUISE tawc,tc_code tc\n" );
