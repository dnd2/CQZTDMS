...
public class BillDetailTicketDao extends BaseDao {
	public static final Logger logger = Logger.getLogger(BillDetailTicketDao.class);
	public static BillDetailTicketDao dao = new BillDetailTicketDao();
	public static BillDetailTicketDao getInstance(){
		return dao;
	}
	@Override
...
public List<Map<String, Object>> getBillDetailTicketSelect(Map<String, String> map ){
	...
	sql.append("      sum( dcr.amount) BILLAMOUNT\n");  
	 
	sql.append("  from day_calculate_report dcr, TM_BUSINESS_AREA tba, vw_org_dealer vod\n");  
	sql.append(" where 1=1 AND dcr.data_type = 'BILL'\n"); 
	sql.append("and dcr.area_id=tba.area_id\n");
	sql.append("and vod.dealer_id=dcr.dealer_id\n");
	...
	if(!CommonUtils.isNullString(mydealerId)){
		sql.append("  AND (DCR.ROOT_DEALER_ID IN(").append(mydealerId).append(")\n");    
	}
	if(!CommonUtils.isNullString(mydealerId)){
		sql.append("  OR DCR.DEALER_ID IN(").append(mydealerId).append("))\n");    
	}
