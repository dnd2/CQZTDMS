...
public class ComplaintAcceptYxDao extends BaseDao{

	private static final ComplaintAcceptYxDao dao = new ComplaintAcceptYxDao();
	
	public static final ComplaintAcceptYxDao getInstance() {
		return dao;
	}
	...
	public PageResult<Map<String, Object>> queryWaitComplaintInfo(String user,String level,String vin,
			String name, String tele, String accuser, String dateStart,
			String dateEnd,String dealStart,String dealEnd,String status,
			String region,String pro,String state,String dealOrg,String dealDealer,String advanced, String model, Integer pageSize, Integer curPage) {
		StringBuffer sql = new StringBuffer();
		sql.append("select distinct customer.ctm_id CTMID,\r\n");
		sql.append("                complaint.CP_ID CPID,\r\n");
		sql.append("                complaint.cp_name CTMNAME,\r\n");
		sql.append("                complaint.CP_PHONE PHONE,\r\n");
		sql.append("                complaint.CP_NO CPNO,\r\n");
		sql.append("                to_char(complaint.CP_ACC_DATE, 'yyyy-MM-dd') ACCDATE,\r\n");
		sql.append("                accuser.name ACCUSER,\r\n");
		...
		sql.append("				complaint.UPDATE_DATE,\r\n ");
		sql.append("				complaint.CREATE_DATE, \r\n");
		sql.append("				complaint.is_huifang, \r\n");
		sql.append("				complaint.cp_deal_dealer, \r\n");
		sql.append("				complaint.cp_deal_org, \r\n");
		sql.append("				'"+user+"' as user_, \r\n");
		sql.append("				nvl(complaint.cp_point_status,'20231000') cp_point_status, \r\n");
		sql.append("				tdr.CD_ID, \r\n");
		sql.append("				ceil((sysdate - (SELECT max(cd_date)FROM TT_CRM_INCOMING_DEAL_RECORD WHERE cp_id = complaint.CP_ID)) * 24) time_out \r\n");
		...
		sql.append("  left join (select tt.*\r\n");
		sql.append("               from tt_crm_incoming_deal_record tt\r\n");
		sql.append("              inner join (select dr.cp_id, max(dr.create_date) cdate\r\n");
		sql.append("                           from tt_crm_incoming_deal_record dr\r\n");
		sql.append("                          group by dr.cp_id) bb\r\n");
		sql.append("                 on bb.cp_id = tt.cp_id\r\n");
		sql.append("                and bb.cdate = tt.create_date) tdr\r\n");
		sql.append("    on tdr.CP_ID = complaint.CP_ID\r\n"); 
		
		sql.append(" where complaint.CP_BIZ_TYPE in ("+Constant.TYPE_COMPLAIN+","+Constant.TYPE_CONSULT+") \r\n" );
		
		sql.append("  and complaint.CP_STATUS in("+state+") ");
		
		if(!XHBUtil.IsNull(dealOrg)){
			sql.append("  and complaint.CP_DEAL_ORG in("+dealOrg+") ");
		}
		if(!XHBUtil.IsNull(level)){
			if("24".equals(level)){
				sql.append("  and (complaint.cp_deal_org !=2014050994697313 and ceil((sysdate - (SELECT max(cd_date)FROM TT_CRM_INCOMING_DEAL_RECORD WHERE cp_id = complaint.CP_ID)) * 24)>24 and ceil((sysdate - (SELECT max(cd_date)FROM TT_CRM_INCOMING_DEAL_RECORD WHERE cp_id = complaint.CP_ID)) * 24)<=48)");
			}else{
				sql.append("  and (ceil((sysdate - (SELECT max(cd_date)FROM TT_CRM_INCOMING_DEAL_RECORD WHERE cp_id = complaint.CP_ID)) * 24)>48 and complaint.cp_deal_org !=2014050994697313)");
			}
		}
		if(!XHBUtil.IsNull(dealDealer)){
			sql.append("  and complaint.CP_DEAL_DEALER in("+dealDealer+") ");
		}
		...
		if(StringUtil.notNull(status)){
			sql.append(" and complaint.CP_point_status = "+status);
		}
		if(StringUtil.notNull(vin)){
			sql.append(" and complaint.CP_VIN like '%"+vin+"%'\n");
		}
...
//		}
	...
	public List<Map<String, Object>> queryDealRecord(long cpid) {
		...
		sql.append("  left join tc_user e on e.user_id = t.CP_RESPONSOR \r\n");
...
//		sbSql.append("       to_char(t.CL_DATE-to_date(to_char(complaint.CP_LIMIT + complaint.CP_TURN_DATE,'yyyy-MM-dd'),'yyyy-MM-dd')) DAYS,\r\n");
	 ...
	 * @return
	 */
	public List<Map<String, Object>> queryInComingRecordByCpId(String cpId){
		StringBuffer sql = new StringBuffer("select * from TT_CRM_INCOMING_DEAL_RECORD where 1=1");
	 ...
	 * @return
	 */
	public Map<String, Object> queryInComingByCpId(String cpId){
		StringBuffer sql = new StringBuffer("select * from TT_CRM_INCOMING_CALL where 1=1");
		List<Object> params = new ArrayList<Object>();
		if (!XHBUtil.IsNull(cpId)) {
			sql.append(" and cp_id = ?");
			params.add(cpId);
		}
		return this.pageQueryMap(sql.toString(), params, this.getFunName());
	}
	public Map<String, Object> getModelName(String cpid){
		StringBuffer sb= new StringBuffer();
		sb.append("\n" );
		sb.append("select t.CP_ID,t.cp_engine_no,\n" );
		sb.append("       t.cp_mileage,\n" );
		sb.append("       t.cp_name,\n" );
		sb.append("       t.cp_phone,\n" );
		sb.append("       TO_CHAR(t.cp_buy_date,'yyyy-mm-dd') as cp_buy_date,\n" );
		sb.append("       (select dealer_name from tm_dealer where dealer_id = t.cp_deal_dealer) as dealer_name,\n" );
		sb.append("       (select distinct t2.model_name\n" );
		sb.append("          from vw_material_group t2\n" );
		sb.append("         where t2.MODEL_ID = t.cp_model_id) as model_name\n" );
		sb.append("  from tt_crm_incoming_call t\n" );
		sb.append(" where 1=1");
		sb.append(" and t.CP_ID = "+cpid+"");

		return this.pageQueryMap(sb.toString(), null, this.getFunName());
	}
	
	public Map<String, Object> getAppInfo(String cpid){
		StringBuffer sb= new StringBuffer();
		sb.append("SELECT t.*, t1.dealer_name, t1.dealer_code,\n" );
		sb.append("       TO_CHAR(t.cp_buy_date,'yyyy-mm-dd') as cp_buy_date\n" );
		sb.append("  FROM Tt_Crm_Closed_App t, tt_crm_incoming_call t2, tm_dealer t1\n" );
		sb.append(" where t.cp_id = t2.cp_id\n" );
		sb.append("   and t2.cp_deal_dealer = t1.dealer_id");
		sb.append(" and t.ID = "+cpid+"");

		return this.pageQueryMap(sb.toString(), null, this.getFunName());
	}
	public List<Map<String, Object>> getFlow(String cpid){
		StringBuffer sb= new StringBuffer();
		sb.append("select (select code_desc from tc_code where code_id = recrod.audit_status) audit_status,\n" );
		sb.append("       to_char(recrod.create_date,'yyyy-mm-dd HH:MI:SS') as create_date,\n" );
		sb.append("       (select name from tc_user where user_id = recrod.create_by) as create_by\n" );
		sb.append("  from tt_crm_cloesd_audit_recrod recrod,\n" );
		sb.append("       Tt_Crm_Closed_App          App,\n" );
		sb.append("       tt_crm_incoming_call       t\n" );
		sb.append(" where recrod.app_id = app.id\n" );
		sb.append("   and app.cp_id = t.cp_id\n" );
		sb.append("   and t.cp_id = "+cpid+"\n" );
		sb.append(" order by create_date");

		return this.pageQuery(sb.toString(), null, this.getFunName());
	}
	public Map<String, Object> getAppInfoByCall(String cpid){
		StringBuffer sb= new StringBuffer();
		sb.append("SELECT t.*, t1.dealer_name, t1.dealer_code,\n" );
		sb.append("       TO_CHAR(t.cp_buy_date,'yyyy-mm-dd') as cp_buy_date\n" );
		sb.append("  FROM Tt_Crm_Closed_App t, tt_crm_incoming_call t2, tm_dealer t1\n" );
		sb.append(" where t.cp_id = t2.cp_id\n" );
		sb.append("   and t2.cp_deal_dealer = t1.dealer_id");
		sb.append(" and t2.cp_ID = "+cpid+"");

		return this.pageQueryMap(sb.toString(), null, this.getFunName());
	}
	
	public PageResult<Map<String, Object>> queryColesdAudit(String dealerCode,String appDate_start,String appDate_end,int type,Integer pageSize, Integer curPage) {
		StringBuffer sb= new StringBuffer();
		sb.append("SELECT t.*,t2.cp_no,t2.cp_cus_id as ctmid,t2.cp_id as cpid,(select name from tc_user where user_Id = t.create_by) as create_by_name, t1.dealer_name, t1.dealer_code\n" );
		sb.append("  FROM Tt_Crm_Closed_App t, tt_crm_incoming_call t2, tm_dealer t1\n" );
		sb.append(" where t.cp_id = t2.cp_id\n" );
		sb.append("   and t2.cp_deal_dealer = t1.dealer_id");
		sb.append("   and t.audit_status = 20231002");
		sb.append("   and t.SUBORDINATE = "+type+"");
		if(!CommonUtils.checkNull(dealerCode).equals("")){
			sb.append("   and t1.dealer_Code = '"+dealerCode+"'");
		}
		if(StringUtil.notNull(appDate_start)){
			sb.append(" and t.CREATE_DATE >=to_date('"+appDate_start+"','yyyy-MM-dd') ");
		}
		if(StringUtil.notNull(appDate_end)){
			sb.append(" and t.CREATE_DATE<=to_date('"+appDate_end+" 23:59:59','yyyy-MM-dd hh24:mi:ss') ");
		}
		return (PageResult<Map<String, Object>>)this.pageQuery(sb.toString(),
				null,
				this.getFunName(),
				pageSize,
				curPage);
