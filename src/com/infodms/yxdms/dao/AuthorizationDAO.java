...
public class AuthorizationDAO extends IBaseDao{
	...
	public List<Map<String, Object>> showOutStoreNum(RequestWrapper request) {
		...
		DaoFactory.getsql(sb, "d.producer_code", supply_code.toUpperCase(), 2);
		DaoFactory.getsql(sb, "dd.dealer_name", dealer_name, 2);
		DaoFactory.getsql(sb, "dd.dealer_code", dealer_code.toUpperCase(), 2);
		DaoFactory.getsql(sb, "d.part_code", part_code.toUpperCase(), 2);
		DaoFactory.getsql(sb, "d.part_name", part_name, 2);
		DaoFactory.getsql(sb, "d.claim_no", claim_no.toUpperCase(), 2);
		if(Utility.testString(in_start_date)){
			sb.append("  and  d.in_date>=to_date('"+in_start_date+" 00:00:00','yyyy-mm-dd hh24:mi:ss') \n");
		}
		if(Utility.testString(in_end_date)){
			sb.append("  and  d.in_date<=to_date('"+in_end_date+" 23:59:59','yyyy-mm-dd hh24:mi:ss') \n");
		}
		...
		DaoFactory.getsql(sb, "P.DOWN_PRODUCT_CODE", supply_code.toUpperCase(), 2);
		DaoFactory.getsql(sb, "d.dealer_name", dealer_name, 2);
		DaoFactory.getsql(sb, "d.dealer_code", dealer_code.toUpperCase(), 2);
		DaoFactory.getsql(sb, "p.DOWN_PART_CODE", part_code.toUpperCase(), 2);
		DaoFactory.getsql(sb, "p.DOWN_PART_NAME", part_name, 2);
		DaoFactory.getsql(sb, "a.claim_no", claim_no.toUpperCase(), 2);		
