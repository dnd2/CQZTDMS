...
//获取当月第一天(ranjian)
function firstDay(startDate){
	var firstDate = new Date();
	firstDate.setDate(1); //第一天
	document.getElementById(startDate).value=new XDate(firstDate).toString("yyyy-MM-dd")
}
//获取当月最后一天(ranjian)
function lastDay(endDate){
	var firstDate = new Date();
	firstDate.setDate(1); 
	var lastDate = new Date(firstDate);
	lastDate.setMonth(firstDate.getMonth()+1);
	lastDate.setDate(0);//最后一天
	document.getElementById(endDate).value=new XDate(lastDate).toString("yyyy-MM-dd")
}
//获取当天的日期【非服务器的时间】(ranjian)
function todayDate(dataName,formatType){
	var todayDate = new Date();
	document.getElementById(dataName).value=new XDate(todayDate).toString(formatType)
}