...
dTree.prototype.node = function (node, nodeId) {
	...
	if (this.config.useIcons) {
		...
		this.isSelect == "true" ? str += '<img id="ckk' + this.obj + nodeId + '" style="cursor: pointer;" onclick="javascript: ' + this.obj + '.chkclick(' + nodeId + ');" src="' + this.icon.unchecked + '" alt="" />' : "";
	}
	if (node.url) {
		if (this.myLink == "true") {
...
dTree.prototype.chkclick = function (nodeId) {
	...
	var ckimg = $('#ckk' + this.obj + nodeId)[0].src; //当前选择中状态
	if (iscontains(ckimg, "unchecked.gif") || iscontains(ckimg, "square.gif")) { //若为全选或部分选择则改为选中状态
		$('#ckk' + this.obj + nodeId)[0].src = this.icon.checked;
		this.modifclick(nodeId, "checked");
	} else { //改为未选中状态
		$('#ckk' + this.obj + nodeId)[0].src = this.icon.unchecked;
	...
	if (funID != "" && funID.indexOf(",") != -1) {
		...
		for (var i = 0; i < sp.length; i++) {
			var nodeId = this.getNodeIdByFunId(sp[i]),
				elem = $('#ckk' + this.obj + nodeId)[0];
			if (elem && elem.src) { // 检查对象是否存在
				src = this.icon.checked;
				this.modifclick(nodeId, "checked");
			}
		}
	} else {
	...
	if ("checked" == st) { //选中
		...
		for (var n = 0; n < this.aNodes.length; n++) {
			if (this.aNodes[n].id.indexOf(funid) == 0) { //是其子节点
				$('#ckk' + this.obj + this.aNodes[n]._ai)[0].src = this.icon.checked;
			}
		}
		//处理所有父节点
		var tfunid = funid;
		while (tfunid.length > 2) {
			...
			for (var n = 0; n < this.aNodes.length; n++) {
				if (this.aNodes[n].id.indexOf(tfunid) == 0 && this.aNodes[n].pid == tfunid && iscontains($('#ckk' + this.obj + this.aNodes[n]._ai)[0].src, "unchecked.gif")) { //其父节点的其它子节点有未选中的
			...
			for (var n = 0; n < this.aNodes.length; n++) {
				if (this.aNodes[n].id.indexOf(tfunid) == 0) {
					if (!stt) {
						$('#ckk' + this.obj + this.aNodes[n]._ai)[0].src = this.icon.square;
						break;
					} else {
						$('#ckk' + this.obj + this.aNodes[n]._ai)[0].src = this.icon.checked;
						break;
		...
		for (var n = 0; n < this.aNodes.length; n++) {
			if (this.aNodes[n].id.indexOf(funid) == 0) { //是其子节点
				...
				$('#ckk' + this.obj + this.aNodes[n]._ai)[0].src = this.icon.unchecked;
			}
		}
		//处理所有父节点
		var tfunid = funid;
		while (tfunid.length > 2) {
			...
			var stt = true; // true: checked , false : square
			for (var n = 0; n < this.aNodes.length; n++) {
				if (this.aNodes[n].id.length == lp && this.aNodes[n].pid == tfunid && (iscontains($('#ckk' + this.obj + this.aNodes[n]._ai)[0].src, "/checked.gif") || iscontains($('#ckk' + this.obj + this.aNodes[n]._ai)[0].src, ("square.gif")))) {
					stt = false; //设置其图标为部分选中
					break;
				}
			}
...
var TreeFunc = {
	jDom: '',
	cols: { id: '', name: '', parent: '' }, // 指定对应列名
	params: {isSelect: 'false', isAll: 'false', link: 'true', nodeUrl: '', myLink: "true"}, // 树初始化参数
	loadData: function (options, callback) {
		this.jDom = options.dom || $('#dtree');  	 		// 默认获取id为dtree的元素
		options.ajax = options.ajax || sendAjax; 	 		// 发送异步请求的方法
		options.formName = options.formName || 'fm'; 		// 指定表单id名
		options.rootPath = options.rootPath || '/CQZTDMS';  // 指定加载图片路径
		this.cols = options.cols || this.cols;				// 显示节点数据的对应字段
		TreeFunc.params.nodeUrl = options.nodeUrl || '';	// 节点上增加链接
		TreeFunc.params.myLink  = options.myLink || 'true'; // 节点上是否增加链接
		TreeFunc.params.myFun   = options.myFun  || '';     // 指定点击链接触发的自定义方法

		this.jDom.html('<img src="' + options.rootPath + '/img/tree/loading.gif" />载入中...');
		options.ajax(options.url, function( response ) {
			var tree = TreeFunc.createTree( response );
			callback( tree );
		}, options.formName);
	},

	createTree: function ( reobj ) {
		TreeFunc.jDom.empty();
		var tree = new dTree('tree', 'dtree', TreeFunc.params.isSelect, TreeFunc.params.isAll, TreeFunc.params.myLink);

		tree.config.folderLinks = true;
		tree.config.closeSameLevel = false;

		if ( TreeFunc.params.myFun ) {
			tree.config.myfun = TreeFunc.params.myFun;
		}

		tree.delegate = function (id) {
			addNodeId = tree.aNodes[id].id;
			var nodeID = tree.aNodes[id].id;
			for (var n = 0; n < tree.aNodes.length; n++) {
				if (tree.aNodes[n].id.contains(addNodeId + "01")) {
					return;
				}
			}
		}
		...
		if (funlistobj.length <= 0) {
			return tree;
		}

		var idIndex = funlistobj[0][TreeFunc.cols['id']] || '',
			nameIndex = funlistobj[0][TreeFunc.cols['name']] || '';

		if (idIndex && nameIndex) {
			for (var i = 0; i < funlistobj.length; i++) {
				var id = funlistobj[i][TreeFunc.cols['id']],
					name = funlistobj[i][TreeFunc.cols['name']],
					parent = funlistobj[i][TreeFunc.cols['parent']];

				if (parent == 0) {
					tree.add( id, '-1', name, TreeFunc.params.nodeUrl );
				} else {
					tree.add( id, parent, name, TreeFunc.params.nodeUrl );
				}
			}
		}
