 ...
 */
 ...
 var columns2 = [
				...
				{header: "选择", dataIndex: 'seriesCode', align:'center',width: '33px' ,renderer:seled},
				{header: "产品编码", dataIndex: 'materialCode', align:'center'},
				{header: "产品名称", dataIndex: 'materialName', align:'center'}
		      ];
 
 function seled(value,meta,record) {
 	return "<input type='radio' onclick='selectDealer("+value+",\""+record.data.materialCode+"\",\""+record.data.materialName+"\")' name='de' id='"+value+"' />";
 }
 
 function selectDealer(did,code,sname) {
	 alert("selectDealer==="+code+" sname=== "+sname);
	 alert($('#Material_Code'));
	 $('#Material_Code').val(code);
	 $('#Material_Name').val(sname);
	 alert($('#Material_Code').val())
 	 closePan();
 }
...
/*============================================分页=================================================*/
	...
	showPages2.prototype.createHtml = function(){ //生成html代码
		...
		//strHtml += createChangeHeightNode('myGrid2');
		// =============表格缩放功能end by chenyu==================================
...
//		}
	...
	showPages2.prototype.formatInputPage = function(evt){ //限定输入页数格式
		...
		if(evt.keyCode == 13 && $("pageInput2").value>=1 && $("pageInput2").value<=this.pageCount){
			getInvoidMaterialInfo($("pageInput2").value);	
		}
...
/*********************************/		
	...
	function getInvoidMaterialInfo(page,Code,Name){		
		alert("getInvoidMaterialInfo");
		productCode = Code;
		productName	=Name;
		showMask();
		submitForm('fm2') ? sendAjax(drlurl+(drlurl.lastIndexOf("?") == -1?"?":"&")+"curPage2="+page,callBack2,'fm2') : (removeMask());
	}
