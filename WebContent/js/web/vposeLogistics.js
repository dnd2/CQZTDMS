...
window.addEvent('domready', function(){
	$('zwlb').setHTML(getItemValue($('POSE_TYPE1').value));
	if($('POSE_TYPE1').value == "10021001") {
...
function modfiPose(obj) {
	...
	//	return false;
	//}
	submitForm('fm') ? sendAjax(modfiPoseUrl+"?POSE_BUS_TYPE1="+$('POSE_BUS_TYPE1').value,addPoseBack1,'fm') : "";
...
function addPose1(obj) {
	...
	if(submitForm('fm')) {
		...
		//} else {
			sendAjax(addPoseUrl+"?DEPT_ID="+$('DEPT_ID').value+"&POSE_TYPE1="+$('POSE_TYPE1').value
					+"&POSE_BUS_TYPE1="+$('POSE_BUS_TYPE1').value,addPoseBack1,'fm');
		//}
	}
 }
function toGoPositionSearch() {
 ...
 function addPoseBack1(json) {
	 if(json.st == "poseCode_error") {
			...
			MyAlert("车厂组织不存在，无法维护物流商信息!") ;
			return ;
		}else if(json.st == "logiCity_error") {
			MyAlert("添加失败，该地市已有物流商管理!") ;
			return ;
		}else {
			toGoPositionSearch();
		}
...
function showDataAuth() {
	...
	//sendAjax(getDataAuthUrl+"?type="+$('POSE_TYPE1').value,showDataAuthBack,'fm');
}
...
function addRole(path) {
	...
	OpenHtmlWindow(path+"/sysmng/sysposition/SysPosition/addSysPositionRoleInit.do?poseType="+$('POSE_TYPE1').value,800,600);
}
