...
|--------------------------------------------------*/
// Node object
function Node(id, pid, name,poseLevel, url, title, target, icon, iconOpen, open) {
	this.id = id;
	this.pid = pid;
	this.name = name;
...
dTree.prototype.add = function(id, pid, name,poseLevel ,url,title, target, icon, iconOpen, open) {
	this.aNodesData[this.aNodesData.length] = new Node(id, pid ,name,poseLevel ,url,title, target, icon, iconOpen, open);    
};
...
dTree.prototype.modifclick = function(nodeId,st) {
	...
	var funid = this.aNodes[nodeId].poseLevel;
	if("checked" == st) { //选中
		//处理所有子节点
		for(var n=0; n<this.aNodes.length; n++) {
			if(this.aNodes[n].poseLevel.indexOf(funid) == 0) { //是其子节点
				$('ckk' + this.obj + this.aNodes[n]._ai).src = a.icon.checked;
			}
		}
		//处理所有父节点
		var tfunid = funid;
		while(tfunid.length>16) {
			tfunid = tfunid.substring(0,tfunid.length-16);
			var lp = tfunid.length+16;
			var stt = true; // true: checked , false : square
			for(var n=0; n<this.aNodes.length; n++) {
				if( this.aNodes[n].poseLevel.length == lp && this.aNodes[n].poseLevel.indexOf(tfunid) == 0 && ($('ckk' + this.obj + this.aNodes[n]._ai).src.contains("unchecked.gif")||$('ckk' + this.obj + this.aNodes[n]._ai).src.contains("square.gif"))) { //其父节点的其它子节点有未选中的
					stt = false; //设置其图标为部分选中
					break;
				}
			}
			for(var n=0; n<this.aNodes.length; n++) {
				if(this.aNodes[n].poseLevel.indexOf(tfunid) == 0) {
					if(!stt) {
		...
		while(tfunid.length>16) {
			...
			var lp = tfunid.length+16;
			for(var n=0; n<this.aNodes.length; n++) {
				if( this.aNodes[n].poseLevel.length == lp && this.aNodes[n].poseLevel.indexOf(tfunid) == 0 && ($('ckk' + this.obj + this.aNodes[n]._ai).src.contains("/checked.gif") || $('ckk' + this.obj + this.aNodes[n]._ai).src.contains("square.gif"))) {
					stt = false; //设置其图标为部分选中
					break;
				}
			}
...
dTree.prototype.draw = function(){	
	...
	for(var i=0 ; i<this.aNodesData.length ; i++){		
		...
		this.aNodes[i]=new Node(oneNode.id,oneNode.pid,oneNode.name,oneNode.poseLevel,oneNode.url,oneNode.title,oneNode.target,oneNode.icon,oneNode.iconOpen,oneNode.open);	}	
	this.rewriteHTML();
}
