 ...
 */
 ...
 var columns2 = [
				...
				{header: "选择", dataIndex: 'ED_ID', align:'center',width: '7%' ,renderer:seled},
				{header: "问题名称", dataIndex: 'ED_QUESTION',width: '20%' , align:'center'},
				{header: "问题类型", dataIndex: 'ED_QUE_TYPE', align:'center'},
				{header: "问题选项", dataIndex: 'ED_CHOICE', align:'center',renderer:setnull},
				{header: "所属题库", dataIndex: 'EX_NAME', align:'center'}
		      ];
 ...
 function seled(value,meta,record) 
 {
 	...
 	if(sIsMulti == "false")
 		return "<input type='radio' onclick='singleSelect("+value+",\""+record.data.ED_ID+"\",\""+record.data.ED_QUESTION+"\")' name='de' id='"+value+"' />";
    else
        return "<input type='checkbox' onclick='multiSelect("+value+",\""+record.data.ED_ID+"\",\""+record.data.ED_QUESTION+"\")' name='"+value+"' id='"+value+"' />";
    
 }
 function setnull(value,meta,record) 
 {
 	//added by andy.ten@tom.com 增加多选
 	if(null==record.data.ED_CHOICE||record.data.ED_CHOICE == "")
 	{	return "<div>&nbsp;&nbsp; </div>";
 	}else
 	{
 		return "<div>"+record.data.ED_CHOICE+" </div>";
 	}
 }
...
function doConfirm(qrId)
{
	...
	parentContainer.setQuestions(ids,qrId);//回调编辑页面JS函数进行问题添加。
	_hide();
	closePan();
}
