...
s += '<div style="width:200px;"><table id="cldTab" border="0" class="tableborder" cellspacing="0" cellpadding="0" width="100%" style="text-align: center">';
s += '<tr align="center"><td class="header"><a href="#" style="color:#08327e;" onclick="refreshcalendar(yy, mm-1);return false" title="上一月"><<</a></td><td colspan="5" style="text-align: center" class="header"><a id="year" style="color:#08327e;" title="点击选择年份" href="#" onclick="showdiv(\'year\');_cancelBubble(event);return false"></a><span style="color:#08327e;">-</span> <a id="month" style="color:#08327e;" title="点击选择月份" href="#" onclick="showdiv(\'month\');_cancelBubble(event);return false"></a></td><td class="header"><a style="color:#08327e;" href="#" onclick="refreshcalendar(yy, mm+1);return false" title="下一月">>></a></td><td class="header">&nbsp;</td></tr>';
s += '<tr class="category"><td>周</td><td>日</td><td>一</td><td>二</td><td>三</td><td>四</td><td> 五</td><td>六</td></tr>';
for(var i = 0; i < 6; i++) {
   s += '<tr class="altbg2">';
   for(var j = 0; j <= 7; j++){
...
s += '<tr><td height="20" colspan="8" style="border-top:1px solid #CCC;background:#f7f7f7;"><a id="clearBtn" href="#" onclick="$(\'calendar\').style.display = \'none\';$(tmp).value=\'\';" >清 空</a></td></tr>';
...
s += '<tr id="hourminute"><td colspan="8" style="border-top:1px solid #CCC; background:#E4D7C2" align="center" height="25">时间： <select id="hour" style="width:40px;">';
...
for(var k = 1920; k <= 2079; k++) {
   s += k != 1920 && k % 10 == 0 ? '</div><div class="col">' : '';
...
function setWeeks(y,d){
	document.getElementById('calendar').style.display = 'none';
	if(d < 10){ d = '0'+d;}
	document.getElementById(controlid).value = String(y)+String(d);	
}
...
for(var i = 2; i < 8; i++){//设置周数
	wek = parseInt(getYearWeek(yy, p, 1)) + (i-2);
	tp = String(yy) + String(wek);
	ff.rows[i].cells[0].innerHTML = '<a href="###" onclick="setWeeks('+ yy +','+ wek + ');return false">' + tp + '</a>';
}

while(x.getMonth() == mm) {
   dd = document.getElementById("d" + (d + mv));
   dd.innerHTML = '<a return false">' + d + '</a>';
...
function showTime(event,controlid1,is) {
	...
	t += '<a id="sureBtn" href="#" onclick="$(\'timeDiv\').style.display = \'none\';setTimes()" style="display:block; background:#0066cc; color:#fff; padding:3px; border:1px solid #FFF;" >确 定</a>';
