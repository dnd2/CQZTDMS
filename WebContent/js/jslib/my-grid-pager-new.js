...
function movespan(obj) {
	...
	document.onmousemove = function() {
		if (dragMoveable) {
			...
			var tmpxy = document.getElementById("tmpxy");
			var xys = new Array();
			var txy = '';
			tmpxy.value = event.x+":"+event.y;
			// 高显需要被替换的列头
			for (var i = 0; i < oTable.cells.length; i++) {
				var overCell = oTable.cells[i];
				if (overCell.tagName.toLowerCase() == "th"&&overCell.innerHTML.toLowerCase().indexOf('img')>-1) {
					pos = getPos(overCell);
					if(txy!=pos[1]+":"+pos[0]){
						txy = pos[1]+":"+pos[0];
						xys[i] = txy;
					...
					if (event.x > pos[1]
							&& event.x < pos[1] + overCell.offsetWidth
							&& event.y > pos[0]
							&& event.y < pos[0] + overCell.offsetHeight) {
			...
			tmpxy.value = txy+";;"+event.x+":"+event.y+";;"+document.parentWindow;
		}
	}
...
// 获取单元格的值
function getCellValue(_cell){
	var _r;
	// 子节点
	var chns = _cell.childNodes;
	// 子节点个数
	var chnLen = chns.length;
	if (chnLen > 0) {
		var firstChn = chns[0];
		
//		if(firstChn&&firstChn.nodeType==1&&firstChn.tagName.toLowerCase()=='span'){
//			_r = _cell.firstChild.firstChild.nodeValue;
//		}else {
//			_r = _cell.firstChild.nodeValue;
//		}
	}
	return _r;
}
...
// ====================表格页面排序end by chenyu===================
  ...
  /* 增加String的format方法 */
    ...
    {
    	dragDivInit();
    	tabArr.push("<table id='myTable' class='table_list' style='border-bottom:1px solid #DAE0EE;tableLayout:fixed' ondrag='return false;'>");
    	//生成合并表头
		var rowspan;
	    	...
	    	for(var i=0; i< this.columns.length; i++)//列名
			{
				...
				if(!columnObj.width)
				{
					if(n==1)
					{
					    ...
					    {
					    	var tableTh = "<th class='"+ colMask + "' onselectstart='return false;' ";
					    	// =================增加表格排序拖动的功能start by chenyu================
					    	var _operate = true ;
					    	for(var idx = 0;idx<withoutHeader.length;idx++){
					    		wh = withoutHeader[idx];
					    		if(columnObj.header.toLowerCase().indexOf(wh)>-1){
					    			_operate = false;
					    			break;
					    		}
					    	}
					    	if(_operate){
					    		tableTh += " onmousedown='movespan(this);'";
					    	}
					    	tableTh += ">";
			    			if(_operate){
				    			// 调整列宽标记
				...
				{	
					if(n==1)
					{
					    ...
					    {
					    	var tableTh = "<th class='"+ colMask + "' >" + columnObj.header ;
						    // =================增加保存用户选中记录的功能start by chenyu===========
				    		// 在全选框后面增加一个隐藏域,存用户选中的复选框value
				    		if(hasSelectAll(columnObj.header)){
				    			tableTh += createHideCheckedNode();
...
//				   break;
			   ...
			   // ==================自定义每页条数开关start by chenyu===================
			   if(keys[i] == "customPageSizeFlag"){
				   customPageSizeFlag = json[keys[i]];
			   }
			   // ==================自定义每页条数开关end by chenyu=====================
			   // ==================列头排序开关start by chenyu@yonyou.com=============
			   // ==================列头排序开关end by chenyu@yonyou.com===============
			   // ==================列宽调整开关start by chenyu@yonyou.com=============
			   // ==================列宽调整开关end by chenyu@yonyou.com===============
			}
