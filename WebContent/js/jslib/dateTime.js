...
/*******************************************************************************
 * 判断字符串strDateTime是否为一个正确的日期时间格式： yyyy-M-d H:m:s或yyyy-MM-dd HH:mm:ss 时间采用24小时制
 ******************************************************************************/
...
function strToDate(str) {
	if (str.indexOf(':') > -1) {
		...
		var re = /^(\d{4})-(\d{1,2})-(\d{1,2})\s*(\d{1,2}):(\d{1,2}):(\d{1,2})$/
			.exec(str.trim());
		var _time = new Date(re[1] - 0, (re[2] || 1) - 1, (re[3] || 1) - 0,
				(re[4] || 0) - 0, (re[5] || 0) - 0, (re[6] || 0) - 0);
		return _time;
	} else {
		var re = /^(\d{4})-(\d{1,2})-(\d{1,2})$/.exec(str.trim());
		var _time = new Date(re[1] - 0, (re[2] || 1) - 1, (re[3] || 1) - 0,
				0 - 0, 0 - 0, 0 - 0);
		return _time;
	}
