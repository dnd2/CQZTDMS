...
<%-- <input id="index" type="hidden" value="'<%=request.getParameter("dealerId")%>'" /> --%>
<table width='100%'>
	<tr>
		<td  nowrap="nowrap"><a href="" target="_blank"></a>
			<iframe id="upIframe" name="upIframe" src="<%=path%>/newCommonUploadIF.jsp" width='100%' height=80  scrolling="no" style="overflow-x:hidden;overflow-y:visible;" frameborder="0"></iframe>
		</td>
	</tr>
</table>
...
<script type="text/javascript">
	function upload(){
		...
		$('fm').action = "<%=path %>/common/NewsFileUploadAction/fileUpload.do";
    	$("fm").submit();
	}
...
function setBackValue(){
  ...
  try{
    ...
    for(var i=1;i<rows.length;i++){
    	 crow=rows[i];
         parentContainer.addUploadRow(crow,index);
