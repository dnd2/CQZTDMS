...
var areaId="<%=areaId%>";
var drlurl = "<%=contextPath%>/sysmng/usemng/SgmDealerSysUser/allDrlQueryByNews.json?COMMAND=1&ORGID="+orgId+"&AREA_ID="+areaId+"&isDealerType="+isDealerType;
...
{	var inputNames = document.getElementById("DEALER_NAME").value;
	...
	if(dealerCode && dealerCode.length > 0)
	{
		...
		if (parent.$('inIframe')) {   
		    var dealers = parentDocument.getElementById(inputCode).value;
		    if(dealers != null && dealers != '' ){ // 陈力需求-把经销商累加上去 YH 2011.3.24
			 parentDocument.getElementById(inputCode).value = dealers +','+ document.getElementById("DEALER_CODE").value;
		    }else {
	...
	<div id='pan' style="z-index: 3000;position:absolute;border:1px solid #5E7692;background: #FFFFFF; width: 715px;height: 379px;">
		<div id='myquery' style="z-index: 3001;position:absolute;border:1px solid #5E7692;width: 715px;height: 60px;">
			...
			<tr>
				...
				<td class="table_query_3Col_label_5Letter" nowrap="nowrap">经销商等级：</td>
				<td class="table_query_3Col_input" nowrap="nowrap">
