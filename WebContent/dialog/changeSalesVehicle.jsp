...
var parentDocument =parentContainer.document;
	...
	var url = "<%=contextPath%>/changeSales/changeSalesManage/ChangeSalesDealer/queryChangeSalesVehicle.json";
	//设置表格标题
	var title= null;
	//设置列名属性
	var columns = [
	...
	function add() {
			...
			for(var i=0; i<objarr.length; i++)
			{
				if(objarr[i].checked == true) 
				{
					...
					modelCodeTemp.push(objarr[i].modelCode);
			...
			parentContainer.returnValue(vehicleIdTemp,modelNameTemp,modelIdTemp,invoiceNoTemp,statusinvdateTemp,orderIdTemp,orderNoTemp,orderIdTemp,channelTemp,vinTemp,normalPriceTemp,flagTemp,customeCodeTemp,modelCodeTemp);
		_hide();
	}
</script>
</html>