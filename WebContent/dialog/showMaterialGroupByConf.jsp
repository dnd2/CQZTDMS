...
var tree_url = "<%=ctx%>/materialGroup/MaterialGroupTree/materialGroupQueryByConf.json";
var inputId="<%=inputId%>";
...
<div id='dtree' class="dtree" style=" margin:2px;float:left;border-right:solid 1px;">
        ...
        a = new dTree('a','dtree','false','false','true',<%=groupLevel%>,'<%=ctx%>/materialGroup/MaterialGroupTree/querySubMaterialListByConf.json');
        </script>
    </div>
	 ...
	 <form id="fm" name="fm" method="post">
		...
		<input type="hidden" id="GROUPLEVEL2" name="GROUPLEVEL2" value="4" />
		<input type="hidden" id="areaId" name="AREAID" value="<%=areaId%>" />
		<input type="hidden" name="tree_root_id" id="tree_root_id" value=""/>
		<table class="table_query" >
			<tr>
				<td nowrap="nowrap" class="table_query_label">配置代码：<input 
					id="groupCode" name="groupCode" class="middle_txt" type="text" /></td>
				<td class="table_query_label" nowrap="nowrap" >配置名称：<input 
...
var url = "<%=ctx%>/materialGroup/MaterialGroupTree/groupListQueryByConf.json";
//设置表格标题
var title= null;
//设置列名属性
...
var columns = [
				...
				{header: "配置代码",dataIndex: 'GROUP_CODE',width:"40px"},
				{header: "配置名称",dataIndex: 'GROUP_NAME',width:"50px"}
		      ];
