...
<!-- created by lishuai103@yahoo.com.cn 20100603 通用选择配件大类 -->
...
<form method="post" name ="fm" id="fm">
    ...
    <tr>
      <td class="table_query_3Col_label_6Letter" nowrap="nowrap">配件大类代码：</td>
      <td class="table_query_3Col_input" nowrap="nowrap">
      	<input class="middle_txt" id="suppCode" name="suppCode" value="" type="text"/>
      </td>
      <td class="table_query_3Col_label_6Letter" nowrap="nowrap">配件大类名称：</td>
      <td>
...
<script type="text/javascript" >
	...
	var url = "<%=contextPath%>/sysmng/usemng/SgmDealerSysUser/allPartTypeQuery.json";
				
	var title = null;

	var columns = [
				{header: "选择", dataIndex: 'id', align:'center',renderer:seled},
				{header: "配件大类代码", dataIndex: 'parttypeCode', align:'center'},
				{header: "配件大类名称", dataIndex: 'parttypeName', align:'center'}
		      ];
		      
	function seled(value,meta,record){
		var isMulti = "<%=isMulti%>";
 		if(isMulti == "false"){
 			return "<input type='radio' onclick='singleSelect("+value+",\""+record.data.parttypeCode+"\")' name='checkCode' />";
 		}else{
        	return "<input type='checkbox' name='checkCode' id='checkCode' value='"+ value + "||" +record.data.parttypeCode+"' />";
        }
    }
