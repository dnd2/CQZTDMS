...
var url = "<%=ctx%>/materialGroup/MaterialGroupTree/groupListQueryPlanSet.json";
//设置表格标题
var title= null;
//设置列名属性
...
function myLink(value,meta,rec){
	var data = rec.data;
	var val = data.GROUP_ID + "," + data.GROUP_CODE + "," + data.GROUP_NAME;
	if($('ISMULTI').value=='true')
	{
		
		return "<input type='checkbox' name='cb' id=\""+data.GROUP_ID+"\"  value='"+val+"' />"
    }else
    {
...
function setCheckModel(){
	...
	for(var i=0;i<groupCheckBoxs.length;i++)
	{
		if(groupCheckBoxs[i].checked)
		{
			if(reCode && reCode.length > 0) {
				var temp = groupCheckBoxs[i].value;
				var attr = temp.split(",");
	...
	if(parent.$('inIframe'))
	{
		parentContainer.addPackage(reId,reCode,reName);
		_hide();	
	}else
	{
		parent.contentWindow.addPackage(reId,reCode,reName);
		parent._hide();
	}
