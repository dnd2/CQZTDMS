...
<div id='dtree' class="dtree" style=" margin:2px;float:left;border-right:solid 1px;width: 20%;height: 320px;overflow: auto;">
        <script type="text/javascript">
        a = new dTree('a','dtree','false','false','true');
        </script>
</div>
	 <div style="float:left;width: 79%;height: 320px;overflow: auto;">
		...
		<table class="table_query" >
			<tr>
				<td nowrap="nowrap" class="table_query_label">组织代码：<input 
					id="orgCode" name="orgCode" class="middle_txt" style="width: 130px;" type="text" /></td>
				<td class="table_query_label" nowrap="nowrap" >组织名称：<input 
					id="orgName" name="orgName" style="width: 130px;" class="middle_txt" type="text" /></td>
				 <td  align="right"><input name="button2" type="button" style="width: 45px;" class="cssbutton" id="queryBtn" onclick="goToSelect();" value="查 询"/>
                  <input name="button" type="button" style="width: 45px;" class="cssbutton" onclick="_hide();" value="关 闭" /></td>
		    </tr>
		    </table>
...
<script type="text/javascript">
<!--
var url = "<%=ctx%>/sysmng/usemng/SgmDealerSysUser/allOrgQuery.json?COMMAND=1&ORGID="+ORGID;
//设置表格标题
var title= null;
//设置列名属性
...
function myLink(value,meta,rec){
	...
	if($('ISMULTI').value=='true')
	{
		...
		return "<input type='checkbox' name='cb' id=\""+data.orgId+"\"  value=\""+data.orgCode+"\" /><input type='hidden' name='cc' id=\""+data.orgId+"\"  value=\""+data.orgId+"\" /><input type='hidden' name='theNames' id='" + data.orgId + "' value='" + data.orgName + "' />"
    }else
    {
...
var setModel = function (orgId,orgName,orgCode){
	...
	if(theName && theName.length > 0 && theName != 'null') {
		parentDocument.getElementById(theName).value=orgName;
		//parentDocument.getElementById(theName).value = orgName ;
	}
	_hide();
}
function setCheckModel(){
	...
	//修复不在inIframe中时参数赋值问题 2012-02-06
	if(parent.$('inIframe') == null) {
		if(top.$(inputId)) {
			top.$(inputId).value = reCode;
		}
	} else {
		//parentDocument.getElementById(theName).value=orgName;
