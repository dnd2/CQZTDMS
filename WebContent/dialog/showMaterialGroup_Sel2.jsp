...
var url = "<%=ctx%>/materialGroup/MaterialGroupTree/groupListQuery_Sel.json?area_id="+area_id;
//设置表格标题
var title= null;
//设置列名属性
...
function setCheckModel(){
	...
	if(!reCode){
		...
		MyDivConfirm("是否提交?",addMaterialGroupAction,[reCode]);
	}
}
...
function addMaterialGroupAction(groupId){
	if(area_id){
		...
		var url="<%=ctx%>/sysbusinesparams/businesparamsmanage/BusinessAreaManage/addMaterialGroup.json?area_id="+area_id+"&groupId="+groupId;
		sendAjax(url,function(json){
			if(json.returnValue=='1'){
				parentContainer.showResult();
				parent._hide();
			}else{
				MyDivAlert("你选择的车系已被其它产地选择,请检查!");
			}
		},'fm');
		
	}else{
		parentContainer.showGroupId(groupId);
		parent._hide();
	}
}
function clsTxt(){
