...
<div id='dtree' class="dtree" style=" margin:2px;float:left;border-right:solid 1px;">
	 ...
	 <form id="fm" name="fm" method="post">
		...
		<table class="table_query" >
			<tr>
				<td nowrap="nowrap" class="table_query_2Col_label_5Letter">经销商代码：</td>
				<td><input id="DRLCODE" name="DRLCODE" class="middle_txt" type="text" /></td>
				<td class="table_query_2Col_label_5Letter" nowrap="nowrap" >经销商名称：</td>
				<td><input id="DELSNAME" name="DELSNAME" class="middle_txt" type="text" /></td>
		    </tr>
		    <tr>
...
<script type="text/javascript">
<!--
var url = "<%=ctx%>/sales/marketmanage/plancommon/MarketDealerTree/getAllDrlQuery.json";
//设置表格标题
var title= null;
//设置列名属性
...
var columns = [
				...
				{header: "选择",   dataIndex: 'DEALER_ID', width:"10px",renderer:myLink},
				{header: "所属区域",dataIndex: 'ORG_NAME',width:"40px"},
				{header: "省份",dataIndex: 'PROVINCE_ID',width:"40px",renderer:getRegionName},
				{header: "经销商代码",dataIndex: 'DEALER_CODE',width:"40px"},
				{header: "经销商简称",dataIndex: 'DEALER_SHORTNAME',width:"50px"}
		      ];
...
var setModel = function (dealerId,dealerShortname,dealerCode,orgName){
	if(parent.$('inIframe'))
	{
		parentContainer.getDealerInfo(dealerId,dealerShortname,dealerCode,orgName);
	  _hide();
	}else
	{
		parentContainer.getDealerInfo(dealerId,dealerShortname,dealerCode,orgName);
...
function setCheckModel(){
	...
	if(reCode.length>0){
		if(parent.$('inIframe'))
	{
			parentContainer.getDealerInfo(reCode,str);
