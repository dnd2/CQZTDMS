...
<%
    ...
    String areaId=request.getParameter("INPUTAREAID");
%>
<script>
<!--
...
function myLink(value,meta,rec){
	...
	{
		return "<input type='checkbox' name='cb' id=\""+data.orgId+"\"  value=\""+data.orgId+"\" /><input type='hidden' name='cn' id=\""+data.orgId+"\"  value=\""+data.orgCode+","+data.orgName+","+data.orgId+"@"+"\" />"
    }else
    {
...
function setCheckModel(){
	var reCode="";
	var reNames="";
	var groupcheck=document.getElementsByName("cn");
	...
	for(var i=0;i<groupCheckBoxs.length;i++)
	{
		if(groupCheckBoxs[i].checked)
		{
			if(reNames && reNames.length > 0){
				reNames += "," + groupcheck[i].value;
			}else{
				reNames=reNames+groupcheck[i].value;
	      }
	...
	parentDocument.getElementById(inputName).value=reNames;
	parentDocument.getElementById(inputId).value=reCode;
	if(parent.$('inIframe'))
	{
		parentContainer.getDealerInfo(reNames,reCode);
		_hide();	
	}else{
		parentContainer.getDealerInfo(reNames,reCode);
		parent._hide();	
