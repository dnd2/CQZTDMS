...
<div id='dtree' class="dtree" style=" margin:2px;float:left;border-right:solid 1px;">
	 ...
	 <form id="fm" name="fm" method="post">
		...
		<table class="table_query" >
		    ...
		    <tr>
				<td nowrap="nowrap" class="table_query_label">城市代码：<input 
					id="CityCode" name="CityCode" class="middle_txt" type="text" /></td>
				<td class="table_query_label" nowrap="nowrap" >城市名称：<input 
					id="CityName" name="CityName" class="middle_txt" type="text" /></td>
				 <td  align="right">
				 <input name="button2" type="button" class="cssbutton" id="queryBtn" onclick="goToSelect();" value="查 询"/>
...
<script type="text/javascript">
<!--
var url = "<%=ctx%>/sysmng/usemng/SgmDealerSysUser/allCityQuery.json?COMMAND=1&ORGID="+ORGID;
//设置表格标题
var title= null;
//设置列名属性
...
// 				{header: "省份代码",dataIndex: 'ORG_CODE',width:"40px"},
				...
				{header: "城市名ID",dataIndex: 'CITY_ID',width:"50px"},
				{header: "城市名称",dataIndex: 'CITY_NAME',width:"50px"}
		      ];
function myLink(value,meta,rec){
	var data = rec.data;
	if($('ISMULTI').value=='true'){	
		return "<input type='checkbox' name='cb' id=\""+data.ORG_ID+"\"  value=\""+data.CITY_ID+"\" /><input type='hidden' name='cc' id=\""+data.ORG_ID+"\"  value=\""+data.ORG_ID+"\" />"
    }else{	
    	return "<input type='radio' name='rd' onclick='setModel(\""+data.ORG_ID+"\",\""+data.CITY_NAME+"\",\""+data.CITY_ID+"\")' />"  
    }
}
