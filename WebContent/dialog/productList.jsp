...
<script>
	var myPage;
	var url = "<%=contextPath%>/common/ModelMng/queryModel.json";
	...
	function myLink(value,meta,record){
		return "<input type='radio' name='rd' onclick='setModel(\""+record.data.modelId+"\",\""+record.data.modelName+"\")' />"
	}
	
	function setModel(modelId,modelName){
		parentDocument.getElementById('modelName').value = modelName;
		parentDocument.getElementById('modelId').value = modelId;
		if('<%=funcname%>' != "" || '<%=funcname%>' != "undefined") {
			parentDocument.getElementById{funcname}+";";
		}
		_hide();
	}
	...
	function clsTxt(){ //清除经销商文本框
		parentDocument.getElementById('modelName').value = "";
		parentDocument.getElementById('modelId').value = "";
		_hide();
	}
