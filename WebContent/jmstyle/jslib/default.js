 ...
 */
$(function () {
	try{parentContainer = __parent();  parentDocument = parentContainer.document;}catch(e){}
});

function __parent() {
	return parent.document.getElementById ('inIframe').contentWindow;
}
...
function setMustStyle(arrInputs)
{
    if(arrInputs && typeof(arrInputs) == "object" && arrInputs.length > 0)
    {
    	...
    	{
    		if(!arrInputs[n].getAttribute("datatype")) continue;
			var dateType = arrInputs[n].getAttribute("datatype");
			var must_s = false;
    ...
    }else
    {
		...
		if(inputs && inputs.length)
		{
			...
			{
				if(!inputs[i].getAttribute("datatype")) continue;
				var dateType = inputs[i].getAttribute("datatype");
				var must_s = false;
				var arr = dateType.split(",");
				if(arr && arr.length > 0)
				{
		...
		if(textareas && textareas.length)
		{
			for(var i=0;i<textareas.length;i++)
			{
				...
				if(!textareas[i].getAttribute("datatype")) continue;
				var dateType = textareas[i].getAttribute("datatype");
				var must_s = false;
				var arr = dateType.split(",");
				if(arr && arr.length > 0)
				{
		...
		if(selects && selects.length)
		{
			for(var i=0;i<selects.length;i++)
			{
				...
				if(!selects[i].getAttribute("datatype")) continue;
				var dateType = selects[i].getAttribute("datatype");
				var must_s = false;
				var arr = dateType.split(",");
				if(arr && arr.length > 0)
				{
