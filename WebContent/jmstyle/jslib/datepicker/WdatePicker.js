...
var PickDate = {
	prevElVal: '',
	init: function( options, className ) {
		var self  = this;
		options   = options || {};
		className = className || 'time_ico';

		jQuery( '.' + className ).click( function() {
			self.pick( options, $( this ) ); 
		});
	},

	pick: function( options, evtObj ) {
		var prevEl = evtObj.prev();

		if ( prevEl.length > 0 ) {
				this.prevElVal = options.el = prevEl.attr( 'id' );
		}

		if ( ! options.dateFmt ) {
			options.dateFmt = 'yyyy-MM-dd'; // With time: yyyy-MM-dd HH:mm:ss
		}
		...
		if ( ! options.onpicking ) {
			options.onpicking = this.pickCallBack;
		}

		WdatePicker( options );
	},

	pickCallBack: function( dp ) {
		$dp.$( PickDate.prevElVal ).value = dp.cal.getDateStr();
	}
};
