...
<table class="table_add" width='100%'  >
	<tr>
		...
		<td   nowrap="nowrap" class="table_add_2Col_input" align='left' >
			<input type="file" name="uploadFile" style="width: 250px"  datatype="1,is_null,2000" id="importFile" value="" onchange="check();" /><span style="color:red">*</span>
		</td>
	</tr>
	<tr>
	   <td></td>
...
<script type="text/javascript">
    function setValue(){
    	...
    	var fjid = "<%=fjid%>";
    	var tab = "";
    	var commonUrl = "<%=request.getContextPath()%>/util/FileDownLoad/fileDownloadQuery.do?fjid="+fjid;
    	if(fileName!=null && fileName !='' && fileName!='null'){
    	   tab = parent.document.getElementById('uptab');
    	    ...
    	    row.cells(0).innerHTML="<a  target='_blank' href='"+commonUrl+"'>"+fileName+"</a><input type='hidden' value='"+fileId+"' name='uploadFileId'/>"+"<input type='hidden' value='"+fileName+"' name='uploadFileName'/>" + "<input type='hidden' name='fjid' value='"+fjid+"'/>";
    	    row.cells(1).innerHTML="<input type=button onclick='delUploadFile(this)' class='normal_btn' value='删 除' />"; */
    	    
    	    var newTd1 = row.insertCell();
			newTd1.innerHTML="<a  target='_blank' href='"+commonUrl+"'>"+fileName+"</a><input type='hidden' value='"+fileId+"' name='uploadFileId'/>"+"<input type='hidden' value='"+fileName+"' name='uploadFileName'/>" + "<input type='hidden' name='fjid' value='"+fjid+"'/>";
			var newTd2 = row.insertCell();
			newTd2.innerHTML="<input type=button onclick='delUploadFile(this)' class='normal_btn' value='删 除' />";
    	}
    }
...
function check(){
	...
	var f = document.getElementById("importFile").value;
	if(f==null||""==f){
		MyAlert("请选择要上传的文件!");
		return false;
	}
