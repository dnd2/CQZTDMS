...
<input type="hidden" id="flag" value="colorflag">
  	<table class="table_query" border="0">
		<tr>
		    ...
		    <td  nowrap="nowrap">
		    	<input name="modelCode" datatype="1,is_null,50" id="modelCode" type="text" class="middle_txt" /><font color="red">*</font>
		    </td>
		    <td  nowrap="nowrap" align="right">车辆型号名称：</td>
		    <td  nowrap="nowrap">
		    	<input name="modelName" datatype="1,is_null,300" id="modelName" type="text" class="middle_txt" /><font color="red">*</font>
		    </td>
	  	</tr>
	</table>
	<table class="table_query" id="package_list">
				<tr class="table_query_th">
					<th colspan="5"><img class="nav"
						src="<%=contextPath%>/img/subNav.gif" />&nbsp;配置列表
...
function save(){
	...
	if(packageIds == null || packageIds.length == 0){
		...
		makeCall("<%=request.getContextPath()%>/sysproduct/productmanage/VehicleModelManage/checkPackage.json",makedecide,{pid:pid});
	}
}	
	...
	function makedecide(json){
		if(json.returnValue == "1"){
			MyConfirm("确认保存?",function(){
				makeNomalFormCall("<%=request.getContextPath()%>/sysproduct/productmanage/VehicleModelManage/addVehicleModel.json",showResult,'fm');
			});	
		}else{
