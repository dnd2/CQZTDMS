...
<div class="wbox">
	<div class="navigation"><img src="<%=contextPath%>/img/nav.gif" />&nbsp;当前位置：  整车销售 &gt; 二手车置换 &gt; 经销商二手车置换审核查询</div>
...
<script type="text/javascript">
	...
	var url = "<%=contextPath%>/sales/displacement/DisplacementCarChk/displacementCarDealerChkQuery.json?COMMAND=1";
	...
	function myLink(dis_id,metaDate,record){
        ...
        return String.format(
        		 "<a href=\"#\" onclick=\"chkArea(" + vehicleId + "," + dis_id + ");\">[${opera}]</a>&nbsp;<a href=\"#\" onclick=\"printDisplancement(" + vehicleId + "," + dis_id + ");\">[打印]</a>");
    }

	function chkArea(vehicleId, dis_id) {
		document.getElementById('vehicle_id').value = vehicleId ;
		document.getElementById('dis_id').value = dis_id ;
		var url = "<%=contextPath%>/sales/displacement/DisplacementCarChk/infoDealerDetailQuery.do" ;
		$('fm').action=url;
		$('fm').submit();
	}
	function printDisplancement(vehicleId, dis_id){
	    var vehicle_id = vehicleId ;
		var dis_id = dis_id ;
		var url = "<%=contextPath%>/sales/displacement/DisplacementCarChk/infoDealerDetailPrint.do?vehicle_id="+vehicle_id+"&dis_id="+dis_id ;
		window.open(url,"经销商二手车置换打印", "height=800, width=1024, top=0, left=0, toolbar=no, menubar=no, scrollbars=yes, resizable=yes,location=no, status=no");
