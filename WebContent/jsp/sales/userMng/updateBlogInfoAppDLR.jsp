...
<body>
<div class="navigation"><img src="<%=contextPath%>/img/nav.gif" />&nbsp;当前位置： 经销商实销管理&gt;微博积分管理&gt;微博积分修改</div>
 <form method="post" name = "fm" id="fm">
  ...
  <input type="hidden" name="blogId" id="blogId" value="${blog.blogId}"/>
	...
	<tr>
		...
		<td><select name="year"><c:forEach items="${yearList}" var="str"><c:if test="${str==blog.year}"><option value="${str}" selected>${str}</option></c:if><c:if test="${str!=blog.year}"><option value="${str}">${str}</option></c:if></c:forEach></select></td>
		<td class="table_query_2Col_label_7Letter">提报月份：</td>
		<td><select name="integ_month"><c:forEach items="${list}" var="str"><c:if test="${str==blog.integMonth}"><option value="${str}" selected >${str}</option></c:if><c:if test="${str!=blog.integMonth}"><option value="${str}">${str}</option></c:if></c:forEach></select></td>
	</tr>
    <tr>
    <td colspan="4">
		...
		<div id="showDiv" style="width:100%;">
			<table class="table_list" style="width:100%;border:1px solid #B0C4DE" id="showTab" border="1" cellpadding="1" bordercolor="#B0C4DE" >
				...
				<c:forEach var="po" items="${detailList}">
					...
					<td><input id="webone${po.DETAIL_ID}"  name="webone" value="${po.BLOG_ONE}"></td>
					<td><input id="webtwo${po.DETAIL_ID}"  name="webtwo" value="${po.BLOG_TWO}"></td>
					<td><input id="webthree${po.DETAIL_ID}"  name="webthree" value="${po.BLOG_THREE}"></td>
					<td><input id="blog_integ${po.DETAIL_ID}" name="blog_integ" value="${po.BLOG_INTEG}"></td>
					<td><a href="#" onclick="rowDelete(${po.DETAIL_ID});">[删除]</a></td>
					</tr>
				</c:forEach>
