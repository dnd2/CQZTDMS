...
<script type="text/javascript">
	var myPage;
	//查询路径           
	var url = "<%=contextPath%>/sales/usermng/UserManage/personSelectInitQuery.json";
	var title = null;
	var columns = [
	...
	function myLink(value,meta,record){
		return String.format("<a href='#' onclick='leaveJob("+record.data.PERSON_ID+")'>[人员离职]</a>/<a href='#' onclick='switchJob("+record.data.PERSON_ID+")'>[体系转换]</a>");
	}
	//人员离职
	function leaveJob(person_id){
			if(confirm("确认离职？")){
				var urls="<%=contextPath%>/sales/usermng/UserManage/personLeaveJob.json?personId="+person_id;
				makeFormCall(urls, leaveReturn, "fm") ;
			}
	}
	
	function leaveReturn(json){
	...
	//体系转换
	function switchJob(person_id){
		if(confirm("确认体系转换？")){
			var url="<%=contextPath%>/sales/usermng/UserManage/personSwitchJob.json?personId="+person_id;
			makeFormCall(url, switchReturn, "fm") ;
		}
	}
	
	function switchReturn(json){
		var subFlag = json.subFlag ;
		if(subFlag == 'success') {
			MyAlert("体系转换成功!") ;
		} else {
	...
	function executeQuery(){
		url= "<%=contextPath%>/sales/usermng/UserManage/personSelectInitQuery.json";
		__extQuery__(1);
	}
...
<div class="wbox">
<div class="navigation"><img src="/dms/img/nav.gif" />&nbsp;当前位置： 经销商实销管理 &gt; 人员管理 &gt; 人员离职</div>
