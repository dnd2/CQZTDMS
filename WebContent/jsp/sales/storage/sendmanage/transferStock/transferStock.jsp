	...
	<table class="table_edit">
			<tr>
				<td align="center" colspan="4">
					<input type="button"  name="button1" class="normal_btn" id="saveButton" onclick="saveTrans();" value="确定入库" /> 
...
{ 	
	if(checkData()==true){
		MyConfirm("确认保存该信息！",saveTfs);
	}
}
function saveTfs()
{ 
	disabledButton(["saveButton","goBack"],true);
	makeNomalFormCall("<%=contextPath%>/sales/storage/sendmanage/TransferStock/saveTransferStock.json",saveTfsBack,'fm','queryBtn'); 
}
function saveTfsBack(json){
...
//取消操作执行
function canelFunc(){
	fm.action = "<%=contextPath%>/sales/storage/sendmanage/TransferStock/transferStockInit.do";
	fm.submit();
}
//打印库位码(确定)
