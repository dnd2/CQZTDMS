...
function changeFleet() {
	...
	document.fm.warehouse__.options.length=i;
	for(var j=0; j<i; j++) {
		document.fm.warehouse__.options[j].value=sWarehouseId.split(',')[j];
		document.fm.warehouse__.options[j].text=sWarehouseName.split(',')[j];
	}
}
...
function submitAction(){
	...
	makeNomalFormCall('<%=contextPath%>/sales/storage/sendmanage/DispatchOrderManage/checkSubmit.json?vehicleId='+vehicleId,showResult,'fm') ;
}
...
function checkSubmit(){
	if(submitForm('fm')){
		...
		var recWare=document.getElementById("recWare").value;
		var warehouse=document.getElementById("warehouse__").value;
		if(recWare!=warehouse){
			MyAlert("收车仓库不一致");
			return;
		}
...
<body onunload='javascript:destoryPrototype();' onload="ChangeDateToString();changeFleet(); loadcalendar();"> 
<div class="wbox">
	...
	<form id="fm" name="fm" method="post"  enctype="multipart/form-data" >
		...
		<table class="table_query" border="0">
			...
			<tr>
				...
				<td>
					<inpu type="hidden" id="dlvWare" name="dlvWare" value="${vehicleInfo.DLV_WARE_ID }" />
					${vehicleInfo.DLV_WARE_NAME }
				</td>
			</tr>
			<tr>
				...
				<td width="20%" class="tblopt"><div align="right">收车仓库：</div></td>
				<td width="30%"  align=left class=datadetail>
				<input type="hidden" id="recWare" name="recWare" value="${vehicleInfo.REC_WARE_ID }" />
				${vehicleInfo.REC_WARE_NAME }
				</td>
			</tr>
			<tr>
				<td width="20%" class="tblopt"><div align="right">实际到车日期：</div></td>
				<td>
					<input name="arrive_date" readonly="readonly" id="t1" value="" type="text" class="middle_txt" datatype="1,is_date,10">
			...
			<tr>
				...
				<td>
					<input type="text" id="inspection_person" name="inspection_person" class="middle_txt" datatype="0,is_digit_letter_cn,15" maxlength="15" />
				</td>
			...
			<tr>
				...
				<td class=datadetail align=left colspan=3>
					<input type="text" id="remark" name="remark" rows="2" cols="50"  class="middle_txt" maxlength="500"  style="width:50%;height:50px" />
					<input type="hidden" id="vehicleId" name="vehicleId" value="${vehicleInfo.VEHICLE_ID}" />
				</td>
			</tr>
			<table style="display:none">
			<TR>
			<TD class=datatitle align=right>是否质损：</TD>
...
<script type="text/javascript" >
	...
	function addTblRow() {
		...
		cell1.innerHTML = '<TD align="center"><div align="center"><input type="text" maxlength="200" name="describe" style="width: 90%" /></div></TD>';
		cell2.innerHTML = '<TD align="center"><div align="center"><input type="text" maxlength="200" name="part" style="width: 90%" /></div></TD>';
