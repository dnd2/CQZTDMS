								...
								for(int ve1=0;ve1<vehicleList.size();ve1++){
									...
									<tr>
										<td><input type='checkbox' id='groupIds' name="groupIds~<%=orgMap.get("ORG_ID") %>_<%=boMap.get("BO_DE_ID") %>"/>
											...
											<input type='hidden' name='NEW_DETAIL_ID' value='newVechile'/>
										</td><!-- 车辆ID -->
										<td><%=ve1+1 %></td><!-- 序号 -->
...
<table class="table_list" style="margin-top: 1px; margin-bottom: 1px;" id="subtab1">
	<tr class="table_list_row2">
		<td align="center" colspan="8">
			<input type="button" name="button1" class="normal_btn" id="oneButton" onclick="oneKey();" value="一键配车" />
...
<script type="text/javascript" >
	...
	//配车完成
	function save(){
		var thisNum=document.getElementById("SHOW_THIS_BOARD_NUM").innerHTML;
		var allocaNum=document.getElementById("SHOW_ALLOCA_NUM").innerHTML
	...
	//--------------------------------------------------------------一键配车START------------------------------------------------------------//
	function oneKey(){
		MyConfirm("确认一键配车！",oneKeyVehicle);	
		
	}
	function oneKeyVehicle(){
		var boId=document.getElementById("boId").value;
		fm.action = "<%=contextPath%>/sales/storage/sendmanage/AllocaManage/oneKey.do?boId="+boId;
		fm.submit();
		MyAlert("配车完成！如果某些物料下无配车记录，说明无该物料的库存车");
	}
	//--------------------------------------------------------------一键配车END------------------------------------------------------------//
</script>
</script>
