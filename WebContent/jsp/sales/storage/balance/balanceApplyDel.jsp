...
<body>
	<div class=navigation><img src="<%=request.getContextPath()%>/img/nav.gif">&nbsp;当前位置：储运管理&gt;结算管理&gt;结算申请&gt;明细查看</div>
	...
	<div class="form-panel">
	  ...
	  <TABLE class=table_query id="moneyTable">
	  	<tr>
			...
			<td class="right">生成时间：</td>
			<td><span id="expectDate_span"><c:out value="${map.CREATE_DATE}"></c:out></span></td>	
		</tr>
		<tr>
			...
			<td class="right">扣款合计：</td>
			<td><span id="dealer_code_span">${map.DEDUCT_MONEY}</span></td>
		</tr>
		<tr>
			<td class="right">补款合计：</td>
			<td><span id="expectLastDate_span">${map.SUPPLY_MONEY}</span></td>
			<td class="right">其他合计：</td>
			<td><span id="dealer_code_span">${map.OTHER_MONEY}</span></td>
			<td class="right">结算合计：</td>
			<td><span id="dealer_code_span">${map.SUM_AMOUNT}</span></td>
		</tr>
	  </TABLE>
	  </div>
	 </div>
    		...
    		<tbody id="tbody1">
    			<c:forEach items="${flist}" var="list" varStatus="status">
    				<tr>
    					...
    					<td align="left">${list.BAL_ADDR_M}</td>
    					<td>${list.LAST_CAR_DATE}</td>
    					<td>${list.DLV_SHIP_TYPE}</td>
    					<td>${list.BILL_AMOUNT}</td>
    					<td>${list.SUPPLY_MONEY}</td>
    					<td>${list.DEDUCT_MONEY}</td>
    					<td>${list.OTHER_MONEY}</td>
    					<td>${list.IS_CHANGE}</td>
    				</tr>
    			</c:forEach>
    		</tbody>
  		</table>
