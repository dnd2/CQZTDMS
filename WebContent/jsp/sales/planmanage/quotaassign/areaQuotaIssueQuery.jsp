...
<title>区域配额确认下发</title>
</head>
<body>
<div class="navigation"><img src="<%=request.getContextPath()%>/img/nav.gif" />&nbsp;当前位置：整车销售 > 计划管理 > 配额分配 > 区域配额确认下发</div>
...
<script type="text/javascript">
	...
	var myPage;
	//查询路径
	var url = "<%=request.getContextPath()%>/sales/planmanage/QuotaAssign/AreaQuotaConfirmIssue/areaQuotaIssueQuery.json";;
	...
	function loginQuery(arg1, arg2) {
		...
		OpenHtmlWindow("<%=request.getContextPath()%>/sales/planmanage/QuotaAssign/AreaQuotaConfirmIssue/areaQuotaIssueDetailPre.do?quotaYear="+quotaYear+"&quotaMonth="+quotaMonth+"&areaId="+areaId+"&orgId="+arg1+"&groupId="+arg2,800,500);
	}
	...
	function quotaIssue(){
		makeNomalFormCall('<%=request.getContextPath()%>/sales/planmanage/QuotaAssign/AreaQuotaConfirmIssue/areaQuotaIssue.json',showResult,'fm');
	}
	
	function showResult(json){
		if(json.returnValue == '1'){
			window.location.href = '<%=request.getContextPath()%>/sales/planmanage/QuotaAssign/AreaQuotaConfirmIssue/areaQuotaConfirmIssuePre.do';
		}else{
