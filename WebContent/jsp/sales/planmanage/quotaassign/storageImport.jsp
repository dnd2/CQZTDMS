...
function upload(){
	if(submitForm('fm')){
		...
		$('fm').action = "<%=contentPath %>/sales/planmanage/QuotaAssign/StorageImport/storageExcelOperate.do";
    	$("fm").submit();
	}
}

function mydown(){
	$('fm').action = "<%=contentPath %>/sales/planmanage/QuotaAssign/StorageImport/downLoad.do";
    $("fm").submit();
}
</script>
</head>
...
<body onload="">
<div class="wbox">
	<div class="navigation"><img src="<%=contentPath %>/img/nav.gif" />&nbsp;当前位置：整车销售 > 计划管理 > 配额调整 > 车厂配额导入
</div>
...
<table class="table_query">
	...
	<tr class="csstr">
	  <td colspan="2" nowrap="nowrap">
	  	3、点“<font color="#FF0000">浏览</font>”按钮，找到您所要上载的<font color="#FF0000">配额计划</font>文件(<strong>"配置代码-配置名称-周度对应配额数量..."</strong>)：
