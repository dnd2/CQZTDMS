		...
		<tr align="center" >
			<th colspan="6">
				<div align="left">
					...
					<input class="cssbutton" type="button" name='saveResButton' onclick='confirmIssue();' value='确认下发' />
				</div>
			</th>	
	  	</tr>
	</table>
...
<script type="text/javascript">
	...
	var myPage;
	//查询路径
	var url = "<%=request.getContextPath()%>/sales/planmanage/QuotaAssign/DealerQuotaConfirmIssue/dealerQuotaIssueQuery.json";;
	...
	function myLink(value,meta,record){
  		return String.format("<a href='#' onclick='loginQuery(\""+ record.data.DEALER_ID +"\",\""+ record.data.GROUP_ID +"\")'>"+value+"</a>");
	}
	...
	function loginQuery(arg1, arg2) {
		...
		OpenHtmlWindow("<%=request.getContextPath()%>/sales/planmanage/QuotaAssign/DealerQuotaConfirmIssue/dealerQuotaIssueDetailPre.do?quotaYear="+quotaYear+"&quotaMonth="+quotaMonth+"&areaId="+areaId+"&dealerId="+arg1+"&groupId="+arg2,800,500);
	}
	
	function confirmIssue(){
		if(submitForm('fm')){
			MyConfirm("是否确认下发?",quotaIssue);
		}
	}
	
	function quotaIssue(){
		makeNomalFormCall('<%=request.getContextPath()%>/sales/planmanage/QuotaAssign/DealerQuotaConfirmIssue/dealerQuotaIssue.json',showResult,'fm');
	}
	
	function showResult(json){
		if(json.returnValue == '1'){
			window.location.href = '<%=request.getContextPath()%>/sales/planmanage/QuotaAssign/DealerQuotaConfirmIssue/dealerQuotaConfirmIssuePre.do';
		}else{
