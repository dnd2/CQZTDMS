...
<body>
<div class="navigation"><img src="<%=request.getContextPath()%>/img/nav.gif" />&nbsp;订单管理 &gt; 销售订单管理&gt;下级经销商订单管理 &gt; 下级经销商周度常规订单预审核</div>
...
<script type="text/javascript">
	...
	function changeRate(id){
		...
		if(singlePrice-MATERIAL_PRICE_MAX>0){
			...
			document.getElementById("buyNO"+id).readOnly = false ;
		}
	...
	function retBackAction() {
		...
		makeNomalFormCall('<%=contextPath%>/sales/balancecentermanage/dealerordermanage/GeneralWeekOrderPreCheck/retBack.json',showResult2,'fm');
	}
	...
	function checkSubmitAction(){
		var areaId = '${areaId}';
	 	makeNomalFormCall('<%=contextPath%>/sales/balancecentermanage/dealerordermanage/GeneralWeekOrderPreCheck/checkSubmitAction.json?areaId='+areaId,showResult,'fm');
	 	setDisTrue("add22","ret","add232") ;
	}
	function showResult(json){
		if(json.returnValue == '2'){
			...
			$('fm').action= "<%=contextPath%>/sales/balancecentermanage/dealerordermanage/GeneralWeekOrderPreCheck/ReqDetailInto.do";
			$('fm').submit();
