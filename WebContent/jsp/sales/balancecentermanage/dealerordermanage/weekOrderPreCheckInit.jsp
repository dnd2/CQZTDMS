...
<body>
<div class="navigation"><img src="<%=request.getContextPath()%>/img/nav.gif" />&nbsp;当前位置：订单管理 > 销售订单管理 > 下级经销商订单管理 > 周度常规订单预审核</div>
...
<table class="table_query" align=center width="95%">
	<tr>
		...
		</select> 周 至 <select name="orderYear2">
			<c:forEach items="${years}" var="po">
				<c:choose>
					<c:when test="${po == curYear}">
						<option value="${po}" selected="selected">${po}</option>
...
<script type="text/javascript">
	...
	var url = "<%=contextPath%>/sales/balancecentermanage/dealerordermanage/GeneralWeekOrderPreCheck/dealerOrderCheckInit_Query.json?COMMAND=1";
	...
	var columns = [
				...
				{header: "订单日期", dataIndex: 'ORDER_WEEK', align:'center'},
	...
	function myLink(value,metaDate,record){
	    return String.format(
	    		 "<a href=\"<%=contextPath%>/sales/balancecentermanage/dealerordermanage/GeneralWeekOrderPreCheck/ReqDetailInto.do?reqId=" + value + "\">[审核]</a>");
	}
