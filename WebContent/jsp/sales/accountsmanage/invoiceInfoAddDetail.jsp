...
function orderBack() {
	...
	document.getElementById("orderBack").disabled = true ;
...
function showBackResult(json){
	if(json.returnValue == '1'){
		document.fm.action= '<%=request.getContextPath()%>/sales/ordermanage/orderaudit/OrderResourceReserveFirst/orderResourceReserveQueryPre.do';
		document.fm.submit();
 		destoryLock();
 		MyAlert("驳回成功！！！");
 		history.back();
		...
		document.getElementById("orderBack").disabled = false ;
...
<body >
<div class="navigation"><img src="<%=request.getContextPath()%>/img/nav.gif" />&nbsp;财务管理 &gt; 发票管理 &gt;发票管理</div>
<form method="post" name="fm" id="fm">
	...
	<table class="table_query" align="center">
	 <tbody id="dealerTr">
		<tr class="cssTable" >
	   	<td align="right" valign="top" nowrap="nowrap" width="15%" ><div align="right">经销商代码：</div></td>
	    <td align="left" valign="top" nowrap="nowrap" width="15%">${map.DEALER_CODE}</td>
	    <td align="right" valign="top" nowrap="nowrap" width="15%" ><div align="right">经销商名称：</div></td>
	    ...
	    <tr class="cssTable" >
	     <%--  <td align="right" valign="top" nowrap="nowrap" class="table_info_3col_label_6Letter">销售订单号：</td>
	      ...
	      <td align="right" valign="top" nowrap="nowrap"><div align="right">订单类型：</div></td>
	      <td align="left" valign="top" nowrap="nowrap" class="table_info_3col_input">
	      	${map.ORDER_TYPE_NAME} 
	      </td>
	      ...
	      <td align = "left" colspan="1">
	      	...
	      	<select id="priceId" name="priceId" disabled style="display:none" onchange="priceTypeChange();">
	        </select>
	      </td>
	       <td align = "right" style="color:#252525;width:70px;text-align:right;">
	       <div align="right"><span id="span5" style="display:none">订单原总价：</span></div>
	       <div align="right">发运仓库：</div>
	       </td>
	  	<td align = "left" style="width:90px;">
	  	<input type="hidden" id="reserveTotalPrice" name="reserveTotalPrice" /><span id="spanReserve" style="display:none"></span>
	  			${map.WAREHOUSE_NAME}
	  	</td>  
	  	
	  	<%--  <td align="right" valign="top" width="15%" nowrap="nowrap" class="table_info_3col_label_6Letter"><div align="right">发运仓库：</div></td>
	    ...
	    <%--  <td align="right" valign="top" nowrap="nowrap" class="table_info_3col_label_6Letter">提报时间：</td>
	     ...
	     <tbody id="fundTr">
	  <tr class="cssTable" >
	   <td align="right" valign="top" nowrap="nowrap" ><div align="right">资金类型：</div></td>
	    <td align="left" valign="top" nowrap="nowrap" class="table_info_3col_input">
	    	${map.TYPE_NAME }
	    	<input type="hidden" value="${map.FUND_TYPE_ID }" id="accountTypeId" name="accountTypeId" />
	    	<select name="fundType" disabled style="display:none" id="fundType" onchange="getAvailableAmount(this.options[this.options.selectedIndex].value);">
	      	</select>
	    </td>
	    <td align="right" valign="top" nowrap="nowrap" ><div style="display:none" align="right">可用余额：</div><div align="right"><span id="span7">订单金额：</span></div></td>
	    <td align="left" valign="top" nowrap="nowrap" class="table_info_3col_input">
	    
	    	<span id="show1" style="display:none"></span><input type="hidden" name="availableAmount" value="">
	    	<input type="hidden" id="payAmount" name="payAmount" /><span id="span8" ></span>
	    </td>
	      <td align = "right" style="color:#252525;width:70px;text-align:right;"><div align="right" style="display:none"><span id="span7" >订单金额：</span></div></td>
	  	<td align = "left" style="width:90px;"><input type="hidden" id="payAmount" name="payAmount" /><span id="span8" style="display:none"></span></td>    
	  ...
	  <tr class="cssTable" >
	     ...
	     <td  align="right" nowrap><div align="right"><span id="rebate_account" style="display:none">返利可用余额：</span></div><div align="right"><span id="rebate_discount">本次返利金额：</span></div></td>
	    <td> <span id="rebate_account_amount" style="display:none"></span><span id="rebate_discount_amount" name="rebate_discount_amount"></span></td>
	    <td  align="right" nowrap><div align="right" style="display:none"><span id="rebate_discount">本次返利金额：</span></div></td>
	    <td> <input type="hidden" id="rebateAmount" name="rebateAmount"/><span id="rebate_discount_amount" name="rebate_discount_amount" style="display:none"></span></td>
	    <!--   <td align = "right" style="color:#252525;width:70px;text-align:right;"><span id="span7">应付金额：</span></td>
	     ...
	     <tr>
	     	<td align="center"><div align="right">发票号：</div></td>
	     	<td><input type="text" class="middle_txt" id="invoiceNO" name="invoiceNO" value="" /><font color="red">*</font></td>
	     	<td><div align="right">开票时间：</div></td>
	     	<td><input name="invoiceDate" readonly="readonly" id="invoiceDate" value="" type="text" class="middle_txt" datatype="1,is_date,10" onFocus="WdatePicker({el:$dp.$('invoiceDate'), maxDate:''})"  style="cursor: pointer;width: 80px;"></td>
	     	<td><div align="right">本次开票金额：</div></td>
	     	<td><span id="invoiceTotal" name="invoiceTotal"></span><input type="hidden" id="invoiceTotalHid" name="invoiceTotalHid" /></td>
	     </tr>
	     </tbody>
	</table>
	<br>
	<table class="table_list">
		<tr align="center" class="tabletitle">
			...
			<th nowrap="nowrap">返利金额</th>
      		...
      		<th nowrap="nowrap">已开票数量</th>
      		<th nowrap="nowrap">本次开票数量</th>
      		<th nowrap="nowrap">开票单价</th>
      		<th nowrap="nowrap">开票总价</th>
      		<th nowrap="nowrap">开票税率</th>
      		<th nowrap="nowrap">开票税额</th>
      		<th></th>
<!-- 			<th>当前库可用库存</th>
			<th>当月任务数</th>
			<th>当月已审核数</th> -->
			...
			<c:forEach items="${list1}" var="list1" varStatus="vstatus">
				<tr align="center" class="table_list_row1">
					...
					<td>
						...
						<input type="text" class="mini_txt" style="display:none" id="reserveAmount${list1.MATERIAL_ID}" name="reserveAmount" value="${list1.RESERVE_AMOUNT}" onchange="reserveAmount1();">
						${list1.RESERVE_AMOUNT}
						<%-- <a href="#" onclick="patchNoSelect('${list1.MATERIAL_ID}','${list1.RESERVE_AMOUNT}');">[审核]</a> --%>
					</td>
					<td><input type="text" id="singlePrice${list1.MATERIAL_ID}" name="singlePrice" style="display:none" size="8" style="text-align:right" value="${list1.SINGLE_PRICE}" onchange="priceTotal();" /><span style="display:none" id="priceDesc${list1.MATERIAL_ID}"></span><script type="text/javascript">document.write(amountFormat(${list1.SINGLE_PRICE}));</script></td>
					<td><span id='price2${list1.MATERIAL_ID}'><script type="text/javascript">document.write(amountFormat(${list1.TOTAL_PRICE}));</script></span></td>
					 <td align="center" style=""><span id='lineRebateAmount${list1.MATERIAL_ID}'><script type="text/javascript">document.write(amountFormat(${list1.REBATE_AMOUNT }));</script></span><input type="hidden" id="lineRebateAmountHid${list1.MATERIAL_ID}" name="lineRebateAmountHid" value=""></td>
		      	  <td align="center" style=""><span id='lineRebateTotalPrice${list1.MATERIAL_ID}'> <script type="text/javascript">document.write(amountFormat( ${list1.TOTAL_PRICE}-${list1.REBATE_AMOUNT }));</script> </span><input type="hidden" id="lineRebateTotalPriceHid${list1.MATERIAL_ID}" name="lineRebateTotalPriceHid" value=""></td>
					...
					<td>${list1.BILL_AMOUNT}<input type="hidden" id="yiBillAmount${list1.MATERIAL_ID}" name="yiBillAmount" value="${list1.BILL_AMOUNT}" /></td>
					<td><input type="text" class="mini_txt" id="billAmount${list1.MATERIAL_ID}" name="billAmount" value="" onchange="billTotalPrice();" <c:if test="${list1.RESERVE_AMOUNT<=list1.BILL_AMOUNT }"> style="display:none"</c:if> ></td>
					<td><input type="text" class="middle_txt" id="billPrice${list1.MATERIAL_ID}" name="billPrice" value="" onchange="billTotalPrice();" <c:if test="${list1.RESERVE_AMOUNT<=list1.BILL_AMOUNT }"> style="display:none"</c:if>></td>
					<td><span id="billTotalPrice${list1.MATERIAL_ID}" name="billTotalPrice"></span><input type="hidden" id="billTotalPriceHid${list1.MATERIAL_ID}" name="billTotalPriceHid" /></td>
					<td><input type="text" class="middle_txt" id="billTaxRate${list1.MATERIAL_ID}" name="billTaxRate" value=""  onchange="billTotalPrice();" <c:if test="${list1.RESERVE_AMOUNT<=list1.BILL_AMOUNT }"> style="display:none"</c:if><c:if test="${list1.RESERVE_AMOUNT>list1.BILL_AMOUNT }"> style="width:20px"</c:if> ><font size="2">%</font></td>
					<td><span id="billTotalTax${list1.MATERIAL_ID}" name="billTotalTax"></span><input type="hidden" id="billTotalTaxHid${list1.MATERIAL_ID}" name="billTotalTaxHid" /></td>
					<td></td>
				</tr>
			</c:forEach> 
		</tbody>
...
<!--			<td id="discountTotal"></td>-->
			...
			<td></td>
			<td><span id="hejiBillTotalAmount"></span></td>
			<td></td>
			<td><span id="hejiBillTotalPrice"></span></td>
			<td></td>
			<td><span id="hejiBillTotalTax"></span></td>
			<td></td>
		</tr>
	</table>
...
<!--			      <input class='cssbutton' name="add22" id="add22" type="button" onclick="materialShow();" value ='新增产品' />-->
	...
	<table class="table_query" style="display:none">
		<tr>
			<td align="right" valign="top" nowrap="nowrap" class="table_info_3col_label_6Letter"><div align="right">发运方式：</div></td>
		    <td align="left" valign="top" nowrap="nowrap" class="table_info_3col_input">
		   		<%--  <script type="text/javascript">
		...
		<tr class="cssTable" id="tmp_license_amountTr">
	      <td width="12%" align="right" valign="top" nowrap="nowrap"><div align="right">临牌数量：</div></td>
		...
		<tbody id="addTr">
			...
			<tr>
				...
				<td align="left" colspan="3" width="500px">${map.PROVICE}&nbsp;${map.CITY}&nbsp;${map.TOWN}&nbsp;${map.CUSTOM_ADDR}</td>
			</tr>
			<tr>
			...
			<tr>
				<td align="right"><div align="right">运送地点：</div></td>
				<td align="left" colspan="3" width="500px">${mapLinkInfo.ADDRESS}</td>
			</tr>
			<tr>
				<td align="right"><div align="right">联系人：</div></td>
		...
		<tr class="table_list_row2">
			...
			<div align="center">
				...
				<input type="hidden" name="reserveTotalAmount" id="reserveTotalAmount" value="0"> 
	      	    ...
	      	    <input type="hidden" name="actReqStatus" id="actReqStatus" value="${map.REQ_STATUS }"/>
	      	    <input type="hidden" name="actRebateAmount" id="actRebateAmount" value="${map.REBATE_AMOUNT }"/>
	      	    <input type="hidden" name="actReqTotalPrice" id="actReqTotalPrice" value="${map.REQ_TOTAL_PRICE }"/>
	      	     <input type="hidden" id="total" name="total" value="0">
				<input type="button" id="queryBtn1" name="button1" class="u-button" onclick="toSaveCheck();" value="保存"/>
				<input type="hidden" id="queryBtn2" name="button2" class="cssbutton" onclick="toCancelCheck();" value="订单取消"/>
				...
				<input type="hidden" name="orderBack" id="orderBack" class="cssbutton" onclick="toBackCheck();" value="驳回"/>
				<input type="hidden" name="lookBal" id="lookBal" class="cssbutton" onclick="lookBalance();" value="查看余额"/>
				<input type="button" name="button3" class="u-button" onclick="history.back();" value="返回"/>
...
function priceOnBlue(thisRate){
	...
	function billTotalPrice(){
		
		var billAmountTotal=0;
		var billPriceTotal=0;
		var billTaxTotal=0;
		var materialId = document.getElementsByName("materialId");
		for(var i=0; i<materialId.length; i++){
			
			var subStr = materialId[i].value;
			var billAmount = document.getElementById("billAmount"+subStr).value;//开票数量
			var billPrice =document.getElementById("billPrice"+subStr).value;//开票单价
			if(billAmount==""){
				billAmount=0;
			}
			if(billPrice==""){
			...
			var billAmount_s = parseInt(billAmount, 10);//开票数量
			var billPrice_s = parseFloat(billPrice, 10);//开票单价
			var lineBillTotalPrice=billAmount_s*billPrice_s;
			document.getElementById("billTotalPriceHid"+subStr).value = lineBillTotalPrice;//金额
			document.getElementById("billTotalPrice"+subStr).innerHTML = lineBillTotalPrice == 0 ? "0" : amountFormat(lineBillTotalPrice);//金额显示
			//**计算税额
			var lineTaxRate=document.getElementById("billTaxRate"+subStr).value;
			if(lineTaxRate==""){
				lineTaxRate=0;
			}
			var lineTaxRate_s=parseFloat(lineTaxRate, 10);
			var lineTaxPrice=(lineTaxRate_s*lineBillTotalPrice)/100
			document.getElementById("billTotalTax"+subStr).innerHTML= amountFormat(lineTaxPrice);
			document.getElementById("billTotalTaxHid"+subStr).value = lineTaxPrice;//金额
			//
			
			billTaxTotal+=lineTaxPrice;
			billAmountTotal +=billAmount_s;
			billPriceTotal +=lineBillTotalPrice;
		}
		document.getElementById("hejiBillTotalAmount").innerHTML = billAmountTotal;
		document.getElementById("hejiBillTotalPrice").innerHTML = amountFormat(billPriceTotal);
		document.getElementById("invoiceTotal").innerHTML = amountFormat(billPriceTotal);
		document.getElementById("hejiBillTotalTax").innerHTML = amountFormat(billTaxTotal);
		document.getElementById("invoiceTotalHid").value=billPriceTotal;
	}
		  ...
		  var rebateTotalAmount=document.getElementById("rebateAmount").value;
			...
			for(var i=0; i<materialId.length; i++){
				...
				//if(obj1.id.length>=6&&obj1.id.substring(0,6)=="amount"){
					...
					document.getElementById("lineRebateTotalPriceHid"+subStr).value=linePrice;
	...
	function showAvailableAmount(json){
		...
		var actReqStatus=document.getElementById("actReqStatus").value;
		var actReqTotalPrice=document.getElementById("actReqTotalPrice").value;
		
		if(actReqStatus==<%=Constant.ORDER_REQ_STATUS_CYBH%>||actReqStatus==<%=Constant.ORDER_REQ_STATUS_05%>){
			obj1.innerHTML = amountFormat(parseFloat(json.returnValue, 10)+parseFloat(actReqTotalPrice,10));
			obj2.value = parseFloat(json.returnValue1, 10)+parseFloat(actReqTotalPrice,10);
	...
	function addRow(json){	
		if(parseFloat(json.info.PRICE, 10) < <%=Constant.MATERIAL_PRICE_MAX%>){	
			...
			newCell.innerHTML = "<input type='text' class='mini_txt' id='reserveAmount"+timeValue+"' name='reserveAmount' value='0' ><a href='#' onclick='patchNoSelect(\""+json.info.MATERIAL_ID+"\")'  datatype='0,is_digit,6'><font color='red'>*</font>[审核]</a>";
	...
	function toSaveCheck(){
		...
		if(submitForm('fm')){
			...
			var yiBillAmount=document.getElementsByName("yiBillAmount");
			var billPrice = document.getElementsByName("billPrice");
			var reserveAmount = document.getElementsByName("reserveAmount");
			for(var i=0 ;i< detailId.length; i++){
				if(billAmount[i].value<0){
					MyAlert("开票数量不能为负数！！");
					return;
				}
				if(billAmount[i].value>(reserveAmount[i].value-yiBillAmount[i].value)){
					MyAlert("开票数量不能大于保留数量！！");
					return;
				}
				if(billAmount[i].value!=null&&billAmount[i].value!=''){
					if(!/^\d+$/.test(billAmount[i].value)){
						MyAlert("开票数量请填写数字!！");
						return;
					}
				}
				if(billAmount[i].value>0&&(billPrice[i].value==""||billPrice[i].value<=0)){
					MyAlert("请填写开票价格!！");
					return;
				}
				if(isNaN(billPrice[i].value)){
					MyAlert("开票价格格式错误!！");
					return;
				}
			}
			var hejiBillTotalAmount = document.getElementById("hejiBillTotalAmount").value;
			if(hejiBillTotalAmount <= 0){
				MyAlert("本次开票数量不能小于或等于0！");
				return;
			}
			var returnStr = "是否确认开票？";
	...
	function putForword(){
		document.getElementById("queryBtn1").disabled = true ;
		makeNomalFormCall("<%=contextPath%>/sales/accountsmanage/InvoiceManage/invociceManageSave.json",showForwordValue,'fm');
	}
	
	function showForwordValue(json){
		if(json.returnValue == '1'){
			MyAlert("开票完成！！！");
			document.fm.action = "<%=contextPath%>/sales/accountsmanage/InvoiceManage/invoiceManageInit.do" ;
			document.fm.submit() ;
		}else if(json.returnValue == '2'){
			MyAlert("数据已被修改,保存失败！");
			document.getElementById("queryBtn1").disabled = false ;
			...
			if(${map.ORDER_TYPE } == <%=Constant.ORDER_TYPE_03%>) {
				//useableBtn($("orderBack"));
				useableBtn(document.getElementById("orderBack"));
			}
		  ...
		  //if(json.rebateList.length==1){
			 ...
			 //MyAlert("===totalAmount=="+totalAmount)
			  ...
			  if(json.rebateAmount>0){
			 	...
			 	var actReqStatus=document.getElementById("actReqStatus").value;
				var actRebateAmount=document.getElementById("actRebateAmount").value;
				//MyAlert(actReqStatus);
				if(actReqStatus==<%=Constant.ORDER_REQ_STATUS_CYBH%>||actReqStatus==<%=Constant.ORDER_REQ_STATUS_05%>){
					//MyAlert(actRebateAmount);
					document.getElementById("rebate_account_amount").innerHTML=amountFormat(parseFloat(json.rebateAmount,10)+parseFloat(actRebateAmount,10));
				}else{
