...
<div class="navigation"><img src="<%=request.getContextPath()%>/img/nav.gif" />&nbsp;当前位置： 集团客户管理 &gt; 集团客户支持&gt; 集团客户打印查询</div>
...
<script type="text/javascript" >
	var myPage;
	//查询路径 
	var url = "<%=contextPath%>/sales/fleetmanage/fleetSupport/FleetSupportApply/fleetSupportQuery.json";
	var title = null;
	var columns = [
				...
				{header: "打印状态",dataIndex: 'IS_PRINT', align:'center'},
				{id:'action',header: "操作", dataIndex: 'FLEET_ID', align:'center',renderer:myLink}
		      ];
    //设置超链接
    function myLink(value,meta,record){
    		return String.format("<a href='#' onclick='searchServiceInfo("+ value +","+record.data.INTENT_ID+")'>[明细]</a>|<a href='#' onclick='markAsExport("+ value +","+record.data.INTENT_ID+")'>[标记为已打印]</a>");
    }
    //标记链接
	function markAsExport(value1,value2){
		curl= "<%=contextPath%>/sales/fleetmanage/fleetSupport/FleetSupportApply/markAsPrintAlready.json";
		cfleetId=value1;
	 	makeCall(curl,markPrintFleetReturn,{fleetId:cfleetId}); 
	}
	
	//标记导出回调函数
  	function markPrintFleetReturn(json){
  		if(json.flag==0){
  			MyAlert("标记失败！");
  		}else{
  			MyAlert("标记成功！");
	...
	function searchServiceInfo(value1,value2){
		$('fm').action= '<%=contextPath%>/sales/fleetmanage/fleetSupport/FleetSupportApply/fleetSupportPrintDetail.do?&fleetId='+value1+'&intentId='+value2;
	 	$('fm').target="_blank";
	 	$('fm').submit();
	}
	//设置超链接
