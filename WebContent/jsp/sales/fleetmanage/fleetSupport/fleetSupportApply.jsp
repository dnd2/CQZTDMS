...
<div class="navigation"><img src="<%=request.getContextPath()%>/img/nav.gif" />&nbsp;当前位置： 集团客户管理 &gt; 集团客户支持&gt; 集团客户支持申请</div>
...
<script type="text/javascript" >
	var myPage;
	//查询路径 
	var url = "<%=contextPath%>/sales/fleetmanage/fleetSupport/FleetSupportApply/fleetSupportApplyQuery.json";
	var title = null;
	var columns = [
    ...
    function myLink(value,meta,record){
    	return String.format("<a href='#' onclick='searchServiceInfo("+ value +")'>[意向明细]</a>");
    }
 	//明细链接
	function searchServiceInfo(value){
		$('fm').action= '<%=contextPath%>/sales/fleetmanage/fleetSupport/FleetSupportApply/fleetSupportApplyDetailQuery.do?&fleetId='+value;
	 	$('fm').submit();
	}
	//设置超链接
