...
<!-- 查询条件 begin -->
	<table class="table_query" >
		...
		<tr>
			...
			<td align="right">&nbsp;</td>
			<td align="left">&nbsp;</td>
			<td align="left"><input name="button2" id="queryBtn" type="button" class="cssbutton" onClick="__extQuery__(1);" value="查询"></td>
		</tr>
...
<script type="text/javascript" >
	var myPage;
	//查询路径 
	var url = "<%=contextPath%>/sales/fleetmanage/fleetSupport/FleetContractsMaintain/dealerContractsMaintainQuery.json";
	var title = null;
	var columns = [
				{header: "申请单位",dataIndex: 'COMPANY_NAME',align:'center'},
				{header: "申请日期",dataIndex: 'SUBMIT_DATE',align:'center'},
				...
				{header: "操作", dataIndex: 'MYFLEETNAME', align:'center',renderer:myLink}
		      ];
    //设置超链接
    function myLink(value,meta,record){
       	return String.format("<a href='#' onclick='searchServiceInfo("+record.data.FLEET_ID+")'>["+value+"]</a>");
    }
 	//明细链接
	function searchServiceInfo(value1){
		$('fm').action= '<%=contextPath%>/sales/fleetmanage/fleetSupport/FleetContractsMaintain/dealerContractsMaintainDetailQuery.do?&fleetId='+value1;
	 	$('fm').submit();
	}
	
	//修改链接
	//function searchServiceupdate(value1,value2){
	//	$('fm').action= '/sales/fleetmanage/fleetSupport/FleetContractsMaintain/fleetContractsUpdate.do?&fleetId='+value1+'&intentId='+value2;
	// 	$('fm').submit();
	//}
