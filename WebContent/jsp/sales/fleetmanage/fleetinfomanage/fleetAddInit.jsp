...
<div class="navigation"><img src="<%=request.getContextPath()%>/img/nav.gif" />&nbsp;当前位置： 集团客户管理 &gt; 集团客户支持&gt; 集团客户新增</div>
...
<!-- 查询条件 begin -->
	<table class="table_query" >
		...
		<tr>
			...
			<td width="20%" align="left"><input name="button2" type=button class="cssbutton" onClick="__extQuery__(1);" value="查询">&nbsp;&nbsp;<input name="button1" type=button class="cssbutton" onClick="fleetAddPre();" value="新增"></td>
		</tr>
	</table>
<!-- 查询条件 end -->   
<!--分页 begin -->
...
<script type="text/javascript" >
	var myPage;
	//查询路径 
	var url = "<%=contextPath%>/sales/fleetmanage/fleetInfoManage/FleetFollow/fleetAddQuery.json";
	var title = null;
	var columns = [
    ...
    function myLink(value,meta,record){
    	return String.format("<a href='#' onclick='searchServiceInfo("+record.data.FLEET_ID+")'>[修改]</a>");
    }
 	//明细链接
	function searchServiceInfo(value1){
		$('fm').action= '<%=contextPath%>/sales/fleetmanage/fleetInfoManage/FleetFollow/fleetUpdatePre.do?&fleetId='+value1;
	 	$('fm').submit();
	}
	//设置超链接
    ...
    function doInit(){
  	...
  	function fleetAddPre(){
  		location.href="<%=contextPath%>/sales/fleetmanage/fleetInfoManage/FleetFollow/fleetAddPre.do"
  	}
</script>
<!--页面列表 end -->
</body>
