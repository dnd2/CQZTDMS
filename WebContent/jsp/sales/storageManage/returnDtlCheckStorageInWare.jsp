...
function passSub() {
	...
	var url = "<%=contextPath%>/sales/storageManage/ReturnVehicleReq/returnDtlPassInWare.json" ;
...
<body > 
<div class="wbox">
<div class="navigation"><img src="<%=contextPath%>/img/nav.gif" />&nbsp;当前位置：  整车销售 &gt; 库存管理 &gt; 退车明细审核(区域)</div>
		...
		<div class="form-body">
			...
			<table class="table_query">
	            ...
	            <tr>
					...
					<td class="left"  colspan="3" width="40%">
							<c:forEach items="${houseList }" var="item">
								${item.WAREHOUSE_NAME }
								<input type="hidden" id="warHouseId" name="warHouseId" value="${item.WAREHOUSE_ID }">
							</c:forEach>
					</td>
					<td class="left" colspan="4" width="50%">
					
						<input type="button"  class="u-button u-submit" name="passIt" id="passIt" value="入  库" onclick="toPass();" />&nbsp;
					   <input type="button" class="normal_btn" name="ReturnIt" id="ReturnIt" value="" onclick="toReturn() ;" style="display: none;"/> 
					</td>
				</tr>
			</table>
		</div>
