...
<div class="form-panel">
	<h2><img src="<%=request.getContextPath()%>/jmstyle/img/search-ico.png" style="margin-bottom: -13px;"/> 车辆验收</h2>
<div class="form-body">
<form id="fm" name="fm" method="post"><input type="hidden"
	...
	<input type="hidden" id="hidForm1" name="hidForm1" value="${flag }"/>
...
<c:if test="${oemFlag == 0}">
 ...
 3、如因为经销商自己入错库房而造成报表数据统计错误的，后果自负。</font></strong>
	...
	<tr>
		<td align="right">
		<div align="right">发运申请单号：</div>
		</td>
		<td align="left"><input type="text" name="dlvNo" id="dlvNo" class="middle_txt"/></td>
...
<table class="table_query" border="0" id="myTal">
	...
	<tr>
		...
		<td>
				<input name="t1" readonly="readonly" id="t1" value="" type="text" class="middle_txt" datatype="1,is_date,10"  onFocus="WdatePicker({el:$dp.$('t1'), maxDate:''})"  style="cursor: pointer;width: 80px;">
<!--               <input name='button3' value=" " type='button' class='time_ico' title="点击选择时间" onclick="showcalendar(event, 't1', false);"/>
 -->		</td>
	...
	<tr>
		...
		<td class=datadetail align=left colspan=3><input type="text" class="middle_txt" id="remarkA" rows="2" cols="50"  style="width:50%;height:50px" /></td>
	</tr>
</table>
<table class="table_query"  border="0" align="center" id="myTal__A">
	<tr>
		<td align="center"><input name="button" type="button" id="checkBtn" class="normal_btn" onclick="checkSubmit();" value="检查完成" /> 
		</td>
	</tr>
</table>
</form>
...
<script type="text/javascript"><!--
	...
	var url = "<%=contextPath%>/sales/storageManage/CheckVehicle/checkList.json?COMMAND=1";
	...
	var columns = [
	            ...
	            {header: "发运单号", dataIndex: 'SENDCAR_ORDER_NUMBER', align:'center',renderer:mySel},
				{header: "承运商", dataIndex: 'SHIP_METHOD_CODE', align:'center',renderer:mySel},
				{header: "司机", dataIndex: 'MOTORMAN', align:'center',renderer:mySel},
				{header: "司机电话", dataIndex: 'MOTORMAN_PHONE', align:'center',renderer:mySel},
				/* {header: "当前物流位置", dataIndex: 'ADDRESS', align:'center',renderer:mySel}, */
		...
		var flag=document.getElementById("hidForm1").value;
		if(flag==1){
	...
	function myLink(vehicle_id,metaDate,record){
		var formatStr="<a href=\"<%=contextPath%>/sales/storageManage/CheckVehicle/toCheck.do?vehlId=" + vehicle_id +"&dlvryDtlId=" + record.data.TTDED_ID + " \"  class='u-anchor'>[验收]</a>";
	...
	function mySel(sendcar_order_number,metaDate,record){
		...
		if(is_red == 1){
			return String.format("<font style='border:1px;color:#FF0000'>" + sendcar_order_number + "</div>");
			//return String.format(sendcar_order_number)
			//document.get
		}else{
		
		
			return String.format("<font style='border:1px; color:#FFFFFF'>" + sendcar_order_number + "</div>");
		}
