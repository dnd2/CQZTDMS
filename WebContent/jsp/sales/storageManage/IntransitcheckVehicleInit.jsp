...
function chkAllErpNo(oChkAll) {
	...
	function getWareList(value) {
		var url = "<%=contextPath%>/sales/storageManage/IntransitCheckVehicle/getWareNew_SUZUKI.json" ;
	...
	function submitAction() {
		...
		var url = "<%=contextPath%>/sales/storageManage/IntransitCheckVehicle/checkAllSubmit.json" ;
...
<script type="text/javascript"><!--
	...
	var url = "<%=contextPath%>/sales/storageManage/IntransitCheckVehicle/checkList.json?COMMAND=1";
	...
	var columns = [
				...
				{header: "承运商联系人", dataIndex: 'MOTORMAN', align:'center',renderer:mySel},
				{header: "承运商联系电话", dataIndex: 'MOTORMAN_PHONE', align:'center',renderer:mySel},
				{header: "当前物流位置", dataIndex: 'CYS_SSDD', align:'center',renderer:mySel},
				{header: "经销商代码", dataIndex: 'DEALER_CODE', align:'center',renderer:mySel},
				{header: "经销商名称", dataIndex: 'DEALER_SHORTNAME', align:'center',renderer:mySel},
	...
	function myLink(vehicle_id,metaDate,record){
		var formatStr="<a href='#' onclick='lookLogisticsInfo("+vehicle_id+")'>[查看物流信息]</a>";
    ...
    function openDlvryERP(sendcarId) {
		var url = '<%=contextPath%>/sales/storageManage/IntransitCheckVehicle/openDlvryERP.do?sendcarId=' + sendcarId;
	...
	function lookLogisticsInfo(vin){
		OpenHtmlWindow('<%=request.getContextPath()%>/sales/storageManage/IntransitCheckVehicle/tolookLogistics.do?vin_id='+vin,800,600);
	}
 </script>
</body>
</html>