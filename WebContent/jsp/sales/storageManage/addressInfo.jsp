...
{  
	...
	var status = '${status}';
	if("10011001" == status+""){
		document.getElementById("addCode").disabled = "disabled";
		document.getElementById("address").disabled = "disabled";
		document.getElementById("receiveOrg").disabled = "disabled";
		...
		document.getElementById("queryBtn1").style.display = "none";
	}
...
function showResult(json) {
	...
	if(!aAreaId || aAreaId == "") {
		...
		for(var i=0; i<iLen; i++) {
			...
			if(bChkFlag) {
				sStr += "<input type=\"checkBox\" name=\"addressAreas\" id=\"" + aAreaId[i] + "\" value=\"" + aAreaId[i] + "\" checked /><label for=\"" + aAreaId[i] + "\">" + aAreaName[i] + "</label>" ;
			} else {
...
<!--  
	...
	<tr>
		...
		<td>
				<script type="text/javascript">
					genSelBoxExp("isCrossing",<%=Constant.IF_TYPE%>,"",true,"u-select","","false",'');
				</script>
				<font color="red">*</font>
		</td>
		<td colspan="7"></td>
...
<script type="text/javascript">
	...
	function submitAction(flag){
		document.getElementById('fm').action= "<%=request.getContextPath()%>/sales/storageManage/AddressAddApply/insertAction.do?flag="+flag;
		document.getElementById('fm').submit();
	}
