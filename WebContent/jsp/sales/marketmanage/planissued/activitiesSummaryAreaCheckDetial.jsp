...
<input type="hidden" value="${dealer}" name="dealerId" id="dealerId">
	...
	<div><img class="nav" src="<%=contextPath%>/img/subNav.gif" />&nbsp;经销商评估建议</div>
	<table width=100% border="0" align="center" cellpadding="1" cellspacing="1" class="table_query">
	     <tr>
...
function getAvailableAmount(){
	...
	//提报提交
	function toConfirm(){
		makeNomalFormCall('<%=request.getContextPath()%>/sales/marketmanage/planissued/ActivitiesSummaryAreaCheck/activitiesSummaryAreaCheck.json',showResult,'fm');
	}
	//返回方法
	function toBack(){
		$('fm').action='<%=request.getContextPath()%>/sales/marketmanage/planissued/ActivitiesSummaryAreaCheck/activitiesSummaryAreaCheckQueryInit.do';
		$('fm').submit();
	}
	//回调方法
