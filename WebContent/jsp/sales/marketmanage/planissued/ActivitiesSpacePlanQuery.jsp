...
<body>
<div class="navigation"><img src="<%=request.getContextPath()%>/img/nav.gif" />&nbsp;当前位置： 市场活动管理 &gt; 活动信息查询</div>
...
<script type="text/javascript" >
	var myPage;
	//查询路径 
	var url = "<%=contextPath%>/sales/marketmanage/planissued/ActivitiesSpacePlanMake/activitiesPlanQuery.json";
	var title = null;
	var columns = [
    ...
    function myLink(value,meta,record){
    	return String.format("<a href='#' onclick='searchServiceInfo("+ value +","+record.data.SPACE_ID+")'>[修改]</a>");
    }
 	//明细链接
	function searchServiceInfo(value1,value2){
		$('fm').action= '<%=contextPath%>/sales/marketmanage/planissued/ActivitiesSpacePlanMake/activitiesPlanQueryDetail.do?&campaignId='+value1+'&executeId='+value2;
	 	$('fm').submit();
	}
    //初始化
    function doInit(){
