...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<head>
        ...
        function toPass(){
            if(document.getElementById("chkIdea").value.length==0){
                MyAlert("审核意见不能为空!");
                return
            }
            if(document.getElementById("chkIdea").value.length>100){
                MyAlert("审核意见不能超过100个字符!");
                return
            }
            if(confirm("是否确认通过？")){
                makeNomalFormCall('<%=request.getContextPath()%>/sales/marketmanage/planissued/ActivitiesPlanOemCheck/doPass.json', showResult2, 'fm');
            }
        }
        function toReject(){
            if(document.getElementById("chkIdea").value.length==0){
            ...
            if (confirm("是否确认驳回？")) {
                makeNomalFormCall('<%=request.getContextPath()%>/sales/marketmanage/planissued/ActivitiesPlanOemCheck/doReject.json', showResult3, 'fm');
            }
        }
        ...
        function goBack(){
            document.fm.action= '<%=contextPath%>/sales/marketmanage/planissued/ActivitiesPlanOemCheck/activitiesPlanOemCheckInit.do';
            document.fm.submit();
        }
        </script>
	...
	<body>
		<div class="navigation">
			...
			&nbsp;当前位置： 市场活动管理 &gt; 活动方案管理 &gt; 市场活动方案审核(车厂)
		</div>
		...
		<%} %>
			...
			<table width=100% border="0" align="center" cellpadding="1"
				...
				<tr>
					...
					<td colspan="4" align="left">
						<textarea name="chkIdea" id="chkIdea" datatype="0,is_textarea,100"
				...
				<tr>
					<td colspan="4" align="center">
						...
						<input type="button" class="cssbutton" name="button6"
							onClick="toPass();" value="通过" />
						<input type="button" class="cssbutton" name="button5"
							onClick="toReject();" value="驳回" />
