...
<title>市场活动方案审核</title>
<% String contextPath = request.getContextPath();  %>
<script type="text/javascript">
</script>
</head>
<body>
<div class="navigation"><img src="<%=request.getContextPath()%>/img/nav.gif" />&nbsp;当前位置： 市场活动管理  &gt; 活动方案管理   &gt; 市场活动方案审核(车厂)</div>
...
<script type="text/javascript" >
	var myPage;
	//查询路径 
	var url = "<%=contextPath%>/sales/marketmanage/planissued/ActivitiesPlanOemCheck/activitiesPlanOemCheckQuery.json";
	var title = null;
	var columns = [
    ...
    function myLink(value,meta,record){
    	return String.format("<a href='#' onclick='searchServiceInfo(" + value + "," + record.data.EXECUTE_ID + "," + record.data.CAMPAIGN_TYPE + "," + record.data.ORG_ID + ")'>[审核]</a>");
    }
 	//明细链接
	function searchServiceInfo(value1,value2,value3,value4){
		$('fm').action= '<%=contextPath%>/sales/marketmanage/planissued/ActivitiesPlanOemCheck/activitiesPlanOemCheckDetail.do?&campaignId='+value1+'&executeId='+value2+'&camType='+value3+'&orgId='+value4;
	 	$('fm').submit();
	}
    //初始化
    function doInit(){
