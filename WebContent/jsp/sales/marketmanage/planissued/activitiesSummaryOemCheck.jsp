...
value="${po.AREA_ID}">${po.AREA_NAME}</option>
					</c:forEach>
				</select>
...
/>
		...
		<tr>
			...
			<td width="20%" align="left">
			    ...
			    <input name="camType" id="camType" type="hidden" value="">
...
<script type="text/javascript" >
	var myPage;
	//查询路径 
	var url = "<%=contextPath%>/sales/marketmanage/planissued/ActivitiesSummaryOemCheck/activitiesSummaryOemCheckQuery.json";
	var title = null;
	var columns = [
    ...
    function myLink(value,meta,record){
    	return String.format("<a href='#' onclick='toCheckDetial("+record.data.CAMPAIGN_ID+","+record.data.EXECUTE_ID + "," + record.data.CAMPAIGN_TYPE + ")'>[审批]</a>");
    }
    //取消链接
    function toCheckDetial(value1,value2, value3){
    	document.getElementById("campaignId").value= value1;
    	document.getElementById("executeId").value= value2;
    	document.getElementById("camType").value= value3;
    	$('fm').action= '<%=contextPath%>/sales/marketmanage/planissued/ActivitiesSummaryOemCheck/showActivitiesSummaryOemCheckDetial.do';
	 	$('fm').submit();
    }
    //初始化
    function doInit(){
