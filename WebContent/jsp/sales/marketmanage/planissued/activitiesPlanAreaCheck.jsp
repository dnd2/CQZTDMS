...
<div class="navigation"><img src="<%=request.getContextPath()%>/img/nav.gif" />&nbsp;当前位置： 市场活动管理 &gt; 活动执行方案审批</div>
	...
	<table class="table_query" >
		...
		<tr>
			...
			<td>
				...
				<input name="button3" type="button"  class="mini_btn" onclick="showOrgDealer('dealerCode','','true','${orgId}');" value="..." />
			</td>
			<td></td>
		</tr>
		<tr>
...
<script type="text/javascript" >
	var myPage;
	//查询路径 
	var url = "<%=contextPath%>/sales/marketmanage/planissued/ActivitiesPlanAreaCheck/activitiesPlanAreaCheckQuery.json";
	var title = null;
	var columns = [
    ...
    function myLink(value,meta,record){
    	return String.format("<a href='#' onclick='searchServiceInfo("+ value +","+record.data.ORG_ID+")'>[审核]</a>");
    }
 	//明细链接
	function searchServiceInfo(value1,value2){
		$('fm').action= '<%=contextPath%>/sales/marketmanage/planissued/ActivitiesPlanAreaCheck/activitiesPlanAreaCheckDetail.do?&campaignId='+value1+'&orgId='+value2;
	 	$('fm').submit();
	}
    //初始化
    function doInit(){
