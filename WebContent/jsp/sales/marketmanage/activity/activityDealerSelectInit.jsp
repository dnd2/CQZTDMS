...
<body>
<div class="navigation"><img src="<%=request.getContextPath()%>/img/nav.gif" />&nbsp;当前位置： 市场活动管理 &gt; 市场活动管理 &gt; 市场活动查询（经销商）</div>
		...
		<tr>
			...
			<td  align="left">
					<script type="text/javascript">
		                genSelBoxExp("status",9192,"",true,"mini_sel","","false",'');
		            </script>
			</td>
		</tr>
		<tr>
			...
			<td width="20%" align="left">
				<input id="bstn" name="queryBtn" type="button" class="cssbutton" onClick="__extQuery__(1);" value="查询">
				<input id="bstn1" name="queryBtn" type="button" class="cssbutton" onClick="downLoad();" value="下载">
			</td>
		</tr>
	</table>
<!--分页 begin -->
...
<script type="text/javascript" >
	var myPage;
	//查询路径
	var url = "<%=contextPath%>/sales/marketmanage/activity/ActivityDealerSelect/doSearch.json";
	var title = null;
	var columns = [
   ...
   //设置超链接
    function myLink(value,meta,record){
    	return String.format("<a href='#' onclick='makePlan("+record.data.ACTIVITY_ID+")'>[进入]</a>");
    }
    function makePlan(activityId){
        document.fm.action= '<%=contextPath%>/sales/marketmanage/activity/ActivityDealerSelect/dtlPre.do?activityId='+activityId;
        document.fm.submit();
    }
    //初始化
    function doInit(){
	...
	function downLoad(){
		document.fm.action='<%=contextPath%>/sales/marketmanage/activity/ActivityDealerSelect/doDownload.json';
