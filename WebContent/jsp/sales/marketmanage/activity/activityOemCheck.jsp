...
function toConfirm(value){
   makeNomalFormCall(g_webAppName + '/sales/marketmanage/activity/ActivityOemCheck/doCheck.json?operateFlag='+value, showResult, 'fm');
}
//回调方法
function showResult(json){
    if (json.returnValue == '1') {
        window.parent.MyAlert("操作成功！");
        $('fm').action = g_webAppName + '/sales/marketmanage/activity/ActivityOemCheck/doInit.do';
        $('fm').submit();
    }
    else {
...
function delAttach(value){
    ...
    document.getElementById("delAttachs").value = delAttachs;
        ...
        function goBack(){
            document.fm.action= '<%=contextPath%>/sales/marketmanage/activity/ActivityOemCheck/doInit.do';
            document.fm.submit();
        }
        function switchData(){
        	var totalPre=$("totalPre").value;
        	var rate=$("ratio").value;
        	$("totalSupport").value=parseFloat(parseFloat(totalPre)*parseFloat(rate)/100).toFixed(2);
        }
        function initData(){
        	var totalPre=$("totalPre").value;
        	var totalSupport=$("totalSupport").value;
        	$("ratio").value=parseFloat(parseFloat(totalSupport)/parseFloat(totalPre)*100).toFixed(2);
        }
        </script>
	...
	<body onload="initData();">
		...
		<input type="hidden" name="status" id="status" value="${ttcp.status}" />
			...
			<table width=100% border="0" align="center" cellpadding="1"
				...
				<tr>
					...
					<input type="text" class="middle_txt" name="totalPre"
							id="totalPre" datatype="0,isMoney,30"
							 value="${ttcp.totalPre}" onchange="switchData();" />
					</td>
					<td class="table_query_2Col_label_5Letter">
						公司支持费用(含税、万元)：
					</td>
					<td>
					<input type="text" class="middle_txt" name="totalSupport"
							id="totalSupport" datatype="0,isMoney,30"
							 value="${ttcp.totalSupport}" readonly/>
					</td>
				</tr>
				<tr>
					<td class="table_query_2Col_label_6Letter">
				...
				<tr>
					...
					<td>
						<input type="text" class="middle_txt" name="ratio"
							id="ratio" datatype="0,isMoney,30"
							 value="${ttcp.supportRatio}"  onchange="switchData();"/>%
					</td>
					</tr>
				<tr>
					<td class="table_query_2Col_label_6Letter">
		...
		<c:forEach items="${list3}" var="list3" varStatus="vstatus1">
            ...
            <div>
				...
				cellspacing="1" class="table_query">
				<tr>
				<td align="center"> <textarea rows="5" cols="30" name="auditRemark" id="auditRemark"></textarea></td>
				</tr>
			</table>
