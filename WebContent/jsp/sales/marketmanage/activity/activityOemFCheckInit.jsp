...
<title>市场活动审核（车厂）查询</title>
<% String contextPath = request.getContextPath();  %>
</head>
<body>
<div class="navigation"><img src="<%=request.getContextPath()%>/img/nav.gif" />&nbsp;当前位置： 市场活动管理 &gt; 市场活动管理 &gt; 市场活动终审(车厂)</div>
...
<script type="text/javascript" >
	var myPage;
	//查询路径
	var url = "<%=contextPath%>/sales/marketmanage/activity/ActivityOemFCheck/doSearch.json";
	var title = null;
	var columns = [
   ...
   //设置超链接
    function myLink(value,meta,record){
    	return String.format("<a href='#' onclick='makePlan("+record.data.ACTIVITY_ID+")'>[审核]</a>");
    }
    function makePlan(activityId){
        document.fm.action= '<%=contextPath%>/sales/marketmanage/activity/ActivityOemFCheck/dtlPre.do?activityId='+activityId;
        document.fm.submit();
    }
    //初始化
    function doInit(){
