...
<title>市场活动方案制定</title>
<% String contextPath = request.getContextPath();  %>
</head>
<body>
<div class="navigation"><img src="<%=request.getContextPath()%>/img/nav.gif" />&nbsp;当前位置： 市场活动管理 &gt; 活动方案管理 &gt; 市场活动填报（经销商）查询</div>
	...
	<tr>
		...
		<tr>
			...
			<td width="20%" align="left">
				...
				<input name="queryBtn" type="button" class="cssbutton" onClick="activityApplyAdd();" value="新增">
			</td>
		</tr>
	</table>
<!-- 查询条件 end -->
...
<script type="text/javascript" >
	var myPage;
	//查询路径
	var url = "<%=contextPath%>/sales/marketmanage/activity/ActivityDealer/doSearch.json";
	var title = null;
	var columns = [
    ...
    function myLink(value,meta,record){
    	return String.format("<a href='#' onclick='makePlan("+record.data.ACTIVITY_ID+")'>[活动修改]</a>");
    }
    function makePlan(activityId){
        document.fm.action= '<%=contextPath%>/sales/marketmanage/activity/ActivityDealer/modPre.do?activityId='+activityId;
        document.fm.submit();
    }
    //初始化
    function doInit(){
	...
	function activityApplyAdd(){
	 document.fm.action= '<%=contextPath%>/sales/marketmanage/activity/ActivityDealer/addPre.do';
     document.fm.submit();
	}
</script>
<!--页面列表 end -->
