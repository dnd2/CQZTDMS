...
<title>市场活动取消和延期</title>
<% String contextPath = request.getContextPath();  %>
</head>
<body>
<div class="navigation"><img src="<%=request.getContextPath()%>/img/nav.gif" />&nbsp;当前位置： 市场活动管理 &gt; 市场活动管理 &gt; 市场活动取消和延期查询</div>
...
<script type="text/javascript" >
	var myPage;
	//查询路径
	var url = "<%=contextPath%>/sales/marketmanage/activity/ActivityCancel/doSearch.json";
	var title = null;
	var columns = [
   ...
   //设置超链接
    function myLink(value,meta,record){
    	return String.format("<a href='#' onclick='makePlan("+record.data.ACTIVITY_ID+","+record.data.STATUS+")'>[取消]</a>/<a href='#' onclick='makePre("+record.data.ACTIVITY_ID+","+record.data.STATUS+")'>[延期]</a>");
    }
    function makePlan(activityId,status){
 
    	if(confirm("确认取消")){
    		makeNomalFormCall('<%=contextPath%>/sales/marketmanage/activity/ActivityCancel/doCancel.json?activityId='+activityId+'&status='+status, showResult, 'fm');
    	}
    }
    //回调方法
function showResult(json){
    if (json.returnValue == '1') {
        ...
        $('fm').action =  '<%=contextPath%>/sales/marketmanage/activity/ActivityCancel/doInit.do';
        $('fm').submit();
    }
    else {
...
function makePre(activityId){
		document.fm.action= '<%=contextPath%>/sales/marketmanage/activity/ActivityCancel/dtlPre.do?activityId='+activityId;
        document.fm.submit();
}
