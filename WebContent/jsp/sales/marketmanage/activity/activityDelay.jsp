...
function toConfirm(value){
   makeNomalFormCall(g_webAppName + '/sales/marketmanage/activity/ActivityCancel/doDelay.json?operateFlag='+value, showResult, 'fm');
}
//回调方法
function showResult(json){
    if (json.returnValue == '1') {
        window.parent.MyAlert("操作成功！");
        $('fm').action = g_webAppName + '/sales/marketmanage/activity/ActivityCancel/doInit.do';
        $('fm').submit();
    }
    else {
...
function delAttach(value){
    ...
    document.getElementById("delAttachs").value = delAttachs;
        ...
        function goBack(){
            document.fm.action= '<%=contextPath%>/sales/marketmanage/activity/ActivityCancel/doInit.do';
            document.fm.submit();
        }
	...
	<body>
		...
		<input type="hidden" name="status" id="status" value="${ttcp.status}" />
            ...
            <div>
				...
				cellspacing="1" class="table_query">
				<tr>
				<td align="center"> <textarea rows="5" cols="30" name="delayRemark" id="delayRemark"></textarea></td>
				</tr>
			</table>
