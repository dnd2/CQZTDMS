...
function toConfirm(value){
   makeNomalFormCall(g_webAppName + '/sales/marketmanage/activity/ActivityAreaCheck/doCheck.json?operateFlag='+value, showResult, 'fm');
}
//回调方法
function showResult(json){
    if (json.returnValue == '1') {
        window.parent.MyAlert("操作成功！");
        $('fm').action = g_webAppName + '/sales/marketmanage/activity/ActivityAreaCheck/doInit.do';
        $('fm').submit();
    }
    else {
...
function delAttach(value){
    ...
    document.getElementById("delAttachs").value = delAttachs;
        ...
        function goBack(){
            document.fm.action= '<%=contextPath%>/sales/marketmanage/activity/ActivityAreaCheck/doInit.do';
            document.fm.submit();
        }
