...
<body>
<div class="navigation"><img src="<%=request.getContextPath()%>/img/nav.gif" />&nbsp;当前位置： 市场活动管理 &gt; 活动方案管理 &gt; 市场活动变更/取消查询(经销商)</div>
...
<script type="text/javascript" >
	var myPage;
	//查询路径
	var url = "<%=contextPath%>/sales/marketmanage/planmanage/ActivitiesPlanChngQuery/doSearch.json";
	var title = null;
	var columns = [
    ...
    function myLink(value,meta,record){
          return String.format("<a href='#' onclick='toCheckRecords("+record.data.CAMPAIGN_ID+","+record.data.SPACE_ID + ","+record.data.PLAN_ID + ","+record.data.REQ_ID + ")'>[审批记录]</a>");
    }
    function toCheckRecords(campaignId,spaceId,planId,reqId){
        document.fm.action= '<%=contextPath%>/sales/marketmanage/planmanage/ActivitiesPlanChngQuery/toCheckRecordsPre.do?campaignId='+campaignId+'&spaceId='+spaceId+'&planId='+planId+'&reqId='+reqId;
        document.fm.submit();
    }
    //初始化
    function doInit(){
