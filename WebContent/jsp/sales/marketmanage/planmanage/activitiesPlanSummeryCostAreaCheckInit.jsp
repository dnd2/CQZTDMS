...
<title>市场活动总结费用审核(事业部)</title>
<% String contextPath = request.getContextPath();  %>
</head>
<body>
<div class="navigation"><img src="<%=request.getContextPath()%>/img/nav.gif" />&nbsp;当前位置： 市场活动管理 &gt; 活动方案管理 &gt; 市场活动总结费用审核(事业部)</div>
...
<script type="text/javascript" >
	var myPage;
	//查询路径
	var url = "<%=contextPath%>/sales/marketmanage/planmanage/ActivitiesPlanSummeryCostAreaCheck/doSearch.json";
	var title = null;
	var columns = [
				...
				{header: "活动总结状态",dataIndex: 'STATUS',align:'center',renderer:getItemValue},
				{header: "费用类型",dataIndex: 'SUMMERY_COST_TYPE',align:'center',renderer:getItemValue},
				{header: "活动总结提报单位",dataIndex: 'DEALER_NAME',align:'center'},
    ...
    function myLink(value,meta,record){
        if (record.data.STATUS==13291002) {
    	    return String.format("<a href='#' onclick='makePlan("+record.data.CAMPAIGN_ID+","+record.data.SPACE_ID + ","+record.data.PLAN_ID + ")'>[审核]</a>");
        }else{
            return String.format("<a href='#' onclick='makePlan("+record.data.CAMPAIGN_ID+","+record.data.SPACE_ID + ","+record.data.PLAN_ID + ")'>[查看]</a>");
        }
    }
    function makePlan(campaignId,spaceId,planId){
        document.fm.action= '<%=contextPath%>/sales/marketmanage/planmanage/ActivitiesPlanSummeryCostAreaCheck/detailPre.do?campaignId='+campaignId+'&spaceId='+spaceId+'&planId='+planId;
        document.fm.submit();
    }
    //初始化
    function doInit(){
