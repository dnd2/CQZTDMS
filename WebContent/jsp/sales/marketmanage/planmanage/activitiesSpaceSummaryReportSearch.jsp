...
<script type="text/javascript" >
	var myPage;
	//查询路径 
	var url = "<%=contextPath%>/sales/marketmanage/planmanage/ActivitiesSpaceSummaryReport/activitiesSummaryReportQuery.json";
	var title = null;
	var columns = [
    ...
    function myLink(value,meta,record){
    	return String.format("<a href='#' onclick='toSummarize("+record.data.CAMPAIGN_ID+","+record.data.EXECUTE_ID+")'>[总结]</a>");
    }
    //取消链接
    function toSummarize(value1,value2){
    	document.getElementById("campaignId").value= value1;
    	document.getElementById("executeId").value= value2;
    	$('fm').action= '<%=contextPath%>/sales/marketmanage/planmanage/ActivitiesSpaceSummaryReport/showActivitiesSummaryReportPage.do';
	 	$('fm').submit();
    }
