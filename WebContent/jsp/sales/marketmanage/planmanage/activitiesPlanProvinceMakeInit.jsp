...
<title>市场活动方案制定(省份)</title>
<% String contextPath = request.getContextPath();  %>
</head>
<body>
<div class="navigation"><img src="<%=request.getContextPath()%>/img/nav.gif" />&nbsp;当前位置： 市场活动管理 &gt; 活动方案管理 &gt; 市场活动方案制定(省系)</div>
...
<script type="text/javascript" >
	var myPage;
	//查询路径
	var url = "<%=contextPath%>/sales/marketmanage/planmanage/ActivitiesPlanProvinceMake/doSearch.json";
	var title = null;
	var columns = [
    ...
    function myLink(value,meta,record){
    	return String.format("<a href='#' onclick='makePlan("+record.data.CAMPAIGN_ID+","+record.data.SPACE_ID + ","+record.data.PLAN_ID + ")'>[制定方案]</a>");
    }
    function makePlan(campaignId,spaceId,planId){
        document.fm.action= '<%=contextPath%>/sales/marketmanage/planmanage/ActivitiesPlanProvinceMake/modPre.do?campaignId='+campaignId+'&spaceId='+spaceId+'&planId='+planId;
        document.fm.submit();
    }
    //初始化
    function doInit(){
