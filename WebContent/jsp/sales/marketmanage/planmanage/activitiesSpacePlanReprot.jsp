...
<body>
<div class="navigation"><img src="<%=request.getContextPath()%>/img/nav.gif" />&nbsp;当前位置： 市场活动管理 &gt; 活动方案管理 &gt; 市场活动方案制定(事业部)</div>
...
<script type="text/javascript" >
	var myPage;
	//查询路径
	var url = "<%=contextPath%>/sales/marketmanage/planmanage/ActivitiesSpacePlanReport/activitiesSpacePlanQuery.json";
	var title = null;
	var columns = [
    ...
    function myLink(value,meta,record){
    	return String.format("<a href='#' onclick='searchServiceInfo("+record.data.CAMPAIGN_ID+","+record.data.ORG_ID + ")'>[制定方案]</a>");
    }
    //明细链接
    function searchServiceInfo(value1, value2){
    	$('fm').action= '<%=contextPath%>/sales/marketmanage/planmanage/ActivitiesSpacePlanReport/activitiesSpacePlanReportDetail.do?&campaignId='+value1+'&orgId=' + value2;
	 	$('fm').submit();
    }
    //取消校验
    ...
    function toSubmit(value){
    	makeNomalFormCall('<%=contextPath%>/sales/marketmanage/planmanage/ActivitiesSpacePlanReport/activitiesPlanCencle.json?&campaignId='+value,showResult,'fm');
    }
    //回调方法
	function showResult(json){
 	...
 	//新增链接
	function toAdd(){
		//setDisTrue('queryBtn','button2') ;
		$('fm').action= '<%=contextPath%>/sales/marketmanage/planmanage/ActivitiesSpacePlanReport/activitiesPlanReportAddInit.do';
	 	$('fm').submit();
	}
    //初始化
    function doInit(){
