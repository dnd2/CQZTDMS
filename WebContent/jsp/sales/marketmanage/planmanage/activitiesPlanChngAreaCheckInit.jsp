...
<title>市场活动变更/取消审核(大区)</title>
<% String contextPath = request.getContextPath();  %>
</head>
<body>
<div class="navigation"><img src="<%=request.getContextPath()%>/img/nav.gif" />&nbsp;当前位置： 市场活动管理 &gt; 活动方案管理 &gt; 市场活动变更/取消审核(事业部)</div>
...
<script type="text/javascript" >
	var myPage;
	//查询路径
	var url = "<%=contextPath%>/sales/marketmanage/planmanage/ActivitiesPlanChngAreaCheck/doSearch.json";
	var title = null;
	var columns = [
				...
				{header: "变更申请状态",dataIndex: 'CHECK_STATUS',align:'center',renderer:getItemValue},
				{header: "活动变更类型",dataIndex: 'CHNG_TYPE',align:'center',renderer:getItemValue},
				{header: "活动变更提报单位",dataIndex: 'DEALER_NAME',align:'center'},
      			{header: "提报日期",dataIndex: 'REQ_DATE',align:'center'},
				{header: "提报批次",dataIndex: 'REQ_NUM',align:'center'},
    ...
    function myLink(value,meta,record){
        if(record.data.CHECK_STATUS==11261018){
            return String.format("<a href='#' onclick='makePlan("+record.data.CAMPAIGN_ID+","+record.data.SPACE_ID + ","+record.data.PLAN_ID + ","+record.data.REQ_ID + ")'>[审核]</a>");
        }else{
            return String.format("<a href='#' onclick='makePlan("+record.data.CAMPAIGN_ID+","+record.data.SPACE_ID + ","+record.data.PLAN_ID + ","+record.data.REQ_ID + ")'>[查看]</a>");
        }
    }
    function makePlan(campaignId,spaceId,planId,reqId){
        document.fm.action= '<%=contextPath%>/sales/marketmanage/planmanage/ActivitiesPlanChngAreaCheck/detailPre.do?campaignId='+campaignId+'&spaceId='+spaceId+'&planId='+planId+'&reqId='+reqId;
        document.fm.submit();
    }
    //初始化
    function doInit(){
