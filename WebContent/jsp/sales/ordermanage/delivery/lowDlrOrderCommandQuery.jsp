...
<title>下级经销商发运指令查询</title>
<% String contextPath = request.getContextPath(); %>
<script type="text/javascript">
</script>
</head>
<body>
<div class="navigation"><img src="<%=request.getContextPath()%>/img/nav.gif" />&nbsp;当前位置： 销售订单管理 &gt; 订单发运 &gt;下级经销商发运指令查询</div>
...
<script type="text/javascript" >
	var myPage;
	//查询路径           
	var url = "<%=contextPath%>/sales/ordermanage/delivery/OrderCommandQuery/lowDlrCommandQuery.json";
	var title = null;
	var columns = [
	...
	function deliveryCommit(){
		$('fm').action= "<%=contextPath%>/sales/ordermanage/delivery/OrderCommandQuery/lowDlrCommandQueryLoad.json";
		$('fm').submit();
		}
	...
	function infoquery(val1,val2){ 
		$('fm').action= "<%=contextPath%>/sales/ordermanage/delivery/OrderCommandQuery/lowCommandQueryInfo.do?reqId="+val1+"&orderType="+val2;
		$('fm').submit();
	}
