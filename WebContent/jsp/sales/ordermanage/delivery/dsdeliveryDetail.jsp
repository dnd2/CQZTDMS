...
function getAvailableAmount() {
	...
	//}
	var url = "<%= contextPath%>/sales/ordermanage/orderreport/DsOrderReport/getAvailableAmount.json" ;
...
<body>
<div class="navigation"><img src="<%=request.getContextPath()%>/img/nav.gif" />&nbsp;当前位置： 销售订单管理 &gt; 订单发运 &gt;电商订单真实发运申请</div>
...
<input type="hidden" value="${curDealerId}" id="dealerIds" name="dealerIds"/>
	...
	<table class="table_list" width="100%">
		<tr class="cssTable">
			...
			<!--
			<th align="center" nowrap>总数量</th>
			<th align="center" nowrap>已申请数量</th>
			-->
			<th align="center" nowrap>可以提报数量</th>
		...
		<c:forEach items="${list}" var="list" varStatus="vstatus">
			<tr class="<c:if test='${vstatus.index%2==0}'>table_list_row1</c:if><c:if test='${vstatus.index%2!=0}'>table_list_row2</c:if>">
				...
				<td id="priceTd${list.MATERIAL_ID}">
					<c:out value="${list.SINGLE_PRICE}"/>
					<input type="hidden" name="singlePrice" value="<c:out value="${list.SINGLE_PRICE}" />" id="singlePrice${list.MATERIAL_ID}"/>
					
				</td>
				
				<td style="display:none;">
					<c:out value="${list.RESOUCE_COUNT}"/>
				</td>
				<td style="display:none;">
					<c:out value="${list.REQ_AMOUNT}"/>
				</td>
				<td>
					<c:out value="${list.ABLE_APPLY}"/>
				</td>
				<td>
					<input type="text" name="applyAmount" id="applyAmount<c:out value="${list.MATERIAL_ID}"/>" datatype="0,is_digit,6"  size="3" value="0"
					onchange="toChangeNum(document.getElementById('singlePrice${list.MATERIAL_ID}').value,<c:out value="${list.MATERIAL_ID}"/>,<c:out value="${list.RESOUCE_COUNT}"/>,<c:out value="${list.DELIVERY_AMOUNT}"/>,this.value);" 
				     ></input>
					<input type='hidden' id='detailId${list.MATERIAL_ID}' name='detailId${list.MATERIAL_ID}' value='${list.MATERIAL_ID}'>
					<input type='hidden' id='tempPrice${list.MATERIAL_ID}' name='tempPrice${list.MATERIAL_ID}' value='<c:out value="${list.SINGLE_PRICE}"/>'>
				</td>
				<td>
					<span id="<c:out value="${list.MATERIAL_ID}"/>">0.00</span>
					<input type="hidden" id="orderPriceSum${list.MATERIAL_ID}" name="orderPriceSum" value="" />	
		...
		<tr class="table_list_row1">
			...
			<td id="orderAmounts" style="display:none;"></td>
			<td id="checkAmounts" style="display:none;"></td>
	...
	<table class="table_query" id="table1">
		<tr class= "tabletitle">
			...
			<td align="left">
				...
				<input type="hidden" name="reqTotalAmount" id="reqTotalAmount" value=""/>
...
<!--        		</select>-->
		...
		<tr>
			...
			<td align = "left" >
				...
				<input type="hidden" id="receiverName" name="receiverName" value="" />
...
<!--			}-->
	...
	function priceTypeChange(){	
		...
		var url = "<%=request.getContextPath()%>/sales/ordermanage/orderreport/DsOrderReport/getDetailSinglePriceList.json";
	...
	function changeAllAccount(){
		...
		document.getElementById("reqTotalAmount").value =allAmount;
	}
	//本次申请数量与价格合计联动
	function toChangeNum(value1,value2,value3,value4,value5){
		...
		if(applyAmount>(checkAmount-callAmount)){
	        ...
	        var id = <c:out value="${list.MATERIAL_ID}"/>
	...
	function applySubmit(){
		if(submitForm('fm')){
			...
			var oSelect = document.getElementById("receiver");
			document.getElementById("receiverName").value = oSelect.options[oSelect.selectedIndex].text;
	...
	//申请发运
	function putForword(){
		makeNomalFormCall("<%=contextPath%>/sales/ordermanage/delivery/DsOrderDelvy/applySubmit.json",showForwordValue,'fm','queryBtn');
	}
	//回调函数
	function showForwordValue(json){
		if(json.returnValue == '1'){
			window.parent.MyAlert("申请成功！");
			window.location.href="<%=contextPath%>/sales/ordermanage/delivery/DsOrderDelvy/doInit.do";
