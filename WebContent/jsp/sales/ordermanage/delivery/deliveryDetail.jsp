...
<div class="navigation"><img src="<%=request.getContextPath()%>/img/nav.gif" />&nbsp;当前位置： 销售订单管理 &gt; 订单发运 &gt;需求订单发运申请（批量）</div>
	...
	<table class="table_query" id="table1">
		<tr>
			...
			<td align = "center" >
				...
				<input type="hidden" name="orderIds" id="orderId" value=""/>
			</td>
		</tr>
	</table>
...
<script type="text/javascript">
	...
	var columns = [
				...
				{header: "可用资源",dataIndex: 'ACT_STOCK',align:'center'},
				{header: "未满足常规订单",dataIndex: 'UNDO_ORDER_AMOUNT',align:'center'},
				{header: "本次申请数量",dataIndex: 'ORDER_NO',align:'center',renderer:applyAmountLink},
				{header: "价格合计",dataIndex: 'ORDER_NO',align:'center',renderer:priceTotalLink}
		      ];
	function myLink(value,meta,record){
		return String.format('<input type="checkBox" value="'+record.data.ORDER_ID+'" detailId="'+record.data.DETAIL_ID+'" name="checkOrder"  onclick="'+"judgeIfSameOrder(this);"+'" />');
	}
	function executeQuery(){
		...
		url = "<%=contextPath%>/sales/ordermanage/delivery/DeliveryApply/applyDetailQuery.json?&orderYearWeek="+orderYearWeek+"&areaId="+areaId+"&dealerId="+dealerId+"&orderNO="+orderNO;
		__extQuery__(1);
	}
	...
	function callAmountLink(value,meta,record){
			...
			str+='<input type="hidden" name="priceListId" value="'+record.data.PRICTLIST_ID+'"/>';
			return String.format(str);
	...
	function priceTotalLink(value,meta,record){
			...
			str+='<input type="hidden" id="orderPriceSum'+record.data.DETAIL_ID+'" name="orderPriceSum" value="'+record.data.TOTAL_PRICE+'" />';
			return String.format(str);
	}
</script>
<script type="text/javascript" >
