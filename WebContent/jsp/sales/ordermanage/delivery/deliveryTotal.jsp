...
<body>
<div class="navigation"><img src="<%=request.getContextPath()%>/img/nav.gif" />&nbsp;当前位置： 销售订单管理 &gt; 订单发运 &gt;需求订单发运申请</div>
...
<input type="hidden" value="${curDealerId}" id="dealerIds" name="dealerIds"/>
	...
	<table class="table_list" width="100%">
		<tr class="cssTable">
			...
			<th align="center" nowrap>可用资源</th>
			<th align="center" nowrap>未满足常规订单数量</th>
...
<!--      		<th nowrap="nowrap">折扣额</th>-->
		...
		<c:forEach items="${list}" var="list" varStatus="vstatus">
			<tr class="<c:if test='${vstatus.index%2==0}'>table_list_row1</c:if><c:if test='${vstatus.index%2!=0}'>table_list_row2</c:if>">
				...
				<td>
					<c:out value="${list.ACT_STOCK}"/>
				</td>
				<td>
					<c:out value="${list.UNDO_ORDER_AMOUNT}"/>
				</td>
				<td>
					<input type="text" name="applyAmount" id="applyAmount<c:out value="${list.DETAIL_ID}"/>" datatype="0,is_digit,6"  size="3" value="${list.APPLY_AMOUNT}"
...
<form name="fm" id="fm">
	<table class="table_query" id="table1">
		<tr class= "tabletitle">
			...
			<td align="right" nowrap="nowrap" class="table_query_2Col_label_6Letter">订单总价：</td>
			<td align="left" ><span id="allAccount">0.00</span></td>
			<td align="left">
		...
		<tr>
			...
			<td align = "left" >
				...
				<td><input class="normal_btn" name="applBack" type="button" id="back"  value="返回" onclick="history.back();"></td>
				<td></td>
			<td></td>
...
//		discount_price = parseFloat(single_Price)*parseFloat(discount_rate/100) * parseFloat(applyAmount)
	...
	function doInit(){
		...
		sumAll();
		changeAllAccount();//设置订单总金额
		setPrictTotal();//设置价格合计
		//selectAction();
	}
	function setPrictTotal(){
		<c:forEach items="${list}" var="list">
				var price= <c:out value="${list.SINGLE_PRICE}"/>;
				var detailId= <c:out value="${list.DETAIL_ID}"/>;
				var checkAmount=<c:out value="${list.CHECK_AMOUNT}"/>;
				var callAmount= <c:out value="${list.CALL_AMOUNT}"/>;
				var applyAmount= <c:out value="${list.APPLY_AMOUNT}"/>;
		       toChangeNum(price,detailId,checkAmount,callAmount,applyAmount);
		   </c:forEach>
	}
	//资金类型与余额联动
	...
	//申请发运
	function putForword(){
		makeNomalFormCall("<%=contextPath%>/sales/ordermanage/delivery/DeliveryApply/applyDelvSubmit.json",showForwordValue,'fm','queryBtn');
	}
	//回调函数
