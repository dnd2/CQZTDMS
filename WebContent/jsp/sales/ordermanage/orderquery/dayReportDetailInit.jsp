...
<table border="0" align="center" class="table_list" id="activeTable">
	...
	%>			
		<tr align="center" class="<c:if test='${i%2==0}'>table_list_row1</c:if><c:if test='${i%2!=0}'>table_list_row2</c:if>">
			<td><a href="#" onclick="regionDetailReport(<%=map.get("ORG_ID") %>)"><%=map.get("REGION_NAME") %></a></td>
			<%
...
<script type="text/javascript" >
	function regionDetailReport(orgId){
		var flag = <%=flag%>
	...
	function refresh(){
		var flag = ${flag};
		$('fm').action="<%=contextPath%>/sales/ordermanage/orderquery/DayReport/detailReport.do?flag="+flag;
		$('fm').submit();
	}
