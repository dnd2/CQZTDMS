...
<body>
<div class="navigation"><img src="<%=request.getContextPath()%>/img/nav.gif" />&nbsp;当前位置： 销售订单管理 &gt; 订单提报 &gt;批售订单查询</div>
...
<!--			</td>-->
			...
			<td colspan="1">
				...
				<%-- <c:if test="${dutyType==10431004}">
            	 	<input name="button2" id="dbtn" class="mini_btn" type="button" value="&hellip;" onclick="showOrgDealer('dealerCode','','true', '${orgId}');" />
           		 </c:if>
            	<c:if test="${dutyType!=10431004}">
            		<input name="button2" id="dbtn" class="mini_btn" type="button" value="&hellip;" onclick="showOrgDealer('dealerCode','','true', '${orgId}','true');" />
        		</c:if> --%>
				<input class="normal_btn" type="button" value="清空" onclick="clrTxt('dealerCode');"/>
			</td>
			<td align="right"><div align="right">选择物料组：</div></td>
			<td>
				<input type="text" class="middle_txt"  id="groupCode" name="groupCode" size="15" readonly="readonly" value="" onclick="showMaterialGroup('groupCode','groupName','true','4','true');"/>
		...
		<tr>
			...
			<td><input type="text" class="middle_txt" style="width:140px" name="orderNo"  value=""/></td>
			<td></td>
		</tr>
		<tr>
			<td></td>
       ...
       &nbsp;至&nbsp;
		...
		<tr>
            ...
            <td></td>
        <td align="right">
             <input style="display:none" name="pageSize" id="pageSize" type="text" class="mini_txt" value="10" />
        </td>
		</tr>
	</table>
<!-- 查询条件 end -->
...
<script type="text/javascript" >
	...
	var curstr;
	//查询路径           
	var url = "<%=contextPath%>/sales/ordermanage/orderquery/OrderSeach/orderSeachQuery.json";
	var title = null;
	var columns = [
	...
	function myLink(value,meta,record){
		...
		var historyCount = history.length ;
  		return String.format("<a href='#' class='u-anchor'  onclick='toDetailCheck(\""+ value +"\",\""+ orderType +"\",\""+ reqId +"\",\"" + historyCount + "\")'>[查看]</a><input type='hidden' name='ver' value='"+record.data.VER+"'/>");
	}

	function getDetail(){
		$('fm').action= '<%=request.getContextPath()%>/sales/ordermanage/orderquery/OrderSeach/orderSeachDetailQuery.json';
	 	$('fm').submit();
		}
	...
	function checkedStorageAudit(_node){
		...
		//MyAlert(document.getElementById("checkStorageId").value);
	}
	//批量审核
	...
	function showForwordValue(json){
		if(json.returnValue == '1'){
			MyAlert("审核通过！！！");
			$('fm').action = "<%=contextPath%>/sales/ordermanage/orderaudit/OrderStorageAudit/orderStorageQueryPre.do" ;
			$('fm').submit() ;
	...
	function lockReturn(json){
		if(json.flag=="1"){
			...
			value3=curstr[2];
			value4=curstr[3];
			document.fm.action= '<%=request.getContextPath()%>/sales/ordermanage/orderquery/OrderSeach/orderSeachDetailQuery.do?orderId='+value1+'&orderType='+value2+'&reqId='+value3+'&historyCount='+value4;
			document.fm.submit();
		}
	}
	//设置年周
