...
<div class="navigation"><img src="<%=request.getContextPath()%>/img/nav.gif" />&nbsp;销售订单管理 &gt; 订单审核&gt; 订单资源初审</div>
<form method="post" name="fm" id="fm">
	...
	<table class="table_query" align="center">
	 <tbody id="dealerTr">
	    ...
	    <td align="left" valign="top" nowrap="nowrap" width="15%">${map.DEALER_NAME1}</td>
	  ...
	  <tr class="cssTable" >
	   <td align="right" valign="top" nowrap="nowrap" ><div align="right" >资金类型：</div></td>
	    <td align="left" valign="top" nowrap="nowrap" class="table_info_3col_input">
	    	<select name="fundType" style="display:none" disabled id="fundType" onchange="getAvailableAmount(this.options[this.options.selectedIndex].value);">
	      	</select>
	      	${map.PRICE_NAME }
	    </td>
	    <td align="right" valign="top" nowrap="nowrap" ><div align="right" style="display:none">可用余额：</div><div align="right"><span id="span7">订单金额：</span></div></td>
	    <td align="left" valign="top" nowrap="nowrap" class="table_info_3col_input">
	    	<span id="show1" style="display:none"></span><input type="hidden" name="availableAmount" value=""><span id="span8" ></span>
	    </td>
	      <td align = "right" style="color:#252525;width:70px;text-align:right;"><div align="right"><span id="span7" style="display:none">订单金额：</span></div></td>
	  ...
	  <tr class="cssTable" >
	     ...
	     <td  align="right" nowrap><div align="right"><span id="rebate_account" >本次返利金额：</span></div></td>
	    ...
	    <td> <input type="hidden" id="rebateAmount" name="rebateAmount"/></td>
		...
		<tbody id="tbody1">
			<c:forEach items="${list1}" var="list1" varStatus="vstatus">
				<tr align="center" class="table_list_row1">
					...
					<td>
					    <input type="hidden" class="mini_txt" id="reserveActualAmount${list1.MATERIAL_ID}" name="reserveActualAmount" value="${list1.RESERVE_AMOUNT}" >
						<input type="text" class="mini_txt" style="display:none" id="reserveAmount${list1.MATERIAL_ID}" name="reserveAmount" value="${list1.RESERVE_AMOUNT}" onchange="reserveAmount1();" >${list1.RESERVE_AMOUNT}
						<%-- <a href="#" onclick="patchNoSelect('${list1.MATERIAL_ID}','${list1.RESERVE_AMOUNT}');">[审核]</a> --%>
					</td>
					<td><input type="text" id="singlePrice${list1.MATERIAL_ID}" name="singlePrice" size="8" style="text-align:right;display:none" value="${list1.SINGLE_PRICE}" onchange="priceTotal();" /><span id="priceDesc${list1.MATERIAL_ID}">${list1.SINGLE_PRICE}</span></td>
	...
	<table class="table_query">
		...
		<%-- <tr class="cssTable" id="tmp_license_amountTr">
	      ...
	      <td colspan="3" align="left" valign="top" nowrap="nowrap">
	      	<input id="tmp_license_amount" name="tmp_license_amount" type="text" style="display:none" class="middle_txt" size="10" maxlength="30" readonly="readonly" value="${map.TMP_LICENSE_AMOUNT}" datatype="1,is_double,20" decimal="2"/>
	      </td>
	    </tr> --%>
		...
		<tr>
			...
			<td align="left" colspan="3">${map.ORDER_REMARK}<textarea name="orderRemark" id="orderRemark" cols="30" rows="3" style="display:none" readonly="readonly" onkeyup="value = value.replace(/#/g, '') ;" onbeforepaste="clipboardData.setData('text',clipboardData.getData('text').replace(/#/g,''));" onkeydown="if(event.keyCode == 8 || event.keyCode == 46 || event.keyCode == 37 || event.keyCode == 38 || event.keyCode == 39 || event.keyCode == 40){} else {limitChars('orderRemark', 70);}" onchange="limitChars('orderRemark', 70)"><c:out value="${map.ORDER_REMARK}"/></textarea></td>
		</tr>
	</table>
	<br>
	<table class="table_list">
	...
	<table class="table_list">
		<tr class="table_list_row2">
			<td align="center">
				...
				<input type="hidden" id="queryBtn1" name="button1" class="cssbutton" onclick="priceChk();" value="审核完成"/>
...
function priceOnBlue(thisRate){
	...
	function putForword(){
		...
		var fundType=document.getElementById("fundType").value;
		makeNomalFormCall("<%=contextPath%>/sales/ordermanage/orderaudit/OrderResourceReserveFirst/orderResourceReserveSave.json?fundType="+fundType,showForwordValue,'fm');
	}
	
	function showForwordValue(json){
		if(json.returnValue == '1'){
			MyAlert("审核完成！！！");
			document.fm.action = "<%=contextPath%>/sales/ordermanage/orderaudit/OrderResourceReserveFirst/orderResourceReserveQueryPre.do" ;
			document.fm.submit() ;
		}else if(json.returnValue == '2'){
			MyAlert("数据已被修改,保存失败！");
			//useableBtn($("queryBtn1"));
