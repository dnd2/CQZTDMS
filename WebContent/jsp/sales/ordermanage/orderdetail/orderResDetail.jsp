...
<script type="text/javascript">
	...
	function updateRespond(){
		var url="<%=contextPath%>/sales/ordermanage/orderdetail/OrderOperation/respondUpdate.json";
	...
	function resResult(json){
		if(json.flag=='1'){
			MyAlert("修改成功！！！");
			location.href="<%=contextPath%>/sales/ordermanage/orderdetail/OrderOperation/doInit.do";
		}else if(json.flag=='0'){
			MyAlert("免责数量大于审核数量和申请数量的差值了！！！")
		}else{
			MyAlert("未知错误，请联系管理员！！！")
		}
...
<div class="navigation"><img src="<%=request.getContextPath()%>/img/nav.gif" />&nbsp;当前位置： 销售订单管理 &gt; 订单发运 &gt;常规订单免责明细</div>
...
<!--				</td>-->
				...
				<td>
					<input type="text" name="respAmount" id="applyAmount<c:out value="${list.DETAIL_ID}"/>" datatype="0,is_digit,6"  size="3" value="${list.RESPOND_AMOUNT}"
...
<!--				</td>-->
		...
		<tr>
			<td align="right"><input name="updateBtn" type=button class="cssbutton" onClick="updateRespond();" value="修改">&nbsp;&nbsp;&nbsp;</td>
			<td align="left">&nbsp;&nbsp;&nbsp;<input name="cancelBtn" type=button class="cssbutton" onClick="history.back();" value="返回"></td>
		</tr>
		</table>
</form>
</body>
