   ...
   <tr class="table_list_th">
	...
	<c:if test="${dList!=null}">
		<c:forEach items="${dList}" var="list">
			...
			<td>${list.GROUP_NAME}<input type="hidden" id="detailId${list.DETAIL_ID}" name="detailId" value="${list.DETAIL_ID}" /></td>
			<td>${list.MATERIAL_CODE}</td>
			<td>${list.MATERIAL_NAME}</td>
			<td>
				${list.ORDER_AMOUNT}
				<input type="hidden" id="orderAmount${list.DETAIL_ID}" name="orderAmount" value="${list.ORDER_AMOUNT}" />
			</td>
			<td>
...
<script type="text/javascript">
		...
		if (submitForm('fm')){
			...
			if(value == '1'){
				...
				for(var i=0;i<orderAmount.length;i++){
					if(parseInt(checkAmount[i].value)>parseInt(orderAmount[i].value)){
	...
	function checkSubmitAction(value){
		makeNomalFormCall("<%=request.getContextPath()%>/sales/ordermanage/orderaudit/MonthOrderCheck/monthOrderCheck_SAL_submit.json?checkStatus="+value,showDateInfo,'fm');
	}
