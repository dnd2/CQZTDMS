...
<body>
<div class="navigation"><img src="<%=request.getContextPath()%>/img/nav.gif" />&nbsp;当前位置： 销售订单管理 &gt; 订单审核 &gt;订单储运审核</div>
<div class="form-panel">
	<h2><img src="<%=request.getContextPath()%>/jmstyle/img/search-ico.png" style="margin-bottom: -13px;"/>订单储运审核</h2>
<div class="form-body">
<form method="post" name="fm" id="fm">
 <input type="hidden" id="checkStorageId"/>
<!-- 查询条件 begin -->
...
<!--			</td>-->
			...
			<td colspan="1">
				<input type="text" class="middle_txt"  name="dealerCode" size="15" value="" id="dealerCode" onclick="showOrgDealer('dealerCode','','true', '${orgId}','true');"/>
            	...
            	<c:if test="${dutyType!=10431004}">
            		<input name="button2" id="dbtn" class="normal_btn" type="button" value="&hellip;" onclick="showOrgDealer('dealerCode','','true', '${orgId}','true');" />
        		</c:if> --%>
		...
		<tr>
			...
			<td>
				<label>
					<script type="text/javascript">
						genSelBoxExp("orderTypeSel",<%=Constant.ORDER_TYPE%>,"${orderTypeSel}",true,"u-select","","false",'<%=Constant.ORDER_TYPE_02%>');
					</script>
				</label>
			</td>
        ...
        <input class="middle_txt"  type="text"  style="width:100px"  id="t1" name="startDate" datatype="1,is_date,10"  value="" onFocus="WdatePicker({el:$dp.$('t1'), maxDate:'#F{$dp.$D(\'t2\')}'})" style="cursor: pointer;width: 80px;"/>
        <!-- <input class="time_ico" type="button" onClick="showcalendar(event, 't1', false);" value="&nbsp;" /> -->&nbsp;至&nbsp;
        <input class="middle_txt"  type="text"  style="width:100px"  id="t2" name="endDate" datatype="1,is_date,10"  value="" onFocus="WdatePicker({el:$dp.$('t2'), minDate:'#F{$dp.$D(\'t1\')}'})"  style="cursor: pointer;width: 80px;"/>
       <!--  <input class="time_ico" type="button" onClick="showcalendar(event, 't2', false);" value="&nbsp;" /> -->
		...
		<tr>
			<td> <input name="queryBtn3" type=button class="cssbutton" onClick="storageAuditBatch();" value="批量接收"></td>
			<td></td>
            <td></td>
            <td align="center">
...
<script type="text/javascript" >
	...
	var curstr;
	//查询路径           
	var url = "<%=contextPath%>/sales/ordermanage/orderaudit/OrderStorageAudit/orderStorageQuery.json";
	var title = null;
	var columns = [
				{header: "选择",   dataIndex: 'ORDER_ID', width:"10px",renderer:myCheck},
	...
	function myLink(value,meta,record){
		...
		var historyCount = history.length ;
  		return String.format("<a href='#'  class='u-anchor' onclick='toDetailCheck(\""+ value +"\",\""+ orderType +"\",\""+ reqId +"\",\"" + historyCount + "\")'>[接收]</a><input type='hidden' name='ver' value='"+record.data.VER+"'/>");
	}

	function getDetail(){
		$('fm').action= '<%=request.getContextPath()%>/sales/ordermanage/orderaudit/OrderResourceReserve/orderResourceReserveQueryLoad.json';
	 	$('fm').submit();
		}
	...
	function myCheck(value,metadate,record){
		...
		return "<input type='checkbox' name='cb' onclick='checkedStorageAudit(this);' id=\""+data.REQ_ID+"\"  value=\""+data.REQ_ID+"\" /><input type='hidden'  value=\""+data.REQ_ID+"\" />"
	...
	function saveCheckedId(checkObj){
		var hideCheckedStorage = document.getElementById("checkStorageId");
		if(hideCheckedStorage&&checkObj){
			if(hideCheckedStorage.value.indexOf(checkObj.value)==-1){
				hideCheckedStorage.value = clearValue(hideCheckedStorage.value+","+checkObj.value,',');
			}
		}
	}
	function removeCheckedId(checkObj){
		...
		if(hideCheckedStorage&&checkObj){
			if(hideCheckedStorage.value.indexOf(checkObj.value)!=-1){
				hideCheckedStorage.value = clearValue(hideCheckedStorage.value.replace(checkObj.value,''),',');
			}
		}
	}
	
	//单个复选框的点击事件
	function checkedStorageAudit(_node){
		//checkboxClick(_node);
		if(_node){
	...
	//批量审核
	function storageAuditBatch(){
		var reqIds=document.getElementById("checkStorageId").value;
		if(reqIds==null||reqIds==""){
			MyAlert("请至少选择一条数据！");
			return;
		}else{
			MyConfirm("是否确认审核通过",auditBatchPass);
		}
	}
	
	function auditBatchPass(){
		var reqIds=document.getElementById("checkStorageId").value;
		//MyAlert(reqIds);
		makeNomalFormCall("<%=contextPath%>/sales/ordermanage/orderaudit/OrderStorageAudit/orderStorageSaveBatch.json?checkStorageId="+reqIds,showForwordValue,'fm');
	}
	...
	function lockReturn(json){
		if(json.flag=="1"){
			...
			value3=curstr[2];
			value4=curstr[3];
			document.fm.action= '<%=request.getContextPath()%>/sales/ordermanage/orderaudit/OrderStorageAudit/orderStorageDetailQuery.do?orderId='+value1+'&orderType='+value2+'&reqId='+value3+'&historyCount='+value4;
			document.fm.submit();
		}
	}
	//设置年周
