...
<body >
<div class="navigation"><img src="<%=request.getContextPath()%>/img/nav.gif" />&nbsp;当前位置： 销售订单管理 &gt; 订单审核 &gt;订单资源初审</div>
<div class="form-panel">
	<h2><img src="<%=request.getContextPath()%>/jmstyle/img/search-ico.png" style="margin-bottom: -13px;"/>订单资源初审</h2>
<div class="form-body">
...
<script type="text/javascript" >
	...
	var curstr;
	//查询路径           
	var url = "<%=contextPath%>/sales/ordermanage/orderaudit/OrderResourceReserveFirst/orderResourceReserveQuery.json";
	var title = null;
	var columns = [
	...
	function myLink(value,meta,record){
		...
		var historyCount = history.length ;
  		return String.format("<a href='#' class='u-anchor' onclick='toDetailCheck(\""+ value +"\",\""+ orderType +"\",\""+ reqId +"\",\"" + historyCount + "\")'>[审核]</a><input type='hidden' name='ver' value='"+record.data.VER+"'/>");
	}
	...
	function lockReturn(json){
		if(json.flag=="1"){
			...
			value3=curstr[2];
			value4=curstr[3];
			document.fm.action= '<%=request.getContextPath()%>/sales/ordermanage/orderaudit/OrderResourceReserveFirst/orderResourceReserveDetailQuery.do?orderId='+value1+'&orderType='+value2+'&reqId='+value3+'&historyCount='+value4;
			document.fm.submit();
		}
	}
	//设置年周
