...
<body>
<div class="navigation"><img src="<%=request.getContextPath()%>/img/nav.gif" />&nbsp;当前位置： 销售订单管理 &gt; 订单审核 &gt;订单资源审核</div>
...
<!--			</td>-->
		...
		<tr>
			...
			<td align="right">销售订单号(备注：需求订单号和补充订单号)：</TD>
		...
		<tr>
			...
			<td>
				<label>
					<script type="text/javascript">
						genSelBoxExp("reqStatus",<%=Constant.ORDER_REQ_STATUS%>,"",true,"short_sel","","false",'<%=Constant.ORDER_REQ_STATUS_02%>,<%=Constant.ORDER_REQ_STATUS_03%>,<%=Constant.ORDER_REQ_STATUS_04%>,<%=Constant.ORDER_REQ_STATUS_06%>,<%=Constant.ORDER_REQ_STATUS_07%>,<%=Constant.ORDER_REQ_STATUS_08%>');
					</script>
				</label>
			</td>
...
<script type="text/javascript" >
	...
	var curstr;
	//查询路径           
	var url = "<%=contextPath%>/sales/ordermanage/orderaudit/OrderResourceReserve/orderResourceReserveQuery.json";
	var title = null;
	var columns = [
	...
	function myLink(value,meta,record){
		...
		var historyCount = history.length ;
  		return String.format("<a href='#' onclick='toDetailCheck(\""+ value +"\",\""+ orderType +"\",\""+ reqId +"\",\"" + historyCount + "\")'>[审核]</a><input type='hidden' name='ver' value='"+record.data.VER+"'/>");
	}
	...
	function toDetailCheck(value1, value2, value3, value4){
		...
		makeSameFormCall(lockUrl,lockReturn,'fm');
	}
	function lockReturn(json){
		if(json.flag=="1"){
			...
			value3=curstr[2];
			value4=curstr[3];
			$('fm').action= '<%=request.getContextPath()%>/sales/ordermanage/orderaudit/OrderResourceReserve/orderResourceReserveDetailQuery.do?orderId='+value1+'&orderType='+value2+'&reqId='+value3+'&historyCount='+value4;
	 		$('fm').submit();
		}
	}
	//设置年周
