...
<body>
<div class="navigation"><img src="<%=request.getContextPath()%>/img/nav.gif" />&nbsp;当前位置： 销售订单管理 &gt; 订单审核 &gt;订做车需求产品审核</div>
...
<script type="text/javascript" >
	var myPage;
	//查询路径           
	var url = "<%=contextPath%>/sales/ordermanage/orderaudit/SpecialNeedProductCheck/specialNeedProductCheckQuery.json";
	var title = null;
	var columns = [
	...
	function myLink(value,meta,record){
  		return String.format("<a href='#' onclick='toDetailCheck(\""+ value +"\",\""+ record.data.VER +"\")'>[审核]</a>");
	}
	//审核链接
	var rowObjNum = null;
	var tabObj = null;
	function toDetailCheck(value,ver){
		...
		//取消弹出窗口 2012-05-14 HXY
		<%-- OpenHtmlWindow('<%=contextPath%>/sales/ordermanage/orderaudit/SpecialNeedProductCheck/specialNeedProductDetailCheckInit.do?&reqId='+value+'&ver='+ver,700,500); --%>
		$('fm').action= '<%=contextPath%>/sales/ordermanage/orderaudit/SpecialNeedProductCheck/specialNeedProductDetailCheckInit.do?&reqId='+value+'&ver='+ver;
		$('fm').method = 'POST';
		$('fm').submit();
	}
	//初始化    
