...
<title>订单审核账号修改</title>
</head>
<body>
<div class="navigation"><img src="<%=request.getContextPath()%>/img/nav.gif" />&nbsp;销售订单管理 &gt; 订单审核账号管理&gt;订单审核账号修改</div>
<form method="post" name="fm" id="fm">
	...
	<tr>
		...
		<input type="text" class="middle_txt" id="orderAcnt" readonly="readonly" name="orderAcnt" value="${acnt}">
		<input type="hidden" id="orderAcntId" name="orderAcntId" value="${userId }">
		<input type="hidden" id="orderOrgId" name="orderOrgId" value="${orderOrgId }">
		<!-- <input name="button3" type="button" class="normal_btn" onclick="showOrderAcnt();" disabled="disabled" value="..." />
		<input class="cssbutton" type="button" disabled="disabled" value="清空" onclick="clrTxt('orderAcnt');"/> -->
		</td>
		<td ><div align="right">用户姓名:</div></td>
		<td align="left"><input type="text" class="middle_txt" id="orderUserName" readonly="readonly" name="orderUserName" value="${name}"></td>
	</tr>
	</table>
	...
	<tr>
	 ...
	 <c:forEach items="${list}" var="po">
	 	<tr>
	 	 <td align="center"><input type="hidden" id="orgId${po.ORG_ID }" name="orgId${po.ORG_ID }" value="${po.ORG_ID }"></td>
	 	 <td align="center">${po.ORG_CODE }</td>
	 	 <td align="center">${po.ORG_NAME }</td>
	 	 <td align="center"><a href='#' onclick='delLarge();'>[删除]</a><input type='hidden' id='orgId${po.ORG_ID}' name='orgId${po.ORG_ID}' value='${po.ORG_ID}'></td>
	 	 </tr>
	 	</c:forEach>
	 </tbody>
	 <tr class="cssTable">
	...
	<table class="table_query">
	  <TR class=cssTable >
	  <td align="center">
		<input class="normal_btn" id="id1save" name="baocun" type="button" value="保存" onClick="confirmMod();">
...
function addRow(json){	
			...
			newCell.innerHTML = "<input type='hidden' id='orgId"+json.info.ORG_ID+"' name='orgId"+json.info.ORG_ID+"' value='"+json.info.ORG_ID+"' />";
...
function confirmMod(){
	...
	MyConfirm("是否确认保存？",orderMod);
}

function orderMod(){
	makeNomalFormCall('<%=request.getContextPath()%>/sales/ordermanage/orderacnt/OrderAcntQuery/orderAcntLargeMod.json', showResult ,'fm');
}
