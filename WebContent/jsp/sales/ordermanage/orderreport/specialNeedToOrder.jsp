...
<body>
<div class="navigation"><img src="<%=request.getContextPath()%>/img/nav.gif" />&nbsp;当前位置：整车销售 > 销售订单管理 > 订做车需求确认 > 订做车订单提报</div>
	...
	<table class=table_query>
		...
		<tr class=cssTable >
			...
			<td colspan="3" align="left">
				...
				<input type="hidden" name="priceTypeId" value="" />
...
function priceOnBlue(thisRate){
	...
	function showFundTypeList(json){
		...
		for(var i=0;i<json.fundTypeList.length;i++){
			...
			if(json.fundTypeList[i].TYPE_ID + "" == '${reqPo.accountTypeId}'){
	...
	function toSubmit(){
		...
		makeNomalFormCall('<%=request.getContextPath()%>/sales/ordermanage/orderreport/SpecialNeedConfirm/specialNeedToReport.json',showResult,'fm');
	}
	//回调方法
	function showResult(json){
		if(json.returnValue == '1'){
			window.parent.MyAlert("提交成功！");
			$('fm').action='<%=request.getContextPath()%>/sales/ordermanage/orderreport/SpecialNeedOrderReoprt/specialNeedOrderReoprtInit.do';
			$('fm').submit();
		}else{
	...
	function addRow(json){
		for(var i=0; i<json.info.length; i++) {
			...
			if(isDuplicate) {
				...
				newCell.align = "center";
				newCell.innerHTML = json.info[i].RESOURCE_AMOUNT; */
				/* newCell = newRow.insertCell(7);
				newCell.align = "center";
				newCell.innerHTML = '<input type="text" class="mini_txt" id="discountRate${po.MATERIAL_ID}" name="discountRate" value="0" onchange="priceOnBlue(this);priceTotal();" datatype="0,is_double,6" decimal="2">'; */
