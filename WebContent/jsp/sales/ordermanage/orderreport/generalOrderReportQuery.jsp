...
function loadData(){
	document.getElementById("queryBtn1").click();
}
</script>
</head>
...
<c:if test="${isReportable==0}"><center style="background-color:yellow;"> <div class='pageTips'>常规订单可提报星期为每周的星期1到星期${x},已经过期,无法提报!</div></center></c:if>
</c:if>
...
<script type="text/javascript">
	...
	var columns = [
				...
				{header: "配额数量", dataIndex: 'QUOTA_AMT', align:'center',renderer:quotaNO},
	...
	function orderSubmit(){
		makeNomalFormCall('<%=request.getContextPath()%>/sales/ordermanage/orderreport/GeneralOrderReport/generalOrderReoprtSubmit.json',showResult,'fm');
	}
