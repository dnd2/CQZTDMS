...
<script type="text/javascript">
	...
	var url = "<%=request.getContextPath()%>/sales/ordermanage/orderreport/DsOrderReport/urgentOrderReoprtQuery.json";
	...
	function myLink(value,meta,record){
		...
		var data = record.data;
  		return String.format("<a href='#' onclick='loginMod(\""+ value +"\",\""+ data.ORDER_NO +"\")'>[修改]</a><a href='#' onclick='confirmDel(\""+ value +"\")'>[删除]</a><a href='#' onclick='confirmSubmit(\""+ value +"\","+data.ORDER_AMOUNT+",\""+data.ORDER_NO+"\")'>[提报]</a>");
	}
	
	function loginAdd(){
		$('fm').action = '<%=request.getContextPath()%>/sales/ordermanage/orderreport/DsOrderReport/dsOrderReoprtAddPre.do';
	 	$('fm').submit();
	}
	...
	function loginMod(arg,orderNO){
		...
		makeSameFormCall(lockUrl,lockReturn1,'fm');
	...
	function lockReturn1(json){
		if(json.flag=="1"){
				...
				$('fm').action = '<%=request.getContextPath()%>/sales/ordermanage/orderreport/DsOrderReport/dsOrderReoprtModPre.do?orderId='+arr1[0]+'&orderNO='+arr1[1];
		 		$('fm').submit();
		}
	}
	...
	function confirmSubmit(arg,amount,orderNO){
		...
		arr2[2]=orderNO;
		var lockUrl='<%=request.getContextPath()%>/util/ResourceLock/checkIsOper.json?billId='+arg;
		makeSameFormCall(lockUrl,lockReturn2,'fm');
	}
	
	function lockReturn2(){
		if(json.flag=="1"){
			MyAlert("有其他人操作");
			return;
		}else{
	...
	function orderSubmit(arg,amount,orderNO){
		makeNomalFormCall('<%=request.getContextPath()%>/sales/ordermanage/orderreport/DsOrderReport/dsOrderReoprtSubmit.json?orderId='+arg+'&amount='+amount+'&orderNO='+orderNO,showResult,'fm');
	}
	...
	function confirmDel(arg){
		...
		arr3[0]=arg;
		var lockUrl='<%=request.getContextPath()%>/util/ResourceLock/checkIsOper.json?billId='+arg;
		makeSameFormCall(lockUrl,lockReturn3,'fm');
	...
	//删除
	function del(arg){
		makeNomalFormCall('<%=request.getContextPath()%>/sales/ordermanage/orderreport/DsOrderReport/dsOrderReoprtDel.json?orderId='+arg,delBack,'fm');
	}
	//删除回调方法：
