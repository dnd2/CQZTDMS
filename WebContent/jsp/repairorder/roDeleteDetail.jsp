...
<%
			...
			Long listRono = (Long) request.getAttribute("listRono");
			String acccode ="";
		%>
...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	function goBack(){
		...
		if(type==5){
			...
			window.location.href = "<%=request.getContextPath()%>/repairOrder/RoMaintainMain/roDeleteForward.do";
		}
	}
		...
		<form method="post" name="fm" id="fm">
			...
			<table border="0" align="center" cellpadding="1" cellspacing="1"
				...
				<tr>
					...
					<td class="table_edit_2Col_label_7Letter" style="display: ">
						工单结算时间：
					</td>
					<td style="display: ">
				...
				<tr>
					...
					<td align="left">
						<input type="hidden" name="vin" value="<%=CommonUtils.checkNull(tawep.getVin())%>"/>
  ...
  <table id="accessories" border="0" align="center" cellpadding="0"
				...
				if(accessoryDtlList!=null && accessoryDtlList.size()>0){
					...
					for(int i=0;i<accessoryDtlList.size();i++){%>
						...
						<td>
						<input name="code" id="code" value="<%=accessoryDtlList.get(i).get("MAIN_PART_CODE") %>" type="text" class="middle_txt" readonly>
						</td>
					</tr>
				<%	}
				}
			...
			<table border="0" cellspacing="0" cellpadding="0" class="table_edit">
				<tr>
					<td align="center">
		                ...
		                <input class="normal_btn" type="button" value="授权历史" onclick="openWindowDialog('<%=contextPath%>/repairOrder/RoMaintainMain/authDetail.do?VIN=<%=tawep.getVin()%>');"/>
		                ...
		                <input type="button"  onclick="del(<%=id %>,<%=listRono %>)" class="normal_btn" id="repairOrderId" value="废弃" />
   ...
   if($('REPAIR_TYPE').value==<%=Constant.REPAIR_TYPE_01%>){
	...
	function del(id,listRono){
		if(id!=""){
			if(listRono>0){
				MyAlert("此工单已经生成索赔单,请先废弃索赔单!");
			}else{
	...
	function deleteApply(id) {
		$('repairOrderId').disabled=true;
			var url = '<%=contextPath%>/repairOrder/RoMaintainMain/deleteRepairOrder.json?ID='+id+"&VER="+<%=tawep.getVer()%>;
			makeNomalFormCall(url,approveBack,'fm','');
}
function approveBack(json) {
				var msg = json.msg;
				var dealerId = json.dealerId;
				var msgTemp = json.msgTemp;
				if(msgTemp!=""){
					MyAlert(msgTemp);
				}else{
					if (msg=="true") {
						MyAlert("废弃成功!");
						if(dealerId==""){
							window.location.href = "<%=contextPath%>/claim/application/ClaimManualAuditing/repairOrderCancel.do";
						}else{
							window.location.href = "<%=contextPath%>/repairOrder/RoMaintainMain/roDeleteForward.do";
						}
					}else{
						MyAlert("两次操作版本不一致,废弃失败!");
						if(dealerId==""){
							window.location.href = "<%=contextPath%>/claim/application/ClaimManualAuditing/repairOrderCancel.do";
						}else{
							window.location.href = "<%=contextPath%>/repairOrder/RoMaintainMain/roDeleteForward.do";
