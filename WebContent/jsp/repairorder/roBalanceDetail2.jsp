...
<%
    		...
    		TtAsWrApplicationExtPO tawep2 = (TtAsWrApplicationExtPO) request.getAttribute("tawep2");
			...
			String reporter = (String)request.getAttribute("reporter");
...
<%-- 	//返回
	function goBack(){
			window.location.href = "<%=contextPath%>/repairOrder/RoMaintainMain/repairOrderQuery.do?type="+type;
	} --%>
	function confirmUpdate() {
		var freeTimes = '${freeTimes}' ;
		var freeTimes2 = '${application.freeTimes}' ;
		if('${flag_if}'){
			if(freeTimes>=freeTimes2){
				//MyAlert('此工单的保养次数与系统数据冲突，不予结算！');
				//return ;
			}
		}
		/*
	...
	{
	   	loadcalendar();
		for(var j=0; i<codeList.length; j++){
		...
		<form method="post" name="fm" id="fm">
			...
			<table border="0" align="center" cellpadding="1" cellspacing="1"
				...
				<tr>
					...
					<td>
						<%=CommonUtils.checkNull(tawep2.getReporter())%>
					</td>
				...
				<tr id="activity" style="display:none">
					<td id="activityTableId">
						活动编号：</td><td id="activityTableId0" colspan="3">
				...
				<% if(Constant.REPAIR_TYPE_05.equalsIgnoreCase(tawep.getRepairTypeCode())){%>
					<tr id="activity" >
						<td id="activityTableId" align="right">
						活动编号：</td>
						<td id="activityTableId0" colspan="3">
						<%=CommonUtils.checkNull(tawep.getCamCode()) %>(<%=CommonUtils.checkNull(tawep.getCampaignName()) %>)
						</td>
				</tr>
				<%}%>
				<% if(Constant.REPAIR_TYPE_04.equalsIgnoreCase(tawep.getRepairTypeCode())){%>
			...
			<table id="partTableId" border="0" align="center" cellpadding="0"
				...
				<tbody id="partTable">
					...
					<tr class="table_list_row1">
						...
						<td><%=partLs.get(i).getLabourCode() %></td>
						<td><script type="text/javascript">
							document.write(getItemValue('<%=tl.getResponsNature().intValue()%>'));
							</script>
							</td>
					<td style="width:80px;"><%=CommonUtils.checkNull(tl.getTroubleDescribe())%></td>
					<td style="width:80px;"><%=CommonUtils.checkNull(tl.getTroubleReason())%></td>
					<td style="width:80px;"><%=CommonUtils.checkNull(tl.getDealMethod())%></td>
		...
		%>
			<table border="0" cellspacing="0" cellpadding="0" class="table_edit">
				<tr>
					...
					<!--<input type="button"  onclick="goBack()" class="normal_btn" id="backId" style="" value="返回" />-->	
