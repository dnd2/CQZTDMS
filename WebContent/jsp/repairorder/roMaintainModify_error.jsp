...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	//返回
	function goBack(){
		window.location.href = "<%=request.getContextPath()%>/feedbackmng/query/ProblemRepairOrderQuery/ProblemRepairOrderQueryForward.do";
	}
	...
	function confirmUpdate0() {
		...
		var url1 = '<%=request.getContextPath()%>/repairOrder/RoMaintainMain/roUpdate.json?commit=1&ITEM_DEL='+itemToDel+'&PART_DEL='+partToDel+'&OTHER_DEL='+otherToDel+'&APPROVAL_YN='+needApprove;
		if(submitForm('fm')){
...
--></SCRIPT>
	...
	<BODY onload="doInit();" onkeydown="keyListnerResp();">
		...
		<form method="post" name="fm" id="fm">
			...
			<input type="hidden" name="mastMileage" id="mastMileage" value="${tvp.mileage }"/>
...
<!--          	<input type="text" name="OUT_SITE" id="OUT_SITE" value="<%=CommonUtils.checkNull(tawep.getOutSite()) %>" datatype="1,is_digit_letter_cn,100" class="middle_txt"/>-->
          	...
          	<td align="left">
<!--          	<input type="text" name="FROM_ADRESS" id="FROM_ADRESS" value="<%=CommonUtils.checkNull(tawep.getFromAdress()) %>" datatype="1,is_digit_letter_cn,100" class="middle_txt"/>-->
          	...
          	<td align="left">
<!--          	<input type="text" name="END_ADRESS" id="END_ADRESS" value="<%=CommonUtils.checkNull(tawep.getEndAdress())%>" datatype="1,is_digit_letter_cn,100" class="middle_txt"/>-->
			...
			<table class="table_edit">
				<tr>
					...
					<input type="button" style="display:none" id="approve" onclick="approve1()" class="long_btn"
							value="预授权申请" />
				    <input class="normal_btn" type="button" value="OEM维修历史TEST" onclick="openWindowDialog('<%=contextPath%>/claim/dealerClaimMng/ClaimBillMaintainMain/queryOEMmaintaimHistory.do?VIN=<%=tawep.getVin()%>');"/>			
					<input class="long_btn" type="button" id="three_package_set_btn" value="下端索赔简报TEST" onclick="threePackageSet();"/>
					<input class="long_btn" type="button" id="three_package_set_btn" value="索赔查询TEST" onclick="claimQuery();"/>					
	...
	//	$('ENGINE_NO').disabled=true;
		...
		function threePackageSet(){
			...
			if (vin==null||vin==''||vin=='null') {
				...
				window.open('<%=contextPath%>/claim/dealerClaimMng/ClaimBillMaintainMain/claimReport.do?VIN='+vin+'&mile='+inMileage+'&codes='+codes+'&roNo='+roNo+'&ENGINE_NO='+ENGINE_NO);
			}
		}
		//索赔查询TEST
		function claimQuery(){
			var RO_NO = document.getElementById("RO_NO").value;
			window.open('<%=contextPath%>/claim/application/ClaimBillStatusTrack/claimBillDetailForward_tow.do?RO_NO='+RO_NO);			 
		}
