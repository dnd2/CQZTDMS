...
<%
    		...
    		String createName="";
    		if(!"".equalsIgnoreCase(request.getParameter("createName"))&&request.getParameter("createName")!=null){
    			 createName=new String(request.getParameter("createName").getBytes("ISO-8859-1"),"UTF-8");
    		}
...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	//返回
	function goBack(){
		window.location.href = "<%=request.getContextPath()%>/repairOrder/RoMaintainMain/roShopForward.do";
	}
	function confirmUpdate() {
		if($('remark_ysq').value.length>100){
				MyAlert("审核内容过长！");
				return false;
		}
		if(confirm("你确定通过吗？")){
			...
			var payment = 0 ;
		//if($('is_payment').checked){
		//	payment = 0 ;
			//}
			fm.action='<%=request.getContextPath()%>/repairOrder/RoMaintainMain/updateForStatus.do?ID=<%=request.getAttribute("ID")%>'+'&payment='+payment;
			fm.submit();
		}
		else{
			return false;
	...
	function confirmUpdate0() {
		...
		fm.action='<%=request.getContextPath()%>/repairOrder/RoMaintainMain/updateForStatus.do?ID=<%=request.getAttribute("ID")%>'+'&payment='+payment;
		fm.submit();
		parentContainer.__extQuery__(1);//xiongchuan添加,返回列表也同时刷新页面
			_hide();
		
	}
	function confirmBack() {
		if($('remark_ysq').value.length<1){
				MyAlert("退回必须填写审核内容！");
				return false;
		}
			if($('remark_ysq').value.length>100){
		...
		if(confirm("你确定退回吗？")){
			var fm = document.getElementById('fm');
			fm.action='<%=request.getContextPath()%>/repairOrder/RoMaintainMain/backForStatus.do?ID=<%=request.getAttribute("ID")%>';
			fm.submit();
			parentContainer.__extQuery__(1);//xiongchuan添加,返回列表也同时刷新页面
			_hide();
		}
...
　　} 
		...
		<form method="post" name="fm" id="fm">
			...
			<table border="0" align="center" cellpadding="1" cellspacing="1"
				...
				<tr>
					...
					<td align="left">
	       			<change:change val="<%=tawep.getRepairTypeCode()%>" type="<%=String.valueOf(Constant.REPAIR_TYPE)%>"/>
	       			<input name="REPAIR_TYPE" id ="REPAIR_TYPE"  value="<%=tawep.getRepairTypeCode()%>" type="hidden"/><!-- 页面很多地方都用到了维修类型进行判断，所有一定要有一个 REPAIR_TYPE属性-->
					</td>
					</tr>
					<tr>
					<td class="table_edit_2Col_label_7Letter">
				...
				<tr id="freeTime" style="display:none">
 				...
 				<c:if test="${isWarning eq '10041001'}">
					<tr>
						<td align="center" colspan="4">
							<a href="#" onclick="threePackageSet();" style="color:red;">预警查看</a>
						</td>
					</tr>
				</c:if>
				</table>
				...
				<tr>
					...
					<td id="MODEL_NAME"><%=CommonUtils.checkNull(tawep.getModel())%></td>
				</tr>
				<tr>
				<td class="table_edit_3Col_label_5Letter">
					...
					<td id="MODEL_NAME"><%=CommonUtils.checkNull(tawep.getPackageName())%></td>
				...
				<tr>
					<td colspan="2" nowrap="true">
						用户名称：<%=CommonUtils.checkNull(tawep.getDeliverer()) %>
					</td>
					<td colspan="2" nowrap="true">
						电话：<%=CommonUtils.checkNull(tawep.getDelivererPhone()) %>
					</td>
					<td colspan="2" nowrap="true">
						手机：<%=CommonUtils.checkNull(tawep.getDelivererMobile()) %>
					</td>
					<td colspan="2" nowrap="true">
						用户地址：<%=CommonUtils.checkNull(tawep.getDelivererAdress()) %>
					</td>
			...
			<table class="table_edit">
				...
				<tr>
					...
					<td>
						<%=CommonUtils.checkNull(tawep.getRepairPartAmount()) %>
					</td>
					<td class="table_edit_3Col_label_6Letter">
						<span class="zi">工时费：</span>
					</td>
					<td align="left" >
						<%=CommonUtils.checkNull(tawep.getLabourAmount()) %>
					</td>
					<td class="table_edit_3Col_label_6Letter">
						<span class="zi">辅料费：</span>
					</td>
					<td align="left" >
						<%=CommonUtils.checkNull(tawep.getAccessoriesPrice()) %>
					</td>
				</tr>
				<tr>
					<td class="table_edit_3Col_label_7Letter">
					...
					<td>
						<%=CommonUtils.checkNull(tawep.getAddItemAmount()) %>
					...
					<td>
						<%=CommonUtils.checkNull(tawep.getCompensationMoney()) %>
					</td>
					<td class="table_edit_3Col_label_6Letter">
						<span class="zi">申请总费用：</span>
					</td>
					<td>
						<%=CommonUtils.checkNull(tawep.getRepairAmount()) %>
					</td>
			...
			<table class="table_edit" style="display: none">
				...
				<tr>
					<td nowrap="true" class="table_edit_2Col_label_5Letter">
						故障描述：
					</td>
					<td class="tbwhite" colspan="3">
						<textarea  style="font-weight: bolder;" maxlength="100" readonly="readonly" rows='3' cols='40'><%=CommonUtils.checkNull(tawep.getTroubleDescriptions()) %></textarea>
					</td>
					<td  nowrap="true" class="table_edit_2Col_label_5Letter">
						故障原因：
					</td>
					<td class="tbwhite" colspan="3">
						<textarea  style="font-weight: bolder;" readonly="readonly" maxlength="100"	 rows='3' cols='40'><%=CommonUtils.checkNull(tawep.getTroubleReason()) %></textarea>
					</td>
				</tr>
				<tr>
					<td  nowrap="true" class="table_edit_2Col_label_5Letter">
					...
					<td class="tbwhite" colspan="3">
						<textarea  id='REPAIR_METHOD' readonly="readonly" maxlength="100" rows='3' cols='40'><%=CommonUtils.checkNull(tawep.getRepairMethod()) %></textarea>
					</td>
					<td nowrap="true" class="table_edit_2Col_label_5Letter">
						申请备注：
					</td>
					<td class="tbwhite" colspan="3">
						<textarea  style="font-weight: bolder;"	readonly="readonly" maxlength="100" rows='3' cols='40'><%=CommonUtils.checkNull(tawep.getRemarks()) %></textarea>
					</td>
				</tr>
			</table>
			<% 
			...
			<table id="itemTableId" border="0" align="center" cellpadding="0" cellspacing="1" class="table_list" >
				...
				<input type="hidden" name="WR_LABOURCODE" id="WR_LABOURCODE" value="<%=CommonUtils.checkNull(tl.getWrLabourcode())%>" />
					...
					<tr class="table_list_row1">
						...
						<td>
							<%=CommonUtils.checkNull(tl.getPartNo())%>
							 <input type="hidden" name="PART_CODE" id="PART_CODE" value="<%=CommonUtils.checkNull(tl.getPartNo())%>" />
						...
						<td><%=partLs.get(i).getLabourName() %></td>
						<td><%
            ...
            <tr>
            	<input id="pkid" type="hidden" name="pkid"  value="${compensationMoneyList.PKID}"/>
                <td  nowrap="true">供应商代码：</td>
                <td>
                <input id="supplier_code" name="supplier_code"  value="${compensationMoneyList.SUPPLIER_CODE}" readonly="readonly" class="middle_txt" />
                </td>
                ...
                <td><input name="pass_price" id="pass_price" value="${compensationMoneyList.PASS_PRICE}" type="text" class="middle_txt" ></td>
  ...
  <table id="accessories" border="0" align="center" cellpadding="0"
            ...
            <table class="table_edit" id="REMARK_ID" align="center">
          	...
          	<tr>
          		<td >预授权内容
          		<textarea id="opinion_ysq" name="opintion" rows="5" cols="50">${list3.REMARK}</textarea>
          		</td>
          		<td >审核内容
          		<textarea id="remark_ysq" name="opintion1" rows="5" cols="50">${list3.OPINION}</textarea>
          		</td>
          	</tr>
          </table>  
          <br/>
		...
		%>
			<table border="0" cellspacing="0" cellpadding="0" class="table_edit">
				...
				<td  style="display:none">
						是否同意费用：
						<input type="checkbox" name="is_payment" id="is_payment"  checked="checked" />是 <span style="color:red;display: none" id="abc123"  >* 若不同意费用,可不勾选</span>
					</td>
					<td align="center">
					  <input class="normal_btn" type="button" value="维修历史" name="historyBtn"onclick="openWindowDialog(1);"/>
						<input class="normal_btn" type="button" value="授权历史" name="historyBtn"onclick="openWindowDialog(2);"/>
						<input class="normal_btn" type="button" value="保养历史" name="historyBtn" onclick="openWindowDialog(3);"/>
						<input type="button" style="display:none" onclick="confirmUpdate()" class="normal_btn" id="bal" style="" value="审核通过" />
						<input type="button" style="display:none" onclick="confirmBack()" class="normal_btn" id="can" style="" value="审核驳回" />
		...
		<script type="text/javascript">
		//保养状态判定按钮的显示控制
		function btn_show_set(){
			var type = '<%=tawep.getRepairTypeCode()%>' ;
			if(11441004 != type){
		...
		function backMain(){
			...
			var  url;
			if(flag==0){
				 url = '<%=contextPath%>/repairOrder/RoMaintainMain/roShopForward.do';
			}
			else{
			 url = '<%=contextPath%>/repairOrder/RoMaintainMain/roViewForDealer.do';
			}
			location=url;
		}
		//配件三包判定按钮方法
		...
		//$('REPAIR_TYPE').disabled=true;
		document.getElementById("opinion_ysq").readOnly = true; 
		$('YIELDLY').disabled=true;
			if (showBal) {
				document.getElementById("bal").style.display='';
				document.getElementById("abc123").style.display='';	
						
			}else {
				document.getElementById("bal").style.display='none';
				document.getElementById("remark_ysq").readOnly = true; //can
				var aa = '<%=tawep.getAccreditAmount()%>' ;
				$('is_payment').disabled = true ;
				if(aa==0){
					$('is_payment').checked = true ;
				}
			}
   ...
   function getTypeChangeStyle(obj) {
    	...
    	if(obj=='<%=Constant.REPAIR_TYPE_01%>') {//一般索赔
    	...
    	}else if(obj=='<%=Constant.REPAIR_TYPE_06%>') {//一般索赔
    		document.getElementById('itemTableId').style.display='';
    		document.getElementById('partTableId').style.display='';
    		document.getElementById('otherTableId').style.display='none';
    		//document.getElementById('feeTableId').style.display='none';
    		...
    		document.getElementById("activity").style.display='none';
    		document.getElementById('outId').style.display='none';
    	}else if(obj=='<%=Constant.REPAIR_TYPE_07%>') {//一般索赔
    		document.getElementById('itemTableId').style.display='';
    		document.getElementById('partTableId').style.display='';
    		document.getElementById('otherTableId').style.display='none';
    		//document.getElementById('feeTableId').style.display='none';
   ...
   if($('REPAIR_TYPE').value==<%=Constant.REPAIR_TYPE_01%>){
	...
	document.getElementById("YIELDLY").disabled = true;
   	var myvalue=document.getElementById("REPAIR_TYPE").value;
