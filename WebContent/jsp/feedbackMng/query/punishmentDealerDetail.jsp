...
<%@page import="com.infodms.dms.bean.PunishmentApplyDealerMantainBean"%><html xmlns="http://www.w3.org/1999/xhtml">
<jsp:include page="${contextPath}/common/jsp_head_new.jsp" />

<head>
<% String contextPath = request.getContextPath(); %>
<%
PunishmentApplyDealerMantainBean DealerMantainBean = (PunishmentApplyDealerMantainBean)request.getAttribute("DealerMantainBean");
List<PunishmentApplyDealerMantainBean> DealerMantainList = (List<PunishmentApplyDealerMantainBean>)request.getAttribute("DealerMantainList");
 %>
 ...
 <form method="post" name = "FRM" enctype="multipart/form-data" >
  <table width="100%" border="0" align="center" cellpadding="1" cellspacing="1" bgcolor="91908E"  class="table_edit">
        ...
        <tr bgcolor="F3F4F8">
          ...
          <td width="30%"><%=DealerMantainBean.getOrderId()%></td>
          <td width="13%"  align="right">&nbsp;</td>
          <td width="39%">&nbsp;</td>
        </tr>
        <tr bgcolor="F3F4F8">
          <td  align="right">经销商：</td>
          <td><%=DealerMantainBean.getDealerName()==null?"":DealerMantainBean.getDealerName()%></td>
          <td  align="right">类型：</td>
          <td>
   
          <script type='text/javascript'>
		       var rewardType=getItemValue('<%=DealerMantainBean.getRewardType()%>');
        ...
        <tr bgcolor="F3F4F8">
          ...
          <td><%=DealerMantainBean.getLinkMan()%></td>
          <td align="right">联系电话：</td>
          <td><%=DealerMantainBean.getTel()%></td>
        </tr>
       <tr id="01"  bgcolor="FFFFFF" style="display:inline">
          <td align="right">奖励方式：</td>
          <td >
           <script type='text/javascript'>
		       var rewardMode=getItemValue('<%=DealerMantainBean.getRewardMode()%>');
		       document.write(rewardMode) ;
		 </script>  
		  <input type="hidden" name="rewardMode" id="rewardMode" value="<%=DealerMantainBean.getRewardMode()%>"/>
          </td>
          <td align="right" bgcolor="F3F4F8" id="03" style="display:none">奖励金额：</td>
          <td bgcolor="F3F4F8" id="rewardMoney">
         <%=DealerMantainBean.getRewardMoney()==null?"":DealerMantainBean.getRewardMoney()%>
          </td>
        </tr>
        <tr id="02"  bgcolor="FFFFFF" style="display:none">
          ...
          <td>
           <script type='text/javascript'>
		       var rewardModePunish=getItemValue('<%=DealerMantainBean.getRewardMode()%>');
		       document.write(rewardModePunish) ;
		 </script>  
		 <input type="hidden" name="rewardModePunish" id="rewardModePunish" value="<%=DealerMantainBean.getRewardMode()%>"/>
          </td>
        ...
        <script type="text/javascript">
          if('<%=DealerMantainBean.getRewardType()%>' == '<%=Constant.PUNISHMENT_01%>'){
  		...
  		}else if('<%=DealerMantainBean.getRewardType()%>' == '<%=Constant.PUNISHMENT_02%>'){
        ...
        <tr bgcolor="FFFFFF">
          ...
          <td bgcolor="FFFFFF"   ><%=DealerMantainBean.getRewardDate()%></td>
        ...
        <tr bgcolor="FFFFFF">
          ...
          <td  colspan="3" align="left" ><span class="tbwhite">
            <%=DealerMantainBean.getRewardContent()%>
          </span></td>
        </tr>
      </table>
    <tr bgcolor="F3F4F8"> 
   ...
   <table class="table_list" style="border-bottom:1px solid #DAE0EE">
     ...
     <c:forEach var="DealerMantainListBean" items="${DealerMantainList}">
		          <tr class="table_list_row1">
			            <td> <c:out value="${DealerMantainListBean.auditDate}"></c:out>
			            </td>
			            <td><span class="tbwhite">
			            <c:out value="${DealerMantainListBean.name}"></c:out>
			            </span></td>
			            <td><span class="tbwhite">
			            <c:out value="${DealerMantainListBean.auditBy}"></c:out>
			            </span>
			            </td>
			            <td>
			           <script type='text/javascript'>
						       var name=getItemValue('${DealerMantainListBean.auditStatus}');
						       document.write(name) ;
						</script>
			            </td>
			            <td>
			            <c:out value="${DealerMantainListBean.auditContent}"></c:out>
			            </td>
		           </tr>
	           </c:forEach>
   </table>         
