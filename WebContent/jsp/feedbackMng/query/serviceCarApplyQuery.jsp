...
<SCRIPT LANGUAGE="JavaScript">
	var myPage;
//查询路径
	var url = "<%=contextPath%>/feedbackmng/query/ServiceCarApplyQuery/applyQuery.json";
	...
	var columns = [
				...
				{header: "工单号", width:'15%', dataIndex: 'orderId',renderer:myLink2},
	...
	//工单的超链接
	function myLink2(value){
		return "<a href='#' onclick='printWin(\""+value+"\")'>["+value+"]</a>";
    }
    function printWin(value){
		window.open("<%=contextPath%>/feedbackmng/apply/ServiceCarApply/servicecarapplyDetail.do?flag=2&ORDER_ID="+value);
    }
	...
	function showExl(){
		//根据用户输入的数据,导出相应的数据
		MyConfirm("确定导出你输入的条件下的所有数据吗?",sureCF);
	}
	function sureCF(){
		$('fm').action = '<%=request.getContextPath()%>/feedbackmng/query/ServiceCarApplyQuery/toExl.do' ;
		$('fm').submit();
	}
		...
		<textarea rows="2" cols="30" id="dealerCode" name="dealerCode"></textarea>
          ...
          <tr>
            <td  class="table_query_2Col_label_6Letter">提报时间：</td>
              <td  nowrap="nowrap">
              <input type="text" class="short_txt" name="CON_APPLY_DATE_START" id="CON_APPLY_DATE_START_ID"  datatype="1,is_date,10" group="CON_APPLY_DATE_START_ID,CON_APPLY_DATE_END_ID" hasbtn="true" callFunction="showcalendar(event, 'CON_APPLY_DATE_START_ID', false);"/>
              至
  			<input type="text" class="short_txt" name="CON_APPLY_DATE_END" id="CON_APPLY_DATE_END_ID"  datatype="1,is_date,10" group="CON_APPLY_DATE_START_ID,CON_APPLY_DATE_END_ID" hasbtn="true" callFunction="showcalendar(event, 'CON_APPLY_DATE_END_ID', false);"/>
  			</td>
  			<td  class="table_query_2Col_label_6Letter">工单状态：</td>
            <td >
            <script type="text/javascript">
	              genSelBoxExp("APP_STATUS",<%=Constant.SERVICE_APPLY_ACTIVE_STATUS_TYPE%>,"",true,"short_sel","","true",'<%=Constant.SERVICE_APPLY_ACTIVE_STATUS_UNREPORT%>');
	            </script>
            </td>
          </tr>
          <tr>
            <td colspan="6" align="center" nowrap>
            	...
            	&nbsp;&nbsp;
            	<input type="button" value="导出" class="normal_btn" onclick="showExl();"/>
            </td>
          </tr>
  </table>
    <!-- 查询条件 end -->
