...
<%@page import="com.infodms.dms.bean.PunishmentApplyCarMantainBean"%>
...
<%
PunishmentApplyCarMantainBean CarMantainBean = (PunishmentApplyCarMantainBean)request.getAttribute("MantainBean");
List<PunishmentApplyCarMantainBean> CarMantainList = (List<PunishmentApplyCarMantainBean>)request.getAttribute("CarMantainList");
 %>
 ...
 <form method="post" name = "FRM" enctype="multipart/form-data" >
  <table width="100%" border="0" align="center" cellpadding="1" cellspacing="1" bgcolor="91908E"  class="table_edit">
        ...
        <tr bgcolor="F3F4F8">
          ...
          <td width="30%"><%=CarMantainBean.getOrderId()%></td>
          <td width="13%"  align="right">&nbsp;</td>
          <td width="39%">&nbsp;</td>
        </tr>
        <tr bgcolor="F3F4F8">
          <td  align="right">经销商：</td>
          <td><%=CarMantainBean.getDealerName()==null?"":CarMantainBean.getDealerName()%></td>
          <td  align="right">类型：</td>
          <td>
   
          <script type='text/javascript'>
		       var rewardType=getItemValue('<%=CarMantainBean.getRewardType()%>');
        ...
        <tr bgcolor="F3F4F8">
          ...
          <td><%=CarMantainBean.getLinkMan()%></td>
          <td align="right">联系电话：</td>
          <td><%=CarMantainBean.getTel()%></td>
        </tr>
  <tr id="01"  bgcolor="FFFFFF" style="display:inline">
          <td align="right">奖励方式：</td>
          <td >
           <script type='text/javascript'>
		       var rewardMode=getItemValue('<%=CarMantainBean.getRewardMode()%>');
		       document.write(rewardMode) ;
		 </script>  
		   <input type="hidden" name="rewardMode" id="rewardMode" value="<%=CarMantainBean.getRewardMode()%>"/>
          </td>
          <td align="right" bgcolor="F3F4F8" id="03" style="display:none">奖励金额：</td>
          <td bgcolor="F3F4F8" id="rewardMoney">
         <%=CarMantainBean.getRewardMoney()==null?"":CarMantainBean.getRewardMoney()%>
          </td>
        </tr>
        <tr id="02"  bgcolor="FFFFFF" style="display:none">
          ...
          <td>
           <script type='text/javascript'>
		       var rewardModePunish=getItemValue('<%=CarMantainBean.getRewardMode()%>');
		       document.write(rewardModePunish) ;
		 </script>  
		  <input type="hidden" name="rewardModePunish" id="rewardModePunish" value="<%=CarMantainBean.getRewardMode()%>"/>
          </td>
        ...
        <script type="text/javascript">
          if('<%=CarMantainBean.getRewardType()%>' == '<%=Constant.PUNISHMENT_01%>'){
  		...
  		}else if('<%=CarMantainBean.getRewardType()%>' == '<%=Constant.PUNISHMENT_02%>'){
        ...
        <tr bgcolor="FFFFFF">
          ...
          <td bgcolor="FFFFFF"   ><%=CarMantainBean.getRewardDate()%></td>
        ...
        <tr bgcolor="FFFFFF">
          ...
          <td  colspan="3" align="left" ><span class="tbwhite">
            <%=CarMantainBean.getRewardContent()%>
          </span></td>
        </tr>
      </table>
    <tr bgcolor="F3F4F8"> 
   ...
   <table class="table_list" style="border-bottom:1px solid #DAE0EE">
     ...
     <c:forEach var="CarMantainListBean" items="${CarMantainList}">
		          <tr class="table_list_row1">
			            <td> <c:out value="${CarMantainListBean.auditDate}"></c:out>
			            </td>
			            <td><span class="tbwhite">
			            <c:out value="${CarMantainListBean.name}"></c:out>
			            </span></td>
			            <td><span class="tbwhite">
			            <c:out value="${CarMantainListBean.orgName}"></c:out>
			            </span>	
			            </td>
			            <td>
			           <script type='text/javascript'>
						       var name=getItemValue('${CarMantainListBean.auditStatus}');
						       document.write(name) ;
						</script>
			            </td>
			            <td>
			            <c:out value="${CarMantainListBean.auditContent}"></c:out>
			            </td>
		           </tr>
	           </c:forEach>
   </table>         
