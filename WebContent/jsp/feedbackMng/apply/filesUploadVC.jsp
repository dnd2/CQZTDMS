...
<table class="table_query">
	<tr>
		...
		<td width="20%">
			<script type="text/javascript">
				genSelBoxExp("status",<%=Constant.SERVICE_CAR_FILES%>,"",true,"short_sel","","true",'');
			</script>
		</td>
	</tr>
	<tr>
...
<script type="text/javascript">
	var url = "<%=contextPath%>/feedbackmng/apply/ServiceCarWCAction/filesQuery.json";		
	var title = null;
	var columns = [
	...
	function myHandler(value,meta,rec){
		...
		if(status == <%=Constant.SERVICE_CAR_FILES_01%> || status == <%=Constant.SERVICE_CAR_FILES_04%> || status == <%=Constant.SERVICE_CAR_FILES_07%> || status == <%=Constant.SERVICE_CAR_FILES_10%> || status == <%=Constant.SERVICE_CAR_FILES_13%>) 
			return '<a href="<%=contextPath%>/feedbackmng/apply/ServiceCarWCAction/filesUploadInit.do?id='+id+'">[修改]</a>' ;
		else return '<a href="<%=contextPath%>/feedbackmng/apply/ServiceCarWCAction/showDetail.do?id='+id+'">[查看]</a>' ;
	}
</script>
</BODY>
</html>