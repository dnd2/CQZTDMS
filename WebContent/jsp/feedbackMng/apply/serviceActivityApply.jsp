...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<div class="wbox">
		<div class="navigation">
			...
			&nbsp;当前位置： 信息反馈管理&gt;信息反馈提报 &gt; 服务活动申请表
		</div>
		<form id="fm" name="fm" method="post">
			<table class="table_query" border="0"
				style="border: 1px solid #DAE0EE">
				<tr>
					<td class="table_query_2Col_label_5Letter">
						工单号：
					</td>
					<td>
						<input name="ORDER_ID" id="ORDER_ID" value="" type="text"
							class="middle_txt" datatype="1,is_digit_letter,18">
					</td>
					<td class="table_query_2Col_label_5Letter">
						活动名称：
					</td>
					<td>
						<input class="middle_txt" type="text" id="ACT_NAME"
							name="ACT_NAME" datatype="1,is_digit_letter_cn" size="16"
							value="" />
					</td>
				</tr>
				<tr>
					...
					<td nowrap="nowrap">
						<input type="text" name="CON_APPLY_DATE_START"
							id="CON_APPLY_DATE_START_ID" datatype="1,is_date,10"
							group="CON_APPLY_DATE_START_ID,CON_APPLY_DATE_END_ID"
							hasbtn="true"
							callFunction="showcalendar(event, 'CON_APPLY_DATE_START_ID', false);" />
						至
						<input type="text" name="CON_APPLY_DATE_END"
							id="CON_APPLY_DATE_END_ID" datatype="1,is_date,10"
							group="CON_APPLY_DATE_START_ID,CON_APPLY_DATE_END_ID"
							hasbtn="true"
							callFunction="showcalendar(event, 'CON_APPLY_DATE_END_ID', false);" />
					<td class="table_query_2Col_label_5Letter">
						活动类型：
					</td>
					<td nowrap>
				...
				<tr>
					<td colspan="6" align="center" nowrap>
						<input class="normal_btn" type="button" name="button" value="查询"
							onClick="__extQuery__(1);setTimeout(showButton,1000);">
						<input class="normal_btn" type="BUTTON" name="button1" value="新增"
							onClick="location='newServiceActivityApplyforward.do';">
					</td>
				</tr>
			</table>
			<!-- 查询条件 end -->
	...
	var myPage;
//查询路径
	var url = "<%=contextPath%>/feedbackmng/apply/ServiceActivityApply/serviceactivityapplyQuery.json?queryType=submit";
	...
	var columns = [
				{id:'chk',header: "<input type='checkbox' name='checkAll' onclick='selectAll(this,\"orderIds\")' />", width:'8%',sortable: false,dataIndex: 'orderId',renderer:myCheckBox},
					{id:'id',header: "工单号", width:'15%', dataIndex: 'orderId',renderer:myLink},
					{header: "活动名称", width:'10%', dataIndex: 'actName'},
					{header: "活动类型", width:'10%', dataIndex: 'actType',renderer:getItemValue},
	...
	function myLink1(value,meta,record){
		if (record.data.actStatus==<%=Constant.SERVICE_APPLY_ACTIVE_STATUS_UNREPORT%>
		||record.data.actStatus==<%=Constant.SERVICE_APPLY_ACTIVE_AREA_STATUS_REJECT%>
		||record.data.actStatus==<%=Constant.SERVICE_APPLY_ACTIVE_SERVICE_STATUS_REJECT%>
		||record.data.actStatus==<%=Constant.SERVICE_APPLY_ACTIVE_CAR_ENTER_STATUS_REJECT%>) {
  		return String.format("<a href=\"<%=contextPath%>/feedbackmng/apply/ServiceActivityApply/serviceactivityapplyUpdatePre.do?ORDER_ID="
			+ value + "\">[修改]</a>");
		}else {
			return "";
		}
		...
		<form name="form1" style="display: none">
			<table id='bt' class="table_list">
				<tr>
					<th align="left">
						<p>
							<INPUT class="normal_btn" type=button value='上报'
								onclick='submitId()' name=modify>
							&nbsp;
							<INPUT class="normal_btn" onclick='deleteId()' type=button
								value='删除' name=modify>
						</p>
					</th>
				</tr>
			</table>
