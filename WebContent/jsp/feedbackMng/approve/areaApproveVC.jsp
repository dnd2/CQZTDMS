...
<BODY onload="doInit();">
<div class="navigation"><img src="<%=contextPath%>/img/nav.gif"/>&nbsp;当前位置： 信息反馈管理&gt;信息反馈提报 &gt;服务车事业部审批</div>
<form id="fm" name="fm">
...
<table class="table_query">
	...
	<tr>
		...
		<td width="20%">
			<script type="text/javascript">
				genSelBoxExp("status",<%=Constant.SERVICE_CAR_APPLY%>,<%=Constant.SERVICE_CAR_APPLY_02%>,true,"short_sel","","true",'');
			</script>
		</td>
	</tr>
	<tr>
...
<script type="text/javascript">
	var url = "<%=contextPath%>/feedbackmng/apply/ServiceCarWCAction/areaQuery.json";		
	var title = null;
	var columns = [
	...
	function myHandler(value,meta,rec){
		...
		if(status == <%=Constant.SERVICE_CAR_APPLY_02%>) 
			return '<a href="<%=contextPath%>/feedbackmng/apply/ServiceCarWCAction/areaApproveInit.do?id='+id+'">[审批]</a>' ;
