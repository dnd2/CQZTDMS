...
<input type="hidden" name="id" value="${map.ID}"/>
    ...
    <table class="table_edit">
    	<tr>
    		...
    		<td width="20%">
    			<input type="text" class="middle_txt" name="standardPrice" value="${map.STANDARD_PRICE}" id="p1" datatype="0,is_double"/>
    		</td>
    		<td width="12%" align="right">申购车折让总支持额度：</td>
    		<td width="20%">
    			<input type="text" class="middle_txt" name="supportQuota" value="${map.SUPPORT_QUOTA}" id="p2" datatype="0,is_double"/>
    		</td>
    	</tr>
    	<tr>
    		<td width="10%" align="right">申购车第一年支持额度：</td>
    		<td width="20%">
    			<input type="text" class="middle_txt" name="supportQuota2" value="${map.SUPPORT_QUOTA2}" id="p3" datatype="0,is_double"/>
    		</td>
    		<td width="12%" align="right">申购车第一年优惠后购车金额：</td>
    		<td width="20%">
    			<input type="text" class="middle_txt" name="discountAmount" value="${map.DISCOUNT_AMOUNT}" id="p4" datatype="0,is_double"/>
    		</td>
    	</tr>
    	<tr>
    	...
    	<tr>
    		<td colspan="4" align="center">
    			...
    			<input type="button" class="normal_btn" value="拒绝" id="btn_03" onclick="approve(2);" />&nbsp;
...
<script type="text/javascript">
	function approve(val){
		myBtnSet() ;
		if(val == 1 && submitForm('fm')==false)return;
		if(val == 2){
		...
		fm.action = '<%=contextPath%>/feedbackmng/apply/ServiceCarWCAction/fanceApprove.do?type='+val ;
	...
	function goBack(){
		location = '<%=contextPath%>/feedbackmng/apply/ServiceCarWCAction/fanceQueryInit.do' ;
	}
	function showOrHid(){
