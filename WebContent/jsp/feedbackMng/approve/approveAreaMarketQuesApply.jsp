...
function window.onload(){
	...
	if(retCode!=null||retCode!=''){
      if(retCode=="approveFailure"){
         MyAlert("工单审批失败!");
         goSearchPage();;
      }else if(retCode=="noApproveAuthor"){
         MyAlert("您对该工单没有审核权限!");
      }
	}
}
function goSearchPage(){
	fm.action = "<%=contextPath%>/feedbackmng/approve/MarketQuestionOrderApproveManager/areaApproveOrderInit.do";
	fm.submit();
}
</script>
<div class="navigation"><img src="<%=contextPath%>/img/nav.gif" />&nbsp;当前位置：信息反馈管理&gt;信息反馈审批 &gt;市场问题处理工单大区审核</div>
 ...
 <table class="table_edit">
  <th colspan="7"><img class="nav" src="<%=contextPath%>/img/subNav.gif" />审核操作</th>
          <tr > 
            ...
            <textarea  name="AUDIT_CONTENT" id="AUDIT_CONTENT"   rows='2' cols='60' ></textarea>
            <input type="button" onclick="marketOrderApprove('p');" class="normal_btn" style="width=8%" value="通过"/>
            <input type="button" onclick="marketOrderApprove('r')" class="normal_btn" style="width=8%" value="驳回"/>
...
<script>
function marketOrderApprove(str){
	var content = document.getElementById("AUDIT_CONTENT").value;
	var orderid = '<%=detailBean.getOrderId()%>';
	if(str=='r'&&content==''){
       MyAlert('驳回请填写审核意见!');
       return;
	}
	if(submitForm('fm')){
		fm.action = "<%=contextPath%>/feedbackmng/approve/MarketQuestionOrderApproveManager/approveMarketOrderInfo.do?audit="+str+"&ORDER_ID="+orderid+"&approveFlag=1&content="+content;
