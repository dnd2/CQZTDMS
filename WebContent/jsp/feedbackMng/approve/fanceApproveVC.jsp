...
<BODY onload="doInit();">
<div class="navigation"><img src="<%=contextPath%>/img/nav.gif"/>&nbsp;当前位置： 信息反馈管理&gt;信息反馈提报 &gt;服务车价格初审</div>
<form id="fm" name="fm">
...
<table class="table_query">
	...
	<tr>
		...
		<td width="20%">
			<script type="text/javascript">
				genSelBoxExp("status",<%=Constant.SERVICE_CAR_APPLY%>,"<%=Constant.SERVICE_CAR_APPLY_03%>",true,"short_sel","","true",'');
			</script>
		</td>
	</tr>
	<tr>
...
<script type="text/javascript">
	...
	function myHandler(value,meta,rec){
		...
		if(status == <%=Constant.SERVICE_CAR_APPLY_03%>) 
			return '<a href="<%=contextPath%>/feedbackmng/apply/ServiceCarWCAction/fanceApproveInit.do?id='+id+'">[审批]</a>' ;
		else if(status == <%=Constant.SERVICE_CAR_APPLY_06%>)
	...
	function goBackLast(val){
		if(confirm('确认回退？')){
			var url = '<%=contextPath%>/feedbackmng/apply/ServiceCarWCAction/goLast.json?type=1&id='+val ;
			makeCall(url,lastBack1,'fm');
		}else return ;
	}
	function lastBack1(json){
		if(json.flag==1){
			__extQuery__(1) ;
			MyAlert('操作成功!');
		}else if(json.flag==2){
