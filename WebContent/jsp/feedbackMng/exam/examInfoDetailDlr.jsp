...
<div class="navigation"><img src="../../../img/nav.gif" />&nbsp;当前位置：信息反馈管理&gt;考试信息管理&gt;考试信息维护</div>
	...
	<table class="table_info" border="0" id="file">
		...
		<tr colspan="8">
			<th>
				...
				<input type="button" value="答案附件提交" class="long_btn" onclick="showUpload('<%=contextPath%>');"/>
			</th>
		</tr>
		<tr>
	...
	<table class="table_edit">
		<tr>
			<td align="center">
				<input type="button" class="normal_btn" id="sureBtn" value="提交" onclick="subAnswer();"/>
				&nbsp;&nbsp;
...
function subAnswer(){
	MyConfirm("你确定所有答案附件已提交?",sureT);
}
function sureT(){
	$('fm').action = '<%=contextPath%>/feedbackmng/exam/ExamDlrAction/saveAnswer.do' ;
	$('fm').submit();
}
function goBack(){
	location = '<%=contextPath%>/feedbackmng/exam/ExamDlrAction/firstUrlInit.do' ;
}
</script>
</BODY>
</html>