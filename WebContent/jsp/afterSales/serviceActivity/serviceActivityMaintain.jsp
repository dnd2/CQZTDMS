...
<script type="text/javascript">
	...
	function setMoreDealerValue(inputId,inputName,userIdsNames,type){
		...
		function saveData(status) {
			...
			var reg1 = /^(?:\d?\d|100)$/;
			if($("#maintain_number").val()=='' || $("#maintain_number").val()==null){
				MyAlert("保养次数不能为空！");
				return;
			}else if(!reg1.test($("#maintain_number").val())||$("#maintain_number").val()==0){
					MyAlert("请输入正确的保养次数!");
					document.getElementById("maintain_number").value="";
					return;
			}
			var reg2 = /(^[1-9]{1,4}?(\.[0-9]{1,2})?$)|(^(0){1}$)|(^[0-4]\.[0-9]([0-9])?$)/;  
			if($("#maintain_money").val()=='' || $("#maintain_money").val()==null){
				MyAlert("保养金额不能为空！");
				return;
			}else if(!reg2.test($("#maintain_money").val())){
				MyAlert("请输入正确的保养金额!");
				document.getElementById("maintain_money").value="";
				return;
			}
...
<body>
<div class="wbox">
<div class="navigation"><img src="<%=request.getContextPath()%>/img/nav.gif" />&nbsp;当前位置：售后服务&gt;维修管理&gt;服务活动管理送保养（<span style="color:red;">如果导入了VIN和里程、车型、实销日期同时做了录入，以导入VIN为准</span>）</div>
<form method="post" name="fm" id="fm" enctype="multipart/form-data">
	<input type="hidden" name="activityId" id="activityId" value="${info.ACTIVITY_ID }"/>
		...
		<input type="hidden" name="activity_type" id="activity_type" value="<%=Constant.SERVICEACTIVITY_TYPE_NEW_02%>"/>
...
<table  class="table_query" >
	...
	<tr >
		...
		<td  >
			<input class="middle_txt" style="width: 40px" id="maintain_number"  name="maintain_number" type="text"  value="${info.MAINTAIN_NUMBER }" maxlength="2" />
		<span style="color:red;">*</span>
		</td>
		<td class="right">保养金额：</td>
		<td >
			<input class="middle_txt" style="width: 64px" id="maintain_money"  name="maintain_money" type="text"  value="${info.MAINTAIN_MONEY }" maxlength="8" />
		<span style="color:red;">*</span>
		</td>
	</tr>
	</table>
