...
//根据外出维修单号获取对应外出维修信息
function getServiceOutRepairInfo(){
  //var vin = document.getElementById("vin");
  var url = "<%=contextPath%>/afterSales/ServiceOrderAction/getServiceOutRepairInfo.json";
  makeNomalFormCall(url,setServiceOutRepairInfo,'fm');
}
//设置外出维修信息
function setServiceOutRepairInfo(json){
  document.getElementById("egressNo").innerHTML = json.map.EGRESS_NO==null?"":json.map.EGRESS_NO;
  document.getElementById("eStartDate").innerHTML = json.map.E_START_DATE==null?"":json.map.E_START_DATE
  document.getElementById("eEndDate").innerHTML = json.map.E_END_DATE==null?"":json.map.E_END_DATE
  document.getElementById("eLicenseNo").innerHTML = json.map.E_LICENSE_NO==null?"":json.map.E_LICENSE_NO
  document.getElementById("eName").innerHTML = json.map.E_NAME==null?"":json.map.E_NAME
  document.getElementById("destination").innerHTML = json.map.DESTINATION==null?"":json.map.DESTINATION
  document.getElementById("reliefMileage").innerHTML = json.map.RELIEF_MILEAGE==null?"":json.map.RELIEF_MILEAGE
}
</script>
</head>
...
<form method="post" name="fm" id="fm">
  ...
  <input type="hidden" name="egressId" id="egressId" value="${serviceOrderMap.EGRESS_ID}"/>
  ...
  <h2>维修配件</h2>
	...
	<c:if test="${serviceOrderPartList!=null}">
	  ...
	  <tr class="table_list_row1">
	  <td style="background-color:#FFFFFF;">
        <input type="checkbox" ${serviceOrderPartList.IS_THREE_GUARANTEE} onclick="return false"/>
      </td>
      <td style="background-color:#FFFFFF;width:120px;">
  ...
  <h2>外出项目</h2>
	...
	<c:if test="${serviceOrderOutProjectList!=null}">
	  <c:forEach items="${serviceOrderOutProjectList}" var="serviceOrderOutProjectList">
	  <tr class="table_list_row1">
      <td style="background-color:#FFFFFF;width:80px;">
        ${serviceOrderOutProjectList.FEE_CODE}
      </td>
	  <td style="background-color:#FFFFFF;width:120px;">
        ${serviceOrderOutProjectList.FEE_NAME}
      </td>
	  <td style="background-color:#FFFFFF;width:40px;">
	    ${serviceOrderOutProjectList.FEE_PRICE}
      </td>
	  <td style="background-color:#FFFFFF;width:300px;">
        ${serviceOrderOutProjectList.FEE_REMARK}
      </td>
	  <td style="background-color:#FFFFFF;width:40px;">
        ${serviceOrderOutProjectList.FEE_PAYMENT_METHOD}
      </td>
	  <td style="background-color:#FFFFFF;width:80px;">
	    ${serviceOrderOutProjectList.FEE_RELATION_MAIN_PART_NAME}
	  </td>
	  </tr>
	  </c:forEach>
	  </c:if>
