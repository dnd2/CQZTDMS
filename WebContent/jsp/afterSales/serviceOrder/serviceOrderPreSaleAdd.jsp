...
var tempFeeId = ""; //外出维修项目字符串,用于重复验证
var tempEgressId = "";//外出维修临时值
//弹窗样式
var alertCss = {type: 1,
...
function serviceOrderSave(){
  ...
  var labourNameStr = "";//工时代码字符串,用于验证所有工时都被至少一个配件选中
  var labourPayNameStr = "";//自费工时代码字符串，用于验证工时是自费的，对应的配件必须全是自费的，否则不允许保存
  ...
  //外出项目
  var feeIds = document.getElementsByName("feeId");//项目ID
  var feeCodes = document.getElementsByName("feeCode");//项目代码
  var feeNames = document.getElementsByName("feeName");//项目名称
  var feePrices = document.getElementsByName("feePrice");//金额(元)
  var feeRemarks = document.getElementsByName("feeRemark");//备注
  var feePaymentMethods = document.getElementsByName("feePaymentMethod");//付费方式
  var feeRelationMainParts = document.getElementsByName("feeRelationMainPart");//关联主因件
  //外出维修
  var egressId = document.getElementById("egressId");//外出维修ID
  var egressNo = document.getElementById("egressNo");//外出维修代码
  ...
  **/
  ...
  if(egressId.value==""){
    count++;
    msg = msg + "<div>" + count + "、外出维修不能为空!</div>\n";
  }
  //外出项目验证
  if(feeIds.length<=1){
    count++;
    msg = msg + "<div>" + count + "、外出项目不能为空!</div>\n";
  }else{
    for(var i=1;i<feeIds.length;i++){
      if(feePrices[i].value==""){
        count++;
        msg = msg + "<div>" + count + "、外出项目-金额(元)不能为空!</div>\n";
      }
      if(feeRelationMainPart[i].value==""){
        count++;
        msg = msg + "<div>" + count + "、外出项目-关联主因件不能为空!</div>\n";
      }
    }
  }
  **/
...
function servicePartDel(tableId,obj){
  ...
  //移出外出项目关联主因件对应配件下拉框值
  var feeRelationMainParts = document.getElementsByName("feeRelationMainPart");//外出项目关联主因件
  for(var i=0;i<feeRelationMainParts.length;i++){
    options = feeRelationMainParts[i].options;
...
function choiceIsMainPart(obj){
  ...
  for(var i=1;i<isMainParts.length;i++){
    if(isMainParts[i]==obj){
      ...
      partCname = partCnames[i].value;
      if(isMainParts[i].value=="10041001"){
        relationMainPartAdd(i,partId,partCname);
  ...
  //外出项目关联主因件
  var feeRelationMainParts = document.getElementsByName("feeRelationMainPart");
  //alert(obj.value);
  if(obj.value=="10041001"){
    for(var i=0;i<feeRelationMainParts.length;i++){
      feeRelationMainParts[i].options.add(new Option(partCname,partId));
    }
  }else if(obj.value=="10041002"){
    for(var i=0;i<feeRelationMainParts.length;i++){
      options = feeRelationMainParts[i].options;
      for(var j=0;j<options.length;j++){
        if(options[j].value==partId){
...
function relationMainPartDel(pos,partId){
  var relationMainParts = document.getElementsByName("relationMainPart");//配件关联主因件
  var options;
...
//选择外出维修
function serviceOutRepairChoose(){
  ...
  var url = "<%=contextPath%>/afterSales/ServiceOrderAction/serviceOutRepairWin.do?vin="+vin;
  OpenHtmlWindow(url,900,500);
}
//选择外出维修-设置
function setServiceOutRepairInfo(egressId,egressNo,eStartDate,eEndDate,eLicenseNo,eName,destination,reliefMileage){
  //前后两次选择的外出维修不一致，则清空外出项目
  if(tempEgressId!=egressId){
    //清空外出项目
    clearOutProject();
  }
  //存储最新外出维修
  tempEgressId = egressId;
  //设置最新外出维修值
  document.getElementById("egressId").value = egressId;
  document.getElementById("egressNo").value = egressNo;
  document.getElementById("eStartDate").innerHTML = eStartDate;
  document.getElementById("eEndDate").innerHTML = eEndDate;
  document.getElementById("eLicenseNo").innerHTML = eLicenseNo;
  document.getElementById("eName").innerHTML = eName;
  document.getElementById("destination").innerHTML = destination;
  document.getElementById("reliefMileage").innerHTML = reliefMileage;
}
//新增外出项目
function serviceOutProjectAdd(){
  var egressId = document.getElementById("egressId").value;//外出维修
  //提示信息
  if(egressId==""){
    MyAlert("请先选择外出维修!");
    return;
  }
  var url = "<%=contextPath%>/afterSales/ServiceOrderAction/serviceOutProjectWin.do";
  OpenHtmlWindow(url,900,500);
}
//新增外出项目-设置
function setServiceOutProjectInfo(feeId,feeCode,feeName){
  if(tempFeeId.indexOf(feeId)>-1){
    MyAlert("该外出项目已存在,请选择其他项目!");
  }else{
    var pos = tableRowAdd("serviceOutProjectTable");
    var feeIds = document.getElementsByName("feeId");
    var feeCodes = document.getElementsByName("feeCode");
    var feeNames = document.getElementsByName("feeName");
    
    feeIds[pos].value = feeId;
    feeCodes[pos].value = feeCode;
    feeNames[pos].value = feeName;
    //写入外出项目字符串
    tempFeeId += feeId + ",";
  }
}
//删除外出项目
function serviceOutProjectDel(tableId,obj){
  var dels = document.getElementsByName("outProjectDel");
  var feeIds = document.getElementsByName("feeId");
  var feePrices = document.getElementsByName("feePrice");
  ...
  for(var i=1;i<dels.length;i++){
    if(dels[i]==obj){
      feePrices[i].value = "";
      tempFeeId = tempFeeId.replace(feeIds[i].value+",","");
    }
    if(feePrices[i].value!=""){
      feePrice = feePrice + parseFloat(feePrices[i].value);
    }
  }
  //设置 申请费用-外出金额
  setApplyFeePrice(feePrice);
  //删除外出项目列
  tableRowDel(tableId,obj);
}
//设置外出项目金额
function setFeePrice(obj){
  var feePrices = document.getElementsByName("feePrice");
  var feePrice = 0;
  for(var i=1;i<feePrices.length;i++){
    if(feePrices[i]==obj){
      if(!isPositive(feePrices[i].value)){
        MyAlert("外出项目金额必须为大于0的正数!");
        feePrices[i].value = "";
      }
    }
    if(feePrices[i]!=""){
...
//设置 申请费用-外出金额
function setApplyFeePrice(feePrice){
  var applyFeePrice = document.getElementById("applyFeePrice");
  var applyFeePriceSpan = document.getElementById("applyFeePriceSpan");
  
  applyFeePrice.value = feePrice.toFixed(2);//保留2位小数
  applyFeePriceSpan.innerHTML = feePrice.toFixed(2);
  //设置 申请费用-总金额
  setApplyPriceTotal();
}
//设置 申请费用-总金额
function setApplyPriceTotal(){
  ...
  var applyFeePrice = document.getElementById("applyFeePrice");//外出金额
  ...
  applyPriceTotal.value = (parseFloat(applyLabourPrice.value)+parseFloat(applyPartPrice.value)+
                           parseFloat(applyFeePrice.value)).toFixed(2);//保留2位小数
  applyPriceTotalSpan.innerHTML = applyPriceTotal.value;
}
//由于进站时间、进站里程数、VIN变动,清空维修项目、维修配件、申请费用
//清空维修项目
...
//清空外出维修
function clearOutRepair(){
  document.getElementById("egressId").value = "";
  document.getElementById("egressNo").value = "";
  document.getElementById("eStartDate").innerHTML = "";
  document.getElementById("eEndDate").innerHTML = "";
  document.getElementById("eLicenseNo").innerHTML = "";
  document.getElementById("eName").innerHTML = "";
  document.getElementById("destination").innerHTML = "";
  document.getElementById("reliefMileage").innerHTML = "";
  //清空外出项目
  clearOutProject();
}
//清空外出项目
function clearOutProject(){
  var serviceOutProjectTable = document.getElementById("serviceOutProjectTable");
  //清空外出项目
  for(var i=serviceOutProjectTable.rows.length-1;i>0;i--){
    serviceOutProjectTable.removeChild(serviceOutProjectTable.rows[i]);
  }
  //清空外出项目临时值
  tempFeeId = "";
  //设置 申请费用-外出金额 为0
...
function clearApply(){
  ...
  var feePrices = document.getElementsByName("feePrice");//外出金额
  ...
  for(var i=0;i<feePrices.length;i++){
    if(feePrices[i].value!=""){
      feePrice = feePrice + parseFloat(feePrices[i].value);
    }
  }
  priceTotal = labourPriceTotal + partPriceTotal + feePrice;
  ...
  applyFeePrice.value = feePrice.toFixed(2);//保留2位小数
  applyFeePriceSpan.innerHTML = feePrice.toFixed(2);//保留2位小数
...
<form method="post" name="fm" id="fm">
  ...
  <input type="hidden" name="egressId" id="egressId" value=""/><!-- 外出维修ID -->
  <!-- 基本信息 -->
  ...
  <h2>基本信息</h2>
	...
	<tr>
	  ...
	  <td align="left">
	    <input id="mileage" name="mileage" maxlength="30" type="text" class="middle_txt" value="8888" onchange="changeMileage()"/>
	    <font style="color:red">*</font>
	  </td>
	</tr>
	<tr>
  ...
  <h2>维修配件</h2>
	...
	<tbody id="servicePartTable">
	  ...
	  <td style="width:12px">&nbsp;</td>
	  <td style="background-color:#FFFFFF;">
	    <input type="hidden" id="isThreeGuarantee" name="isThreeGuarantee" value="10041001"/><!-- 默认三包 -->
        <input type="checkbox" checked id="isThreeGuaranteeCheckbox" name="isThreeGuaranteeCheckbox" onclick="return false"/>
      </td>
      <td style="background-color:#FFFFFF;width:120px;">
  ...
  <h2>外出维修</h2>
    ...
    <table border="0" align="center" cellpadding="1" cellspacing="1" class="table_query">
	<tr>
	  <td class="table_add_2Col_label_5Letter" style="text-align:right;width:125px;">外出单号：</td>
	  <td align="left" style="width:250px">
	    <input id="egressNo" name="egressNo" maxlength="30" value="" type="text" class="middle_txt" onclick="serviceOutRepairChoose()" readonly />
        <input class="normal_btn" type="button" value="清空" onclick="clearOutRepair();"/>
      </td>
	  <td class="table_add_2Col_label_5Letter" style="text-align:right;width:125px;">开始时间：</td>
	  <td align="left" style="width:245px">
        <span id="eStartDate">--/--</span>
      </td>
	  <td class="table_add_2Col_label_5Letter" style="text-align:right;width:125px;">结束时间：</td>
	  <td align="left" style="width:245px">
        <span id="eEndDate">--/--</span>
      </td>
	</tr>
	<tr>
	  <td class="table_add_2Col_label_5Letter" style="text-align:right">派车车牌号：</td>
	  <td align="left"><span id="eLicenseNo">--/--</span></td>
	  <td class="table_add_2Col_label_5Letter" style="text-align:right">外出人：</td>
	  <td align="left"><span id="eName">--/--</span></td>
	  <td class="table_add_2Col_label_5Letter" style="text-align:right">出差目的地：</td>
	  <td align="left"><span id="destination">--/--</span></td>
	</tr>
	<tr>
	  <td class="table_add_2Col_label_5Letter" style="text-align:right">单程救急里程：</td>
	  <td align="left"><span id="reliefMileage">--/--</span></td>
  ...
  <h2>外出项目</h2>
    ...
    <tr class="table_list_th">
	  ...
	  <th class="noSort">关联主因件</th>
	  <th class="noSort">
	    <a id="itemBtn" type="button" class="normal_btn" value="新增" name="button422" onClick="serviceOutProjectAdd();" />新增</a>
	  </th>
	</tr>
	<tbody id="serviceOutProjectTable">
	<tr class="table_list_row1" style="display:none">
	  <td style="width:12px">&nbsp;</td>
      <td style="background-color:#FFFFFF;width:120px;">
        <input type="hidden" name="feeId" id="feeId" value=""/>
        <input id="feeCode" name="feeCode" maxlength="30" value="" type="text" class="middle_txt" style="width:80px" readonly/>
      </td>
	  <td style="background-color:#FFFFFF;">
        <input id="feeName" name="feeName" maxlength="30" value="" type="text" class="middle_txt" style="width:120px" readonly/>
      </td>
	  <td style="background-color:#FFFFFF;">
        <input id="feePrice" name="feePrice" maxlength="30" value="" type="text" class="middle_txt" style="width:40px" onchange="setFeePrice(this)"/>
        <font style="color:red">*</font>
      </td>
	  <td style="background-color:#FFFFFF;">
        <input id="feeRemark" name="feeRemark" maxlength="30" value="" type="text" class="middle_txt" style="width:300px" />
      </td>
	  <td style="background-color:#FFFFFF;">
	    &nbsp;
	    <select id="feePaymentMethod" name="feePaymentMethod" class="u-select" style="width:40px">
          <option value="11801001">自费</option>
          <option value="11801002" selected>索赔</option>
        </select>
        <font style="color:red">*</font>
	  </td>
	  <td style="background-color:#FFFFFF;">
	    &nbsp;
	    <select id="feeRelationMainPart" name="feeRelationMainPart" class="u-select" style="width:80px">
          <option value="">请选择</option>
        </select>
        <font style="color:red">*</font>
	  </td>
	  <td style="background-color:#FFFFFF;">
	    <a href="#" id="outProjectDel" name="outProjectDel" class="u-anchor" onClick="serviceOutProjectDel('serviceOutProjectTable',this);">删除</a>
	  </td>
	</tr>
	</tbody>
    </table>
  ...
  <h2>申请费用</h2>
    ...
    <tr>
	  ...
	  <td align="left">
	    <input type="hidden" id="applyFeePrice" name="applyFeePrice" value="0.00"/>
	    <span id="applyFeePriceSpan">0.00</span>
	  </td>
