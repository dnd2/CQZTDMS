...
function servicePartAdd(){
  ...
  var repairDateBegin = document.getElementById("repairDateBegin").value;//维修开始时间
  var arrivalDate = document.getElementById("arrivalDate").value;//进站时间
  var mileage = document.getElementById("mileage").value;//进站里程数
  var vinValid = document.getElementById("vinValid").value;//是否有效VIN
  var vin = document.getElementById("vin").value;//vin
  ...
  var partFareRate = document.getElementById("partFareRate").value;//经销商加价率
  var _isThreeGuarantee = document.getElementById("_isThreeGuarantee").value;//是否三包
  ...
  if(repairDateBegin==""){
    count++;
    msg = msg + "<div>" + count + "、请先选择维修开始时间!</div>\n";
  }
  if(mileage==""){
    count++;
  ...
  if(msg == ""){
    var url = "<%=contextPath%>/afterSales/ServiceOrderAction/servicePartWin.do?&repairType="+repairType
    		+ "&ruleId="+ruleId+"&arrivalDate="+arrivalDate+"&repairDateBegin="+repairDateBegin+"&mileage="+mileage
    		+ "&vin="+vin+"&purchasedDate="+purchasedDate+"&partFareRate="+partFareRate
    		+ "&_isThreeGuarantee="+_isThreeGuarantee;
    OpenHtmlWindow(url,900,500);
  }else{
    MyAlert(msg);
    return;
...
function setServicePartInfo(isThreeGuarantee,partId,partCode,partCname,partWarType,salePrice1,partPrice,partNumMax){
  if(tempPartId.indexOf(partId)>-1){
    ...
    var partNumMaxs = document.getElementsByName("partNumMax");
    ...
    partNumMaxs[pos].value = partNumMax;
...
<form method="post" name="fm" id="fm">
  ...
  <input type="hidden" name="_isThreeGuarantee" id="_isThreeGuarantee" value="10041001"/><!--三包-->
  <!-- 基本信息 -->
  ...
  <h2>维修配件</h2>
	...
	<tbody id="servicePartTable">
	  ...
	  <td style="background-color:#FFFFFF;">
	    &nbsp;
	    <input type="hidden" name="partNumMax" id="partNumMax" value=""/>
