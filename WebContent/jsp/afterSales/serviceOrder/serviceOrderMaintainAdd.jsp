...
function setVehicleInfo(json){
	...
	if(json.code=="succ"){
		...
		document.getElementById("curFreeTimes").value = json.map.CUR_FREE_TIMES==null?"":json.map.CUR_FREE_TIMES;
		document.getElementById("curFreeTimesSpan").innerHTML = json.map.CUR_FREE_TIMES==null?"":json.map.CUR_FREE_TIMES;
		
		//之前已清空申请费用,现在只需重新设置首保时候的值即可
		if(Number(document.getElementById("curFreeTimes").value)==1){//当前保养次数等于1为首保
		  document.getElementById("applyMaintainPrice").value = json.map.FREE==null?"":json.map.FREE;
		  document.getElementById("applyMaintainPriceSpan").innerHTML = json.map.FREE==null?"":json.map.FREE;
		}
		...
		document.getElementById("curFreeTimes").value = "";
		document.getElementById("curFreeTimesSpan").innerHTML = "-/-";
...
function setServiceProjectInfo(labourId,labourCode,cnDes,labourHour,labourPrice){
  if(tempLabourId.indexOf(labourId)>-1){
    ...
    var relationLabours = document.getElementsByName("relationLabour");//关联工时
    var curFreeTimes = document.getElementById("curFreeTimes").value;//当前保养次数
    ...
    //设置工时付费方式
    setLabourPaymentMethod(labourPaymentMethods[pos],curFreeTimes);
    //选择工时付费方式
...
//设置工时付费方式
function setLabourPaymentMethod(labourPaymentMethod,curFreeTimes){
  //alert(curFreeTimes);
  labourPaymentMethod.options.length = 0;
  if(Number(curFreeTimes)>1){//当前保养次数大于1为例保
    labourPaymentMethod.options.add(new Option("自费","11801001"));
  }else{//首保
    labourPaymentMethod.options.add(new Option("索赔","11801002"));
  }
}
//选择工时付费方式
...
function setServicePartInfo(isThreeGuarantee,partId,partCode,partCname,partWarType,salePrice1,partPrice,partNumMax){
  if(tempPartId.indexOf(partId)>-1){
    ...
    //设置配件付费方式
    setPartPaymentMethod(partPaymentMethods[pos],curFreeTimes);
    //选择配件付费方式
...
//设置配件付费方式
function setPartPaymentMethod(partPaymentMethod,curFreeTimes){
  partPaymentMethod.options.length = 0;
  if(Number(curFreeTimes)>1){//当前保养次数大于1为例保
    partPaymentMethod.options.add(new Option("自费","11801001"));
  }else{//首保
    partPaymentMethod.options.add(new Option("索赔","11801002"));
  }
}
//选择配件付费方式
...
function setApplyLabourHour(labourHour){
  var curFreeTimes = document.getElementById("curFreeTimes");
  ...
  if(curFreeTimes.value==1){//当前保养次数为1为首保,首保不计算申请费用-配件金额
    return;
  }
...
function setApplyPriceTotal(){
  ...
  var applyMaintainPrice = document.getElementById("applyMaintainPrice");//首保金额
  ...
  applyPriceTotal.value = (parseFloat(applyLabourPrice.value)+parseFloat(applyPartPrice.value)+parseFloat(applyMaintainPrice.value)).toFixed(2);//保留2位小数
...
function clearApply(){
  ...
  var applyPartPrice = document.getElementById("applyPartPrice");
  var applyPartPriceSpan = document.getElementById("applyPartPriceSpan");
  var applyMaintainPrice = document.getElementById("applyMaintainPrice");
  var applyMaintainPriceSpan = document.getElementById("applyMaintainPriceSpan");
  ...
  applyMaintainPrice.value = 0.00;
  applyMaintainPriceSpan.innerHTML = "--/--";
  ...
  <h2>基本信息</h2>
	...
	<tr>
	  <td colspan=6 style="text-align:center;font-size:15px;font-weight:bold;">
        <input type="hidden" name="curFreeTimes" id="curFreeTimes" value=""/>
                    当前为第
        <span id="curFreeTimesSpan" style="font-size:20px;font-weight:bold;color:#FF0000;">-/-</span>
                    次保养.
	  </td>
	</tr>
    </table>
  ...
  <h2>维修配件</h2>
	...
	<tbody id="servicePartTable">
      ...
      <td style="background-color:#FFFFFF;width:120px;">
        ...
        <input type="checkbox" id="isThreeGuaranteeCheckbox" name="isThreeGuaranteeCheckbox" onclick="return false" style="display:none"/>
  ...
  <h2>申请费用</h2>
	...
	<tr>
	  ...
	  <td align="left">
	    ...
	    <span id="applyLabourHourSpan">--/--</span>
	  </td>
	  ...
	  <td align="left">
	    ...
	    <span id="applyLabourPriceSpan">--/--</span>
	  </td>
	  ...
	  <td align="left">
	    ...
	    <span id="applyPartPriceSpan">--/--</span>
	  </td>
    </tr>
    <tr>
	  ...
	  <td align="left">
        <input type="hidden" id="applyMaintainPrice" name="applyMaintainPrice" value="0.00"/>
	    <span id="applyMaintainPriceSpan">--/--</span>
      </td>
    ...
    <tr>
	  ...
	  <td align="left">
	    ...
	    <span id="applyPriceTotalSpan">--/--</span>
	  </td>
    </tr>
    </table>
    </div>
