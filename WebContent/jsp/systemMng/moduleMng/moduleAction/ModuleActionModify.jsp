...
<div class="wbox">
<div class="navigation"><img src="<%=contextPath%>/img/nav.gif"/>&nbsp;当前位置：系统管理&gt;权限管理&gt; 模块操作维护(修改)</div>
<form name="fm" id="fm" method="post" enctype="multipart/form-data">
<input type="hidden" id="ACTION_ID" name="ACTION_ID" value="${dataMap.ACTION_ID}" jset="maindata"/>
<table id="tb1" class="table_query">
    <tr>  
        <td width="10%">模块选择:</td>
        <td width="20%">
            <input type="hidden" name="MODULE_ID" id="MODULE_ID" value="${dataMap.FUNC_ID}" readonly jset="maindata"/>
            <input type='text' name="MODULE_NAME" id="MODULE_NAME" value="${dataMap.FUNC_NAME}" readonly jset="maindata">
        </td>
        
        <td width="10%">操作编码:</td>
        <td width="20%">
            <input type='text' name="ACTION_CODE" id="ACTION_CODE" value="${dataMap.ACTION_CODE}" jset="maindata">
        </td>
        <td width="10%">操作名称:</td>
        <td width="20%">
            <input type='text' name="ACTION_NAME" id="ACTION_NAME" value="${dataMap.ACTION_NAME}" jset="maindata">
        </td>                                  
    </tr>
    <tr>
        <td width="10%">操作类型:</td>
...
fm.ACTION_TYPE.value="${dataMap.ACTION_TYPE}";

/*回调函数,popSelect的第二个参数
function showSelect(data){
	fm.MODULE_ID.value=data.ID;
	fm.MODULE_NAME.value=data.DESCR;
}
*/
