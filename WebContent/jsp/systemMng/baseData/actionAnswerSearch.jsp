...
<title>功能使用情况查询</title>
</head>
<body onload="loadcalendar();">
<div class="wbox">
<div class="navigation"><img src="<%=contextPath%>/img/nav.gif" />&nbsp;当前位置： 系统管理 &gt; 系统运行情况 &gt;功能使用情况查询</div>
...
<table class="table_query" border="0">
	<tr>
		<td class="table_query_2Col_label_4Letter" nowrap="nowrap">查询时间：</td>
			<td   nowrap="nowrap">
				<input id="strTime" datatype="1,is_date,20" group="strTime,endTime" class="short_txt" type="text" name="STRTIME" readonly="readonly"/>
				<input class="time_ico" type="button" onclick="showcalendar(event, 'strTime', false);" value="&nbsp;" />至
				<input id="endTime" datatype="1,is_date,20" class="short_txt" type="text" name="ENDTIME" readonly="readonly"/>
			...
			<td nowrap="nowrap" >
			<input class="middle_txt" value="" onblur="isCloseTreeDiv(event,this,'dtree')" id="FUNNAME" name="FUNNAME" onclick="showFUNC()" style="cursor: pointer;" readonly="readonly" type="text" />
			</td>
...
var func_tree_url = "<%=contextPath%>/sysmng/systemRunSituation/AnswerSearch/initFunTree.json";

var url = "<%=contextPath%>/sysmng/systemRunSituation/AnswerSearch/AnswerUseSearch.json?COMMAND=1";
...
var columns = [
				...
				{header: "开始时间", dataIndex: 'startTime',orderCol:"STR_RESPONSE_TIME"},
				{header: "结束时间", dataIndex: 'endTime',orderCol:"END_RESPONSE_TIME"},
				{header: "公司", dataIndex: 'company',orderCol:"company_name"},
				{header: "用户", dataIndex: 'userName',orderCol:"name"},
				{header: "Action", dataIndex: 'acName',orderCol:"action_desc"},
				{id:'action',header: "响应时间（毫秒）",dataIndex: 'answerTime',orderCol:"response_time"}
			  ];
</script>
</body>
</html>