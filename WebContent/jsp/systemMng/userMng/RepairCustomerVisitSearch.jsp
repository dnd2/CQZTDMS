...
<script language="JavaScript">
    var myPage;
    var url = "<%=contextPath%>/sysmng/usemng/RepairCustomerVisit/Query.json";		
	var title = null;
	var columns = [
				{id:'id',header: "客户ID", width:'10%', dataIndex: 'CUS_ID'},
				{header: "客户名称", width:'10%', dataIndex: 'CUSTOMER_NAME'},
				{header: "回访日期", width:'10%', dataIndex: 'VISIT_DATE',renderer:formatDate},
				{header: "电话", width:'10%', dataIndex: 'PHONE'},
				{header: "车型", width:'10%', dataIndex: 'GROUP_CODE'},
				{header: "车牌号", width:'10%', dataIndex: 'LICENSE_NO'},
				{header: "是否三包内", width:'10%', dataIndex: 'THREE_GUARANTEES',renderer:getItemValue},
				{header: "维修项目", width:'10%', dataIndex: 'REPAIR_ITEM',renderer:getItemValue},
...
function updateCus(value){
     location = '<%=contextPath%>/sysmng/usemng/RepairCustomerVisit/modfiInit.do?CUS_ID='+value;
 }	  
function dropCus(value){
	MyConfirm("是否确认删除？",drop,[value]);
}
function drop(value){
	makeNomalFormCall('<%=contextPath%>/sysmng/usemng/RepairCustomerVisit/delete.json?CUS_ID='+value,dropCall,'fm','');
}
...
<body onload='doInit()'>
<div class="navigation"><img src="<%=contextPath%>/img/nav.gif" />&nbsp;当前位置：信息反馈管理 &gt; 信息反馈提报 &gt; 维修客户回访查询(经销商端)</div>
...
<tr>		
	...
	<tr>
		<td align="center" colspan="4">
			...
			<input class="normal_btn" type="button" value="新 增" onclick="window.location.href='<%=contextPath%>/sysmng/usemng/RepairCustomerVisit/addInit.do'" value="新 增" />
		</td>
	</tr>
</table>
  <!-- 查询条件 end -->
