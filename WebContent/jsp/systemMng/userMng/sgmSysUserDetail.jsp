...
<head>
	...
	<script type="text/javascript">
		...
		var getPoseByIdsUrl = "<%=contextPath%>/sysmng/usemng/SgmSysUser/addPoseQueryByIPoseIds.JSON";
		var dels = 2;
		...
		function addPose(path) {
			setPoseIds();
			OpenHtmlWindow(path+"/sysmng/usemng/SgmSysUser/addPoseQueryInit.do?POSE_IDS="+$('#POSE_IDS')[0].value,800,500, '信息');
		}

		function toGoSysUserSearch() {
			window.location = "<%=contextPath%>/sysmng/usemng/SgmSysUser/querySgmSysUserInit.do";
		}
...
<div class="wbox">
	<div class="navigation"><img src="<%=contextPath%>/img/nav.gif" />&nbsp;当前位置： 系统管理 &gt; 权限管理 &gt; 用户维护</div>
			...
			<div class="form-body">
				<table class="table_query" border="0">
					<tr>
						...
						<td id="YHZH"><%=userPO.getAcnt() %></td>
						<td class="right">密码：</td>
						<td><input class="middle_txt" type="password" id="PASSWORD" name="PASSWORD" /></td>
					...
					<tr>
						...
						<td>
							<input class="middle_txt" datatype="0,is_letter_cn,10" maxlength="10" type="text" id="NAME" name="NAME" value="<%=userPO.getName() %>"/>
						</td>
						<td class="right">E-mail：</td>
						<td>
							<input name="EMAIL" datatype="1,is_email,49" maxlength="49" id="EMAIL" type="text" class="middle_txt" value="<%=userPO.getEmail() == null ? "" : userPO.getEmail() %>" /></td>
						<td class="right">性别：</td>
						<td>
							<script type="text/javascript"> 
								genSelBox("GENDER",<%=Constant.GENDER_TYPE%>,<%=userPO.getGender() %>,false,"","");
							</script>
						</td>
					</tr>
					<tr>
						...
						<td>
							<input class="middle_txt" datatype="1,is_digit,11" maxlength="11" type="text" id="HAND_PHONE" name="HAND_PHONE" value="<%=userPO.getHandPhone() == null ? "" : userPO.getHandPhone() %>" /><font id="sjj" color="bule">&nbsp;</font>
						</td>
						<td class="right">电话：</td>
						<td>
							<input class="middle_txt" datatype="1,is_phone,19" maxlength="19" type="text" id="PHONE" name="PHONE" value="<%=userPO.getPhone() == null ? "" : userPO.getPhone() %>"/>
						</td>
						<td class="right">状态：</td>
						<td>
							<script type="text/javascript">genSelBox("USER_STATUS",<%=Constant.STATUS%>,<%=userPO.getUserStatus() %>,false,"","");</script>
						</td>
					</tr>
				</table>
			</div>
	...
	<table class="table_list" style="border-bottom:1px solid #DAE0EE" id="roll">
		<tr class="table_list_th tb-title">
	...
	<table border="0" cellpadding="0" cellspacing="0" width="100%">
		<tr >
			<td align="center">
				...
				<input name="button2" type="button" class="normal_btn" onclick="sub('<%=contextPath%>/sysmng/usemng/SgmSysUser/modfiSgmSysUser.json')" value="保 存"/>&nbsp;
				<input name="button" type="button" class="normal_btn" onclick="toGoSysUserSearch()" value="取 消"/>
			</td>
		</tr>
	</table>
</div>
