...
<script>
var myPage;
var url = "<%=contextPath%>/sysmng/dealer/ChangeServiceDealerInfo/queryAuditServiceDealerInfo1.json";
var title= null;
var columns = [
...
// 				{header: "审核人", width:'10%', dataIndex: 'AUDIT_BY_NAME'},
...
function batchAccept(){		
	if (confirm("确认要批量通过?")){
		...
		var url = '<%=contextPath%>/sysmng/dealer/ChangeServiceDealerInfo/batchAccept1.do?ID='+delIds;
	    fm.action = url;
	    fm.submit();
	}
}
function batchRebut(){		
	if (confirm("确认要驳回?")){
		...
		var url = '<%=contextPath%>/sysmng/dealer/ChangeServiceDealerInfo/batchRebut1.do?ID='+delIds;
	    fm.action = url;
	    fm.submit();
	}
}
