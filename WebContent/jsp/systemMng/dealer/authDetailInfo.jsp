...
<body>
<div class="navigation"><img src="<%=contextPath%>/img/nav.gif" />&nbsp;当前位置： 个人信息管理&gt;个人信息管理&gt;经销商信息变更&gt;审核</div>
 ...
 <input type="hidden"  name="sJDealerId"  value=""  id="sJDealerId" />
    <table width=100% border="0" align="center" cellpadding="1" cellspacing="1" class="table_query">
		  ...
		  <tr>
		    ...
		    <td>${poValue.dealerCode }<input type="hidden" name="dealerId" value="${poValue.dealerId }" /><input type="hidden" name="ID" value="${ID }" /></td>
	  ...
	  <table width=100% border="0" align="center" cellpadding="1" cellspacing="1" class="table_query">
	    ...
	    <!-- <tr>
		    ...
		    <td>修改为：</td>
		    <td>${orgCode }<input type="hidden" name="dealerHigherOrg" value="${poValue.dealerHigherOrg }"/></td>
	      </tr> -->
	      <tr>
		    ...
		    <td>修改为：</td>
		    <td><script type="text/javascript">writeItemValue(${poValue.dealerType })</script><input type="hidden" name="dealerType" value="${poValue.dealerType }"/></td>
	      </tr>
	      <tr>
	    	<td class="table_query_2Col_label_6Letter">经销商公司：</td>
	    	<td>${poValue.dealerCompanyBck }</td>
		    <td>修改为：</td>
		    <td>${companyName }<input type="hidden" name="companyId" value="${poValue.dealerCompany }"/></td>
	      </tr>
	      <tr>
		    ...
		    <td>修改为：</td>
		    <td><select disabled="disabled"  class="min_sel" id="txt4" onchange="_genCity(this,'txt5')"></select><input type="hidden" name="dealerProvinces" value="${poValue.dealerProvinces }"/></td>
	      </tr>
	      <tr>
		    ...
		    <td>修改为：</td>
		    <td><select disabled="disabled" class="min_sel" id="txt5" onchange="_genCity(this,'txt6')"></select><input type="hidden" name="dealerCity" value="${poValue.dealerCity }"/></td>  
	      </tr>
	      <tr>
		    ...
		    <td>修改为：</td>
		    <td class="table_query_4Col_input" nowrap="nowrap"><select disabled="disabled" class="min_sel" id="txt6" name="COUNTIES2"></select><input type="hidden" name="counties" value="${poValue.counties }"/></td>
	      </tr>
	      <tr>
		    ...
		    <td>修改为：</td>
		    <td class="table_query_4Col_input" nowrap="nowrap">${poValue.township }<input type="hidden" name="township" value="${poValue.township }"/></td>
	      </tr>
	      <tr>
	    	<td class="table_query_3Col_label_6Letter" nowrap="nowrap">开票单位：</td>
	    	<td>${poValue.billingUnitBck }</td> 
		    <td>修改为：</td>
		    <td>${poValue.billingUnit }<input type="hidden" name="billingUnit" value="${poValue.billingUnit }"/></td> 
	      </tr>
	      <tr>
	    	<td class="table_query_2Col_label_6Letter">详细地址：</td>
	    	<td>${poValue.detailsAddressBck }</td>
		    <td>修改为：</td>
		    <td>${poValue.detailsAddress }<input type="hidden" name="detailsAddress" value="${poValue.detailsAddress }"/></td>
	      </tr>
	      <tr>
	    	<td class="table_query_2Col_label_6Letter">备注：</td>
	    	<td>${poValue.dealerRemakeBck }</td>	
		    <td>修改为：</td>
		    <td>${poValue.dealerRemake }<input type="hidden" name="dealerRemake" value="${poValue.dealerRemake }"/></td>	
	      </tr>
	      <tr>
		    ...
		    <td>修改为：</td>
		    <td><script type="text/javascript">writeItemValue(${poValue.imageLevel })</script><input type="hidden" name="imageLevel" value="${poValue.imageLevel }"/></td>
	      </tr>
	      <tr>
		    ...
		    <td>修改为：</td>
		    <td><script type="text/javascript">writeItemValue(${poValue.dealerLevel})</script><input type="hidden" name="dealerLevel" value="${poValue.dealerLevel }"/></td>
	      </tr>
	      ...
	      <tr>
		    ...
		    <td>修改为：</td>
		    <td>${dealerName }<input type="hidden" name="dealerHigher" value="${poValue.dealerHigher }"/></td>
	      </tr>
	      </c:if>
	      <tr>
		    ...
		    <td>修改为：</td>
		    <td>${poValue.zipCode }<input type="hidden" name="zipCode" value="${poValue.zipCode }"/></td>
	      </tr>
	      <tr>
	    	<td class="table_query_2Col_label_6Letter">法人：</td>
	    	<td>${poValue.legalBck }</td>
		    <td>修改为：</td>
		    <td>${poValue.legal }<input type="hidden" name="legal" value="${poValue.legal }"/></td>
	      </tr>
	      <tr>
	    	<td class="table_query_2Col_label_6Letter">负责人：</td>
	    	<td>${poValue.personChargeBck }</td>
		    <td>修改为：</td>
		    <td>${poValue.personCharge }<input type="hidden" name="personCharge" value="${poValue.personCharge }"/></td>
	      </tr>
	      <tr>
		    ...
		    <td>修改为：</td>
		    <td class="table_query_4Col_input" nowrap="nowrap">${poValue.webmasterPhone }<input type="hidden" name="webmasterPhone" value="${poValue.webmasterPhone }"/></td>
	      </tr>
	      <tr>
	    	<td class="table_query_2Col_label_6Letter">值班电话：</td>
	    	<td>${poValue.dutyPhoneBck }</td>
		    <td>修改为：</td>
		    <td>${poValue.dutyPhone }<input type="hidden" name="dutyPhone" value="${poValue.dutyPhone }"/></td>
	      </tr>
	      <tr>
	    	<td class="table_query_2Col_label_6Letter">电话：</td>
	    	<td>${poValue.phoneBck }</td> 
		    <td>修改为：</td>
		    <td>${poValue.phone }<input type="hidden" name="phone" value="${poValue.phone }"/></td> 
	      </tr>
	      <tr>
	    	<td class="table_query_2Col_label_6Letter">传真：</td>
	    	<td>${poValue.faxBck }</td>
		    <td>修改为：</td>
		    <td>${poValue.fax }<input type="hidden" name="fax" value="${poValue.fax }"/></td>
	      </tr>
	      <tr>
	    	<td class="table_query_2Col_label_6Letter">Email：</td>
	    	<td>${poValue.emailBck }</td>
		    <td>修改为：</td>
		    <td>${poValue.email }<input type="hidden" name="email" value="${poValue.email }"/></td>
	      </tr>
	      <tr>
	      	<td colspan="4">
	      	<%
	      ...
	      <tr>
	      	...
	      	<table class="table_info" border="0" id="file">
			<tr>
    			<td>审核意见：<textarea rows="5" cols="100" name="info"></textarea></td>
  			</tr>
			</table> 
	      	</td>
	      </tr>
	      <tr>
	    	<td colspan="4" align="center">
	    		<input type="button" value="审核" name="cancelBtn"  class="normal_btn" onclick="auth();" />
	    		<input type="button" value="退回" name="cancelBtn"  class="normal_btn" onclick="reject();" />
...
function auth(){
	if(confirm("确认审核经销商信息吗?")){
		fm.action = '<%=contextPath%>/sysmng/dealer/DealerInfo/authDealerChange.do';
		fm.submit();
	}
}
...
function reject(){
	...
	if(confirm("确认退回经销商信息吗?")){
		fm.action = '<%=contextPath%>/sysmng/dealer/DealerInfo/rejectDealerChange.do';
		fm.submit();
	}
}
</script>
