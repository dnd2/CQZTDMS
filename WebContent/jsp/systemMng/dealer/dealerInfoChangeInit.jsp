  ...
  <img src="<%=contextPath%>/img/nav.gif" />&nbsp;当前位置： 个人信息管理&gt;个人信息管理&gt;经销商信息变更</div>
   <table  class="table_query">
          <tr>
              ...
              <td>
				<script type="text/javascript">
					genSelBoxExp("status",<%=Constant.DEALER_CHANGE%>,"-1",true,"short_sel",'',"false",'');
		    	</script>
              </td>                        
          </tr>
          <tr>
         	  <td align="right" nowrap="true">提报日期：</td>
...
<script type="text/javascript" >
var myPage;
	var url = "<%=request.getContextPath()%>/sysmng/dealer/DealerInfo/queryDealerInfoChange.json";
	var title = null;
	...
	function myLink(value,meta,record){
		if(record.data.STATUS==<%=Constant.DEALER_CHANGE_01%>){
			return String.format("<a href=\"#\" onclick='viewDetailInfo(\""+value+"\")'>[查看]</a><a href=\"#\" onclick='updateInfo(\""+value+"\")'>[修改]</a><a href=\"#\" onclick='reported(\""+value+"\")'>[上报]</a>");
		}
		if(record.data.STATUS==<%=Constant.DEALER_CHANGE_02%> || record.data.STATUS==<%=Constant.DEALER_CHANGE_03%>){
			return String.format("<a href=\"#\" onclick='viewDetailInfo(\""+value+"\")'>[查看]</a>");
		}
		if(record.data.STATUS==<%=Constant.DEALER_CHANGE_04%>){
			return String.format("<a href=\"#\" onclick='viewDetailInfo(\""+value+"\")'>[查看]</a><a href=\"#\" onclick='updateInfo(\""+value+"\")'>[修改]</a>");
		}
	}

	function viewDetailInfo(id){
		window.location="<%=contextPath%>/sysmng/dealer/DealerInfo/dealerDetailInfo.do?id="+id;
	}
	function updateInfo(id){
		window.location="<%=contextPath%>/sysmng/dealer/DealerInfo/updateDealerDetailInfo.do?id="+id;
	}
	function reported(id){
		MyConfirm("是否确认上报？",reportChange,[id]);
	}
  ...
  function subFun(){
    fm.action="<%=contextPath%>/sysmng/dealer/DealerInfo/addDealerInfo.do";
    fm.submit();  
  }
//关闭方法：
