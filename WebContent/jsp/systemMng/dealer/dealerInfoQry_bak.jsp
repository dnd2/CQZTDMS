...
<table class="table_query" border="0">
	...
	<tr>
		...
		<label>
				<script type="text/javascript">
					genSelBoxExp("DEALERTYPE",<%=Constant.DEALER_TYPE%>,"-1",true,"short_sel",'onchange=fwset(this.value)',"false",'');
				</script>
		</label>
		</td>
	...
	<tr>
		<td class="table_query_3Col_label_6Letter" nowrap="nowrap">昌铃形象等级：</td>
		<td class="table_query_4Col_input" nowrap="nowrap"> 
		<label>
				<script type="text/javascript">
					genSelBoxExp("IMAGELEVEL",<%=Constant.IMAGE_LEVEL%>,"-1",true,"short_sel",'',"false",'');
				</script>
		</label>
		</td>
		<td class="table_query_3Col_label_6Letter" nowrap="nowrap">昌汽形象等级：</td>
		<td class="table_query_4Col_input" nowrap="nowrap">
			<script type="text/javascript">
					genSelBoxExp("IMAGELEVEL2",<%=Constant.IMAGE_LEVEL%>,"-1",true,"short_sel",'',"false",'');
				</script>
		</td>
	</tr>
	<tr>
		...
		<td class="table_query_4Col_input" nowrap="nowrap">
		<select class="min_sel" id="brandq" name="brandq">
			<option value="" >-请选择-</option>
		   <c:forEach items="${brand}" var="ber">
				<option value="${ber.GROUP_CODE}">${ber.GROUP_NAME}</option>
	      </c:forEach>
	      </select>
		</td>
	...
	<tr>
		<td id="sh_seach" style="display: none" class="table_query_2Col_label_6Letter">服务站状态：</td>
			<td id="sh_xxzt" class="table_query_2Col_label_6Letter">信息状态：</td>
		    <td class="table_query_4Col_input">
		    <label>
				<script type="text/javascript">
					genSelBoxExp("SERVICE_STATUS",<%=Constant.DLR_SERVICE_STATUS%>,"",'true',"min_sel",'',"false",'');
				</script>
		   </label>
		    </td>
		    <td>是否特殊客商：</td>
...
function myLink(dealer_id){
    return String.format(
    		 "<input type='button' class='normal_btn' onclick='searchDetail("+dealer_id+")' value='查看'></a>");
} 

function searchDetail(obj){
	var url = "<%=contextPath%>/sysmng/dealer/DealerInfoQuery/queryDealerInfoDetail.do?DEALER_ID="+obj;
