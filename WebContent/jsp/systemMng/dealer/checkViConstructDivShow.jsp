...
<!-- 			</table> -->
				...
				<c:if test="${dmap.YEAR_FLAG==1}">
	             <tr >
	                ...
	                <input cn_name="建设日期" class="time_txt" readonly="readonly" id="CONSTRUCT_SDATE" name="CONSTRUCT_SDATE"
	                                                    datatype="1,is_date,10" maxlength="10" value="<fmt:formatDate value="${dMap_main.CONSTRUCT_SDATE}" pattern="yyyy-MM-dd"/>" style="width:65px"
	                                                    group="CONSTRUCT_SDATE,CONSTRUCT_EDATE"/>
	                    至
	                    <input class="time_txt" readonly="readonly" id="CONSTRUCT_EDATE"  name="CONSTRUCT_EDATE" datatype="1,is_date,10" value="<fmt:formatDate value="${dMap_main.CONSTRUCT_EDATE}"  pattern="yyyy-MM-dd"/>" style="width:65px"
	                ...
	                <input class="time_txt" cn_name="实地验收时间"  id="ACCEPT_DATE" name="ACCEPT_DATE" 
	                      datatype="1,is_date,10" maxlength="10" value="<fmt:formatDate value="${dMap_main.ACCEPT_DATE}" pattern="yyyy-MM-dd"/>" style="width:65px" /></td>
	             </tr>
          </c:if>
		 ...
		 <td align="right">VI形象验收日期：</td>
             <td align="left">
             	<input name="VI_CONFRIM_DATE" readonly="readonly" cn_name="VI形象验收日期"  type="text"  class="short_time_txt" id="VI_CONFRIM_DATE"  onclick="calendar();" readonly="readonly"  value="<fmt:formatDate value="${dMap_main.CHECKED_DATE}" pattern="yyyy-MM-dd"/>"/>
             </td>
       ...
       <input class="time_txt" cn_name="支持月度开始时间" readonly="readonly" onclick="calendar();"  id="SUPPORT_SDATE" name="SUPPORT_SDATE"
		 ...
		 <tr>
			   ...
			   <td align="right">提车支持款：</td>
               <td align="left">
                <change:select sql="select p.policy_name ,to_char(p.deploy_id)||'_'||to_char(p.dismoney) as deploy_id  from tt_sales_policy_deploy p where p.deploy_status = 1   and trunc(sysdate) >= trunc(start_date) and trunc(sysdate) <= trunc(stop_date)  " noTop="false" fieldCode="deploy_id" fieldName="policy_name" style="short_sel" name="AMOUNT" func="readonly=readonly"/> 
               </td>
		 </tr>
                <tr>
        <td align="right">备注：</td>
		 	...
		 	<table id="year1" width=100% border="0"  align="center" cellpadding="1" cellspacing="1" class="table_query">
			 	<tr width=100% border="0" align="center" cellpadding="1" cellspacing="1">
					<td colspan="4" class="table_query_4Col_input" style="text-align: center">
			 		<input name="closeBtn" type="button" class="normal_btn" onclick="_hide();" value="关 闭" id="closeBtn" /> &nbsp;
			 		</td>
			 	</tr>
		 	</table>
		 </form>
