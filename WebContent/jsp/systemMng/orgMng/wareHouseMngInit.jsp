...
<script type="text/javascript">
	...
	var myPage;
	var url = "<%=contextPath%>/sysmng/orgmng/WareHouseMng/wareHouseQuery.json";
	...
	var columns = [
				...
				{id:'action',header: "操作", walign:'center',idth:70,dataIndex: 'WAREHOUSE_ID',renderer:myLink} 
		      ];      
	// 
	function myLink(value,meta,record){ 
        	 return String.format(
        		 "<a href=\"#\" onclick='queryInvoice(\""+ value +"\")'>[编辑]</a>");
    }
    ...
    function queryInvoice(value){
    	location = '<%=contextPath%>/sysmng/orgmng/WareHouseMng/wareHouseEdit.do?WAREHOUSE_ID='+value;
    }
	//查询条件重置
	function requery() {
	...
	function wareHouseAdd(){
		 window.self.location = "<%=contextPath%>/sysmng/orgmng/WareHouseMng/wareHouseAdd.do";		 
	}
	
	function clearDate(){
		document.getElementById('houseCode').value = '' ;
		document.getElementById('houseName').value = '' ;
		document.getElementById('hourseDate').value = '' ;
		document.getElementById('linkMain').value = '' ;
...
<div class="wbox">
	...
	<form id="fm" name="fm" method="post">
		...
		<table class="table_query" border="0">
			<tbody>
			<tr>
				<td width="12%" class="tblopt right">仓库代码：</td>
				<td class="left" width="10%">
					<input type="text" class="middle_txt" id="houseCode" name="houseCode" value=""/>
				</td>
				
				<td width="12%" class="tblopt right">仓库名称：</td>
				<td class="left" width="10%">
					<input type="text" class="middle_txt" id="houseName" name="houseName" value=""/>
				</td>
				
				<td class="tblopt right">创建日期：</td>
				<td class="left">
      				<input name="hourseDate"  id="hourseDate" value="${date }" type="text" class="middle_txt"  style="width:100px" onFocus="WdatePicker({el:$dp.$('hourseDate'), maxDate:'#F{$dp.$D(\'payDate1\')}'})" />
		      			<input  id="payDate1"  type="hidden"   />
    			</td>
			</tr>
			<tr>
				<td width="12%" class="tblopt right">联系人：</td>
				<td class="left" width="10%">
					<input type="text" class="middle_txt" id="linkMain" name="linkMain" value=""/>
				</td>
				<td class="tblopt right" width="15%">联系电话：</td>
				<td class="left">
      				<input type="text" class="middle_txt" id="tel" name="tel" value=""/>
    			</td>
    			<td class="tblopt right" width="15%">地址：</td>
				<td class="left">
      				<input type="text" class="middle_txt" id="address" name="address" value=""/>
    			</td>
			</tr>
			<tr>
