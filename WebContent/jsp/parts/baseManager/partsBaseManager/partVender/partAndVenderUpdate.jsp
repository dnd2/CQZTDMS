...
function confirmAdd(){
	...
	MyConfirm('确认保存？', function(){
		btnDisable();
		var url = '<%=contextPath%>/parts/baseManager/partsBaseManager/PartAndVenderManager/updateRelation.json';
...
<div class="wbox">
    <div class="navigation">
    	<img src="<%=contextPath%>/img/nav.gif"/>&nbsp;当前位置：配件管理 &gt; 基础信息管理&gt;&nbsp;计划相关信息维护&gt;&nbsp;供应商管理&gt;&nbsp;配件供应商维护&gt;&nbsp;修改
    </div>
    <form id="fm" name="fm" method="post">
    	<input type="hidden" name="SV_ID" value="${ps.SV_ID }" />
    	<input type="hidden" name="OLD_VENDER_ID" value="${ps.DEALER_ID }" />
    	<input type="hidden" id="PART_ID" name="PART_ID" value="${ps.PART_ID }" />
    	<div class="form-panel">
        <h2><img src="<%=contextPath%>/img/subNav.gif"/>修改信息</h2>
	        <div class="form-body">
		        <table class="table_query">
		            <tr>
		                <td class="right">供应商编码：</td>
		                <td>
		                	<input type="hidden" name="VENDER_ID" id="VENDER_ID" value="${ps.DEALER_ID }" />
		                    <input type="text" readonly="readonly" class="middle_txt" name="VENDER_CODE" id="VENDER_CODE" datatype="0,is_null" value="${ps.DEALER_CODE }" maxlength="50"/>
		                	<input type="button" class="mini_btn" onclick="getDealerInfoke('dealerCode_in','dealerName_in')" value="...">
		                </td>
		                
		                <td class="right">供应商名称：</td>
		                <td>
		                	<input type="text" readonly="readonly" class="middle_txt" name="VENDER_NAME" id="VENDER_NAME" value="${ps.DEALER_NAME }"/>
		                </td>
		                
		                <td class="right">是否有效：</td>
		                <td>
		                	<script type="text/javascript">
		                        genSelBox("state", <%=Constant.IF_TYPE%>, '${ps.STATE}', false, "");
		                    </script>
		                </td>
		            </tr>
		            ...
		            <tr>
		                ...
		                <td>
		                    <input type="text" readonly="readonly" class="middle_txt" id="PART_OLDCODE" value="${ps.PART_OLDCODE }"/>
		                </td>
		                <td class="right">配件名称：</td>
		                <td>
		                	<input type="text" readonly="readonly" class="middle_txt" id="PART_CNAME" value="${ps.PART_CNAME }"/>
		                </td>
		                
		                <td class="right">配件件号：</td>
		                <td>
		                	<input type="text" readonly="readonly" class="middle_txt" id="PART_CODE" value="${ps.PART_CODE }"/>
		                </td>
		            </tr>
		            <tr>
