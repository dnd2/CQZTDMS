...
<div class="wbox">
<div class="navigation"><img src="<%=request.getContextPath()%>/img/nav.gif" />&nbsp;当前位置：配件管理 > 基础信息管理 > 仓储相关信息维护 > 配件形态转换
</div>
      ...
      <tr>
		 <td align="center" colspan="6">
		  <input class="normal_btn" type="button" value="查询" name="BtnQuery" id="queryBtn" onClick="__extQuery__(1);"/>
           <input class="normal_btn" type="button" value="查询历史" name="BtnQueryh" id="BtnQueryh" onClick="viewRecord();"/>
		 </td>
	  </tr>
	</table>
...
<script type="text/javascript" >
	...
	var columns = [
					...
					{header: "转换数量", dataIndex: 'ITEM_QTY', style: 'text-align:left',renderer:moveNum},
					{header: "转换编码", dataIndex: 'PART_OLDCODE', style: 'text-align:center',renderer:convertCode},
					{header: "转换货位", dataIndex: 'LOC_CODE', style: 'text-align:left',renderer:choice},
    ...
    function convertCode(value,meta,record){
    	var id = record.data.PART_ID +","+ record.data.LOC_ID;
		var text = "<input class='middle_txt' type='text' name='convertCode_"+id+"' id='convertCode_"+id+"' onchange='checkPart(this,\""+id+"\");'/>";
  ...
  //选择
	...
	function myLink(value,meta,record)
	{
		...
		text = text + "<input name='WH_ID_"+id+"' id='WH_ID_"+id+"' type='hidden' value='"+whId+"'>";
  		//var yiwei = "<input class='normal_btn' type='button' value='[转换]' onClick='jumpConvert(\""+record.data.PART_ID+"\",\""+whId+"\");'/>";
	...
	function checkPart(th,locId){
		var part_oldcode = th.value;
		var url2 = "<%=contextPath%>/parts/baseManager/partsBaseManager/PartFormConvert/checkPartExists.json";
		var para = "PART_OLDCODE="+part_oldcode+"&locId="+locId;
		makeCall(url2,forBack3,para);
	}
	function forBack3(json){
		if(json.FLAG != "1"){
			var partId = json.locId;
			if(partId!=""){document.getElementById("convertCode_" + partId).value = "";}
			parent.MyAlert("配件【"+json.PART_OLDCODE+"】不存在！");
		}else{
            LOC_ID = json.locId;
            setPartCode(json.PART_OLDCODE,json.PART_OLDCODE,json.PART_CODE,json.PART_CNAME);
        }
	}
    ...
    function forBack2(json){
        if(json.returnValue != 1){
            ...
            parent.MyAlert("该货位编码在仓库【"+json.whName+"】中不存在,请先维护再操作！ ");
        }else{
            codeSet(json.LOC_ID,json.LOC_CODE,json.LOC_CODE);
        }
    }
    function jumpConvert(partId,whId){
    	var url2 = g_webAppName + "/parts/baseManager/partsBaseManager/PartFormConvert/jumpConvert.do?partId="+partId+"&whId="+whId;
		...
		if(validate(id)){
			...
			for (var i = 1; i < mt.rows.length; i++) {
		        ...
		        if (pl==id) {
	                ...
	                if(partOldCode.trim() == convertCode.trim()){
                    ...
                    if(confirm("确定要形态转换?")) {
                        ...
                        var paramas = "PART_ID=" + partId + "&LOC_ID=" + locId + "&PART_OLDCODE=" + partOldCode + "&PART_CNAME=" + partCname + "&MOVE_NUM=" + number + "&LOC_CODE=" + loc + "&WH_ID=" + whId + "&convertCode=" + convertCode;
    ...
    function viewRecord(){
        OpenHtmlWindow("<%=contextPath%>/parts/baseManager/partsBaseManager/PartFormConvert/init.do?FLAG="+2,800,500);
    }
</script>
</html>