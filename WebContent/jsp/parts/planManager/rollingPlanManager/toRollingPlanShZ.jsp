...
<div class="wbox">
    <div class="navigation">
    	<img src="<%=contextPath%>/img/nav.gif"/>&nbsp;当前位置：总部采购管理&gt; 采购计划管理 &gt; 计划编制&gt; 修改
    </div>
        ...
        <table class="table_query" >
            ...
            <tr>
                <td class="center" colspan="6">
                	...
                	<input type="button" class="u-button " onclick="addPlan();" value="新增"/>&nbsp;
                	
                	<input type="button" class="u-button" onclick="savePlan()" value="保存"/>&nbsp;
                	<input type="button" class="u-button" onclick="deletePlan()" value="删除"/>&nbsp;
                	<input type="button" class="u-button" onclick="goback();" value="返回"/>&nbsp;
                	<input type="button" class="u-button" onclick="shPass();" value="转订单"/>&nbsp;
                </td>
            </tr>
        </table>
        </div>
    ...
    <script type="text/javascript">
        ...
        function cksall(self){
        	var bl = $(self).prop('checked');
        	$('input[name="ck"]').prop('checked',bl);
        }
        ...
        function setPlanQty(value,meta,record){
        	...
        	var str = '<input type="text" style="width:66px;" id="PLAN_QTY_'+PLINE_ID+'" name="PLAN_QTY_'+PLINE_ID+'" value="'+value+'">';
        ...
        function setRemark(value,meta,record){
        	var PLINE_ID = record.data.PLINE_ID;
        	var str = '<input type="text" id="SALES_REMARK_'+PLINE_ID+'" name="SALES_REMARK_'+PLINE_ID+'" value="'+value+'">';
        	return str;
        }
        //返回
        function goback(){
        	
        		location = '<%=contextPath%>/parts/planManager/PartPlanManager/toPartPlanValidationGysZ.do';
        ...
        function savePlan(){
        	...
        	if(len>0){
        		...
        		for(var i=0;i<lens;i++){
        			var bl = $('input[name="ck"]').eq(i).prop('checked');
        			if(bl==true){
        				//判断数字，最小包装量，大于0
        				var lineId = $('input[name="ck"]').eq(i).val();
        				var PLAN_QTY = $('#PLAN_QTY_'+lineId).val();
        				var MIN_PKG = $('#MIN_PKG_'+lineId).val();
        				if(isNaN(PLAN_QTY)){
        	...
        	MyConfirm('确认保存?',function(){
        		var urlkey = "<%=contextPath%>/parts/planManager/PartPlanManager/savePlanByPlineId.json";
       			sendAjax(urlkey, getResultSave, "fm");
        	})
        }
        ...
        function deletePlan(){
        	...
        	MyConfirm('确认删除?',function(){
	       			...
	       			sendAjax(urlkey, getResultDelete, "fm");
        		})
        	}else{
        		MyAlert('请选择要删除的数据!');
        		return;
        ...
        function shPass(){
        		...
        		MyConfirm('确认审核通过计划?',function(){
	        		var urlkey = "<%=contextPath%>/parts/planManager/PartPlanManager/shPlanByPlineId.json";
	       			sendAjax(urlkey, getResultSh, "fm");
        		})
