...
function myLink(value, meta, record) {
    ...
    var str = "";
    if ('<%=Constant.PART_MV_STATUS_02 %>' == state) {
        str = "<a href=\"#\" onclick='viewOrder(\"" + changeId + "\")'>[查看]</a>&nbsp;<a href=\"#\" onclick='checkOrder(\"" + changeId + "\")'>[审核]</a>&nbsp;<a href=\"#\" onclick='rejectOrder(\"" + changeId + "\")'>[驳回]</a>";
    }
    else {
...
//驳回
function rejectOrder(value) {
    MyConfirm("确定驳回?", rejectOrderAction, [value]);
}

function rejectOrderAction(value) {
    btnDisable();
    var url = "<%=contextPath%>/parts/storageManager/partDlrStockMvManager/partDlrStockMvAction/rejectMVorder.json?chgId=" + value + "&curPage=" + myPage.page;
...
function viewOrder(parms) {
    var optionType = "view";
    OpenHtmlWindow("<%=contextPath%>/parts/storageManager/partDlrStockMvManager/partDlrStockMvAction/viewMVDtlInint.do?chgId=" + parms + "&optionType=" + optionType, 800, 500);
}

//审核
function checkOrder(value) {
    var optionType = "check";
	...
	<div class="wbox">
		<form method="post" name="fm" id="fm">
			<div class="navigation">
				<img src="<%=request.getContextPath()%>/img/nav.gif" /> &nbsp;当前位置： 配件管理 &gt; &gt;配件仓储管理 &gt; 配件调拨审核
				...
				<input type="hidden" name="orderState" id="orderState" value="<%=Constant.PART_MV_STATUS_02%>" />
			</div>
