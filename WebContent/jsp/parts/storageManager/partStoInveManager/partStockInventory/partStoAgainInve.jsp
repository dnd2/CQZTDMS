...
<body>
  <form method="post" name ="fm" id="fm">
	<div class="wbox">
		<div class="navigation"><img src="<%=request.getContextPath()%>/img/nav.gif" />
		  &nbsp;当前位置： 配件仓库管理&gt;库存盘点管理&gt;库存盘点&gt;库存复盘打印
		  <input type="hidden" name="changeId" id="changeId" value="${changeId }"/>
		  <input type="hidden" name="invType" id="invType" value="${invType }"/>
		  <input type="hidden" name="optType" id="optType" value="againPrint"/>
		</div>
		<table class="table_query">
		  <th style="text-align: center;">配件复盘信息</th>
		  <tr>
		   <td align="center">
  ...
  <script type="text/javascript" >
	...
	var columns = [
				...
				{header: "封存库存", dataIndex: 'FC_QTY', align:'center'},
				{header: "备注", dataIndex: 'REMARK', align:'center'},
				{header: "盘点库存", dataIndex: 'PART_ID', align:'center',renderer:returnText}
		      ];

	function returnText(value,meta,record)
	{
		return String.format("<input type='hidden' name='inveNum_"+value+"' id='inveNum_"+value+"' value='' />");
	}
	...
	function exportPartStockExcel(){
		document.fm.action="<%=contextPath%>/parts/storageManager/partStoInveManager/stockInventoryAction/expAgainInvExcel.do";
		document.fm.target="_self";
		document.fm.submit();
	}
