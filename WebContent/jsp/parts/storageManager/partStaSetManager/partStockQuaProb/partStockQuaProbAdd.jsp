...
jQuery.noConflict();
 ...
 var url = "<%=contextPath%>/parts/storageManager/partStaSetManager/partStockQuaProbAction/showPartStockBase.json";
...
function addCell(partId,partCode, partOldcode, partCname,itemQty,normalQty,bookedQty,fcQty,unit){
		...
		var stockValue = <%=Constant.PART_STOCK_STATUS_BUSINESS_TYPE_05%>;
		var stockName = "质量问题";
...
//		var cgType = genSelBoxExpStr("cgType_"+ partId,<%=Constant.PART_STOCK_STATUS_CHANGE_TYPE%>,<%=Constant.PART_STOCK_STATUS_CHANGE_TYPE_01%>,true,"short_sel","","false","").toString();
	...
	function uploadExcel(){
		btnDisable();
		fm.action = "<%=contextPath%>/parts/storageManager/partStaSetManager/partStockQuaProbAction/partStockQuaProbUpload.do";
		fm.submit();
	}
	...
	function savePlan(){
		btnDisable();
		var url = "<%=contextPath%>/parts/storageManager/partStaSetManager/partStockQuaProbAction/saveStockInfos.json";	
	...
	function getResult(json){
		...
		if(null != json){
	        if (json.errorExist != null && json.errorExist.length > 0) {
	        	...
	        	btnDisable();
	        	MyAlert("保存成功!");
	            window.location.href = "<%=contextPath%>/parts/storageManager/partStaSetManager/partStockQuaProbAction/partStockQuaProbInit.do";
	        } else {
	            MyAlert("保存失败，请联系管理员!");
	        }
		}
	...
	function exportExcelTemplate(){
		fm.action = "<%=contextPath%>/parts/storageManager/partStaSetManager/partStockQuaProbAction/exportExcelTemplate.do";
		fm.submit();
	}
	...
	function goBack(){
		btnDisable();
		fm.action = "<%=contextPath%>/parts/storageManager/partStaSetManager/partStockQuaProbAction/partStockQuaProbInit.do";
		fm.submit();
	}
...
<div class="wbox">
  <div class="navigation"><img src="<%=contextPath%>/img/nav.gif" />&nbsp;当前位置：配件仓库管理&gt;库存状态变更&gt;配件质量问题处理&gt;新增</div>
  <div >
						...
						<c:if test="${list !=null}">
						  <c:forEach items="${list}" var="list" varStatus="_sequenceNum">
							...
							<tr class="table_list_row2">
							  ...
							  <td align="center" nowrap>
							    <input   name="bzType_${list.partId}" id="bzType_${list.partId}" value="<%=Constant.PART_STOCK_STATUS_BUSINESS_TYPE_05%>" type="hidden" />质量问题 
							  </td>
							  <td align="center" nowrap>
