...
jQuery.noConflict();
 ...
 var url = "<%=contextPath%>/parts/storageManager/partStaSetManager/partStockIOUProbAction/showPartStockBase.json";
...
function addCell(partId,partCode, partOldcode, partCname,itemQty,normalQty,bookedQty,fcQty,unit){
		...
		var stockValue = <%=Constant.PART_STOCK_STATUS_BUSINESS_TYPE_06%>;
		var stockName = "借条";
//		var bzType = genSelBoxExpStr("bzType_"+ partId,<%=Constant.PART_STOCK_STATUS_BUSINESS_TYPE%>,"",true,"short_sel","","false","").toString();
	...
	function uploadExcel(){
		btnDisable();
		fm.action = "<%=contextPath%>/parts/storageManager/partStaSetManager/partStockIOUProbAction/partStockIOUProbUpload.do";
		fm.submit();;
	}
	...
	function savePlan(){
		btnDisable();
		var url = "<%=contextPath%>/parts/storageManager/partStaSetManager/partStockIOUProbAction/saveStockInfos.json";	
	...
	function getResult(json){
		...
		if(null != json){
	        if (json.errorExist != null && json.errorExist.length > 0) {
	        	...
	        	btnDisable();
	        	MyAlert("保存成功!");
	            window.location.href = "<%=contextPath%>/parts/storageManager/partStaSetManager/partStockIOUProbAction/partStockIOUProbInit.do";
	        } else {
	            MyAlert("保存失败，请联系管理员!");
	        }
		}
	...
	function exportExcelTemplate(){
		fm.action = "<%=contextPath%>/parts/storageManager/partStaSetManager/partStockIOUProbAction/exportExcelTemplate.do";
		fm.submit();
	}
...
<div class="wbox">
  <div class="navigation"><img src="<%=contextPath%>/img/nav.gif" />&nbsp;当前位置：配件仓库管理&gt;配件状态变更&gt;配件借条处理&gt;新增</div>
  <div >
						...
						<c:if test="${list !=null}">
						  <c:forEach items="${list}" var="list" varStatus="_sequenceNum">
							...
							<tr class="table_list_row2">
							  ...
							  <td align="center" nowrap>
							    <input   name="bzType_${list.partId}" id="bzType_${list.partId}" value="<%=Constant.PART_STOCK_STATUS_BUSINESS_TYPE_06%>" type="hidden" />借条
							  </td>
							  <td align="center" nowrap>
