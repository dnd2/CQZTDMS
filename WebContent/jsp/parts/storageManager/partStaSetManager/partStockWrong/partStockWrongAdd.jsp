...
var url = "<%=contextPath%>/parts/storageManager/partStaSetManager/partStockWrongAction/showPartStockBase.json";
...
function addCell(partId, partCode, partOldcode, partCname, itemQty, normalQty, bookedQty, fcQty, unit) {
    ...
    var stockValue = <%=Constant.PART_STOCK_STATUS_BUSINESS_TYPE_04%>;
    var stockName = "来货错误";
...
function uploadExcel() {
    btnDisable();
    fm.action = "<%=contextPath%>/parts/storageManager/partStaSetManager/partStockWrongAction/partStockWrongUpload.do";
    fm.submit();
}
...
function savePlan() {
    btnDisable();
    var url = "<%=contextPath%>/parts/storageManager/partStaSetManager/partStockWrongAction/saveStockInfos.json";
...
function getResult(json) {
    ...
    if (null != json) {
        if (json.errorExist != null && json.errorExist.length > 0) {
            ...
            btnDisable();
            MyAlert("保存成功!");
            window.location.href = "<%=contextPath%>/parts/storageManager/partStaSetManager/partStockWrongAction/partStockWrongInit.do";
        } else {
            MyAlert("保存失败，请联系管理员!");
        }
    }
...
function exportExcelTemplate() {
    fm.action = "<%=contextPath%>/parts/storageManager/partStaSetManager/partStockWrongAction/exportExcelTemplate.do";
    fm.submit();
}
...
<div class="wbox">
<div class="navigation"><img src="<%=contextPath%>/img/nav.gif"/>&nbsp;当前位置：配件仓库管理&gt;配件状态变更&gt;配件来货错误处理&gt;新增</div>
<div>
    ...
    <c:if test="${list !=null}">
        <c:forEach items="${list}" var="list" varStatus="_sequenceNum">
            ...
            <td align="center" nowrap>
                <input type="checkbox" value="${list.partId}" id="cell_${_sequenceNum.index+1}" name="cb"
                       checked="checked"/>
            </td>
            ...
            <td align="center">
                <input name="partOldcode_${list.partId}" id="partOldcode_${list.partId}" value="${list.partOldcode}"
                       type="hidden"/>${list.partOldcode}
            </td>
            <td align="center" nowrap>
                <input name="partCname_${list.partId}" id="partCname_${list.partId}" value="${list.partCname}"
                       type="hidden" class="cname_${list.partId}"/>${list.partCname}
            </td>
            <td align="center" nowrap>
                <input name="partCode_${list.partId}" id="partCode_${list.partId}" value="${list.partCode}"
                       type="hidden"/>${list.partCode}
            </td>
            <td align="center" nowrap>
                <input name="unit_${list.partId}" id="unit_${list.partId}" value="${list.unit}"
                       type="hidden"/>${list.unit}
            </td>
            <td align="center" nowrap>
                <input name="normalQty_${list.partId}" id="normalQty_${list.partId}" value="${list.normalQty}"
                       type="hidden"/>${list.normalQty}
            </td>
            <td align="center" nowrap>
                <input name="fcQty_${list.partId}" id="fcQty_${list.partId}" value="${list.fcQty}"
                       type="hidden"/>${list.fcQty}
            </td>
            <td align="center" nowrap>
                <input name="bzType_${list.partId}" id="bzType_${list.partId}"
                       value="<%=Constant.PART_STOCK_STATUS_BUSINESS_TYPE_04%>" type="hidden"/>来货错误
            </td>
            <td align="center" nowrap>
                <input type="hidden" name="cgType_${list.partId}" id="cgType_${list.partId}"
                       value="<%=Constant.PART_STOCK_STATUS_CHANGE_TYPE_01%>"/>封存
            </td>
            <td align="center" nowrap>
                <input class="short_txt" name="returnQty_${list.partId}" id="returnQty_${list.partId}"
                       value="${list.returnQty}" type="hidden"/>${list.returnQty}
            </td>
            <td align="center" nowrap>
                <input class="short_txt" name="remark_${list.partId}" id="remark_${list.partId}" value="${list.remark}"
                       type="hidden"/>${list.remark}
            </td>
            <td>
                <input type="button" class="short_btn" name="deleteBtn" value="删 除"
                       onclick="deleteTblRow('${_sequenceNum.index+2}');"/>
            </td>
            </tr>
        </c:forEach>
    </c:if>
