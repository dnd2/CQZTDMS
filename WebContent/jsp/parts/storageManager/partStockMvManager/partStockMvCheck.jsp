...
function checkAction() {
    btnDisable();
    var url = "<%=contextPath%>/parts/storageManager/partStockMvManager/partStockMvAction/checkMvOrder.json?checkFlag=1";
    ...
    if (null != json) {
        if (json.error != null) {
            ...
            MyAlert("操作成功！", function(){
	            window.location.href = '<%=contextPath%>/parts/storageManager/partStockMvManager/partStockMvAction/checkMvInit.do';
            });
        } else {
...
function rejectOrderAction() {
    btnDisable();
    var url = "<%=contextPath%>/parts/storageManager/partStockMvManager/partStockMvAction/checkMvOrder.json?checkFlag=2";
    sendAjax(url, getResult, 'fm');
}
				...
				<c:if test="${list !=null}">
					<c:forEach items="${list}" var="list" varStatus="_seq">
						<tr class="table_list_row1">
							<td align="center" nowrap>${_seq.index+1}<input type="hidden" value="${list.PART_IDS}${_seq.index+1}" name="cb" checked="checked" /></td>
							<td align="center">
							...
							<td align="center" nowrap>
								<input class="short_txt" name="CHECK_QTY${list.PART_IDS}${_seq.index+1}" id="CHECK_QTY${list.PART_IDS}${_seq.index+1}" value="${list.APPLY_QTY}" type="text" style="background-color: #FF9; text-align: center" />
							</td>
