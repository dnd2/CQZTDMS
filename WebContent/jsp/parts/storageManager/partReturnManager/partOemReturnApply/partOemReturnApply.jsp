...
function insertNormalQtyInput(value, meta, record) {
    var inId = record.data.IN_ID;
    var output = '<input type="hidden" id="NORAML_QTYH' + inId + '" value="' + value + '"/>\n'+value;
    return output;
}
...
function check(obj, inId, h) {
	...
	var normalQty = $('#NORAML_QTY'+h+inId).val();
    if(normalQty < count){
    	obj.value = "";
...
function addCell(inId, partCode, partOldcode, partCname, locCode, batchNo, inQty, normalQty, applyQty1, applyQty, returnQty) {
    ...
    cell1.innerHTML = '<tr><td align="center" nowrap><input  type="checkbox" value="' + inId + '" id="cb' + (tbl.rows.length - 1) + '" name="cb" checked="true" onclick="chkPart1();"/></td>';
    ...
    cell12.innerHTML = '<td align="center" nowrap><input type="hidden" id="NORMAL_QTY' + inId + '" name="NORAML_QTY' + inId + '" value="' + normalQty + '"/>' + normalQty + '</td>';
...
//         	if(parseInt(returnQty) + parseInt(applyQty) + parseInt(obj.value) > parseInt(inQty)){
    ...
    MyConfirm("确定"+msg+"?", function(){
        btnDisable();
        var url = '<%=contextPath%>/parts/storageManager/partReturnManager/PartOemReturnApplyManager/submitReturnApply.json?type='+type;
...
<body>
	<div class="wbox">
		...
		<form name="fm" id="fm" method="post" enctype="multipart/form-data">
			...
			<input type="hidden" name="selCheckCode" id="selCheckCode" value="" />
				...
				<div class="form-body">
					<table class="table_query" width=100% border="0" align="center" cellpadding="1" cellspacing="1">
						...
						<tr>
							...
							<td>
								<input id="IN_CODE" name="IN_CODE" type="text" class="middle_txt" readonly="readonly" />
						...
						<tr>
							...
							<td colspan="3">
								<textarea class="form-control" name="reason" id="reason" style="width: 80%; display: inline;" rows="4" datatype="0,is_null,20000"></textarea>
							</td>
						</tr>
					</table>
				</div>
			...
			<table style="width: 100%;">
				<tr>
					<td align="center">
						<input type="button" name="saveBtn" id="saveBtn" value="保存" onclick="fmsubmit(1, '保存');" class="u-button" />
						<input type="button" name="saveBtn" id="saveBtn" value="提交" onclick="fmsubmit(2, '提交');" class="u-button" />
				...
				<div style="display: none;" id="partDiv">
					<table id="headTable" class="table_query" width=100% border="0" align="center" cellpadding="1" cellspacing="1">
						<tr>
							...
							<td>
								<script type="text/javascript">
									genSelBox("PRODUCE_STATE", <%=Constant.PART_PRODUCE_STATE%>, "", true, "u-select", "", "false");
								</script>
							</td>
						</tr>
						<tr>
