...
<head>
    ...
    <title>月度目标查询(终端)</title>
</head>
<body onload="dtlQuery();">
<form name="fm" method="post" enctype="multipart/form-data" id="fm">
    <div class="navigation"><img src="<%=request.getContextPath()%>/img/nav.gif">当前位置&gt;配件管理&gt;服务商月度目标&gt;月度目标完成(终端)</div>
    <table class="table_query">
        <input type="hidden" id="queryType" name="queryType" value="0"/>
        <th colspan="8"><img class="nav" src="<%=request.getContextPath()%>/img/subNav.gif"/> 查询条件</th>
        <tr>
            ...
            <td width="30%">
                ...
                <input type="button" class="normal_btn" onclick="clrTxt('dealerCode');" value="清 空" id="clrBtn"/>
            </td>
            <td width="10%" align="right">选择年：</td>
            <td width="15%"><select id="year" onchange="getPlanVer();" name="year">
                <%
            ...
            <td width="10%" align="right">选择月：</td>
            <td width="20%">
    ...
    <div style="width: 100%; height: 30px; float: left; " id="countDiv">
     <div style="width: 50%; height: 30px; line-height: 30px; float: left; text-align: right; background-color: #DAE0EE;">
       <font style="font-weight: bolder;">总&nbsp;&nbsp;计：&nbsp;&nbsp;&nbsp;&nbsp;</font>
     </div>
     <div style="width: 7%; height: 30px; line-height: 30px; float: left; text-align: center; background-color: #DAE0EE;">
     ...
     <div style="width: 7%; height: 30px; line-height: 30px; float: left; text-align: center; background-color: #DAE0EE;">
       <font id="bbsaleTotal"></font>
     </div>
        <div style="width: 7%; height: 30px; line-height: 30px; float: left; text-align: center; background-color: #DAE0EE;">
            <font id="bbrate"></font>
        </div>
        <div style="width: 7%; height: 30px; line-height: 30px; float: left; text-align: center; background-color: #DAE0EE;">
            <font id="gysaleTotal"></font>
        </div>
        <div style="width: 7%; height: 30px; line-height: 30px; float: left; text-align: center; background-color: #DAE0EE;">
            <font id="gyrate"></font>
        </div>
     <div style="width: 8%; height: 30px; line-height: 30px; float: left; text-align: center; background-color: #DAE0EE;">
...
<script type="text/javascript">
    ...
    function dtlQuery() {
        ...
        url = "<%=contextPath%>/parts/monthPlan/MonthPlanManager/planQueryZz.json";
        if (queryType == '0' || month == '') {
        	...
        	if(type == '0'){
	            columns = [
	                ...
	                {header: "服务商名称", dataIndex: 'DEALER_NAME', align: 'center', style: 'width: 30%;',style: 'text-align:left'},
	                {header: "任务金额", dataIndex: 'AMOUNT', align: 'center', style: 'width: 15%;'},
	                {header: "购买金额", dataIndex: 'SELLMONEY', align: 'center', style: 'width: 15%;'},
	                {header: "本部购买", dataIndex: 'BB_AMOUNT', align: 'center', style: 'width: 15%;'},
	                {header: "本部占比", dataIndex: 'BB_RATIO', align: 'center', style: 'width: 10%;'},
	                {header: "中心购买", dataIndex: 'GY_AMOUNT', align: 'center', style: 'width: 15%;'},
	                {header: "中心占比", dataIndex: 'GY_RATIO', align: 'center', style: 'width: 10%;'},
    ...
    function collQuery() {
    	url = "<%=contextPath%>/parts/monthPlan/MonthPlanManager/planCountQueryZz.json";
    	var year = document.getElementById("year").value;
    	var month = document.getElementById("month").value;
    	var month2 = document.getElementById("month2").value;
    	var countDivObj = document.getElementById("countDiv");
    ...
    function normalAmountQuery(){
    	var url = "<%=contextPath%>/parts/monthPlan/MonthPlanManager/normalCountQueryZz.json";	
		sendAjax(url,countResult,'fm');
		
    }

    function countAmountQuery(){
    	var url = "<%=contextPath%>/parts/monthPlan/MonthPlanManager/countAmountQueryZz.json";	
		sendAjax(url,countResult,'fm');
    }
    ...
    function countResult(json){
		if(null != json){
            ...
            document.getElementById("bbsaleTotal").innerText = addKannma(parseFloat(json.bbsaleTotal).toFixed(2));
            document.getElementById("gysaleTotal").innerText = addKannma(parseFloat(json.gysaleTotal).toFixed(2));
            document.getElementById("saleRate").innerText = parseFloat(json.rate).toFixed(2) + "%";
            document.getElementById("bbrate").innerText = parseFloat(json.bbrate).toFixed(2) + "%";
            document.getElementById("gyrate").innerText = parseFloat(json.gyrate).toFixed(2) + "%";
		}
	}
  ...
  //千分格式
    ...
    function downloadFile() {
        fm.action = '<%=contextPath%>/parts/monthPlan/MonthPlanManager/downloadDataZz.do';
        fm.submit();
    }
    function downloadFile1() {
        fm.action = '<%=contextPath%>/parts/monthPlan/MonthPlanManager/downloadDataZz1.do';
        fm.submit();
    }
</script>
</body>
