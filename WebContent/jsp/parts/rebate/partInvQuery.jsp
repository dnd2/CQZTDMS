...
<form name="fm" id="fm" method="post" enctype="multipart/form-data">
    <div class="navigation"><img src="<%=request.getContextPath()%>/img/nav.gif">当前位置&gt财务管理&gt;返利关联&gt;配件收入统计表
    </div>
        ...
        <tr>
            ...
            <td>
                <script type="text/javascript">
                    genSelBoxExp("ACCOUNT_TYPE", <%=Constant.PART_ACCOUNT_PURPOSE_TYPE%>, "<%=Constant.PART_ACCOUNT_PURPOSE_TYPE_01%>", true, "", "", "false", '');
                </script>
            </td>
        </tr>
        <tr align="center">
...
<script type="text/javascript">
    ...
    //初始化查询TABLE
    var myPage;
    var url = "<%=contextPath%>/parts/financeManager/dealerRateManager/RebateManager/invQuery.json";
    var title = null;
    var columns = [];
    function callBack(json) {
        ...
        if (ps.records != null) {
            columns = [
                ...
                {header: "合计金额", dataIndex: 'WD_AMOUNT', align: 'center', style: 'text-align:right'}
            ];
            ...
            tmpRow.insertCell(3).innerHTML = "<TD>合计</TD>";
            tmpRow.insertCell(4).innerHTML = "<TD style='TEXT-ALIGN: right'>" + json.SWD_AMOUNT + "</TD>";
            for (var i = 0; i <= yfkeys.length; i++) {
                tmpRow.insertCell(4 + (i+1)).innerHTML = "<TD style='TEXT-ALIGN: right'>" + eval('json.SYFM' + yfkeys[i].substr(3)) + "</TD>";
            }
    ...
    //下载
    function expExcel() {
        fm.action = "<%=contextPath%>/parts/financeManager/dealerRateManager/RebateManager/expInvExcel.do";
        fm.target = "_self";
        fm.submit();
    }
