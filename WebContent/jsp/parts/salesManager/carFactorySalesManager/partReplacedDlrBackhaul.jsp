...
<head>
    <script type="text/javascript">
        ...
        function disabledBtn() {
            var queryBtn = document.getElementsByName("queryBtn");
            if (queryBtn.length == 1) {
                document.getElementsByName("queryBtn")[0].disabled = true;
                ;
            }
        }
        //生成CELL
        ...
        function createSalesOrderConfirm() {
            ...
            for (var i = 0; i < cb.length; i++) {
                selFlag = true;
                var remark = $("remark_" + cb[i].value).value;//行备注
                var itemQty = $("item_qty_" + cb[i].value).value;//当前库存
                var itemQty_new = $("item_qty_new_" + cb[i].value).value;//新件当前库存
                var wrQty = $("WR_QTY_" + cb[i].value).value;//售后已使用数量
                var maxQty = $("DEF_QTY_" + cb[i].value).value;//允许最大回运数量
                var backQty = $("BACKHAUL_QTY_" + cb[i].value).value;//老编码回运数量
                var backQty_new = $("BACKHAUL_QTY_NEW_" + cb[i].value).value;//新编码回运数量
                if (remark == "") {
                    msg += "请填写第" + (i + 1) + "行的备注!</br>";
                }
                //需要校验销售量是否填写
                if (backQty == "") {
                    msg += "请填写第" + (i + 1) + "行的旧编码回运数量!</br>";
                } else {
                    if (!check(backQty_new)) {
                        msg += "第" + (i + 1) + "行的【新编码回运数量】必须为整数!</br>";
                        MyAlert(msg);
                        return;
                    }
                    if (!check(backQty)) {
                        msg += "第" + (i + 1) + "行的【旧编码回运数量】必须为整数!</br>";
                        MyAlert(msg);
                        return;
                    }

                    if ((parseFloat(backQty) + parseFloat(backQty_new)) > parseFloat(maxQty)) {
                        msg += "第" + (i + 1) + "行的【新旧编码总回运数量】不能大于最大回运数量</br>";
                    }
                    if (parseFloat(backQty) > parseFloat(itemQty)) {
                        msg += "第" + (i + 1) + "行的【旧编码回运数量】不能大于当前库存数量</br>";
                    }
                    if (parseFloat(backQty_new) > parseFloat(itemQty_new)) {
                        msg += "第" + (i + 1) + "行的【新编码回运数量】不能大于当前库存数量</br>";
                    }
            ...
            MyConfirm("确定生成回运单?", createSaleOrder, [uncheckedId]);
            enableCb();
        }
        ...
        function createSaleOrder(uncheckedId) {
            ...
            disableAllBtn();
            var url = "<%=contextPath%>/parts/salesManager/carFactorySalesManager/PartReplacedDlrOrder/createBack.json?uncheckedId=" + uncheckedId;
            sendAjax(url, getResult, 'fm');
        }
        function getResult(jsonObj) {
            ...
            if (jsonObj != null) {
                ...
                if (success) {
                    ...
                    MyAlert(success);
                    window.location.href = '<%=contextPath%>/parts/salesManager/carFactorySalesManager/PartReplacedDlrOrder/partReplacedDlrBackhaulInit.do?flag=true';
                } else if (error) {
                    MyAlert(error);
                } else if (exceptions) {
                    MyAlert(exceptions.message);
        ...
        function initAccount() {
            if (${mainMap.accountFlag}) {
                ...
                var str1 = '<td>账户余额:</td>';
                var str2 = '<td><input id="accountSum" style="border:0px;background-color:#F3F4F8;"  name="accountSum" type="hidden" class="normal_txt" value="${mainMap.accountSumNow}" readonly /></td>';
                var str3 = '<td>可用金额:</td>';
                var str4 = '<td><input id="accountKy" style="border:0px;background-color:#6F9;"  name="accountKy" type="hidden" value="${mainMap.accountKyNow}" class="normal_txt" readonly /></td>';
                var str5 = '<td>冻结金额:</td>';
                var str6 = '<td><input id="accountDj"  name="accountDj" type="hidden" style="border:0px;background-color:#F3F4F8;" value="${mainMap.accountDjNow}" class="normal_txt" readonly /></td>';
                td1.innerHTML = str1;
                td2.innerHTML = str2;
                td3.innerHTML = str3;
                td4.innerHTML = str4;
        ...
        function doInit() {
            ...
            document.getElementById("transpayType").value =
            ${mainMap.IS_TRANSFREE} ==
            10041001 ? 92381001 : 92381002;
            if (${flag==2}) {
...
<form name="fm" id="fm" method="post" enctype="multipart/form-data">
    ...
    <input
        ...
        value="${mainMap.FREIGHT2}"/> <input name="soId" id="soId"
                                             value="${mainMap.SO_ID}" type="hidden"/>

    <div class="wbox">
        <div class="navigation">
            <img src="<%=contextPath%>/img/nav.gif"/>&nbsp;当前位置: 配件管理>配件采购管理&gt;切换订单回运&gt;生成回运单
        </div>
        <FIELDSET>
            <table class="table_add" border="0"
                ...
                <tr>
                    ...
                    <td width="20%" align="left"><input class="middle_txt" type="text" value="${WULIU_CODE}"
                                                        id="WULIU_CODE"
                                                        name="WULIU_CODE"/>
                        <font style="color: red">*</font>
                    </td>
                    <td width="10%" align="right">物流公司：</td>
                    <td width="20%" align="left"><input class="middle_txt" type="text" value="${WULIU_COMPANY}"
                                                        id="WULIU_COMPANY" name="WULIU_COMPANY"/>
                        <font style="color: red">*</font>
                    </td>
                    <td width="10%" align="right">服务商库房：</td>
                ...
                <tr>
                    ...
                    <td colspan="5" style="text-align: left;color: red">
                        <ol>
                            <li>此处可以回运库存切换件和装车切换件新件(未使用过）.</li>
                            <li>新老编码相同的只需要填写【老编码回运数量】，不相同的需要分别填写【老编码回运数量】和【新编码回运数量】.</li>
                            <li>默认【老编码回运数量】为本单的库存切换件数量，回运数量可以调整，但不可大于切换总数量(本部发运)-售后已使用数量.</li>
                            <li>服务商在收到切换件号后，请及时入库.</li>
                            <li>回运时回运数量不可大于当前库存.</li>
                            <li>当前回运单只可以回运一次，请慎重操作.</li>
                        </ol>
                    </td>
                </tr>
            </table>
        ...
        <table id="file" class="table_list" border="0"
            ...
            <tr>
                <th colspan="16" align="left">
                    <img class="nav" src="<%=contextPath%>/img/subNav.gif"/>配件信息 <span
            ...
            <tr bgcolor="#FFFFCC">
                ...
                <td>切换件编码</td>
                <td>切换件名称</td>
                <td>备注<font style="color: red">分别备注<br>库存件装车件数量</font></td>
            </tr>
            <c:forEach items="${detailList}" var="data" varStatus="status">
                <c:if test="${data.CHECK_QTY!=0&&data.DEF_BACK_QTY!=0}">
                    <c:if test="${data.ISNEED_FLAG==10041001}">
                        <tr class="table_list_row1">
                            ...
                            <td align="left">${data.PART_CNAME}
                                <input type="hidden"
                                       id="partCname_${data.PART_ID}"
                                       name="partCname_${data.PART_ID}"
                                       value="${data.PART_CNAME}"/>
                            </td>
                            <td align="center">${data.STOCK_QTY}
                                <input type="hidden"
                                       id="item_qty_${data.PART_ID}"
                                       name="item_qty_${data.PART_ID}"
                                       value="${data.STOCK_QTY}"/>
                            </td>
                            <td align="center">${data.REPART_STOCK_QTY}
                                <input type="hidden"
                                       id="item_qty_new_${data.PART_ID}"
                                       name="item_qty_new_${data.PART_ID}"
                                       value="${data.REPART_STOCK_QTY}"/>
                            </td>
                            <td align="center">${data.BUY_QTY}
                            ...
                            <td align="center">${data.CHECK_QTY}
                                <input type="hidden" class="short_txt"
                                       value="${data.CHECK_QTY}"
                                       style="background-color: #FFFFCC; text-align: center"
                            ...
                            <input type="hidden" class="short_txt" value="${data.PART_CODE}"
                                   style="background-color: #FFFFCC; text-align: center"
                                   id="part_code_${data.PART_ID}" name="part_code_${data.PART_ID}"/>
                            <td>${data.DEF_BACK_QTY}
                                <input type="hidden" class="short_txt"
                                       value="${data.DEF_BACK_QTY}"
                                       id="DEF_QTY_${data.PART_ID}"
                                       name="DEF_QTY_${data.PART_ID}"/>
                            </td>
                            <td align="center">${data.WR_QTY}
                                <input type="hidden" class="short_txt"
                                       value="${data.WR_QTY}"
                                       id="WR_QTY_${data.PART_ID}"
                                       name="WR_QTY_${data.PART_ID}"/>
                            </td>
                            <td align="center">${data.BACKHAUL_QTY}
                                <input type="hidden" class="short_txt"
                                       value="${data.BACKHAUL_QTY}"
                                       id="BACKED_QTY_${data.PART_ID}"
                                       name="BACKED_QTY_${data.PART_ID}"/>
                            </td>
                            <td>
                                <input type="text" class="short_txt"
                                       value="${data.CHECK_QTY}"
                                       style="background-color: #FFFFCC; text-align: center;font-size: 15px;color: red;width: 40px"
                            ...
                            <td>
                                <c:if test="${data.PART_ID != data.REPART_ID}">
                                    <input type="text" class="short_txt"
                                           value="0"
                                           style="background-color: #FFFFCC; text-align: center;font-size: 15px;color: red;width: 40px"
                                           id="BACKHAUL_QTY_NEW_${data.PART_ID}"
                                           name="BACKHAUL_QTY_NEW_${data.PART_ID}"/>
                                </c:if>
                                <c:if test="${data.PART_ID == data.REPART_ID}">
                                    <input type="hidden" class="short_txt"
                                           value="0"
                                           style="background-color: #FFFFCC; text-align: center;border: none"
                                           id="BACKHAUL_QTY_NEW_${data.PART_ID}"
                                           name="BACKHAUL_QTY_NEW_${data.PART_ID}" readonly/>
                                </c:if>
                                <input type="hidden" class="short_txt"
                                       value="${data.NBACKED_QTY}"
                                       id="NBACKED_QTY_${data.PART_ID}"
                                       name="NBACKED_QTY_${data.PART_ID}"/>
                            </td>
                            <td>${data.REPART_OLDCODE}
                                <input type="hidden"
                                ...
                                <input type="hidden"
                                       class="short_txt"
                                       value="${data.REPART_CODE}"
                                       id="repart_code_${data.PART_ID}"
                                       name="repart_code_${data.PART_ID}"/>
                                <input type="hidden"
                                       class="short_txt"
                                       value="${data.REPART_ID}"
                                       id="repart_id_${data.REPART_ID}"
                                       name="repart_id_${data.REPART_ID}"/>
                            </td>
                            <td align="left">${data.REPART_NAME}
                                <input type="hidden" class="short_txt"
                                       value="${data.REPART_NAME}" style=""
                            ...
                            <input type="hidden" id="unit_${data.PART_ID}"
                                   name="unit_${data.PART_ID}" value="${data.UNIT}"/>
                            <input type="hidden" id="TRANS_TYPE" name="TRANS_TYPE" value="1"/>
                            <input type="hidden" id="minPackage_${data.PART_ID}"
                                   name="minPackage_${data.PART_ID}" value="${data.MIN_PACKAGE}"/>
                            <input type="hidden" id="buyPrice_${data.PART_ID}"
                                   name="buyPrice_${data.PART_ID}" value="${data.BUY_PRICE}"/>
                            <input type="hidden" id="PAY_TYPE" name="PAY_TYPE"
                                   value="${mainMap.PAY_TYPE}"/>
                            ...
                            <td><input type="text" class="min_txt"
                                       value="${data.REMARK}" style="text-align: left" id="remark_${data.PART_ID}"
        ...
        <table border="0" class="table_query">
            <tr align="center">
                <td>&nbsp; <input class="normal_btn" type="button" value="回运"
                                  onclick="createSalesOrderConfirm();"/> &nbsp;&nbsp; <input
