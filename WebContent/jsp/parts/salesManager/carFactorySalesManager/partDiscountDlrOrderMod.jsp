...
<title>配件促销订单提报-修改</title>
<script type="text/javascript">
var myPage;
var url = "<%=contextPath%>/parts/salesManager/carFactorySalesManager/PartDlrOrder/showDiscountPartBase.json";
var title = null;
var columns = [
...
//返回
function goBack() {
    if ($('#state')[0].value ==<%=Constant.CAR_FACTORY_SALES_MANAGER_ORDER_STATE_02%>) {
        window.location.href = '<%=contextPath%>/parts/salesManager/carFactorySalesManager/PartDlrOrderCheck/partDlrOrderCheckInit.do';
        return;
    }
    window.location.href = '<%=contextPath%>/parts/salesManager/carFactorySalesManager/PartDlrOrder/partDiscountDlrOrderInit.do';
...
function showAcount(accountId, accountSum, accountKy, accountDj) {
    ...
    if (accountId != "") {
        ...
        document.getElementById("accountSumTextTd").innerHTML = '<input id="accountSum" style="border:0px;background-color:#F3F4F8;"  name="accountSum" type="text" class="normal_txt" value="' + accountSum + '" readonly /><font color="blue">元</font>';
        ...
        document.getElementById("accountDjTextTd").innerHTML = '<input id="accountDj"  name="accountDj" type="text" style="border:0px;background-color:#F3F4F8;" value="' + accountDj + '" class="normal_txt" readonly /><font color="blue">元</font>';
...
<div class="wbox">
<div class="navigation"><img src="<%=contextPath%>/img/nav.gif"/>&nbsp;当前位置：配件管理&gt;配件采购管理&gt;配件促销订单提报&gt;修改</div>
...
<table class="table_add" border="0" style="line-height:30px;" cellSpacing=1 cellPadding=1 width="100%">
    <tr>
        ...
        <td>${mainMap.ORDER_CODE}
        	<input type="hidden" style="border:0px;background-color:#F3F4F8;" value="${mainMap.ORDER_ID}" name="orderId"
                   id="orderId"/>
            <input readonly type="hidden" style="border:0px;background-color:#F3F4F8;"
                                        value="${mainMap.ORDER_CODE}" name="orderCode" id="orderCode"/></td>
    </tr>
    <tr>
        <td align="right">订货单位：</td>
        <td>${mainMap.DEALER_NAME}
        	<input readonly class="middle_txt" type="hidden" style="border:0px;background-color:#F3F4F8;"
                   value="${mainMap.DEALER_NAME}" name="dealerName" id="dealerName"/></td>
        <td align="right">订货人：</td>
        <td>${mainMap.BUYER_NAME}
        	<input readonly class="middle_txt" type="hidden" style="border:0px;background-color:#F3F4F8;"
                   value="${mainMap.BUYER_NAME}" name="buyerName" id="buyerName"/></td>
        <td align="right">订货日期：</td>
        <td>${mainMap.CREATE_DATE}
        	<input readonly type="hidden" style="border:0px;background-color:#F3F4F8;" value="${mainMap.CREATE_DATE}"
                   name="now" id="now"/>${mainMap.CREATE_DATE}</td>
    </tr>
    <tr>
        <td align="right">接收单位：</td>
        <td>
            <input name="RCV_ORG" class="middle_txt" id="RCV_ORG" type="text" value="${mainMap.RCV_ORG}" size="20"
                   readonly="readonly"/>
    ...
    <tr>
        ...
        <td colspan="3">
            <input name="ADDR" class="middle_txt" id="ADDR" type="text" size="20" value="${mainMap.ADDR}"
    ...
    <tr>
        ...
        <td><input id="RECEIVER" name="RECEIVER" type="text" value="${mainMap.RECEIVER}" class="middle_txt"/>
            <font color="RED">*</font></td>
        <td align="right"><span class="table_add_3Col_label_6Letter">接收人电话：</span></td>
        <td><input id="TEL" name="TEL" type="text" class="middle_txt" readonly value="${mainMap.TEL}"/>
            <font color="RED">*</font></td>
        <td align="right">邮政编码：</td>
        <td><input id="POST_CODE" name="POST_CODE" type="text" class="middle_txt" value="${mainMap.POST_CODE}"
                   readonly/>
            <font color="RED">*</font></td>
    </tr>
    <tr id="tr1">
        <%-- <td class="table_add_3Col_label_6Letter">到站名称：</td>
         <td><input id="STATION" name="STATION" type="text" class="normal_txt" value="${mainMap.STATION}" readonly />
           <font color="RED">*</font></td>
   --%>
        <td id="accountSumNameTd" class="table_add_3Col_label_6Letter">账户余额：</td>
        <td id="accountSumTextTd"><input id="accountSum" style="border:0px;background-color:#F3F4F8;" name="accountSum"
                                         type="text" class="middle_txt" value="${dataMap.currAcountMap.ACCOUNT_SUM}"
                                         readonly/></td>
        <td id="accountKyNameTd" class="table_add_3Col_label_6Letter">可用金额：</td>
        <td id="accountKyTextTd"><input id="accountKy" style="border:0px;background-color:#6F9;" name="accountKy"
                                        type="text" value="${dataMap.currAcountMap.ACCOUNT_KY}" class="middle_txt"
                                        readonly/><font color="blue">元</font></td>
        <td id="accountDjNameTd" class="table_add_3Col_label_6Letter"><span
                class="table_add_3Col_label_6Letter">冻结金额：</span></td>
        <td id="accountDjTextTd"><input id="accountDj" name="accountDj" type="text"
                                        style="border:0px;background-color:#F3F4F8;"
                                        value="${dataMap.currAcountMap.ACCOUNT_DJ}" class="middle_txt" readonly/><font
                color="blue">元</font></td>
    </tr>
    <tr id="tr2">
        <td id="AmountNameTd" align="right">订货总金额：</td>
        <td id="AmountTextTd"><input id="Amount" name="Amount" type="text" style="border:0px;background-color:#F3F4F8;"
                                     value="${mainMap.ORDER_AMOUNT} " class="middle_txt" readonly/><font
                color="blue">元</font></td>
        <td align="right"><span class="table_add_3Col_label_6Letter">运费：</span></td>
        <td><input id="freight" name="freight" type="text" class="middle_txt" style="background-color: #ffff80"
                   value="${mainMap.FREIGHT}" readonly/></td>
        <td align="right">产地：</td>
        <td>
            <script type="text/javascript">
                genSelBoxExp("produceFac", <%=Constant.YIELDLY%>, "${mainMap.PRODUCE_FAC}", false, "u-select", "onchange='changeProduceFac()'", "false", '');
            </script>
            <font color="RED">*</font>
        </td>
    </tr>
    <tr id="tr3">
        ...
        <td id="DISCOUNTTextTd"><input readonly class="middle_txt" type="text"
                                       style="border:0px;background-color:#F3F4F8;" value="${mainMap.DISCOUNT}"
                                       name="DISCOUNT" id="DISCOUNT"/></td>
        <td align="right"><span class="table_add_3Col_label_6Letter"></span></td>
        <td></td>
        <td align="right"></td>
        <td></td>
    </tr>
...
<caption><img class="panel-icon nav" src="<%=contextPath%>/img/subNav.gif"/>配件信息</caption>
    ...
    <c:forEach items="${detailList}" var="data">
        <tr class="table_list_row1">
            ...
            <td><INPUT class=normal_btn onclick=deleteTblRow(this); value=删除 type=button name=queryBtn>
...
<table border="0" class="table_query">
    <tr >
        <td class="center">
        	<input class="normal_btn" type="button" value="保存" name="button1" onclick="saveOrderConfirm();">
            &nbsp;
