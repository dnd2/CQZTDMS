...
function uploadExcel(code){
    if(confirm("确定上传订单?，点击确定后请耐心等待.........!")){
    	btnDisable();
		var importFileName = $("#uploadFile1")[0].value;
		if(importFileName==""){
		    MyAlert("请选择上传文件!");
	        btnEnable();
			return false;
		...
		fm.action = "<%=contextPath%>/parts/salesManager/carFactorySalesManager/PartDlrOrder/uploadExcel.do?";
		fm.submit();
	}
}
jQuery(
...
<tr>
    <td>
        ...
        <input type="file" name="uploadFile1" id="uploadFile1" style="width: 250px" datatype="0,is_null,2000" value=""/>
        &nbsp;
        <input type="button" id="upbtn" class="normal_btn" value="确定" onclick="uploadExcel('')"/></td>
        <input type="button" id="upbtn" class="normal_btn" value="直接生成订单" style="width:80px" onclick="uploadExcel('save')"/>
   </td>
</tr>
</div>
</form>
