...
<div class="navigation"><img src="<%=request.getContextPath()%>/img/nav.gif" />&nbsp;当前位置： 选择供应中心</div>
<form method="post" name ="fm" id="fm">
	<input id="venderName" name="venderName" type="hidden" value="" />
	<input id="venderId" name="venderId" type="hidden" value="" />
	<input id="venderCode" name="venderCode" type="hidden" value="" />
	<table class="table_edit">
    <tr>
      <td   align="right" nowrap="nowrap">供应中心编码：</td>
      <td class="table_query_3Col_input" nowrap="nowrap">
      	<input class="middle_txt" id="childOrgCode" name="childOrgCode" value="" type="text"/>
      </td>
      <td   align="right" nowrap="nowrap">供应中心名称：</td>
      <td>
...
<script type="text/javascript" >
	...
	var url = "<%=contextPath%>/report/partReport/partSalesReport/PartSalesDtlReport/queryChildOrg.json";
				
	var title = null;

	var columns = [
				{header: "选择", dataIndex: 'CHILDORG_ID', align:'center',renderer:seled},
				{header: "供应中心编码", dataIndex: 'CHILDORG_CODE', align:'center'},
				{header: "供应中心名称", dataIndex: 'CHILDORG_NAME', align:'center'}
		      ];
    ...
    function singleSelect(orgId,orgCode,orgName){
    	 $('venderName').value = orgName;
    	 $('venderId').value = orgId;
    	 $('venderCode').value = orgCode;
    	 _hide();
    }
  //关闭弹出窗口的时候执行该方法
    function doSupp()
	{   
		var childorgName = "<%=venderName%>";
		var childorgId = "<%=venderId%>";
		var childorgCode = "<%=venderCode%>";
		var childorgName1 = document.getElementById("venderName").value;
		var childorgId1 = document.getElementById("venderId").value;
		var childorgCode1 = document.getElementById("venderCode").value;
		if(childorgName && childorgName.length > 0){
			...
			parentDocument.getElementById(childorgName).value = document.getElementById("venderName").value;
			}
		}
		if(childorgId && childorgId.length > 0){
			...
			parentDocument.getElementById(childorgId).value = document.getElementById("venderId").value;
			}
		}
		if(childorgCode && childorgCode.length > 0){
			if(parent.$('inIframe') == null){//修复不在inIframe中时参数赋值问题
				if(top.$(childorgCode)) {
					top.$(childorgCode).value = childorgCode1;
				}
			}else{
			parentDocument.getElementById(childorgCode).value = document.getElementById("venderCode").value;
			}
		}
	}
