...
<title>采购订单提报（常规订单、紧急订单、促销订单）>新增</title>
<script language="javascript" src="<%=request.getContextPath()%>/jsp/parts/salesManager/carFactorySalesManager/waitingTip.js"></script>
...
$(document).ready(function(){
  ...
  <%-- if ($('#ORDER_TYPE')[0].value ==<%=Constant.CAR_FACTORY_SALES_MANAGER_ORDER_TYPE_01%>) {
      $('#TRANS_TYPE')[0].value = 2;
  } --%>
  //初始化默认值
...
//     {header: "车型", dataIndex: 'MODEL_NAME', style: 'text-align: left;'},
    {header: "备注", dataIndex: 'REMARK', style: 'text-align: left;'}
];


function getSaleText(value, meta, record) {
    if (${orderType} != <%=Constant.CAR_FACTORY_SALES_MANAGER_ORDER_TYPE_03%>){
...
function inputText(value, meta, record) {
    ...
    return "<input type='hidden' value='" + value + "' />" + "<input type='hidden' value='" + record.data.IS_SPECIAL + "' />" + info;
}
 ...
 * 配件table每一行
 */
function addCell(partId, partOldcode, partCname, unit, itemQty, minPackage, buyPrice, 
		upOrgStock,isLack, isReplaced, saleQty) {
	var tbl = document.getElementById('file');
 ...
 */
function countAll(){
    if ($("#TRANS_TYPE")[0].value == '2' && $("#ORDER_TYPE")[0].value == "<%=Constant.CAR_FACTORY_SALES_MANAGER_ORDER_TYPE_02%>") {
...
function validateFm() {
	...
	if (jQuery('#ORDER_TYPE').val() ==<%=Constant.CAR_FACTORY_SALES_MANAGER_ORDER_TYPE_03%>) {//特殊订单填写备注
        if (jQuery('#isSpecialOrPlan').val() == "1") {
            if (jQuery("textarea[name='textarea']").val() == "") {
                msg += "请填写备注! (<font color='red'>特殊配件</font>)</br>";
            }
        }
    }
    ...
    for (var i = 0; i < cb.length; i++) {
        if (cb[i].checked) {
            cb[i].disabled = false;
            //校验订货单价
            if (document.getElementById("buyPrice1_" + cb[i].value).value == "" || document.getElementById("buyPrice1_" + cb[i].value).value == '0.00') {
...
function repOrder() {
    ...
    var url = "<%=contextPath%>/parts/salesManager/carFactorySalesManager/PartDlrOrder/saveOrder.json?discount=discount";
    makeNomalFormCall(url, getResult, 'fm');
}
...
<form name="fm" id="fm" method="post" enctype="multipart/form-data">
    ...
    <input type="hidden" value="${dataMap.dealerName}" name="dealerName" id="dealerName"/>
    <input type="hidden" value="${dataMap.buyerName}" name="buyerName" id="buyerName"/>
    <input type="hidden" value="${dataMap.now}" name="now" id="now"/>
    <input type="hidden" value="<%=Constant.CAR_FACTORY_SALES_PAY_TYPE_01%>" name="PAY_TYPE" id="PAY_TYPE"/>
    <input type="hidden" value="<%=Constant.YIELDLY_01%>" name="produceFac" id="produceFac"/>
	<div id="div1" class="wbox">
        <div class="navigation"><img src="<%=contextPath%>/img/nav.gif"/>&nbsp;当前位置：配件管理&gt;配件采购管理&gt;采购订单提报&gt;新增</div>
        <div class="form-panel">
            <h2><img class="panel-icon nav" src="<%=contextPath%>/img/subNav.gif"/>采购订单信息</h2>
            <div class="form-body">
        <table class="table_add" border="0" cellSpacing=1 cellPadding=1 width="100%"
               style="line-height:30px;">
            <tr>
                <td align="right" nowrap style="display: none">配件类型：</td>
                <td style="display: none">
                    <select id="isSpecialOrPlan" name="isSpecialOrPlan"  class="u-select">
            ...
            <tr>
                ...
                <td>${dataMap.SELLER_NAME}
                    <input name="SELLER_NAME" class="middle_txt" style="border: none" id="SELLER_NAME" type="hidden"
                           value="${dataMap.SELLER_NAME}" size="20" readonly="true"/>
                ...
                <td>
                    <script type="text/javascript">
                        genSelBoxExp("PAY_TYPE", <%=Constant.CAR_FACTORY_SALES_PAY_TYPE%>, "<%=Constant.CAR_FACTORY_SALES_PAY_TYPE_01%>", true, "", "disabled", "false", '');
                    </script>
                </td>
                <td align="right">配件：</td>
                <td>
                    <script type="text/javascript">
                        genSelBoxExp("produceFac", <%=Constant.YIELDLY%>, "${dataMap.produceFac}", false, "", "disabled", "false", '');
                    </script>
                </td>
            </tr>
            <tr>
                <td align="right">订货单位：</td>
                <td>${dataMap.dealerName}</td>
                <td align="right">订货人：</td>
                <td>${dataMap.buyerName}</td>
                <td align="right">订货日期：</td>
                <td>${dataMap.now}</td>
            </tr>
            <tr>
                <td align="right">接收单位：</td>
                <td>${defaultValueMap.defaultRcvOrg}
                    <input name="RCV_ORG" class="middle_txt" style="border: none" id="RCV_ORG" type="hidden"
                           value="${defaultValueMap.defaultRcvOrg}" size="20" readonly="true"/>
            ...
            <tr>
                ...
                <td><input id="STATION" name="STATION" type="text" class="middle_txt"
                           value="${defaultValueMap.defaultStation}" readonly value=""/>
            ...
            <tr id="tr2">
                <td id="accountSumNameTd" align="right">
                    <c:if test="${defaultValueMap.flag=='upload'}">  账户余额：</c:if>
                </td>
                <td id="accountSumTextTd">
                    <c:if test="${defaultValueMap.flag=='upload'}">
                        <input id="accountSum" style="border:0px;background-color:#F3F4F8;" name="accountSum"
                               type="text" class="middle_txt" value="${defaultValueMap.accountSum}" readonly/>
                        <font color="blue">元</font>
                    </c:if>
                </td>
                <td id="accountKyNameTd" align="right">
                    <c:if test="${defaultValueMap.flag=='upload'}">可用金额：</c:if>
                </td>
                ...
                <td id="accountDjNameTd" align="right">
                    <c:if test="${defaultValueMap.flag=='upload'}">  冻结金额：</c:if>
                </td>
                <td id="accountDjTextTd">
                    <c:if test="${defaultValueMap.flag=='upload'}">
                        <input id="accountDj" style="border:0px;background-color:#F3F4F8;" name="accountDj" type="text"
                               class="middle_txt" value="${defaultValueMap.accountDj}" readonly="readonly"/>
                        <font color="blue">元</font>
                    </c:if>
                </td>
            ...
            <tr id="tr3">
                ...
                <td valign="middle">
                	<input id="Amount" name="Amount"  readonly="readonly" type="text" value="0.00" class="middle_txt"  
                           style="background-color: #ffff80;border:none"/>
                    <font color="blue">元</font>
                </td>
                <td align="right"><span align="right">订单金额：</span></td>
                <td>
                	<input id="partAmount" name="partAmount" type="text" value="0.00" readonly="readonly"
                           style="background-color: #ffff80;border:none" class="middle_txt"/>
                    <font color="blue">元</font>
                </td>
                <td align="right"><span align="right">运费金额：</span></td>
                <td>
                	<input id="freight" name="freight" type="text" value="0.00" readonly="readonly"
                           style="background-color: #ffff80;border:none" class="middle_txt"/>
                    <font color="blue">元</font>
                </td>
            </tr>
            <tr>
                ...
                <td id="DISCOUNTTextTd">
                	<input  type="text"  name="DISCOUNT" id="DISCOUNT" class="middle_txt" readonly="readonly"
            ...
            <tr>
                ...
                <td colspan="5">
                	<textarea name="textarea" cols="80" rows="4" value="${dataMap.textarea}"></textarea>
                </td>
            </tr>
        </table>
        </div>
        ...
        <table id="file" class="table_list" border="1" cellSpacing=1 cellPadding=1 width="100%">
            ...
            <tr>
                ...
                <th>最小包装量</th>
                <th>订货数量</th>
                <th>订货单价(折扣前)<font color="blue">(单位：元)</font></th>
                <th>订货单价(折扣后)<font color="blue">(单位：元)</font></th>
                <th>订货金额<font color="blue">(单位：元)</font></th>
                <th>上级是否有库存</th>
                <th>备注</th>
				<th>操作</th>
            </tr>
            <c:forEach items="${detailList}" var="data">
                <tr>
                    ...
                    <td>
                        <input type="text" id="buyQty_${data.PART_ID}" name="buyQty_${data.PART_ID}"
                               style="background-color:#FFFFCC;text-align:center" 
                               class="short_txt" value="${data.BUYQTY}"
                    ...
                    <td>
                        <input type="hidden" id="upOrgStock_${data.PART_ID}" name="upOrgStock_${data.PART_ID}"
                               value="${data.UPORGSTOCK}"/>
                        ${data.UPORGSTOCK}
                        <input type='hidden' id='isReplaced_${data.PART_ID}' name='isReplaced_${data.PART_ID}'
                               value='${data.IS_REPLACED}'/>
                    </td>
                    <td>
                        <input type="text" id="remark_${data.PART_ID}" name="remark_${data.PART_ID}" value="${data.REMARK}" class="short_txt" />
                        <input type='hidden' id='isLack_${data.PART_ID}' name='isLack_${data.PART_ID}' value='${data.IS_LACK}'>
                    </td>
                    <td>
                        <input type="button" name="queryBtn" class="short_btn"  value="删除"
                               onclick="deleteTblRow(this);"/>
                        <input type='hidden' id='stockQty_${data.PART_ID}' name='stockQty_${data.PART_ID}' 
                            value='${data.ITEM_QTY}'>
                    </td>
                </tr>
            </c:forEach>
        ...
        <table border="0" class="table_query">
            <tr>
                <td  class="center">
<!--                     <input class="cssbutton" type="button" value="上传文件" name="button1" onclick="showUpload();">  -->
<!--                     &nbsp; -->
                    <input class="normal_btn" type="button" value="保存" name="button1" onclick="partReplaceValidate('save');">
                    &nbsp;
            ...
            <div style="display: none; heigeht: 5px" id="partDiv">
                <table class="table_query" width=100% border="0" align="center"
                    ...
                    <tr>
                        ...
                        <td class="left" >
                            <input type="text" id="PART_OLDCODE" name="PART_OLDCODE" class="middle_txt" 
                                   datatype="1,is_noquotation,30"  onblur="isCloseDealerTreeDiv(event,this,'pan')" />
                        </td>
                        <td class="right" >配件名称：</td>
                        <td class="left" >
                            &nbsp;
                            <input type="text" id="PART_CNAME" name="PART_CNAME"
                                   datatype="1,is_noquotation,30"  class="middle_txt"
                                   onblur="isCloseDealerTreeDiv(event,this,'pan')" />
                        </td>
                    </tr>
                </table>
                <table width="100%">
                    <tr>
                        <td align="center">
                            <input class="normal_btn"  type="button" name="queryBtn"
