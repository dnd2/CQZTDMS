...
<head>
<title>配件紧急订单提报</title>
</head>
<script language="javascript">
    //初始化查询TABLE
    var myPage;
    var url = "<%=contextPath%>/parts/salesManager/carFactorySalesManager/PartDlrOrder/queryPartDlrOrder.json?ORDER_TYPE="+<%=Constant.CAR_FACTORY_SALES_MANAGER_ORDER_TYPE_01%>;
    var title = null;
    var columns = [
    ...
    function modifyOrder(value) {
        disableAllClEl();
        window.location.href = "<%=contextPath%>/parts/salesManager/carFactorySalesManager/PartDlrOrder/modifyDlrOrder.do?modifyFlag=emergency&&orderId=" + value;
    }
    //作废
  ...
  //提交
    ...
    function addOrder() {
        disableAllClEl();
        window.location.href = "<%=contextPath%>/parts/salesManager/carFactorySalesManager/PartDlrOrder/addEmergencyOrder.do?";
...
<form name="fm" id="fm" method="post" enctype="multipart/form-data">
	<input id="ORDER_TYPE" name="ORDER_TYPE" type="hidden" value="<%=Constant.CAR_FACTORY_SALES_MANAGER_ORDER_TYPE_01%>" />
    <div class="wbox">
        <div class="navigation"><img src="<%=contextPath%>/img/nav.gif"/>&nbsp;当前位置： 配件管理 &gt;配件采购管理&gt;配件紧急订单提报</div>
