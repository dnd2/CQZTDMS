...
<head>
    ...
    <script type="text/javascript">
        ...
        function getModel(dealerId, dealerCode, dealerName) {
            ...
            if (optType == "1") {
                $('RCV_ORGID').value = dealerId;
                $('RCV_CODE').value = dealerCode;
                $('RCV_ORG').value = dealerName;
                var url = g_webAppName + "/parts/salesManager/carFactorySalesManager/PartDlrOrder/getAcount.json?CHILDORG_ID=" + dealerId;
                sendAjax(url, getAccountBack, 'fm');
            } else if (optType == "2") {
                var url = g_webAppName + "/parts/salesManager/carFactorySalesManager/PartSoManage/getDefault.json?id=" + dealerId;
                sendAjax(url, getDefaultResult, 'fm');
            }
        }
        function getAccountBack(jsonObj) {
            if (jsonObj != null) {
                var accountId = jsonObj.accountMap.ACCOUNT_ID;
                var accountSum = jsonObj.accountMap.ACCOUNT_SUM;
                var accountDj = jsonObj.accountMap.ACCOUNT_DJ;
                var accountKy = jsonObj.accountMap.ACCOUNT_KY;
                initAccount(accountId, accountSum, accountDj, accountKy);
            }
        }
        function reNumIdx() {
            var tbl = document.getElementById("file");
        ...
        function countQtyAll() {
            var flag = true;
            var qtyCount = parseFloat(0);
            var cb = document.getElementsByName("cb");
            for (var i = 0; i < cb.length; i++) {
                if ("" != cb[i].value && null != document.getElementById("buyQty_" + cb[i].value) && "" != document.getElementById("buyQty_" + cb[i].value).value) {
                    //只有CHECKED的才能计算unFormatNum
                    if (cb[i].checked) {
                        qtyCount = (parseFloat(unFormatNum(qtyCount)) + parseFloat(unFormatNum(document.getElementById("buyQty_" + cb[i].value).value))).toFixed(2);
                    }
                }
                if (!cb[i].checked) {
                    flag = false;
            ...
            document.getElementById("orderCount").value = formatNum(qtyCount);
        }
        //生成CELL中的HTML
        //&quot;
        function createCellsInnerHtml(id, name, onchangeEvent, style, type, value, trHFlag, trEFlag, hiddenHtml) {
            ...
            var inputHtml = "";
            if (type == 'button') {
                inputHtml = '<input  type="button" class="cssbutton"  name="queryBtn" value="删除" onclick="deleteTblRow(this);" />';
            }
            if (type == 'text') {
        ...
        function setType(optType) {
            if (optType == "1") {
                document.getElementById("optType").value = "1";
            } else if (optType == "2") {
                document.getElementById("optType").value = "2";
            }
        }
    </script>
</head>
<body onload="loadcalendar();autoAlertException();doInit();countAll();enableAllBtn();">
<form name="fm" id="fm" method="post" enctype="multipart/form-data">
    <input type="hidden" name="accountId" id="accountId" value="${dataMap.accountId}"/>
    <input type="hidden" name="temp" id="temp"/>
    <input type="hidden" name="sellerTemp" id="sellerTemp"/>
    <input type="hidden" name="seq" id="seq" value="">
    <input type="hidden" name="state" id="state">
    <input type="hidden" name="optType" id="optType" value="0">

    <div class="wbox">
        <div class="navigation"><img src="<%=contextPath%>/img/nav.gif"/>&nbsp;当前位置：配件管理&gt;配件销售管理&gt;配件订单审核&gt;订单新增
        </div>
        <table class="table_add" border="0"
            ...
            <tr>
                ...
                <td width="24%">
                    <select name="ORDER_TYPE" id="ORDER_TYPE"  class="u-select" onchange="chgOrderType(this.value)">
                        <option value="<%=Constant.CAR_FACTORY_SALES_MANAGER_ORDER_TYPE_02%>">常规订单</option>
                        <option value="<%=Constant.CAR_FACTORY_SALES_MANAGER_ORDER_TYPE_01%>">紧急订单</option>
                        <option value="<%=Constant.CAR_FACTORY_SALES_MANAGER_ORDER_TYPE_03%>">特殊订单</option>
                        <%-- <option value="<%=Constant.CAR_FACTORY_SALES_MANAGER_ORDER_TYPE_09%>">领用订单</option>--%>
                ...
                <td>${dataMap.dealerName}
                    <input name="SELLER_NAME" id="SELLER_NAME" type="hidden" size="20" readonly="true"
                           value="${dataMap.dealerName}"/>
                    <input name="SELLER_CODE" class="middle_txt" id="SELLER_CODE" type="hidden" readonly="true"
                           value="${dataMap.dealerCode}"/>
                    <input name="SELLER_ID" class="middle_txt" id="SELLER_ID" type="hidden" value="${dataMap.dealerId}"
                           readonly="true"/>
                </td>
                <td align="right" style="display:none">配件品牌：</td>
                <td style="display:none">
                    <script type="text/javascript">
            ...
            <tr>
                ...
                <td>
                    <input name="dealerName" class="middle_txt" id="dealerName" value="${dataMap.SELLER_NAME}"
                           type="text"
                    ...
                    <input name="button2" type="button" class="mini_btn" value="..."
                           onclick="setType('1');showOrgDealer('dealerCode','dealerId','false','','','','<%=Constant.DEALER_TYPE_DWR%>','dealerName');"/>
                    <!--
            <input name='dlbtn1' id='dlbtn1' class='mini_btn' type='button' value='...' 
             ...
             -->
                ...
                <td><input readonly type="text" style="border:0px;background-color:#F3F4F8;" value="${dataMap.now}"
                           name="now"
                           id="now"/></td>
            </tr>
            <tr>
                ...
                <td>
                    <input name="RCV_ORG" class="middle_txt" id="RCV_ORG" type="text"
                    ...
                    <input name="button2" type="button" class="mini_btn" value="..."
                           onclick="setType('2');showOrgDealer('RCV_CODE','RCV_ORGID','false','','','','<%=Constant.DEALER_TYPE_DWR%>','RCV_ORG');"/>
                    <!--
            <input name='dlbtn2' id='dlbtn2' class='mini_btn' type='button' value='...'
             ...
             -->
                ...
                <td id="brandTd_1" align="right">品牌：</td>
                <td id="brandTd_2">
                    <select name="brand" id="brand" class="short_sel" onchange="changeBreand()">
            ...
            <tr>
                ...
                <td colspan="3">
                    <input name="ADDR" class="maxlong_txt" id="ADDR" type="text" size="20"
            ...
            <tr id="tr3">
                ...
                <td align="right">锁定运费：</td>
                <td><input id="isLock" name="isLock" type="checkbox" disabled>运费金额：<input id="freight" name="freight"
            ...
            <tr>
                <td align="right">订货总数：</td>
                <td>
                    <input type="text" id="orderCount" style="border:none" value="0" readonly="readonly"/>
                </td>
                ...
                <td id="DISCOUNTTextTd">
                    <input readonly type="text" style="border:none" value="${dataMap.discount}" name="DISCOUNT"
                           id="DISCOUNT"/>
                </td>
            </tr>
            <tr>
        ...
        <table id="file" class="table_list" border="1"
            ...
            <c:forEach items="${detailList}" var="data">
                <tr>
                    ...
                    <td>
                        <input type="button" class="cssbutton" name="queryBtn" value="删除"
            ...
            <tr align="center">
                <td>
                    ...
                    <input class="cssbutton" type="button" value="保存" name="button1"
                           onclick="partReplaceValidate('rep');">
                    &nbsp;
