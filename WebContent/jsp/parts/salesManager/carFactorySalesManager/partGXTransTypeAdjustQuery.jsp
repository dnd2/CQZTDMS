...
<title>广宣品发运方式调整</title>

</head>
<body onload="doInit();">
<form name="fm" id="fm" method="post" enctype="multipart/form-data">
    <div class="wbox">
        <div class="navigation"><img src="<%=contextPath %>/img/nav.gif" alt=""/>&nbsp;当前位置： 配件管理 &gt; 本部销售管理  &gt; 广宣品发运方式调整 </div>
        <table border="0" class="table_query">
            ...
            <tr>
               ...
               <td width="24%"><input class="middle_txt" type="text" id="wl_NO" name="wl_NO"/></td>
            </tr>
...
var url = "<%=contextPath%>/parts/salesManager/carFactorySalesManager/PartGxTransTypeAdjust/partGxTransTypeAdjustQuery.json";
...
var columns = [
    ...
    {header: "整车发运单号", dataIndex: 'ASS_NO', align: 'center'},
    {header: "服务商代码", dataIndex: 'DEALER_CODE', align: 'center'},
    {header: "服务商名称", dataIndex: 'DEALER_NAME', style:'text-align:left;'},
    {header: "包装号", dataIndex: 'PKG_NO', align: 'center'},
    {header: "发运方式", dataIndex: 'TRANS_TYPE', align: 'center', renderer: getItemValue},
    {header: "承运物流", dataIndex: 'TRANS_ORG', align: 'center'},
    {header: "物流单号", dataIndex: 'WL_NO', align: 'center'},
    {header: "物流发运时间", dataIndex: 'WL_DATE', align: 'center'},
    {header: "计划生成日期", dataIndex: 'CREATE_DATE', align: 'center'},
    {header: "要求最终发运日期", dataIndex: 'FINAL_DATE', align: 'center'}
...
function modifyTransType(){
	...
	var wl_NO = $("wl_NO").value;
	
	if(!transType||!transportOrg||!wl_NO){
		MyAlert("请选择发运方式和承运物流和填写物流单号!");
		return;
	}
    ...
    if(confirm("确定修改?")){
    	btnDisable();
    	var url = "<%=contextPath%>/parts/salesManager/carFactorySalesManager/PartGxTransTypeAdjust/updatePartGxPlan.json";
...
function detailOrder(value, pkgNo) {
    ...
    OpenHtmlWindow("<%=contextPath%>/parts/salesManager/carFactorySalesManager/PartGxTransTypeAdjust/detailPlanOrderInit.do?planCode=" + value + "&pkgNo=" + pkgNo+"&buttonFalg="+buttonFalg,900,400);
}

function modify(value, pkgNo) {
    disableAllClEl();
    window.location.href = "<%=contextPath%>/parts/salesManager/carFactorySalesManager/PartGxTransTypeAdjust/modifyPlanOrderInit.do?planCode=" + value+"&pkgNo="+pkgNo;
}
...
//导出
function expExcel() {
    fm.action = "<%=contextPath%>/parts/salesManager/carFactorySalesManager/PartGxTransTypeAdjust/expExcel.do";
    fm.target = "_self";
    fm.submit();
}
</script>
