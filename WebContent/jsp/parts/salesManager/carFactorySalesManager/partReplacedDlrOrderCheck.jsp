...
<head>
    ...
    <script type="text/javascript">
        ...
        function createReplacedOrderConfirm() {
            var msg = "";
            var uncheckedId = "";
...
//         if (cb[i].checked) {
                ...
                //需要校验销售量是否填写
                if (document.getElementById("check_Qty_" + cb[i].value).value == "") {
                    msg += "请填写第" + (document.getElementById("check_Qty_" + cb[i].value).parentElement.parentElement.rowIndex - 1) + "行的审核数量!</br>";
                } else {
                    if (!check(document.getElementById("check_Qty_" + cb[i].value).value)) {
                        msg += "第" + (i + 1) + "行的审核数量必须是正整数!</br>";
                        MyAlert(msg);
                        return false;
                    }
                    if (parseFloat(document.getElementById("check_Qty_" + cb[i].value).value) > parseFloat(document.getElementById("buyQty_" + cb[i].value).value) && (!giftFlag)) {
                        msg += "第" + (document.getElementById("check_Qty_" + cb[i].value).parentElement.parentElement.rowIndex - 1) + "行的审核数量不得大于替换数量!</br>";
                    }
                }
                if (parseFloat(document.getElementById("item_qty_" + cb[i].value).value) < parseFloat(document.getElementById("check_Qty_" + cb[i].value).value)) {
                    msg += "第" + (document.getElementById("check_Qty_" + cb[i].value).parentElement.parentElement.rowIndex - 1) + "行配件的库存不足!</br>";
                }
//         } 
//         else {
//             uncheckedId += "," + cb[i].value;
//             document.getElementById("check_Qty_" + cb[i].value).value = 0;
//             cb[i].disabled = true;
//         }
            }
...
//     }
            ...
            MyConfirm("确定生成审核单?", createReplacedOrder, [uncheckedId]);
            enableCb();
        }
        ...
        function createReplacedOrder(uncheckedId) {
            disableAllBtn();
            var url = "<%=contextPath%>/parts/salesManager/carFactorySalesManager/PartReplacedDlrOrder/createOrderCheck.json?uncheckedId=" + uncheckedId;
            sendAjax(url, getResult, 'fm');
        }
        ...
        function doInit() {
            ...
            document.getElementById("state").value = "92161002";
...
<form name="fm" id="fm" method="post" enctype="multipart/form-data">
    ...
    <input
        ...
        value="${mainMap.FREIGHT2}"/> <input name="soId" id="soId"
                                             type="hidden" value="${mainMap.SO_ID}"/>

    <div class="wbox">
        <div class="navigation">
            <img src="<%=contextPath%>/img/nav.gif"/>&nbsp;当前位置:
            配件管理>配件仓储管理>配件替换管理&gt;切换订单审核&gt;审核
        </div>
        <FIELDSET>
            <table class="table_add" border="0"
                ...
                <tr>
                    ...
                    <td><select name="whId" id="whId"
                                class="short_sel" onchange="stockSearch()">
                    ...
                    <td width="10%" align="right">切换类型：</td>
                    <td>
                        <script type="text/javascript">document.write(getItemValue(${mainMap.PART_TYPE}))</script>
                    </td>
                    <td width="10%" align="right">活动编码:</td>
                    <td>${mainMap.ACTIVITY_CODE}</td>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td colspan="5"><textarea name="remark" id="remark" cols="80" rows="4"></textarea></td>
                </tr>
            </table>
        </FIELDSET>
            ...
            <c:forEach items="${detailList}" var="data">
                <tr>
                    ...
                    <td><input type="text" class="short_txt"
                               value="${data.BUY_QTY}"
                               style="background-color: #FFFFCC; text-align: center"
                               id="check_Qty_${data.PART_ID}" name="check_Qty_${data.PART_ID}" readonly/>
                    </td>
                    <input type="hidden" id="partOldcode_${data.PART_ID}"
                           name="partOldcode_${data.PART_ID}" value="${data.PART_OLDCODE}"/>
                    <input type="hidden" id="partCname_${data.PART_ID}"
                           name="partCname_${data.PART_ID}" value="${data.PART_CNAME}"/>
                    <input type="hidden" class="short_txt" value="${data.REPART_NAME}"
                           style="" id="repart_cname_${data.PART_ID}"
                           name="repart_cname__${data.PART_ID}"/>
        ...
        <table border="0" class="table_query">
            <tr align="center">
                <td>&nbsp; <input class="normal_btn" type="button" value="审核"
                                  onclick="createReplacedOrderConfirm();"/>
                    <!-- 						onclick="createReplacedOrderConfirm();" />  -->
                    &nbsp;&nbsp; <input class="normal_btn" type="button" value="返 回"
                                        name="back" id="back" onclick="goBack()"/></td>
            </tr>
        </table>
    </div>
</form>
