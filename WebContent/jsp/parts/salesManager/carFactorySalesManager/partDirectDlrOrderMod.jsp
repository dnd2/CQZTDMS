...
<head>
    ...
    <script type="text/javascript">
        ...
        function addCell(partId, partOldcode, partCname, unit, itemQty, minPackage, buyPrice, upOrgStock, isLack, isReplaced, saleQty) {
            ...
            var buyPrice1 = (parseFloat(buyPrice) * parseFloat(discount)).toFixed(2);
...
//    createCells(rowObj, 4, ("partCode_" + partId), ("partCode_" + partId), "", "", "hidden", partCode, false, false, "");
        ...
        function getResult(jsonObj) {
            ...
            if (jsonObj != null) {
                ...
                if (success) {
                    MyAlert(success);
                    if (document.getElementById("state").value ==<%=Constant.CAR_FACTORY_SALES_MANAGER_ORDER_STATE_02%>) {
...
<form name="fm" id="fm" method="post" enctype="multipart/form-data">
    ...
    <input type="hidden" name="PAY_TYPE" id="PAY_TYPE" value="${mainMap.PAY_TYPE}">

    <div class="wbox">
        <div class="navigation"><img src="<%=contextPath%>/img/nav.gif"/>&nbsp;当前位置：配件管理&gt;配件采购管理&gt;配件委托订单提报&gt;修改
        </div>
		     ...
		     <div class="form-body">
		        <table class="table_add" border="0" style="line-height:30px;" cellSpacing=1 cellPadding=1 width="100%">
		            <tr>
		                ...
		                <td>
		                    <input type="hidden" value="${mainMap.ORDER_ID}" name="orderId" id="orderId"/>
		                    <input type="hidden" value="${mainMap.ORDER_CODE}" name="orderCode" id="orderCode"/>
		                    ${mainMap.ORDER_CODE}
		                </td>
		                <td align="right" nowrap>销售单位：</td>
		                <td>
		                    <input name="SELLER_NAME" class="middle_txt" id="SELLER_NAME" type="hidden"
		                           value="${mainMap.SELLER_NAME}"/>${mainMap.SELLER_NAME}
		                    <input name="SELLER_CODE" class="middle_txt" id="SELLER_CODE" type="hidden"
		                           value="${mainMap.SELLER_CODE}"
		            ...
		            <tr>
		                ...
		                <td><input class="middle_txt" type="hidden" value="${mainMap.DEALER_NAME}" name="dealerName" id="dealerName"/>
		                    ${mainMap.DEALER_NAME}
		                </td>
		                <td align="right">订货人：</td>
		                <td><input class="middle_txt" type="hidden" value="${mainMap.BUYER_NAME}" name="buyerName" id="buyerName"/>
		                           ${mainMap.BUYER_NAME}
		                </td>
		                <td align="right">订货日期：</td>
		                <td><input type="hidden" value="${mainMap.CREATE_DATE}" name="now" id="now"/>
		                    ${mainMap.CREATE_DATE}
		                </td>
		            </tr>
		            <tr>
		                <td align="right" nowrap>接收单位：</td>
		                <td colspan="3">${mainMap.RCV_ORG}
		                    <input name="RCV_ORG" class="middle_txt" id="RCV_ORG" type="hidden" value="${mainMap.RCV_ORG}"
		                           size="20"
		                           readonly="readonly"/>
		                ...
		                <td align="right" style="display: none">品牌配件：</td>
		                <td style="display: none">
		                    <script type="text/javascript">
		            ...
		            <tr>
		                ...
		                <td colspan="3">
		                    <input name="ADDR" class="middle_txt" id="ADDR" type="text" size="20" value="${mainMap.ADDR}" style="width:450px;"
		                           readonly="readonly"/>
		            ...
		            <tr id="tr1">
		                ...
		                <td id="accountSumTextTd">
		                	<input id="accountSum" name="accountSum" type="hidden" class="middle_txt"
		                     	value="${dataMap.currAcountMap.ACCOUNT_SUM}"
		                        />
		                    ${dataMap.currAcountMap.ACCOUNT_SUM}元
		                </td>
		                <td id="accountKyNameTd" align="right">可用金额：</td>
		                <td id="accountKyTextTd">
		                	<input id="accountKy" style="border:0px;background-color:#6F9;" class="middle_txt"
		                           name="accountKy" type="text" value="${dataMap.currAcountMap.ACCOUNT_KY}"/>
		                    <font color="blue">元</font>
		                </td>
		                <td id="accountDjNameTd" align="right">冻结金额：</td>
		                <td id="accountDjTextTd">
		                	<input id="accountDj" name="accountDj" type="hidden" value="${dataMap.currAcountMap.ACCOUNT_DJ}" class="normal_txt" readonly/>
		                    ${dataMap.currAcountMap.ACCOUNT_DJ}
		                    <font color="blue">元</font>
		                </td>
		            </tr>
		            <tr id="tr2">
		                <td id="AmountNameTd" align="right">订单总金额：</td>
		                <td id="AmountTextTd">
		                	<input id="Amount" name="Amount" type="text" value="${mainMap.ORDER_AMOUNT} " class="middle_txt" readonly
		                           style="background-color: #ffff80;border: none"/>
		                    <font color="blue">元</font>
		                </td>
		                <td align="right"><span align="right">运费金额：</span></td>
		                <td>
		                	<input id="freight" name="freight" type="text" value="${mainMap.FREIGHT}" class="middle_txt"
		                           style="background-color: #ffff80;border: none" readonly/>
		                    <font color="blue">元</font>
		                </td>
		                <td id="DISCOUNTNameTd" align="right">折扣率：</td>
		                <td id="DISCOUNTTextTd">
		                	<input name="DISCOUNT" id="DISCOUNT" readonly class="middle_txt" type="text" value="${mainMap.DISCOUNT}" />
		                </td>
		            </tr>
		            <tr>
		                <td align="right">备注：</td>
		                <td colspan="5"><textarea name="textarea" cols="80" rows="4">${mainMap.REMARK}</textarea>
        ...
        <table id="file" class="table_list" style="border-bottom:1px solid #DAE0EE">
            ...
            <c:forEach items="${detailList}" var="data">
                <tr class="table_list_row1">
                    ...
                    <td><INPUT class="u-button" onclick=deleteTblRow(this); value=删除 type=button name=queryBtn>
