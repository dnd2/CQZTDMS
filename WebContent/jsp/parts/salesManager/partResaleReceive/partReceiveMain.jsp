...
<title>配件领用单</title>
<style>table.table_query input.short_txt:first-child{margin-left:0}</style>
...
function myLink(value, meta, record) {
    ...
    if (saveState == state || rejState == state) {
        ...
        if (outQtys > 0) {
            ...
            str = "<a href=\"#\" onclick='viewDetail(\"" + changeId + "\")'>[查看]</a>&nbsp;<a href=\"#\" onclick='ConformReback(\"" + changeId + "\")'>[撤回]</a>&nbsp;<a href=\"#\" onclick='openPtPage(\"" + changeId + "\")'>[打印]</a>";
        }
    }
    else {
        str = "<a href=\"#\" onclick='viewDetail(\"" + changeId + "\")'>[查看]</a>&nbsp;<a href=\"#\" onclick='openPtPage(\"" + changeId + "\")'>[打印]</a>";
...
function ConformReback(retailId) {
    MyConfirm("确定撤回该提交订单?", commitReback, [retailId]);
}
...
function commitReback(retailId) {
    btnDisable();
    var url = "<%=contextPath%>/parts/salesManager/partResaleReceiveManager/partResRecAction/rebackOrderInfos.json?retailId=" + retailId + "&curPage=" + myPage.page;
    sendAjax(url, getResult, 'fm');
}
		...
		<form method="post" name="fm" id="fm">
			<div class="navigation">
				...
				<c:if test="${oemId != null}">
					<c:choose>
						<c:when test="${oemId  eq parentOrgId }">&nbsp;当前位置：配件管理 &gt; 内部领用管理 &gt; 内部领用单</c:when>
						<c:otherwise>&nbsp;当前位置： 配件管理 &gt; 零售领用管理 &gt; 配件领用单</c:otherwise>
					</c:choose>
				</c:if>
					...
					<table class="table_query">
						...
						<tr>
							<td class="center" colspan="4">
								...
								<input class="u-button" type="button" value="新增领用单" onclick="addRec()" />
