...
#partDiv .table_query{background-color: transparent}
.table_query td.bottom-button,table td.bottom-button{padding: 10px 0}
</style>
<body>
	<div class="wbox">
		<form name="fm" id="fm" method="post" enctype="multipart/form-data">
			...
			<div class="navigation">
				...
				<c:if test="${oemId != null}">
					<c:choose>
						<c:when test="${oemId  eq parentOrgId }">&nbsp;当前位置：配件管理 &gt; 内部领用管理 &gt; 内部领用单 &gt; 新增领用</c:when>
						<c:otherwise>&nbsp;当前位置：配件管理 &gt; 零售领用管理 &gt; 配件领用单 &gt; 新增领用</c:otherwise>
					</c:choose>
				</c:if>
			</div>
				...
				<div class="form-body">
					<table class="table_query">
						<tr>
							...
							<td>${changeCode}<input type="hidden" name="changeCode" id="changeCode" value="${changeCode}" />
							</td>
							<td class="right">制单人：</td>
							<td>${marker}</td>
							<td class="right">仓库：</td>
							<td width="28%">
								<select name="whId" id="whId" class="u-select" onchange="WHChanged()">
						...
						<tr>
							...
							<td colspan="5">
								<textarea class="form-control align" style="width: 83.6%" id="remark" name="remark" cols="4" rows="4"></textarea>
							</td>
						</tr>
					</table>
				</div>
