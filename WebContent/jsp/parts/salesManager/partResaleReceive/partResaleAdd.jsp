...
$(function(){
	var whName = document.getElementById('whId').selectedOptions[0].text;
	$("#whName").val(whName);
});
var myPage;
...
var columns = [
    ...
    {header: "零售单价", dataIndex: 'RES_PRICE', right: 'right;'},
    {header: "零售数量", dataIndex: 'PART_ID', align: 'center', renderer: returnText}
];
    ...
    for (var i = 1; i < mt.rows.length; i++) {
        if (mt.rows[i].cells[1].firstChild.checked) {
            ...
            if (validateCell(loc)) {
                ...
                var price = mt.rows[i].cells[11].innerText;  //零售价格
...
function savePlan() {
    btnDisable();
    var url = "<%=contextPath%>/parts/salesManager/partResaleReceiveManager/partResRecAction/saveOrderInfos.json";
...
function getResult(json) {
    ...
    if (null != json) {
        if (json.errorExist != null && json.errorExist.length > 0) {
            ...
            btnDisable();
            MyAlert("保存成功!单号:" + json.orderCode, function(){
            	window.location.href = "<%=contextPath%>/parts/salesManager/partResaleReceiveManager/partResRecAction/partResInit.do";
            });
        } else if (json.Exception.message != null && json.Exception.message.length > 0) {
...
function confirmSubmit() {
    ...
    for (var i = 0; i < cb.length; i++) {
        if (cb[i].checked) {
            ...
            if (returnQty == "") {
                msg += "请填写第" + document.getElementById("idx_" + cb[i].value).value + "行的零售数量!</br>";
                flag = true;
            } else if(returnQty < 1){
                msg += "第" + document.getElementById("idx_" + cb[i].value).value + "行的零售数量不能小于1!</br>";
                flag = true;
            } else {
                var normalQty = parseInt(document.getElementById("normalQty_" + cb[i].value).value);
                if (normalQty < returnQty) {
                    msg += "第" + document.getElementById("idx_" + cb[i].value).value + "行的零售数量不能大于可用库存!</br>";
                    flag = true;
                }
            }
        } else {
...
function goBack() {
    btnDisable();
    fm.action = "<%=contextPath%>/parts/salesManager/partResaleReceiveManager/partResRecAction/partResInit.do";
    fm.submit();
}
	...
	<div class="wbox" style="min-width: 1100px;">
		<form name="fm" id="fm" method="post" enctype="multipart/form-data">
			...
			<div class="navigation">
				<img src="<%=contextPath%>/img/nav.gif" />&nbsp;当前位置：配件管理 &gt; 零售领用管理 &gt; 配件零售单 &gt; 新增零售
			</div>
					...
					<div class="form-body">
						<table class="table_query">
							<tr>
								...
								<td>
									<select name="whId" id="whId" style="width: 150px;" class="u-select" onchange="WHChanged(this)">
							...
							<tr>
								...
								<td>
									<input class="middle_txt" type="text" name="telPhone" id="telPhone" value="" />
								</td>
								<td class="right">用途：</td>
								<td>
									<input class="middle_txt" type="text" name="purpose" id="purpose" value="" />
								</td>
							</tr>
							<tr>
					...
					<table>
						<tr>
							<td>
								...
								&nbsp;
								<input type="button" id="upbtn" class="u-button u-submit" value="确 定" onclick="confirmUpload()" />
							</td>
						</tr>
					</table>
				</div>
