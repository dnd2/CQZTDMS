...
<form method="post" name ="fm" id="fm" enctype="multipart/form-data">
    <input type="hidden" name="BALANCE_CODE1" id="BALANCE_CODE1"  value="${BALANCE_CODE1}" />
    ...
    <c:if test="${rejectFlag eq 1}">
	    <tr>
		      <td class="right"><font color="red">驳回原因：</font></td>
		      <td  colspan="5">${remark1}</td>
	   </tr>
    </c:if>
    <tr>
     <td class="center" colspan="6"><input name="BtnQuery" id="queryBtn" type="button" class="u-button u-query" onclick="__extQuery__(1);" value="查 询"/>
         &nbsp;<input name="button" type="button" class="normal_btn" onclick="myback();"
                      value="返回"/>
                      
                      <!-- zhumingwei add 2013-10-25 导出明细 -->
                      &nbsp;
                    <input class="normal_btn" type="button" value="导 出" name="button2" onclick="exportExcel()"/>
   </tr>
	</table>
</div>
</div>
...
<script type="text/javascript" ><!--
   // autoAlertException();//输出错误信息
	...
	var columns = [
				...
				//{header: "<input type='checkbox' name='checkAll' id='checkAll' onclick='selAll(this)' />", width:'8%',sortable: false,dataIndex: 'IN_ID',renderer:myCheckBox},
    ...
    function exportExcel() {
        fm.action = "<%=contextPath%>/parts/purchaseOrderManager/PurchaseOrderBalanceManager/exportExcel.do";
        fm.submit();
    }
--></script>
</div>
