...
<form name="fm" id="fm" method="post" enctype="multipart/form-data">
    ...
    <c:forEach items="${allList}" var="subList" varStatus="countter">
    <div name=thisblock id=thisblock style="width:21.5cm; height: 10.5cm;" class="PageNext">
        <TABLE border=0 border=0 bordercolor=black cellpadding=0 cellspacing=0 width="95%">
            <br><center><font size="+1"><b>重庆君马新能源汽车有限公司配件进货单</b></font></center>
        </TABLE>
        <table border=0  cellpadding=0  cellspacing=0 width="100%" >
            <tr style="height: 18; line-height: 18px;">
                <td width="45%" nowrap>  验货单号：${dataMap.mainMap.chkCode} </td>
                <td width="30%">   采购订单号：${dataMap.mainMap.orderCode} </td>
                <td width="29%" >  库房： ${dataMap.mainMap.whName} </td>
            </tr>
            <tr style="height: 18; line-height: 18px;">
                <td width="45%">  供货商：${dataMap.mainMap.venderName} </td>
                <td width="30%" nowrap>   税率：${dataMap.mainMap.rate} </td>
                <td width="29%" nowrap > 制单人：${dataMap.mainMap.name} </td>
            </tr>
            <tr style="height: 18; line-height: 18px;">
                <td width="45%">   供货厂家：${dataMap.mainMap.makerName} </td>
                <td width="30%" nowrap>   验货时间：${dataMap.mainMap.date} </td>
                <td width="29%" nowrap > 制单日期：${dataMap.mainMap.curDate} </td>
            </tr>
        </table>
        ...
        <TABLE id="file" border=1 bordercolor=black cellpadding=0   cellspacing=0 width="100%" style="margin-top: 5px;" >
            ...
            <tr style="height: 19; line-height: 19px;">
                    ...
                    <td style="text-align: center"> ${data.PART_OLDCODE}  </td>
                    <td style="text-align: center" >
                    ...
                    <td style="text-align: center;" > ${data.GENERATE_QTY} </td> 
                    <td style="text-align: center" > ${data.UNIT} </td>
                    <td style="text-align: center" > ${data.PLAN_PRICE} </td>
                    <td style="text-align: center" > ${data.PLAN_PRICE} </td>
                    <td style="text-align: center" > ${data.FAMOUNT} </td>
    ...
    <TABLE border=0  width="95%" >
		...
		<td align="left" width="90%" colspan="3">备注：&nbsp;&nbsp;${dataMap.mainMap.remark}</td>
         ...
         <td align="left" width="30%">计划金额：&nbsp;&nbsp;${dataMap.mainMap.amount}</td>
          ...
          <tr style="height: 18; line-height: 18px;">
          <td align="left" width="30%">计划员确认：&nbsp;&nbsp;
          </td>
          <td align="left" width="30%">验收员确认：&nbsp;&nbsp;</td>
          <td align="left" width="30%">保管员确认：&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;${countter.count}/${listAcount}</td>
          </tr>
    </TABLE>
    </div>
    </c:forEach>
