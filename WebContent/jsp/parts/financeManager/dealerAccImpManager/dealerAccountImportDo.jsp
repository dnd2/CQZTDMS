...
<div class=navigation><img src="<%=request.getContextPath()%>/img/nav.gif">&nbsp;服务商资金导入确认</div>
...
<table class="table_list" >  
  <tr class="table_list_row1" style="background-color:#DAE0EE; ">
    <td width="8%" align="center">序号</td>
  	<td width="23%" align="center">服务商编码</td>
  	<td width="23%" align="center">服务商名称</td>
  	<td width="23%" align="center">金额(元)</td>
 ...
 <c:set var="count" value="${1}"/>
  ...
  %>
  	 <tr class="table_list_row1">
	  	...
	  	<td align="center"><%=map.get("2") %><input type="hidden" name="amount${count }" value="<%=map.get("2") %>"/></td>
	  	<td align="center"><%=map.get("3") %>
	  	<input type="hidden" name="remark${count }" value="<%=map.get("3") %>"/>
...
<script type="text/javascript">
 ...
 //服务商资金确认导入：
	function add(parms) {
		if(confirm("确定导入?")){
			btnDisable();
		    var url = '<%=contextPath%>/parts/financeManager/dealerAccImpManager/partDealerAccImpAction/dealerAccountAdd.json';
	   ...
	   if(null != json)
	   {
		   if (json.errorExist != null && json.errorExist.length > 0) {
			   MyAlert(json.errorExist);
	       	   ...
	       	   MyAlert("导入成功!");
	           window.location.href = "<%=contextPath%>/parts/financeManager/dealerAccQueryManager/partDealerAccQueryAction/partDealerAccQueryInit.do";
	       } else {
	           MyAlert("导入失败，请联系管理员!");
	       }
	   }
