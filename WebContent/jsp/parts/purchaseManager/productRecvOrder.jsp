...
jQuery.noConflict();
 ...
 var url = "<%=contextPath%>/parts/purchaseManager/PurchaseOrderManager/showPartStockBase.json";
...
function returnText(value,meta,record) 
{
	...
	return "<input type='hidden' name='Num_"+value+"' id='Num_"+value+"' value="+planQty+" '/>" + planQty;
}
		...
		for(var i = 1 ;i<mt.rows.length ; i ++){
			...
			if(mt.rows[i].cells[1].firstChild.checked){
				...
				if(validateCell(partId)){
					...
					var price = mt.rows[i].cells[12].innerText;  //计划价格
...
function addCell(partId, partCode, partOldcode, partCname,normalQty,unit,price,number){
		...
		cell8.innerHTML = '<td align="center" nowrap><input onchange="dataTypeCheck('+(tbl.rows.length-2)+','+partId+',this)" name="returnQty_'+partId+'" id="returnQty_'+partId+'" type="hidden" value="'+number+'"/>'+number+'</td>';
	...
	function uploadExcel(){
		btnDisable();
		fm.action = "<%=contextPath%>/parts/purchaseManager/PurchaseOrderManager/partResRecUpload.do";
		fm.submit();;
	}
	...
	function savePlan(){
		btnDisable();
		var url = "<%=contextPath%>/parts/purchaseManager/PurchaseOrderManager/saveOrderInfos.json";	
	...
	function goBack(){
		btnDisable();
		//fm.action = "<%=contextPath%>/parts/purchaseManager/PurchaseOrderManager/purchaseOrderQueryInit.do";
		//fm.submit();
		window.history.back();
	}
...
<input type="hidden" name="planOrderCode" id="planOrderCode" value="${planOrderCode }"/>
<div class="wbox">
  <div class="navigation"><img src="<%=contextPath%>/img/nav.gif" />&nbsp;当前位置：配件管理 &gt; 采购计划管理 &gt; 配件采购订单 &gt; 采购订单转领件订单</div>
  <div >
  <table class="table_query"  bordercolor="#DAE0EE">
    ...
    <tr>
      ...
      <td  width="25%" align="left" >
        <input type="hidden" id="venderId" name="venderId" value="${venderId }" /> ${venderName }
        <input type="hidden" id="venderName" name="venderName" value="${venderName }" /> 
	  </td>
      <td width="10%" align="right">仓库：</td>
      <td  width="20%" align="left" >
        <input type="hidden" id="whId" name="whId" value="${selectedWhId }" /> ${selectedWhName }
        <input type="hidden" id="whName" name="whName" value="${selectedWhName }" /> 
	  </td>
    </tr>
    <tr>
				...
				<FIELDSET >
					...
					<th colspan="6">
						...
						<input type="button" class="normal_btn" name="addPartViv"
							id="addPartViv" value="收 起" onclick="addPartDiv()" />
					</th>
					</LEGEND>
					<div style="display: block; heigeht: 5px" id="partDiv">
