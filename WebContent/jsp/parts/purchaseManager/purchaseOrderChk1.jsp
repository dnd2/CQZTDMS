...
<input type="hidden" id="isDeft" name="isDeft" value="1"/>
<c:choose>
    ...
    <c:if test="${list !=null}">
	  <c:forEach items="${list}" var="list" varStatus="_sequenceNum" step="1">
		...
		<tr class="table_list_row2">
		  ...
		  <td align="center" nowrap>
		  <input   name="MAKER_ID1${list.POLINE_ID}" id="MAKER_ID1${list.POLINE_ID}" value="${list.MAKER_ID}" type="hidden" />${list.MAKER_NAME}
		  </td>
		  <td align="center" nowrap>
...
function queryResults()
{
	...
	if("1" == vflag)
	{
		columns = [
					...
					{header: "制造商名称", dataIndex: 'VENDER_NAME', align:'center'},
    ...
    for (var i = 1; i < mt.rows.length; i++) {
        if (mt.rows[i].cells[1].childNodes[0].checked) {
            ...
            if (validateCell(pId)) {
                ...
                var makerName = mt.rows[i].cells[11].innerText;  // 制造商名称
                var makerId = $("VENDER_ID"+pId).value;
...
function addCell(pId,partId, partCode, partOldcode, partCname, unit, planQty, buyQty,buyPrice, amount, geQty, chkQty, venderName, makerName, remark,partId,makerId,venderId) {
    ...
    cell13.innerHTML = '<td align="center" nowrap><input   name="MAKER_ID1' + pId + '" id="MAKER_ID1' + pId + '" value="' + makerId + '" type="hidden" />' + makerName + '</td>';
