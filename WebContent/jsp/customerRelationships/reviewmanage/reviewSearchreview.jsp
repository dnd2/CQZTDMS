    ...
    <tr class="table_list_row0">
     ...
     <c:forEach var="ql" items="${result.reviewInof}">
      <tr class="table_list_row1">
	      ...
	      <td><a href='#' onclick="setContentStyle('RD_CONTENT_${ql.RD_ID}','span_${ql.RD_ID}');">[修改</a>&nbsp;&nbsp;<a href='#' onclick="javascript:saveContent(${ql.RD_ID},document.getElementById('RD_CONTENT_${ql.RD_ID}').value);">更新]</a></td>
      </tr>
   	 </c:forEach>
...
function closeSaveReview(){
	fm.action = "<%=contextPath%>/customerRelationships/reviewmanage/ReviewManage/reviewSearchInit.do";
	fm.submit();
}
function dialAction(){
...
function afterCall(json)
   ...
   function saveContent(rd_id,content){
	   var cUrl= '<%=contextPath%>/customerRelationships/reviewmanage/ReviewManage/saveReviewContent.json?Rd_ID='+rd_id+'&Rd_CONTENT='+content;
