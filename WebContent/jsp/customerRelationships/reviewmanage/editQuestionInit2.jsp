...
<title>客户化关系管理&&回访管理&&问卷管理&&编辑问卷&&编辑问题 </title>
...
<body onload="gcheck();">
  <div class="navigation"><img src="<%=contextPath%>/img/nav.gif" />&nbsp;当前位置： 客户化关系管理 &gt; 回访管理 &gt; 问卷管理 &gt; 编辑问卷 &gt; 编辑问题 
</div>
  <form method="post" name="Form1" id="Form1">
  <input type="hidden" name="QR_ID" id="QR_ID" value="${question[0].QR_ID}"/>
  <input type="hidden" name="QD_ID" id="QD_ID" value="${question[0].QD_ID}"/>
  ...
  <table width="100%" align="center" cellpadding="2" cellspacing="1" class=table_query>
    <tbody>
      <tr class="">
        ...
        <td width="241">
            <select onchange="ChangeQuestionType(this.selectedIndex)" id="ddlQuestionType" name="ddlQuestionType">
              <option  value="">--请选择--</option>
              <c:forEach var="ql" items="${choiceList}">
                <c:choose>
                  <c:when test="${ql.CODE_ID==question[0].QD_QUE_TYPE}">
        ...
        <td width="62"><div align="right">题号 ： </div>
        <td width="529"><input type="text" class="normal_txt" id="QD_NO"  name="QD_NO" value="${question[0].QD_NO}" />
        <font color="#ff0000" face="宋体">*</font>      </tr>
		<script type="text/javascript" >
	       if((${question[0].QD_QUE_TYPE} == <%=Constant.QD_QUE_TYPE_3%>) || (${question[0].QD_QUE_TYPE} == <%=Constant.QD_QUE_TYPE_4%>)){
	    	   document.write('	<div id="TextMode" style="DISPLAY: ">');
		    	   if('${question[0].QD_TXT_TYPE}'=='<%=Constant.qd_txt_type_1%>')
		    	   {
      ...
      <tr class="">
        ...
        <td colspan="3"><textarea id="txtQuestion" style="WIDTH: 600px" rows="5" name="txtQuestion"   >${question[0].QD_QUESTION}</textarea>
          <font color="#ff0000" face="宋体">*</font><br />
          <span style="FONT-FAMILY: 宋体; COLOR: red">问题内容最多允许输入500个字。</span></td>
      </tr>

    <script type="text/javascript" >
       if(${question[0].QD_QUE_TYPE} == <%=Constant.QD_QUE_TYPE_1%> || ${question[0].QD_QUE_TYPE} == <%=Constant.QD_QUE_TYPE_2%>){
    	   document.write('<tr class="" id="SectionBar" style="DISPLAY: ">');
       }else{
    	   document.write('<tr class="" id="SectionBar" style="DISPLAY:none; ">');
       }
    </script>   
    <tr><td style="WIDTH: 17%" align="right">选择题选项： </td>
        <td colspan="3"><textarea id="txtSelection" style="WIDTH: 600px; dislplay: " rows="5" name="txtSelection"  >${question[0].QD_CHOICE}</textarea>
...
var myPage;
	...
	function saveAction()
	{
		...
		if(flag)
		{
			 var url= "<%=contextPath%>/customerRelationships/reviewmanage/ReviewManage/questionEditFact2.json";
	...
	function showResult(json){
		var msg=json.msg;
		if(msg=='01'){
			document.Form1.saveFlag.value="isSave";
			//MyAlert(${questionair[0].QD_NO});
			MyAlertForFun("修改成功",closeSave);
	...
	//返回当前问卷编辑主页
	function closeSave(){
		Form1.action = '<%=contextPath%>/customerRelationships/reviewmanage/ReviewManage/editQuestionairInit.do?QR_ID=${question[0].QR_ID}';
		Form1.submit();
	}
