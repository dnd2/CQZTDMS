...
<body>
	<div class="navigation"><img src="<%=contextPath%>/img/nav.gif" />&nbsp;当前位置： 客户关系管理 &gt; 报告管理 &gt;问卷模版统计</div>
	<form method="post" name = "fm" id="fm">
		<table  border="0" align="center" cellpadding="1" cellspacing="1" class="table_query">
			<th colspan="6"><img class="nav" src="<%=contextPath%>/img/subNav.gif" />问卷模版统计</th>
			
			<tr>
				<td align="right" nowrap="true">问卷：</td>		
				<td align="left" nowrap="true">
					<select id="qrId" name="qrId">
						<c:forEach var="ql" items="${questionList}">
							<option value="${ql.QR_ID}" title="${ql.QR_NAME}">${ql.QR_NAME}</option>
						</c:forEach>
					</select>
				</td>
...
<script type="text/javascript">
	function downExcelQuery(){
		fm.action = '<%=contextPath%>/customerRelationships/reportformsManage/QuestionReportforms/questionReportformsExcel.do';
		fm.submit();
	}
	...
	var myPage;
    //查询路径
	var url = "<%=contextPath%>/customerRelationships/reportformsManage/QuestionReportforms/queryQuestionReportforms.json";
				
	var title = null;
	var columns =  [
				{header: "序号", dataIndex: 'center', renderer:getIndex},
				{header: "问题",dataIndex: 'QUESTION',align:'center'},
				{header: "答案",dataIndex: 'ANSWER',align:'center'},
				{header: "数量",dataIndex: 'TOTAL',align:'center'}
		      ];
