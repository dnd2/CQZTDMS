...
<body>
	<div class="navigation"><img src="<%=contextPath%>/img/nav.gif" />&nbsp;当前位置： 客户关系管理 &gt; 投诉咨询管理 &gt;待处理咨询处理</div>
		...
		<table width="100%" class="tab_edit">
				...
				<td align="left" >
					<select id="contType" name="contType" class="short_sel">
						...
						<c:forEach var="bc" items="${bclist}">
							<c:choose>
								<c:when test="${complaintAcceptMap.BIZCONT == bc.CODEID}">
									<option value="${bc.CODEID}" title="${bc.CODEDESC}" selected="selected">${bc.CODEDESCVIEW}</option>
								</c:when>
								<c:otherwise>
			...
			<tr style="display: none" id="dealuserTr">
				...
				<td align="left" colspan="3">
					...
					<input type="hidden" value="${complaintAcceptMap.VAR}" id="var" name="var">
				</td>
			</tr>		
	...
	<script type="text/javascript">
		function save(){
			...
			if(check()){
				...
				makeNomalFormCall('<%=contextPath%>/customerRelationships/complaintConsult/WaitConsultSearch/waitConsultSearchUpdateSubmit.json',saveBack,'fm','');
			}
		}
		...
		function saveBack(json){
			...
			}else if(json.success != null && json.success=='varFalse'){
				MyAlert("对不起，您所处理的数据已经更新!");
			}else{
				MyAlert("保存失败,请联系管理员!");
			}
		...
		function check(){ 
			...
			if(document.getElementById('complaintTrOne').style.display != "none"){
				...
				if(""!=document.getElementById('cpObject').value && ""!=document.getElementById('vehicleCompany').value){
					msg+="抱怨对象只能选其一!</br>"
				}else if(""==document.getElementById('cpObject').value && ""==document.getElementById('vehicleCompany').value){
					msg+="抱怨对象必选其一!</br>"
				}
		...
		function sendPage(){
			...
			window.close();
			//window.location.href = "<%=contextPath%>/customerRelationships/complaintConsult/WaitConsultSearch/waitConsultSearchInit.do";
		}
