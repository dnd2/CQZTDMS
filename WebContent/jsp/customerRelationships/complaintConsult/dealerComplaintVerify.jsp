...
<body>
	<div class="navigation"><img src="<%=contextPath%>/img/nav.gif" />&nbsp;当前位置： 客户关系管理 &gt; 投诉咨询管理 &gt;经销商延期大区审核</div>
...
<script type="text/javascript">
	function downExcelQuery(){
		fm.action = '<%=contextPath%>/customerRelationships/complaintConsult/DealerComplaintVerify/dealerComplaintVerifyDownExcel.do';
		fm.submit();
	}
	var myPage;
    //查询路径
	var url = "<%=contextPath%>/customerRelationships/complaintConsult/DealerComplaintVerify/queryDealerComplaintVerify.json";
	...
	function myLink(value,meta,record){
		var str = String.format("<input name='detailBtn' type='button' class='normal_btn' onclick='verifyDelay(\""+ record.data.CPID +"\",\""+ record.data.CTMID +"\")' value='审核'/>");
	...
	function verifyDelay(cpid,ctmid,status){
		window.location.href='<%=contextPath%>/customerRelationships/complaintConsult/DealerComplaintVerify/verifyDelay.do?cpid='+cpid+'&ctmid='+ctmid ;
	}
