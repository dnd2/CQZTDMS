...
<body onload="changeBizTypeEvent(9506,'${complaintAcceptMap.BIZCONT}',false)">
	<div class="navigation"><img src="<%=contextPath%>/img/nav.gif" />&nbsp;当前位置： 客户关系管理 &gt; 投诉咨询管理 &gt;投诉转咨询处理（管理员）</div>
	<form method="post" name = "fm" id="fm">
		<table width="100%" class="tab_edit">
			...
			<tr>
				...
				<td align="left" >
					<input type="hidden" id="bizType" name="bizType" class="short_sel" value="9506"/>
					<select class="short_sel" disabled="disabled">
						<c:forEach var="bt" items="${bizTypeList}">
							<c:choose>
								<c:when test="${9506 == bt.id}">
...
<script type="text/javascript">
		function check(){
			...
			if(document.getElementById('complaintTrOne').style.display != "none"){
				if( ""==document.getElementById('vehicleCompany').value){
					msg+="抱怨对象不能为空!</br>"
				}
			}
		...
		function save(){
			...
			if(check()){
				...
				makeNomalFormCall('<%=contextPath%>/customerRelationships/complaintConsult/WaitComplaintSearch/complaintTurnAdviceUpdateSubmit.json',saveBack,'fm','');
			}
		}
