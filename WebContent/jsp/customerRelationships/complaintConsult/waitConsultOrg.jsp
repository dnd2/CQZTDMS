...
<script type="text/javascript">
	function doInit(){
   		...
   		if($("bizType").value==<%=Constant.VOUCHER_TYPE_01%>){
   			checkSet();
   		}
...
<body >
	<div class="navigation"><img src="<%=contextPath%>/img/nav.gif" />&nbsp;当前位置： 客户关系 &gt; 投诉咨询管理 &gt;咨询处理（大区）</div>
...
<script type="text/javascript">
		...
		function save(){
			...
			if(check()){
				...
				makeNomalFormCall('<%=contextPath%>/customerRelationships/complaintConsult/ComplaintAcceptOrg/adviceUpdateSubmit.json',saveBack,'fm','');
			}
		}
		...
		//页面跳转：
		function goBack(){
			window.location.href = "<%=contextPath%>/customerRelationships/complaintConsult/ComplaintAcceptOrg/getComplaintAcceptOrgInit.do";
		}
