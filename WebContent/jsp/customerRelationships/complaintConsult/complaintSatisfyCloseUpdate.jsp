...
<body>
<div class="wbox">
<div class="navigation"><img src="../../../img/nav.gif" width="11" height="11" />&nbsp;当前位置：  客户信息 &gt;投诉满意关闭处理界面 </div>
</div>
        ...
        <td><table id="Table2" width="100%" class="tab_edit">
          <tbody>
            ...
            <tr>
              ...
              <td colspan="5">
              	<select disabled="disabled" class="short_sel">
              		<option value="<%=Constant.PLEASED%>" selected="selected">满意</option>
              		<option value="<%=Constant.GENERAL%>">一般</option>
              		<option value="<%=Constant.YAWP%>">不满意</option>
              	</select>
              	<input type="hidden" id="crResult1" name="crResult"  value="<%=Constant.PLEASED%>"/>
              </td>
            </tr>
			<tr id="ccontTr">
...
<script type="text/javascript">
	...
	function save(cpid){
		if(check()){
			document.getElementById("three_package_set_btn").disabled = true;
   			makeNomalFormCall('<%=contextPath%>/customerRelationships/complaintConsult/WaitComplaintSearch/complaintSatisfyCloseUpdateSubmit.json?id=customerReturn&cpid='+cpid,saveBack,'fm','');
		}
	}
	...
	function saveBack(json){
		...
		}else if(json.success != null && json.success == '0'){
			MyAlert("历史数据抱怨对象为空,请投诉管理员先确认抱怨对象!");
		}else{
			MyAlert("保存失败,请联系管理员!");
		}
	}
