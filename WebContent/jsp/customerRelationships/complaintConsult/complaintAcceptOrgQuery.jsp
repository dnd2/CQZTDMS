...
<body>
	<div class="navigation"><img src="<%=contextPath%>/img/nav.gif" />&nbsp;当前位置： 客户关系管理 &gt; 投诉咨询管理 &gt;投诉咨询处理（大区）</div>
...
<script type="text/javascript">
	var myPage;
    //查询路径
	var url = "<%=contextPath%>/customerRelationships/complaintConsult/ComplaintQuery/queryComplaintSearchOrg.json";
	...
	function myLink(value,meta,record){
		...
		var str3="<input name='detailBtn' type='button' class='middle_btn' onclick='comComplaintEdit(\""+ record.data.CPID +"\",\""+ record.data.BIZTYPE +"\")' value='处理'/>&nbsp;";
		var str4="<input name='detailBtn' type='button' class='middle_btn' onclick='complaintReport(\""+ record.data.CPID +"\")' value='上报'/>&nbsp;";
		var str=str1;
		if(record.data.STATUS == <%=Constant.VOUCHER_STATUS_04%>){
				str+=str3+str2;
	...
	function complaintDealerSet(cpid,biztype){
		OpenHtmlWindow('<%=contextPath%>/customerRelationships/complaintConsult/ComplaintAcceptOrg/complaintDealerSetInit.do?cpId='+cpid+'&bizType='+biztype,800,300);
	}
	//关闭
	function complaintClose(){
		fm.action = '<%=contextPath%>/customerRelationships/complaintConsult/ComplaintAcceptOrg/complaintKHClose.do?cpId='+cpid;
		fm.submit();
	}
	//处理
	function comComplaintEdit(cpid){
		fm.action = '<%=contextPath%>/customerRelationships/complaintConsult/ComplaintAcceptOrg/doComplaintOrg.do?cpId='+cpid;
		fm.submit();
	}
	//上报
	function complaintReport(cpid){
		if(confirm("确定上报吗？")){
		makeNomalFormCall('<%=contextPath%>/customerRelationships/complaintConsult/ComplaintAcceptOrg/complaintReportInit.json?cpId='+cpid,setBack,'fm','detailBtn');
			}
		}
