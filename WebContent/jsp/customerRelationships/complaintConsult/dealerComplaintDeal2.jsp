...
<body>
	<div class="navigation"><img src="<%=contextPath%>/img/nav.gif" />&nbsp;当前位置： 客户关系管理 &gt; 投诉咨询管理 &gt;投诉经销商处理</div>
...
<script type="text/javascript">
	function downExcelQuery(){
		fm.action = '<%=contextPath%>/customerRelationships/complaintConsult/DealerComplaintDeal/dealerComplaintDealDownExcel.do';
		fm.submit();
	}
	var myPage;
    //查询路径
	var url = "<%=contextPath%>/customerRelationships/complaintConsult/DealerComplaintDeal/queryDealerComplaintDeal.json";
	...
	function applyDelay(cpid){		
		window.location.href='<%=contextPath%>/customerRelationships/complaintConsult/DealerComplaintDeal/applayDelay.do?cpid='+cpid ;
	}
	function updateComplaint(cpid,ctmid,status){
		window.location.href='<%=contextPath%>/customerRelationships/complaintConsult/WaitComplaintSearch/waitComplaintSearchUpdate11.do?cpid='+cpid+'&ctmid='+ctmid+'&stauts='+status+'&pageName=Dealer' ;
	}
