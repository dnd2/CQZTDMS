...
<body>
<div class="navigation"><img src="<%=contextPath%>/img/nav.gif" />&nbsp;当前位置： 客户关系管理 &gt; 客户投诉管理 &gt;客户投诉处理(区域)</div>
 ...
 <input type="hidden" name="orgId" id="orgId" value="<%=request.getAttribute("orgId")%>"/>
    <table width=100% border="0" align="center" cellpadding="1" cellspacing="1" class="table_query">
          ...
          <tr>
            ...
            <td>
            	<script type="text/javascript">
            	genSelBoxExp("compStatus",<%=Constant.COMP_STATUS_TYPE%>,"",false,"min_sel","onchange='selectAction()'","false",'<%=Constant.COMP_STATUS_TYPE_01%>,<%=Constant.COMP_STATUS_TYPE_03%>,<%=Constant.COMP_STATUS_TYPE_04%>');
                </script>
                  处理中状态：
                <span id="statuschng">
                <script type="text/javascript">
            	genSelBoxExp("auditResult",<%=Constant.AUDIT_RESULT_TYPE%>,"",true,"min_sel",'',"true",'');
                </script>
			    </span>
          ...
          <input name="searchBtn" type="button" class="normal_btn" onclick="__extQuery__(1);" value="查询" /></td>
         </tr>
     </table> 
...
<form name="form1" style="display:none">
   <table align="center" class="table_query" >
      <tr>
        <th colspan="3" align="center">选择分配的经销商</th>
      </tr>
      <tr  class="table_list_row">
        <td width="10%">经销商:</TD>
        <td width="40%">
          <input name="dealerCode" type="text" id="dealerCode" size="40" readonly/>
        ...
        <td width="50%">
        <input name="assignBtn" type="button" class="normal_btn" onclick="assignDealer();" value="分配" />
        <input class="cssbutton" type="button" value="清空" onclick="clrTxt('dealerId','dealerCode');"/></td>
      </tr>
  </table>
</form>
<!--页面列表 begin -->
<script type="text/javascript" >
	...
	var myPage;
    //查询路径
	var url = "<%=contextPath%>/customerRelationships/complaint/ComplaintDisposalArea/queryComplaintDisposalArea.json";
...
//设置超链接  begin      
	...
	function myLink(value,meta,record){
  		return String.format("<a href='#' onclick='disposalComplaintArea(\""+ value +"\",\""+record.data.STATUS+"\")'>[处理]</a>");
	}
	
	//处理的ACTION设置
	function disposalComplaintArea(value,status){
		fm.action = '<%=contextPath%>/customerRelationships/complaint/ComplaintDisposalArea/disposalComplaintArea.do?compId=' + value+"&status="+status;
	 	fm.submit();
	}
	
	
	
	//动态显示投诉状态和处理状态的联动(该页面的状态默认是处理中)
	function selectAction(){
		if(document.fm.compStatus.value == '<%=Constant.COMP_STATUS_TYPE_04%>') {
	...
	function assignDealer(){
		...
		if (cnt == 0) {
			...
			return;
	    	 ...
	    	 MyConfirm("确认分配给该经销商？",assignConfirm);
		}
	}
	
	// 分配给区域
	function  assignConfirm(){
	    var dealerId = document.form1.dealerId.value;
		makeNomalFormCall('<%=contextPath%>/customerRelationships/complaint/ComplaintDisposalArea/assignComplaintToDealer.json?dealerId='+dealerId,showBack,'fm');
	}
