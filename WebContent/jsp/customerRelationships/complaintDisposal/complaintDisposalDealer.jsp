...
<body>
<div class="navigation"><img src="<%=contextPath%>/img/nav.gif" />&nbsp;当前位置： 客户关系管理 &gt; 客户投诉管理 &gt;客户投诉处理(经销商)</div>
...
<script type="text/javascript" >
	...
	var myPage;
    //查询路径
	var url = "<%=contextPath%>/customerRelationships/complaint/ComplaintDisposalDealer/queryComplaintDisposalDealer.json";
...
//设置超链接  begin      
	...
	function disposalComplaintArea(value,status){
		fm.action = '<%=contextPath%>/customerRelationships/complaint/ComplaintDisposalDealer/disposalComplaintDealer.do?compId=' + value+"&status="+status;
	 	fm.submit();
	}
...
function selType(){
	...
	function assignDealer(){
	 var cnt = 0;
		var l=document.fm.compIds.length;
