...
<body>
<div class="navigation"><img src="<%=contextPath%>/img/nav.gif" />&nbsp;当前位置： 客户关系管理 &gt; 客户投诉管理 &gt;客户投诉维护(服务站)</div>
...
function saveModify(){
	...
	if(submitForm('fm')){
		fm.action = "<%=contextPath%>/customerRelationships/complaint/ComplaintDisposalDealer/saveModify.do?";
    	fm.submit();
	}
}
...
function complaintCloseToDo()
{
	if(submitForm('fm'))
	{
		fm.action = "<%=contextPath%>/customerRelationships/complaint/ComplaintDisposalDealer/closeComplaintByDealer.do?";
    	fm.submit();
    }
}
