 ...
 <form method="post" name = "fm" >
   ...
   <table width=100% border="0" align="center" cellpadding="1" cellspacing="1" class="table_query">
    ...
    <tr>
     ...
     <td><input class="middle_txt" id="compDealerCode" name="compDealerCode" value="<c:out value="${complaintMap.DEALER_CODE}"/>" type="text"/>
         <input class="mark_btn" type="button" value="&hellip;" onclick="showOrgDealer('compDealer','compDealerId','true','',true)"/> 
  ...
  <TABLE class="table_query">
		...
		<TR>
		  ...
		  <TD><div align="left">
		   <script type="text/javascript">
            	genSelBoxExp("compType",<%=Constant.COMP_TYPE_TYPE%>,"<c:out value="${compType}"/>",false,"min_sel","","false",'');
            </script>
  ...
  <table class="table_query">
		...
		<TR>
			...
			<%if(actionList!=null&&actionList.contains(listCheckBox.get(i).get("CODE_ID"))) {%>
				<input type="checkbox" name="auditAction" value="<%=listCheckBox.get(i).get("CODE_ID") %>" checked disabled> <%=listCheckBox.get(i).get("CODE_DESC") %>&nbsp;&nbsp;
			<%}else{ %>
			  <input type="checkbox" name="auditAction" value="<%=listCheckBox.get(i).get("CODE_ID") %>" disabled><%=listCheckBox.get(i).get("CODE_DESC") %>&nbsp;&nbsp;
			<%} %>
			<%} %>
			</div>			
			</TD>
		...
		<TR>
			...
			<TD>&nbsp;</TD>
			<TD><c:out value="${detailMap.NAME}"/></TD>
		</TR>
		<TR>
			<TD>
			...
			<TD>
			<div align="left">
			   <input type="hidden" name="auditResult" id="auditResult" value="<c:out value="${detailMap.AUDIT_RESULT}"/>"/>
              <script type='text/javascript'>
				writeItemValue('<c:out value="${detailMap.AUDIT_RESULT}"/>');
			  </script>
			  &nbsp;&nbsp;
			  <span id="partcode" style="visibility:hidden">备件编码：
              <c:out value="${detailMap.PART_CODE}"/></span>
              &nbsp;&nbsp;
              <span id="supplier" style="visibility:hidden">上级保供单位：
              <c:out value="${detailMap.SUPPLIER}"/></span>
			</div></TD>
		</TR>
		<TR>
			<TD>
			...
			<TD>&nbsp;</TD>
			<TD>
            	<textarea name="auditContent" readonly
				style='border: 1px solid #94BBE2; width: 95%; overflow: hidden; word-break: break-all;'rows=5><c:out value="${detailMap.AUDIT_CONTENT}"/></textarea></TD>
		</TR>
        <TR>
		  <td colspan="3" align="center"><input name="saveBtn" type="button" class="normal_btn" onclick="saveModify()" value="保存" />&nbsp;&nbsp;&nbsp;&nbsp;
			 <input name="cnfrmClsBtn"  id="cnfrmClsBtn" type="button" class="long_btn" onclick="confirmClose()" value="确认关闭投诉"/></td>
	    </TR>
  </table>
  <br>
    <table class="table_query">
	  ...
	  <tr>
	    <td><div align="center">区域
	      <input name="orgId" type="text" id="orgId" size="40" />
		  <input name="orgSel" type="button" class="mini_btn" onclick="" value="&hellip;" />	
		</div></td>	
	    <td><input name="assign" type="button" class="normal_btn" onclick="assignDisposal()" value="分配" disabled />
		&nbsp;&nbsp;&nbsp;&nbsp;</td>		
      </tr>
	</table>
	<br>
...
function selectAction(){
		if(document.fm.auditResult.value == '<%=Constant.AUDIT_RESULT_TYPE_02%>'||document.fm.auditResult.value == '<%=Constant.AUDIT_RESULT_TYPE_03%>') {
			...
			document.getElementById("partcode").style.visibility = "visible";
			document.getElementById("supplier").style.visibility = "visible";
			
		}else{
		    document.getElementById("partcode").style.visibility = "hidden";
...
function showCloseBtn(){
    ...
    if(document.fm.status.value=='<%=Constant.COMP_STATUS_TYPE_02%>'){
    	document.getElementById("cnfrmClsBtn").disabled = true;
    }else{
    	document.getElementById("cnfrmClsBtn").disabled = false;
    }
}
...
function selType(){
	if(document.fm.compType.value == '<%=Constant.COMP_TYPE_TYPE_01%>') {
			var sc = genSelBoxStrExp('compType2',<%=Constant.SEDAN_QUALITY%>,"<c:out value="${complaintMap.COMP_TYPE}"/>",false,"min_sel","","false",'');
			var pu = document.getElementById("compTypechng").innerHTML = sc;
			
		}else if(document.fm.compType.value == '<%=Constant.COMP_TYPE_TYPE_02%>'){
			var sc = genSelBoxStrExp('compType2',<%=Constant.SEDAN_MEDIUM_COMPLAINS%>,"<c:out value="${complaintMap.COMP_TYPE}"/>",false,"min_sel","","false",'');
			var pu = document.getElementById("compTypechng").innerHTML = sc;
		
		}else if(document.fm.compType.value == '<%=Constant.COMP_TYPE_TYPE_03%>'){
			var sc = genSelBoxStrExp('compType2',<%=Constant.SEDAN_SERVES%>,"<c:out value="${complaintMap.COMP_TYPE}"/>",false,"min_sel","","false",'');
			var pu = document.getElementById("compTypechng").innerHTML = sc;
		
		}
		else if(document.fm.compType.value == '<%=Constant.COMP_TYPE_TYPE_04%>'){
			var sc = genSelBoxStrExp('compType2',<%=Constant.SEDAN_ACCESSORIES%>,"<c:out value="${complaintMap.COMP_TYPE}"/>",false,"min_sel","","false",'');
			var pu = document.getElementById("compTypechng").innerHTML = sc;
		
		}
		else if(document.fm.compType.value == '<%=Constant.COMP_TYPE_TYPE_05%>'){
			var sc = genSelBoxStrExp('compType2',<%=Constant.SALE_CONSULTS%>,"<c:out value="${complaintMap.COMP_TYPE}"/>",false,"min_sel","","false",'');
			var pu = document.getElementById("compTypechng").innerHTML = sc;
		
		}
		else if(document.fm.compType.value == '<%=Constant.COMP_TYPE_TYPE_06%>'){
			var sc = genSelBoxStrExp('compType2',<%=Constant.SALE_COMPLAINS%>,"<c:out value="${complaintMap.COMP_TYPE}"/>",false,"min_sel","","false",'');
			var pu = document.getElementById("compTypechng").innerHTML = sc;
		
		}
		else if(document.fm.compType.value == '<%=Constant.COMP_TYPE_TYPE_07%>'){
			var sc = genSelBoxStrExp('compType2',<%=Constant.SEDAN_HELPS%>,"<c:out value="${complaintMap.COMP_TYPE}"/>",false,"min_sel","","false",'');
			var pu = document.getElementById("compTypechng").innerHTML = sc;
		
		}
		else if(document.fm.compType.value == '<%=Constant.COMP_TYPE_TYPE_08%>'){
			var sc = genSelBoxStrExp('compType2',<%=Constant.SEDAN_SERVES_DEALER%>,"<c:out value="${complaintMap.COMP_TYPE}"/>",false,"min_sel","","false",'');
			var pu = document.getElementById("compTypechng").innerHTML = sc;
		
		}
		else if(document.fm.compType.value == '<%=Constant.COMP_TYPE_TYPE_09%>'){
			var sc = genSelBoxStrExp('compType2',<%=Constant.SEDAN_ACTIVITY%>,"<c:out value="${complaintMap.COMP_TYPE}"/>",false,"min_sel","","false",'');
			var pu = document.getElementById("compTypechng").innerHTML = sc;
		
		}else if(document.fm.compType.value == '<%=Constant.COMP_TYPE_TYPE_10%>'){
			var sc = genSelBoxStrExp('compType2',<%=Constant.SEDAN_OTHER_PROBLEMS%>,"<c:out value="${complaintMap.COMP_TYPE}"/>",false,"min_sel","","false",'');
			var pu = document.getElementById("compTypechng").innerHTML = sc;
