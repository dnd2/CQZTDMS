...
<script type="text/javascript">
 ...
 function addSure() {
	 ...
	 var url = "<%=contextPath%>/groups/DivideGroupsAction/groupsAdd.json" ;
...
<body> 
<div>
	<div class="navigation"><img src="<%=contextPath%>/img/nav.gif" />&nbsp;当前位置：  整车销售 &gt; 分组管理 &gt; 新增分组</div>
		<form id="fm" name="fm" method="post">
			<table class="table_query">
				<tr>
					<td align="right"><label for="groupsName">组名称：</label></td>
					<td><input type="text" class="middle_txt" name="groupsName" id="groupsName" value="" datatype="0,is_null,25" maxlength="25" /></td>
					<td align="right"><label for="groupsType">组类型：</label></td>
					<td>
						<script type="text/javascript">
							genSelBoxExp("groupsType",<%=Constant.DIVIDE_GROUP_TYPE%>,"",true,"short_sel","onchange=\"clrArray() ;changeType() ;\"","false",'') ;
						</script>
					</td>
					<td></td>
				</tr>
			...
			<div id="groupsDtl" style="display:none">
				...
				<tr>
					...
					<td>
						...
						<input type="button" class="normal_btn" name="queryBtn" id="queryBtn" value="查 询" onclick="changeType() ;" />
					</td>
				</tr>
				</table>
...
<script type="text/javascript">
	...
	function changeType() {
		...
		if(parseInt(document.getElementById("groupsType").value) == <%=Constant.DIVIDE_GROUP_TYPE_DLR_COMPANY%>) {
			...
			url = "<%=contextPath%>/groups/DivideGroupsAction/dlrCompanyQuery.json" ;
			columns = [
