...
<title>提交问题查询（经销商）</title>
</head>
<body>
<div class="navigation"><img src="<%=request.getContextPath()%>/img/nav.gif" />&nbsp;当前位置：帮助答疑 > 帮助答疑 > 提交问题查询（经销商）</div>
<form id="fm" name="fm">
...
<tr>
	<td nowrap="nowrap" align="right">
		...
		genSelBox("questionClass_su",<%=Constant.QUETION_TYPE%>,"","true");
		</script>
	</td>
</tr>
...
<tr>
		...
		<td align="left">
		<input type="text" id="questionDesc_su" name="questionDesc_common" value="" class="normal_txt" size="40" maxlength="30"/>&nbsp;
...
<script type="text/javascript" >
	var myPage;
	//查询路径 
	var url = "<%=contextPath%>/help/questionSolve/QuestionReported/getSaveUpdate.json";
	var title = null;
	var columns = [
...
function getQuesDes(value,metaDate,record){
	var sQuesDes = document.getElementById("questionDesc_su").value ;
...
function myLink(value,meta,record){
		...
		return String.format(
				"<a href=\"#\" onclick='viewDetail(\""+qid+"\")'>[明细]</a> <a href=\"#\" onclick='deleteSave(\""+qid+"\")'>[删除]</a>"
		)
}

	function deleteSave(){
		document.fm.action='<%=request.getContextPath()%>/help/questionSolve/QuestionReported/deleteSave.do';
		document.fm.submit();
		}

	function viewDetail(qid){
	var	url="<%=contextPath%>/help/questionSolve/QuestionReported/saveUpdate.do?QID="+qid;
