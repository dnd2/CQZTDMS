...
<script type="text/javascript" src="<%=request.getContextPath()%>/js/crm/defeat/defeatRediect.js"></script>
</head>
<body onload="loadcalendar();">
<div class="wbox">
<div class="navigation"><img src="<%=contextPath%>/img/nav.gif" />&nbsp;2当前位置： 潜客管理 &gt; 战败/失效管理 &gt; 战败/失效审核</div>
<form id="fm" name="fm" method="post">
<input  type="hidden" id="curPaths" value="<%=contextPath%>"/>
<input type="hidden" name="defeatId" id="defeatId" value="${tpc.leadsCode}" />
...
<!--				<input type="text" class="middle_txt" name="groupName" id="name" value="${tc.codeDesc}" datatype="0,is_textarea,30" disabled />	-->
		...
		<td align="center" colspan="6">
			<input type="button" class="normal_btn" onclick="auditPassRedirect();" value="通  过" id="auditSub" />
	...
	<hr />
			<center>
				...
				<select name="adviser" id="adviser">
	         	 	<c:forEach items="${userList}" var="po">
	         	 		<c:if test="${po.USER_ID==tpap.adviser}"><option value="${po.USER_ID}" selected="selected" >${po.NAME}</option></c:if>
	         	 		<c:if test="${po.USER_ID!=tpap.adviser}"><option value="${po.USER_ID}"  >${po.NAME}</option></c:if>
	         	 	</c:forEach>
	           </select>
	           	意向等级：
           ...
           <center>
			...
			<%-- <table class="table_query"  width="95%" align="center"  style="display: none"  id="groupRadio">
					<tr>
						<td align="center" >
							<center>
							<input type="radio"  id="follow_radio" name="group_radio" onclick="followClick()"/>跟进
