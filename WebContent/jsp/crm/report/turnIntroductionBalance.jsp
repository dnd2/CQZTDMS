...
function getDealerList(dealer_id,flag){
	...
	var openUrl='<%=contextPath%>/crm/report/TurnIntroductionReport/getDealerList.do'+parameter;
...
<table border="0" cellpadding="3" cellspacing="0" width="100%" class="scrollTable">
	...
	<tr class="scrollColThead">
		...
		<th   class="scrollRowThead scrollCR" title='指定时间内、产生的老客户介绍数量'>老客户转介绍数</th>
		<th   class="scrollRowThead scrollCR" title='订单数量除以老客户介绍数量' >老客户转介绍率</th>	
		<th   class="scrollRowThead scrollCR" title='指定时间内、产生的朋友转介绍数量' >朋友转介绍数</th>	
		<th   class="scrollRowThead scrollCR" title='订单数量除以朋友转介绍数量' >朋友转介绍率</th>
	</tr>
		<c:forEach items="${balanceList}" var="list">
		 <tr >
			...
			<td nowrap >${list.LKCOUNT}&nbsp; </td>
			<td nowrap >${list.LKRATE}&nbsp; </td>
			<td nowrap >${list.QBCOUNT}&nbsp; </td>
			<td nowrap >${list.QBRATE}&nbsp; </td>
		 </tr>
		</c:forEach>
</table>
</div>
