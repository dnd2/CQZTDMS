...
function getDealerList(dealer_id,flag){
	...
	var openUrl='<%=contextPath%>/crm/report/orderFormRecordReport/getDealerList.do'+parameter;
...
<table border="0" cellpadding="3" cellspacing="0" width="100%" class="scrollTable">
	...
	<tr class="scrollColThead">
		...
		<th   class="scrollRowThead scrollCR" title='指定时间内、产生的订单数量' >新增订单</th>
		<th   class="scrollRowThead scrollCR" title='指定时间内、产生的退单数量' >新增退单</th>
		<th   class="scrollRowThead scrollCR" title='指定时间内、产生的订单总计数量' >订单数合计</th>
		...
		<c:forEach items="${balanceList}" var="list">
		 <tr >
			...
			<td nowrap >${list.DDSCOUNT}&nbsp; </td>
			<td nowrap >${list.TDSCOUNT}&nbsp; </td>
			<td nowrap >${list.DDHJCOUNT}&nbsp; </td>
		 </tr>
		</c:forEach>
</table>
</div>
