...
<body onunload='javascript:destoryPrototype();  ' onload="__extQuery__(1);loadcalendar();">
	...
	var columns =null;
	var type=${dutyType };
	if(type=="10431003" || type=="10431004"){
	 columns = [
	...
	function myLink(value,meta,record){
		if(record.data.ORDER_STATUS =="修改锁定" || record.data.ORDER_STATUS =="退单锁定" || ((record.data.ORDER_STATUS =="修改锁定待车厂审核" || record.data.ORDER_STATUS =="退单锁定待车厂审核")&& dutyType!="10431005")) {
	...
	function checkAuditUrl(orderId,orderStatus){
		window.location.href="<%=contextPath%>/crm/order/OrderManage/orderAuditInit.do?orderId="+orderId+"&orderStatus="+orderStatus;
	}
	</script>
