...
<script type="text/javascript">
	...
	function ifInviteChange(){
		...
		var shopDateStyle = document.getElementById("shop_date_style");
		if(ifInvite=='10041001') {//邀约成功
			...
			shopDateStyle.style.display = "block";
		} else {//邀约失败
			...
			orderTable.style.display = "none";
			failureTable.style.display = "none";
			followRadio.checked = true;
			shopDateStyle.style.display = "none";
		}
	}
	function doSave(){
	   ...
	   // } 
		...
		if(ifInvite=='10041001') {//邀约成功
			if(inviteShopDate==null||inviteShopDate=="") {
				...
				$('fm').action = "<%=contextPath%>/crm/taskmanage/TaskManage/doTask.do?if_invite=10041001&taskType=invite&typeFlag=''&customerId="+customerId+"&taskId="+taskId;
				$('fm').submit();
			}
		}
		if(followRadio.checked == true) {
			...
			if(nextFollowDate==null||nextFollowDate=="") {
				...
				document.getElementById("saveButton").disabled = true;
				document.getElementById("insertBtn").disabled = true;
				$('fm').action = "<%=contextPath%>/crm/taskmanage/TaskManage/doTask.do?taskType=invite&typeFlag=followRadio&customerId="+customerId+"&taskId="+taskId;
				$('fm').submit();
			}
			...
			document.getElementById("saveButton").disabled = true;
			document.getElementById("insertBtn").disabled = true;
			$('fm').action = "<%=contextPath%>/crm/taskmanage/TaskManage/doTask.do?taskType=invite&typeFlag=inviteRadio&customerId="+customerId+"&taskId="+taskId;
			$('fm').submit();
			...
			document.getElementById("saveButton").disabled = true;
			document.getElementById("insertBtn").disabled = true;
			$('fm').action = "<%=contextPath%>/crm/taskmanage/TaskManage/doTask.do?taskType=invite&typeFlag=orderRadio&customerId="+customerId+"&taskId="+taskId;
			$('fm').submit();
			...
			document.getElementById("saveButton").disabled = true;
			document.getElementById("insertBtn").disabled = true;
			$('fm').action = "<%=contextPath%>/crm/taskmanage/TaskManage/doTask.do?taskType=invite&typeFlag=defeatRadio&customerId="+customerId+"&taskId="+taskId;
			$('fm').submit();
			...
			document.getElementById("saveButton").disabled = true;
			document.getElementById("insertBtn").disabled = true;
			$('fm').action = "<%=contextPath%>/crm/taskmanage/TaskManage/doTask.do?taskType=invite&typeFlag=failureRadio&customerId="+customerId+"&taskId="+taskId;
			$('fm').submit();
		}
	}
...
<body onunload='javascript:destoryPrototype();'>
	<div class="wbox">
		<div class="navigation">
			<img src="<%=contextPath%>/img/nav.gif" />&nbsp;当前位置>潜客管理>日程管理>任务管理>计划邀约任务
		</div>
		<form id="fm" name="fm" method="post">
			...
			<c:if test="${directorAudit=='60191002' }">
			<div>
				<b style="display: inline-block; float: left">主管审核意见</b><hr style="display: inline-block;">
			</div>
			<br>
			<table class="table_query" width="95%" align="center">
				<tr id="yaoyuejihuax">
					<td align="center" colspan="1">需求分析</td>
					<td align="center" colspan="1">邀约目标</td>
					<td align="center" colspan="1">赢得客户信任设计</td>
					<td align="center" colspan="1">感动客户情景设计</td>
				</tr>
				<tr id="yaoyuejihuax2">
					<td align="center" colspan="1"><textarea rows="10" cols="30" id="xqfx2" name="xqfx2" readonly="readonly" style="background-color: #EEEEEE;">${x1 }</textarea></td>
					<td align="center" colspan="1"><textarea rows="10" cols="30" id="yymb2" name="yymb2" readonly="readonly" style="background-color: #EEEEEE;">${x2 }</textarea></td>
					<td align="center" colspan="1"><textarea rows="10" cols="30" id="ydkhxrsj2" name="ydkhxrsj2" readonly="readonly" style="background-color: #EEEEEE;">${x3 }</textarea></td>
					<td align="center" colspan="1"><textarea rows="10" cols="30" id="gdkhqjsj2" name="gdkhqjsj2" readonly="readonly" style="background-color: #EEEEEE;">${x4 }</textarea></td>
				</tr>
				<tr>
					<td align="right" colspan="1">主管审核建议：</td>
					<td align="left" colspan="3">
						<textarea rows="5" cols="70" id="audit_info" name="audit_info" readonly="readonly" style="background-color: #EEEEEE;">${auditRemark }</textarea>
					</td>
				</tr>
			</table>
			<br>
			...
			<div>
				<b style="display: inline-block; float: left">邀约内容</b><hr style="display: inline-block;">
			</div>
			<br>
			<table class="table_query" width="95%" align="center">
			<!-- 二级级联菜单 -->
			...
			<c:forEach items="${customerList }" var="customerList">
				<tr>
					<td align="right" width="6%">客户姓名：</td>
					<td width="14%"><input id="customer_name" name="customer_name" type="text" readonly="readonly" style="background-color: #EEEEEE;"
					...
					<td width="19%">
					<input type="hidden" id="invite_type" name="invite_type" value="${inviteType }"/>
		      			<div id="ddtopmenubar29" class="mattblackmenu">
						<ul> 
							<li>
								<a style="width:103px;" rel="ddsubmenu29" href="###" isclick="true" onclick="stree.loadtree(this, '<%= request.getContextPath() %>/crm/data/DataManage/initData.json?codeId=6047', loadInviteType);" deftitle="--请选择--">
								<c:if test="${inviteType==null}">--请选择--</c:if><c:if test="${inviteType!=null}">${inviteType2}</c:if></a>
								<ul id="ddsubmenu29" class="ddsubmenustyle"></ul>
							</li>
						</ul>
					</td>
					<td align="right" width="8%">邀约类型：</td>
					<td>
					<input type="hidden" id="invite_way" name="invite_way" value=""/>
		      			<div id="ddtopmenubar28" class="mattblackmenu">
						<ul> 
							<li>
								<a style="width:103px;" rel="ddsubmenu28" href="###" isclick="true" onclick="stree.loadtree(this, '<%= request.getContextPath() %>/crm/data/DataManage/initData.json?codeId=6021', loadInviteWay);" deftitle="--请选择--">
								--请选择--</a>
				...
				<tr>
					<td align="right" width="10%">意向等级：</td>
					<td width="14%">
					...
					<td align="right" width="9%">销售流程进度：</td>
					<td width="12%">
					...
					<td><select id="if_invite2" onchange="ifInviteChange()">
				...
				<tr id="shop_date_style">
					<td width="10%" align="right">预约到店时间：</td>
					<td width="14%">
						<div align="left">
							<input type="text" value="" name="invite_shop_date" id="invite_shop_date" group="invite_shop_date"
				...
				<tr>
					<td align="right" colspan="1" width="6%">邀约结果：</td>
					<td align="left" colspan="5">
						<textarea rows="5" cols="70" id="invite_info" name="invite_info"></textarea>
					</td>
				</tr>
				<tr>
			...
			<table class="table_query" width="95%" align="center" id="groupRadio" style="display: none">
				<tr>
					<td align="right" width="20%">
						<input type="radio"  id="follow_radio" name="group_radio" onclick="followClick()">跟进</input>
