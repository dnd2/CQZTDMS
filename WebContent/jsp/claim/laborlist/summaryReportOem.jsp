    ...
    <table  class="table_query">
		...
		<tr>
			...
			<td width="20%" align="left">
				<div id="div2">
					<script>
						genSelBoxExp("status","<%=Constant.TAXABLE_SERVICE_SUM%>","<%=Constant.TAXABLE_SERVICE_SUM_WAIT%>",true,"short_sel","","false","");
					</script>
				</div>
			</td>
...
<script type="text/javascript">
var url = '<%=contextPath%>/claim/laborlist/LaborListSummaryReport/mainQuery.json' ;
var title = null ;
var columns = [
...
function myHandler(value,meta,record){
	if(record.data.AUTH_STATUS == '11881000'){
	  return "<a href='#' onclick='getInvoice(\""+record.data.SUM_PARAMETER_ID+"\",\""+1+"\")'>[收票]</a>|<a href="+"<%=contextPath%>/claim/laborlist/LaborListSummaryReport/showDetailOem.do?id="+record.data.SUM_PARAMETER_ID+"&code="+record.data.SUM_PARAMETER_NO+">[明细]</a>";
	}else if(record.data.AUTH_STATUS == '11881001'){
	  return "<a href='#' onclick='getInvoice(\""+record.data.SUM_PARAMETER_ID+"\",\""+2+"\")'>[财务挂账]</a>|<a href="+"<%=contextPath%>/claim/laborlist/LaborListSummaryReport/showDetailOem.do?id="+record.data.SUM_PARAMETER_ID+"&code="+record.data.SUM_PARAMETER_NO+">[明细]</a>";
	}else {
	  return '<a href="<%=contextPath%>/claim/laborlist/LaborListSummaryReport/showDetailOem.do?id='+record.data.SUM_PARAMETER_ID+'&code='+record.data.SUM_PARAMETER_NO+'">[明细]</a>' ;
	  }
}
