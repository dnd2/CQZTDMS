...
<form method="post" name ="fm" id="fm">
    ...
    <table  class="table_query">
		...
		<tr>
			...
			<td width="20%" align="left">
				<input type="text" name = "report_amount" id="report_amount"/>
				<input type="hidden" name = "report_amount1" id="report_amount1" value="0"/>
			</td>
		</tr>
		<tr>
...
function setNotice(no,amounts,dlr_id,code,name,b_id){
	...
	for(var i=0;i<no.length;i++){
		...
		if(flag){
			...
			$('report_amount1').value=round($('report_amount1').value*1+amounts[i]*1,2);
	...
	$('report_amount').value = $('report_amount1').value;
}
...
function delRow(obj,value){
	...
	$('report_amount1').value=round($('report_amount1').value*1-value*1,2);
	$('report_amount').value = $('report_amount1').value
}
...
function saveAdd(){
	if(Number($('report_amount').value)>Number($('report_amount1').value)){
		MyAlert('发票金额改大!');
		return;
	}
...
function sureAdd(){
	$('fm').action = '<%=contextPath%>/claim/laborlist/LaborListAction/save11.do' ;
	$('fm').submit();
}
</script>
</BODY>
