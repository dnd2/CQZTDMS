...
<input type="hidden" name="sum_id" id="sum_id" value="<%=map.get("SUM_PARAMETER_ID")%>"/>
    <table  class="table_query">
		...
		<tr>
			...
			<td width="20%" align="left">
				<input type="text" name="reportAmount" id="reportAmount" value="${reportAmount }" />
			</td>
		</tr>
	</table>
	<br />
	...
	<table class="table_edit">
		<tr>
			<td colspan="4" align="center">
				<input type="button" value="收票" id="_btn1" class="normal_btn" onclick="getInvoice(1);"/>
				&nbsp;
				<input type="button" value="财务挂账" id="_btn2" class="normal_btn" onclick="getInvoice(2)"/>
				&nbsp;
...
function getInvoice(value){
	...
	if($('reportAmount').value=="" || $('reportAmount').value==null){
		MyAlert('发票金额不能为空!');
		return;
	}
	var id = $('sum_id').value ;
	var url = '<%=contextPath%>/claim/laborlist/LaborListSummaryReport/getInvoice11.json?id='+id+'&flag='+value ;
	makeNomalFormCall(url,getCallback,'fm');
}
function getCallback(json){
	if(json.SUCCESS=='LOCK'){
		MyAlert("其他用户正在收票，请稍候再收票!");
		return;
	}
	if(json.flag){
		MyAlert('操作已成功！');
