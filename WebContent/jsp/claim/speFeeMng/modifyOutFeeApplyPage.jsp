 ...
 <form method="post" name ="fm" id="fm">
  ...
  <table width=100% border="0" align="center" cellpadding="1" cellspacing="1" bgcolor="91908E"  class="table_edit">
     ...
     <tr bgcolor="F3F4F8">
       ...
       <td>
          <input name="out_start_date" id="out_start_date" value="<%=CommonUtils.checkNull(detailSpeBean.getStart_date())%>" type="text" class="short_txt" datatype="0,is_date,10" group="out_start_date,out_end_date" hasbtn="true" callFunction="showcalendar(event, 'out_start_date', false);" blurback="true">
       </td>
       <td align="right">外出结束日期：</td>
       <td>
          <input name="out_end_date" id="out_end_date" value="<%=CommonUtils.checkNull(detailSpeBean.getEnd_date())%>" type="text" class="short_txt" datatype="0,is_date,10" group="out_start_date,out_end_date" hasbtn="true" callFunction="showcalendar(event, 'out_end_date', false);" blurback="true">
       </td>
     ...
     <tr bgcolor="F3F4F8">
       ...
       <td>
          <input type="text" name="out_person_num" id="out_person_num" value="<%=CommonUtils.checkNull(detailSpeBean.getPerson_num())%>" datatype="0,isDigit,5" class="short_txt"/>
       </td>
       <td align="right">出差人员：</td>
       <td>
          <textarea id="out_name" name="out_name" rows="2" cols="20" datatype="0,is_textarea,60"><%=CommonUtils.checkNull(detailSpeBean.getPerson_name())%></textarea>
       </td>
       <td align="right">总里程：</td>
       <td><input type="text" name="single_mileage" id="single_mileage" value="<%=CommonUtils.checkNull(detailSpeBean.getSingle_mileage())%>" datatype="0,isMoney,10" class="short_txt"/></td>
     </tr>
     <tr bgcolor="F3F4F8">
       <td align="right">过路过桥费：</td>
       <td>
          <input type="text" name="road_bridge_fee" id="road_bridge_fee" value="<%=CommonUtils.checkNull(detailSpeBean.getPass_fee())%>" datatype="0,isMoney,10" blurback="true" class="short_txt"/>
       </td>
       <td align="right">车辆或交通补助：</td>
       <td>
          <input type="text" name="vehicle_extra_fee" id="vehicle_extra_fee" value="<%=CommonUtils.checkNull(detailSpeBean.getTraffic_fee())%>" datatype="0,isMoney,10" blurback="true" class="short_txt"/>
       </td>
       <td align="right">住宿费：</td>
       <td>
          <input type="text" name="lodging_fee" id="lodging_fee" value="<%=CommonUtils.checkNull(detailSpeBean.getQuarter_fee())%>" datatype="0,isMoney,10" blurback="true" class="short_txt"/>
       </td>
     </tr>
     <tr bgcolor="F3F4F8">
       <td align="right">餐补费：</td>
       <td>
          <input type="text" name="eat_extra_fee" id="eat_extra_fee" value="<%=CommonUtils.checkNull(detailSpeBean.getEat_fee())%>" datatype="0,isMoney,10" blurback="true" class="short_txt"/>
       </td>
       <td align="right">人员补助：</td>
       <td>
          <input type="text" name="person_extra_fee" id="person_extra_fee" value="<%=CommonUtils.checkNull(detailSpeBean.getPerson_subside())%>" datatype="0,isMoney,10" blurback="true" class="short_txt"/>
       </td>
       <td align="right">总费用：</td>
       <td>
          <input type="text" name="total_fee" id="total_fee" value="<%=CommonUtils.checkNull(detailSpeBean.getTotal_fee())%>" class="short_txt" readonly="readonly"/>
       </td>
     </tr>
     <tr bgcolor="F3F4F8">
       <td align="right">申请内容：</td>
       <td colspan="3">
          <textarea id="apply_content" name="apply_content" rows="4" cols="50" datatype="0,is_null,600"><%=CommonUtils.checkNull(detailSpeBean.getApply_content())%></textarea>
       </td>
     </tr>
  </table>
  <!-- 展示附件 -->
  ...
  <table width=100% border="0" align="center" cellpadding="1" cellspacing="1" bgcolor="91908E"  class="table_edit">
     ...
     <tbody id="itemTable">
      <%for(int count=0;count<vehicleList.size();count++){
       ...
       <tr id="tr_row<%=count%>">
         <td id="vin_row<%=count%>">
            <input type="text" id="vin<%=count%>" value="<%=CommonUtils.checkNull(vehicleInfo.getVin())%>" class="middle_txt" readonly="readonly" onclick="showVIN(<%=count%>)"/>
         </td>
         <td id="engine_no<%=count%>">
            <%=CommonUtils.checkNull(vehicleInfo.getEngine_no())%><input type="hidden" name="engine_no<%=CommonUtils.checkNull(vehicleInfo.getVin())%>" id="engine_no<%=CommonUtils.checkNull(vehicleInfo.getVin())%>" value="<%=CommonUtils.checkNull(vehicleInfo.getEngine_no())%>"/>
         </td>
         <td id="vehicle_type<%=count%>">
            <%=CommonUtils.checkNull(vehicleInfo.getModel())%><input type="hidden" name="vehicle_type<%=CommonUtils.checkNull(vehicleInfo.getVin())%>" id="vehicle_type<%=CommonUtils.checkNull(vehicleInfo.getVin())%>" value="<%=CommonUtils.checkNull(vehicleInfo.getModel())%>"/>
         </td>
         <td id="factory_date<%=count%>">
            <%=CommonUtils.checkNull(vehicleInfo.getProduct_date())%><input type="hidden" name="factory_date<%=CommonUtils.checkNull(vehicleInfo.getVin())%>" id="factory_date<%=CommonUtils.checkNull(vehicleInfo.getVin())%>" value="<%=CommonUtils.checkNull(vehicleInfo.getProduct_date())%>"/>
         </td>
         <td id="vin_mileage<%=count%>">
            <input type="text" id="vin_mileage<%=CommonUtils.checkNull(vehicleInfo.getVin())%>" name="vin_mileage<%=CommonUtils.checkNull(vehicleInfo.getVin())%>" class="short_txt" datatype="1,is_digit,5" value="<%=CommonUtils.checkNull(vehicleInfo.getMileage())%>"/>km
         </td>
         <td id="customer_name<%=count%>">
            <input type="text" id="customer_name<%=CommonUtils.checkNull(vehicleInfo.getVin())%>" name="customer_name<%=CommonUtils.checkNull(vehicleInfo.getVin())%>" class="short_txt" datatype="1,is_letter_cn,30" value="<%=CommonUtils.checkNull(vehicleInfo.getCustomer_name())%>"/>
         </td>
         <td id="customer_phone<%=count%>">
            <input type="text" id="customer_phone<%=CommonUtils.checkNull(vehicleInfo.getVin())%>" name="customer_phone<%=CommonUtils.checkNull(vehicleInfo.getVin())%>" class="short_txt" datatype="1,is_phone,30" value="<%=CommonUtils.checkNull(vehicleInfo.getCustomer_phone())%>"/>
         </td>
         <td id="sale_date<%=count%>">
            <%=CommonUtils.checkNull(vehicleInfo.getSale_date())%><input type="hidden" name="sale_date<%=CommonUtils.checkNull(vehicleInfo.getVin())%>" id="sale_date<%=CommonUtils.checkNull(vehicleInfo.getVin())%>" value="<%=CommonUtils.checkNull(vehicleInfo.getSale_date())%>"/>
         </td>
         <td id="remark<%=count%>">
            <textarea id="remark<%=CommonUtils.checkNull(vehicleInfo.getVin())%>" name="remark<%=CommonUtils.checkNull(vehicleInfo.getVin())%>" rows="1" cols="15" datatype="1,is_textarea,200"><%=CommonUtils.checkNull(vehicleInfo.getRemark())%></textarea>
         </td>
         <td>
            <input type="button" class="normal_btn"  value="删除"  name="delBtn<%=count%>" id="delBtn<%=count%>" onclick="delItem(<%=count%>)"/>
         </td>
       </tr>
      <%}%>
	 </tbody>
    ...
    <tr > 
      <th height="12" align=center>
       <input type="button" onclick="save();" class="normal_btn" style="width=8%" value="保存"/>
        &nbsp;&nbsp;
       <input type="button" onclick="report();" class="normal_btn" style="width=8%" value="上报"/>
        &nbsp;&nbsp;
       <input type="button" onclick="askClose();" class="normal_btn" style="width=8%" value="返回"/></th>
    </tr>
  </table>
  <!-- 资料显示区结束 -->
...
function addVehicleInfo(){
	...
	addTable.rows[length].cells[0].innerHTML =  "<td id='vin_row"+length+"'><input type='text' id='vin"+length+"' value='' class='middle_txt' readonly='readonly' onclick='showVIN("+length+")'>";
	...
	addTable.rows[length].cells[9].innerHTML = "<input type='button' class='normal_btn' id='delBtn"+length+"' value='删除'  name='delBtn"+length+"' onClick='javascript:delItem("+length+");'/>";
}
//获取VIN的方法 调用索赔单维护VIN显示
function showVIN(row_id){
	var cr_id='<%=beanInfo.getId()%>';
...
function setVIN(row_id,VIN,LICENSE_NO,MODEL_NAME,SERIES_NAME,REARAXLE_NO,GEARBOX_NO,ENGINE_NO,COLOR,PRODUCT_DATE,PURCHASED_DATE,CUSTOMER_NAME,CERT_NO,MOBILE,ADDRESS_DESC,HISTORY_MILE,MODEL_ID,BRAND_NAME){
   ...
   if(flag=="1"){
	...
	document.getElementById("vin_mileage"+row_id).innerHTML = '<input type="text" id="vin_mileage'+VIN+'" name="vin_mileage'+VIN+'" datatype="0,is_digit,5" class="short_txt" value="'+HISTORY_MILE+'"/>km';
	document.getElementById("customer_name"+row_id).innerHTML = '<input type="text" id="customer_name'+VIN+'" name="customer_name'+VIN+'" datatype="0,is_letter_cn,30" class="short_txt" value="'+CUSTOMER_NAME+'"/>';
	document.getElementById("customer_phone"+row_id).innerHTML = '<input type="text" id="customer_phone'+VIN+'" name="customer_phone'+VIN+'" datatype="0,is_phone,30" class="short_txt" value="'+v_MOBILE+'"/>';
	...
	document.getElementById("remark"+row_id).innerHTML = '<textarea "remark'+VIN+'" name="remark'+VIN+'" datatype="0,is_textarea,200" rows="1" cols="15" ></textarea>';
	//将新添加的vin缓存到vin_str中
...
function delItem(row_id){
	...
	var cr_id='<%=beanInfo.getId()%>';
    //删除
	var submit_url="<%=contextPath%>/claim/speFeeMng/OutFeeApplyManager/delVinInfo.json?del_vin="+del_vin+"&row_id="+row_id+"&xh_ord_id="+cr_id;
...
//更新列表
function updateMyTable(row_id){
	var addTable = document.getElementById("itemTable");
	var history_length=addTable.rows.length;//删除前列表长度
	var del_row_id=row_id;//删除的row_id
	//删除该行
	addTable.removeChild(addTable.rows[del_row_id]);
	//改变列的值
	for(var i=0;i<history_length;i++){
       if(i>del_row_id){
          var updateRow=i-1;
          document.getElementById("tr_row"+i).id="tr_row"+updateRow;
          document.getElementById("vin_row"+i).id="vin_row"+updateRow;
          document.getElementById("vin"+i).onclick="showVIN('"+updateRow+"')";
          document.getElementById("vin"+i).id="vin"+updateRow;
          document.getElementById("engine_no"+i).id="engine_no"+updateRow;
          document.getElementById("vehicle_type"+i).id="vehicle_type"+updateRow;
          document.getElementById("factory_date"+i).id="factory_date"+updateRow;
          document.getElementById("vin_mileage"+i).id="vin_mileage"+updateRow;
          document.getElementById("customer_name"+i).id="customer_name"+updateRow;
          document.getElementById("customer_phone"+i).id="customer_phone"+updateRow;
          document.getElementById("sale_date"+i).id="sale_date"+updateRow;
          document.getElementById("remark"+i).id="remark"+updateRow;
          document.getElementById("delBtn"+i).id="delBtn"+updateRow;
          document.getElementById("delBtn"+i).name="delBtn"+updateRow;
          document.getElementById("delBtn"+i).onclick="delItem('"+updateRow+"')";
          document.getElementById("delBtn"+i).value="删除";
       }       
	}
}
//将VIN从vin_str去除
...
function doInit(){
	loadcalendar();
	vin_str ='<%=vehicleStr.substring(0,vehicleStr.length()-1)%>';
	var out_start_date=document.getElementById("out_start_date").value;
	var out_end_date=document.getElementById("out_end_date").value;
	calBetweenDays(out_start_date,out_end_date);
}
