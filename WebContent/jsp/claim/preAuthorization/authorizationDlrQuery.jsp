...
<SCRIPT LANGUAGE="JavaScript">
	var myPage;
	//查询路径
	var url = "<%=contextPath%>/claim/preAuthorization/Authorization/authorizationDlrQuery.json";
	...
	function myLink1(value,meta,record){
		...
		if('<%=Constant.RO_FORE_01%>'==record.data.REPORT_STATUS || '<%=Constant.RO_FORE_02%>'==record.data.REPORT_STATUS ||'<%=Constant.RO_FORE_05%>'==record.data.REPORT_STATUS ||'<%=Constant.RO_FORE_06%>'==record.data.REPORT_STATUS){
			...
			url2+="<a href='#' onClick='printRenge(\""+value+"\",\""+VIN+"\");'>[打印]</a>";
			return String.format(url2);
		}
	}
	function printRenge(foId,vin){
		window.open('<%=contextPath%>/AuthorizationAction/authorizationPrint.do?foId='+foId+'&vin='+vin,"打印预授权", "height=700, width=1024, top=0, left=0, toolbar=no, menubar=no, scrollbars=yes, resizable=yes,location=no, status=no");
	}
	function queryPer(){
...
<BODY onload="doInit()">
<div class="navigation"><img src="<%=request.getContextPath()%>/img/nav.gif" />&nbsp;当前位置：售后服务管理&gt;索赔预授权&gt;索赔单预授权查询</div>
