...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<BODY >
		...
		<form method="post" name="fm" id="fm">
			...
			<input type = "hidden" value = "" id ="SAVEORCOMIT" name ="SAVEORCOMIT"/>
				...
				<tr>
					...
					<td>
						<input type='text' name='vin' id='vin'     maxlength="18"
							 class="middle_txt" value ="${vinInfo.VIN }"
				...
				<td align="right" class="table_edit_3Col_label_5Letter">
					...
					<td>
						<input type='text' name='MILEAGE' id='MILEAGE'
							 class="middle_txt" datatype= "0,isDigit,10" value ="${foInfoMap.IN_MILEAGE }" />
					</td>
			...
			<table id="itemTableId" border="0" align="center" cellpadding="0"
				...
				<tbody id="itemTable">
					...
					<td>
						...
						genSelBoxExp("REPAIR_TYPE",<%=Constant.REPAIR_TYPE%>,"${foInfoMap.APPROVAL_TYPE }",false,"min_sel","","false",'<%=Constant.REPAIR_TYPE_04%>,<%=Constant.REPAIR_TYPE_05%>,<%=Constant.REPAIR_TYPE_08%>');
				       	</script>
					</td>
					<td>
					...
					<td>
						<input type="text" name="MAKER_CODE" class="short_txt"   id="MAKER_CODE" value = "${foInfoMap.MAKER_CODE }" datatype="0,is_null" readonly="readonly" /><input type="button" onClick="choose_producer_code();" id="chooseProducer" name="chooseProducer"  class="mini_btn" value="..." />
					</td>
					<td>
						<input type="text" name="MAX_AMOUNT" class="short_txt"   id="MAX_AMOUNT" value = "${foInfoMap.MAX_AMOUNT }" datatype="1,isMoney,10" />
					</td>
					<td>
						<script type="text/javascript">
						genSelBox("IS_RETURN",<%=Constant.IS_RETURN%>,"${foInfoMap.IS_RETURN }",false,"min_sel","","false",'');
				       	</script>
					</td>
					<td>
			...
			<table border="0" align="center" cellpadding="1" cellspacing="1"
				...
				<tr>
					...
					<td  class="tbwhite">
						<textarea name="ERROR_DESC" id="errorDesc" style="font-weight: bold;" datatype="1,is_textarea,1000" maxlength="1000" rows='3' cols='28'>${foInfoMap.ERROR_DESC }</textarea>
					</td>
					...
					<td  class="tbwhite">
						<textarea name='ERROR_REASON' style="font-weight: bold;" id="errorReason"	datatype="1,is_textarea,1000" maxlength="1000" rows='3' cols='28'>${foInfoMap.ERROR_REASON }</textarea>
					</td>
					<td  class="tbwhite">
						处理方案：
					</td>
					<td  class="tbwhite">
						<textarea name='ERROR_RESULT'  style="font-weight: bold;"id='errorResult'	datatype="1,is_textarea,1000" maxlength="1000" rows='3' cols='28'>${foInfoMap.REMARK }</textarea>
					</td>
				</tr>
...
<!-- 					</td> -->
				...
				<tbody id="itemTableAccessories">
					<td><input type="text" name="APPLY_AMOUNT" class="middle_txt"   id="APPLY_AMOUNT" value="${foInfoMap.APPLY_AMOUNT }" datatype="1,isMoney,10"  maxlength="10" /></td>
					<td><input name="AUDIT_AMOUNT" id="AUDIT_AMOUNT"  type="text" readonly="readonly" class="middle_txt" value="${foInfoMap.AUDIT_AMOUNT }" datatype="1,isMoney,10" maxlength="10" /></td>
					<td><input name="APPLY_REMARK" id="APPLY_REMARK" value="${foInfoMap.APPLY_REMARK }" type="text" class="middle_txt" datatype="1,is_null,120"/></td>
				</tbody>
			</table>
...
<!-- 					</td> -->
				...
				<tbody id="itemTableAccessories">
					<td><input type="text" name="OUT_APPLY_AMOUNT" class="middle_txt"   id="OUT_APPLY_AMOUNT" maxlength="10" value = "${foInfoMap.OUT_APPLY_AMOUNT }" datatype="1,isMoney,10" /></td>
					<td><input type="text" name="OUT_AUDIT_AMOUNT" readonly="readonly" class="middle_txt"   id="OUT_AUDIT_AMOUNT" datatype="1,isMoney,10" value = "${foInfoMap.OUT_AUDIT_AMOUNT }"/></td>
					<td><input type="text" name="OUT_APPLY_REMARK" class="middle_txt"   id="OUT_APPLY_REMARK" value = "${foInfoMap.OUT_APPLY_REMARK }"  datatype="1,is_null,120" /> </td>
				</tbody>
			</table>
...
<!-- 		                <input class="normal_btn" type="button" id="btn3" value="保养历史" onclick="freeMaintainHistory();"/>&nbsp; -->
						...
						<c:if test="${ foInfoMap.REPORT_STATUS == '11561004'}">
							<input type="button" onClick="confirmUpdate('1');" id="audit_but" class="normal_btn"
							style="" value="保存" />&nbsp;
							<input type="button" onClick="confirmUpdate('2');" id="reject_but" class="normal_btn"
								style="" value="上报" />&nbsp;
						</c:if>
						
						<c:if test="${ foInfoMap.REPORT_STATUS == '11561003'}">
							<input type="button" onClick="confirmUpdate('3');" id="audit_but" class="normal_btn"
							style="" value="保存" />&nbsp;
							<input type="button" onClick="confirmUpdate('4');" id="reject_but" class="normal_btn"
								style="" value="上报" />&nbsp;
						</c:if>
						
						<input type="button" onClick="goBack();" id="back_but" name="back_but" class="normal_btn" class="normal_btn"
							style="" value="返回" />
					</td>
<!-- 					<td colspan="2"> -->
...
<!-- 					</td> -->
		...
		{
	   		loadcalendar();
	   		var repairType = '${foInfoMap.APPROVAL_TYPE }';
		...
		function confirmUpdate(obj){
			...
			if("1"== obj || "3"== obj){
				MyConfirm("是否保存修改？",updateLimit);
			}else{
				MyConfirm("是否上报？",updateLimit);
			}
		  ...
		  function updateLimit()
		  {
				...
				var url = "<%=contextPath%>/claim/preAuthorization/Authorization/updatePreAuthoriza.json";
				sendAjax(url,updateLimitBack,'fm');
		  }
		  
		  function updateLimitBack(json){
			if(json.succ=="1"){
