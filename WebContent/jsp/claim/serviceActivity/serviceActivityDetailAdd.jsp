...
function doInit()
{
   if(document.getElementById("activityType").value=="10561005"){
...
<table class="table_edit" >
  ...
  <tr>
    ...
    <td colspan="3" align="left"><textarea name="solution" id="solution" class="SearchInput" disabled="disabled" rows="8" style="width: 95%" datatype="0,is_digit_letter_cn,200"><%=ActivityBean.getSolution()==null?"":ActivityBean.getSolution()%></textarea></td>
  </tr>
  <tr>
    <td align="right">索赔申请指导：</td>
    <td colspan="3" align="left"><textarea name="claimGuide" id="claimGuide" class="SearchInput" disabled="disabled" rows="8" style="width: 95%" datatype="0,is_digit_letter_cn,200"><%=ActivityBean.getClaimGuide()==null?"":ActivityBean.getClaimGuide()%></textarea></td>
  </tr>
        	 ...
        	 <th colspan="8"  ><img src="../../../img/subNav.gif" alt="" class="nav" />
          	...
          	<tr>
					...
					<td class="tbwhite" colspan="3">
						<textarea name='TROUBLE_DESC' maxlength="100" disabled="disabled" readonly="readonly"	id='TROUBLE_DESC' rows='2' cols='28'><%=ActivityBean.getTroubleDesc() %></textarea>
					</td>
					<td class="table_edit_2Col_label_5Letter">
						故障原因：
					</td>
					<td colspan="3" class="tbwhite">
						<textarea name='TROUBLE_REASON'  disabled="disabled" id='TROUBLE_REASON' maxlength="100" readonly="readonly"  rows='2' cols='28'><%=ActivityBean.getTroubleReason()%></textarea>
					</td>
				</tr>
				<tr>
					<td class="table_edit_2Col_label_5Letter">
					...
					<td colspan="3" class="tbwhite">
						<textarea name='REPAIR_METHOD'  disabled="disabled" id='REPAIR_METHOD' readonly="readonly" maxlength="100" rows='2' cols='28'><%=ActivityBean.getRepairMethod()%></textarea>
					</td>
					<td class="table_edit_2Col_label_5Letter">
						申请备注：
					</td>
					<td colspan="3" class="tbwhite">
						<textarea name='APP_REMARK'  disabled="disabled" id='APP_REMARK'	 readonly="readonly" maxlength="100" rows='2' cols='28'><%=ActivityBean.getAppRemark()%></textarea>
					</td>
				</tr>
          </table>
	</div>
...
function open_conent(val)
	...
	function openModel(){
		...
		var url = '<%=contextPath%>/claim/serviceActivity/ServiceActivityManageModel/serviceActivityManageModeljuede.json?activityId='+${activityId}; 
		makeNomalFormCall(url,insertModel,'fm');
	}
	
	function insertModel(json)
	{
		if(json.ret == 'true')
		{
			OpenHtmlWindow('<%=contextPath%>/claim/serviceActivity/ServiceActivityManageModel/serviceActivityManageModelQuery.do?activityId='+${activityId}+"&type=1",800,500);
		}else
		{
		   MyAlert('车型已经是最大权限无需增加！');
		}
	}
	...
	function openMilage(){
		var url = '<%=contextPath%>/claim/serviceActivity/ServiceActivityManage/getActivityMilagejude.json?activityId='+${activityId}; 
		makeNomalFormCall(url,insertMilage,'fm');
		
	}
	function insertMilage(json)
	{
	    if(json.ret == 'true')
		{
			OpenHtmlWindow('<%=contextPath%>/claim/serviceActivity/ServiceActivityManage/getActivityMilage.do?activeId='+${activityId}+'&type=1',400,200);
		}else
		{
		   MyAlert('里程已经是最大权限无需增加！');
		}
		
	}
	function openVehicleAge(){
		var url = '<%=contextPath%>/claim/serviceActivity/ServiceActivityManageVehicleAge/serviceActivityManageVehicleAgejude.json?activityId='+${activityId}; 
		makeNomalFormCall(url,insertVehicleAge,'fm');
	}
	function insertVehicleAge(json)
	{
	    if(json.ret == 'true')
	    {
	    	var beforeVehicle ="";
			...
			OpenHtmlWindow('<%=contextPath%>/claim/serviceActivity/ServiceActivityManageVehicleAge/serviceActivityManageVehicleAgeQuery.do?activityId='+${activityId}+'&beforeVehicle='+beforeVehicle+'&type=1',500,300);
	    }else
	    {
	    	MyAlert('车龄为最大限制无需新增！');
	    }
	...
	function openCharactor()
	{
	   MyAlert('车辆性质范围已经最广无需修改');
	}
	
	
	function openProduceBase(){
	   
		var url = '<%=contextPath%>/claim/serviceActivity/ServiceActivityManageProduceBase/serviceActivityManageProduceBasejude.json?activityId='+${activityId}; 
		makeNomalFormCall(url,insertProduceBase,'fm');
		
		
	}
	function insertProduceBase(json)
	{
	    if(json.ret == 'true')
	    {
    		OpenHtmlWindow('<%=contextPath%>/claim/serviceActivity/ServiceActivityManageProduceBase/serviceActivityManageProduceBaseQuery.do?activityId='+${activityId}+'&type=1',800,500);
	    }else
	    {
	      MyAlert('产地已经最广无需添加');
	    }
	...
	function downloadTemplate(){
	 ...
	 function openVIN(){
	 	var url = '<%=contextPath%>/claim/serviceActivity/ServiceActivityVinImport/serviceActivityVinjuede.json?activityId='+${activityId}; 
		makeNomalFormCall(url,insertVIN,'fm');
	}
	
	function insertVIN(json)
	{
	   if(json.ret == 'true')
	   {
	     OpenHtmlWindow('<%=contextPath%>/claim/serviceActivity/ServiceActivityVinImport/serviceActivityVinImportInit.do?activityId='+${activityId}+"&flag=modify"+"&type=1",800,500);
	   }else
	   {
	   		MyAlert('vin范围最大无需添加！');
	   }
