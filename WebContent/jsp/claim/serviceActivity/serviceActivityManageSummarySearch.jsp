        ...
        <table  class="table_query">
          ...
          <tr>
			...
			<td  class="table_query_3Col_label_5Letter">经销商名称：</td>
	          <td>   <input type="text"  name="dealer_name"  id="dealer_name"  class="middle_txt"  size="25" />
  		   </td>            
          </tr>  
          ...
          <tr>
            ...
            <td align="left">
                  <script type="text/javascript">
					genSelBoxExp("activityType",<%=Constant.SERVICEACTIVITY_TYPE%>,"",true,"short_sel","","false",'');
 				</script>
			</td>
...
<script type="text/javascript" >
	var myPage;
    //查询路径
	var url = "<%=contextPath%>/claim/serviceActivity/ServiceActivityManageSummarySearch/serviceActivityManageSummarySearchQuery.json";
	...
	var columns = [
				...
				{header: "客户类型  ",dataIndex: 'ACTIVITY_TYPE' ,align:'center',renderer:getItemValue},
	...
	function mySelect(value,meta,record){
         ...
         "<a href=\"#\" onclick='sel(\""+record.data.SUBJECT_ID+"\",\""+record.data.DEALER_ID+"\")'>[查看]</a>");
	}
	//详细页面
	function sel(value,DEALER_ID){
	     fm.action = "<%=contextPath%>/claim/serviceActivity/ServiceActivityManageSummaryDealerSearch/serviceActivitySummaryQuery.do?subject_id="+value+"&DEALER_ID="+DEALER_ID ;
		fm.submit();
	}
	...
	{
	    var subject_no = document.getElementById('subject_no').value;
	    if(subject_no.length == 0)
	    {
	      MyAlert('请输入主题编号');
	    }else
	    {
	    	fm.action = '<%=contextPath%>/report/service/DealerFinFDetail/subjectNoExcel.do';
			fm.submit();
	    }
