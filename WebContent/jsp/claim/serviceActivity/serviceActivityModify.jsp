...
function modifyActivity(){
	...
	//设定主题时设定的结束日期
	var subEndDate = $("subEndDate").value;
...
// 		MyAlert("信息录入结束日期不能为空");
	...
	if(!checkDate(modifyEndDate, subEndDate)){
		MyAlert("新设定的信息录入结束日期必须小于等于主题设定的信息录入结束日期");
		return;
	}
	...
	makeNomalFormCall('<%=contextPath%>/claim/serviceActivity/ServiceActivityManage/ServiceActivityManageModify.json',returnBack,'fm');
}
...
<input type="hidden" id="subEndDate" name="subEndDate" value="${subEndDate}" />
...
<table class="table_edit" >
  ...
  <tr>
    ...
    <td><%=ActivityBean.getStartdate()%>至<input name="modifyEndDate" type="text" class="short_time_txt" value="${ActivityBean.enddate}" id="modifyEndDate" readonly="readonly"/> 
			<input name="button" value=" " type="button" class="time_ico" onclick="showcalendar(event, 'modifyEndDate', false);" /></td>
  </tr>
  <tr>
...
<table width="95%" border="0" align="center" cellpadding="4"
  ...
  <tr align="center">
    <td><input type="button" name="vehicleInfo" value="车辆信息" class="normal_btn" onclick="openCharactor();" id="commitBtn"/>
      ...
      <input type="button" name="bt_back" class="normal_btn" onclick="subModify();" value="修改"/></td>
  </tr>
</table>
</form>
<script type="text/javascript">
