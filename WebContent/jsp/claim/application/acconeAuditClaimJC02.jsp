...
<table align="center" class="table_query" border='0'>
	<tr>
		...
		<td align="right" nowrap="true">待审人员：</td>
        <td class="table_query_4Col_input" nowrap="nowrap">
        	<select name="person">
        		<option value=""  >-请选择-</option>
        		<option value="4000010041" >司丽军</option>
        		<option value="4000010042" >余燕</option>
        		<option value="4000010043" >王江红</option>
        		<option value="4000010040" >江宇</option>
        		<option value="4000010039">郑友明</option>
        	</select>
		</td>
	</tr>
	<tr>
...
<script type="text/javascript">
		...
		var myPage;
		var url = "<%=contextPath%>/claim/application/ClaimManualAuditing/acconeAuditClaimOneByOneJC02.json?COMMAND=1";
		var title = null;
   ...
   loadcalendar(); 
	...
	function accAudut(value,meta,record)
	{
		...
		{
			var resObj = String.format("<a href='#' onclick=judeclaimdetail("+record.data.DEALER_ID+","+record.data.BALANCE_YIELDLY+",'"+record.data.REPORT_DATE+"',"+record.data.CLAIM_TYPE+")>["+record.data.TOTAL+"]</a>");
  			return resObj;
		}else if(record.data.CLAIM_TYPE == '10661006')
		{
			var resObj = String.format("<a href='#' onclick=judeclaimdetail("+record.data.DEALER_ID+","+record.data.BALANCE_YIELDLY+",'"+record.data.REPORT_DATE+"',"+record.data.CLAIM_TYPE+")>["+record.data.TOTAL+"]</a>");
	...
	//查询索赔单明细
	function judeclaimdetail(id,balance_yieldly,report_date,claim_type)
	{
	     var url='<%=contextPath%>/claim/application/ClaimManualAuditing/judeclaimdetail.json?dealer_id='+id+"&balance_yieldly="+balance_yieldly+"&claim_type="+claim_type+"&report_date="+report_date;
		  makeNomalFormCall(url,resutclaimdetail,'fm');
	}
	function  resutclaimdetail(json)
	{
	  if(json.fag == 'true')
	  {
	  	claimdetail(json.id,json.balance_yieldly,json.report_date,json.claim_type);
	  }else
	  {
	     MyAlert('请先确认特殊费用');
	  }
	...
	function claimdetail(id,balance_yieldly,report_date,claim_type){
		...
		fm.action = '<%=contextPath%>/claim/application/ClaimManualAuditing/balancetoal.do?dealer_id='+id+"&balance_yieldly="+balance_yieldly+"&claim_type="+claim_type+"&report_date="+report_date+"&admin="+1;
		fm.submit();
	}
