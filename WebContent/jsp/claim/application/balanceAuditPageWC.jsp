...
<form method="post" name="fm" id="fm">
	...
	<tr><td>
		...
		<table border="1" class="table_list" name="all_group_"  id="labourTable">
			<tr class=''>
			    ...
			    <th width="10%">故障名称</th>
			    <th width="12%">顾客问题</th>
			    <th width="18%" nowrap="true">索赔工时名称</th>
				...
				<th width="2%">工时结算数</th>
				<th width="5%">作业次数</th>
				<th width="4%" nowrap>工时结算金额</th>
				<!-- <th width="20%">审核意见</th> -->
				
				<th width="10%" align="center">换上配件代码</th>
				<th width="10%" align="center">换上配件名称</th>
				<th width="5%">配件结算数量</th>
				<th width="5%">换件次数</th>
				<th width="5%" nowrap>配件结算金额</th>
				<th width="6%">审核意见</th>
			</tr>
			<%
	    ...
	    <%}else{ %>
	           追加工时：<input type="text" id="APPEND_LABOUR_ID" name="APPEND_LABOUR"
	            ...
	            price="1" limitValue="<%=CommonUtils.checkNull(tawep.getAppendlabourNum())%>" prevValue="<%=CommonUtils.checkNull(tawep.getAppendlabourNum())%>"
	              onkeyup="clearNoNum(this);linkAgeOver('APPEND_LABOUR_ID','aLabourId',false,true);"/>
	...
	<input style="width:150px;color:#1E3988;
		...
		<input class="normal_btn"	type="button" value="同意" name="bt_back2" onclick="confirmCustom('<%=Constant.CLAIM_APPLY_ORD_TYPE_07%>','fm',this);"/>
...
<script type="text/javascript">
	  ...
	  function jsonSubmit(statusValue,frmName,btn){
		  createTip();
		  var turl = '<%=contextPath%>'+'/claim/application/ClaimManualAuditing/balanceAuditingWC.json?status='+statusValue;;
