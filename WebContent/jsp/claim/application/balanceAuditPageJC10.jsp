...
<body>
<input type=hidden name='flagMsg' id='flagMsg' value="${flagMsg}"/>
<%
...
<form method="post" name="fm" id="fm">
	<table width="100%">
	 <tr><td>
		...
		<input type="hidden" id="typestats" name="typestats" value="${typestats}"/>
		<input type="hidden" id="curidsqu" name="curidsqu" value="${curidsqu}"/>
		...
		<table class="table_edit" id="baseTabId">
			<tr>
				...
				<td align="right" nowrap="true">车牌号:</td>
		...
		<table border="1" class="table_list" name="all_group_"  id="labourTable">
			...
			%>
			     ...
			     >
					 ...
					 <%} %>
				     <td>
				     	&nbsp;&nbsp;&nbsp;<input disabled="disabled" style="text-align:center" type="text" name="LOBOURCOUNT" id="LOBOURCOUNT<%=i%>" class="short_txt" datatype="0,is_double,20" decimal="2"  
				     ...
				     <td align="center">
			     		  <input disabled="disabled" style="text-align:center;width: 60px" name="LOBOURAMOUNT" id="LOBOURAMOUNT<%=i%>" type="text" class="no_border_txt"
					 ...
					 <!-- <td>
					 	 <input disabled="disabled" type="text" name="LOBOURREMARK" id="LOBOURREMARKID<%=i%>" class="normal_txt" value="<%=CommonUtils.checkNull(mainLabourPO.getAuthRemark()) %>" datatype="1,is_null,100"/>
					 </td> -->
					...
					<td>
							     <input disabled="disabled" style="text-align:center" type="text" name="PARTCOUNT" id="PARTCOUNT<%=j%>" 
					...
					<td align="center">
								      <input disabled="disabled" type="hidden" name="PARTCOUNT_APPLY<%=j%>" value="<%=mainPartPO.getQuantity() %>"></input>
									  <input disabled="disabled" style="text-align:center" name="PARTAMOUNT" id="PARTAMOUNT<%=j%>" type="text" class="short_txt" 
									  	 decimal="2" 
					...
					<td><script type="">
					document.write(getItemValue(<%=CommonUtils.checkNull(mainPartPO.getResponsibilityType()) %>));</script> </td>   
					<td>&nbsp;&nbsp;<input disabled="disabled" type="text" name="PARTREMARK" id="PARTREMARKID<%=i%>" class="normal_txt" value="<%=CommonUtils.checkNull(mainPartPO.getAuthRemark()) %>" datatype="1,is_null,100"/></td>
				    </tr>  
				    ...
				    <%} %>
		     	 	...
		     	 	<%} %>
								...
								<td><%="".equals(CommonUtils.checkNull(mainLabourPO.getLabourCount()))?
				    				 "0":CommonUtils.checkNull(mainLabourPO.getLabourCount())%></td>
								<td><%=CommonUtils.checkNull(mainLabourPO.getBalanceAmount())%></td>
								<td><%=CommonUtils.checkNull(mainPartPO.getPartCode()) %>&nbsp;
								<input type="hidden" name="PART_ID" value="<%=mainPartPO.getPartId() %>"/>
								<input type="hidden" name="PART_CODE" value="<%=CommonUtils.checkNull(mainPartPO.getPartCode()) %>"/>
								</td>
								...
								<td>
								  <input disabled="disabled" style="text-align:center" type="text" name="PARTCOUNT" id="PARTCOUNT<%=j%>" 
							     	 decimal="2"  class="short_txt" 
							     	value="<%=CommonUtils.checkNull(mainPartPO.getBalanceQuantity()) %>" price="<%=mainPartPO.getPrice()%>" 
							     	prevValue="<%=CommonUtils.checkNull(mainPartPO.getBalanceQuantity())%>" limitValue="<%=CommonUtils.checkNull(mainPartPO.getAmount())%>" 
							     	onblur="partMoy(<%=j%>);"/>&nbsp;
								    <input disabled="disabled" type="hidden" id="partMo<%=j%>" value="<%=CommonUtils.checkNull(mainPartPO.getBalanceQuantity()) %>" />   	
								</td>	     
								<td><%=dao.getCount(claimId,CommonUtils.checkNull(mainPartPO.getPartCode()))%></td>
								<td align="center">
								...
								<%
									...
									{
										for(TtAsWrApplicationPartPO partPO : applicationPartList)
										{
											if((""+partPO.getPartId()).equals( ""+mainPartPO.getPartId()) )
											{%>
												<td><input type="text" onblur="isdiscount(<%=j%>,this)" value="<%= partPO.getPartDiscount() %>"  name="DISCOUNT" />  </td>
											<%}
										}
									}else
									{%>
										<td><input type="text" onblur="isdiscount(<%=j%>,this)" value=""  name="DISCOUNT" />  </td>
									<% } %>
								<td> 
								<script type="">
		...
		<table class="table_list" style="border-bottom: 1px solid #DAE0EE;" id="otherTable">
			...
			<%for (int i=0;i<otherLs.size();i++) { %>
			    <tr class="<%=(i%2)==0?"table_list_row1":"table_list_row2"%>">
					...
					<td>
			     	<input disabled="disabled" name="OTHERAMOUNT" value="<%=CommonUtils.checkNull(otherLs.get(i).getBalanceAmount())%>" 
					...
					<td>
						<input disabled="disabled"  type="text" name="audit_con" class="middle_txt" value="<%=CommonUtils.checkNull(otherLs.get(i).getAuditCon()) %>" />
					</td>
		   		</tr>
		   	<%}%>
		</table>
	...
	<div align="left">
	    ...
	    <%}else{ %>
	           追加工时：<input disabled="disabled" type="text" id="APPEND_LABOUR_ID" name="APPEND_LABOUR"
	            value="<%if(tawep.getAppendlabourNum()!=null){%><%=tawep.getAppendlabourNum()%><%}else{ %>0<%}%>"  decimal="2" 
	            price="1" limitValue="<%=CommonUtils.checkNull(tawep.getAppendlabourNum())%>" prevValue="<%=CommonUtils.checkNull(tawep.getAppendlabourNum())%>"
	              onblur="lableMoy();"/>
	    ...
	    <input type="hidden" id="aLabourId_ray" value="<%=CommonUtils.checkNull(tawep.getAppendlabourNum())%>"/>
		<!--  需授权原因：<%=CommonUtils.checkNull(authReason.getApprovalReason())%>
		<br/>-->
		<input name="CON_REMARK"  id="CON_REMARK" datatype="1,is_null,300" type="hidden"  class="middle_txt" value="<%=CommonUtils.checkNull(tawep.getRemark())%>"/>
	</div>
	...
	<br/>
	<div align="center">
	<c:if test="${curidsqu == 1}"><input class="normal_btn" disabled="disabled" type="button" value="上一页"  onclick="up();"/></c:if>
	<c:if test="${curidsqu != 1}"><input class="normal_btn" type="button" value="上一页"  onclick="up();"/></c:if>
	<c:if test="${curidsqu == total}"><input class="normal_btn"  disabled="disabled" type="button" value="下一页" name="historyBtn" onclick="down();"/></c:if>
	<c:if test="${curidsqu != total}"><input class="normal_btn" type="button" value="下一页" name="historyBtn" onclick="down();"/></c:if>
	
	 
	<input style="width:150px;color:#1E3988;
		...
		<input class="normal_btn"	type="button" value="同意" name="bt_back2" onclick="confirmCustom('${ClaimType}','fm',this);"/>
        <input class="normal_btn" type="button" value="返回" onclick="backC()"/>
        &nbsp;&nbsp;&nbsp;
...
<script type="text/javascript">
      var tempBtn = null;
      var APPEND = document.getElementById("APPEND_LABOUR_ID").value;
      function lableMoy()
      {
      	
          var aa = document.getElementsByName('fjid');
          if(aa.length == 0)
          {
          	MyAlert('请先上传附件 !');
          	 document.getElementById("APPEND_LABOUR_ID").value=APPEND;
	         linkAgeb('APPEND_LABOUR_ID','aLabourId',false,true);
          }else
          {
          	var curValue=document.getElementById('APPEND_LABOUR_ID').value;
	      	if(isNaN(curValue)){
	              MyAlert('请录入数字!');
	              document.getElementById("APPEND_LABOUR_ID").value=APPEND;
	              linkAgeb('APPEND_LABOUR_ID','aLabourId',false,true);
      ...
      function isdiscount(j,obj)
      {
      	var PARTCOUNT=document.getElementById('PARTAMOUNT'+j).value * 1;
      	if(isNaN(obj.value))
      	{
      		MyAlert('请录入数字!');
      		obj.value = '0';
      	}
      	if(PARTCOUNT < obj.value*1 )
      	{
      		MyAlert('不能超过配件金额!');
      		obj.value = '0';
      	}
      ...
      function  partMoy(j)
		{
			var PARTCOUNT=document.getElementById('PARTCOUNT'+j).value;
			var partMo=document.getElementById('partMo'+j).value;
	      	if(isNaN(PARTCOUNT)){
	              MyAlert('请录入数字!');
	              document.getElementById('PARTCOUNT'+j).value = partMo;
	              linkAgeOverb('PARTCOUNT'+j,'PARTAMOUNT'+j,true,true);
			      linkAgeb('PARTCOUNT'+j,'partCountId'+j,false);
	          }else if((PARTCOUNT != 0 ))
	          {
	          	MyAlert('只能输入原来的值或者0');
	          	document.getElementById('PARTCOUNT'+j).value = partMo;
		...
		function linkAgeOverb(sourceId,targetId,userPrice,isCheckLimit){
       ...
       function linkAgeb(sourceId,targetId,userPrice,isCheckLimit){
	  ...
	  function jsonSubmit(statusValue,frmName,btn){
		  createTip();
		  var turl = '<%=contextPath%>'+'/claim/application/ClaimManualAuditing/balanceAuditingJC.json?status='+statusValue;;
		  setTimeout("closeBalance()",3000);
		  makeNomalFormCall(turl,showResult,frmName);
		  disableLink(btn,false);
	  }
	  //通过JSON提交审核结果 statusValue 状态 
	  function jsonSubmit01(statusValue,frmName,btn){
		  createTip();
		  var turl = '<%=contextPath%>'+'/claim/application/ClaimManualAuditing/balanceAuditingJC.json?status='+statusValue;;
		  setTimeout("closeBalance()",3000);
		  makeNomalFormCall(turl,showResult,frmName);
	  ...
	  //确定审核
	  function confirmCustom01(statusValue,frmName,btn) {
	  	  var flagMsg = ${flagMsg};
	  	  var flag = window.confirm('是否确认退回？');
  	      if(flag){
  	    	  jsonSubmit01(statusValue,frmName,btn);
  	      }
	  }
	function backC()
	{
	  ...
	  function showResult(json){
	       var typestats = document.getElementById('typestats').value;
	       if(typestats == '0')
	       {
	       	 var fm = document.getElementById("fm");
   ...
   function up()
   {
         document.getElementById('curidsqu').value =  document.getElementById('curidsqu').value*1 -2;
   		 var fm = document.getElementById("fm");
		 if('0' == '${YX}')
		  {
			   fm.action = '<%=contextPath%>/claim/application/ClaimManualAuditing/balanceAuditingPageYXJC01.do?isAudit=true';
		 	   fm.submit();
		  }else{
			  fm.action = '<%=contextPath%>/claim/application/ClaimManualAuditing/balanceAuditingPageJC01.do?isAudit=true';
		 	  fm.submit();
   ...
   function down()
   {
	  ...
	  */
	  function linkAgeOver(sourceId,targetId,userPrice,isCheckLimit){
		  ...
		  if(isOver){
			  ...
			  targetEle.value = (sourceEle.prevValue*sourcePrice).toFixed(2);
		  }else{
			  if(typeof(targetEle.text)!='undifined' && targetEle.text){
				  targetEle.text = targetValue;
			  }else{
	  ...
	  */
	  ...
	  限制结算时追加工时超过申报工时费
	 *****/ 
	  function AppendLabour(sourceId,targetId){
	  	var oldNum = $(targetId).value;
	  	var newNum = $(sourceId).value;
	  	if(parseFloat(newNum)>parseFloat(oldNum)){
	  		MyAlert('结算金额不能超过申请金额！');
	  	}
	  }
