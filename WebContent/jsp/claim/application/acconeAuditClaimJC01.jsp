...
<input type="hidden" value="${bansorg}" name="bansorg" id="bansorg"/>
<table align="center" class="table_query" border='0'>
	<tr>
		...
		<td align="left" nowrap="true">
		<script type="text/javascript">
			genSelBoxExp("claimStatus",<%=Constant.CLAIM_APPLY_ORD_TYPE%>,"<%=Constant.CLAIM_APPLY_ORD_TYPE_07%>",true,"short_sel","","false",'<%=Constant.CLAIM_APPLY_ORD_TYPE_01%>,<%=Constant.CLAIM_APPLY_ORD_TYPE_02%>,<%=Constant.CLAIM_APPLY_ORD_TYPE_03%>,<%=Constant.CLAIM_APPLY_ORD_TYPE_04%>,<%=Constant.CLAIM_APPLY_ORD_TYPE_05%>,<%=Constant.CLAIM_APPLY_ORD_TYPE_08%>,<%=Constant.CLAIM_APPLY_ORD_TYPE_09%>');
		</script>
		</td>
	</tr>
	<tr>
...
<script type="text/javascript"><!--
		var myPage;
		var url = "<%=contextPath%>/claim/application/ClaimManualAuditing/acconeAuditClaimOneByOneJC01.json?COMMAND=1";
		var title = null;
	...
	function accAudut(value,meta,record)
	{
  		...
  		var authcodeval = record.data.AUTH_CODE;
  	  		if((<%=Constant.CLAIM_APPLY_ORD_TYPE_07.toString()%>)==claimStatus || (<%=Constant.CLAIM_APPLY_ORD_TYPE_06.toString()%>)==claimStatus)
  	  			resObj = String.format("<a href='#' onclick='audit(\""+ value +"\",\""+record.data.BALANCE_ID+"\")'>[重新审批]</a>");
  	  		if((<%=Constant.CLAIM_APPLY_ORD_TYPE_10.toString()%>)==claimStatus || (<%=Constant.CLAIM_APPLY_ORD_TYPE_12.toString()%>)==claimStatus )	
  	  		resObj = String.format("<a href='#' onclick='audit(\""+ value +"\",\""+record.data.BALANCE_ID+"\")'>[重新抵扣]</a>");
  		return resObj;
	}
