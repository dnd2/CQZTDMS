...
<form method="post" name="fm" id="fm">
	...
	<tr><td>
		...
		<input type="hidden" name="dealerName" value="<%=request.getAttribute("dealerName")%>"/>    <!-- 经销商名称 -->
	...
	<tr><td>
		...
		<table border="1" class="table_list" name="all_group_"  id="labourTable">
			<tr class=''>
				...
				<%}else{ %>
					<th width="18%" nowrap>索赔工时名称</th>
				<%} %>
				<%if(codeId==Constant.chana_jc){ %>
					<th width="8%" nowrap>顾客问题</th>
				<%} %>
				<th width="5%" nowrap>工时结算数</th>
				<th width="7%" nowrap>作业次数</th>
				<th width="10%" nowrap>工时结算金额</th>
				<!-- <th width="20%">审核意见</th> -->
				
				<th width="10%" nowrap>换上配件代码</th>
				<th width="15%" nowrap>换上配件名称</th>
				<th width="5%" nowrap>配件结算数量</th>
				<th width="6%" nowrap>换件次数</th>
				<th width="8%" nowrap>配件结算金额</th>
				<th width="8%" nowrap>责任性质</th>
				<th width="6%" nowrap>审核意见</th>
			</tr>
			<%
	...
	<div align="center"> 
        ...
        <input class="normal_btn" type="button" value="返回" onclick="window.location.href='<%=contextPath%>/claim/application/ClaimManualAuditing/acconeAuditClaimOneByOneJC.do'"/>
	</div>
	</td></tr>
	<%}%>
	</table>
...
<script type="text/javascript">
	  ...
	  function jsonSubmit(statusValue,frmName,btn){
		  ...
		  var turl = '<%=contextPath%>'+'/claim/application/ClaimManualAuditing/claimAuditingJC.do?isContinue=true&status='+statusValue;;
		  frm.action = turl;
	  ...
	  function skipCurr()
	  {
		  ...
		  var turl = '<%=contextPath%>' + '/claim/application/ClaimManualAuditing/auditingClaimSkipJC.do';
		  frm.action = turl;
		  frm.submit();
	  }
	  ...
	  function closeWindow(){
		  var frm = document.getElementById("fm");
		  var turl = '<%=contextPath%>' + '/claim/application/ClaimManualAuditing/acconeAuditClaimOneByOneJC.do';
		  frm.action = turl;
		  frm.submit();
	  }
