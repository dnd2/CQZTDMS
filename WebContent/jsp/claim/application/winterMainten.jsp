...
var myPage;
	//查询路径
	var url = "<%=contextPath%>/claim/application/ClaimBillStatusTrack/winterMaintenQuery.json";
	...
	var columns = [
					...
					{header: "开始时间", width:'15%', dataIndex: 'startDate',renderer:formatDate},
					{header:'结束时间',width:'15%',dataIndex:'endDate',renderer:formatDate2},
	...
	var roNo = record.data.roNo;
		...
		if(record.data.status==<%=Constant.SERVICEACTIVITY_STATUS_02%>){
			return String.format("<a href='#' onclick='OpenHtmlWindow(\"<%=contextPath%>/claim/application/ClaimBillStatusTrack/winterMaintenDetailForward.do?id="+record.data.id+ "\","+width+","+height+")' >[明细]</a><a href=\"#\" onclick=\"modWinter('"+record.data.id+"','"+record.data.status+"')\">[修改]</a>");
		}else{
  			return String.format("<a href='#' onclick='OpenHtmlWindow(\"<%=contextPath%>/claim/application/ClaimBillStatusTrack/winterMaintenDetailForward.do?id="+record.data.id+ "\","+width+","+height+")' >[明细]</a><a href=\"#\" onclick=\"modWinter('"+record.data.id+"','"+record.data.status+"')\">[修改]</a><a href=\"#\" onclick=\"delWinter('"+record.data.id+"')\">[删除]</a><a href=\"#\" onclick=\"publishWinter('"+record.data.id+"')\">[发布]</a>");
		}
	}
	
	function modWinter(id,status) {
		location="<%=contextPath%>/claim/application/ClaimBillStatusTrack/winterMaintenModForward.do?id="+id+"&status="+status;   
...
<div class="navigation"><img src="../../../img/nav.gif" />&nbsp;当前位置：售后服务管理&gt;索赔基础数据&gt;冬季保养维护</div>
