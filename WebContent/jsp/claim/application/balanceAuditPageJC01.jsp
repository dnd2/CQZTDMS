...
<form method="post" name="fm" id="fm">
	<table width="105%">
	 <tr><td>
		...
		<input type="hidden" id="balance_yieldly" name="balance_yieldly" value="${balance_yieldly}"/>
		...
		<table class="table_edit" id="baseTabId">
			...
			<tr>
				...
				<td align="left" nowrap="true"><%=CommonUtils.checkNull(tawep.getClaimName())%>  (<%=CommonUtils.checkNull(tawep.getCampaignCode())%>--<%=CommonUtils.checkNull(tawep.getActivityName())%>)&nbsp;</td>
				<%if(!Constant.CLA_TYPE_07.toString().equals(tawep.getClaimType().toString())){ %>	
	...
	<tr><td>
		...
		<table border="1" class="table_list" name="all_group_"  id="labourTable">
			<tr class=''>
			    ...
			    <!-- <th width="5%">操作</th> -->
			    <th width="8%">索赔工时代码</th>
			    <%if(codeId==Constant.chana_jc){ %>
					<th width="8%">索赔工时名称</th>
				<%}else{ %>
				...
				<th width="8%">换上配件代码</th>
				<th width="8%">换上配件名称</th>
				<th width="6%">配件结算数量</th>
				<th width="6%">换件次数</th>
			...
			%>
			     ...
			     >
					...
					<td><script type="">
					document.write(getItemValue(<%=CommonUtils.checkNull(mainPartPO.getDeductRemark()) %>));</script> </td> 
				    ...
				    <%} %>
		     	 	...
		     	 	<%} %>
								>
								<td><%=CommonUtils.checkNull(mainLabourPO.getWrLabourcode() )%> <input type="hidden" name="LABOUR_ID" value="<%=mainLabourPO.getLabourId()%>"/></td>
								<td><%=CommonUtils.checkNull(mainLabourPO.getWrLabourname() )%></td>
								<%if(codeId==Constant.chana_jc){ %>
								<td></td>
								<%} %>
								<td>&nbsp;&nbsp;&nbsp;<input style="text-align:center" type="text" name="LOBOURCOUNT" id="LOBOURCOUNT<%=i%>" class="short_txt" datatype="0,is_double,20" decimal="2"  
				     	value="<%=CommonUtils.checkNull(mainLabourPO.getBalanceQuantity()) %>" price="<%=mainLabourPO.getLabourPrice()%>" 
				     	prevValue="<%=CommonUtils.checkNull(mainLabourPO.getBalanceQuantity())%>" limitValue="<%=CommonUtils.checkNull(mainLabourPO.getLabourAmount())%>" 
				     	onkeyup="linkAgeOver('LOBOURCOUNT<%=i%>','LOBOURAMOUNT<%=i%>',true,true);linkAge('LOBOURCOUNT<%=i%>','labourCountId',false);linkAge('LOBOURCOUNT<%=i%>','partAmountId',false);"/>&nbsp;</td>
								<td><%="".equals(CommonUtils.checkNull(mainLabourPO.getLabourCount()))?
			     		  ...
			     		  nextDeal="linkAge('LOBOURAMOUNT<%=i%>','labourAmountId',false);"/>
								...
								<td><script type="">
									document.write(getItemValue(<%=CommonUtils.checkNull(mainPartPO.getDeductRemark()) %>));</script> 
									</td> 
								<td>&nbsp;&nbsp;<input type="text" name="PARTREMARK" id="PARTREMARKID<%=i%>" class="normal_txt" value="<%=CommonUtils.checkNull(mainPartPO.getAuthRemark()) %>" datatype="1,is_null,100"/></td>
								</tr>
							    <%}}}%>
	...
	<div align="left">
	    ...
	    &nbsp;&nbsp;工时单价: <%=CommonUtils.currencyFormat(tawep.getLabourPrice(),minFractionDigits,defaultValue)%>
	            &nbsp;&nbsp; 追加工时原因: <input type="text" name="appendlabour_reason" maxlength="90" id="appendlabour_reason" value="<%=CommonUtils.checkNull(tawep.getAppendlabourReason())%>" />
	    	    <br/> 
	    <%} %>
	...
	<input style="width:150px;color:#1E3988;
		...
		<input class="normal_btn"	type="button" value="不同意" name="bt_back2" onclick="confirmCustom01('${ClaimType1}','fm',this);"/>
...
<script type="text/javascript">
      ...
      function lableMoy()
      {
          ...
          var aa = document.getElementById('appendlabour_reason').value;
          if(aa.length == 0)
          {
          	MyAlert('请添加追击工时原因 ！');
	...
	function backC()
	{
	  ...
	  function showResult(json){
	       ...
	       {
	          if('${balance_yieldly}' == '95411001')
	          {
	          	  var fm = document.getElementById("fm");
			 	 fm.action = '<%=contextPath%>/claim/application/ClaimManualAuditing/acconeAuditClaimOneByOneJC.do';
			  	 fm.submit();
	          }else if('${balance_yieldly}' == '95411002')
	          {
	          	 var fm = document.getElementById("fm");
