...
<table align="center" class="table_query" border='0'>
    <tr align="center" >
      <td align="right">主题编码:</td>
      <td  align="left"><input  type="text" name="subjectno" /></td>
      <td align="right" >活动代码:</td>
      <td align="left" ><input  type="text" name="activtycode" /></td>
      <td  class="table_query_2Col_label_7Letter">索赔单号：</td>
      <td><input name="RO_NO" id="RO_NO" value="" maxlength="20" type="text" datatype="1,is_digit_letter,20" class="middle_txt" />
    </tr>
	<tr>
		<td align="center" colspan="4">
			 <input class=normal_btn onclick="__extQuery__(1);" value=查询 type=button name=bt_back/>
			 <c:if test="${type == 1}"><input class=normal_btn onclick="goImport();" value=导出 type=button name=bt_back/></c:if>
...
<script type="text/javascript">
		var myPage;
		var url = "<%=contextPath%>/claim/application/DealerBalance/balancetoal.json?COMMAND=1";
		var title = null;
		...
		var columns = [
				...
				{header: "申请劳务费用(元)",dataIndex: 'LABOUR_AMOUNT1',align:'center',renderer:formatCurrency},
				{header: "申请材料费用(元)",dataIndex: 'PART_AMOUNT',align:'center',renderer:formatCurrency},
				{header: "结算劳务费用(元)",dataIndex: 'BALANCE_LABOUR',align:'center',renderer:getNum},
				{header: "结算材料费用(元)",dataIndex: 'BALANCE_PART_AMOUNT',align:'center',renderer:getNum1},
	...
	function goImport()
	{
	     fm.action = "<%=contextPath%>/claim/application/DealerBalance/SettlementSummaryDelExport.do";
	     fm.submit();
	}
	...
	function getNum(value,meta,record)
	{
		...
		{
		    if(record.data.BALANCE_LABOUR == null)
		    {
		    	return '0';
		    }else
		    {
		    	return record.data.BALANCE_LABOUR;
		    }
			
		}
	}
	function getNum1(value,meta,record)
	{
		if(record.data.STATUS == '10791006' || record.data.STATUS == '10791008'  )
		{
			return '0';
		...
		{
		    if(record.data.BALANCE_PART_AMOUNT == null)
		    {
		    	return '0';
		    }else
		    {
		    	return record.data.BALANCE_PART_AMOUNT;
		    }
