...
<form method="post" name="fm" id="fm">
	<table width="100%">
	<tr><td>
		<input type="hidden" name="claimId" value="<%=tawep==null?"-1":tawep.getId() %>"/><!-- 索赔申请单ID -->
		<input type="hidden" name="id" value="<%=request.getAttribute("BALANCE_ID")%>"/><!-- 结算单ID-->
		<input type="hidden" name="claimNo" value="<%=request.getAttribute("claimNo")%>"/>    <!-- 索赔单号 -->
		<input type="hidden" name="vin" value="<%=request.getAttribute("vin")%>"/>            <!-- VIN -->
		<input type="hidden" name="modelCode" value="<%=request.getAttribute("modelCode")%>"/><!-- 物料组 -->
		<input type="hidden" name="startDate" value="<%=request.getAttribute("startDate")%>"/><!-- 制单开始日期 -->
		<input type="hidden" name="endDate" value="<%=request.getAttribute("endDate")%>"/>    <!-- 制单结算日期 -->
		<input type="hidden" name="CLAIM_TYPE" value="<%=request.getAttribute("CLAIM_TYPE")%>"/>    <!-- 索赔单类型 -->
		<input type="hidden" name="dealerCode" value="<%=request.getAttribute("dealerCode")%>"/>    <!-- 经销商代码 -->
		<input type="hidden" name="cantNotAudit" value="<%=request.getAttribute("cantNotAudit")==null?"":request.getAttribute("cantNotAudit")%>"/>    <!-- 不能审核的单子 -->
	</td></tr>
	<% Object HASNEXT = request.getAttribute("HASNEXT");
	   String isHas = HASNEXT==null?"false":HASNEXT.toString();
	   if("true".equals(isHas)){
	%>
	 <tr><td>
		...
		<table class="table_edit" id="baseTabId">
			...
			<tr>
				...
				<script type="text/javascript">
					...
					genSelBoxExp("CLAIM_ZHISHUN",<%=Constant.CLAIM_ZHISHUN%>,"<%=CommonUtils.checkNull(tawep.getDegradationType())%>","true","short_sel",'true',"true","true");
					$('CLAIM_ZHISHUN').disabled=true;
				</script>
				</td>
				<%} %>
	...
	<tr><td>
		...
		<table border="1" class="table_list" name="all_group_" style="border-bottom: 1px solid #DAE0EE;" id="labourTable">
			<tr class=''>
				...
				<th width="5%">工时结算数</th>
				<th width="6%">作业次数</th>
				<th width="10%" nowrap>工时结算金额</th>
				<!-- <th width="20%">审核意见</th> -->
				
				<th width="10%" nowrap>换上配件代码</th>
				<th width="15%" nowrap>换上配件名称</th>
				<th width="5%">配件结算数量</th>
				<th width="6%">换件次数</th>
				<th width="8%" nowrap>配件结算金额</th>
				 <%if(codeId==Constant.chana_jc){ %>
			   	<th width="8%" nowrap>责任性质</th>
			     <%} %>
			...
			%>
			     ...
			     >
			     	 ...
			     	 <td>
			     	 <input type="hidden" name="LABOUR_ID" value="<%=mainLabourPO.getLabourId()%>"/>
			     	 <%=CommonUtils.checkNull(mainLabourPO.getTroubleCodeName() )%>&nbsp;</td>
			     	 <%if(codeId==Constant.chana_wc){ %>
				     ...
				     <td>
				     	...
				     	onkeyup="linkAgeOver('LOBOURCOUNT<%=i%>','LOBOURAMOUNT<%=i%>',true,true);linkAge('LOBOURCOUNT<%=i%>','labourCountId',false);"/>&nbsp;
				     </td>
				     <%if(codeId==Constant.chana_jc){ %>
				     ...
				     <%} %>
				     <td><%="".equals(CommonUtils.checkNull(mainLabourPO.getCount()))?
					    				 "0":CommonUtils.checkNull(mainLabourPO.getCount())%>
					 </td>
					 ...
					 <td><%=CommonUtils.checkNull(mainPartPO.getPartCode()) %>&nbsp;
								 ...
								 <td>
								 	<%=dao.getCount(tawep.getId().toString(),CommonUtils.checkNull(mainPartPO.getPartCode()))%>
							     </td>
								 <td>
									  ...
									  <input style="text-align:center" name="PARTAMOUNT" id="PARTAMOUNT<%=j%>" type="text" class="short_txt" 
							     		...
							     		onkeypress="return numberAutoCheck();"
							     		/>
			    				 ...
			    				 <%} %>
			    				 <td>
							     	&nbsp;&nbsp;<input type="text" name="PARTREMARK" id="PARTREMARKID<%=i%>" class="normal_txt" value="<%=CommonUtils.checkNull(mainPartPO.getAuthRemark()) %>" datatype="1,is_null,100"/>
							     </td>
				    </tr> 
				     <%} %>
		...
		<table class="table_list" style="border-bottom: 1px solid #DAE0EE;" id="otherTable">
			...
			<%for (int i=0;i<otherLs.size();i++) { %>
			    <tr class="<%=(i%2)==0?"table_list_row1":"table_list_row2"%>">
					...
					<td>
						<input type="text" name="audit_con" class="middle_txt" />
					</td>
		   		</tr>
		   	<%}%>
		</table>
	...
	<br/>
	<div align="center"> 
		<input class="normal_btn" type="button" value="站长三包员联系信息" name="contactInfo"
		...
		<input class="long_btn"	type="button" value="同意并跳转下条" name="bt_back2" onclick="confirmCustom('<%=Constant.CLAIM_APPLY_ORD_TYPE_07%>','fm',this);"/>
		<input class="long_btn"	type="button" value="下一条" name=btn_skip onclick="skipCurr();"/>
        <input class="normal_btn" type="button" value="返回" onclick="closeWindow();"/>
	</div>
	</td></tr>
	<%}%>
	</table>
...
<script type="text/javascript">
      ...
      function checkMoney(i,balAmt,amt){
          ...
          if(new Number(curValue)>new Number(amt)){
	  ...
	  function jsonSubmit(statusValue,frmName,btn){
		  ...
		  var turl = '<%=contextPath%>'+'/claim/application/ClaimManualAuditing/claimAuditing.do?isContinue=true&status='+statusValue;;
		  frm.action = turl;
	  ...
	  function skipCurr()
	  {
		  ...
		  var turl = '<%=contextPath%>' + '/claim/application/ClaimManualAuditing/auditingClaimSkip.do';
		  frm.action = turl;
		  frm.submit();
	  }
	  ...
	  function closeWindow(){
		  var frm = document.getElementById("fm");
		  var turl = '<%=contextPath%>' + '/claim/application/ClaimManualAuditing/acconeAuditClaimOneByOne.do';
		  frm.action = turl;
		  frm.submit();
	  }
	  ...
	  */
	  ...
	  function isCheck(id,a,j,d){
		  if(a>accMulFloat(($('PARTCOUNT'+j).value),d)){
			  MyAlert('结算金额不能超过申请金额!');
...
<% if(!"true".equals(isHas)){%>
		<script type="text/javascript">
			MyConfirm("已经审核到最后一条！",closeWindow,[],closeWindow,[]);
		</script>&nbsp;
<%} %>
</body>
</html>