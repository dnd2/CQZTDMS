...
<table align="center" class="table_query" border='0'>
	<tr>
		...
		<td align="left" nowrap="true">
		  <input class="short_txt" id="report_date" name="report_date" datatype="0,is_date,10"
                 maxlength="10"/>
          <input class="time_ico" value=" " onclick="showcalendar(event, 'report_date', false);" type="button"/> 
          &nbsp至&nbsp
          <input class="short_txt" id="last_report_date" name="last_report_date" datatype="0,is_date,10"
                 maxlength="10"/>
          <input class="time_ico" value=" " onclick="showcalendar(event, 'last_report_date', false);" type="button"/>
		</td>
		<td align="right" nowrap="true">确认时间月份：</td>
		<td align="left" nowrap="true">
		  <input class="short_txt" id="DIRECTOR_DATE" name="DIRECTOR_DATE" datatype="1,is_date,10"
                 maxlength="10"/>
          <input class="time_ico" value=" " onclick="showcalendar(event, 'DIRECTOR_DATE', false);" type="button"/> 
          &nbsp至&nbsp
          <input class="short_txt" id="LAST_DIRECTOR_DATE" name="LAST_DIRECTOR_DATE" datatype="1,is_date,10"
                 maxlength="10"/>
          <input class="time_ico" value=" " onclick="showcalendar(event, 'LAST_DIRECTOR_DATE', false);" type="button"/>
		</td>
...
<script type="text/javascript">
		...
		var myPage;
		var url = "<%=contextPath%>/claim/application/ClaimManualAuditing/balance_detail.json?COMMAND=1";
		var title = null;
		...
		var columns = [
					...
					{header: "收单日期",dataIndex: 'TICKETS_ENDDATE',align:'center'},
					{header: "科员开票日期",dataIndex: 'CREATE_DATE',align:'center'},
					{header: "室主任审核日期",dataIndex: 'DIRECTOR_DATE',align:'center'},
					{header: "服务站打印日期",dataIndex: 'REVIEW_APPLICATION_TIME',align:'center'},
					{header: "付款凭证日期",dataIndex: 'HHCREAT_DATE',align:'center'},
   ...
   loadcalendar(); 
	...
	function claimdetail(id,balance_yieldly,report_date,claim_type){
		...
		fm.action = '<%=contextPath%>/claim/application/DealerBalance/balancetoal.do?dealer_id='+id+"&report_date="+report_date;
		fm.submit();
	}
