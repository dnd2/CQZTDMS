...
<script type="text/javascript" >
	var myPage;
	var url = "<%=contextPath%>/claim/application/ClaimManualAuditing/repairOrderCancel.json?query=true";
	var title = null;
	var columns = [  
	...
	function myLink(value,meta,record){
		return String.format("<a href='#' onclick='roCancel(\""+record.data.RO_NO+"\",\""+record.data.ID+"\")'>[废弃]</a>");
	}
	function roCancel(ro_no,id){
		MyConfirm("是否确认废除该工单？",roCancelDo,[ro_no,id]);
	}
	function roCancelDo(ro_no,id){
		location.href = "<%=contextPath%>/repairOrder/RoMaintainMain/roDeleteDetail.do?roNo="+ro_no+"&type=4&ID="+id;
		//sendAjax('<%=contextPath%>/repairOrder/RoMaintainMain/roCancelDo.json?ro_no='+ro_no,roCancelDoBack,'fm');
	}
