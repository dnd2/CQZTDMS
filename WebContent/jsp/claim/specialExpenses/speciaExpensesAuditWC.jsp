...
<div class="navigation"><img src="<%=request.getContextPath()%>/img/nav.gif" />&nbsp;当前位置： 售后服务管理&gt;特殊费用管理&gt;市场工单审核(结算室)</div>
    ...
    <table class="table_query" >
		<tr>
          ...
          <tr>
            ...
            <td style="display:none;" align="right" nowrap="nowrap" >费用类型：</td>
            <td style="display:none;" colspan="6" nowrap="nowrap">
              	<script type="text/javascript">
...
<script type="text/javascript" >
	var myPage;
//查询路径
	var url = "<%=contextPath%>/claim/specialExpenses/SpecialExpensesManage/auditQuerySpeciaExpensesWC.json?COMMAND=1";
...
//设置超链接  begin      
	...
	{
		fm.action = '<%=contextPath%>/claim/specialExpenses/SpecialExpensesManage/auditSpecialExpensesInfoWC.do?id=' + val1 + "&feeType=" + val2;
	 	fm.submit();
	}
	...
	function cliamAllAudit()
    {
	    ...
	    frm.action = "<%=contextPath%>/claim/specialExpenses/SpecialExpensesManage/auditSpecialExpensesInfoByOneWC.do";
	    frm.submit();
    }
    //批量审批
	...
	{
		makeNomalFormCall("<%=contextPath%>/claim/specialExpenses/SpecialExpensesManage/auditSpeciaExpensesBatch01WC.json?",showAuditValue,'fm','queryBtn'); 
	}
