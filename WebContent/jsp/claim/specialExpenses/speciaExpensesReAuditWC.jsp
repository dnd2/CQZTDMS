   ...
   <!-- 查询条件 begin -->
    <table class="table_query" >
		<tr>
			...
			<td align="left">
				<script type="text/javascript">
				genSelBoxContainStr("spefeeStatus",<%=Constant.SPEFEE_STATUS%>,'<%=Constant.SPEFEE_STATUS_06%>',false,"short_sel","","true",'<%=Constant.SPEFEE_STATUS_06%>,<%=Constant.SPEFEE_STATUS_07%>');
		    	</script>
			</td>
		</tr>
          <tr>
...
<script type="text/javascript" >
	var myPage;
	var url = "<%=contextPath%>/claim/specialExpenses/SpecialExpensesManage/reauditQuerySpeciaExpensesWC.json?COMMAND=1";
	...
	function myLink(val,meta,rec){
		...
		url1 = '<a href="<%=contextPath%>/claim/application/ClaimManualAuditing/secondCheckForward.do?ID='+rec.data.ID+'&feeType='+rec.data.FEE_TYPE+'&flag=0">[二次复核]</a>';
		url2 = '<a href="<%=contextPath%>/claim/specialExpenses/SpecialExpensesManage/reauditDoInitWC.do?id='+rec.data.ID+'&feeType='+rec.data.FEE_TYPE+'">[审核]</a>' ;
		if(rec.data.STATUS==<%=Constant.SPEFEE_STATUS_07%>){
			return String.format(url1);
		}else{
