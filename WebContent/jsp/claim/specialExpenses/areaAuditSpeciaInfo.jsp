...
<div class="navigation"><img src="<%=request.getContextPath()%>/img/nav.gif" />&nbsp;当前位置： 售后服务管理 &gt; 特殊费用提报 &gt; 特殊费用详细</div>
    ...
    <table class="table_list">
       <tr > 
         <th height="12" align=center>
           <div id="a1">
	           <input type="button" id="passBtn" onclick="audit(1)" class="normal_btn" style="width=8%" value="通过"/>
	           <input type="button" id="rejectBtn" onclick="audit(2)" class="normal_btn" value="拒绝" /> 
...
<script type="text/javascript">
	...
	{	
		...
		MyConfirm("确认提交！",addSpecia, [val],myBtnSet);
	}
	function myBtnSet(){
	...
	{
		makeNomalFormCall("<%=contextPath%>/claim/specialExpenses/SpecialExpensesManage/areaAuditSpeciaExpenses.json?auditFlag="+val,addSpeciaBack,'fm','queryBtn'); 
	}
	...
	function addSpeciaBack(json)
	{
		...
		{
			parent.MyAlert("操作成功！");
			fm.action = "<%=contextPath%>/claim/specialExpenses/SpecialExpensesManage/areaSpecialExaminesFor.do";
			fm.submit();
		}
		else
		{
