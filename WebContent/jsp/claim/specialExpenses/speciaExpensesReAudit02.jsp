...
<input type="hidden" name="feeType" value="<c:out value="${map.FEE_TYPE}"/>"/>
<input type="hidden" name="balanceId" value="<c:out value="${map.CLAIMBALANCE_ID}"/>"/>
<input type="hidden" id="c1" value="${map.PASS_FEE1}" />
<input type="hidden" id="c2" value="${map.TRAFFIC_FEE1}" />
<input type="hidden" id="c3" value="${map.QUARTER_FEE1}" />
<input type="hidden" id="c4" value="${map.EAT_FEE1}" />
<input type="hidden" id="c5" value="${map.PERSON_SUBSIDE1}" />
...
<script type="text/javascript">
	...
	function addSpecia(val)
	{
		$('btn1').disabled = true ;
		$('btn2').disabled = true ;
		makeNomalFormCall("<%=contextPath%>/claim/specialExpenses/SpecialExpensesManage/reauditSpeciaExpenses01.json?auditFlag="+val,addSpeciaBack,'fm','queryBtn'); 
	}
	...
	function addSpeciaBack(json)
	{
		...
		{
			parent.MyAlert("操作成功！");
			fm.action = "<%=contextPath%>/claim/specialExpenses/SpecialExpensesManage/reauditMainInit.do";
			fm.submit();
		}
		else
		{
