...
<table align="center" class="table_query">
	...
	<tr>
		<td colspan="4" align="center">
			...
			&nbsp;&nbsp;
			  <c:if test="${type != 1}">
						<span id="aaaa"><input class="normal_btn" type="button" disabled="disabled" name="addBtn" id="addBtn" value="开票" 
				onclick="forwordToAddPage();"/></span> 
			</c:if>	
		</td>
	</tr>
</table>
...
<script type="text/javascript">
	...
	{ 
			return String.format("<a href='#' onclick=queryInfo01('"+record.data.REMARK+"')>[查看]</a><a href='#' onclick=queryInfo('"+record.data.REMARK+"') >[打印]</a>");
	}
	...
	{
		 window.open('<%=contextPath%>/claim/authorization/BalanceMain/queryFineInfoPrint.do?id='+val,"开票通知单打印", "height=700, width=1024, top=0, left=0, toolbar=no, menubar=no, scrollbars=yes, resizable=yes,location=no, status=no");
	}
	//跳转到新增结算页面
