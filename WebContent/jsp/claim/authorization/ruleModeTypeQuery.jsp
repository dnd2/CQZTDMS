...
<body onbeforeunload="returnBefore();">
  ...
  <form name='fm' id='fm'>
  <input id="WRGROUP_ID" name="WRGROUP_ID" type="hidden" />
  <input id="WRGROUP_NAME" name="WRGROUP_NAME" type="hidden" />
  <table class="table_list" id="myTable2" style="border-bottom:1px solid #DAE0EE" >
      ...
      <th>车型大类名称</th>
       ...
       <tr class="table_list_row1">
       	  <td>
			<input type="checkbox" id="${addlist.WRGROUP_ID}" name="businesscodeIds" value="${addlist.WRGROUP_ID}"/>
       	  </td>
          <td>
          <c:out value="${addlist.WRGROUP_CODE}"></c:out>
          </td>
          <td>
          <c:out value="${addlist.WRGROUP_NAME}"></c:out>
          </td>
        </tr>
    </c:forEach>    
</table>
...
{   
	...
	var code = document.getElementById("WRGROUP_NAME").value;
	var id = document.getElementById("WRGROUP_ID").value;
	if(code && code.length > 0){
		parentDocument.getElementById(ggg).value = document.getElementById("WRGROUP_NAME").value;
	}
	if(id && id.length > 0){
		parentDocument.getElementById(codeids).value = document.getElementById("WRGROUP_ID").value;
	}	
}
/**
*索赔类型的选择确认方法：
...
function claimTypeConfirm()
{
	...
	if(codes && codes.length > 0)
	   $('WRGROUP_NAME').value = codes;
	if(ids && ids.length > 0)
	   $('WRGROUP_ID').value = ids;
	_hide();
	closePan();
}
</script>  
