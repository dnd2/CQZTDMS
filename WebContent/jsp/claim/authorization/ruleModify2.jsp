...
<input type="hidden" name="WRGROUP_ID" id="WRGROUP_ID"  />
  <table class="table_add">
	    <tr>
	    	<td  class="table_add_3Col_label_5Letter" rowspan="<%=levellist.size()/3+2%>">授权角色： </td>
	    </tr>
    ...
    <tr class="<%=className%>">
	  ...
	  <%if (0 == i){%>
			--
	  <%}else{%>
			<select name="BOOLEAN_COMPARISION" size="1">
		<%
...
%>
		<script type="text/javascript">
		genSelBoxExp("COMPARISON_OP",<%=Constant.LOGIC_TYPE%>,<%=tempItem.get("COMPARISON_OP").toString().trim()%>,false,"short_sel","","true",'');
		</script>
<%		}else{
%>
		<script type="text/javascript">
			genSelBoxExp("COMPARISON_OP",<%=Constant.COMP_TYPE%>,<%=tempItem.get("COMPARISON_OP").toString().trim()%>,false,"short_sel","","true",'');
		</script>
<%		}
%>
	  </td>
...
<%} %>      
	  ...
	  <td align="center"> <input name="ELEMENT_POSITION" type="text" value="<%= tempItem.get("ELEMENT_POSITION")%>" class="mini_txt" onchange="javascript:positionChange(this)"/></td>
...
<table class="table_add">
	<tr>
		<td class="table_add_2Col_label_5Letter">授权项：</td>
		<td>
		     <script type="text/javascript">
...
function addItem()
{
	...
	//检查顺序
	var td5 = '<input name=\"ELEMENT_POSITION\" class=\"mini_txt\" datatype=\"1,is_digit,3\"  type=\"text\" size=\"6\" onchange=\"javascript:positionChange(this)\">';
	//操作：
...
function positionChange(obj){	
	var i = obj.parentElement.parentElement.rowIndex;
	if(obj.value == 0){
		editRule.childNodes[i].childNodes[0].innerHTML="---";
	}
		...
		editRule.childNodes[i].childNodes[0].innerHTML='<select name=\"BOOLEAN_COMPARISION\" size=\"1\"><%	for (int j=0; j<boolComp.length; j++){ %><option value=\"<%= boolComp[j] %>\"><%= boolComp[j] %></option><% } %></select>';
	}
}
/**
*车型代码弹出框：
...
function selClaimType(r){
	...
	function checkForm(){
			...
			makeFormCall('<%=contextPath%>/claim/authorization/RuleMain2/ruleUpdate.json',showResult,'fm');			
	}
	function goBack(){
		history.go(-1);
	}
