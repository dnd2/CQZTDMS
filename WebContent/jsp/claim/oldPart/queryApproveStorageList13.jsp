...
<div class="navigation"><img src="<%=contextPath%>/img/nav.gif"/>&nbsp;当前位置： 售后服务管理&gt;索赔旧件管理&gt;条码旧件签收入库查询</div>
  <form id="fm" name="fm">
    ...
    <TABLE class="table_query">
       ...
       <tr>
          ...
          <td>
            <script type="text/javascript">
             genSelBoxExp("back_type",<%=Constant.BACK_LIST_STATUS%>,"${back_type }",true,"short_sel","","true",'<%=Constant.BACK_LIST_STATUS_05%>');
            </script>
          </td>         
       ...
       <tr>
          ...
          <option title='请选择' value="wx">-请选择-</option>
           <option title='已扫描' value="ys">已扫描</option>
          </select>
          </td>        
...
<script type="text/javascript">
   ...
   function myLink(value,meta,record){
	   ...
	   if(back_type=='<%=Constant.BACK_LIST_STATUS_02%>'||back_type=='<%=Constant.BACK_LIST_STATUS_03%>'||back_type=='<%=Constant.BACK_LIST_STATUS_05%>'){
		   ...
		   return String.format("<a href='#' onclick=isCheck12("+value+")>[查询]</a>"+"<a href='#' onclick=isCheck1("+value+")>[打印]</a>");
   ...
   function showDetail21(id){

		ok='ok';
		if(ok=='ok'){
			//location.href= "<%=contextPath%>/claim/oldPart/ClaimOldPartApporoveStorageManager/goApproveAndStoredPage41.do?CLAIM_ID="+id;
			fm.action="<%=contextPath%>/claim/oldPart/ClaimOldPartApporoveStorageManager/goApproveAndStoredPage412.do?CLAIM_ID="+id;
		    fm.method="post";
		    fm.submit();
		}else{
			MyAlert('前期有未审核的旧件!');
