...
<div class="navigation"><img src="<%=contextPath%>/img/nav.gif"/>&nbsp;当前位置： 售后服务管理&gt;索赔旧件管理&gt;超期旧件审核</div>
  <form id="fm" name="fm">
...
<script type="text/javascript">
   var myPage;
   //查询路径
   var url = "<%=contextPath%>/claim/oldPart/ClaimOldPartApporoveStorageManager/queryOverdueApproveList.json";
   ...
   function myLink(value,meta,record){
	   ...
	   var currStatus = record.data.status;
		   ...
		   return String.format(
	               "<a href='#' onclick=isCheck12("+value+")>[查询]</a>"+"<a href='#' onclick=isCheck("+value+")>[确认超期]</a>"+"<a href='#' onclick=isCheck1("+value+")>[修改为正常]</a>");
   ...
   function isCheck(id){
	   if (confirm('确定超期吗？')){    
	   fm.action="<%=contextPath%>/claim/oldPart/ClaimOldPartApporoveStorageManager/updateOverdueCQ.do?CLAIM_ID="+id;
       fm.method="post";
       fm.submit();
	   }
   }
   ...
   function isCheck1(id){
	   if (confirm('确定修改为正常吗？')){    
	   fm.action="<%=contextPath%>/claim/oldPart/ClaimOldPartApporoveStorageManager/updateOverdue.do?CLAIM_ID="+id;
       fm.method="post";
       fm.submit();
	   }
   }
