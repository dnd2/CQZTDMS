...
<title>旧件回运单申请修改</title>
<% 
   String contextPath = request.getContextPath();
%>
</head>
<body >
<div class="navigation"><img src="<%=contextPath%>/img/nav.gif"/>&nbsp;当前位置：售后服务管理&gt;索赔旧件管理&gt;旧件回运单申请修改</div>
 <form method="post" name ="fm" id="fm">
 	<input type=hidden name='COMMAND' value='1'/>
 	<input type=hidden name='RETURN_APPLY_ID' value='${RETURN_APPLY_ID }'/>
       <table id="itemTableId" border="0" align="center" cellpadding="0"
				...
				<td>
					...
					<tr>
					<td><input type="text" class="short_txt" name="RETURN_NO" value="${bean.RETURN_NO }" /><span style="color:red">*</span><a href="#"  onclick="selectMainTime(this);">选择</a>
						<input type=hidden name="RETURN_DETAIL_ID" value="${bean.RETURN_DETAIL_ID }"/>
						<input type=hidden name="DETAIL_ID" value="${bean.DETAIL_ID }"/>
					</td>
					<td><input type="text" class="short_txt" name="PART_CODE" value="${bean.PART_CODE }" /></td>
					<td><input type="text" class="short_txt" name="PART_NAME" value="${bean.PART_NAME }" /></td>
					<td><input type="text" class="short_txt" name="CLAIM_NO" value="${bean.CLAIM_NO }" /></td>
					<td><input type="text" class="short_txt" name="BARCODE_NO" value="${bean.BARCODE_NO }"/></td>
					<td><input type="text" class="short_txt" name="DEDUCT_REMARK" value="${bean.DEDUCT_REMARK }"/>
						<input type="hidden" name="DEDUCT_REMARK_CODE" value="${bean.DEDUCT_REMARK_CODE }"/>
					</td>
...
function setMainPartCode(id,returnNo,partCode,partName,
		claimNo,barcodeNo,deductRemark) {
		    ...
		    myobj.cells.item(0).innerHTML='<input type="text" class="short_txt" name="RETURN_NO" value="'+returnNo+'" size="10" maxlength="11" id="RETURN_NO" readonly/><span class="tbwhite"><a href="#" onClick="javascript:selectMainTime(this)">选择</a></span>'+
...
//动态生成表格
	...
	if (tableId=='returnApplyTable') {
		...
		addTable.rows[length].cells[0].innerHTML = '<td><input type="text"  class="short_txt" name="RETURN_NO" datatype="0,is_null"  value=""  id="RETURN_NO	"/><span style="color:red">*</span><a href="#"  onclick="selectMainTime(this);">选择</a>'+
		'<input type=hidden name="DETAIL_ID" value=""/></td>';
		addTable.rows[length].cells[1].innerHTML =  '<td><input type="text"  class="short_txt" name="PART_CODE" datatype="0,is_null"  value="" size="10" id="PART_CODE"/></td>';
		addTable.rows[length].cells[2].innerHTML =  '<td><input type="text" class="short_txt" name="PART_NAME" datatype="0,is_null" id="PART_NAME"  size="3" style="length:5"/></td>';
		addTable.rows[length].cells[3].innerHTML =  '<td><input type="text" class="short_txt" name="CLAIM_NO" datatype="0,is_null" id="CLAIM_NO"  size="3"/></td>';
		addTable.rows[length].cells[4].innerHTML =  '<td><input type="text" class="short_txt" name="BARCODE_NO" datatype="0,is_null"  value="" size="10" id="BARCODE_NO"/></td>';
		addTable.rows[length].cells[5].innerHTML =  '<td><input type="text" class="short_txt" name="CODE_DESC" datatype="0,is_null"  value="" size="10" id="CODE_DESC"/></td>';
		addTable.rows[length].cells[6].innerHTML =  '<td><td><input type="button" class="normal_btn"  value="删除"  name="button42" onClick="javascript:delTransportItem(this);"/></td></td>';
	}
	...
	function addSpecia(val)
	{
	  if(val == 1){
		...
		makeNomalFormCall("<%=contextPath%>/claim/oldPart/ClaimOldPartReturnApplyManager/modifyReturnApplyDetail.json?flag="+val,addSpeciaBack,'fm','queryBtn');
	}
