...
<div class="navigation"><img src="<%=contextPath%>/img/nav.gif"/>&nbsp;当前位置： 售后服务管理&gt;索赔旧件管理&gt;条码旧件签收入库</div>
  <form id="fm" name="fm">
    ...
    <TABLE class="table_query">
       ...
       <tr>
          ...
          <td>
            <script type="text/javascript">
             genSelBoxExp("back_type",<%=Constant.BACK_LIST_STATUS%>,'<%=Constant.BACK_LIST_STATUS_02%>',true,"short_sel","","true",'<%=Constant.BACK_LIST_STATUS_05%>');
            </script>
          </td>         
       ...
       <td class="table_query_2Col_label_6Letter">是否超期：</td>
          <td><select name='sfcq' class='short_sel' id='sfsm'>
          <option title='请选择' value="wx">-请选择-</option>
           <option title='' value="ycq" >已超期</option>
           <option title='' value="wcq" >未超期</option>
          </select>
          </td>        
          
          <td class="table_query_2Col_label_6Letter">是否超期审核中：</td>
          <td><select name='sfshz' class='short_sel' id='sfsm'>
          <option title='请选择' value="wx">-请选择-</option>
           <option title='' value="yshz" >是</option>
           <option title='' value="wshz" >否</option>
          </select>
          </td>        
       <tr>
        </td>         
          <td class="table_query_2Col_label_6Letter">是否已扫描：</td>
          <td><select name='sfsm' class='short_sel' id='sfsm'>
          <option title='请选择' value="wx">-请选择-</option>
           <option title='已扫描' value="ys" selected>已扫描</option>
          </select>
          </td>        
           <td class="table_query_2Col_label_6Letter">是否经过超期审核：</td>
          <td><select name='sfcqsh' class='short_sel' id='sfcqsh'>
          <option title='请选择' value="wx">-请选择-</option>
           <option title='已扫描' value="s" selected>是</option>
            <option title='已扫描' value="f" selected>否</option>
          </select>
          </td>        
       </tr>
       <tr>
...
<script type="text/javascript">
   var myPage;
   //查询路径
   var url = "<%=contextPath%>/claim/oldPart/ClaimOldPartApporoveStorageManager/queryApproveList12.json";
   ...
   var columns = [
				...
				{id:'action',header: "操作",sortable: false,dataIndex: 'id',renderer:myLink,align:'center'},
  				...
  				{header: "是否已经超期", dataIndex: 'is_overdue', align:'center'},
  				{header: "是否扫描",dataIndex: 'is_sm',align:'center'},
   ...
   function myLink(value,meta,record){
 	 ...
 	 var is_overduecheck =record.data.is_overduecheck ;
 	   if(is_overduecheck=='<%=Constant.STATUS_ENABLE%>'){
		   return String.format("超期审核中!!!");
		   }
 	   else{
		   ...
		   if(back_type=='<%=Constant.BACK_LIST_STATUS_02%>'||back_type=='<%=Constant.BACK_LIST_STATUS_03%>'||back_type=='<%=Constant.BACK_LIST_STATUS_05%>'){
	
			   if(DateUtil.Format("yyyy-MM-dd",MonthFirstDay)<record.data.create_dates){
			   return String.format("<a href='#' onclick=isCheck12("+value+")>[入库]</a>"+"<a href='#' onclick=isCheck1("+value+")>[打印]</a>");
   ...
   function isCheck12(id){
	 //  var url= "<%=contextPath%>/claim/oldPart/ClaimOldPartApporoveStorageManager/goApproveAndStoredPage311.json?id="+id;
