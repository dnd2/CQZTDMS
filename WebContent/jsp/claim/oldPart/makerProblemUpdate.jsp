...
<div class="navigation"><img src="<%=contextPath%>/img/nav.gif"/>&nbsp;当前位置： 售后服务管理&gt;索赔旧件管理&gt;供应商问题修改</div>
  <form id="fm" name="fm">
   ...
   <TABLE class="table_query">
     <tr>
       ...
       <tr>
	         ...
	         <td align="left" nowrap="nowrap">
	          <script type="text/javascript">
	            genSelBoxExp("status",<%=Constant.PROBLEM_TYPE%>,<%=Constant.PROBLEM_TYPE_02%>,true,"short_sel","","false",'');
	           </script>
	          </td>
       ...
       <input name="remark" type="hidden">
        <input name="problem_id" type="hidden">
  </table>
  <!--分页 begin -->
...
<script type="text/javascript">
   ...
   var columns = [
  				...
  				{header: "备注", dataIndex: 'REMARK', align:'center',renderer:updateType}
  		      ];
  		      __extQuery__(1);
   function updateType(value,meta,record){
	   if(record.data.STATUS=="99971001"){//已经处理
		   return String.format(value);
	   }else{//未处理
		   return String.format("<input length='50px;' id='"+record.data.PROBLEM_ID+"' value='"+value+"'>");
	   }
   }
   ...
   function myLink(value,meta,record){
	   if(record.data.STATUS=='99971002'){
		   return String.format("<a href='<%=contextPath%>/claim/oldPart/OldPartMakerProblemManager/makerProblemDetail.do?problem_id="+value+"'>明细</href>&nbsp;&nbsp;&nbsp;<a href='#' onclick='updateRemark("+value+")'>处理</href>");
	   }else{
   ...
   function updateRemark(problem_id){
	   var remark=document.getElementById(problem_id).value;
	   $("remark").value=remark;
	   var url='<%=contextPath%>/claim/oldPart/ClaimOldPartOutStorageManager/updateRemark.json?remark='+remark+'&problem_id='+problem_id;
	   sendAjax(url, back, 'fm');
   }
   function back(json){
	   if(json.succ==1){
		   MyAlert("处理成功");
		   window.location.href='<%=contextPath%>/claim/oldPart/OldPartMakerProblemManager/makerProblemUpdate.do';
	   }else{
		   MyAlert("处理失败");
	  }
   }
