...
<title>旧件运输方式修改</title>
<% 
   String contextPath = request.getContextPath();
%>
</head>
<body >
<div class="navigation"><img src="<%=contextPath%>/img/nav.gif"/>&nbsp;当前位置：售后服务管理&gt;索赔旧件管理&gt;旧件运输方式修改</div>
 <form method="post" name ="fm" id="fm">
 	<input type=hidden value="1" name="COMMAND" id="COMMAND"/>
 	<input type=hidden value="${DEALER_CODE }" name="dealerCode" id="dealerCode"/>
 	<input type=hidden value="${listBean[0].transportId }" name="TRANSPORT_ID" id="TRANSPORT_ID"/>
       <table id="itemTableId" border="0" align="center" cellpadding="0"
				...
				<c:forEach items="${listBean}" var="bean">
					<tr>
						<td><input type="text" id="TRANSPORT_NAME" datatype="0,is_null" name="TRANSPORT_NAME" value="${bean.transportName }"/>
							<input type="hidden" id="DETAIL_ID" name="DETAIL_ID" value="${bean.detailId }"/>
						</td>
						<td><input type="text" class="phone_txt" id="LINK_PHONE" datatype="0,is_phone,11" name="LINK_PHONE" value="${bean.linkPhone }"/></td>
						<td><input type="text" class="little_txt" id="ARRIVE_PLACE" datatype="0,is_null" name="ARRIVE_PLACE" value="${bean.arrivePlace }"/></td>
						<td><input type="text" class="little_txt" id="SEND_PERSON" datatype="0,is_null" name="SEND_PERSON" value="${bean.sendPerson }"/></td>
						<td><input type="text" class="phone_txt" id="SEND_PHONE" datatype="0,is_phone,11" name="SEND_PHONE" value="${bean.sendPhone }"/></td>
						<td>
						<script type="text/javascript">
						genSelBoxExp("ARRIVE_WAY",<%=Constant.SP_JJ_TRANSPORT_SENDWAY%>,"${bean.arriveWay }",false,"min_sel","","false",'');
						</script>
						</td>
						<td><input type="text" class="little_txt" id="PRICE_WEIGHT" datatype="0,is_double" name="PRICE_WEIGHT" value="${bean.priceWeight }"/></td>
						<td><input type="text" class="little_txt" id="PRICE_CUBIC" datatype="0,is_double" name="PRICE_CUBIC" value="${bean.priceCubic }"/></td>
						<td><input type="text" class="little_txt" id="PRICE_OTHER" datatype="1,is_double" name="PRICE_OTHER" value="${bean.priceOther }"/></td>
						<td><input type="text" class="little_txt" id="SEND_COSTS" datatype="1,is_double" name="SEND_COSTS" value="${bean.sendCosts }"/></td>
						<td>
							<script type="text/javascript">
							genSelBoxExp("RETURN_TYPE",<%=Constant.BACK_TRANSPORT_TYPE%>,"${bean.returnType }",false,"min_sel","","false",'');
							</script>
						</td>
						<td><input type="text" id="REMARK" datatype="1,is_textarea,1000" name="REMARK" value="${bean.remark }"/></td>
						<td><input type="button" class="normal_btn"  value="删除"  name="button42" onClick="javascript:delTransportItem(this);"/></td>
					</tr>
				</c:forEach>
     ...
     <table class="table_list">
      <tr > 
       <td height="12" align="center">
       		<input type="button" id="save_btn" onclick="saveDo(1);" class="normal_btn" style="width=8%" value="保存"/>
      		<input type="button" id="commit_btn" onclick="saveDo(2);" class="normal_btn" style="width=8%" value="提交"/>
          <input type="button" id="back_btn"  onclick="history.back();" class="normal_btn" style="width=8%" value="返回"/>
       </td>
      </tr>
    </table>  
...
//动态生成表格
	...
	if (tableId=='transportTable') {
		...
		addTable.rows[length].cells[0].innerHTML = '<td><input type="text" name="TRANSPORT_NAME" datatype="0,is_null"  value="" size="20" id="TRANSPORT_NAME"/>'+
		'<input type="hidden" name="DETAIL_ID" value="" id="DETAIL_ID"/>'
		+'</td>';
		addTable.rows[length].cells[1].innerHTML =  '<td><input type="text" class="phone_txt" name="LINK_PHONE" datatype="0,is_phone,11"  value="" size="10" id="LINK_PHONE"/></td>';
		...
		addTable.rows[length].cells[11].innerHTML = '<td><input type="text" name="REMARK"  datatype="1,is_textarea,1000" value="" size="20" id="REMARK"/></td>';
	...
	{   
		...
		var msg = val==1?'保存':'提交';
		MyConfirm("确认"+msg+"！",checkConfirm, [val]);
	}
	function checkConfirm(val,transportId)
	{
		if(val == 1){
		...
		makeNomalFormCall("<%=contextPath%>/claim/oldPart/ClaimOldPartTransportManager/modifyTransportDetail.json?flag="+val,addSpeciaBack,'fm','queryBtn'); 
	}
