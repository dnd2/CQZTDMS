...
　　} 
	...
	<BODY onload="doInit();" onkeydown="keyListnerResp();">
		...
		<form method="post" name="fm" id="fm">
			...
			<input type="hidden" name="IN_MILEAGE" value="<%=CommonUtils.checkNull(tawep.getInMileage())%>"/>
            ...
            <table class="table_edit" id="outId" 
            	<%if(Constant.REPAIR_TYPE_04.toString().equals(tawep.getRepairTypeCode().toString())){ %>
            		style="display:none"
            	<%} %>
            >
          	<tr>
			...
			<table border="0" cellspacing="0" cellpadding="0" class="table_edit">
				<tr>
					<td align="center">
						...
						<input type="button"  onclick="approve1()" class="normal_btn" id="mySureBtn" value="确定" />
		...
		<script type="text/javascript">
			function goBack2(){
				location = '<%=contextPath%>/repairOrder/RoMaintainMain/roForward.do' ;
			}
			...
			function approve1() {
				...
				MyDivConfirm("您确定提交预授权申请吗？",app)
			}
			function app(){
				var url = '/CHANADMS/repairOrder/RoMaintainMain/accredit.json';
   ...
   if($('REPAIR_TYPE').value==<%=Constant.REPAIR_TYPE_01%>){
	...
	document.getElementById("REPAIR_TYPE").disabled = false;
	document.getElementById("YIELDLY").disabled = true;
