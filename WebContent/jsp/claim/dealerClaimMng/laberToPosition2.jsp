...
<form name='fm' id='fm' method="post">
  ...
  <img src="<%=contextPath%>/img/nav.gif" />&nbsp;当前位置： 售后服务管理&gt;售后部位&gt;工时打折明细</div>
  <table>
  <tr><td><input type="hidden"  id="largess_type" name ="largess_type" value="${largess_type}"/><input type="hidden"  id="activityId" name ="activityId" value="${activityId}"/> </td>
   ...
   <tr>            
    ...
    <br/> 
    <tr><td colspan="4" align="center"><input  class=normal_btn onclick=__extQuery__(1);  value=查询 type=button name=button/> 
    <input  class=normal_btn onclick="_hide();"  value=关闭 type=button name=button/>
   &nbsp;&nbsp; 折扣：<input  class=little_txt id=id1 disabled="disabled"  value="${zhekou }%" type=text name=button/>
...
var myPage;
	...
	var columns = [
				{header: "工时代码",sortable: false,dataIndex: 'PROJECT_CODE',align:'center'},				
				{header: "工时名称",sortable: false,dataIndex: 'PROJECT_NAME',align:'center'}
		      ];
	__extQuery__(1);
</script>
</form>
