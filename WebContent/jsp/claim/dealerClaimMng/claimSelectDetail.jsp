...
<body>
<%
    int IsInvoice = (Integer)request.getAttribute("IsInvoice");  
...
<form method="post" name="fm" id="fm">
		...
		<table class="table_list" name="all_group_" style="border-bottom: 1px solid #DAE0EE;" id="labourTable">
			...
			%>
		     	 ...
		     	 <tr class="<%=(i%2)==0?"table_list_row1":"table_list_row2"%>">
			     	 ...
			     	 <td> <input type="checkbox" <%  if(IsInvoice==1 ){%>  disabled="true"  <%}%>  name="labourIds"   value="<%=mainLabourPO.getLabourId()%>"/><input type="hidden" name="labourIdsCode" value="<%=mainLabourPO.getLabourCode()%>"  />  </td>    
				     ...
				     <td rowspan="2">
						<textarea  rows="5" name="authremark<%=mainLabourPO.getLabourId()%>" id="authremark<%=mainLabourPO.getLabourId()%>" readonly="readonly" maxlength="100"><%=CommonUtils.checkNull(mainLabourPO.getAuthRemark()) %></textarea>
				     </td>
				    </tr>  
	...
	<div align="center" > 
	  <c:if test="${type != 1}">
	  <input class="normal_btn" type="button" value="作废" onclick="unselectCounter();"/>
	  </c:if>
...
<script type="text/javascript">
	  function unselectCounter(){
		   	var r=document.getElementsByName("labourIds");
			    for(var i=0;i<r.length;i++){
			  	if(r[i].checked) {
						 MyConfirm("是否反索赔作废？",commitCounter);
					}else{
						 MyAlert("请选择反索赔工时！");
				}
	       }
	  ...
	  function backdoSummit(json){
		  if(json.succ=="1"){
			  var labourIds= document.getElementsByName('labourIds');
		  		fm.action='<%=contextPath%>/claim/dealerClaimMng/ClaimBillTrack/claimSelectLabourids.do?countlabourIds='+labourIds.length;
				fm.method="post";
				fm.submit();
		  }else{
			  MyAlert("提示：插入记录数据失败！");
