...
<script language="JavaScript" >
	...
	var url = "<%=contextPath%>/claim/dealerClaimMng/ClaimBillMaintainMain/queryPartCode5.json";
				
	var title = null;

	var columns = [
				{id:'action',header: "选择",sortable: false,dataIndex: 'partId',renderer:mySelect,align:'center'},
				{header: "自费工单", dataIndex: 'roNo', align:'center'},
	...
	function mySelect(value,metaDate,record){
		 return String.format("<input type='radio' name='rd' onclick='setMainPartCode(\""+record.data.partId+"\",\""+record.data.partCode+"\",\""+record.data.partName+"\",\""+record.data.claimPriceParam+"\",\""+record.data.supplierCode+"\",\""+record.data.supplierName+"\",\""+record.data.fore+"\",\""+record.data.partLevel+"\",\""+record.data.roNo+"\")' />");
	}

	function setMainPartCode(v1,v2,v3,v4,v5,v6,v7,v8,v9){
		 //调用父页面方法
		 if(v2==null||v2=="null"){
		 	v2 = "";
		 }
 		...
 		if (parent.$('inIframe')) {
 			parentContainer.setMainPartCode(v1,v2,v3,v4,v5,v6,v7,v8,v9);
 			parentContainer.setPartCodes();
 		} else {
			parent.setMainPartCode(v1,v2,v3,v4,v5,v6,v7,v8,v9);
			parentContainer.setPartCodes();
		}
