...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	var claimType = <%=tawep.getRepairTypeCode() %>;
function init(){
			/**
    		更具工单类型转变为索赔类型
    		*/
    		
    		if(claimType=='<%=Constant.REPAIR_TYPE_01%>'){
    			claimType = <%=Constant.CLA_TYPE_01%>;
    		}else if(claimType=='<%=Constant.REPAIR_TYPE_04%>'){ //免费保养
    			claimType = <%=Constant.CLA_TYPE_02%>;
    		}else if(claimType=='<%=Constant.REPAIR_TYPE_05%>'){
    			claimType =<%=Constant.CLA_TYPE_06%> ;
    		}else if(claimType=='<%=Constant.REPAIR_TYPE_03%>'){
    			claimType =<%=Constant.CLA_TYPE_07%> ;
    		}else if(claimType=='<%=Constant.REPAIR_TYPE_02%>'){
    			claimType = <%=Constant.CLA_TYPE_09%>;
    		}else if(claimType=='<%=Constant.REPAIR_TYPE_06%>'){
    			claimType =<%=Constant.CLA_TYPE_10%> ;
    		}else if(claimType=='<%=Constant.REPAIR_TYPE_07%>'){
    			claimType = <%=Constant.CLA_TYPE_12%>;
    		}else if(claimType=='<%=Constant.REPAIR_TYPE_08%>'){
    			claimType = <%=Constant.CLA_TYPE_11%>;
    		}
    		$('CLAIM_TYPE').value = claimType;
	if(claimType==<%=Constant.CLA_TYPE_02 %>){
		$('otherTableId').style.display="none";
		document.getElementById('total').innerHTML='总金额(含税)：'+<%=tawep.getFree() %>+'元整';
	}else{
		$('otherTableId').style.display="";
		document.getElementById('total').innerHTML='总金额(含税)：'+<%=tawep.getRepairAmount() %>+'元整';
	}
	}
</SCRIPT>
	</HEAD>
	<BODY onload="init();" onkeydown="keyListnerResp();">
		<div class="navigation">
			<img src="../../../img/nav.gif" />
			&nbsp;当前位置：售后服务管理&gt;索赔申请&gt;索赔单新增
		</div>

		<form method="post" name="fm" id="fm">
			<input type="hidden" name="roId" id ="roId" value="<%=tawep.getId()%>" />
			<input name="CLAIM_TYPE" id="CLAIM_TYPE" type="hidden"  value=""/>
			<input type="hidden" name="claimType" value="<%=tawep.getRepairTypeCode()%>"/>
				...
				<tr>
					...
					<td>
						...
						<script type="text/javascript">
							document.write(getItemValue(''+claimType+''));
	       				</script>
						</span>
					</td>
				</tr>
			...
			<table id="otherTableId" align="center" cellpadding="0"
				...
				<th colspan="8" align="left">
					<img class="nav" src="../../../img/subNav.gif" />
					&nbsp;<a href="#" onclick="hiddenTable('otherTable');" >其他项目</a>
				</th>
				...
				<tbody id="otherTable">
					...
					<tr class="table_list_row1" >
						<td>
						   ...
						   <%=otherLs.get(i).get("ADD_ITEM_CODE") %> 
						   <input type="hidden"  id="ITEM_CODE" name="ITEM_CODE" value="<%=otherLs.get(i).get("ADD_ITEM_CODE") %>" />
						 
						</td>
						<td>
						 <%=CommonUtils.checkNull(otherLs.get(i).get("ADD_ITEM_NAME") )%> 
						 <input type="hidden"  id="ITEM_NAME" name="ITEM_NAME" value="<%=CommonUtils.checkNull(otherLs.get(i).get("ADD_ITEM_NAME") )%> " />
						</td>
						<td>
						  <input type="text"  class="little_txt" name="ITEM_AMOUNT" id="ITEM_AMOUNT" 

   						 readonly="readonly" maxlength="10"  value="<%=CommonUtils.checkNull(otherLs.get(i).get("ADD_ITEM_AMOUNT"))%>"  />
						</td>
						<td title="<%=otherLs.get(i).get("REMARK")%>">
						<input type="text" name="ITEM_REMARK"  id="ITEM_REMARK" datatype="0,is_null,1000" value="<%=otherLs.get(i).get("REMARK")%>" class="middle_txt"  />
						</td>
					</tr>
					<%
						}
		...
		<table border="0" align="center" cellpadding="1" cellspacing="1"
				...
				<th colspan="6">
					<img class="nav" src="../../../img/subNav.gif" />
					申请费用：<span style="color:red" id="total">总金额(含税)：</span>
				</th>
			</table>
            <!-- 添加附件 -->
