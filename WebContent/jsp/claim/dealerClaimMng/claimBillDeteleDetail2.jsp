...
function init(){
	...
	%>
	    	<table border="0" align="center" cellpadding="1" cellspacing="1"
				...
				<tr>
					...
					<td  class="tbwhite">
						<textarea name='ADUIT_REMARK' disabled="disabled" id='AUDIT_REMARK'	datatype="1,is_textarea,1000" maxlength="1000" rows='3' cols='28'><%=CommonUtils.checkNull(mainCodeList.get(a).get("REMARK")) %></textarea>
					</td>
					...
					<td  class="tbwhite">
						<textarea name='TROUBLE_REASON' disabled="disabled" id='TROUBLE_REASON'	datatype="1,is_textarea,1000" maxlength="1000" rows='3' cols='28'><%=CommonUtils.checkNull(mainCodeList.get(a).get("TROUBLE_REASON")) %></textarea>
					</td>
					<td  class="tbwhite">
						处理结果：
					</td>
					<td  class="tbwhite">
						<textarea name='DEAL_METHOD' id='DEAL_METHOD' disabled="disabled"	datatype="1,is_textarea,1000" maxlength="1000" rows='3' cols='28'><%=CommonUtils.checkNull(mainCodeList.get(a).get("DEAL_METHOD")) %></textarea>
					</td>
				</tr>
					...
					<tr class="table_list_row1">
						<td>
						 <%=CommonUtils.checkNull(tl.getDownPartCode())%> 
						  <input type="hidden" name="DOWN_PART_CODE" id="DOWN_PART_CODE<%=tl.getPartId().toString()+i %>" value="<%=CommonUtils.checkNull(tl.getPartCode())%>" />
						</td>
						<td>
	...
	<!-- 样式表格结束 -->
		...
		<input class="normal_btn" type="button" value="废弃" id="delBtn" onclick="del(<%=id %>);"/>
		...
		function del(id){
			if(id!=""){
				...
				MyAlert("未获取到单据标识,请联系管理员！");
		}
	}
function deleteApply(id) {
$('delBtn').disabled=true;
	var url= "<%=contextPath%>/claim/dealerClaimMng/ClaimBillTrack/deleteApplication.json?ID="+id;
...
function delAfter(json){
		...
		if(str==1){
			MyAlert("废弃成功!");
			location.href="<%=contextPath%>/claim/dealerClaimMng/ClaimBillTrack/claimBillDeleteForward.do";
		}else{
			MyAlert("废弃失败,请联系管理员!");
	}
}
 function setStyles ( i ){
		...
		function threePackageSet(){
				...
				var arr = document.getElementsByName('DOWN_PART_CODE');
