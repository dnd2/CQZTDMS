...
function init(){
	...
	if($('claimType').value==<%=Constant.CLA_TYPE_09 %> || $('claimType').value==<%=Constant.CLA_TYPE_13 %>){
	...
	<BODY onload="init();" onkeydown="keyListnerResp();">
		...
		<form method="post" name="fm" id="fm">
			...
			<input type="hidden" name="attIds" id="attIds" value=""/>
	...
	%>
	    	<table border="0" align="center" cellpadding="1" cellspacing="1"
				...
				<tr>
					...
					<%
					if(mainCodeList.get(a).get("AUDIT_STATUS").toString().equalsIgnoreCase(Constant.PART_AUDIT_STATUS_01.toString())){%><!-- 审核中 -->
				...
				<tr>
					...
					<%
					if(mainCodeList.get(a).get("AUDIT_STATUS").toString().equalsIgnoreCase(Constant.PART_AUDIT_STATUS_01.toString())){%><!-- 审核中 -->
						<textarea name='REMARK<%=mainCodeList.get(a).get("PART_ID") %>'  id='REMARK'	datatype="0,is_null,1000" maxlength="1000" rows='3' cols='28'><%=CommonUtils.checkNull(mainCodeList.get(a).get("REMARK")) %></textarea>
					<%}else{%><!-- 挂起 -->
		 				<textarea name='REMARK<%=mainCodeList.get(a).get("PART_ID") %>' readonly="readonly"  id='REMARK' rows='3' cols='28'><%=CommonUtils.checkNull(mainCodeList.get(a).get("REMARK")) %></textarea>
		 			<%}%> 
					...
					<td  class="tbwhite">
						<%
					if(mainCodeList.get(a).get("AUDIT_STATUS").toString().equalsIgnoreCase(Constant.PART_AUDIT_STATUS_01.toString())){%><!-- 审核中 -->
					<textarea name='TROUBLE_REASON<%=mainCodeList.get(a).get("PART_ID") %>'  id='TROUBLE_REASON'	datatype="0,is_null,1000" maxlength="1000" rows='3' cols='28'><%=CommonUtils.checkNull(mainCodeList.get(a).get("TROUBLE_REASON")) %></textarea>
					<%}else{%><!-- 挂起 -->
		 			<textarea name='TROUBLE_REASON<%=mainCodeList.get(a).get("PART_ID") %>' readonly="readonly"  id='TROUBLE_REASON' rows='3' cols='28'><%=CommonUtils.checkNull(mainCodeList.get(a).get("TROUBLE_REASON")) %></textarea>
					<%}%>
					</td>
					<td  class="tbwhite">
						处理结果：
					...
					<%
					if(mainCodeList.get(a).get("AUDIT_STATUS").toString().equalsIgnoreCase(Constant.PART_AUDIT_STATUS_01.toString())){%><!-- 审核中 -->
						<textarea name='DEAL_METHOD<%=mainCodeList.get(a).get("PART_ID") %>' id='DEAL_METHOD' 	datatype="0,is_null,1000" maxlength="1000" rows='3' cols='28'><%=CommonUtils.checkNull(mainCodeList.get(a).get("DEAL_METHOD")) %></textarea>
					<%}else{%><!-- 挂起 -->
		 				<textarea name='DEAL_METHOD<%=mainCodeList.get(a).get("PART_ID") %>' readonly="readonly" id='DEAL_METHOD'  rows='3' cols='28'><%=CommonUtils.checkNull(mainCodeList.get(a).get("DEAL_METHOD")) %></textarea>
					<%}%>
					</td>
				</tr>
				</table>
				...
				<tbody id="itemTable">
					...
					<tr class="table_list_row1">
						...
						<td>
							 <%=CommonUtils.checkNull(tl.getBalanceAmount())%> 
						</td>
			...
			<table id="partTableId" border="0" align="center" cellpadding="0"
				...
				<tbody id="partTable">
					...
					<tr class="table_list_row1">
						<td>
						<input name="oldPartCode" readonly="readonly" id ="oldPartCode<%=tl.getPartId()%>" value="<%=CommonUtils.checkNull(tl.getDownPartCode())%>" class="middle_txt" />
						 <%
					if(mainCodeList.get(a).get("AUDIT_STATUS").toString().equalsIgnoreCase(Constant.PART_AUDIT_STATUS_01.toString())){%><!-- 审核中 -->
						<input onclick="selectDownPartCode('<%=tl.getPartId()%>','<%=tl.getDownPartCode() %>');" type="button" id="btn2<%=tl.getPartId()%>" style="display: " class="normal_btn" value="选择" name="downCode<%=tl.getPartId()%>"  />
				
				<%	}else{%><!-- 挂起 -->
		 				<input onclick="selectDownPartCode('<%=tl.getPartId()%>','<%=tl.getDownPartCode() %>');" type="button" id="btn2<%=tl.getPartId()%>" style="display:none " class="normal_btn" value="选择供" name="downCode<%=tl.getPartId()%>"  />
					<%	}%> 
						</td>
						<td >
						<input name="oldPartName" id="oldPartName<%=tl.getPartId()%>" value="<%=CommonUtils.checkNull(tl.getDownPartName())%>" class="middle_txt" readonly="readonly" />
			...
			<table class="table_list"id="accessoriess"  cellpadding="0" cellspacing="1">
             ...
             <%=accessoryDtlList.get(i).get("WORKHOUR_NAME") %>
               ...
               <td>
               <%=accessoryDtlList.get(i).get("APP_PRICE") %>
               </td>
               <td>
	...
	<!-- 样式表格结束 -->
		...
		var paId="";
		var tempCode="";
		function selectDownPartCode(id,partcode){
			paId="";
			tempCode="";
		//参数：旧件的主键ID，旧件代码
		 	var vin=$('VIN').value;
		 	paId = id;
		 	tempCode=partcode;
	 		OpenHtmlWindow('<%=contextPath%>/claim/dealerClaimMng/ClaimBillMaintainMain/selectDownPartCodeForward.do?GROUP_ID='+modelId+'&partCode='+partcode+'&vin='+vin,800,500);
				}
		function setDownPartCode(partId,partCode,partName,stockPrice,supplierCode,supplierName,oldPartCode) {
			//由于为了不修改以前的选择弹出框,所以返回参数较多，但是只使用到了 前3个
			if(confirm("确认修改?")){	
			$('saveBtn').disabled=true;
			$('btn2'+paId).disabled=true;
			var url= "<%=contextPath%>/claim/application/ClaimManualAuditing/modifyDownCode.json?ID="+paId+"&code="+partCode+"&tempCode="+tempCode+"&claimId="+$('ID').value+"&oldPartCode="+oldPartCode;
			makeNomalFormCall(url,molACodefter,'fm','');
		}
		}		
	function molACodefter(json){
		var str = json.ACTION_RESULT;
		if(str==1){
			MyAlert("修改成功!");
			$('saveBtn').disabled=false;
			$('btn2'+paId).disabled=false;
			$('oldPartName'+paId).value=json.partname;
			$('oldPartCode'+paId).value=json.partcode;
		}else{
			MyAlert("修改失败,请联系管理员!");
			$('saveBtn').disabled=false;
			$('btn2'+paId).disabled=false;
		...
		var url= "<%=contextPath%>/claim/application/ClaimManualAuditing/modifySuppCode.json?ID="+id+"&code="+code+"&adType=1";
 ...
 function setStyles ( i ){
		...
		function threePackageSet(){
				...
				var arr = document.getElementsByName('oldPartCode');
