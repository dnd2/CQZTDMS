...
<%@page import="com.infodms.dms.po.TtAsWrVinRulePO"%>
<%
	String contextPath = request.getContextPath();
	//TtAsWrVinRulePO rulePo = (TtAsWrVinRulePO)request.getAttribute("ruleList");
%>
...
//新增三包规则 开始
	function checkedAdd(){
		...
		//检验是否存在
		makeFormCall('<%=contextPath%>/claim/basicData/ClaimWrRule/checkUnique.json?sta=2',checkUnique,'fm');	
	}
	...
	function checkUnique(json){
		if(json.success != null && json.success == "false"){
			...
			MyConfirm("是否确认修改？",claimWrRuleAdd);
		}else{
	...
	function claimWrRuleAdd(){
		...
		makeFormCall( '<%=contextPath%>/claim/basicData/ClaimWrRule/claimWrRuleUpdate.json',showResult,'fm');
	}
 ...
 function doCusChange(){
	 var vrtype = document.getElementById("vrType").value;
	  	if(vrtype == '<%=Constant.VR_TYPE_1 %>'){
		  	...
		  	document.getElementById("partWrType").value = "${wrRuleList.partWrType}";
		  	...
		  	document.getElementById('trid').disabled = false;
	  	  	if(partWrType == '<%=Constant.PART_WR_TYPE_1%>'){
	  	  		...
	  	  		document.getElementById("part").value = "${wrRuleList.vrPartCode}";
	  	  	}else if(partWrType == '<%=Constant.PART_WR_TYPE_3%>'){
	  	  		document.getElementById("part").disabled = true;
	  	  		document.getElementById("pos").disabled = false;
	  	  		document.getElementById("pos").value = "${wrRuleList.vrPartCode}";
	 	  	}
		}
 }
 //设置为只读 ***start
...
<body onload="doCusChange();selectReadOnly('spanid');selectReadOnly('spanid1');">
  <div class="navigation">
        <img src="<%=contextPath %>/img/nav.gif" />&nbsp;当前位置： 售后服务管理&gt;索赔基础数据&gt;三包预警规则维护
  </div>
  <form method="post" name="fm" id="fm">
  	<input type="hidden" name="id" class=""  value="${wrRuleList.vrId}"/>
     <table class="table_edit" >
          <tr>
            <td class="table_edit_2Col_label_7Letter">预警类型：</td>
            <td align="left"><span id="spanid">
                <script type="text/javascript">
	   					    genSelBoxExp("vrType",<%=Constant.VR_TYPE%>,"${wrRuleList.vrType}",true,"short_sel","","true",'');
	  		      </script>
	  		      </span>
            </td>
            ...
            <td align="left">
                <script type="text/javascript">
	   					    genSelBoxExp("vrLevel",<%=Constant.VR_LEVEL%>,"${wrRuleList.vrLevel}",true,"short_sel","","true",'');
	  		      </script>
            </td>
            <td>&nbsp;</td>
            <td>&nbsp;</td>
           ...
           <tr id="trid">
            ...
            <td align="left">
	             <select id="part" name="part" class="short_sel" >
						<option value=''>-请选择-</option>
						<c:forEach var="tmPtPartBasePO" items="${partList}">
								<option value="${tmPtPartBasePO.partCode}" title="${tmPtPartBasePO.partName}" >${tmPtPartBasePO.partName}</option> 
						</c:forEach>
					</select>
					<font color="red">*</font>
            </td>
          ...
          <tr id="day1" style="display:">
            ...
            <td align="left">
                <input type="text"  name="vrLaw1"  id="vrLaw1" class="little_txt" value="${wrRuleList.vrLaw }"/>天
                <font color="red">*</font>
            </td>
           <td class="table_edit_2Col_label_7Letter">预警天数：</td>
            <td align="left">
                <input type="text"  name="vrWarranty1"  id="vrWarranty1" class="little_txt" value="${wrRuleList.vrWarranty }"/>天
                <font color="red">*</font>
            </td>
             <td>&nbsp;</td>
            <td>&nbsp;</td>
           ...
           <tr id="day2" style="display:none">
            ...
            <td align="left">
                <input type="text"  name="vrLaw2"  id="vrLaw2" class="little_txt" value="${wrRuleList.vrLaw }"/>次
                <font color="red">*</font>
            </td>
           <td class="table_edit_2Col_label_7Letter">预警：</td>
            <td align="left">
                <input type="text"  name="vrWarranty2"  id="vrWarranty2" class="little_txt" value="${wrRuleList.vrWarranty }"/>次
                <font color="red">*</font>
            </td>
             <td>&nbsp;</td>
            <td>&nbsp;</td>
            ...
            <td align="left" colspan="5" >
                <textarea  name="lawStandard"  id="lawStandard" style="width:60%;height: 100%;">${wrRuleList.vrLawStandard }</textarea><font color="red">*</font>
            </td>
            </tr>
          <tr>
