<%@page import="com.infodms.dms.po.TtAsWrPartLegalPO"%>
...
<%
	...
	TtAsWrPartLegalPO po = (TtAsWrPartLegalPO)request.getAttribute("TtAsWrPartLegalPO");
%>
...
function save(){
		...
		if(part_code==null || part_code==''){
			...
			var url="<%=request.getContextPath()%>/claim/basicData/PartsLegal/saveAddParts.json";
...
function showResult22(json){
	if(json.flag){
		MyAlert("新增成功");
		location=  "<%=contextPath%>/claim/basicData/PartsLegal/modifyPartsLegal.do?fID="+json.fId;
	}else{
		MyAlert(json.msg);
	}
}
...
<form name='fm' id='fm' method="post">
  ...
  <img src="<%=contextPath%>/img/nav.gif" />&nbsp;当前位置： 售后服务管理&gt;索赔基本数据&gt;零件法定名称配件新增</div>
  <div class="form-panel">
   ...
   <table  class="table_query">
          <tr>
          	...
          	<td>
          		<input name = "ID" id="ID" type="hidden" value="<%=po.getPartLegalId()%>" /><%=po.getPartLegalCode() %>
          	</td>
          	<td style="text-align:right">零件法定名称：</td>
          	<td><%=po.getPartLegalName() %>
          </tr>
          <tr>
          	<td style="text-align:right">配件代码：</td>
          	<td>
          	<input type="text" class="middle_txt" name="PART_CODE"  onclick="selectPart();" value="" size="10" id="PART_CODE" readonly />
