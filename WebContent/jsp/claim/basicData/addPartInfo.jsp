...
<form method="post" name='fm' id='fm'>
  ...
  <img src="<%=contextPath%>/img/nav.gif" />&nbsp;当前位置： 售后服务管理&gt;索赔基本数据&gt;严重安全性能故障法定名称维护&gt;添加配件信息</div>
  <div class="form-panel">
   ...
   <table class="table_query">
          <tr>
            ...
            <td>
                 <input name="partCode" id="partCode"  type="text" class="middle_txt"/>
            </td>
            <td style="text-align:right">配件名称：</td>
            <td>
				<input name="partName" id="partName"  type="text" class="middle_txt"/>
            </td>
          </tr>
		   <tr>    
			   <td colspan="4" style="text-align:center">
	            <input class="normal_btn" type="button" name="queryBtn" value="查询"  onclick="__extQuery__(1)"/>
	            <input name="buttonImport" type="button" class="normal_btn" onclick="addParts();" value="新增配件" />
	            <input class="normal_btn" type="button" value="删除配件" name="del" onclick="onDel();"/>
	            <input name="buttonImport" type="button" class="normal_btn" onclick="importParts();" value="导入配件" />
...
var myPage;
	var url = "<%=request.getContextPath()%>/claim/basicData/FailureMaintainMain/queryFaultPart.json";
	var title = null;
...
function importParts(){
		...
		fm.action = "<%=contextPath%>/claim/basicData/FailureMaintainMain/partImport.do?faultId="+faultId;
		fm.submit();
}
function addParts(){
    var faultId=document.getElementById("faultId").value;
	fm.action = "<%=contextPath%>/claim/basicData/FailureMaintainMain/addParts.do?faultId="+faultId;
	fm.submit();
}
//返回
function onBack(){
...
function changeSubmit() {
	var url="<%=contextPath%>/claim/basicData/FailureMaintainMain/FaultPartDel.json";
...
function showResult(json){
	...
	if(msg=='01'){
		...
		MyAlert('操作失败,联系DCS系统运维团队，问题提报热线:023-67543333');
	}
}
</script>
</div>
