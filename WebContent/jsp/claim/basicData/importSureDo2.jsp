...
<table class="table_list" >  
  <tr class="table_list_row1" style="background-color:#DAE0EE; ">
    <td width="8%" align="center">序号</td>
  	<td width="23%" align="center">工时代码</td>
  	<td width="23%" align="center">工时名称</td>
  	<td width="23%" align="center">金额</td>
  	<td width="23%" align="center">状态 </td>
  </tr>
 <c:set var="count" value="${1}"/>
  ...
  %>
  	 <tr class="table_list_row1">
	  	...
	  	<td align="center"><%=map.get("2") %><input type="hidden" name="name${count }" value="<%=map.get("2") %>"/></td>
	  	<td align="center"><%=map.get("3") %></td>
	  	<td align="center"><%=map.get("4") %>
	  	<input type="hidden" name="price${count }" value="<%=map.get("3") %>"/>
	  	<input type="hidden" name="status${count }" value="<%=map.get("4") %>"/>
	  	</td>
...
<script type="text/javascript">
 ...
 //服务商资金确认导入：
	function add(parms) {
		if(confirm("确定导入?")){
			btnDisable();
		    var url = '<%=request.getContextPath()%>/claim/basicData/AccPriceMaintainMain/importAdd.json';
   ...
   function showResult(json) {
	   btnEnable();
		  if (json.success != null && json.success == "true") {
	       	   MyAlert("导入成功！");
	           window.location.href = "<%=request.getContextPath()%>/claim/basicData/AccPriceMaintainMain/accessoryPriceMaintainInit.do";
	       } else {
	           MyAlert("导入失败，请联系管理员！");
	       }
   }
   ...
   function goBack(){
		btnDisable();
		fm.action = "<%=request.getContextPath()%>/claim/basicData/AccPriceMaintainMain/inportPer.do";
		fm.submit();
	}
