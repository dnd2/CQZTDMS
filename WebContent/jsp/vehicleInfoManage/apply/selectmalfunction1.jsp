...
<body onload="__extQuery__(1)">
	...
	<form method="post" name = "fm" id="fm">	
		<table  border="0" align="center" cellpadding="1" cellspacing="1" class="table_query">
			<th colspan="4"><img class="nav" src="<%=contextPath%>/img/subNav.gif" /> 选择故障件</th>
			
			<tr>
				<td align="right" nowrap="true">故障类别代码：</td>
				<td align="left" nowrap="true">
					<input type="text" id="malcode" name="malcode"/>
				</td>
				
				<td align="right" nowrap="true">故障现象：</td>
				<td align="left" nowrap="true">
...
<script type="text/javascript">
	...
	var myPage;
    //查询路径
	var url = "<%=contextPath%>/vehicleInfoManage/check/QualityInfoReportVerify/selectmalfunction1.json";
				
	var title = null;

	var columns = [
				{id:'action',header: "选择", width:'8%',sortable: false,dataIndex: 'CLAIM_NO',renderer:myCheckBox},
				{header: "故障类别代码", dataIndex: 'MAL_CODE', style: 'text-align:center'},
				{header: "故障现象", dataIndex: 'MAL_NAME', style: 'text-align:center'},
				{header: "创建日期", dataIndex: 'CREATE_DATE', style: 'text-align:center'}
		      ];
	
	function myCheckBox(value,metaDate,record){
		return String.format("<input name='radio' type='radio' onclick='changeCheck1(this,\""+ record.data.MAL_CODE +"\",\""+ record.data.MAL_NAME +"\")'/>");
	}
	...
	function changeCheck1(checkBox,malcode,malname){
		if(checkBox.checked){
			if (parent.$('inIframe')) 
			{
				parentContainer.selectmalfunctionBack1(malcode,malname);
			}
		   _hide();
		}
	}
