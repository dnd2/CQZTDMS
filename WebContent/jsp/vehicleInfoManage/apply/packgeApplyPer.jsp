...
<img src="<%=contextPath%>/img/nav.gif" />&nbsp;当前位置:售后服务管理&gt;车辆信息管理&gt;三包凭证补办申请查询
</div>
<form name="fm" id="fm">
<!-- 查询条件 begin -->
<table width=100% border="0" align="center" cellpadding="1" cellspacing="1" class="table_query">
	<tr>
      ...
      <td>
      	<script type="text/javascript">
        	genSelBoxExp("status",<%=Constant.PACKGE_CHANGE_STATUS%>,"",true,"short_sel","","false",'');
        </script>
      </td>
    </tr>
...
<script type="text/javascript" >
	var myPage;
	var url = "<%=contextPath%>/vehicleInfoManage/apply/VehicleInfoChangeApply/packgeApplyQuery.json";
	var title = null;
	var columns = [
	...
	//
	function addRecord() {
		fm.action = '<%=contextPath%>/vehicleInfoManage/apply/VehicleInfoChangeApply/packgeApplyAdd.do?type=0';
		 fm.method="post";
		fm.submit();
	}
	//点击索赔申请
	function oper(value,meta,record) {
		if (record.data.APPLY_STATUS == 95441001||record.data.APPLY_STATUS == 95441003||record.data.APPLY_STATUS ==95441005) {
	...
	function viewModDetail(id) {
		fm.method = 'post';
		fm.action = '<%=contextPath%>/vehicleInfoManage/apply/VehicleInfoChangeApply/packgeApplyAdd.do?type=1&id='+id;
		fm.submit();
	}	
	//详细信息查看页面
	...
	if (confirm("是否上报?")){
		var url = '<%=contextPath%>/vehicleInfoManage/apply/VehicleInfoChangeApply/packgeApplySave.json?type=3&id='+id;
