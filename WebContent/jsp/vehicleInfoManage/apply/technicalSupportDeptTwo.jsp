...
<img src="<%=contextPath%>/img/nav.gif" />&nbsp;当前位置：售后服务管理&gt;车辆信息管理&gt;二级质量信息审核
</div>
     ...
     <tr>
     	...
     	<td>
     		<script type="text/javascript">
        		genSelBoxExp("status",<%=Constant.Quality_Verify%>,"<%=Constant.Quality_Verify_03%>",true,"","","false",'<%=Constant.Quality_Verify_01%>');
       		</script>
     	</td>
...
<script type="text/javascript" >
	var myPage;
	var url = "<%=contextPath%>/vehicleInfoManage/apply/QualityReportInfo/technicalSupportDeptTwoInfo.json";
	var title = null;
	var columns = [
	...
	function myLink(value,meta,record){
		if(record.data.VERIFY_STATUS == 95531003 && record.data.ISEND!="isend" ){
			return String.format("<input name='detailBtn' type='button' class='normal_btn' onclick='audit(\""+ value +"\")' value='审核'/><input name='detailBtn' type='button' class='normal_btn' onclick='viewDetail(\""+ value +"\")' value='详细'/>");
		}else if(record.data.VERIFY_STATUS == 95531006 && record.data.ISEND!="isend" ){
			return String.format("<input name='detailBtn' type='button' class='normal_btn' onclick='audit(\""+ value +"\")' value='发送'/>");
		}else{
	...
	function audit(value){		
		window.location.href='<%=contextPath%>/vehicleInfoManage/apply/QualityReportInfo/auditByTechnicalDept.do?qualityid='+value+'&updateId=1';
	}
	function wrapOut(){
