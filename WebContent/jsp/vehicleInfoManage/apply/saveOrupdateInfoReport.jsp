...
// 		$("#reportDate").val(currTime);	
		var type = $("#type").val();
		if("update"==type){//修改时执行
			$("#reportDate").val("${po.reportDate }");	
		}	
	...
	<table border="1" cellpadding="1" cellspacing="1" class="tab_edit" width="100%" style="text-align: center;">
		...
		<tr>
			<td nowrap="true">
				...
				<div style="display: none;">及时性得分：<input type="text" disabled="disabled" class="middle_txt" name="inTimeScore"  readonly="readonly" value="${po.inTimeScore }"/>  
				填报质量得分：<input type="text" disabled="disabled" class="middle_txt" name="addQualityScore"  readonly="readonly" value="${po.addQualityScore }"/>  
				&nbsp;&nbsp;&nbsp;总分：<input type="text" disabled="disabled" class="middle_txt"  name="countScore"  readonly="readonly" value="${po.countScore }"/>  
			    </div>
			</td>
		</tr>
		<tr>
    		...
    		<input type="radio" id="isnotfit" name="isFit" class="isfit" value="否"/>否
				&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
				&nbsp;&nbsp;&nbsp;&nbsp;	填报日期:
				<input type="text" id="reportDate" name="reportDate"class="middle_txt"  readonly="readonly" value="<fmt:formatDate value='${newdate }' pattern='yyyy-MM-dd'/>"/> 
			</td>
		</tr>
	</table>
</div>
<table border="1"   class="tab_edit" width="100%" style="text-align: left;">
    ...
    <tr >
    	...
    	<td nowrap="true">
    		<input type="text" id="mileage" readonly="readonly" class="middle_txt" name="mileage"  value="${po.mileage}" datatype="0,is_double" decimal="1"/>
    	</td>
    	<td nowrap="true">
    		生产日期:
    	</td>
    ...
    <tr >
    	...
    	<td colspan="5" nowrap="true">
    		&nbsp;行驶速度：<input id="runSpeed" onblur="checkNum(this);" type="text" class="short_txt" name="runSpeed"  maxlength="4" value="${po.runSpeed}"/>Km/h;
    		&nbsp;发动机转速：<input type="text"onblur="checkNum(this);" id="engineSpeed" class="short_txt"  name="engineSpeed" maxlength="4" value="${po.engineSpeed}"/>rpm; 
    		&nbsp;其他：<input type="text" name="carStatusRemark" maxlength="40" class="middle_txt" value="${po.carStatusRemark }" />
    	</td>
    <tr >
    	<td nowrap="true">
    		路面情况:
		...
		<table  class="table_list" border="0" >
				...
				<c:forEach items="${dealRecordList}" var="dealR">
					<tr class="table_list_row2">
						<td class="center">${dealR.AUDIT_DATE}</td>
						<td class="left">${dealR.AUDIT_ADVICE}</td>
						<td class="center">${dealR.AUDIT_NAME}</td>
						<td class="center">${dealR.STATUS}</td>
					</tr>
				</c:forEach>
			</table>
		<br/>
...
<script type="text/javascript">
	...
	$("#saveOrupdate").live("click",function(){
		...
		var str="mileage,reportName,contactType,runSpeed,engineSpeed,firstProblemCode,firstProblemName,firstProblemSupplierCode,listGroup";
		var textStr="里程,填报人,联系方式,行驶速度,发动机怠速,主故障件代码,主故障件名称,主故障件供应商代码,目录分组";
        if(str!=""){
			var strs= str.split(",");
		...
		if(checkPass()==true){
        	...
        	var date1=$("#serviceDate").val();
			var date2=$("#faultDate").val();
			if(""==date1 || ""==date2){
				MyAlert("提示：请选择故障日期或者维修日期！");
				return;
        	}
	...
	function saveOrUpdateMsg(){
		var url="<%=contextPath%>/vehicleInfoManage/apply/QualityReportInfo/saveOrupdateInfo.do";
		sendDo(url);
	}
	$("#report").live("click",function(){
		...
		if(checkPass()==true){
        	...
        	if("null"==date2 || "null"==date1 || null==date1 || null==date2 ){
        		MyAlert("提示：故障日期或者维修日期含有非法字符！");
				return;
            }
	...
	function checkPass(){
		textAlert="";
		var fields="theme,customerComplainNeed,problemReason,checkStepFit,dealAdvice";
		checkNull(fields);
		if(textAlert!=""){
			return false;
		}
	...
	//填写索赔单号再取到VIN然后带出信息
	function changeDataFrist(claim_no,vin,create_date,type){
		 ...
		 if("no"==type){
			 ...
			 $("#mileage").removeAttr("readonly","");$("#faultDate,#serviceDate").val(create_date);
				$("#vin").focus(); 
				 $("#faultDate,#serviceDate").val("");
	...
	$("#isnotfit").live("click",function(){
		...
		MyAlert("提示：请手动输入底盘号！");
		$("#vin").focus(); 
		$("#faultDate").removeAttr("readonly","");
		$("#serviceDate").removeAttr("readonly","");
		$("#faultDate").bind("focus",function(){
			calendar();
		});
		$("#serviceDate").bind("focus",function(){
			calendar();
		});
		
	});
	//======================2015-8-5维修选择vin实现维修选择否的功能
	function changeDataFrist1(vin){//
		$("#carUseType").val("");//什么类型的车
		$("#carType").val("");//车型ID
		$("#carClass").val("");//车系ID
		$("#carNo").val("");//车牌号
