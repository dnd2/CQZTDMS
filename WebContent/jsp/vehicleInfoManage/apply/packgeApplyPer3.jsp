...
<img src="<%=contextPath%>/img/nav.gif" />&nbsp;当前位置:售后服务管理&gt;车辆信息管理&gt;三包凭证补办申请车厂审核查询
</div>
<form name="fm" id="fm">
<!-- 查询条件 begin -->
...
<input name="fax" id ="fax" type="hidden" value=""/>
<input name="address" id =""address"" type="hidden" value=""/>
	<tr>
      ...
      <td>
      	<script type="text/javascript">
        	genSelBoxExp("status",<%=Constant.PACKGE_CHANGE_STATUS%>,"",true,"short_sel","","false",'<%=Constant.PACKGE_CHANGE_STATUS_01%>,<%=Constant.PACKGE_CHANGE_STATUS_02%>,<%=Constant.PACKGE_CHANGE_STATUS_03%>');
        </script>
      </td>
    </tr>
  <tr>            
...
<script type="text/javascript" >
	var myPage;
	var url = "<%=contextPath%>/vehicleInfoManage/apply/VehicleInfoChangeApply/packgeApplyQuery3.json";
	var title = null;
	var columns = [
				...
				{header: "打印时间", dataIndex: 'PRINT_TIME', align:'center'},
				{header: "打印人", dataIndex: 'NAME', align:'center'},
	...
	function oper(value,meta,record) {
		 if (record.data.APPLY_STATUS == 95441006) {
			return String.format("<a href=\"#\" onclick='viewDetail(\""+record.data.ID+"\")'>[明细]</a>"+"<a href=\"#\" onclick='viewPrint(\""+record.data.ID+"\")'>[打印]</a>");
		}else if (record.data.APPLY_STATUS == 95441004) {
	...
	function viewPrint(id) {
		...
		//fm.method = 'post';
		//fm.action = '<%=contextPath%>/vehicleInfoManage/apply/VehicleInfoChangeApply/packgeAppviewPrint.do?id='+id;
		//fm.submit();
		var str = window.showModalDialog('<%=contextPath%>/jsp/vehicleInfoManage/apply/inputInfo1.jsp',id,'dialogWidth=600px;dialogHeight=400px');
		if(str!="" && str !=undefined){
		if(confirm("确认打印?")){
		window.open('<%=contextPath%>/vehicleInfoManage/apply/VehicleInfoChangeApply/packgeAppviewPrint.do?id='+id+"&name="+encodeURI(str.split("$")[0])+"&fax="+str.split("$")[1]+"&address="+encodeURI(str.split("$")[2]),"三包凭证打印", "height=1000, width=1000, top=0, left=0, toolbar=no, menubar=no, scrollbars=yes, resizable=yes,location=no, status=no");
		__extQuery__(1);
		}else {
		return false;
		 }
