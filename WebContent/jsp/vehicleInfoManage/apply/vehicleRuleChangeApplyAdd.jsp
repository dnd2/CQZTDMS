...
<script type="text/javascript">
	...
	claimMonth,claimMelieage,ruleListId,flagCheck) {
		...
		myobj.cells.item(0).innerHTML='<input type="text" class="short_txt PART_CODE" name="PART_CODE"   value="'+partCode+'" size="10" id="PART_CODE" readonly="true"/><span class="tbwhite"><a href="#" onClick="javascript:selectMainPartCode(this)">选择</a></span>';
...
<form name="fm" id="fm">
<input type="hidden" name='beforeVin' id='beforeVin' />
	<table class="table_edit" id="vehicleInfo">
		...
		<tr>
			...
			<td width="22%" align="left">${createBy }</td>
			<td width="10%" align="right" nowrap="nowrap">制单公司：</td>
			<td width="22%" align="left">${comName }</td>
			<td width="12%" align="right" nowrap="nowrap">制单时间：</td>
			<td width="50%" nowrap="nowrap">${createDate }</td>
		</tr>
		...
		<tr style="display:none;">
			...
			<td></td>
				...
				<th colspan="12" align="left">
					<img class="nav" src="<%=contextPath%>/img/nav.gif" />
					维修配件
				</th>
				...
				<tr align="center" class="table_list_row1">
					<td><input type="text" class="short_txt" name="PART_CODE"  value="" size="10" id="PART_CODE" readonly/><span class="tbwhite"><a href="#" onClick="javascript:selectMainPartCode(this)">选择</a></span></td>
					...
					<td><input type="text" class="short_txt" onafterpaste="this.value=this.value.replace(/\D/g,'')" onkeyup="this.value=this.value.replace(/\D/g,'')" name="claimMonthNew" id="claimMonthNew"/></td>
					<td><input type="text" class="short_txt" onafterpaste="this.value=this.value.replace(/\D/g,'')" onkeyup="this.value=this.value.replace(/\D/g,'')" name="claimMelieageNew" id="claimMelieageNew"/></td>
					<td style="display:none;"><input type="button" class="normal_btn"  value="删除"  name="button42" onClick="javascript:delPartItem(this,\'part\');"/></td>
				</tr>
				<tbody id="partTable">
				</tbody>
			</table>
			...
			<td colspan="7">
				<textarea rows="4" cols="120" id="applyRemark" name='applyRemark'></textarea>
			</td>
			</tr>
			</table>
	 <!-- 添加附件 开始  -->
...
function showVehicle(json) { 
	...
	if(flag=='NO'){
		...
		//如果VIN发生修改
	if($('beforeVin').value!=$('vin').value&&$('beforeVin').value!=''){
		//清除配件信息
		$('PART_CODE').value='';
		$('PART_NAME').value='';
		$('CLAIM_MONTH').value='';
...
function doCommit(flag) {
	...
	if('1'==flag){
		MyConfirm('确认保存修改？',myCommit,[flag]);	
	}else{
...
function myCommit(flag){
	var url = '<%=contextPath%>/vehicleInfoManage/apply/VehicleInfoChangeApply/ruleChangeApplyAdd.json?COMMAND=1&flag='+flag;
	//fm.action = url;
	//fm.method = "post";
	//fm.submit();
