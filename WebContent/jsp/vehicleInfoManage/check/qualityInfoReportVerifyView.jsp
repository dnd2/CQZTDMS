...
<%--<img src="<%=contextPath%>/img/nav.gif" />&nbsp;当前位置:售后服务管理&gt;车辆信息管理&gt;质量信息申报详细--%>
<%--</div>--%>
<form name="fm" id="fm">
	...
	<table class="tab_edit" width="100%" align="center">
		...
		<tr>
			...
			<td align="center">VIN代码</td>
			<td><input name="VIN" id="VIN" type="text" value="${ttSalesQualityInfoReportPO.vin}" class="long_txt" readonly="readonly"/></td>
		</tr>
		<tr>
			<td align="center">行驶里程</td>
			<td><input name="mileage" id="mileage" value="<fmt:parseNumber>${ttSalesQualityInfoReportPO.mileage}</fmt:parseNumber>" type="text" class="long_txt" readonly="readonly"/>KM</td>
...
<!-- 				<input type="button" value="..." class="mini_btn" onclick="selectPart()"> -->
				...
				<c:forEach var="importantLevel" items="${importantLevelList}">
					<c:choose>
						<c:when test="${importantLevel.isCheck}">
							<input type="checkbox" name="importantLevels" value="${importantLevel.value}" checked="checked" readonly="readonly" disabled="disabled">${importantLevel.key}
						</c:when>
						<c:otherwise>
							<input type="checkbox" name="importantLevels" value="${importantLevel.value}" readonly="readonly" disabled="disabled">${importantLevel.key}
						</c:otherwise>
					</c:choose>
				</c:forEach>
			</td>
		...
		<c:if test="${ttSalesQualityInfoReportPO.verifyStatus == 95531003}">
		<tr>
			<td colspan="7">部门意见</td>
		</tr>
		<tr>
			<td colspan="7">
				<textarea rows="5" style="width: 98%" id="qdApproval" name="qdApproval" readonly="readonly">${ttSalesQualityInfoReportPO.qdApproval}</textarea>
			</td>
		</tr>
		</c:if>
	</table>
	<br/>
	 <!-- 添加附件 开始  -->
	    		...
	    		<tr>
	        		<th>
						...
						<font color="red">
			     		...
			     		<input type="button" class="normal_btn" align="right" id="addfile" disabled="disabled" onclick="showUpload('<%=contextPath%>')" value ='添加附件'/><font color="red">*</font>
					</th>
				</tr>
				<tr>
		...
		<table width=100% border="0" cellspacing="0" cellpadding="0" id="tt">
			<tr>
