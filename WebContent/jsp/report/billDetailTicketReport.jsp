...
<table border="0" cellpadding="3" cellspacing="0" width="100%" class="scrollTable">
	<tr class="scrollColThead">
		<td class="scrollRowThead"colspan="9" align="center">启票明细表</td>
	</tr>
	<tr class="scrollColThead">
		<td class="scrollRowThead" colspan="9" align="left"><div align="left">起止时间：${beginTime}--${endTime}</div></td>
	</tr>
	<tr class="scrollColThead">
	...
	if(list!=null&&len!=0){
		...
		int sba_rn = 0;
		for(int i=0;i<len;i++){
			int ba = Integer.parseInt(list.get(i).get("BILLAMOUNT").toString());
			if(i==0){
				sba = ba;
				sba_rn = ba;
				tba = ba;
				...
				%>
				<tr>
					<td nowrap="nowrap"><%=list.get(i).get("ORG_NAME")==null?"":list.get(i).get("ORG_NAME").toString() %>&nbsp;</td>
					<td nowrap="nowrap"><%=list.get(i).get("REGION_NAME")==null?"":list.get(i).get("REGION_NAME").toString() %>&nbsp;</td>
					<td nowrap="nowrap"><%=list.get(i).get("ROOT_DEALER_NAME")==null?"":list.get(i).get("ROOT_DEALER_NAME").toString() %>&nbsp;</td>
					<td  nowrap="nowrap"><%=list.get(i).get("DEALER_NAME")==null?"":list.get(i).get("DEALER_NAME").toString() %>&nbsp;</td>
					<td nowrap="nowrap"><%=list.get(i).get("SERIES_NAME")==null?"":list.get(i).get("SERIES_NAME").toString() %>&nbsp;</td>
					<td  nowrap="nowrap"><%=list.get(i).get("PACKAGE_CODE")==null?"":list.get(i).get("PACKAGE_CODE").toString() %>&nbsp;</td>
					<td  nowrap="nowrap"><%=list.get(i).get("PACKAGE_NAME")==null?"":list.get(i).get("PACKAGE_NAME").toString() %>&nbsp;</td>
					<td  nowrap="nowrap"><%=list.get(i).get("AREA_NAME")==null?"":list.get(i).get("AREA_NAME").toString() %>&nbsp;</td>
					<td nowrap="nowrap"><%=list.get(i).get("BILLAMOUNT")==null?"":list.get(i).get("BILLAMOUNT").toString() %>&nbsp;</td>
				</tr>
				<%
			}else{
				...
				if(after.equals(before)&&after_rn.equals(before_rn)){
					...
					%>
					<tr>
						<th colspan="8" nowrap="nowrap"><CENTER>省份合计</CENTER>&nbsp;</th>
						<th nowrap="nowrap"><%=sba %>&nbsp;</th>
					</tr>
					<%
					...
					if(after_on.equals(before_on)){
						...
						%>
						<tr>
							<th class="scrollRowThead" colspan="8" nowrap="nowrap"><center>大区合计</center>&nbsp;</th>
							<th class="scrollRowThead" nowrap="nowrap"><%=sba_rn %>&nbsp;</th>
						</tr>
						<%
						sba_rn = 0;
						sba_rn += ba;
	...
	%>	
						<tr>
						<th class="scrollRowThead" colspan="8" nowrap="nowrap"><CENTER>合计</CENTER>&nbsp;</th>
						<th class="scrollRowThead" nowrap="nowrap"><%=tba %>&nbsp;</th>
						</tr>
