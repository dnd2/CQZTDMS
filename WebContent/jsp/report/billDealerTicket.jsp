...
function submitFRM(){
	...
	if(b<a){
		...
		document.FRM.action='<%=request.getContextPath()%>/report/BillTicketAllReports/getBillTicketAllReport.do';
...
function downloadFRM(){
	...
	if(b<a){
		...
		document.getElementById("t2").value=b;
		document.FRM.action='<%=request.getContextPath()%>/report/BillTicketAllReports/download.json';
...
<body>
<div class="navigation"><img src="<%=request.getContextPath()%>/img/nav.gif" />&nbsp;当前位置：整车销售报表 > 整车销售报表 > 启票汇总表</div>
<form method="POST" name="FRM" id="FRM">
<input type="hidden" name="file" value="commonXml/BillTicketAll.xml"></input>
<input type="hidden" name="dealerId" size="15" id="dealerId" value="${dealerId}" />
<input type="hidden" name="mydealerId" size="15" id="mydealerId" value="${mydealerId}" />
  ...
  <tr>
		...
		<td align="right">选择车型：</td>
			<td class="table_list_th"><input type="hidden" name="area_id" id="area_id" value="" />
				...
				<input name="button3" type="button" class="mini_btn" onclick="showMaterialCarType_market('groupCode','modelId','true','3')" value="..." />
