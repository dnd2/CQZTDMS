...
<table border="0" cellpadding="3" cellspacing="0" width="100%" class="scrollTable">
	...
	<th colspan="22" class="scrollRowThead scrollCR juzuo"><strong>启票汇总表</strong></th>
	</tr>
			...
			if(i==0){
				...
				<tr>
					...
					for(int j=0;j<len_name;j++){
						...
						%>
						<td><%=Integer.parseInt(list_all.get(i).get("BILLAMOUNT"+j).toString())==0?"":list_all.get(i).get("BILLAMOUNT"+j).toString() %>&nbsp;
						</td>
						<% 
					}
					%>
				...
				if(after.equals(before)&&after11.equals(before11)&&after12.equals(before12)){
					...
					if(after1.equals(before1)&&after23.equals(before13)){
						...
						%>
						<th class="scrollRowThead"><%=myTotal==0?"":myTotal %>&nbsp;</th>
						</tr>
						<%
	...
	%>
				...
				for(int i=0;i<len_name;i++){
					%>
					<th class="scrollRowThead"><span id="BILLAMOUNT_<%=i %>"><%=total_all[i] ==0?"":total_all[i]%></span>&nbsp;</th>
					<%
					myTotal += total_all[i];
				}
				}
