...
<script language="javaScript"><!--
	function submitFRM(){
		...
		//}else{
			document.FRM.action='<%=request.getContextPath()%>/report/Strage2StatuReport/getStrage2StatuReport.do';
	...
	function downloadFRM(){
		...
		//	MyAlert("经销商和区域不能同时存在！");
		//}else{
			document.FRM.action='<%=request.getContextPath()%>/report/Strage2StatuReport/Strage2StatuReportDownload.json';
	...
	function download(){
		if(document.getElementById("orgCode")){
			if(document.getElementById("dealerCode").value!=""&&document.getElementById("orgCode").value!="") {
				MyAlert("经销商和区域不能同时存在！");
			}
		}else{
			document.FRM.action='<%=request.getContextPath()%>/report/Strage2StatuReport/download.json';
			document.FRM.target="_self";
			document.FRM.submit();
		}
	}
...
<body>
<div class="navigation"><img src="<%=request.getContextPath()%>/img/nav.gif" />&nbsp;当前位置：整车销售管理 > 整车销售管理 > 车型库存状态表</div>
