...
<div class="navigation"><img src="<%=request.getContextPath()%>/img/nav.gif" />&nbsp;当前位置： 报表管理&gt;整车销售报表&gt;经销商开票</div>
...
var command  = document.getElementById("command").value;
	if(command == '1' ){
		url = "<%=contextPath%>/report/reportOne/DealerCheckReport/getDealerSalesInfo.json";
	}
	if(command == '3' || command == '4'){
		url = "<%=contextPath%>/report/reportOne/DealerCheckReport/getOemDealerSalesInfo.json";
	}
...
function exportExcel(){
	...
	var command  = document.getElementById("command").value;
	if(command == '1' ){
		fm.action = "<%=contextPath%>/report/reportOne/DealerCheckReport/toDealerSalesExcel.do";
	}
	if(command == '3' || command == '4'){
		fm.action = "<%=contextPath%>/report/reportOne/DealerCheckReport/toOemDealerSalesExcel.do";
	}
		fm.submit();
}

function firstQuery(){
	var url_1 = "<%=contextPath%>/report/reportOne/DealerCheckReport/getSeriesList.json";
