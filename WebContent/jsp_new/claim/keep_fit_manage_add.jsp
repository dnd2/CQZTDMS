...
<script type="text/javascript">
	...
	function backByVin(json){
		var t=json.info;
		var ro=json.roNodate;
		var Winter=json.Winterdate;
		if(null!=Winter){
			$(".winter_show").show();
			$("#winter_money").val(Winter.AMOUNT);
		}
		if(null==ro){
			
			$("#in_mileage").val("");
		}else{
			$("#in_mileage").val(ro.IN_MILEAGE);
		}
		if(null==t){
			$("#color").val("");
			$("#engine_no").val("");
			...
			$("#guarantee_date").val(t.INVOICE_DATE.substr(0,16));
			$("#model_code").val(t.MODEL_CODE);
		}
	}
	...
	function backKeepFitData(labours2,parts2,ro_no,vin){
		$("#vin").val(vin);
		sendAjax('<%=contextPath%>/ClaimAction/showInfoByVin.json?vin='+vin+"&ro_no="+ro_no,backByVin,'fm');
		$("#ro_no").val(ro_no);
		$(".deleteTr").remove();
		var tab=$("#tab_2");
		var str="";
...
<input class="middle_txt" id="model_code" value="${t.MODEL_CODE }" name="model_code" type="hidden"  />
...
<table border="0" cellpadding="1" cellspacing="1" class="table_edit" width="100%" style="text-align: center;">
	...
	<c:if test="${null!=t.WINTER_MONEY}">
	   <tr class="winter_show"  >
	    ...
	    <td>
	     <input class="middle_txt" type="text" name="winter_money" value="${t.WINTER_MONEY}"  id="winter_money" readonly="readonly" style="color: red;" />
	    </td>
	    <td width="12.5%"></td>
	    <td width="12.5%"></td>
	</tr>
	</c:if>
	<tr class="winter_show" style="display: none;" >
	    <td width="12.5%"></td>
	    <td nowrap="true" width="10%" style="color: red;">冬季保养补偿费：</td>
	    <td>
	     <input class="middle_txt" type="text" name="winter_money"  id="winter_money" readonly="readonly" style="color: red;" />
	    </td>
	    <td width="12.5%"></td>
	    <td width="12.5%"></td>
	</tr>
       ...
       <tr>
		...
		<tr>
			<td height="12" align=left width="33%" colspan='2'>
				<textarea readonly="readonly" name='AUDIT_REMARK' id='AUDIT_REMARK'	 maxlength="100" rows='2' cols='120'>${t.AUDIT_REMARK }</textarea>
			</td>
		</tr>
</table>
	    </c:if>		
