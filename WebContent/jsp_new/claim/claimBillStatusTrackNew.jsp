...
var myPage;
	//查询路径
	var url = "<%=contextPath%>/ClaimBalanceAction/applicationQuery2.json";
	...
	var urlUpdate="<%=contextPath%>/ClaimBalanceAction/claimBalanceAuditingPage.do?goBackType=2&view=print&roNo="+roNo+"&id="+ value + "&claim_type="+claim_type;
	var  urlss="";
		...
		if(screenH!=null && screenH!='undefined')
			...
			return String.format("<a href=\"#\" onclick=\"showPrintPage1('"+value+"')\">[标签]</a>");
				...
				urlss+="<a href='#' onclick='OpenHtmlWindow(\"<%=contextPath%>/ClaimBalanceAction/claimBalanceAuditingPage.do?goBackType=2&roNo="+roNo+"&id="
					+ value + "&claim_type="+claim_type+"&view=view\","+width+","+height+")' >[明细]</a><a href=\"#\" onclick=\"showPrintPage1('"+value+"')\">[标签]</a>";
				if(claim_type!=10661002 && claim_type!=10661011){
					urlss+="<a target='_blank' href='"+urlUpdate+"');'>[打印]</a>";
				 }	
  		return String.format(urlss);
	}
	}
	...
	function findamount(){
  	  var url = "<%=contextPath%>/ClaimAction/findamount.json?type=audit";
...
class="middle_txt"/></td>      
	    ...
	    <tr>
    	  ...
    	  <tr>
            <td colspan="4" align="center" nowrap><input id="queryBtn" class="normal_btn" type="button" name="button" value="查询"  onClick="queryPer();findamount();" />
			</td>
            <td  align="right" ></td>
          </tr>
           <tr>
