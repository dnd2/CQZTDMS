...
var myPage;
	//查询路径
	var url = "<%=contextPath%>/ClaimBalanceAction/claimManualAuditingQuery.json";
	...
	var columns = [
					...
					{header: "退回次数",sortable: false,dataIndex: 'BACK_TIMES',align:'right',renderer:mybacklink},
		...
		function claimAuditing(value,meta,record){
	    	...
	    	return String.format("<a href=\"#\" onclick=auditingClaim("+value+","+record.data.CLAIM_TYPE+",'"+t+"') >[审核]</a>");
	    }      
		function auditingClaim(value,type,create_date){
			...
			var tarUrl = '<%=contextPath%>/ClaimBalanceAction/claimBalanceAuditingPage.do?id='+ value+"&claim_type="+type;
           	fm.action=tarUrl;
	    	fm.method="post";
	    	fm.submit();
		}
...
function changeSubmit(type) {
	...
	var url="<%=contextPath%>/ClaimAction/passOrRebutByIds.json?id="+id+"&type="+type;
...
<div class="navigation"><img src="../img/nav.gif" />&nbsp;当前位置：售后服务管理&gt;索赔申请&gt;索赔申请审核作业
</div>
...
<!-- 		    </script> -->
	...
	<tr>
		...
		<td align="left">
			<select name="ORG_ID" class="short_sel" onchange="changeOrg(this.value)">
	...
	<tr>
		...
		<td align="left">
		   <select name="pose_id" id="pose_id" class="short_sel">
   		     <option value="">-请选择-</option>
   		    <c:forEach items="${listpo}" var="t">
   		       <c:if test="${poseId==t.POSE_ID }">
	   		       <option value="${t.POSE_ID }" selected="selected">
	   		          ${t.NAME }
	   		        </option>
   		       </c:if>
   		       <c:if test="${poseId!=t.POSE_ID }">
	   		       <option value="${t.POSE_ID }" >
	   		          ${t.NAME }
	   		        </option>
   		       </c:if>
   		    </c:forEach>
