...
var myPage;
//查询路径
var url = "<%=contextPath%>/ClaimAction/keepFitManageList.json?query=true";
...
var columns = [
	      ...
	      function myLink(value,meta,record){
		    ...
		    if(status==10791001 || status==10791006){
		    	var urlUpdate="<%=contextPath%>/ClaimAction/keepFitUpdateInit.do?id="+value;
		    	url+="<a href='"+urlUpdate+"'>[修改]</a>";
		    }
	    	var urlView="<%=contextPath%>/ClaimAction/keepFitView.do?id="+value;
	      ...
	      function cancel(obj,id){
	    	  id_del=id;
	    	  MyConfirm("是否确认撤销上报？",cancelsubmit,"");
	      }
	      function cancelsubmit(){
	    	  sendAjax('<%=contextPath%>/ClaimAction/keepFitCancel.json?id='+id_del,backCancel,'fm');
	      }
	      ...
	      function delsubmit(){
	    	  sendAjax('<%=contextPath%>/ClaimAction/keepFitDelete.json?id='+id_del,backDel,'fm');
	      }
	      ...
	      function add(){
	    	  window.location.href='<%=contextPath%>/ClaimAction/keepFitAdd.do';
	      }
	      function findamount(){
	    	  var url = "<%=contextPath%>/ClaimAction/findamount.json?type=keep";
	      ...
	      function exportToexcel(){
	    	  window.location.href='<%=contextPath%>/ClaimAction/exportkeepFitManageList.do';
		  }
</script>
<!--页面列表 end -->
</head>
...
<table width=100% border="0" align="center" cellpadding="1" cellspacing="1" class="table_query">
	...
	<tr>
		...
		<td width="15%" nowrap="true">
		     <script type="text/javascript">
		       genSelBoxExp("status",<%=Constant.CLAIM_APPLY_ORD_TYPE%>,"",true,"short_sel","","false",'10791001,10791009,10791010,10791011,10791012,10791014,10791016');
		    </script>
		</td>
