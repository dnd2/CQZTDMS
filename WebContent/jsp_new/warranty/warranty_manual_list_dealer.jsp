...
var myPage;
//查询路径
var url = "<%=contextPath%>/WarrantyManualAction/warrantyManualList.json?query=true";
...
var columns = [
				...
				{header: "申请状态", dataIndex: 'STATUS', align:'center'},
	...
	function myLink(value,meta,record){
		...
		var url="";
		if(95451001==status||95451003==status){
			url+="<a href='#' onclick='del(\""+value+"\");'>[删除]</a>";
			var urlUpdate='<%=contextPath%>/WarrantyManualAction/updateWarrantyManual.do?id='+value;
			url+="<a href='"+urlUpdate+"');'>[修改]</a>";
		}
		var urlView='<%=contextPath%>/WarrantyManualAction/viewWarrantyManual.do?id='+value;
		url+="<a href='"+urlView+"');'>[明细]</a>";
		return String.format(url);
	}
	function del(id){
		var urlDel='<%=contextPath%>/WarrantyManualAction/delWarrantyManual.json?&id='+id;
	...
	function add(){
		var url='<%=contextPath%>/WarrantyManualAction/addWarrantyManualInit.do';
...
<img src="../jsp_new/img/nav.gif" />&nbsp;当前位置：售后服务管理&gt;车辆信息管理&gt;质保手册服务站查询
</div>
<form name="fm" id="fm">
<!-- 查询条件 begin -->
