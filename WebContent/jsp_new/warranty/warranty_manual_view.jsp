...
<table border="0" cellpadding="1" cellspacing="1" class="table_edit" width="100%" style="text-align: center;">
	...
	<tr>
    	...
    	<td nowrap="true" width="15%" >
    		<input class="middle_txt" id="send_date"  value="<fmt:formatDate value="${t.SEND_DATE }" pattern='yyyy-MM-dd HH:mm'/>" readonly="readonly"  name="send_date" type="text" maxlength="30" />
    	</td>
	...
	<tr>
	 ...
	 <c:forEach items="${list }" var="t" varStatus="status">
		<tr>
			...
			<script type="text/javascript">
         		genSelBoxExp("return_type",'9443',"${t.PAY_TYPE}",true,"short_sel","","false",'');
         	 </script>
			</td>
			<td nowrap="true" width="22%" ><input type="text" name="report_remark"  class="long_txt" value="${t.REPORT_REMARK }"/></td>
			<td nowrap="true" width="10%" >
			<c:if test="${t.OPERATION==1}">
				<span>同意</span>
			</c:if>
			<c:if test="${t.OPERATION==0}">
				<span>不同意</span>
			</c:if>
		</tr>
	</c:forEach>
