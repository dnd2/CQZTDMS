...
var myPage;
//查询路径
var url = "<%=contextPath%>/SpecialAction/specialDealerList.json?query=true";
...
var columns = [
	...
	function myLink(value,meta,record){
		...
		if(20331001==status||20331004==status||20331006==status||20331008==status||20331010==status||20331012==status){
			...
			var urlUpdate='<%=contextPath%>/SpecialAction/updateWarrantyManual.do?id='+value;
			url+="<a href='"+urlUpdate+"');'>[修改]</a>";
		}
		var urlView='<%=contextPath%>/SpecialAction/viewWarrantyManual.do?id='+value;
		url+="<a href='"+urlView+"');'>[明细]</a>";
		return String.format(url);
	}
	function del(id){
		var urlDel='<%=contextPath%>/SpecialAction/delWarrantyManual.json?&id='+id;
	...
	function chooseType(spe_type){
  	  window.location.href='<%=contextPath%>/SpecialAction/addspecialDealerInit.do?spe_type='+spe_type;
    }
</script>
<!--页面列表 end -->
</head>
