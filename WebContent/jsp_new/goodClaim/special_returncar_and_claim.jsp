...
<script type="text/javascript" >
	var myPage;
    //查询路径
	var url = "<%=contextPath%>/GoodClaimAction/findSpecia.json";
	var title = null;
    var columns = [
	...
	function myLink(value,meta,record){ //,renderer:getItemValue
		...
		if("20501001"==status){
			...
			var urlUpdate='<%=contextPath%>/GoodClaimAction/viewSpecialApplyDetailed.do?id='+value+"&type=update&special_type="+special_type;
			url+="<a href='"+urlUpdate+"');'>[修改]</a>";
			url+="<a href='#' onclick='Report(\""+ID+"\");'>[提报]</a>";
		}
		if("20501006"==status){
			var urlUpdate='<%=contextPath%>/GoodClaimAction/viewSpecialApplyDetailed.do?id='+value+"&type=update";
			url+="<a href='"+urlUpdate+"');'>[修改]</a>";
			url+="<a href='#' onclick='Report(\""+ID+"\");'>[提报]</a>";
		}
		if("20501002"==status){
		...
		var urlView='<%=contextPath%>/GoodClaimAction/viewSpecialApplyDetailed.do?id='+value+'&special_type='+special_type;
		url+="<a href='"+urlView+"');'>[明细]</a>";
		return String.format(url);
	}
	function del(ID){
		var urlDel='<%=contextPath%>/GoodClaimAction/delSpecialapply.json?&id='+ID;
	...
	//提报
	function  Report(ID){
	   urlReport='<%=contextPath%>/GoodClaimAction/SpecialApplyReport.json?id='+ID+"&type=Report";
	...
	//撤销
	function Revoke(ID){
	   urlReport='<%=contextPath%>/GoodClaimAction/SpecialApplyReport.json?id='+ID+"&type=Revoke";
	   sendAjax(urlReport,function(json){
			if(json.succ=="1"){
				MyAlert("提示：操作成功！");
				__extQuery__(1);
	...
	function chooseType(spe_type){
  	  window.location.href='<%=contextPath%>/GoodClaimAction/addspecialgoodwill.do?spe_type='+spe_type;
    }
//设置超链接 end
