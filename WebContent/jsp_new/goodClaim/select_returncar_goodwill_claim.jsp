...
<script type="text/javascript">
   ...
   function clearInput(inputId,did){
	...
	//导出
	function to_excel(){
		 fm.action="<%=contextPath%>/GoodClaimAction/Toexcelspecialapply.do";
	      fm.submit();
	}
	...
	//查总额
	function Querycount(){
	  var url = "<%=contextPath%>/GoodClaimAction/Querycountspecil.json";
...
<script type="text/javascript">
	var myPage;
   //查询路径
	var url = "<%=contextPath%>/GoodClaimAction/findSpeciaAudit.json";
	var title = null;
    var columns = [
	...
	function myLink(value,meta,record){
		...
		var urlView='<%=contextPath%>/GoodClaimAction/viewSpecialApplyDetailed.do?id='+id+"&type=AudieDetail"+"&special_type="+SPECIAL_TYPE;
		url+="<a href='"+urlView+"');'>[明细]</a>";
		return String.format(url);
	}
	//审核页面
	function audit(id,SPECIAL_TYPE,VIN){
	   window.location="<%=contextPath%>/GoodClaimAction/AuditReturnCar.do?id="+id+"&VIN="+VIN+"&special_type="+SPECIAL_TYPE;
	}
</script>
			<!--页面列表 end -->
</form>
