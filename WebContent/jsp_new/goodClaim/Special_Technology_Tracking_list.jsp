...
var myPage;
//查询路径
var url = "<%=contextPath%>/GoodClaimAction/TechnologyTracking.json?type=query";
...
var columns = [
	...
	function myLink(value,meta,record){
		...
		var url="";
		if(20331009==status||20331010==status){
			var urlUpdate='<%=contextPath%>/GoodClaimAction/CancelAudit.json?id='+value+'&special_type='+special_type+'&url=specialClaimSettlementList';
             url+="<input name='detailBtn' type='button' class='normal_btn' onclick='urlUpdate(\""+ urlUpdate +"\")' value='撤销审核'/>  ";
		}
		if(20331011==status){
	...
	function urlUpdate(urlUpdate){
		sendAjax(urlUpdate,function showFunc(json){
              if(json.succ==1){
                    MyAlert("提示：操作成功！");
                    __extQuery__(1);
                  }else{
...
<img src="../jsp_new/img/nav.gif" />&nbsp;当前位置：售后服务管理&gt;特殊费用管理&gt;特殊费用技术部状态跟踪<span style="color: red;">(只能查询2015-05-26以后的数据)</span>
</div>
<form name="fm" id="fm">
<!-- 查询条件 begin -->
...
<!--		     <option value="20331001,20331002,20331003,20331004,20331005,20331006,20331007,20331008,20331009,20331010,20331012">其它状态</option>-->
	...
	<tr>
        ...
        <td width="10%" class="table_query_2Col_label_6Letter" nowrap="true">上报日期:</td>
       <td width="30%" nowrap="true">
		...
		<td width="10%" nowrap="true" class="table_query_2Col_label_5Letter">技术部审核日期:</td>
		<td width="30%" nowrap="true">
