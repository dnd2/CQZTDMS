...
<script language="JavaScript" >
	var myPage;
	var url = "<%=contextPath%>/MainTainAction/choosepageCode.json?query=true";
	...
	function mySelect(value,metaDate,record){
		 return String.format("<input type='radio' name='rd' onclick='setMainWork(\""+record.data.PACKAGE_CODE+"\",\""+record.data.PACKAGE_NAME+"\")' />");
	}
	...
	function setMainWork(v1,v2){
 		...
 		if (parent.$('inIframe')) {
 			parentContainer.setpackageCode(v1,v2);
 		} else {
			parent.setpackageCode(v1,v2);
		}
	 	//关闭弹出页面
	 	parent._hide();
	}
